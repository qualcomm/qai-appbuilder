...
> ℹ️ Install all VOICE_AI/TTS dependencies in one go (including `--no-deps` items and mocks); see `models/<id>/NOTES.md § Quick Start`. Do not trial-and-error package by package.
...
> ⚠️ **VOICE_AI `.bin` is a proprietary format loadable only with `qai_appbuilder.QNNContext`; never use `qnn-net-run` / `snpe-net-run`** — those CLIs expect standard QNN context binaries and will fail. Quantized sub-models need native dtype handling per Issue 5.
...
**Rule:** locate packages via **fixed shallow paths only** (`C:\WoS_AI\<model>\`, `~\.qaihub\`) — NEVER recurse `C:\`, `C:\Shared`, `C:\Users`, or unscoped `C:\WoS_AI`.
Normal flow: Step 1 (webfetch link) → curl to `C:/WoS_AI/<model>/`. If `qai_hub` installed, use it directly.
> ⚠️ Also applies to sub-agent task prompts: rewrite "search `C:\Shared` for packages" into the fixed shallow-directory check above.
