...
CLI reference: see argparse --help below or references/builder_end2end_conversion.md.
"""
...
def _get_host_toolchain_dir(sdk_root: str) -> str:
    ...
    if host_os == "windows-arm64":
        ...
        # Probed in order for a real qairt-converter. In 2.48 only -gcc11.2 ships
        # the host tools; the rest are kept because other SDK versions place the
        # converter differently and a missing dir is skipped harmlessly.
        # "aarch64-linux-clang" was removed: it appears in NO QAIRT SDK and in
        # no SDK doc, so it could never match -- unlike -gcc8.2, which the 2.48
        # docs do reference.
        candidates = [
    ...
    ext = ".exe" if host_os.startswith("windows-") else ""
    for d in candidates:
...
def apply_qairt_env(cfg: dict) -> tuple:
    ...
    sdk = (
        cfg.get("qairt_root", "")
        or cfg.get("qairt_sdk_root", "")
        or os.environ.get("QAIRT_SDK_ROOT", "")
    )
    if sdk:
        os.environ["QAIRT_ROOT"] = sdk
        os.environ["QAIRT_SDK_ROOT"] = sdk
        os.environ["QNN_SDK_ROOT"] = sdk

    if sdk and not os.path.isdir(sdk):
        print(f"[ERROR] QAIRT SDK root does not exist: {sdk}")
        print("  This path comes from qairt_env.json (qairt_root / legacy qairt_sdk_root) or the")
...
def _htp_version_to_target_soc_model(htp_version: str) -> str:
    """Map --htp_version to a qairt-converter/quantizer --target_soc_model name.

    Thin wrapper over _soc_targets (single source of truth) -- kept because
    callers and tests reference this name. Returns "" if unknown (caller must
    then skip --target_soc_model rather than substitute a guess).
    """
    return htp_version_to_target_soc_model(htp_version)
...
def _detect_htp_version(sdk_root: str, host_arch: str) -> str | None:
    ...
    the SDK supports, and is preferred over guessing from a --target_platform
    name when the caller did not explicitly pin a version.

    Returns ``"v75"`` etc. on success, or ``None`` if detection fails or is
    not applicable (caller should fall back to DEFAULT_HTP_VERSION).
    """
    import re as _re

    # Linux only -- Windows keeps the existing default="v73"-style behaviour.
    if sys.platform == "win32":
        return None
    ...
    # Sanity-check: the detected tag must be a valid HTP target (v66 is the
    # legacy DSP backend, not HTP -- see _soc_targets.HTP_VERSIONS) AND the
    # SDK must actually ship that Hexagon version.
    if detected in HTP_VERSIONS and (Path(sdk_root) / "lib" / f"hexagon-{detected}").is_dir():
        return detected
...
def step3_context_binary(python: str, script_dir: str, sdk: str,
                         ...
                         profiling: bool,
                         soc_model: int = None, vtcm_mb: int = None,
                         dsp_arch: str = None, pd_session: str = None) -> str:
    """DLC -> context binary (.bin). Returns bin_path.

    Host routing:
      * windows-arm64 / linux-aarch64  -> qai_dev_gen_contextbin.py (real HTP).
      * windows-x64   / linux-x64      -> qai_dev_gen_contextbin_x86.py.
        Historically this always produced a CPU-backend context, which
        validates numerics but does NOT execute on Hexagon HTP. Passing an
        explicit target (soc_model / vtcm_mb / dsp_arch) now selects the HTP
        backend and cross-compiles for that SoC instead.
    """
    ...
    # Explicit HTP target selection, if any. Shared by both host branches.
    #
    # A user-supplied --config already specifies the target (soc_id / soc_model /
    # vtcm_mb / dsp_arch), so it is mutually exclusive with the target flags:
    # the generator rejects the combination rather than let two sources disagree.
    # Fail here with a clear message instead of building a doomed command line.
    target_args = []
    for _flag, _val in (("--soc_model", soc_model), ("--vtcm_mb", vtcm_mb),
                        ("--dsp_arch", dsp_arch), ("--pd_session", pd_session)):
        if _val is not None:
            target_args += [_flag, str(_val)]

    # A missing config path is a HARD error, on EVERY host branch (this sits
    # before the use_x86 split on purpose: a guard inside one branch would let
    # the other silently degrade). Previously a typo'd/absent --config simply
    # made _has_config False, the flag was dropped, and the run continued with
    # the target flags -- producing a .bin for a COMPLETELY WRONG target with
    # rc=0, only discovered when loading it on the device. Same semantics as
    # qai_dev_gen_contextbin.run_generator()'s entry guard (rc=2).
    if config_file and not Path(config_file).exists():
        print("[ERROR] config_file not found: " + str(config_file))
        print("  --config was given but that path does not exist. Refusing to "
              "fall back to the target flags: that would silently build a .bin "
              "for the wrong target. Fix the path or drop --config.")
        sys.exit(2)

    _has_config = bool(config_file)
    if _has_config and target_args:
        print("[ERROR] --config cannot be combined with "
              "--soc_model/--vtcm_mb/--dsp_arch/--pd_session: the config file "
              "already specifies the HTP target. Pass one or the other.")
        sys.exit(2)

    if use_x86:
        # --htp_version has no counterpart on the x86 generator (there the
        # Hexagon arch comes from --dsp_arch). Forwarding nothing would silently
        # drop an explicitly requested version while STEP 3 above still prints
        # "(HTP <version>)", so derive --dsp_arch from it instead. Only when the
        # user did not spell out --dsp_arch, and only when they actually asked
        # for a non-default version: doing it unconditionally would flip the
        # default CPU-backend route to HTP and change existing behaviour.
        if (htp_version and dsp_arch is None and not _has_config
                and htp_version != DEFAULT_HTP_VERSION):
            print(f"[INFO] Host = {host_os}: --htp_version {htp_version} maps to "
                  f"--dsp_arch {htp_version} on the x86 generator.")
            target_args += ["--dsp_arch", str(htp_version)]
        # Without an explicit target this stays on the CPU backend, exactly as
        # before. With one, the x86 generator cross-compiles for that SoC.
        if target_args:
            print(f"[INFO] Host = {host_os}: generating HTP-backend context binary "
                  f"for an explicit target ({' '.join(target_args)}).")
            backend = "htp"
        else:
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--htp_version", default=None,
                   ...
                   help="Target Hexagon/HTP version for the context binary. "
                        "Linux: auto-detected via qnn-platform-validator when "
                        "omitted (falls back to " + DEFAULT_HTP_VERSION + " if "
                        "detection is unavailable). Windows: defaults to "
                        + DEFAULT_HTP_VERSION + ". Which versions a given host "
                        "can build for depends on the Stub libs it ships -- see "
                        "_soc_targets.HTP_VERSIONS.")
    p.add_argument("--soc_model", type=int, default=None,
                   help="Explicit target SoC id for the context binary, e.g. 117 "
                        "for IPQ9650. Default: derived from --htp_version. On an "
                        "x86 host this also switches the generator to the HTP backend.")
    p.add_argument("--vtcm_mb", type=int, default=None,
                   help="Target VTCM budget in MB for the context binary, e.g. 2.")
    p.add_argument("--dsp_arch", default=None,
                   help="Target DSP arch written into the HTP config, e.g. v81.")
    p.add_argument("--pd_session", default=None,
                   help="PD session attribute for the HTP config, e.g. unsigned.")
    p.add_argument("--target_platform", default=None,
                   help="Platform NAME instead of raw numbers, e.g. IPQ9650 or "
                        "SC8480XP. Fills in --soc_model/--dsp_arch from "
...
def main():
    ...
    # Resolve a platform NAME into the numbers the HTP config needs, before any
    # branch reads args.soc_model/args.dsp_arch, so both the DLC and the x86
    # delegation path see identical values.
    ...
    # ------------------------------------------------------------------
    # Users should NOT normally pass --htp_version by hand: on Linux this is
    # auto-detected from the attached DSP hardware, which is authoritative
    # and avoids a SoC-name typo silently producing a .bin for the wrong
    # target. --dsp_arch/--target_platform (resolved above) still take
    # priority when the caller explicitly named a cross-compile target.
    if args.htp_version is None:
        ...
        if detected:
            ...
            args.htp_version = DEFAULT_HTP_VERSION
            print(f"[INFO] HTP version  = {DEFAULT_HTP_VERSION}  "
                  "(auto-detection unavailable, using default)")
    else:
