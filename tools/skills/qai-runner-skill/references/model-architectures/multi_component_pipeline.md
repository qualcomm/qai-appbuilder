...
Replace the single `MODEL_NAME` / `ONNX_FILE` pair in `plan.md` with a component list:
...
Run `qai_convert_fp.py` (or `qai_convert_int.py`) once per component:
...
(intercepted by the `qai` wrapper) and chains their outputs:
...
Run via the `qai` wrapper so each session is routed through QNN HTP:
...
## 9. `plan.md` checklist additions for multi-component projects
...
- Diffusion pipelines: text_encoder + unet + vae_decoder (+ optional safety_checker)
- Encoder-decoder ASR: encoder + decoder (e.g. Whisper)
- Multi-modal: vision_encoder + text_decoder (e.g. LLaVA, CLIP)
- Any pipeline where components have different input shapes, precisions, or backends

## Setup

List components in `plan.md` Project Config:
```
...
Each component follows Steps 1–8 independently, but with shared decisions:

| Decision | Scope |
|----------|-------|
| `FLOW` (QNN / SNPE) | Shared — all components use the same flow |
| `PRECISION` | Per-component — unet may use INT8 while text_encoder stays FP16 |
| `--htp_version` | Shared — same target device |
| Calibration data | Per-component — each needs its own representative inputs |
| Context binary | Per-component — separate `.bin` files |

## Inference script

The `infer_{MODEL}.py` chains all components:
```python
# Pseudo-code — adapt shapes/dtypes per model
text_out = text_encoder_ctx.Inference([tokens])
noise_pred = unet_ctx.Inference([latents, text_out[0], timestep])
image = vae_ctx.Inference([noise_pred[0]])
```

Record all component context binary paths in `inference_manifest.json`.

## Validation

- Cosine / decoded-output check applies to the **final pipeline output**, not per-component
- Per-component cosine is optional (useful for debugging accuracy drops)
- Record per-component and pipeline-level results in `REPORT.md`

## See Also

- `references/model-architectures/model_split.md` — splitting **one** large model whose context binary exceeds the
  DSP SMMU window (different problem from an inherently multi-component pipeline)
