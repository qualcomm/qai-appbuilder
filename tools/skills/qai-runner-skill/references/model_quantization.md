...
> **Format by tool:**
> - **`qai_convert_int.py` (QNN onnx converter flow)** → `input:=` prefix (shown below).
> - **`qairt-quantizer` ** → plain paths for a single-input model; for a
>   **multi-input** model put all inputs of one sample on one line, using the
>   `name:=path` form. Details + the `run_pipeline.py` false-positive warning →
>   [snpe_conversion.md § Calibration list format](snpe_conversion.md#calibration-list-format-single-input-vs-multi-input).

`calibration_list.txt` for **`qai_convert_int.py` ( `input:=` prefix):**
```
...
**Multi-input model** (`qairt-quantizer`) — one line per sample, all inputs on it:
```
inputs_embeds:=calib/s0_embeds.raw position_ids:=calib/s0_pos.raw mask:=calib/s0_mask.raw
inputs_embeds:=calib/s1_embeds.raw position_ids:=calib/s1_pos.raw mask:=calib/s1_mask.raw
```
Every input the graph declares must appear on every line. Dump these from a real
inference run, not synthesised values — for stateful graphs (KV cache) the cache
tensors' ranges only become representative after several real steps.
...
#### Legacy SNPE fallback

`snpe-dlc-quant` is the older SNPE quantization tool. Use it only with an
older SDK that does not provide `qairt-quantizer`, and verify its options with
the SDK-local `--help` output:
...
| Intent | `run_pipeline.py` (Flow A, default) | `qairt-quantizer` (Flow A backend) | `qai_convert_int.py`  |
|--------|-------------------------------------|------------------------------------|-------------------------------|
...
| Calibration list | `--calib_list <file>` | `--input_list <file>` | `--input_list <file>` (**`input:=` prefix**) |
...
> ⚠️ **Two distinct spellings, same meaning:** `run_pipeline.py`/`qai_convert_int.py` use `--act_bw`/`--weight_bw`/`--bias_bw`; `qairt-quantizer` uses `--act_bitwidth`/`--weights_bitwidth`/`--bias_bitwidth` (note the `s` in `weights`). Calibration-list format → § Step 2 above. Full `run_pipeline.py` flag reference → [builder_end2end_conversion.md § run_pipeline.py — Full Argument Reference](builder_end2end_conversion.md); `qairt-quantizer` full command → [snpe_conversion.md § ONNX → DLC Quantization](snpe_conversion.md#onnx--dlc-quantization-w8a8--w8a16-via-qairt-quantizer).

## Key Points
- **FP16:** Fast conversion, no calibration needed
- **INT8/INT16:** Better performance, requires calibration data
- **DLC quantization:** Prefer `qairt-quantizer`; `snpe-dlc-quant` is legacy
...
**Solutions (in priority order):** ① Split the output tensor — remove the end-of-graph `Concat` so each sub-tensor (e.g. boxes vs scores) gets its own scale; stays pure W8A8 (→ `graph/patterns/Extended/Split/O_Output_Decoupling_Staticization_Skill.md`); ② Switch to W8A16 (16-bit activations); ③ Add more multi-class calibration samples (≥50, covering all classes); ④ Apply per-channel quantization to the output.
