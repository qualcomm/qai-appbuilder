...
REM     2. Delete venvs + signed Python (WDAC fallback) + PortableGit + Node.js (%LOCALAPPDATA%\QAIModelBuilder\)
...
REM %LOCALAPPDATA%\QAIModelBuilder tree -- venvs (.venv_arm64_313 /
REM .venv_x64_313 / .venv_x64_310), the WDAC-fallback signed Python under
REM python\3.13 + python\3.10, PortableGit, Node.js -- and, below, uv
REM interpreters for BOTH architectures, so removal is host-arch-independent.
REM (A signed Python REUSED from an existing system install, e.g. an official
REM python.org build already registered outside our tree, is left untouched:
REM it is not ours to remove.)
...
REM (factory/config/ports.json legacy_packaged) inside a Windows excluded port
REM range, forcing the supervisor to fall back to another candidate. Read
REM the runtime endpoint URL the API wrote on startup and POST to its
REM actual /api/system/reboot path; if the file is missing (server already
REM stopped or pre-endpoint-file install) fall back to the factory/config/ports.json
REM legacy_packaged port. Both paths are best-effort — curl errors are
REM silently swallowed and the kill-by-image-path step below catches
REM anything that did not stop gracefully.
REM NO port literal is authored here: the fallback port comes solely from
REM factory/config/ports.json (legacy_packaged). If that read fails we leave
REM REBOOT_URL empty and skip the port-based ping entirely — the endpoint-file
REM URL below (the ACTUAL bound port) and the kill-by-image-path step that
REM follows both cover that case, so there is no need to guess a number.
set "REBOOT_PORT="
set "REBOOT_URL="
for /f "usebackq delims=" %%L in (`powershell -NoProfile -Command "try { (Get-Content '%~dp0factory\config\ports.json' -Raw -Encoding UTF8 | ConvertFrom-Json).legacy_packaged } catch { '' }"`) do set "REBOOT_PORT=%%L"
if defined REBOOT_PORT set "REBOOT_URL=http://localhost:!REBOOT_PORT!/api/system/reboot"
...
if defined REBOOT_URL curl -s -X POST "!REBOOT_URL!" >nul 2>&1
