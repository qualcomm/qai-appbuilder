...
EXPECTED_TABLES: frozenset[str] = frozenset({
    ...
    # NOTE: ``ai_coding_skill`` is created by 004 and DROPPED again by
    # 072_drop_ai_coding_skill.sql (skill-registry consolidation), so it must
    # NOT be listed here — this check runs AFTER all migrations, where the
    # table is legitimately absent on a correctly migrated database.
    #
    # Listing it made this stage report "missing tables after migration", and
    # because the install pipeline short-circuits on a failed stage
    # (``runner.py``: ``if mode == "apply" and not ok: return result``), the
    # THREE stages after it never ran. The user-visible symptom was a fresh
    # Setup leaving the model picker empty: seeding the internal edition's
    # cloud-provider catalog is ``edition_secrets``, the LAST stage.
