...
// Language-neutral port registry — the single source of truth for ALL ports
// across backend (Python), frontend (this file) and desktop (Tauri/Rust).
// Edit <repo-root>/factory/config/ports.json; never hard-code port literals here.
// Read via readFileSync (same pattern as package.json above) so no
// tsconfig `resolveJsonModule` / import-assertion setup is required.
const ports = JSON.parse(
  readFileSync(
    fileURLToPath(new URL("../factory/config/ports.json", import.meta.url)),
    "utf-8",
  ),
) as {
  backend: number;
...
export default defineConfig(({ mode }) => {
  ...
  // Backend dev target. Default (ports.dev_backend_proxy from
  // factory/config/ports.json) matches the bare-uvicorn V2 debug backend and
  // api-contract.md §6.4.
  // Override via QAI_DEV_BACKEND_HTTP / _WS in .env.local for non-default
  // backends; never hard-code other hosts in application source.
  const devBackend = ports.dev_backend_proxy;
  const backendHttp = env.QAI_DEV_BACKEND_HTTP ?? `http://127.0.0.1:${devBackend}`;
  const backendWs = env.QAI_DEV_BACKEND_WS ?? `ws://127.0.0.1:${devBackend}`;
