...
export default defineConfig({
  ...
  test: {
    ...
    // Enable CSS handling so third-party UI libs that ship raw ``.css`` side
    // effects (element-plus) don't crash test collection with "Unknown file
    // extension .css". Vitest hands the imports to Vite's CSS pipeline, so
    // component tests still exercise a real mount with real class names.
    css: true,
    // Element-plus ships bare ``.css`` side-effect imports (e.g.
    // ``import 'element-plus/theme-chalk/base.css'``) inside its ES entry;
    // vitest externalises node_modules by default, so those never reach
    // Vite's CSS pipeline and crash with "Unknown file extension .css".
    // Inlining the package hands its imports to Vite, which our ``css: true``
    // above then handles. Kept scoped to element-plus so we do not
    // unnecessarily re-bundle other deps.
