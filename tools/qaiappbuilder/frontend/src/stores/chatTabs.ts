...
import {
  ...
  peekSubAgentStream,
  nextSubAgentReconnectDelayMs,
  resetSubAgentReconnectAttempts,
  trackSubAgentReconnectTimer,
  cancelSubAgentReconnectTimer,
  trackSubAgentOrchestratorRegistration,
  releaseSubAgentOrchestratorRegistration,
} from "./chatTabs/transientHandles";
// N.46 — cross-channel WS coordination. `_scheduleSubAgentReconnect` consults
// the suspend state so it never burns a backoff rung against a link the
// orchestrator already knows is down.
import { isWebSocketSuspended } from "@/composables/chat/useWebSocketOrchestrator";
...
/**
 * Poll period for the sub-agent truth-source reconciliation loop (P1).
 *
 * Named, not a magic number: it IS the user-visible latency budget for
 * "子 Agent 完成后卡片在数秒内变 ✓ 已完成". A detached ("background") spawn's
 * completion never arrives as a frame on the parent tab, so this interval is
 * the ONLY thing that clears its spinner mid-session. 3s keeps that well
 * inside "数秒" while costing one cheap GET per still-running sub-agent — and
 * the loop disarms the moment none are left, so an idle conversation is free.
 */
const SUBAGENT_RECONCILE_MS = 3000;

/** Terminal background-process statuses (mirrors the backend's
 *  `TERMINAL_STATUSES`). Liveness is derived by EXCLUSION so a status added
 *  later counts as live by default — enumerating the live values instead is
 *  what dropped `ready`, the steady state of a dev server past its readiness
 *  probe, and would have cleared the tab's busy state while it kept running. */
const BGP_TERMINAL_STATUSES = new Set<string>(["exited", "failed", "stopped"]);


/**
 * Handle of the single in-flight reconciliation interval, or `null` when
 * disarmed. Module-scope (not tab state) because the loop is global: it always
 * reconciles whichever main tab is active, so one timer serves every tab and
 * cannot leak per-tab duplicates. Cleared by `_stopSubAgentReconcilePoll`
 * (drained set) and by `reset()` so specs start disarmed.
 */
let _subAgentReconcileTimer: number | null = null;
...
export const useChatTabsStore = defineStore("chatTabs", {
  ...
  getters: {
    ...
    /**
     * Whether the active tab is "busy" — generating OR mid-abort OR has
     * background work still in flight (sub-agents spawned by an earlier
     * turn that are still running after the main turn has settled).
     *
     ...
     *
     * §UX-idle-with-bg-work (2026-08-07): a completed main turn may have
     * spawned sub-agents whose runs continue in the background after the
     * main assistant text finished streaming. In that window ``status`` is
     * ``idle`` but the tab is clearly still "working" from the user's
     * perspective — the same visual affordances (typing indicator + Stop
     * button) SHOULD render.  ``hasActiveBackgroundWork`` folds that state
     * in.  Truth source is the tab's own message tree (the ``subagent_*``
     * frames maintain block ``status`` live), so this adds zero new
     * event/polling machinery.
     */
    isActiveTabBusy(): boolean {
      const tab = this.activeTab;
      if (tab === null) return false;
      if (tab.status === "streaming" || tab.status === "aborting") return true;
      return this.hasActiveBackgroundWork;
    },

    /**
     * True iff any sub-agent block in the active tab's message tree is in
     * a non-terminal state (``running`` / ``aborting``).  Used by
     * :func:`isActiveTabBusy` to keep the streaming-visual affordances
     * (typing indicator + Stop button) rendered while background sub-agents
     * are still churning after the main turn settled.
     *
     * §F3-async-contract (2026-08-07): the earlier ``"parked"`` variant
     * (a sub-agent stuck at the 15s auto-park threshold) is gone — under
     * the new async agent contract every sub-agent is "running in the
     * bridge" from t=0 until it terminates, so ``running`` alone captures
     * the "still doing work" state.  A block only leaves ``running`` when
     * its ``SubAgentSessionTerminated`` event arrives (via
     * BackgroundJobDispatcher + snapshot refresh); at that point the
     * block flips to ``done`` and this getter naturally returns ``false``.
     *
     * N.36 (2026-08-08) — this getter is now **O(1)**: it reads the size of
     * the tab's ``activeSubAgentIds`` index instead of walking every message's
     * every sub-agent block.  The old scan was O(messages × blocks) and ran on
     * every render of the typing indicator AND the Stop button (via
     * ``isActiveTabBusy``), so a long conversation with many dispatch rounds
     * paid that walk continuously while nothing was even running.
     *
     * Single truth source, explicitly: ``subAgentBlocks[].status`` remains the
     * PERSISTED truth (a reload rebuilds the index from it — see
     * ``_deriveActiveSubAgentIds``); ``activeSubAgentIds`` is a derived index
     * maintained by the ``subagent_*`` frame handlers.  The two are NOT kept
     * as parallel implementations of the same predicate — the block scan that
     * used to live here is GONE, so there is exactly one code path answering
     * "is background work live?".
     *
     * Bg-process tool cards (``tool_name === "background_process"``) fetch
     * their own live status from ``/api/background-process`` + a
     * ``BackgroundProcessUpdated`` SSE subscription in
     * ``BackgroundProcessCard.vue`` for their own badge.  That is per-CARD
     * state, which is not enough for the tab-level question: a card only
     * exists while its message is on screen, and the user's read of "is the
     * agent still working?" is the typing indicator + Stop button, not a
     * badge inside one card.  ``activeBgProcessIds`` therefore mirrors the
     * same SSE frames into a tab-level index (no new polling), and this
     * getter is true while EITHER kind of work is live.
     *
     * Both halves are ``.size`` reads, so the predicate stays O(1).
     */
    hasActiveBackgroundWork(): boolean {
      const tab = this.activeTab;
      if (tab === null) return false;
      return (
        tab.activeSubAgentIds.size > 0 || tab.activeBgProcessIds.size > 0
      );
    },

    /**
     * Per-tab form of the §UX-idle-with-bg-work predicate: is THIS tab still
     * working from the user's point of view?
     *
     * `isActiveTabBusy` / `hasActiveBackgroundWork` answer that only for the
     * tab on screen, which is right for the composer's Stop button and the
     * typing indicator. The tab strip's per-tab dot and the sidebar's
     * per-conversation dot ask the SAME question about a tab that is NOT
     * active, so they cannot use those getters — before this existed they read
     * `tab.status` alone and therefore showed a BACKGROUND tab as idle while a
     * sub-agent or a backgrounded exec of that tab kept running.
     *
     * Returns the status to surface, or `null` when the tab is genuinely
     * quiet: `"streaming"` / `"aborting"` / `"error"` pass through; an `idle`
     * tab with live background work reports `"streaming"` so the existing
     * dot/label styling needs no new state. Same O(1) `.size` reads as
     * `hasActiveBackgroundWork` — no scan, no new event machinery.
     */
    tabLiveStatus(): (tab: ChatTab) => ChatTabStatus | null {
      return (tab: ChatTab): ChatTabStatus | null => {
        if (tab.status !== "idle") return tab.status;
        return tab.activeSubAgentIds.size > 0
          || tab.activeBgProcessIds.size > 0
          ? "streaming"
          : null;
      };
    },
  ...
  actions: {
    ...
    /**
     * Signal every context-badge reader to re-fetch `GET /context` now.
     *
     * Called by the `/compact` slash-command family, which mutates the
     * conversation's compaction checkpoint over REST WITHOUT running a
     * streaming turn — so none of the badge's usual triggers
     * (`conversationId` / `modelId` / `provider` change, `streaming → idle`)
     * fire and the badge would keep rendering the pre-compaction figures.
     */
    bumpCtxRefresh(): void {
      this.ctxRefreshNonce += 1;
    },
    ...
    openTab(input: OpenTabInput = {}): ChatTab {
      ...
      const tab: ChatTab = {
        ...
        // N.35 — flat live-sub-agent index (see the field doc in
        // `_chatTabsTypes.ts`). Deliberately NOT reset by `setStreaming` /
        // `confirmDone` / `confirmAbort` / `recordError`: background
        // sub-agents OUTLIVE the parent turn that spawned them, so wiping
        // the index at a turn boundary is exactly the bug the index exists
        // to avoid (the Stop button would vanish while work continues).
        // Only a terminal frame for a given sub-agent removes its id.
        activeSubAgentIds: new Set<string>(),
        // Same lifetime as ``activeSubAgentIds``: a tab starts with no
        // background process of its own in flight.
        activeBgProcessIds: new Set<string>(),
        streamingUsage: null,
        streamingPerf: null,
    ...
    closeTab(tabId: TabId, mode: "keep" | "destroy" = "keep"): void {
      ...
      // N.38 — and its reconnect ladder: a closed tab must never resurrect a
      // subscription (the scheduled retry also guards on tab existence, so
      // this is belt-and-braces plus a leak of the Map entry avoided).
      resetSubAgentReconnectAttempts(tabId);
      cancelSubAgentReconnectTimer(tabId);
      // N.46 — and its orchestrator registration, so a page-level resume can
      // never wake a subscription for a tab that no longer exists.
      releaseSubAgentOrchestratorRegistration(tabId);
    ...
    closeAllTabs(): void {
      for (const tab of this.tabs) {
        ...
        resetSubAgentReconnectAttempts(tab.id);
        cancelSubAgentReconnectTimer(tab.id);
        releaseSubAgentOrchestratorRegistration(tab.id);
      }
      this.tabs = [];
    ...
    async _hydrateRestoredSubAgentTab(
      ...
      this._patchTab(tabId, {
        ...
        // N.35/N.36 — a reload has no frames to replay, so the live
        // sub-agent index is rebuilt from the persisted block statuses
        // we just adopted (they are the truth source; the index is the
        // derived O(1) read `hasActiveBackgroundWork` uses).
        activeSubAgentIds: deriveActiveSubAgentIds(messages),
        ...(saModelId !== null
    ...
    /**
     * Enqueue at the HEAD of the queue — priority path for a deferred
     * mid-turn injection (`inject_as_new_turn`).
     *
     * Why the head and not the tail (user ruling 2026-08-09): a Ctrl+Enter
     * injection is an explicit "handle this NOW" signal — the user typed it
     * DURING the run precisely because it must not wait behind whatever was
     * queued with plain Enter earlier. When the run loop never found an
     * inter-round seam to fold it into (the last tool round was already
     * done, or the turn called no tool at all), the text must still reach
     * the model, and it must reach it BEFORE the ordinary queue backlog.
     *
     * Ordering among injections is preserved upstream: the backend merges
     * every un-drained injection of the turn into ONE text (the
     * `inject_as_new_turn` frame carries the FIFO list and
     * `handleInjectAsNewTurn` joins it with blank lines), so a single head
     * insert keeps the user's own ordering while placing all of them ahead
     * of the backlog.
     *
     * Unlike `enqueueMessage` this NEVER reports `"full"`: discarding text
     * the user already sent (and whose optimistic bubble the frame handler
     * just removed) would be a silent loss of intent. A priority insert may
     * therefore exceed `MAX_QUEUE_SIZE` by at most the injections of one
     * turn — bounded, and strictly better than dropping them.
     */
    enqueueMessageFront(
      tabId: TabId,
      text: string,
    ...
    /**
     * Reset the in-flight turn's rendered assistant output so a `from_seq=0`
     * broadcaster BACKFILL replay (data-plane WS dropped mid-turn → re-attach
     * via `/api/chat/active-runs/{tab}/ws`) becomes the SINGLE source of truth
     * for this turn — no duplicate bubbles, no misaligned text.
     *
     * WHY this is required (State-Truth-First): `applyFrame(…, backfill=true)`
     * is APPEND semantics, not replace. Chunks accumulate onto
     * `streamingContent` / the per-round message via the coalescing buffers
     * (see `appendStreamingChunk` chatTabs.ts:2037-2077 and `appendRoundChunk`
     * :2124-2171), and `tool_call` / round-open frames create NEW round
     * assistant messages keyed by `roundMessageIds[round_index]`
     * (frameHandlers.ts). The primary turn WS (`/api/chat/ws`) delivers frames
     * WITHOUT a `sequence`, so on a mid-turn drop the client cannot know which
     * frames it already applied and MUST re-attach at `from_seq=0` — replaying
     * the ENTIRE turn. Replaying onto the already-rendered turn would duplicate
     * every assistant bubble + double the trailing text (the reported "用户消息
     * 错位 / 内容显示两遍" corruption). Unlike the sub-agent re-subscribe path
     * (chatTabs.ts:4331 `resumeStreaming`), which resumes at
     * `from_seq=lastApplied+1` and therefore preserves routing, this path has
     * no cursor and must reset.
     *
     * Scope of the reset — surgical, NOT a full clear:
     *   - Drops ONLY the trailing assistant/tool messages of the CURRENT turn
     *     (everything AFTER the last `role:"user"` message). Prior turns and
     *     the current turn's user prompt are preserved — the broadcaster
     *     buffers only assistant `StreamFrame`s, never the user message, so
     *     the user turn must survive the reset.
     *   - Wipes all streaming-transient routing (mirrors `setStreaming`'s reset
     *     set) so the backfill opens fresh round messages from a clean slate.
     *   - Drops the coalescing buffers so no pre-drop partial text bleeds into
     *     the replay.
     *   - Keeps `status: "streaming"` (the turn never ended — it is running in
     *     the background) and `lastError: null`.
     */
    resetTurnForResume(tabId: TabId): void {
      const tab = this.tabs.find((t) => t.id === tabId);
      if (tab === undefined) {
        return;
      }
      ...
      clearStreamingBuffer(tabId);
      // Truncate to the end of the current turn's user prompt: find the LAST
      // user message and drop every assistant/tool message the interrupted
      // turn had already rendered after it. A turn always begins with the
      // user message, so this leaves prior turns + the prompt intact and
      // hands a clean slate to the backfill.
      let lastUserIdx = -1;
      for (let i = tab.messages.length - 1; i >= 0; i = i - 1) {
      ...
      const messages =
        lastUserIdx >= 0
          ? tab.messages.slice(0, lastUserIdx + 1)
          : tab.messages;
      this._patchTab(tabId, {
        status: "streaming",
        messages,
    ...
    applyFrame(
      ...
      handler(current, frame, {
        ...
        // Token-pool balance (QAI Service): global, not per-tab. Resolved
        // lazily inside the callback so this store does not take a load-time
        // dependency on the quota store for a frame most turns never carry.
        applyQuotaUsage: (quota) => {
          useQuotaStore().applyFrameQuota(quota);
        },
        isBackfill: backfill,
      });
      ...
      if (frame.frame_type === "subagent_start") {
        ...
        // P1 — a spawn just went live on this tab. Arm the truth-source
        // reconciliation poll: a DETACHED ("background") spawn's completion
        // never arrives as a `subagent_done` frame on this stream (it is
        // delivered as a SYSTEM_NOTICE instead), so polling the persisted
        // session status is the only thing that can clear its spinner during
        // this session. Idempotent, and self-disarming once every card settles.
        this._ensureSubAgentReconcilePoll();
      }
    },
    ...
    pushUserMessage(tabId: TabId, prompt: string): string | null {
      ...
      if (tab.status !== "idle") {
        // Atomic gate: every send path funnels through here, and the status flip
        // to "streaming" below happens in the SAME synchronous block — so two
        // callers racing on one tab can never both open a turn.
        return null;
      }
      const id = nextMessageId();
      const msg: ChatMessage = {
    ...
    appendMessage(tabId: TabId, message: ChatMessage): void {
      ...
      const messages = [...tab.messages, message];
      // N.35/N.36 — a message committed OUTSIDE the frame stream (turn commit,
      // orphan promotion, a caller seeding a rendered transcript) can introduce
      // sub-agent blocks the live index never saw a START frame for. Since the
      // block statuses in `messages` are the truth source the index derives
      // from, re-derive whenever the message list is appended to — otherwise
      // `hasActiveBackgroundWork` would under-report live work committed this
      // way, and the Stop button / typing indicator would go quiet while a
    ...
    async openSubAgentTab(subagentId: string): Promise<TabId | null> {
      ...
      if (existing !== undefined) {
        ...
        // Opening the panel is also a moment where the parent's inline card may
        // still be spinning with nothing left to settle it: after a page reload
        // there is no `subagent_start` frame to arm the truth-source poll, and
        // the user reaching for the panel is exactly when a stale spinner is
        // most visible. Idempotent + self-disarming, so arming here costs one
        // no-op check when the loop is already running.
        this._ensureSubAgentReconcilePoll();
        return existing.id;
      }
    ...
    async _openSubAgentTabInner(subagentId: string): Promise<TabId | null> {
      ...
      // Same reason as the reuse path in `openSubAgentTab`: arm the
      // truth-source poll so the PARENT's inline card can settle even when this
      // session never saw a `subagent_start` frame (post-reload open). The
      // standalone tab settles itself from the live stream; the parent's card
      // does not.
      this._ensureSubAgentReconcilePoll();
      return tab.id;
    },
     ...
     * `message.subAgentBlocks[]`. Two paths reach here, both structurally
     * unable to receive a terminal frame on the parent tab:
     *
     *   1. User Stop — the block is flipped to `"aborting"` optimistically, but
     *      the parent tab has no per-child WS subscription, so the terminal
     *      `subagent_done` / `subagent_error` frame never arrives on it.
     *   2. P1 — NORMAL completion of an async ("background") spawn. Under the
     *      async spawn contract the parent stream emits
     *      `subagent_start(status="background")` and a handle ack, then the
     *      sub-agent runs detached; its completion is delivered as a
     *      SYSTEM_NOTICE, NOT as a `subagent_done` frame on this stream (see
     *      `stream_frame.subagent_done` §F3-async-contract). So NO terminal
     *      frame is ever emitted for it and the card spins forever.
     *
     * This helper is the WS-independent settle: it walks ALL tabs and rebuilds
     * any block whose `subagent_id` matches and whose status is still
     * non-terminal (`running` / `background` / `aborting`) into `terminal`
     * (mirrors `handleSubagentDone`). It ALSO retires the id from each tab's
     * `activeSubAgentIds` index — that index is what pins the typing indicator
     * and the composer's Stop affordance (`hasActiveBackgroundWork` is an O(1)
     * read of its size), so settling the block without retiring the id would
     * stop the spinner but leave the conversation looking busy forever.
     *
     * Idempotent — blocks already `done` / `error` are left untouched, so
     * calling it after a real terminal frame is a no-op.
     */
    _settleInlineSubAgentBlocks(
      subagentId: string,
      ...
      for (const tab of this.tabs) {
        ...
        // Retire the live-work id first, and unconditionally: the block scan
        // below early-`continue`s per message, and a tab whose sub-agent
        // message was cleared mid-run would otherwise strand the id forever
        // (same ordering rationale as `handleSubagentDone`).
        tab.activeSubAgentIds.delete(subagentId);
      ...
      // Keep the SubAgentRail chip in step with the card (same pairing
      // `handleSubagentDone` maintains).
      this._updateSubAgentIndexStatus(subagentId, terminal);
    },

    /**
     * Truth-source reconciliation for still-spinning sub-agent cards (P1).
     *
     * Root cause this defends against: under the async spawn contract a
     * detached sub-agent's completion NEVER arrives as a `subagent_done` frame
     * on the parent conversation's stream — it is delivered out-of-band as a
     * SYSTEM_NOTICE (`stream_frame.subagent_done` §F3-async-contract). The
     * parent tab's block therefore keeps the `background` status
     * `subagent_start` created it with, and `SubAgentBlock.vue` spins forever
     * with a live "stop" button for a sub-agent that already finished.
     *
     * AGENTS.md §7 铁律 3: a terminal verdict must come from真实探测, and an
     * optimistic live status must be corrected by polling — never trusted as
     * terminal on its own. The真值 here is the persisted session status behind
     * `GET /api/chat/subagents/{id}`, whose `status` is the backend's
     * `SubAgentSessionStatus` (`running` / `done` / `error` / `interrupted` /
     * `user_owned`).
     *
     * Scope: every id in the ACTIVE main tab's `activeSubAgentIds` (the exact
     * set that pins the spinner + Stop affordance). `user_owned` and `running`
     * are LIVE — left alone. A fetch failure leaves the block live too, so a
     * transient network error never fabricates a terminal state.
     */
    async _reconcileActiveSubAgents(): Promise<void> {
      // EVERY main tab, not just the active one (bug fixed 2026-08-10): a
      // sub-agent outlives the turn that spawned it, and the user routinely
      // switches tabs while it runs. Reconciling only `activeTab` left the
      // spinner turning forever on every OTHER tab — and because the poll
      // disarms itself when the ACTIVE tab has nothing pending, a spawn on a
      // background tab was never polled even once (observed: three tabs each
      // holding an async unit, zero `/api/chat/subagents/` requests).
      const ids = new Set<string>();
      for (const tab of this.tabs) {
        if (tab.kind === "subagent") continue;
        for (const sid of tab.activeSubAgentIds) ids.add(sid);
      }
      ...
      await Promise.all(
        [...ids].map(async (sid) => {
          let status: string;
          try {
            const detail = await apiJson<{ status?: string }>(
              "GET",
              `/api/chat/subagents/${encodeURIComponent(sid)}`,
            );
            status = typeof detail.status === "string" ? detail.status : "";
          } catch {
            // Truth unavailable (offline / 404 on an unpersisted spawn) — keep
            // the card live rather than inventing a terminal state.
            return;
          }
          // Non-terminal per the backend enum: still genuinely working, or
          // handed to the user. Either way the spinner is honest.
          if (status === "" || status === "running" || status === "user_owned") {
            return;
          }
          // `done` succeeded; `error` / `interrupted` (and any future terminal
          // value) did not — the card must not claim success for a failed run.
          this._settleInlineSubAgentBlocks(
            sid,
    ...
    /**
     * Arm / disarm the periodic truth-source reconciliation poll (P1).
     *
     * Idempotent and self-disarming: while ANY main tab has an id in
     * `activeSubAgentIds` a single interval runs `_reconcileActiveSubAgents`
     * every {@link SUBAGENT_RECONCILE_MS}; once every tab's set drains the
     * interval clears itself, so an idle window issues ZERO requests. The poll
     * interval IS the "数秒内变 ✓ 已完成" latency budget.
     *
     * Scope is deliberately WINDOW-wide, not active-tab (bug fixed
     * 2026-08-10): one timer serves every tab, and keying the disarm check on
     * `activeTab` shut the loop down while background tabs still had live
     * spawns — their spinners then ran forever because nothing else clears a
     * detached spawn during the session.
     *
     * This is the polling half of AGENTS.md §7 铁律 3 ("乐观回写…必须有探测轮询
     * 兜底"): the `background` status written by `subagent_start` is the
     * optimistic value, and this loop is what corrects it — the completion of
     * a detached spawn never arrives as a frame on this tab.
     *
     * No-op without a DOM timer (SSR / stripped-global specs), matching
     * `_scheduleSubAgentReconnect`'s degradation: the reload path
     * (`historyMapper`) still settles those cards.
     */
    _ensureSubAgentReconcilePoll(): void {
      if (typeof window === "undefined" || typeof window.setInterval !== "function") {
        return;
      }
      if (_subAgentReconcileTimer !== null) return;
      _subAgentReconcileTimer = window.setInterval(() => {
        let pending = 0;
        for (const tab of this.tabs) {
          if (tab.kind === "subagent") continue;
          pending += tab.activeSubAgentIds.size;
        }
        if (pending === 0) {
          // Nothing left to correct anywhere — disarm so an idle window polls
          // nothing. A later spawn re-arms via `applyFrame`'s START branch.
          this._stopSubAgentReconcilePoll();
          return;
        }
        void this._reconcileActiveSubAgents();
      }, SUBAGENT_RECONCILE_MS);
    },

    /** Disarm the reconciliation poll (drained set / store reset). */
    _stopSubAgentReconcilePoll(): void {
      if (_subAgentReconcileTimer === null) return;
      if (typeof window !== "undefined" && typeof window.clearInterval === "function") {
        window.clearInterval(_subAgentReconcileTimer);
      }
      _subAgentReconcileTimer = null;
    },
    ...
    /**
     * Record a background process's live/terminal state for tab-level busy
     * display (§UX-idle-with-bg-work).
     *
     * Fed from the `background_process.updated` / `.deleted` global-SSE
     * frames (mirrored in `App.vue`, the subscription that already exists —
     * this adds no polling). `status` is the process's status for an
     * `updated` frame, or `null` for `deleted`.
     *
     * Only `starting` / `running` count as live; every other status (and
     * `null`) retires the id.
     *
     * Attribution: these frames carry the manager's `session_id`, not a chat
     * tab id, so a live process is attributed to whichever tab is active when
     * its frame arrives — that is where the user is looking. Over-reporting
     * busy is the safe direction: it never claims work finished when it has
     * not.
     *
     * Retirement, by contrast, is applied to EVERY tab. If it only touched
     * the active tab, switching tabs mid-run would strand the id on the tab
     * that added it: its terminal frame would land on the new tab (a no-op
     * there) and the original tab would show a typing indicator and a Stop
     * button forever. Clearing an id the tab does not hold is free, so the
     * broad delete costs nothing and closes that leak.
     *
     * Idempotent: re-delivering the same status is a no-op.
     */
    noteBackgroundProcessState(
      processId: string,
      status: string | null,
      /**
       * The process's OWNING conversation (`info.session_id` on the
       * `background_process.updated` frame). Resolves which tab gets the busy
       * marker.
       *
       * Why not `activeTab` (bug fixed 2026-08-09): the frame arrives whenever
       * the process changes state, which may be long after the user switched
       * tabs. Attributing it to whatever tab happens to be on screen marked an
       * UNRELATED tab busy (its Stop button lit up, Enter started queueing)
       * while the tab that actually owns the process reported idle. The
       * `deleted` frame carries no `info`, hence optional — retirement already
       * sweeps every tab, so it needs no owner.
       */
      sessionId?: string,
    ): void {
      if (processId === "") return;
      // Live == NOT terminal. Enumerating the live values instead silently
      // dropped `ready` — the steady state of a dev server that passed its
      // readiness probe — so the tab would report idle while it kept running.
      const live = status !== null && !BGP_TERMINAL_STATUSES.has(status);
      if (live) {
        // Owner first (State-Truth-First); fall back to the active tab only
        // when the frame carried no owner, which keeps pre-owner callers and
        // the single-tab case behaving exactly as before.
        const tab =
          sessionId !== undefined && sessionId !== ""
            ? (this.tabs.find((t) => t.conversationId === sessionId) ?? null)
            : this.activeTab;
        if (tab === null) return;
        tab.activeBgProcessIds.add(processId);
        return;
      }
      for (const tab of this.tabs) {
        tab.activeBgProcessIds.delete(processId);
      }
    },
        ...
        for (const t of this.tabs) {
          if (
            ...
            (t.subagentMeta?.status === "running" ||
              t.subagentMeta?.status === "background")
          ) {
            return true;
          }
        }
        for (const entries of Object.values(this.subAgentIndex)) {
          for (const e of entries) {
            if (
              ...
              (e.status === "running" || e.status === "background")
            ) {
              return true;
            }
          }
        ...
        for (let mi = 0; mi < tab.messages.length; mi += 1) {
          ...
          for (let bi = 0; bi < blocks.length; bi += 1) {
            ...
            if (b.status === "aborting") {
              ...
              b.status === "running" ||
              // N.18: a detached (`background`) sub-agent is just as
              // interruptible as an inline running one.
              b.status === "background" ||
    ...
    _subscribeSubAgentStream(tabId: TabId, subagentId: string): void {
      ...
      // N.38 — a subscription is starting NOW, so any retry still armed for
      // this pair is obsolete; cancel it so the two cannot both connect.
      cancelSubAgentReconnectTimer(tabId, subagentId);
      ...
      // N.46 — join the page-wide WS orchestrator so this broadcaster socket
      // participates in the cross-channel visibility / online-offline
      // coupling, exactly like the main data-plane and active-run sockets.
      //
      // `onSuspend` drops the socket rather than merely pausing the retry: the
      // backend keeps the run alive and buffers every frame in the broadcaster
      // (terminal entries survive `retention_seconds`), and the re-subscribe
      // carries `from_seq=<lastApplied+1>`, so a suspended tab loses NOTHING —
      // it just stops holding a connection that cannot deliver. Closing it
      // ALSO stops the proxy/server from counting an idle socket per hidden
      // tab. The `close` handler's own reconnect path is inert while
      // suspended (`_scheduleSubAgentReconnect` checks `isWebSocketSuspended`),
      // so the drop cannot spiral into a retry storm.
      //
      // `onResume` re-enters the ONE reconnect entry point, which re-applies
      // all four abandon guards — a tab closed or a run finished during the
      // outage is not resurrected.
      //
      // De-duped per (tabId, subagentId) by `trackSubAgentOrchestratorRegistration`,
      // so a re-subscribe (tab-switch reuse / drop reconnect) reuses the live
      // registration instead of stacking a second one.
      trackSubAgentOrchestratorRegistration(tabId, subagentId, {
        onSuspend: () => {
          cancelSubAgentReconnectTimer(tabId, subagentId);
          const ctrl = peekSubAgentStream(tabId, subagentId);
          if (ctrl !== undefined && !ctrl.signal.aborted) {
            ctrl.abort();
          }
        },
        onResume: () => {
          // The outage is over. Retry NOW from the rung it interrupted rather
          // than waiting out a delay armed before the page went away.
          this._scheduleSubAgentReconnect(tabId, subagentId);
        },
      });
      ...
      const ensureStreaming = (): void => {
        ...
        // N.38 — this subscription just proved healthy (a frame arrived), so
        // the reconnect ladder resets to its 1s rung. Without this an
        // intermittent link would keep escalating: a drop after a recovered
        // 30s-rung outage would wait 30s again instead of retrying promptly.
        resetSubAgentReconnectAttempts(tabId, subagentId);
      ...
      void (async () => {
        ...
        ws.addEventListener("message", (mev: MessageEvent) => {
          ...
          if (kind === "done" || kind === "error") {
            ...
            terminal = true;
            // N.38 — a terminal envelope means the run is OVER: forget the
            // reconnect ladder so a future subscription (a resume / wake of
            // this sub-agent) starts at the 1s rung rather than inheriting a
            // stale 30s delay from an earlier outage.
            resetSubAgentReconnectAttempts(tabId, subagentId);
            cancelSubAgentReconnectTimer(tabId, subagentId);
            // N.46 — and leave the orchestrator: a finished run must not be
            // re-subscribed by a page-level resume.
            releaseSubAgentOrchestratorRegistration(tabId, subagentId);
        ...
        ws.addEventListener("close", () => {
          ...
          if (!terminal && !controller.signal.aborted) {
            ...
            flushBackfill();
            refreshFromSnapshot();
            // N.38 — AND re-establish the live stream. The snapshot above is a
            // one-shot catch-up; without a reconnect the tab silently drops off
            // the live stream for the rest of the run after a single network
            // blip, and only a manual switch-away-and-back restores it (the
            // reported symptom). The two are complementary, not redundant:
            // snapshot fills the gap, reconnect resumes the flow.
            this._scheduleSubAgentReconnect(tabId, subagentId);
          }
        });
    ...
    /**
     * N.38 — re-establish a dropped sub-agent broadcaster subscription on a
     * bounded exponential backoff (1s → 2s → 4s → 8s → 30s ceiling, ±20%
     * jitter). N.46 — the ladder itself now comes from
     * `useWebSocketOrchestrator.nextBackoffMs` (the single truth source shared
     * with every other WS consumer); `transientHandles` keeps only the
     * per-(tabId, subagentId) attempt bookkeeping. Values are unchanged, so
     * there is no behaviour jump for the user.
     *
     * This is deliberately the ONE and ONLY reconnect entry point for sub-agent
     * streams — the scheduling policy, the liveness guards and the re-subscribe
     * call all sit here rather than being sprinkled through the `close`
     * handler.
     *
     * N.46 cross-channel coordination: a retry is NOT armed while the
     * orchestrator reports the page suspended (offline, or hidden ≥60s).
     * Retrying then would burn the ladder up to its 30s rung against a link
     * that cannot carry a frame, so the user who comes back finds the tab
     * waiting 30s instead of reconnecting promptly. The orchestrator's
     * `onResume` re-arms from the CURRENT rung instead. Suspension is a
     * cross-channel concern; the guards below are sub-agent-specific — hence
     * both, not one replacing the other.
     *
     * Why this is NOT a parallel subscription mechanism: it re-enters
     * `_subscribeSubAgentStream`, so the recovered connection inherits every
     * hard-won property of the existing path unchanged — the
     * `from_seq=<lastApplied+1>` cursor (server elides frames already
     * rendered, so the catch-up neither duplicates nor drops), the backfill
     * batching, and the take-over skip guard. Building a second subscriber
     * would fork the cursor and double-write the transcript.
     *
     * Four conditions abandon the retry — each one means "there is nothing
     * left to stream", so retrying would poll a dead endpoint forever:
     *   1. the tab was closed (gone from `tabs`);
     *   2. the subscription was deliberately torn down (controller aborted by
     *      `clearSubAgentStream` on close, or by a newer subscription);
     *   3. the RUN reached a terminal status per the backend's own
     *      ``subagentMeta.status`` (refreshed by the snapshot fetch the close
     *      handler fires just before us) — NOT the tab's local
     *      ``status``, which that same refresh always settles to ``idle``;
     *   4. a take-over turn owns the tab's main transport — the same condition
     *      `_subscribeSubAgentStream` early-returns on.
     */
    _scheduleSubAgentReconnect(tabId: TabId, subagentId: string): void {
      // No timers in a non-DOM/SSR environment (and specs that stub only
      // WebSocket) — degrade to "snapshot only", exactly the pre-N.38
      // behaviour.
      if (typeof window === "undefined" ||
          typeof window.setTimeout !== "function") {
        return;
      }
      // Cross-channel suspend (N.46): the orchestrator knows the page is
      // offline / long-hidden. Do not consume a rung now — `onResume` calls
      // back here the moment the page is usable again, and the attempt counter
      // is untouched so the retry resumes at the rung the outage interrupted.
      if (isWebSocketSuspended()) {
        return;
      }
      const delay = nextSubAgentReconnectDelayMs(tabId, subagentId);
      // Track the handle so a tab close / terminal settle / superseding
      // subscription can CANCEL the pending retry. Without this an armed timer
      // outlives its subscription and builds a socket for a stream nobody is
      // watching any more.
      const timerId = window.setTimeout(() => {
        const tab = this.tabs.find((t) => t.id === tabId);
        // (1) tab closed.
        if (tab === undefined) return;
        // (2) subscription deliberately torn down / superseded.
        const ctrl = peekSubAgentStream(tabId, subagentId);
        if (ctrl !== undefined && ctrl.signal.aborted) return;
        // (3) the RUN itself already finished. The authority here is
        // ``subagentMeta.status`` — the backend's own verdict, refreshed by the
        // ``_refreshSubAgentTab`` that the close handler fired just before us.
        //
        // Deliberately NOT ``tab.status``: the snapshot refresh ALWAYS settles
        // the tab's local state machine out of ``streaming`` (that is its job —
        // stop the spinner now that the socket is gone), so gating on
        // ``tab.status === "streaming"`` would reject EVERY reconnect and the
        // whole feature would be dead code. The tab's local status describes
        // "is a socket feeding me right now", which is false by construction at
        // this point; the run's liveness is a different question.
        const runStatus = tab.subagentMeta?.status;
        if (runStatus !== undefined && runStatus !== "running") return;
        // (4) a live take-over owns the main transport; the broadcaster holds
        // only the PREVIOUS run's frames (see the skip-guard rationale in
        // `_subscribeSubAgentStream`). `_subscribeSubAgentStream` re-checks
        // this itself, so this is a cheap early out rather than the guard.
        this._subscribeSubAgentStream(tabId, subagentId);
      }, delay);
      trackSubAgentReconnectTimer(tabId, subagentId, timerId);
    },
      ...
      this._patchTab(tabId, {
        ...
        ...(adoptMessages
          ? {
              messages,
              todoList: extractLatestTodoList(messages),
              // N.35/N.36 — the snapshot we just adopted IS the new
              // truth for this tab's blocks, so rederive the index
              // from it rather than trusting incrementally-tracked
              // ids that may predate the snapshot.
              activeSubAgentIds: deriveActiveSubAgentIds(messages),
            }
          : {}),
      ...
      try {
        ...
        if (still !== undefined && still.messages.length === 0) {
          this._patchTab(tabId, {
            ...
            todoList: extractLatestTodoList(messages),
            // N.35/N.36 — rebuild the live sub-agent index from the
            // adopted transcript (no frames replay on reload).
            activeSubAgentIds: deriveActiveSubAgentIds(messages),
          });
          // P1 — a reload can adopt a transcript that still carries live
          // sub-agent blocks (`historyMapper` settles only those the persisted
          // completion notice covers; a genuinely still-running one stays
          // live). Arm the reconciliation poll so those cards settle from真值
          // as soon as the backend reports terminal — this session has no
          // `subagent_start` frame left to arm it.
          this._ensureSubAgentReconcilePoll();
        } else {
          ...
          if (cfg.participants.length > 0) {
            ...
            if (t2 !== undefined && t2.conversationId === convId) {
              ...
              for (let i = 0; i < t2.messages.length; i++) {
                ...
                // Fill `senderName` only when the participant is still in the
                // roster (a removed participant → null → we keep the raw id
                // as the label, avoiding a stale/empty name). But `senderColor`
                // is derived independently: `resolveSpeakerColor` has a hash
                // fallback for unknown participants (coordinator sentinel +
                // removed roles alike), so the live floor and the reloaded
                // bubble share the same theme-aware token — otherwise a
                // coordinator bubble reloads with NO avatar background (`live`
                // has one) and every historically-removed role loses its
                // colour on reload. Only fill each field when it is still
                // missing so live-updated messages are never clobbered.
                const needName =
                  msg.senderName === undefined || msg.senderName === null;
                const needColor =
                  msg.senderColor === undefined || msg.senderColor === "";
                if (!needName && !needColor) continue;
                const resolvedName = needName
                  ? resolveSpeakerName(t2, msg.senderId)
                  : null;
                const resolvedColor = needColor
                  ? resolveSpeakerColor(t2, msg.senderId)
                  : undefined;
                // Nothing to change (unusual — needName true but name still
                // unresolvable AND colour already set). Skip the splice to
                // preserve array identity.
                if (resolvedName === null && resolvedColor === undefined) {
                  continue;
                }
                t2.messages.splice(i, 1, {
                  ...msg,
                  ...(resolvedName !== null ? { senderName: resolvedName } : {}),
    ...
    /**
     * Force-refetch the newest page for the OPEN main-agent tab bound to
     * `conversationId`, replacing its `messages` — unlike
     * {@link loadHistoryMessages} this does NOT short-circuit on a non-empty
     * tab. Used when an out-of-band turn (a scheduled task firing on its
     * bound conversation) persisted an assistant turn the live chat-stream
     * never pushed to this tab: the headless runner drains the turn's frames
     * itself (they are dropped at `_publish` source and reach a client only
     * via the SSE/WS route the runner bypasses), so the message is durable in
     * the DB but the already-open tab shows nothing until a manual reload.
     * This is that reload, driven by the `scheduling.task_fired` event.
     *
     * No-op when: no open tab matches, the tab has no persisted
     * conversationId, or the tab is mid-turn (`streaming` — never clobber a
     * live user turn; the scheduler's busy-skip means a fired turn never
     * coexists with a live one, this is defence in depth). Best-effort: a
     * fetch failure leaves the current messages untouched.
     */
    async reloadConversationMessages(conversationId: ConversationId): Promise<void> {
      if (conversationId === "") {
        return;
      }
      ...
      try {
        ...
        // Re-check the tab still exists, still binds this conversation, and
        // has not started streaming during the round-trip (never overwrite a
        // live turn the user just began).
        const still = this.tabs.find((t) => t.id === tabId);
        // Never let a transient EMPTY fetch clobber a tab that already shows
        // content: a summary/messages race (or an out-of-band fire event
        // arriving a hair before the row is queryable) can momentarily return
        // an empty page. Overwriting with `[]` blanks the pane AND — because
        // `loadHistoryMessages` short-circuits on `messages.length > 0` — it
        // can only be `[]`→`[]` thereafter, so the conversation stays blank
        // until a full reload/restart (the reported "打开会话空白，重启才恢复").
        // State-Truth-First: when we failed to fetch a real page, keep the
        // last known-good messages rather than assert an empty truth.
        if (
          still !== undefined &&
          still.conversationId === conversationId &&
          still.status !== "streaming" &&
          !(messages.length === 0 && still.messages.length > 0)
        ) {
          this._patchTab(tabId, {
            messages,
            messagesOldestPos: startPos,
        ...
        // Best-effort — a refresh hiccup must never disturb the tab.
      }
    },
    ...
    reset(): void {
      _resetChatTabsTransient();
      // P1 — the reconciliation interval outlives `tabs` (module-scope handle),
      // so a reset that dropped every tab would leave it ticking against an
      // empty store (and leak across specs).
      this._stopSubAgentReconcilePoll();
      this.tabs = [];
      this.activeTabId = null;
    },
