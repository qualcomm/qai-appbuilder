...
interface DiscussionConfigWire {
  ...
  // Coordinator (主持人) unified moderator+judge config (tail-appended,
  // optional). Absent ⇒ null / "" (legacy conversations untouched).
  coordinator_model_id?: string | null;
  coordinator_prompt?: string | null;
  // Coordinator permission override — narrows the coordinator's default tool +
  // skill sets. Wire keys map 1:1 (`allowed_tools` / `enabled_skills`); the
  // camelCase view model is `CoordinatorConfig`. Absent / non-object ⇒ null
  // (backend dynamic defaults apply). MERGE semantics: sending
  // `coordinator_config: null` clears the override, sending an object replaces
  // it, omitting the key on PATCH keeps the persisted value.
  coordinator_config?: {
    allowed_tools?: string[] | null;
    enabled_skills?: string[] | null;
  } | null;
...
/** Parse a wire ``coordinator_config`` object into the camelCase view model.
 *  Absent / non-object / all-null wire ⇒ view-model ``null`` (backend applies
 *  its dynamic defaults on read). An EMPTY list on a key is a legitimate
 *  narrowing ("no tools" / "no skills") and is preserved verbatim. */
function wireToCoordinatorConfig(
  raw: DiscussionConfigWire["coordinator_config"],
): CoordinatorConfig | null {
  if (raw == null || typeof raw !== "object") return null;
  const out: CoordinatorConfig = {};
  if (Array.isArray(raw.allowed_tools)) {
    out.allowedTools = raw.allowed_tools.filter(
      (x): x is string => typeof x === "string",
    );
  }
  if (Array.isArray(raw.enabled_skills)) {
    out.enabledSkills = raw.enabled_skills.filter(
      (x): x is string => typeof x === "string",
    );
  }
  return out.allowedTools === undefined && out.enabledSkills === undefined
    ? null
    : out;
}

/** Serialise a view-model ``CoordinatorConfig`` back to the wire shape.
 *
 *  Contract with the backend :class:`SetDiscussionConfigUseCase.execute`:
 *  - ``null`` = "clear the override, restore the dynamic default". The
 *    backend PATCH treats a missing ``coordinator_config`` key as "keep
 *    untouched" (PATCH semantics) and an EXPLICIT ``{}`` as "clear the
 *    override" — so an incoming view-model ``null`` MUST be translated to
 *    ``{}`` on the wire, not left as ``null``. (Sending ``null`` would
 *    silently no-op: the backend would keep whatever override was already
 *    persisted, and the user's "Reset to defaults" click would have no
 *    effect.)
 *  - An object narrows one or both dimensions; an omitted key on the object
 *    keeps that dimension on the backend dynamic default (partial narrow).
 */
function coordinatorConfigToWire(
  cfg: CoordinatorConfig | null,
): Record<string, unknown> {
  if (cfg == null) return {};
  const out: Record<string, unknown> = {};
  if (cfg.allowedTools !== undefined) out.allowed_tools = cfg.allowedTools;
  if (cfg.enabledSkills !== undefined) out.enabled_skills = cfg.enabledSkills;
  return out;
}
...
export const useDiscussionStore = defineStore("discussion", () => {
  ...
  async function fetchConfig(conversationId: string): Promise<DiscussionConfig> {
    ...
    return {
      ...
      // Coordinator (主持人): absent / blank model ⇒ null; absent prompt ⇒ "".
      coordinatorModelId:
        typeof cfg.coordinator_model_id === "string" &&
        cfg.coordinator_model_id !== ""
          ? cfg.coordinator_model_id
          : null,
      coordinatorPrompt:
        typeof cfg.coordinator_prompt === "string" ? cfg.coordinator_prompt : "",
      coordinatorConfig: wireToCoordinatorConfig(cfg.coordinator_config),
  ...
  async function patchConfig(
    ...
    if (patch.coordinatorModelId !== undefined)
      body.coordinator_model_id = patch.coordinatorModelId;
    if (patch.coordinatorPrompt !== undefined)
      body.coordinator_prompt = patch.coordinatorPrompt;
    if (patch.coordinatorConfig !== undefined) {
      body.coordinator_config = coordinatorConfigToWire(patch.coordinatorConfig);
    }
