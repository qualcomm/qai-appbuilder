...
export const useAppBuilderStore = defineStore("appBuilder", () => {
  ...
  // App ids whose model-missing pre-check message the user explicitly
  // dismissed via "clear logs". While present, fetchApps() will NOT
  // re-synthesize the ``failed`` model-not-installed state for that app, so
  // a cleared panel stays clear across list refreshes AND page reloads.
  // Cleared when the app is (re)started or deleted. Persisted to
  // localStorage so "clear" survives a browser refresh (the model is still
  // absent, but the user has already seen and dismissed the guidance).
  const _DISMISSED_LS_KEY = "appBuilder.dismissedModelMissing";
  function _loadDismissed(): Set<string> {
    try {
      const raw = localStorage.getItem(_DISMISSED_LS_KEY);
      if (raw) {
        const arr = JSON.parse(raw);
        if (Array.isArray(arr)) return new Set(arr.filter((x) => typeof x === "string"));
      }
    } catch {
      // localStorage unavailable / malformed — start empty (non-fatal).
    }
    return new Set<string>();
  }
  const _dismissedModelMissing = _loadDismissed();
  function _persistDismissed(): void {
    try {
      localStorage.setItem(
        _DISMISSED_LS_KEY,
        JSON.stringify([..._dismissedModelMissing]),
      );
    } catch {
      // Best-effort — an in-memory set still suppresses within this session.
    }
  }
  ...
  async function fetchApps(): Promise<void> {
    try {
      ...
      // Auto-synthesize a failed run-state for apps whose bundled models are
      // not installed so the UI can auto-expand the log panel with setup
      // guidance on page load (model-missing auto-expand UX).
      for (const app of apps.value) {
        if (
          app.models_ready === false &&
          app.models_missing?.length &&
          !_dismissedModelMissing.has(app.id)
        ) {
          // Only set if no active run-state exists (don't overwrite a real
          // running/starting/stopping state from a previous run attempt).
          const existing = appRunStates.value[app.id];
          if (!existing || existing.status === "stopped") {
            const first = app.models_missing[0];
            const modelId = (first?.model_id ?? "") as string;
            const guide = (first?.setup_guide_url ?? "") as string;
            const provisioning = (first?.provisioning ?? "download") as string;
            let msg: string;
            if (provisioning === "user-provided" && guide) {
              msg =
                `app '${app.id}' cannot run: model(s) require manual setup: ` +
                `${modelId}. See setup guide: ${guide}`;
            } else {
              msg =
                `app '${app.id}' cannot run: model(s) not installed: ` +
                `${modelId}. Install the model(s) from the App Builder ` +
                "model panel (or re-run the app's install script) and try again.";
            }
            appRunStates.value = {
              ...appRunStates.value,
              [app.id]: {
                ...
                processId: null,
                manualCommand: null,
                error: "app_builder.model_not_installed",
                errorMessage: msg,
                errorDetails: { missing: app.models_missing },
              },
            };
          }
        }
  ...
  async function deleteApp(appId: string): Promise<void> {
    ...
    _appNotReadyStreaks.delete(appId);
    _appStartingInFlight.delete(appId);
    _dismissedModelMissing.delete(appId);
    _persistDismissed();
  ...
  async function runApp(
    ...
    appRunStates.value = {
      ...
      [appId]: {
        ...
        error: null,
        errorMessage: null,
        errorDetails: prevSlot?.errorDetails ?? null,
      },
    };
    _appStartingInFlight.add(appId);
    // User re-engaged this app: if they had dismissed a prior model-missing
    // message, allow it to resurface should the launch fail the same way.
    _dismissedModelMissing.delete(appId);
    _persistDismissed();
    ...
    try {
      ...
      // POST resolved: the launch lifecycle is over — the response status is
      // authoritative. Release the startup clamp so real state shows through.
      _appStartingInFlight.delete(appId);
      ...
      // Bug #3: surface the backend error inline. On 409 port-in-use / 422
      // model-not-installed the frontend previously stayed on the
      // optimistic ``starting`` state and the log panel showed "no logs"
      // — the user saw no useful signal. Force ``failed`` and capture the
      // human-readable message so the log panel renders it inline (see
      // ``ModeFrameAppBuilder.vue``: it prefers ``runState.errorMessage``
      // over the "no logs" placeholder when the run is in ``failed``).
      const errorMessage =
        e instanceof Error && typeof e.message === "string"
          ? e.message
          : null;
      // ApiError carries the structured backend envelope's `details` (e.g.
      // for `model_not_installed`: `{missing: [{model_id, provisioning,
      // setup_guide_url, ...}]}`). Keep it on the slot so the log panel
      // can render richer affordances than the flat message string —
      // notably "See setup guide: <path>" for user-provided packs (P6).
      const errorDetails =
      ...
      // Force one final log fetch so the /logs ring-buffer tail is captured
      // even when the status poll missed the last tick before the failure.
      // AppProjectStartFailedError.details.log_tail on `errorDetails` is the
      // failure-time snapshot; /logs gives the ring-buffer version — either
      // can carry the real signal (e.g. an ImportError line that beat the
      // snapshot). Logs-only: we JUST set status='failed' with the concrete
      // error, and /logs may return a stale 'stopped' status now that the
      // child exited — we MUST NOT let that clobber the failed slot.
      // Best-effort: a fetch failure is swallowed and does NOT shadow the
      // original run error we re-throw below.
      try {
        ...
        console.warn(
          "[appBuilder] runApp post-failure logs fetch failed:",
    ...
    try {
      ...
      appRunStates.value = {
        ...
        [appId]: {
          ...
          errorDetails: prev?.errorDetails ?? null,
        },
      };
    } catch (e) {
      console.warn(
  ...
  /**
   * Clear an app's inline log panel: drop the retained log tail and, if the
   * current state is a ``failed`` (e.g. the model-missing pre-check message),
   * reset the slot to a neutral ``stopped`` so the red error stops lingering.
   *
   * For a model-not-installed failure the app id is remembered in
   * ``_dismissedModelMissing`` so ``fetchApps`` will not immediately
   * re-synthesize the same message on the next list refresh — the user has
   * seen it. Starting or deleting the app clears that suppression.
   */
  function clearAppLog(appId: string): void {
    appLogs.value = { ...appLogs.value, [appId]: "" };
    const prev = appRunStates.value[appId];
    if (prev?.status === "failed") {
      if (prev.error === "app_builder.model_not_installed") {
        _dismissedModelMissing.add(appId);
        _persistDismissed();
      }
      appRunStates.value = {
        ...appRunStates.value,
  ...
  /** Whether the user has dismissed the model-missing message for this app
   *  (persisted across reloads). The auto-expand-on-open UX honours this so a
   *  cleared panel does not pop back open on the next menu open / refresh. */
  function isModelMissingDismissed(appId: string): boolean {
    return _dismissedModelMissing.has(appId);
  }
  ...
  // Bug #4 defence-in-depth: how many consecutive not-ready polls the FE
  // needs before it flips a previously-``ready``/``running`` app back to
  // ``starting``. A single-poll flap (stale process still answering
  // /health while the new one is binding) no longer bounces the run
  // button. Reset to 0 on any ready/running/terminal poll.
  const _appNotReadyStreaks = new Map<string, number>();
  const _APP_STARTING_DEBOUNCE = 2;

  // Startup lifecycle lock: while a runApp() POST is in flight the app is
  // genuinely mid-launch. During this window the /logs status can briefly
  // report ``stopped`` (process spawned but not yet registered) or oscillate
  // ``starting``↔non-ready before ``/health`` settles. Clamping those samples
  // to ``starting`` keeps the Run button stably disabled instead of flapping
  // stopped→running (the "button flickers once during startup" bug). The
  // clamp is released the moment runApp()'s POST resolves (ready/running) or
  // fails (failed) — the authoritative terminal states always win.
  const _appStartingInFlight = new Set<string>();
  ...
  async function refreshAppStatus(appId: string): Promise<string | null> {
    try {
      ...
      appLogs.value = { ...appLogs.value, [appId]: res.output };
      const prev = appRunStates.value[appId];
      // Bug #4: debounce ``ready``/``running`` → ``starting`` transitions.
      // A single stale /health probe can briefly report ``starting`` even
      // though the app is fine — flipping the badge on that one sample
      // makes the run button oscillate. Require 2 consecutive not-ready
      // polls before we accept the downgrade. Any terminal / steady
      // status is applied immediately.
      let effectiveStatus = res.status;
      // Startup-lifecycle clamp (fixes the mid-startup button flicker):
      // while runApp()'s POST is still in flight the app is definitively
      // launching. The /logs status can transiently read ``stopped`` (the
      // process is spawned but ``_active`` registration / ``/health`` has
      // not settled) which would momentarily re-enable the Run button.
      // Hold the badge at ``starting`` for any non-terminal sample; only a
      // real ``running``/``ready``/``failed`` is allowed through mid-flight
      // (and the POST resolution itself releases the clamp + sets the state).
      if (_appStartingInFlight.has(appId)) {
        if (
          res.status !== "running" &&
          res.status !== "ready" &&
          res.status !== "failed"
      ...
      const wasStable =
        prev?.status === "ready" || prev?.status === "running";
      if (wasStable && effectiveStatus === "starting") {
        const streak = (_appNotReadyStreaks.get(appId) ?? 0) + 1;
        if (streak < _APP_STARTING_DEBOUNCE) {
          _appNotReadyStreaks.set(appId, streak);
      ...
      // Failed-status preservation: ``failed`` is a FRONT-END-SYNTHESIZED
      // status (the backend only knows starting/running/ready/stopped/exited
      // as raw process states — "failed" is our label for "run attempt
      // errored"). The /logs endpoint returns the process's real state,
      // which is ``stopped`` after a crash. Without this guard, the poll
      // that follows a failed run overwrites the just-set failed slot with
      // ``stopped``, hiding the error badge and the Ask-Agent button. Only
      // allow ``failed`` to be replaced when the app truly re-engages
      // (running/ready/starting) or the user clears the slot (clearAppLog
      // sets it to stopped explicitly).
      if (prev?.status === "failed") {
        if (
