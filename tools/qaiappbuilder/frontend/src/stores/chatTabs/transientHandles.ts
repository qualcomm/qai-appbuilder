...
// N.46 — the ONE backoff ladder every WS consumer shares, plus the
// cross-channel registry (visibility / online-offline coupling). Pure
// functions with no store back-reference, so this introduces no module cycle.
import {
  nextBackoffMs,
  registerWebSocketChannel,
  type WebSocketChannelHooks,
...
// ---------------------------------------------------------------------------
// Sub-agent WS reconnect backoff (N.38; ladder owned by the orchestrator
// since N.46).
// ---------------------------------------------------------------------------
// Consecutive failed-connection count per (tabId, subagentId), used ONLY to
// pick the next backoff delay. Reset to 0 the moment a subscription proves
// healthy (first frame applied), so an intermittent link does not inherit a
// long delay from an outage that already recovered.
//
// Module-level `Map` (non-reactive) for the same reason as every other handle
// here: it is transport bookkeeping, never rendered.
//
// N.46 — the LADDER ITSELF (1s → 2s → 4s → 8s → 30s ceiling, ±20% jitter) is
// no longer defined here: it lives in `useWebSocketOrchestrator.nextBackoffMs`,
// the single truth source every WS consumer shares. What stays here is the
// per-(tabId, subagentId) ATTEMPT BOOKKEEPING, which is inherently
// sub-agent-specific (sibling sub-agents must not share a ladder position) and
// has nothing to do with cross-channel coordination. Values are unchanged, so
// the swap is a semantic no-op for the user.
const _subAgentReconnectAttempts = new Map<TabId, Map<string, number>>();

/** Next reconnect delay in ms for this (tabId, subagentId), and bump the
 *  attempt counter. Returns a jittered value off the shared ladder. */
export function nextSubAgentReconnectDelayMs(
  tabId: TabId,
  subagentId: string,
): number {
  let inner = _subAgentReconnectAttempts.get(tabId);
  if (inner === undefined) {
    inner = new Map();
    _subAgentReconnectAttempts.set(tabId, inner);
  }
  const attempt = inner.get(subagentId) ?? 0;
  inner.set(subagentId, attempt + 1);
  return nextBackoffMs(attempt);
}

/** Forget the attempt count for this (tabId, subagentId) — or every sub-agent
 *  in `tabId` when omitted. Called when a subscription proves healthy (so the
 *  ladder restarts at 1s next time) and on tab close / terminal settle. */
export function resetSubAgentReconnectAttempts(
  tabId: TabId,
  subagentId?: string,
): void {
  if (subagentId === undefined) {
    _subAgentReconnectAttempts.delete(tabId);
    return;
  }
  const inner = _subAgentReconnectAttempts.get(tabId);
  if (inner === undefined) return;
  inner.delete(subagentId);
  if (inner.size === 0) _subAgentReconnectAttempts.delete(tabId);
}

/** Test/reset helper — drops every tab's reconnect bookkeeping. */
export function _resetSubAgentReconnectAttempts(): void {
  _subAgentReconnectAttempts.clear();
}

/** Pending reconnect timer handle per (tabId, subagentId). At most one is
 *  outstanding per pair: a second schedule for the same pair replaces the
 *  first, so a burst of close events cannot fan out into N racing retries. */
const _subAgentReconnectTimers = new Map<TabId, Map<string, number>>();

/** Remember a scheduled reconnect timer, cancelling any prior one for the same
 *  (tabId, subagentId). Returns nothing — the caller has already scheduled. */
export function trackSubAgentReconnectTimer(
  tabId: TabId,
  subagentId: string,
  timerId: number,
): void {
  let inner = _subAgentReconnectTimers.get(tabId);
  if (inner === undefined) {
    inner = new Map();
    _subAgentReconnectTimers.set(tabId, inner);
  }
  const prior = inner.get(subagentId);
  if (prior !== undefined) _clearTimer(prior);
  inner.set(subagentId, timerId);
}

/** Cancel a pending reconnect timer for this (tabId, subagentId) — or every
 *  sub-agent in `tabId` when omitted. Called on tab close, on a terminal
 *  settle, and when a fresh subscription supersedes the pending retry, so a
 *  torn-down stream can never resurrect itself later. */
export function cancelSubAgentReconnectTimer(
  tabId: TabId,
  subagentId?: string,
): void {
  const inner = _subAgentReconnectTimers.get(tabId);
  if (inner === undefined) return;
  if (subagentId === undefined) {
    for (const id of inner.values()) _clearTimer(id);
    _subAgentReconnectTimers.delete(tabId);
    return;
  }
  const id = inner.get(subagentId);
  if (id !== undefined) _clearTimer(id);
  inner.delete(subagentId);
  if (inner.size === 0) _subAgentReconnectTimers.delete(tabId);
}
...
/** Test/reset helper — cancels and forgets every pending reconnect timer. */
export function _resetSubAgentReconnectTimers(): void {
  for (const inner of _subAgentReconnectTimers.values()) {
    for (const id of inner.values()) _clearTimer(id);
  }
  _subAgentReconnectTimers.clear();
}

// ---------------------------------------------------------------------------
// Sub-agent WS orchestrator registrations (N.46).
// ---------------------------------------------------------------------------
// One registration per (tabId, subagentId) with the cross-channel
// `useWebSocketOrchestrator`, so the sub-agent broadcaster socket participates
// in the page-wide visibility / online-offline coupling. Kept next to the
// timers because it shares their lifecycle exactly: armed when a subscription
// starts, released on tab close / terminal settle.
//
// The registration handle is opaque here on purpose — this module owns the
// bookkeeping, `chatTabs._subscribeSubAgentStream` owns the hooks.
const _subAgentOrchestratorRegs = new Map<
  TabId,
  Map<string, WebSocketChannelRegistration>
>();

/** Register `hooks` for this (tabId, subagentId) with the orchestrator, unless
 *  one is already live for the pair (a re-subscribe reuses it — the hooks are
 *  identical, and re-registering would just inflate the refcount). */
export function trackSubAgentOrchestratorRegistration(
  tabId: TabId,
  subagentId: string,
  hooks: WebSocketChannelHooks,
): void {
  let inner = _subAgentOrchestratorRegs.get(tabId);
  if (inner === undefined) {
    inner = new Map();
    _subAgentOrchestratorRegs.set(tabId, inner);
  }
  if (inner.has(subagentId)) return;
  inner.set(
    subagentId,
    registerWebSocketChannel(`subagent-ws:${tabId}/${subagentId}`, hooks),
  );
}

/** Release the orchestrator registration for this (tabId, subagentId) — or
 *  every sub-agent in `tabId` when omitted. Called on the same edges as
 *  `cancelSubAgentReconnectTimer` so a closed tab stops being woken by a
 *  page-level resume. */
export function releaseSubAgentOrchestratorRegistration(
  tabId: TabId,
  subagentId?: string,
): void {
  const inner = _subAgentOrchestratorRegs.get(tabId);
  if (inner === undefined) return;
  if (subagentId === undefined) {
    for (const reg of inner.values()) reg.unregister();
    _subAgentOrchestratorRegs.delete(tabId);
    return;
  }
  inner.get(subagentId)?.unregister();
  inner.delete(subagentId);
  if (inner.size === 0) _subAgentOrchestratorRegs.delete(tabId);
}

/** Test/reset helper — releases every orchestrator registration. */
export function _resetSubAgentOrchestratorRegistrations(): void {
  for (const inner of _subAgentOrchestratorRegs.values()) {
    for (const reg of inner.values()) reg.unregister();
  }
  _subAgentOrchestratorRegs.clear();
}
...
export function _resetChatTabsTransient(): void {
  ...
  // N.38 — reconnect backoff bookkeeping + any timer still armed. Cancelling
  // the timer is REQUIRED, not just tidy: a pending retry that fires after a
  // reset would build a socket for a torn-down subscription (and, in specs,
  // leak a connection into the next test).
  _resetSubAgentReconnectAttempts();
  _resetSubAgentReconnectTimers();
  // N.46 — and the orchestrator registrations, so a spec's channels do not
  // linger and receive the next spec's suspend/resume fan-out.
  _resetSubAgentOrchestratorRegistrations();
}
