...
/** Extract a machine-readable `error_code` from a tool result envelope.
 *  A tool that FAILS in a user-fixable way `return`s (does not raise) a
 *  dict envelope `{ ok: false, error_code, message }` (e.g. `web_search`
 *  on a TLS-untrusted enterprise gateway → `chat.llm.tls_cert_untrusted`).
 *  The registry layer preserves a returned dict's `error_code` verbatim, so
 *  it arrives here on `payload.result`. We surface it as a structured field
 *  on the tool card (`ChatToolCall.errorCode`) so the card can render the
 *  SAME actionable affordance (disable-TLS-and-retry) the turn-level error
 *  bubble uses — driven off a code, never a brittle message-substring match.
 *  Returns undefined when the result is not such an envelope. */
export function readToolResultErrorCode(result: unknown): string | undefined {
  if (result === null || typeof result !== "object") return undefined;
  const r = result as Record<string, unknown>;
  if (r.ok !== false) return undefined;
  return typeof r.error_code === "string" && r.error_code !== ""
    ? r.error_code
    : undefined;
}
...
/** N.35/N.36 — rebuild the flat live-sub-agent index from PERSISTED blocks.
 *
 *  The index (`ChatTab.activeSubAgentIds`) is normally maintained
 *  incrementally by the `subagent_*` handlers below, but a RELOAD has no
 *  frames to replay: work that finished while the page was closed will never
 *  emit a terminal frame to this client. So every path that adopts a fetched
 *  transcript (`loadHistoryMessages`, `_openSubAgentTabInner`,
 *  `_refreshSubAgentTab`) rebuilds the whole index from the blocks it just
 *  mapped — the persisted `status` field is the truth source the index derives
 *  from.
 *
 *  Live statuses mirror `hasActiveBackgroundWork`'s old scan exactly:
 *  `running` (inline), `background` (N.18 detached spawn) and `aborting`
 *  (client-side optimistic stop). Blocks with no `subagent_id` are skipped —
 *  an un-persisted block has no stable identity to key the index by, and a
 *  terminal frame could never remove it. */
export function deriveActiveSubAgentIds(
  ...
  for (const msg of messages) {
    ...
    for (const b of blocks) {
      ...
      if (typeof sid === "string" && sid !== "") live.add(sid);
    }
  }
  return live;
}

/** The five wire values of `SubAgentErrorKind`, as a membership table.
 *  MIRROR of the backend domain enum (see the union's doc in
 *  `_chatTabsTypes.ts`); a `Record<..., true>` keyed by the union makes the
 *  compiler reject adding a kind to the union without listing it here. */
const _SUB_AGENT_ERROR_KINDS: Record<SubAgentErrorKind, true> = {
  preflight: true,
  isolation: true,
  execution: true,
  budget: true,
...
/**
 * Does a sub-agent's error message describe an EXHAUSTED QGenie allowance?
 *
 * Only the daily / weekly / cost limits count. `RequestsPerMinute` and
 * `TokensPerMinute` are rolling windows that recover on their own within a
 * minute — surfacing a "switch your quota class" prompt for those would nag
 * about a condition that fixes itself, and switching would not even help.
 *
 * Matched on the upstream's own `limit_type` vocabulary (the spec's
 * `RateLimitErrorType` enum, carried inside its `RateLimitExceeded` 429 body),
 * which reaches us embedded in the sub-agent's error string because the tool
 * layer stringifies the upstream body. A plain substring test is deliberate:
 * the message has passed through error formatting and is not reliably JSON by
 * the time it lands here, so parsing would be more fragile, not less.
 *
 * Conservative by construction — an unrecognised message yields `false`, which
 * simply defers the quota check to the terminal END frame (the pre-existing
 * path) rather than raising a prompt on an unrelated failure.
 */
const _EXHAUSTION_LIMIT_TYPES = [
  "ModelTokensPerDay",
  "CloudTokensPerDay",
  "OnPremTokensPerDay",
  "ModelTokensPerWeek",
  ...
  "CostPerDay",
  "CostPerMonth",
  "CostPerMonthOverflow",
  "CostVpSharedPool",
] as const;

export function looksLikeQuotaExhaustion(message: string): boolean {
  return _EXHAUSTION_LIMIT_TYPES.some((kind) => message.includes(kind));
}

/** Narrow an untrusted `subagent_done.error_kind` payload value to the union.
 *  An unrecognised kind (a backend newer than this client) returns `undefined`
 *  so the card degrades to the plain `error` string rather than rendering an
 *  unstyled slot keyed by a value `ERROR_STYLES` has no entry for. */
function _asSubAgentErrorKind(raw: unknown): SubAgentErrorKind | undefined {
  return typeof raw === "string" && raw in _SUB_AGENT_ERROR_KINDS
    ? (raw as SubAgentErrorKind)
    : undefined;
}
...
export interface FrameHandlerContext {
  ...
  isBackfill?: boolean;
  /** Publish a `quota_usage` frame's balance to the token-pool store.
   *  Optional because the balance is GLOBAL, not per-tab: unlike every other
   *  callback here it does not touch the tab at all, and a caller that has no
   *  reason to surface a balance (tests, a non-pool deployment) simply omits
   *  it and the handler no-ops. Keeping it on `ctx` rather than importing the
   *  store here preserves this module's rule — handlers never call Pinia
   *  directly, so they stay unit-testable without a Pinia instance. */
  applyQuotaUsage?: (quota: unknown) => void;
}
...
function senderFields(
  ...
  Pick<ChatMessage, "senderId" | "senderName" | "senderColor" | "isCoordinator">
> {
  ...
  return {
    ...
    ...(tab.streamingSenderColor !== null
      ...
      : {}),
    ...(tab.streamingIsCoordinator ? { isCoordinator: true } : {}),
  };
}
...
const handleToolResult: FrameHandler = (tab, frame, ctx) => {
  ...
  // Structured, user-fixable failure code (e.g. TLS-untrusted from web_search).
  // The backend now carries it as a dedicated `tool_result` frame field
  // (`payload.error_code`, a string) because the rendered `result` text drops
  // the code; prefer that, and fall back to scanning a structured `result`
  // envelope for older frames. Undefined for ordinary successes / raised
  // ToolError sentinels.
  const errorCode =
    typeof payload.error_code === "string" && payload.error_code !== ""
      ? payload.error_code
      : readToolResultErrorCode(payload.result);
  // A user-fixable error envelope (ok:false + error_code) is a failure even
  // though its serialized text lacks the `[tool_error]` sentinel prefix — so
  // the card settles into the red error state and can show its fix affordance.
  const isFailure = isError || errorCode !== undefined;
  ...
  // Auto-background terminal marker (backend appended field §3.1). `true`
  // means the underlying command outran the foreground-wait threshold and
  // was handed to the background-process manager — the tool call returned
  // early but the command IS STILL RUNNING. Distinct from done (nothing
  // finished) and from error (nothing failed). We read the STRUCTURED
  // field, never the `[Auto-parked]` prose in `result`.
  const backgrounded =
    payload.backgrounded === true ? true : undefined;
  const backgroundProcessId =
    typeof payload.background_process_id === "string" &&
    payload.background_process_id !== ""
      ? payload.background_process_id
      : undefined;
  // Semantic terminal marker (backend appended field §3.1). Today's values are
  // `"stopped"` / `"restarted"` from background_process(action=stop|restart);
  // any other string is ignored so a future backend addition can't crash the
  // card if this frontend has not learned about the new state yet.
  const rawTerminalKind =
    typeof payload.terminal_kind === "string" ? payload.terminal_kind : "";
  const terminalKind: "stopped" | "restarted" | undefined =
    rawTerminalKind === "stopped"
      ? "stopped"
      : rawTerminalKind === "restarted"
        ? "restarted"
        : undefined;
  // Priority: error > backgrounded > terminal_kind > done. An error envelope
  // stays an error even if the tool also set a distinct terminal kind; a
  // running-under-bg outcome dominates a "stopped" one (they can't both be
  // true for the same call in practice, but the ladder is defined for safety).
  const settledStatus:
    | "error"
    | "backgrounded"
    | "stopped"
  ...
  calls[slot.callIdx] = {
    ...
    // User-fixable failure code (undefined on success/cancel) — drives the
    // card's inline fix affordance (e.g. disable-TLS-and-retry for web_search).
    errorCode,
  };
  const nextMsg: ChatMessage = { ...msg, toolCalls: calls };
  const patchMessages = [
  ...
  // Quota warning advisory (web_search): when the backend embeds a
  // `quota_warning` dict in the tool result payload, surface it as an inline
  // system notice (reuses the same isCommandReply rendering path as
  // turn_warning) so the user knows their keyed-engine allowance is running low.
  const quotaWarning =
    payload.quota_warning !== null &&
    typeof payload.quota_warning === "object" &&
    typeof (payload.quota_warning as Record<string, unknown>).message === "string"
      ? (payload.quota_warning as { message: string; kind: string })
      : null;
  if (quotaWarning !== null) {
    patchMessages.push({
      id: ctx.nextMessageId(),
      role: "assistant",
      content: `⚠️ ${quotaWarning.message}`,
      isCommandReply: true,
    } as ChatMessage);
  }

  ctx.patchTab({ messages: patchMessages });
};
...
const handleEnd: FrameHandler = (_tab, frame, ctx) => {
  ...
  // §3.1 tail-append: the persisted assistant Message id for this turn (same
  // supplementary end frame that carries `request_id`). Capture it so
  // `confirmDone` commits the trailing assistant bubble with the DB id — which
  // makes per-message fork (`up_to_message_id`) work on a just-finished turn
  // without a reload. Absent on legacy backends → fall back to a local id.
  const amid = payload.assistant_message_id;
  if (typeof amid === "string" && amid !== "") {
    ctx.patchTab({ streamingAssistantMessageId: amid });
  }
  ...
  // QGenie dual-bucket quota: a turn just finished, which is the one moment
  // its daily counters can have moved. Raise a monotonic signal so a setup
  // context (the sidebar gauge) can refresh the snapshot and, if the active
  // allowance is nearly spent, offer to switch to the other one.
  //
  // Signalled rather than fetched here for the same reason as
  // `budget_exceeded` above: this store has no i18n / dialog instance, and a
  // quota read is an ~860 ms network call that must not sit in a frame
  // handler. `Date.now()` keeps it strictly increasing so consecutive turns
  // each trigger the watcher.
  //
  // `IS_INTERNAL` first: the QGenie provider ships only with the internal
  // edition, so an external build has nothing to check and the whole branch
  // folds away at build time.
  //
  // Then gated OFF the SUPPLEMENTARY end frames, which a single turn also
  // emits: `reason: "snapshot"` follows the real terminator
  // (streaming.py:6201) and `budget_exceeded` is handled above. Signalling on
  // those too would make one turn trigger two or three forced quota reads —
  // three upstream calls each, against a ~60 s budget — throttling the very
  // endpoint this feature depends on.
  if (
    IS_INTERNAL &&
    payload.reason !== "snapshot" &&
    payload.reason !== "budget_exceeded"
  ) {
    ctx.patchTab({ qgenieQuotaCheckSignal: Date.now() });
  }
};
...
const handleQuotaUsage: FrameHandler = (_tab, frame, ctx) => {
  // Token-pool balance (QAI Service). The broker appends its balance to the
  // stream just before the terminal frame, so the sidebar gauge tracks each
  // answer for free — no polling loop, no extra round-trip.
  //
  // Backend payload shape (qai/chat/domain/stream_frame.py quota_usage):
  //   { quota: { allocated, used, remaining, reserved?, reset_at?,
  //              period?, unit? } }
  //
  // Global state, not tab state: the allowance belongs to the signed-in user,
  // so this is the one handler that patches no tab. Validation lives in the
  // store (`toBalance`) rather than here, so the seed path and the frame path
  // cannot drift on what counts as a usable balance.
  const payload = asPayload(frame);
  if (payload === null) return;
  ctx.applyQuotaUsage?.(payload.quota);
};
...
const handleSubagentStart: FrameHandler = (tab, frame, ctx) => {
  ...
  // N.35/N.36 — flat live index. A START frame means this sub-agent is doing
  // work RIGHT NOW, whether inline (`running`) or detached (`background`), so
  // it belongs in the index either way; only its terminal frame removes it.
  // Mutating the Set in place is what makes `hasActiveBackgroundWork` O(1) —
  // and Vue 3's reactive proxy instruments `Set.add`, so the getter recomputes
  // (verified by `chatTabs.activeSubAgentIds.spec.ts`, not assumed).
  // Skipped without a `subagent_id`: an un-persisted block has no stable key,
  // and no terminal frame could ever remove it (it would pin the Stop button
  // on forever).
  if (subagentId !== undefined) tab.activeSubAgentIds.add(subagentId);
...
const handleSubagentDone: FrameHandler = (tab, frame, ctx) => {
  ...
  //
  // §F3-async-contract (2026-08-07): under the new async agent contract
  // every ``subagent_done`` frame reaching the front-end is a GENUINE
  // terminal completion (there is no synthetic parked-done anymore — see
  // ``stream_frame.subagent_done`` docstring and the accumulator's
  // matching branch).  So this handler always flips the block to
  // ``done`` — no more parked-vs-done split.  ``hasActiveBackgroundWork``
  // instead reads ``running`` / ``aborting`` (the pre-terminal states),
  // which are the ONLY states a still-working block can be in now.
  const payload = asPayload(frame);
  if (payload === null) return;
  const idx = payload.index;
  if (typeof idx !== "number") return;
  ...
  const subagentId =
    typeof subagentIdRaw === "string" && subagentIdRaw !== ""
      ? subagentIdRaw
      : undefined;
  // N.54 typed-failure tail: a FAILED run terminates through THIS frame too
  // (the async contract mints one terminal frame type per run), carrying the
  // classification trio. When `error_kind` is present the block is an ERROR
  // block, not a `done` one — so the status below follows the kind.
  const errorKindRaw = payload["error_kind"];
  const errorKind = _asSubAgentErrorKind(errorKindRaw);
  const errorMessageRaw = payload["error_message"];
  const errorMessage =
    typeof errorMessageRaw === "string" && errorMessageRaw !== ""
      ? errorMessageRaw
      : undefined;
  const recoveryHintRaw = payload["recovery_hint"];
  const recoveryHint =
    typeof recoveryHintRaw === "string" && recoveryHintRaw !== ""
      ? recoveryHintRaw
      : undefined;

  // N.35/N.36 — a terminal frame ALWAYS retires the id from the live index,
  // and it must do so BEFORE the block-patching early-returns below: if the
  // turn's sub-agent message is already gone (tab cleared / reset mid-run),
  // skipping the delete would strand the id and pin the typing indicator +
  // Stop button on forever with nothing left to stop.
  if (subagentId !== undefined) tab.activeSubAgentIds.delete(subagentId);
  ...
  const next = blocks.map((b) => {
    ...
    return {
      ...b,
      status: errorKind !== undefined ? ("error" as const) : ("done" as const),
      rounds,
      ...(subagentId !== undefined ? { subagent_id: subagentId } : {}),
      ...(errorKind !== undefined ? { errorKind } : {}),
      ...(errorMessage !== undefined ? { errorMessage } : {}),
      ...(recoveryHint !== undefined ? { recoveryHint } : {}),
      // Keep the historical single-string `error` field in step so the
      // plain-message render path never goes blank on a typed failure
      // (mirrors the backend accumulator, which does the same for reloads).
  ...
  if (mutated) patchSubAgentBlocks(msgIdx, [...tab.messages], next, {}, ctx);
  // SubAgentRail index cache — mark the rail chip's status so a greyed
  // (closed-tab) chip visually reflects the terminal state without waiting for
  // a snapshot re-fetch. A typed failure marks the chip `error`, not `done` —
  // otherwise the rail would claim success for a failed run. No-op when the
  // payload carries no `subagent_id` (legacy backend without persistence — the
  // chip stays at its last-known status; harmless).
  if (subagentId !== undefined) {
    ctx.updateSubAgentIndexStatus(
      subagentId,
      errorKind !== undefined ? "error" : "done",
    );
  }
};
...
const handleSubagentError: FrameHandler = (tab, frame, ctx) => {
  ...
  const errSubagentId =
    typeof errSubagentIdRaw === "string" && errSubagentIdRaw !== ""
      ? errSubagentIdRaw
      : undefined;
  // N.35/N.36 — retire the id from the live index before the block-patch
  // early-returns, for the same "never strand a live id" reason as
  // `handleSubagentDone`.
  if (errSubagentId !== undefined) {
    tab.activeSubAgentIds.delete(errSubagentId);
  }
  const activeId = tab.activeSubAgentMessageId;
  if (activeId === null) return;
  const msgIdx = tab.messages.findIndex((m) => m.id === activeId);
  ...
  // A sub-agent that died on an exhausted quota must not wait for the parent
  // turn to finish before the user is told. Left to the terminal END frame
  // alone, the parent would first burn another round reasoning about (or
  // retrying past) a `[sub-agent error: …]` string it cannot act on — and the
  // red block shows only the raw upstream text, with no hint that a quota
  // class can be switched.
  //
  // Sub-agents share the parent's routing (and, unless a profile pins another
  // model, its model), so the exhausted bucket is the one the gauge is already
  // showing; signalling here just brings the same check forward.
  if (IS_INTERNAL && looksLikeQuotaExhaustion(message)) {
    ctx.patchTab({ qgenieQuotaCheckSignal: Date.now() });
  }
};
...
const handleSpeakerChanged: FrameHandler = (tab, frame, ctx) => {
  ...
  // Coordinator (主持人) marker: the unified moderator+judge floor renders a
  // fixed host identity + badge (NOT the borrowed roster role). Absent ⇒ false
  // (ordinary speaker bubbles unchanged).
  const isCoordinator = payload.is_coordinator === true;
  ...
  const patch: Partial<ChatTab> = {
    ...
    streamingIsCoordinator: isCoordinator,
  };
      ...
      ...(tab.streamingSenderId !== null
        ? {
            ...
            ...(tab.streamingSenderModelId !== null
              ...
              : {}),
            // Carry the PRIOR speaker's coordinator marker forward so the
            // committed bubble renders the fixed host identity, not the role.
            ...(tab.streamingIsCoordinator ? { isCoordinator: true } : {}),
          }
        : {}),
      meta: { streaming: true },
...
/**
 * M6 aside_message frame handler.  Renders a small system-note card in the
 * conversation flow (NOT a user bubble): background-process completion
 * notifications and similar producer-owned asides.
 *
 * ``role`` MUST NOT be ``"user"``: the message row keys its whole chrome off
 * ``msg.role === 'user'`` (right-aligned lane, user avatar, "HH:MM 你" header),
 * so a user-roled aside is painted as something the user typed — it is not.
 * ``"system"`` puts it in the non-user lane and ``meta.aside`` still selects
 * the muted ``msg-aside`` treatment.
 */
const handleAsideMessage: FrameHandler = (tab, frame, ctx) => {
  const payload = asPayload(frame);
  if (payload === null || typeof payload["text"] !== "string") return;
  const text = (payload["text"] as string).trim();
  if (text === "") return;
  const kind =
    typeof payload["kind"] === "string" ? (payload["kind"] as string) : "system_note";
  const asideMsg: ChatMessage = {
    id: ctx.nextMessageId(),
    role: "system",
    content: text,
    createdAt: Date.now(),
    ...(tab.conversationId !== null
      ? { conversationId: tab.conversationId }
      : {}),
    meta: { aside: true, asideKind: kind },
  };
  ctx.patchTab({ messages: [...tab.messages, asideMsg] });
};

/**
 * Handle an `inject_as_new_turn` frame (V2 enhancement).  Emitted at the
 * tail of a turn's normal-completion path when the injection registry
 * still held pending Ctrl+Enter texts the run loop never drained (typical
 * cases: user pressed Ctrl+Enter after the last tool round, or during a
 * turn that never called any tool so the inter-round seam never fired).
 *
 * Behaviour, byte-identical to the user pressing Enter with the same
 * text:
 *   1. Drop every still-pending injection bubble (`meta.injected` +
 *      `meta.pending`) from the tab's message list — they never made it
 *      onto the wire and their optimistic content is about to reappear
 *      as a real user bubble via the standard submit path.
 *   2. Merge the FIFO payload texts with a blank-line separator (multi-
 *      inject sessions keep the user's ordering) and stash it on the tab
 *      as `pendingDeferredInject`.
 *   3. A `ChatView` watcher observes `pendingDeferredInject` and, once
 *      the tab settles to `idle` (after the current turn's END frame is
 *      applied), calls `useChatTurnSubmit.submitToTab` to resubmit
 *      through the same path pressing Enter would take.  The watcher
 *      clears the field right after handing the text off.
 *
 * Image parity: images ride inside each text as `![](url)` markdown; the
 * merged text preserves them unchanged so the standard submit path
 * resolves the refs to vision blocks again (no re-upload).
 */
const handleInjectAsNewTurn: FrameHandler = (tab, frame, ctx) => {
  const payload = asPayload(frame);
  if (payload === null) return;
  ...
  // Step 1: drop pending injection bubbles — they never reached the model
  // on this turn, and the merged text is about to reappear as a real user
  // bubble via the standard submit path.  Committed injection bubbles
  // (already reconciled by `handleInjectedMessage`) carry `meta.injected`
  // WITHOUT `meta.pending`, so this filter leaves them untouched.
  const nextMessages = tab.messages.filter((m) => {
  ...
  // Step 2: hand the merged text to the ChatView re-submit watcher.
  const merged = texts.join("\n\n");
  ctx.patchTab({
    messages: nextMessages,
    pendingDeferredInject: merged,
  });
};
...
export const FRAME_HANDLERS: Readonly<Record<string, FrameHandler>> = {
  ...
  aside_message: handleAsideMessage,
  quota_usage: handleQuotaUsage,
  inject_as_new_turn: handleInjectAsNewTurn,
};
