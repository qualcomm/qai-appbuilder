    ...
    ...(m.subAgentBlocks !== undefined
      ? {
          subAgentBlocks: m.subAgentBlocks.map((b) => ({
            ...
            //     /`background` → `error` (V1 parity — the interrupt icon,
            //     `error: "interrupted"` when absent).  `background` (N.18) is
            //     included because the owner-tab abort cascades to detached
            //     children too; leaving it live would commit a block that spins
            //     forever with no frame left to settle it.
            ...
            //     stuck "stopping…"). `running` / `background` blocks on a clean
            //     finish are left as-is (their own terminal frame settles them;
            //     legacy V1 parity preserved, and a detached child legitimately
            //     outlives the parent turn).
            ...(terminal === "error" &&
            (b.status === "running" ||
              b.status === "background" ||
              b.status === "aborting")
...
function assembleSettled(
  ...
  //
  //    Id selection: on a CLEAN `done`, prefer the backend-supplied persisted
  //    id (`tab.streamingAssistantMessageId`, from the terminal end frame) so
  //    the committed bubble matches the DB row — this is what lets per-message
  //    fork (`up_to_message_id`) work on a just-finished turn without a reload.
  //    On abort/error (`interrupted`) the turn did not cleanly persist a final
  //    assistant message, so keep the client-local id. Falls back to the local
  //    id when the backend supplied none (legacy).
  const trailingId =
    !interrupted && tab.streamingAssistantMessageId
      ? tab.streamingAssistantMessageId
      : newMessageId;
    ...
    ...(tab.streamingSenderId !== null
      ? {
          ...
          ...(tab.streamingSenderModelId !== null
            ...
            : {}),
          // Coordinator (主持人): the unified moderator+judge trailing bubble
          // renders the fixed host identity, not a borrowed role name.
