...
export const useImplementationStore = defineStore("implementation", () => {
  ...
  /** POST to trigger LLM-based plan generation from the discussion conclusion.
   *  Returns the freshly generated plan. 201 on success, 409 if plan already exists. */
  async function generateImplementationPlan(
    conversationId: string,
  ): Promise<ImplementationPlanWire> {
    return await apiJson<ImplementationPlanWire>(
      "POST",
      `/api/chat/conversations/${encodeURIComponent(conversationId)}/implementation/generate`,
    );
  }
