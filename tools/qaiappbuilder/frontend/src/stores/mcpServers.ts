...
import {
  addMcpServer,
  addRegistrySource as apiAddRegistrySource,
  browseRegistry as apiBrowseRegistry,
  installFromCatalog,
  listCatalog,
  listMcpServers,
  listRegistrySources as apiListRegistrySources,
  pollSourceOAuthToken,
  refreshCatalog as apiRefreshCatalog,
  removeMcpServer,
  removeRegistrySource as apiRemoveRegistrySource,
...
export const useMcpServersStore = defineStore("mcpServers", () => {
  ...
  const registrySources = ref<McpRegistrySource[]>([]);
  const sourcesLoading = ref(false);

  // ── Registry source OAuth state ─────────────────────────────────────────
  // sourceId of the source currently waiting for OAuth (empty when idle).
  const sourceOAuthPending = ref<string>("");
  // Human-readable status shown while waiting for the user to authenticate.
  ...
  async function browseRegistry(opts?: {
    ...
    try {
      ...
      const regErr = res.registry_error ?? "";
      // Auto-trigger OAuth for registry sources that return 401.
      if (regErr.includes("http_status_401") && opts?.sourceId && opts.sourceId.startsWith("custom:")) {
        await _triggerSourceOAuth(opts.sourceId, opts);
        return;
      }
      if (reset) {
      ...
      marketRegistryError.value = regErr;
    } catch (e) {
  ...
  // ── Registry source OAuth helper ────────────────────────────────────────

  async function _triggerSourceOAuth(
    sourceId: string,
    browseOpts?: { search?: string; reset?: boolean; sourceId?: string },
  ): Promise<void> {
    if (sourceOAuthPending.value === sourceId) return; // already in progress
    sourceOAuthPending.value = sourceId;
    sourceOAuthStatus.value = "opening_browser";
    marketRegistryError.value = "";
    marketLoading.value = false;
    try {
      const { session_id } = await startSourceOAuth(sourceId);
      sourceOAuthStatus.value = "waiting_for_user";
      // Poll every 2 s for up to 5 min.
      const POLL_INTERVAL = 2000;
      const MAX_ATTEMPTS = 150;
      for (let i = 0; i < MAX_ATTEMPTS; i++) {
        await new Promise((r) => setTimeout(r, POLL_INTERVAL));
        const poll = await pollSourceOAuthToken(sourceId, session_id);
        if (poll.status === "done") {
          sourceOAuthStatus.value = "authenticated";
          sourceOAuthPending.value = "";
          // Re-run the browse that originally triggered OAuth.
          await browseRegistry({ ...browseOpts, reset: true });
          return;
        }
        if (poll.status === "error") {
          sourceOAuthStatus.value = "";
          sourceOAuthPending.value = "";
          marketRegistryError.value = `OAuth failed: ${poll.error ?? "unknown"}`;
          return;
        }
      }
      // Timed out.
      sourceOAuthStatus.value = "";
      sourceOAuthPending.value = "";
      marketRegistryError.value = "OAuth timed out — please try again";
    } catch (e) {
      sourceOAuthStatus.value = "";
      sourceOAuthPending.value = "";
      marketRegistryError.value = `OAuth error: ${e instanceof Error ? e.message : String(e)}`;
      marketLoading.value = false;
    }
  }

  // ── Registry source management methods ─────────────────────────────────

  async function loadRegistrySources(): Promise<void> {
    sourcesLoading.value = true;
    try {
      registrySources.value = await apiListRegistrySources();
    } catch {
  ...
  async function addRegistrySource(url: string, name?: string, token?: string, cookie?: string): Promise<McpRegistrySource> {
    const source = await apiAddRegistrySource(url, name, token, cookie);
    // If the source already exists in the list (preset/builtin credential update),
    // replace in-place; otherwise append as a new entry.
    const existing = registrySources.value.findIndex((s) => s.id === source.id);
    if (existing >= 0) {
      const updated = [...registrySources.value];
      updated[existing] = source;
      registrySources.value = updated;
    } else {
      registrySources.value = [...registrySources.value, source];
    }
    return source;
  }

  async function removeRegistrySource(sourceId: string): Promise<void> {
    await apiRemoveRegistrySource(sourceId);
    registrySources.value = registrySources.value.filter((s) => s.id !== sourceId);
  }
