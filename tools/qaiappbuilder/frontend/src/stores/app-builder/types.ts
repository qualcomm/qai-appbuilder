...
export type AppEntry = components["schemas"]["AppEntry"] & {
  /** Whether all bundled models are installed on disk. */
  models_ready?: boolean;
  /** Missing model descriptors (only when models_ready is false). */
  models_missing?: Array<Record<string, unknown>> | null;
  /** Whether this app is a builtin (read-only, cannot be deleted). */
  builtin?: boolean;
};
...
export interface AppRunState {
  ...
  error?: string | null;
  /**
   * Human-readable error message from the last failed run (bug #3): the
   * ``ApiError.message`` body of a 409 / 422 rejection, kept alongside the
   * localizable ``error`` code so the log panel can render the concrete
   * "port X is held by Y (pid Z)" / "model Q not installed at …" detail
   * inline instead of the "no logs" placeholder. ``null`` when the last
   * op succeeded or the failure carried no message.
   */
  errorMessage?: string | null;
  /**
   * Structured ``ApiError.details`` from the last failed run — kept so the
   * log panel can render richer affordances than the flat ``errorMessage``
   * string. In particular, ``app_builder.model_not_installed`` (P6) carries
   * ``details.missing[]`` with per-model ``provisioning`` +
   * ``setup_guide_url`` fields, letting the frontend render "See setup
   * guide: <path>" for user-provided packs (Real-ESRGAN) instead of the
   * generic "install from the model panel" hint. ``null`` when the last op
   * succeeded or the failure carried no ``details``.
   */
