...
/** One weight-like file the backend's search saw but did not accept.
 *  Wire mirror of `WeightSearchRejectedPayload` (`_dto.py:512`). */
export interface WeightSearchRejectedDTO {
  readonly path: string;
  readonly size_bytes: number;
 ...
 *
 *  Wire mirror of `WeightSearchDiagnosticsPayload` (`_dto.py:526`). Emitted by
 *  `POST /import/scan-bins` when `results` is empty AND a `model_workdir` was
 *  supplied, and by a 409 `POST /import/auto-export` under
 *  `body.details.diagnostics`. Coexists with `needs_normalize` — the two answer
 *  different questions and are NOT mutually exclusive.
 *
 *  `message` is the backend's pre-rendered `WeightSearchReport.to_message()`
 *  block (which directories were walked, what was rejected and why, the
 *  suggested next step), so the UI can display it verbatim or splice it into
 *  the "ask the agent to find the weights" prompt without re-deriving anything.
 *  Every field is required on the wire (the backend defaults them), but the
 *  optional markers below keep an older/partial payload from breaking the UI. */
export interface WeightSearchDiagnosticsDTO {
  /** Directory the search rooted at (normally `<workdir>/output`). */
  readonly root: string;
  readonly root_exists: boolean;
  readonly searched_recursively?: boolean;
  /** Subdirectories seen under `root` (relative names). */
  readonly subdirs?: string[];
  readonly rejected?: WeightSearchRejectedDTO[];
  /** Non-weight extensions encountered (`.onnx`, `.json`, …). */
  readonly other_extensions?: string[];
  readonly found_precisions?: string[];
  readonly requested_precision?: string | null;
  /** Pre-rendered, human-readable diagnostic block. */
...
export interface BinScanResponseDTO {
   ...
   *  `<model_workdir>/output/` explaining what blocked the search. `null` when
   *  the scan found variants. Independent of `needs_normalize`. */
  readonly diagnostics?: WeightSearchDiagnosticsDTO | null;
}
...
export interface AutoExportResponseDTO {
  ...
  /** Optional (tail-appended): weight-search evidence when the export could
   *  not find a usable NPU weight. On the 409 failure path the same shape also
   *  arrives under `ApiError` `body.details.diagnostics`. */
  readonly diagnostics?: WeightSearchDiagnosticsDTO | null;
}
