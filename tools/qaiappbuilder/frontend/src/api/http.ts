...
// ---------------------------------------------------------------------------
// Auth readiness gate — suppresses the 401 flood on initial page load.
// ---------------------------------------------------------------------------
// Business API requests (apiJson, apiRaw, apiBlob, apiUpload) wait for this
// gate before firing. Without it, the SPA dispatches dozens of requests the
// instant it mounts (conversations, models, config…), all of which 401 before
// the user has a chance to sign in — flooding the console with errors and
// wasting round-trips.
//
// The gate resolves once:
//   1. Auth is disabled (all requests pass through), OR
//   2. The user is authenticated (cookie valid), OR
//   3. The iframe login completes and auth.refresh() succeeds.
//
// Registered from App.vue after the initial auth.refresh() check.
// Requests to /api/auth/* and /auth/* bypass the gate (bootstrap paths).

let _authGateResolve: (() => void) | null = null;
let _authGate: Promise<void> | null = new Promise<void>((resolve) => {
  _authGateResolve = resolve;
});

/** Release the auth gate — called from App.vue once auth state is known. */
export function releaseAuthGate(): void {
  if (_authGateResolve) {
    _authGateResolve();
...
/**
 * Re-arm the auth gate (called on logout so subsequent business requests
 * wait until re-login completes before firing).
 */
export function rearmAuthGate(): void {
  if (_authGate !== null) return; // Already armed.
  _authGate = new Promise<void>((resolve) => {
    _authGateResolve = resolve;
  });
}

/** Internal: wait for the gate unless the path is an auth bootstrap route. */
async function waitForAuth(path: string): Promise<void> {
  if (_authGate === null) return;
  // Auth bootstrap paths bypass the gate.
