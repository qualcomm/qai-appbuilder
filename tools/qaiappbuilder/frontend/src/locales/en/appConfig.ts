...
const appConfig = {
  ...
  sslVerifyDesc: "When on, all outbound HTTPS connections (model service, web_fetch, MCP, sign-in) verify the server's TLS certificate. Turn <b>off</b> if your model service uses a self-signed or corporate-gateway certificate. Applies immediately to the web_fetch tool; a restart is required for it to take full effect on the model-service connection.",
  ...
  bashAutoBackgroundEnabledLabel: "Auto-background long commands on a timer",
  bashAutoBackgroundThresholdLabel: "Threshold (ms)",
  bashAutoBackgroundThresholdDesc: "Off by default. When on, a long-running command (build / pytest / model conversion) is quietly handed to the background-process manager once it has run this long, even if nobody is waiting on it. Leaving it off does not make anything block: sending a new message still moves the running command to the background immediately, and Stop still cancels it. Range 1000-600000 ms.",
  maxSubagentRecursionDepthLabel: "Max sub-agent recursion depth",
  maxSubagentRecursionDepthDesc: "1 = only the main agent may spawn sub-agents; 2 (default) = one nested level; higher may cause resource exhaustion.",
  qgenieQuotaWarnPercentLabel: "QGenie quota warning threshold (%)",
  qgenieQuotaWarnPercentDesc: "QGenie meters each model into two independent daily allowances (API/SDK and IDE/CLI). When the one in use reaches this share, you are offered a switch to the other. 100 = only warn once exhausted. Applies to QGenie models only.",
  debugTitle: "Debug",
  ...
  maxRoundsDesc: "Optional safety cap on tool-call rounds per turn. 0 = unlimited (recommended). The no-progress circuit breaker already prevents infinite loops.",
  maxRoundsLabel: "Max Rounds (optional cap)",
  turnWarningLabel: "Turn Warning",
  turnWarningDesc: "Show a reminder when the conversation gets long, suggesting to start a new session.",
  turnWarningStartLabel: "Warning Start (turns)",
  turnWarningStartDesc: "First warning appears after this many user messages. Default: 30.",
  turnWarningStepLabel: "Warning Interval (turns)",
  turnWarningStepDesc: "Subsequent warnings appear every N turns after the start. Default: 10.",
  orderLabel: "Order:",
