...
const appConfig = {
  ...
  sslVerifyDesc: "開啟後，所有出站 HTTPS 連線（模型服務、web_fetch、MCP、登入）都會校驗伺服器的 TLS 憑證。若你的模型服務使用自簽名憑證或企業閘道憑證，請<b>關閉</b>此項。修改對 web_fetch 工具立即生效；對模型服務連線需重啟後完全生效。",
  ...
  bashAutoBackgroundEnabledLabel: "依時長自動把長命令轉背景",
  bashAutoBackgroundThresholdLabel: "閾值（毫秒）",
  bashAutoBackgroundThresholdDesc: "預設關閉。開啟後，長時間執行的命令（build / pytest / 模型轉換）只要執行滿這麼久，就會被靜默交給背景行程管理員 —— 即使沒有人在等它。保持關閉不會讓任何東西卡住：你傳送新訊息時，正在執行的命令仍會立刻轉入背景，「停止」也照常可以取消它。範圍 1000-600000 毫秒。",
  maxSubagentRecursionDepthLabel: "子 Agent 最大遞歸深度",
  maxSubagentRecursionDepthDesc: "1 = 只有主 Agent 可以派子 Agent；2（預設）= 允許一層巢狀；更高值可能導致資源耗盡。",
  qgenieQuotaWarnPercentLabel: "QGenie 配額提醒閾值（%）",
  qgenieQuotaWarnPercentDesc: "QGenie 為每個模型提供兩套獨立的當日配額（API/SDK 與 IDE/CLI）。目前使用的一側達到此比例時，會詢問你是否切換到另一側。100 = 僅在用盡時提醒。僅對 QGenie 模型生效。",
  debugTitle: "除錯",
  ...
  maxRoundsDesc: "每輪對話工具呼叫輪次的可選安全上限。0 = 無限制（推薦）。無進展斷路器已能防止死迴圈。",
  maxRoundsLabel: "最大輪次（可選上限）",
  turnWarningLabel: "對話輪次提醒",
  turnWarningDesc: "當對話變長時顯示提醒，建議開啟新對話。",
  turnWarningStartLabel: "首次提醒閾值（輪）",
  turnWarningStartDesc: "使用者訊息數達到此值時首次提醒。預設：30。",
  turnWarningStepLabel: "提醒間隔（輪）",
  turnWarningStepDesc: "首次提醒後，每隔 N 輪再次提醒。預設：10。",
  orderLabel: "順序：",
