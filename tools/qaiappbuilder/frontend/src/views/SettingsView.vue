...
const SearchSettingsPanel = defineAsyncComponent(
  () => import("@/components/chat/SearchSettingsPanel.vue"),
);
...
const ScheduledTasksPanel = defineAsyncComponent(
  () => import("@/components/chat/ScheduledTasksPanel.vue"),
);
...
type SettingsTab = "app" | "cloud-models" | "coding-modes" | "ai-coding" | "agent" | "search" | "hooks" | "mcp" | "scheduled-tasks";
...
/** localStorage key remembering the last Settings tab the user was on.
 *
 *  Returning to Settings after visiting another view (chat, downloads…) used
 *  to always land on "app": the tab lived in a `ref` that died with the
 *  component, so anyone working in e.g. Scheduled Tasks had to re-select it on
 *  every round trip. Same persistence pattern as the sidebar / theme prefs in
 *  `stores/ui.ts`.
 */
const LAST_TAB_KEY = "qai_settings_last_tab";

function _readLastTab(): SettingsTab | null {
  try {
    const stored = localStorage.getItem(LAST_TAB_KEY);
    if (stored !== null && _SETTINGS_TABS.has(stored as SettingsTab)) {
      return stored as SettingsTab;
    }
  } catch {
    // localStorage unavailable (private mode / embedded webview) — ignore.
  }
  return null;
}

function _initialTab(): SettingsTab {
  // An explicit `?tab=` deep-link always wins: it is the caller stating intent
  // (e.g. "Edit prompts…" jumping to coding-modes), which must not be
  // overridden by where the user happened to be last.
  const q = route.query.tab;
  if (typeof q === "string" && _SETTINGS_TABS.has(q as SettingsTab)) {
    return q as SettingsTab;
  }
  return _readLastTab() ?? "app";
}
...
watch(
  ...
  (tab) => {
    ...
    // Remember the tab so re-entering Settings resumes here instead of
    // resetting to "app".
    try {
      localStorage.setItem(LAST_TAB_KEY, tab);
    } catch {
      // localStorage unavailable — the memory is a convenience, never a
      // prerequisite for the panel working.
    }
  },
  { immediate: true },
);
...
const settingsTabs = computed(() => [
  ...
  { id: "scheduled-tasks", label: "⏰ " + t("settings.tab.scheduledTasks") },
]);
...
const subtitle = computed(() => {
  ...
  if (activeTab.value === "search") return t("settings.subtitle.search");
  if (activeTab.value === "hooks") return t("settings.subtitle.hooks");
  if (activeTab.value === "mcp") return t("settings.subtitle.mcp");
  if (activeTab.value === "scheduled-tasks") return t("settings.subtitle.scheduledTasks");
...
const showRefresh = computed(
  () =>
    ...
    // Search reloads its own engine roster on mount and after every mutation.
    activeTab.value !== "search" &&
    activeTab.value !== "hooks" &&
    activeTab.value !== "mcp" &&
    activeTab.value !== "scheduled-tasks",
);
...
<template>
  ...
  >
    ...
    <div class="config-tab-content">
      ...
      <!-- Search tab — engine roster: routing, per-engine on/off, API keys -->
      <SearchSettingsPanel
        v-if="visited.has('search')"
      ...
      <!-- Scheduled Tasks tab — WebUI manager: list / edit content +
           tool & skill permission whitelists, pause / resume / run / remove. -->
      <ScheduledTasksPanel
        v-if="visited.has('scheduled-tasks')"
        v-show="activeTab === 'scheduled-tasks'"
      />
