...
const GrantsPanel = defineAsyncComponent(
  () => import("@/components/security/GrantsPanel.vue"),
);
...
const tabs = computed<TabDef[]>(() => [
  ...
  {
    // Authorization tab — pending count badge now lives here (this is
    // the "to-do queue" surface); Audit stays clean as the observation
    // log.
    id: "authorize",
    label: t("security.tabAuthorize"),
...
<template>
  ...
  >
    ...
    <!-- Authorization (standing grants + pending requests + dep queue). -->
    <div v-else-if="activeTab === 'authorize'">
      <GrantsPanel />
    </div>

    <!-- Skill Capabilities (read-only visibility into loaded skills +
         per-skill mode switch; editing is done via on-disk
         ``skill.policy.json`` — see ``SkillCard.vue`` docstring). -->
