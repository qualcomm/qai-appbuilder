 ...
 *     `exec_paths` / `trusted_binaries` — exact V1 field parity).
 *
 *     NOT every discovered skill is active: the endpoint aggregates the
 *     registry snapshot (`active: true`) PLUS filesystem-scanned skills and
 *     operator-authored per-skill overrides, which surface with
 *     `active: false` when no `skill.policy.json` was loaded for them. That
 *     is exactly what drives the "含未激活" badge below
 *     (`hasInactiveRead` / `hasInactiveWrite`), so the flag must be carried
 *     through, never assumed.
