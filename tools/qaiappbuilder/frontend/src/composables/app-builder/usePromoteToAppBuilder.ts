...
import {
  ...
  type WeightSearchDiagnosticsDTO,
...
/**
 * Test-only: clear the module-level "generated this session" registry.
 *
 * `_generatedWorkdirs` lives at module scope so the "generated" fact survives
 * closing/reopening the panel and composable rebuilds (see the block comment
 * above it). That same persistence makes it leak ACROSS unit tests in the same
 * module: a workdir a prior test generated stays marked, so a later test
 * reusing that path would false-positive `showCommitCard`. Tests must call
 * this in `beforeEach` for deterministic isolation. Not part of the public
 * composable API — never called by production code.
 */
export function __resetGeneratedWorkdirsForTests(): void {
  _generatedWorkdirs.clear();
  _generatedBump.value++;
}
...
export interface UsePromoteToAppBuilder {
  ...
  /** Backend weight-search evidence for the EMPTY case: which directory was
   *  walked, which subdirs / weight-like files were seen, why each was
   *  rejected, plus a pre-rendered `message` block. Set only when the scan
   *  returned no variants (the backend emits it only then); `null` otherwise.
   *  Independent of `needsNormalize` — both may be set at once. Feeds the
   *  compact "clues" block and seeds the AI-search prompt. */
  readonly diagnostics: Ref<WeightSearchDiagnosticsDTO | null>;
  ...
  /** Inject a prompt into the CURRENT chat session asking the in-session agent
   *  to check this workspace's model weights and record the result in
   *  `inference_manifest.json`, so the user can just click Import again.
   *
   *  Backs the card's "AI search" affordance, which is reachable regardless of
   *  the scan outcome (a scan that FOUND variants can still have found the
   *  wrong ones). `opts.verify` picks the review-toned prompt for that case —
   *  the default wording opens with "could not find a usable weight", which
   *  would contradict a variant list already on screen. Returns the prompt
   *  that was enqueued, or `null` when there is no workdir to search (nothing
   *  to ask about). */
  askAiToFindWeights: (opts?: { verify?: boolean }) => string | null;
  // ── formatting helpers ──
...
export function usePromoteToAppBuilder(
  ...
  onImported: () => void,
  // Backend-verified promote-ready signal (State-Truth-First, AGENTS §5).
  // Optional so existing/test callers keep working. Callers pass the count of
  // precision variants the BACKEND detected on disk for the active
  // conversation (``usePromoteReadyDetection().detectedVariants.length`` —
  // the same truth that lights the ready-dot). When it flips 0 → >0 (the
  // model just finished building and its ``output/*.bin`` landed), we force a
  // re-scan even though ``sessionModelWorkdir`` did NOT change: the backend
  // and the frontend fallback resolve the SAME ``C:\WoS_AI\<model>`` string,
  // so the workdir ``watch`` below never re-fires when the backend detection
  // arrives — leaving ``variants`` stuck at the empty result of an earlier
  // scan taken BEFORE the artifacts existed. This signal is the missing
  // truth-driven trigger that reconciles the card with what the backend
  // already verified on disk. Defaults to a constant 0 ref when omitted.
  verifiedVariantsCount?: Ref<number>,
): UsePromoteToAppBuilder {
  const { t } = useI18n();

  // Normalise the optional backend-verified signal to a real ref so the watch
  // below always has a source (a constant 0 ref means "never re-triggers",
  // preserving the prior behaviour for callers that don't pass it).
  const verifiedCount = verifiedVariantsCount ?? ref(0);
  ...
  // Weight-search evidence for the empty case (see the interface doc). Kept
  // beside `needsNormalize` because both are derived from the SAME scan
  // response and both are only meaningful when `variants` came back empty.
  const diagnostics = ref<WeightSearchDiagnosticsDTO | null>(null);
  ...
  // Monotonic request token guarding `scanCandidates` against the same
  // out-of-order / stale-overwrite race `fetchVariants` guards (this was the
  // ROOT of "生成完成后回到第一个界面"): during a Generate the backend is busy
  // running the heavy auto-export, so a candidate dry-run dispatched at mount
  // (or mid-generation) — which resolves EMPTY because `app_pack/` did not
  // exist yet — could land AFTER the completion re-scan and clobber the
  // freshly-generated candidate back to `[]`. That flips `hasCandidates` (and
  // thus `showCommitCard`) false, dropping the card back to the pick-precision
  // stage. Only the latest dispatch may write the candidate/dry-run state.
  let candidateScanToken = 0;

  // ── candidate branch actions ─────────────────────────────────────────────
  async function scanCandidates(): Promise<void> {
    if (!hasWorkdir.value) return;
    // Snapshot the workdir at dispatch so a late response is matched against
    // the workspace it was scanned for, not whatever `sessionModelWorkdir`
    // resolves to when it lands.
    const workdir = sessionModelWorkdir.value;
    const token = ++candidateScanToken;
    loading.value = true;
    error.value = "";
    ...
    try {
      const res = await importDryRun([workdir]);
      // Drop this response if a newer scan superseded it, or the user
      // navigated to a different workspace while it was in flight.
      if (token !== candidateScanToken || workdir !== sessionModelWorkdir.value)
        return;
      ...
      // A superseded/stale dispatch must not surface its error (or clear the
      // newer scan's good result) — only the latest owns the state.
      if (token !== candidateScanToken || workdir !== sessionModelWorkdir.value)
        return;
      ...
      if (token === candidateScanToken) loading.value = false;
    }
  }
  ...
  async function fetchVariants(): Promise<void> {
    ...
    try {
      ...
      // Same empty-case gating for the weight-search evidence: it explains why
      // the walk found nothing, so it is meaningless (and the backend sends
      // `null`) once variants exist. Not exclusive with `needs_normalize`.
      diagnostics.value = rows.length === 0 ? (res.diagnostics ?? null) : null;
  ...
  // ── backend-verified re-scan trigger (State-Truth-First, AGENTS §5) ───────
  // ROOT-CAUSE FIX for "the ready-dot lights up (backend found the model in
  // C:\WoS_AI\<model>\output) but the just-opened card says '未找到 / no
  // precision artifacts', and only closing+reopening the panel fixes it".
  //
  // Why the workdir ``watch`` above is NOT enough: ``sessionModelWorkdir``
  // resolves to the SAME ``C:\WoS_AI\<model>`` string via the message-scan
  // fallback WHILE the model is still building (the SKILL summary prints the
  // path before ``output/*.bin`` lands), so an early ``fetchVariants`` scans
  // an empty ``output/`` → ``variants=[]``. When the build finishes the backend
  // detects the variants and the frontend fills ``detectedWorkdir`` with the
  // byte-for-byte SAME path (backend ``_extract_model_workdir_from_text``
  // mirrors the frontend regex), so the workdir value never changes and the
  // ``watch`` never re-fires — ``variants`` stays stuck at the empty result.
  //
  // The backend's verified variant count IS the real on-disk truth. When it
  // flips 0 → >0 (or otherwise increases) we re-scan the workspace so the card
  // reflects what the backend already confirmed, WITHOUT depending on the
  // workdir string changing. Covers both the ``v-show`` (always-mounted) and
  // "panel left open during the build" cases the reopen-workaround can't.
  watch(verifiedCount, (count, prev) => {
  ...
  // Example `inference_manifest.json` body handed to the agent. Built HERE
  // rather than inside the i18n message because vue-i18n's message compiler
  // treats `{` as the start of an interpolation, so a literal JSON object
  // cannot appear in a locale string. It is a code literal (no prose), so
  // keeping it out of the locale files costs no translatability.
  const MANIFEST_EXAMPLE =
    '{"precision": "<token>", "context_binary": "output/<path relative to the workspace>"}';

  /**
   * Hand the empty-scan problem to the agent already running in THIS chat
   * session instead of leaving the user at a dead end.
   *
   * Mechanism mirrors the production-verified
   * `useAppBuilderChatBridge.sendToChat()` (`:368-442`): compose the text,
   * push it through the `usePendingChatSubmit` intent queue, then collapse the
   * workbench so the user sees the reply stream. The queue is the ONLY
   * sanctioned injection path — `ChatComposer` drains it into its own `text`
   * ref and runs the same submit a user-typed message takes, so the LLM
   * actually answers. Talking to the transport (or `appendMessage`, which only
   * adds a display row) directly would break chat ⇄ app_builder context
   * isolation (AGENTS §3.2) and revive a fixed V2 regression.
   *
   * The prompt carries the backend's pre-rendered `diagnostics.message`
   * verbatim, so the agent starts from the evidence (walked dirs, rejected
   * files + reasons, suggested next step) instead of re-deriving it — and the
   * hard bounds (no recursive `C:\` / `C:\WoS_AI` walk, depth ≤ 3) are stated
   * in the prompt because they are the agent's constraints, not ours.
   */
  function askAiToFindWeights(opts?: { verify?: boolean }): string | null {
    const workdir = sessionModelWorkdir.value;
    // Nothing to search: without a workspace path the prompt would ask the
    // agent to walk an unbounded root — exactly the forbidden behaviour.
    if (workdir === "") return null;
    // Join with the separator the workdir itself uses so the prompt never shows
    // a mixed `C:\WoS_AI\model/output` path — the agent reads these as literal
    // paths to walk / write, and a mixed one reads like a typo.
    const sep = workdir.includes("\\") ? "\\" : "/";
    const join = (leaf: string): string =>
      workdir.endsWith(sep) ? `${workdir}${leaf}` : `${workdir}${sep}${leaf}`;
    const evidence = diagnostics.value?.message?.trim() ?? "";
    // Review mode quotes the list currently ON SCREEN so the agent can compare
    // it against the disk — that comparison IS the ask, and it is the exact
    // check the buggy scan (one 293 MB file reported as both FP16 and FP32)
    // failed. Labels, not raw tokens: the user is looking at the labels.
    const prompt = opts?.verify === true
      ? t("modelBuilder.promote.aiSearch.promptVerify", {
          workdir,
          outputDir: join("output"),
          manifestPath: join("inference_manifest.json"),
          precisions:
            variants.value.length > 0
              ? variants.value.map((v) => v.label).join(", ")
              : t("modelBuilder.promote.aiSearch.noPrecisions"),
          manifestExample: MANIFEST_EXAMPLE,
        })
      : t("modelBuilder.promote.aiSearch.prompt", {
          workdir,
          outputDir: join("output"),
          manifestPath: join("inference_manifest.json"),
          diagnostics:
            evidence !== ""
              ? evidence
              : t("modelBuilder.promote.aiSearch.noDiagnostics"),
          manifestExample: MANIFEST_EXAMPLE,
        });
    enqueueChatSubmit(prompt);
    // Collapse the workbench so the injected turn + the agent's reply are
    // visible (same final step as `sendToChat`).
