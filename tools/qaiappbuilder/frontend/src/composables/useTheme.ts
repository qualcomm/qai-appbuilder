...
function applyDocumentTheme(theme: ResolvedTheme): void {
  ...
  // element-plus uses the OPPOSITE convention (dark tokens gated on
  // ``html.dark``). Kept in lockstep with our ``.light`` class so third-party
  // widgets that read element's tokens (ElDatePicker etc.) follow the app
  // theme automatically — no per-widget dark prop plumbing.
