 ...
 *
 * Keyless (SSO) providers are exempt from the missing-key detection. The QAI
 * Service token pool authenticates with a short-lived per-user JWT minted from
 * the Okta login, so it reports `has_api_key === false` forever and there is
 * no key a user could supply. The backend marks it `auth_mode: "sso"` and
 * `isKeylessProvider()` filters it out — without that filter the composer's
 * pre-send interception blocked every token-pool turn behind a key dialog
...
/**
 * Whether `meta` describes a provider that authenticates WITHOUT a stored key.
 *
 * Such a provider (`auth_mode === "sso"` — today the QAI Service token pool)
 * mints its credential at request time from the signed-in identity, so its
 * `has_api_key` is permanently false and no amount of user input can change
 * that. Every "needs a key" code path must skip it.
 *
 * An absent / unrecognised `auth_mode` returns false, so a backend that does
 * not send the field keeps the previous behaviour exactly.
 */
function isKeylessProvider(meta: CloudProviderMeta): boolean {
  return meta.auth_mode === "sso";
}
...
async function doFetch(): Promise<void> {
  ...
  try {
    ...
    //
    // Keyless providers are EXCLUDED. A provider whose `auth_mode` is `"sso"`
    // (the QAI Service token pool) authenticates with a short-lived per-user
    // JWT minted from the SSO login, so its `has_api_key` is permanently
    // false — there is no key for the user to enter. Collecting it here used
    // to make the composer's pre-send interception block every turn on a
    // token-pool model and open a key dialog that could never be satisfied,
    // so the request never even reached the backend (which would have
    // resolved the JWT correctly). Trust the backend's declared auth mode
    // rather than pattern-matching the provider id: the id is deployment data
    // from the factory seed and could be renamed.
    ...
    for (const [id, meta] of Object.entries(providers)) {
      ...
      if (hasModels && meta.has_api_key === false && !isKeylessProvider(meta)) {
        // Strip the read-only derived fields from the config we keep for the
        // PUT (they are not part of the writable config document).
        const cfg: Record<string, unknown> = { ...(meta as Record<string, unknown>) };
        delete cfg.has_api_key;
        delete cfg.auth_mode;
        missing.add(id);
        cfgById[id] = cfg;
