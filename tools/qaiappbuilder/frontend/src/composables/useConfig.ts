...
/**
 * `forge_config.chat.search` — Agent search-tool provider selection.
 * Written by the "Search Source" group in AgentSettingsPanel and read
 * per-turn by the backend search-config reader (no restart required).
 *   - mode:             which providers are eligible for the web_search tool.
 *   - default_provider: which provider the tool uses when the caller omits one
 *                       (only meaningful when mode === "both").
 */
export interface ChatSearchConfig {
  mode: "cebot_only" | "web_only" | "both";
  default_provider: "cebot" | "web" | "auto";
}
