 ...
 *     (V1 app.js:3227).
 *   - `inputPlaceholder` — placeholder text that reflects the streaming-here
 *     vs idle state (V1 index.html:1461).
...
export function useComposerSubmit(opts: {
  ...
  // In-flight guard for ONE keypress→send cycle (bug fixed 2026-08-10:
  // "我只发了一次，它生成了两条，而且还出错了").
  //
  // `onSubmit` awaits `uploadPendingImages()` BEFORE it emits and before it
  // clears `text`. `uploadPendingImages` is async even with no images attached,
  // so it always yields at least one microtask — and inside that window the
  // composer still holds the old text while the tab is still `idle`, so a
  // second Enter passes `canSubmit` and sends the SAME text again. The two
  // sends then race the backend: the first flips the tab to `streaming`, the
  // second hits `ConversationTab.start_stream` and dies with
  // `chat.tab_state_invalid: requires status=IDLE, got streaming` — one
  // keypress, two user bubbles, one of them a red error.
  //
  // A module-local ref (not tab state) because this guards a single composer's
  // keypress cycle, not anything persisted: the composer is per-active-tab and
  // the flag always clears in a `finally`, including on an upload failure.
  ...
  const canSubmit = computed(() => {
    ...
    // A send already in flight blocks the button + the Enter path: the tab is
    // still `idle` until the backend accepts the first one (see
    // `submitInFlight`).
    if (submitInFlight.value) return false;
  ...
  const showStop = computed(() => {
    ...
    // app.js:3227 isStreamingHere). Falls back to chat-tab activity
    // otherwise.  ``isActiveTabBusy`` covers ``streaming``, ``aborting``
    // AND ``idle + hasActiveBackgroundWork`` (a completed main turn
    // whose spawned sub-agents are still running), so the same Stop
    // affordance surfaces in ALL "the tab is still working"
    // situations — visually identical to a live streaming turn.
    if (claudeCode.isCCMode.value) return claudeCode.streaming.value;
    if (openCode.isOCMode.value) return openCode.streaming.value;
    return store.isActiveTabBusy;
  });
  ...
  // so we expose the two that apply: busy-here vs idle.
  //
  // The BUSY text differs by mode on purpose: in normal chat the two keys have
  // distinct, useful behaviours worth advertising (Enter queues for after the
  // run, Ctrl+Enter folds into the RUNNING turn), but CC/OC sessions support
  // NEITHER — `tryEnqueueWhileStreaming` and `canInject` both bail out for
  // them, so advertising the keys there would promise something that silently
  // does nothing.
  const inputPlaceholder = computed<string>(() => {
    if (!showStop.value) return t("input.placeholder");
    if (claudeCode.isCCMode.value || openCode.isOCMode.value) {
      return t("input.streamingPlaceholder");
    }
    return t("input.busyPlaceholder");
  });
  ...
  async function onSubmit(): Promise<void> {
    ...
    if (trimmed === "") return;
    // Claim the cycle BEFORE the first `await` so a second Enter arriving in
    // the upload window cannot pass `canSubmit` and send the same text twice.
    submitInFlight.value = true;
    try {
      // CC/OC mode (V1 叠加式 app.js:3250-3308): route the message to the
      // active coding session via the shared core instead of the normal chat
      // transport. Images are not forwarded to OC (V1 line 3298: OC send has
      // no image arg).
      if (claudeCode.isCCMode.value) {
        text.value = "";
      ...
      // Always release, including on an upload/transport throw — otherwise one
      // failed send would wedge the composer for the rest of the session.
  ...
  function onKeydown(ev: KeyboardEvent): void {
    // Ctrl+Enter (or Cmd+Enter on macOS) mirrors the down-arrow INJECT button:
    // while THIS tab streams, fold the input into the SAME run at the next
    // inter-round seam (`injectWhileStreaming`) instead of enqueuing a fresh
    // turn like a plain Enter does. `canInject` already gates on normal chat
    // mode + streaming/aborting + non-empty input, so when it is false (idle
    // tab, empty input, CC/OC) we fall through to the normal submit path — a
    // Ctrl+Enter on an idle tab just sends, never a silent no-op.
    if (
      ev.key === "Enter" &&
  ...
  async function tryEnqueueWhileStreaming(): Promise<boolean> {
    ...
    // Queue while this tab is BUSY — which is not the same as "streaming".
    // A main turn that spawned a sub-agent completes and goes ``idle`` while
    // the sub-agent keeps working; the tab still shows the typing indicator
    // and the Stop button (``isActiveTabBusy`` folds that in, and ``showStop``
    // above already reads it). Testing ``tab.status`` alone made Enter bypass
    // the queue in exactly that window and open a fresh turn instead — the
    // user sees a busy conversation yet their message jumps the line.
    if (!store.isActiveTabBusy) return false;
    // Same upload-window race as `onSubmit` (see `submitInFlight`): `text` is
    // only cleared AFTER the await, so a second Enter landing in that window
    // would queue the same text twice. Consume the keypress (`true`) rather
    // than falling through to `onSubmit`, which would send it as a fresh turn.
    if (submitInFlight.value) return true;
    submitInFlight.value = true;
    try {
      const trimmed = text.value.trim();
