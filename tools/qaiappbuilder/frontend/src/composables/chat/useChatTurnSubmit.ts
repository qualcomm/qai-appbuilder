...
export function useChatTurnSubmit(): {
  ...
  async function submitToTab(id: string, prompt: string): Promise<void> {
    ...
    // no provider at all.
    //
    // Keyless cloud providers (the QAI Service token pool, `auth_mode: "sso"`)
    // do NOT need an entry here: `useCloudModelStatus` never collects them as
    // "missing a key" in the first place, so `providerMissingKey` is false for
    // them. Filtering at that single source keeps every consumer of the flag
    // consistent instead of maintaining a second skip list here.
    const skipKeyCheck =
      modelId === "" ||
      modelId === "qai-default" ||
