...
import { attachActiveChatRun } from "./useActiveChatRunAttach";
  ...
  async function sendWs(prompt: string): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      client.open({
        ...
        onResumeNeeded: () => {
          // Data-plane WS dropped MID-TURN (network jitter / OS sleep / tab
          // suspend / idle-proxy cut). The backend did NOT tear the turn down:
          // it detached the turn to a background task still running to
          // completion and republishing every frame to the in-process
          // broadcaster (TTL 600s). Do NOT surface an error / resend — keep
          // the tab `streaming` and RE-ATTACH to the still-running turn,
          // continuing the stream where it left off (backfill replay → live).
          if (!inFlight) {
            // The turn already settled (terminal frame won the race with the
            // drop). Nothing to resume.
            return;
          }
          ws = null;
          // Reset the interrupted turn's rendered assistant output so the
          // `from_seq=0` backfill becomes the single source of truth — no
          // duplicate bubbles / doubled text. `applyFrame(backfill=true)` is
          // APPEND semantics (chatTabs.ts appendStreamingChunk:2120 /
          // appendRoundChunk:2207 / handleChunk round routing), and the
          // primary WS frames carry NO `sequence`, so we cannot resume at a
          // cursor and MUST replay the whole turn onto a clean slate.
          store.resetTurnForResume(tabId);
          attachActiveChatRun(
            tabId,
            `/api/chat/active-runs/${tabId}/ws`,
            {
              // The attach path already drove the store terminal
              // (`confirmDone`); reuse `handleDone` for the transport-side
              // post-turn refreshes (title / sidebar counts) + settleResolve.
              // `confirmDone` is idempotent (no-op once idle) so no double
              // commit.
              onDone: () => handleDone(),
              // The attach path already `recordError`'d the tab; settle the
              // in-flight promise as rejected so the caller's await unwinds.
              onError: (err) =>
                settleReject(
              ...
              // No active run to attach to (turn ended before the drop, or
              // broadcaster TTL expired) / re-attach connection lost before a
              // terminal. Degrade gracefully: surface a retryable network
              // error so the tab leaves `streaming` (never stuck) and the user
              // can resend — same channel the old mid-turn path used.
              onClosedWithoutTerminal: () => {
                if (!inFlight) {
                  return;
                }
                handleErrorEnvelope({
