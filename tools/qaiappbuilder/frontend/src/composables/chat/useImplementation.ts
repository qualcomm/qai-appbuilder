...
export interface UseImplementation {
  ...
  retry(): Promise<void>;
  /** Generate a plan from the discussion conclusion (LLM extractor). */
...
export function useImplementation(tabIdRef?: Ref<TabId | null>): UseImplementation {
  ...
  /** Send a message through the ordinary chat send path (composer/transport).
   *  This is how both execution-control triggers AND the "generate plan" preset
   *  reach the backend — exactly as if the user had typed the text. The backend
   *  recognizes the localized trigger text (control router) or the @coordinator
   *  mention (directed deep task) without any dedicated route. */
  ...
  async function start(): Promise<void> {
    await sendChatMessage(t("chat.implementation.controlMsg.start"));
  }
  async function pause(): Promise<void> {
    await sendChatMessage(t("chat.implementation.controlMsg.pause"));
  }
  async function resume(): Promise<void> {
    await sendChatMessage(t("chat.implementation.controlMsg.resume"));
  }
  async function stop(): Promise<void> {
    await sendChatMessage(t("chat.implementation.controlMsg.stop"));
  }
  async function retry(): Promise<void> {
    await sendChatMessage(t("chat.implementation.controlMsg.retry"));
  }

  /** Generate an implementation plan by SENDING a preset @coordinator message
   *  through the ordinary chat send path — equivalent to the user typing
   *  "@<host> 请把讨论整理成实施计划…". The coordinator (host), which has
   *  write/plan/read tools by default, streams a turn that writes the plan doc,
   *  calls the `implementation_plan` tool to register lightweight items, and
   *  gives a short in-chat summary. No dedicated /generate endpoint is hit. */
  async function generatePlan(): Promise<void> {
    const coordinator = t("chat.discussion.coordinator.displayName");
    const preset = t("chat.implementation.generatePlanMsg", { coordinator });
