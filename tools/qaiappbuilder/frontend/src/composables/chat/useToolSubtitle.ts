 ...
 *     hide the subtitle slot (e.g. `apply_patch`, unknown
 *     tools, missing args).
 *
...
export function toolMeta(toolName: string): ToolMeta {
  switch (toolName) {
    ...
    case "web_fetch":
      return { icon: "🌐", categoryKey: "chat.toolCategory.web_fetch" };
    case "web_search":
    ...
    case "sub_agent":
      return { icon: "🎛️", categoryKey: "chat.toolCategory.sub_agent" };
    case "skill":
...
export function subtitleFromToolCall(
  ...
  switch (toolName) {
    ...
    case "sub_agent": {
      // The primary distinguishing arg is ``action`` — ``list`` / ``status``
      // / ``inspect`` / ``wait`` / ``wait_any`` / ``stop``.  ``id`` (present
      // for status/inspect/wait/stop) narrows it further so the card tells
      // the user "which sub-agent" at a glance.
      const action = str("action");
      const id = str("id");
      if (action !== null && id !== null) return `${action} · ${id}`;
