...
// N.46 — the ONE backoff ladder shared by every WS consumer. This channel
// borrows the formula only (see `scheduleReconnect` for why it does not join
// the suspend/resume registry).
import { nextBackoffMs } from "@/composables/chat/useWebSocketOrchestrator";
...
export function createChatControlChannel(
  ...
  /**
   * Arm the next handshake retry, or settle into `closed` (REST keeps serving)
   * once the budget is spent.
   *
   * N.46 — the delay comes from `useWebSocketOrchestrator.nextBackoffMs`, the
   * ONE backoff implementation in the frontend, with this channel's policy
   * (`backoffBaseMs` base, doubling across the whole budget, no jitter). Delays
   * are byte-identical to the pre-N.46 `backoffBaseMs * 2 ** attempt`.
   *
   * This channel deliberately does NOT register suspend/resume hooks with the
   * orchestrator, and that is not an omission:
   *   * it is a page-global SINGLETON (one socket for every tab), so there is
   *     no per-tab connection growth for a page-level policy to contain;
   *   * REST is always the fallback (`sendAnswer` / `sendStop` return `false`
   *     and the caller POSTs instead), so a non-ready channel never wedges a
   *     user action — suspending it would change nothing the user can perceive;
   *   * its retry budget is reset by the next `ensureOpen()` (i.e. by the next
   *     user action), so an outage that exhausts the budget is self-healing
   *     without a resume edge. The budget-burn problem that makes suspension
   *     load-bearing for the data-plane client does not exist here.
   * What WOULD have been a duplicate — a second backoff ladder — is gone: the
   * formula above is the shared one.
   */
  function scheduleReconnect(): void {
    ...
    const delay = nextBackoffMs(attempt, {
      ...
      // Uncapped doubling — bounded by ATTEMPT COUNT (`maxReconnect`), so a
      // delay ceiling is unreachable here.
