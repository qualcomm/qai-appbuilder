...
import { silentLogin, popupLogin } from "@/api/auth";
...
export function useChatErrorActions(): UseChatErrorActions {
  ...
  /**
   * `sign_in_again` — re-mint the SSO-derived per-user token, then re-run.
   *
   * For a keyless (SSO) provider the missing credential is a broker token that
   * only `/callback` can mint, so the remedy is a sign-in round-trip — NOT an
   * API-key dialog. Try the hidden-iframe forced re-exchange first: when the
   * IdP session is still live it is transparent (no visible login). If that
   * fails (IdP session gone / iframe blocked) fall back to the popup, which
   * works here because this runs in a real click gesture.
   *
   * `beginLogin`/`endLogin` bracket the attempt so a 401 arriving mid-flow does
   * not stack a second login prompt on top of this one.
   */
  async function signInAgain(ctx: ChatErrorActionContext): Promise<void> {
    auth.beginLogin();
    try {
      try {
        await silentLogin({ reexchange: true });
      } catch {
        // Silent path unavailable — ask visibly rather than dead-ending.
        await popupLogin();
      }
    } catch {
      // Popup blocked or the user dismissed it: leave the error bubble in
      // place so the button can be pressed again.
      toast.warning(t("chatErrors.signInFailed"));
      return;
    } finally {
      auth.endLogin();
    }
    // Credential re-minted. Re-run the failed turn, but only if it is still
    // the current one (the sign-in round-trip is async — the user may have
    // moved on while the popup was open).
  ...
  async function runChatErrorAction(
    ...
    switch (actionId) {
      ...
      case CHAT_ERROR_ACTION.openApiKeyFlow:
        ...
        return;
      case CHAT_ERROR_ACTION.signInAgain:
        await signInAgain(ctx);
        return;
