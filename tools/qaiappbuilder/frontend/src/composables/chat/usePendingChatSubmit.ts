...
export function decideExternalSubmitAction(
  status: string,
  /**
   * Whether the tab is busy in the BROADER sense the Stop button and the
   * Enter-enqueue path use (`chatTabs.isActiveTabBusy`): a live stream OR a
   * still-running sub-agent / backgrounded exec / background process while the
   * main agent already went `idle`.
   *
   * Without this a programmatic send (App Builder "send to chat", and any other
   * external dispatcher) submitted straight into a tab that still had work in
   * flight — the ordinary Enter key enqueues there, so the two paths disagreed
   * and the external one interrupted the running work. Optional so the pure
   * `status`-only contract stays valid for callers that have no store handle;
   * omitting it reproduces the pre-2026-08-09 behaviour.
   */
  busy = false,
