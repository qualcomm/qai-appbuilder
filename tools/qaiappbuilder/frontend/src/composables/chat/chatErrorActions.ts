...
export const CHAT_ERROR_ACTION = {
  ...
  /** Re-run the SSO sign-in so a keyless (SSO-authenticated) provider gets a
   *  fresh per-user token. Distinct from `openApiKeyFlow`: this provider has
   *  no key to configure, so the API-key dialog would be a dead end. */
...
const signInAgain: ChatErrorAction = {
  id: A.signInAgain,
  labelKey: "chatErrors.actions.signInAgain",
  style: "primary",
};
...
const SPECS: readonly ChatErrorSpec[] = [
  ...
  {
    // Same underlying condition as `provider_api_key_missing` (no outbound
    // credential) but for a provider whose credential comes from the SSO
    // login — the QAI Service token pool. Offering "Set API key" here would
    // strand the user: the pool has no key to set. The fix is to sign in
    // again, which re-mints the per-user token.
    code: "chat.llm.sso_token_missing",
    messageKey: "chatErrors.messages.ssoTokenMissing",
    primaryAction: signInAgain,
  },
  {
 ...
 *
 * Exported so the table can be checked wholesale rather than code-by-code:
 * this registry stores i18n *key strings* and never resolves them, so a spec
 * pointing at a missing key looks perfectly healthy here and only shows up as
 * a raw `chatErrors.messages.…` string in the user's error bubble. A test that
 * walks THIS list catches that; a test carrying its own hand-copied list of
 * codes silently stops covering whatever is added later.
 */
export const ALL_CHAT_ERROR_SPECS: readonly ChatErrorSpec[] = SPECS;
