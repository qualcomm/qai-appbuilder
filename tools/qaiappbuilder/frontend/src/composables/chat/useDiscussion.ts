...
export interface UseDiscussion {
  ...
  /** Set the coordinator (主持人) model id (persists). Null / "" ⇒ unset, so the
   *  backend resolves the ladder coordinator → roster[0] → default. */
  setCoordinatorModelId(modelId: string | null): Promise<void>;
  /** Set the coordinator (主持人) extra prompt (persists). Empty ⇒ backend
   *  built-in default only. */
  setCoordinatorPrompt(text: string): Promise<void>;
  /** Set the coordinator (主持人) permission override (persists). ``null`` ⇒
   *  clear the override so the backend applies its dynamic defaults (all tools
   *  minus ``agent`` + all globally-enabled skills). An object narrows one or
   *  both dimensions; an omitted key keeps that dimension on the backend
   *  default; an empty list narrows to "no tools" / "no skills". */
  setCoordinatorConfig(config: CoordinatorConfig | null): Promise<void>;
...
export function useDiscussion(tabIdRef?: Ref<TabId | null>): UseDiscussion {
  ...
  async function setCoordinatorModelId(modelId: string | null): Promise<void> {
    await patchSetting({ coordinatorModelId: modelId });
  }
  async function setCoordinatorPrompt(text: string): Promise<void> {
    await patchSetting({ coordinatorPrompt: text });
  }
  async function setCoordinatorConfig(
    config: CoordinatorConfig | null,
  ): Promise<void> {
    await patchSetting({ coordinatorConfig: config });
  }
