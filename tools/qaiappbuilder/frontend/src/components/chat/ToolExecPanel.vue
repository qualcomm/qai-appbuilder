...
interface Props {
  ...
  canCancel?: boolean;
  /** USER-FIXABLE failure code from the tool result envelope (see
   *  `ChatToolCall.errorCode`). When it is `chat.llm.tls_cert_untrusted`
   *  (web_search behind an enterprise TLS gateway), the card renders an inline
   *  notice + a "disable TLS verification and retry" button that runs the SAME
   *  action the turn-level error bubble offers. Undefined ⇒ no affordance. */
  errorCode?: string;
}
...
const emit = defineEmits<{
  ...
  /** The user clicked "disable TLS verification and retry" on this card's
   *  inline SSL-error affordance. The parent runs the shared executor
   *  (confirm → disable global ssl_verify → re-run the turn) — identical to
   *  the turn-level error bubble's action. */
  "configure-tls": [];
  /** Open (or switch to) the conversation a scheduled task belongs to.
   *  Emitted from the ``scheduled_task`` result panel's per-task
   *  "open conversation" button; the parent calls ``chatTabs.openTab``. */
...
//
// This set is the SINGLE source of truth for default-expand. The mapper
...
// truncation for expanded tools" contract to keep in lock-step. Folding is
// purely a front-end concern handled here.
//
// 2026-08-09: semantics INVERTED from a default-collapse blacklist to a
// default-expand whitelist. Rationale: most tool_call cards carry long,
// low-signal payloads (foreign-path scans, notebook maps, chipset lookups,
// file writes, ...) — the assistant narrative already summarises them.
// Only a short list of tools carries content the user actively needs to
// SEE unfolded when it happens:
//   - panel-rich cards (``show_*``) with dedicated GFM / step-table / map
//     rendering — pointless collapsed (4)
//   - core lifecycle: start/stop_model_builder (visible task boundaries)
//     and confirm_model_builder (F-stage handoff, the payoff moment) (3)
//   - approve_plan: the D-stage gate whose input (mismatch_resolutions)
//     is the user's actual decision content (1)
// Everything else (restart / request_fix / reject_plan / approve_restart /
// reject_restart / enqueue / leave_queue / scan / lookup / write_file / ...)
// defaults to collapsed; click the header ▼ to expand. Running cards still
// auto-expand while in-flight — folding only kicks in on done/error.
const DEFAULT_EXPANDED_TOOLS = new Set<string>([
  // panel-rich (dedicated card content, GFM / step-table / map)
  "show_step_progress",
  "show_pipeline",
  "show_briefing",
  "show_notebook_map",
  // core lifecycle — visible boundaries of the whole task
  "start_model_builder",
  "stop_model_builder",
  "confirm_model_builder",
  // approval gate whose input carries the user's decision
  "approve_plan",
]);
...
// DEFAULT_EXPANDED_TOOLS default with the auto-collapse watcher active
// (tool in the whitelist → start expanded; otherwise → start collapsed).
const ui = useUiStore();
const collapsed = ref<boolean>(
  ui.toolCardsCollapsed !== null
    ? ui.toolCardsCollapsed
    : !DEFAULT_EXPANDED_TOOLS.has(props.toolName),
);
...
//
// Tools NOT in DEFAULT_EXPANDED_TOOLS start collapsed already, so the
// auto-collapse-on-done branch is a no-op for them (they're already
// collapsed and running progress lives in the header chip).
//
...
// ── Inline SSL-error affordance (web_search behind an enterprise TLS gateway) ─
// A tool that fails with the user-fixable envelope
// `{ ok:false, error_code:"chat.llm.tls_cert_untrusted", ... }` surfaces its
// code here as `props.errorCode`. When it matches, the card shows a notice +
// a "disable TLS verification and retry" button whose click is delegated to
// the parent (ChatMessageList) so it runs the EXACT SAME executor the
// turn-level error bubble uses (`useChatErrorActions.configureTlsAndRetry`):
// a security-warning confirm → save global `ssl_verify:false` → re-run the
// turn. Single source of truth for "how to disable SSL"; no duplicate logic.
const showTlsCertError = computed(
  () => props.errorCode === "chat.llm.tls_cert_untrusted",
);

// ── Running-forces-expanded (2026-07-20 UX, revised 2026-08-09) ─────────
// PRIOR behaviour: while the tool was executing, `effectiveCollapsed` was
// forced to `false` so the user could watch live progress (spinner /
// streamed output / waiting dots). On done → snap to `collapsed`.
//
// REVISED (2026-08-09): after inverting the default set from a
// default-collapse blacklist to a default-EXPAND whitelist (8 tools), the
// intent is that non-whitelist tools stay COMPACT the whole time — the
// header chip already shows "🔧 tool_name (running)" which is signal
// enough. Auto-expanding during-run defeats the whole whitelist point:
// the user would see the huge foreign-path / notebook-map / lookup_soc
// payloads expand mid-run and only fold on done, which is exactly the
// noise the whitelist was meant to remove.
//
// New rule: `effectiveCollapsed` == `collapsed`. Running cards fold too;
// whitelist tools (start/stop/confirm/approve/show_*) are already the ones
// worth watching live because their initial `collapsed` = false.
// Users can still click the header to expand any collapsed card on demand.
...
/** Whether this call was auto-backgrounded — the foreground wait exceeded the
 *  threshold while fresh user input arrived, so the still-RUNNING child was
 *  handed to the background-process manager and the tool call returned early.
 *  Labelling that "已完成" is a lie the user can act on: the command has not
 *  finished, and the elapsed time shown is the threshold, not its runtime.
 *
 *  Read from the STRUCTURED `status === "backgrounded"` flowed by the backend
 *  on the `tool_result` frame's `backgrounded` field — mirrors the `cancelled`
 *  pathway (`StreamFrame.tool_result` docstring: appended so the UI does not
 *  have to sniff the `[已取消]` prefix). We keep the marker OUT of `result`
 *  prose parsing on purpose. */
const isBackgrounded = computed<boolean>(
  () => props.status === "backgrounded",
);
/** Whether this call terminated a background process (`stop`). Distinct from
 *  `done`: nothing failed, but the outcome is termination, not completion. */
const isStopped = computed<boolean>(() => props.status === "stopped");
/** Whether this call replaced a background process (`restart`). */
const isRestarted = computed<boolean>(() => props.status === "restarted");
...
const statusLabel = computed(() => {
  ...
  if (isBackgrounded.value) return t("chat.toolBackgrounded");
  if (isStopped.value) return t("chat.toolStopped");
  if (isRestarted.value) return t("chat.toolRestarted");
 ...
 *      (e.g. `apply_patch`, missing args). The template
...
// When this card is a ``scheduled_task`` tool whose result is the list
// payload ``{ok:true, tasks:[...]}``, render each task as a compact row with
// an "open conversation" button (so the user can jump to the conversation a
// task belongs to — most useful for ``scope:"all"`` listings). Any parse
// failure or non-list result falls back to the plain text output block below.
...
const scheduledTaskRows = computed<ScheduledTaskRow[] | null>(() => {
  if (props.toolName !== "scheduled_task") return null;
  ...
  for (const t of tasks) {
    if (typeof t === "object" && t !== null && "task_id" in t) {
      rows.push(t as ScheduledTaskRow);
    }
  }
  return rows;
});

function onOpenConversation(conversationId: string | undefined): void {
  if (typeof conversationId === "string" && conversationId !== "") {
    emit("open-conversation", conversationId);
  }
}

// ── computer tool screenshot(s) ────────────────────────────────────────────
// The ``computer`` tool embeds its capture URL in its result text so the shot
// renders as a thumbnail INSIDE this tool card (appearing WITH/AFTER the tool
// call, not above it as a separate assistant bubble). Two embed forms:
//   • ``![screenshot](/api/images/files/..)`` markdown — audience model/both;
//     this same ref is what the vision-block resolver feeds to the model.
//   • ``<!--screenshot:/api/images/files/..-->`` comment — audience=user; the
//     shot is shown to the user only, NOT fed to the model (no ref → no vision
//     block → no token cost). The comment is invisible in rendered text.
// Either way the card displays the thumbnail; clicking opens the shared
// lightbox (same viewer a user-uploaded image uses). URLs point to the
// on-disk original — never a downscaled copy.
const _SHOT_RE =
  /!\[[^\]]*\]\((\/api\/images\/files\/[^)\s]+)\)|<!--screenshot:(\/api\/images\/files\/[^>\s]+)-->/g;
const screenshotUrls = computed<string[]>(() => {
  if (props.toolName !== "computer") return [];
  const raw = props.result;
  if (typeof raw !== "string" || raw === "") return [];
  const urls: string[] = [];
  for (const m of raw.matchAll(_SHOT_RE)) {
    const url = m[1] || m[2];
...
<template>
  ...
  <div
    ...
    :class="{
      'tool-exec-panel--done': !isRunning,
      'tool-card--backgrounded': isBackgrounded,
    }"
  >
    ...
    >
      <div class="tool-exec-header-left">
        <span
          ...
          aria-hidden="true"
        ></span>
        <!-- A backgrounded call is NOT done: the child is still running under
             the background-process manager, so the ✓ would misreport it. -->
        <span
          v-else-if="isBackgrounded"
          class="tool-exec-done-icon"
          aria-hidden="true"
        >🚀</span>
        <!-- Stopped / restarted terminal states: distinct glyphs so the card
             does not read as a plain completion. They are structurally
             flowed by `terminal_kind` on the tool_result frame — not sniffed
             out of `result` prose (see ChatToolCall.status docs). -->
        <span
          v-else-if="isStopped"
          class="tool-exec-done-icon"
          aria-hidden="true"
        ...
        <!-- Tool-name chip is a FALLBACK: shown only when the tool has no
             ...
             subtitle (read/list/write/edit/glob/grep/exec/web_fetch/web_search/
             ...
             tools without a subtitle (apply_patch/unknown/
    ...
    <!-- Inline SSL-error affordance. Rendered OUTSIDE the collapsible body so
         it stays visible even after the card auto-collapses on completion —
         an actionable error must not hide. Shown only for the user-fixable
         `chat.llm.tls_cert_untrusted` code; every other tool error keeps its
         plain card unchanged. The button delegates to the parent, which runs
         the SAME executor the turn-level error bubble uses (confirm → disable
         global ssl_verify → re-run the turn). -->
    <div
      v-if="showTlsCertError"
      class="tool-exec-tls-error"
      role="alert"
      data-testid="tool-exec-tls-error"
    >
      <span class="tool-exec-tls-error-msg">⚠ {{ t("chatErrors.messages.tlsCertUntrusted") }}</span>
      <button
        type="button"
        class="tool-exec-tls-error-btn"
        data-testid="tool-exec-tls-retry-btn"
        @click.stop="onConfigureTls"
      >
        {{ t("chatErrors.actions.disableTlsAndRetry") }}
      </button>
    </div>
    ...
    >
      ...
      <!-- Scheduled-task list panel: clickable rows for a ``scheduled_task``
           list result. Each row can jump to the task's conversation. Falls
           back to the plain output block below when the result is not a task
           list (create/pause/remove/... or a parse failure). -->
      <div
        v-if="scheduledTaskRows !== null"
        class="scheduled-task-list"
      >
        <div
          v-if="scheduledTaskRows.length === 0"
          class="scheduled-task-empty"
        >{{ t("chat.scheduledTaskNone") }}</div>
        <div
        ...
        >
          <div class="scheduled-task-main">
            <span class="scheduled-task-name">{{ row.name || row.task_id }}</span>
            <span
              v-if="row.schedule"
              class="scheduled-task-schedule"
            >{{ row.schedule }}</span>
            <span
              v-if="row.state"
              class="scheduled-task-state"
            >{{ row.state }}</span>
          </div>
          <button
            v-if="row.conversation_id"
            type="button"
            class="scheduled-task-open"
            @click="onOpenConversation(row.conversation_id)"
          >{{ t("chat.scheduledTaskOpenConversation") }}</button>
        </div>
      </div>

      <!-- computer tool screenshot(s): thumbnails rendered INSIDE the card,
           above the raw output. Click to open the shared lightbox. -->
      <div
        v-if="screenshotUrls.length > 0"
        class="tool-exec-screenshots"
        data-testid="tool-exec-screenshots"
      >
        <img
          v-for="(url, i) in screenshotUrls"
          :key="url + ':' + i"
          :src="url"
          class="tool-exec-screenshot-thumb"
          alt="screenshot"
        />
      </div>
      ...
      <div
        v-if="result && scheduledTaskRows === null"
...
.tool-exec-tls-error {
  /* Sits between the header and the (collapsible) body; always visible. A
     soft danger-tinted strip so the actionable error stands out without
     screaming — the button carries the destructive weight. */
  display: flex;
  flex-wrap: wrap;
  align-items: center;
  gap: 0.6em;
...
.tool-exec-tls-error-msg {
  flex: 1 1 12em;
  min-width: 0;
}

.tool-exec-tls-error-btn {
  flex: 0 0 auto;
  padding: 3px 10px;
...
.tool-exec-tls-error-btn:hover {
  filter: brightness(0.94);
}

/* Scheduled-task list panel (``scheduled_task`` list result). Compact rows,
   each with an "open conversation" button. Self-contained, no V1 collision. */
.scheduled-task-list {
  display: flex;
