  ...
  >
    ...
    <template v-else>
      ...
      >
        ...
        <!-- Discoverability for the keyboard path to this same list: ↑/↓ in the
             composer browses these entries without opening the popover. Mirrors
             the `<kbd>` footer hint pattern in `TaxonomyPickerDropdown.vue`. -->
        <div class="riph-kbd-hint">
          <kbd>↑</kbd><kbd>↓</kbd> {{ t("promptHistory.kbdHint") }}
        </div>
      </div>
    </template>
  </div>
