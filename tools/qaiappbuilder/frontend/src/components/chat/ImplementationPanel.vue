...
// ── Generate plan (P0-方案A: show a CTA when implementation is enabled but no
// plan has been generated yet) ─────────────────────────────────────────────────
/** Show the panel with just a 'generate' button when implementation is enabled
 *  but no plan has been generated yet. */
const canGenerate = computed(
  () =>
    discussion.config.value?.isDiscussion === true &&
    discussion.config.value?.implementationEnabled === true &&
    phase.value === "none",
);
...
<template>
  ...
  >
    ...
    <!-- ── Generate plan CTA (shown when implementation is enabled but no plan yet) ── -->
    <div v-if="canGenerate" class="impl-generate">
      <p class="impl-generate-hint">{{ t("chat.implementation.generateHint") }}</p>
      <button
        type="button"
