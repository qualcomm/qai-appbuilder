...
const props = withDefaults(
  defineProps<{
    ...
    /** Field names to hide entirely. Used by the coordinator editor to strip
     *  ``displayName`` / ``persona`` / ``color`` — identity dimensions the
     *  built-in coordinator does not let the user change (its host name is
     *  fixed by i18n and its avatar uses the shared accent token). Every
     *  remaining field (model, tools, skills) behaves identically. */
    hiddenFields?: ReadonlyArray<"displayName" | "modelId" | "persona" | "color">;
  }>(),
  {
...
const isHidden = (
  name: "displayName" | "modelId" | "persona" | "color",
): boolean => props.hiddenFields.includes(name);
...
<template>
  ...
  >
    <label v-if="!isHidden('displayName')" class="role-field">
    ...
    <label v-if="!isHidden('modelId')" class="role-field">
    ...
    <label v-if="!isHidden('persona')" class="role-field">
    ...
    <div v-if="!isHidden('color')" class="role-field">
