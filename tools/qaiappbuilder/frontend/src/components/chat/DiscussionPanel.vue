...
import CoordinatorEditorSection from "@/components/chat/CoordinatorEditorSection.vue";
...
const showCoordinatorEditor = ref(false);
...
// ── Coordinator (主持人) — unified moderator + judge model + prompt ──────────
// The coordinator drives both the moderator (speaker selection) and the final
// judge/summary turn. Its model reuses the SAME cloud-model dropdown source as
// the session-level model knobs; an empty selection ⇒ null (backend resolves
// the ladder coordinator → roster[0] → default).
function onCoordinatorModelSelect(e: Event): void {
  const value = (e.target as HTMLSelectElement).value;
  void discussion.setCoordinatorModelId(value === "" ? null : value);
}

function onCoordinatorPromptInput(e: Event): void {
  void discussion.setCoordinatorPrompt((e.target as HTMLTextAreaElement).value);
}
...
// ── Built-in coordinator (主持人) card + inline editor ──────────────────────
// The coordinator is a runtime-synthesised sentinel participant injected by
// the backend when discussion + enable_judge are ON. We surface it as a
// first-class participant card so the user can (a) see it belongs to the
// discussion, (b) @-mention it, and (c) narrow its tools / skills — but not
// rename it or delete it (identity is fixed by :data:`COORDINATOR_SENDER_ID`).
const showCoordinatorCard = computed<boolean>(
  () => isDiscussion.value && config.value?.enableJudge === true,
);

function openCoordinatorEdit(): void {
  // Close any open participant editor so only one form is visible at a time.
  showEditor.value = false;
  editingId.value = null;
  showCoordinatorEditor.value = true;
}

function closeCoordinatorEditor(): void {
  showCoordinatorEditor.value = false;
}
...
<template>
  <div class="discussion-panel" data-testid="discussion-panel">
    ...
    <template v-if="isDiscussion && config">
      ...
      <div class="discussion-row discussion-row--mode">
        ...
        <button
          type="button"
          class="discussion-mode-manage-btn"
          :title="t('chat.discussion.modes.manage')"
          data-testid="discussion-mode-manage"
          @click="libraryReturnTab = 'mode'; showLibrary = true"
        >
          ⚙️
        </button>
      </div>
      ...
      <!-- ── Coordinator (主持人) — unified moderator + judge (V2) ── -->
      <div class="discussion-convergence" data-testid="discussion-coordinator">
        <span class="discussion-convergence-title">{{
          t("chat.discussion.coordinator.title")
        }}</span>
        <!-- Coordinator model — reuses the shared cloud-model dropdown source.
             Empty ⇒ backend ladder (coordinator → roster[0] → default). -->
        <div class="discussion-row discussion-row--with-hint">
          <label class="discussion-switch-label">
            <span class="discussion-label">{{
              t("chat.discussion.coordinator.model")
            }}</span>
            <span class="discussion-switch-hint">{{
              t("chat.discussion.coordinator.modelHint")
            }}</span>
          </label>
          <select
            class="discussion-model-input"
            data-testid="discussion-coordinator-model"
            :value="config.coordinatorModelId ?? ''"
            @change="onCoordinatorModelSelect"
          >
            <option value="">
            ...
            <option
              v-if="config.coordinatorModelId != null && isCloudModelMissing(config.coordinatorModelId)"
              :value="config.coordinatorModelId"
            >
        ...
        <!-- Coordinator prompt — advisory text appended to moderator + judge. -->
        <div class="discussion-row discussion-row--column">
          <label class="discussion-label" for="discussion-coordinator-prompt">{{
            t("chat.discussion.coordinator.prompt")
          }}</label>
          <textarea
            id="discussion-coordinator-prompt"
            class="discussion-prompt-input"
            rows="4"
            data-testid="discussion-coordinator-prompt"
            :value="config.coordinatorPrompt ?? ''"
            :placeholder="t('chat.discussion.coordinator.promptPlaceholder')"
            @change="onCoordinatorPromptInput"
          ></textarea>
        </div>
      </div>
      ...
      <div class="discussion-participants">
        <div class="discussion-participants-header">
          ...
          <div class="discussion-participants-header-actions">
            <button
              ...
              @click="libraryReturnTab = 'agent'; showLibrary = true"
            >
        ...
        <ul class="discussion-list" data-testid="discussion-list">
          <!-- Built-in coordinator card — no delete, no call-on (backend
               falls back to the sentinel automatically when needed). Only
               visible when discussion mode + judge round are both on, which
               is the exact condition under which the backend injects the
               sentinel into the roster (orchestrate_discussion.py:1052). -->
          <li
            v-if="showCoordinatorCard"
            class="discussion-item discussion-item--coordinator"
            data-testid="discussion-coordinator-participant"
          >
            <span
              class="discussion-item-avatar discussion-item-avatar--coordinator"
              aria-hidden="true"
            >🎙️</span>
            <div class="discussion-item-body">
              <span class="discussion-item-name">
                {{ t("chat.discussion.coordinator.displayName") }}
                <span
                  class="discussion-item-badge"
                  data-testid="discussion-coordinator-badge"
                >{{ t("chat.discussion.coordinator.badge") }}</span>
              </span>
              <span class="discussion-item-meta">
                {{ config.coordinatorModelId || t("chat.discussion.blankModel") }}
              </span>
            </div>
            <div class="discussion-item-actions">
              <button
                ...
                :title="t('chat.discussion.coordinator.editTitle')"
                data-testid="discussion-coordinator-edit"
                @click="openCoordinatorEdit"
              >
                ✏️
              </button>
            </div>
          ...
          <!-- Roster empty state — only when there are truly no user roles
               (the coordinator alone does not count). Users can still start
               the discussion because the coordinator will speak when addressed
               via @-mention, but the message keeps the "add an agent" hint. -->
          <li
            v-if="participants.length === 0"
            class="discussion-empty discussion-empty--inline"
          ...
          >
            ...
            <div class="discussion-item-body">
              ...
              <span class="discussion-item-meta" data-testid="discussion-participant-model">
                <!-- Blank model_id is a VALID "use the tab's current model"
                     state (backend falls back — 留空则用当前标签页模型), so show a
                     clear label instead of an empty/unspecified string that
                     looks broken (Fix 4). Display-only: the persisted value
                     stays blank so the fallback keeps working. -->
                {{ p.model_id || t("chat.discussion.blankModel") }}
        ...
        <!-- ── Inline coordinator editor (opens when the ✏️ on the coordinator
             card is clicked; mounted lazily so the tool-catalog / skills fetch
             only runs while the editor is visible). -->
        <CoordinatorEditorSection
          v-if="showCoordinatorEditor"
          @close="closeCoordinatorEditor"
        />
    ...
    <TemplateLibraryDialog
      ...
      :visible-tabs="libraryReturnTab === 'mode' ? ['mode'] : ['agent', 'roster']"
...
.discussion-mode-manage-btn:hover {
