...
// ─── Scroll to selected model on open ────────────────────────────────────────

const modelListRef = ref<HTMLElement | null>(null);

/** Scroll the `.model-item.selected` element into view inside the list. */
function scrollToSelected(): void {
  const list = modelListRef.value;
  if (!list) return;
  const selected = list.querySelector(".model-item.selected") as HTMLElement | null;
  if (!selected) return;
  selected.scrollIntoView({ block: "center", behavior: "instant" });
}

// After any data source finishes loading, the selected item may now be rendered.
// Watch the loading flags and query-services list; scroll once the selected
// element appears in the DOM.
watch(
  [localLoading, cloudLoading, queryServices],
  () => {
...
/** The search box is focused EXPLICITLY rather than via the HTML `autofocus`
 *  attribute.  `autofocus` is only honoured while the focus still sits on
 *  `<body>`; this dropdown always opens from a click on the model-selector
 *  button, so the button already holds focus and the browser silently drops
 *  the request ("Autofocus processing was blocked because a document already
 *  has a focused element") — the box never actually got focus.  An explicit
 *  `.focus()` after mount has no such precondition, and matches how every
 *  other panel in the app focuses its primary input (AppCommandPalette /
 *  RenameDialog / SetApiKeyDialog / ConversationWorkspaceDialog). */
