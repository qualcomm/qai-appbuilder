...
import IntegratedNoticesChip from "@/components/chat/IntegratedNoticesChip.vue";
...
import { useWdacStatus } from "@/composables/useWdacStatus";
...
const emit = defineEmits<{
  ...
  // Per-message 🔀 button (V2 fork enhancement 2026-07-25). Emitted when
  // the user clicks the 🔀 next to an assistant bubble's ⧉ copy button;
  // ChatView hosts the ForkConversationDialog in message-anchor mode and
  // POSTs the fork with `up_to_message_id = payload.id`.
  "fork-from-message": [payload: { id: string }];
...
// Open (or switch to) the conversation a scheduled task belongs to, from the
// ``scheduled_task`` list panel's per-row button. Prefer an already-open tab
// (just switch focus); otherwise open a new tab bound to that conversation
// (``openTab`` loads its history). Ignores an empty/current id.
function onOpenScheduledConversation(conversationId: string): void {
  if (typeof conversationId !== "string" || conversationId === "") return;
  const existing = store.tabs.find(
    (tb) => tb.conversationId === conversationId,
  );
  if (existing !== undefined) {
    store.switchTab(existing.id);
    return;
  }
  store.openTab({ conversationId });
}
...
// ─── WDAC / code-integrity heads-up (Model Builder blocker) ─────────────────
// When the host enforces WDAC user-mode code integrity / Smart App Control,
// the kernel refuses to load the UNSIGNED QAIRT converter .pyd, so model
// conversion (Model Builder) cannot work. Surface a dismissible warning on
// the welcome screen with how-to-disable guidance. Best-effort + fail-open:
// `useWdacStatus` only warns when the backend positively confirms enforcement.
const wdacStatus = useWdacStatus();
const showWdacWarning = computed(
  () =>
    showWelcome.value &&
    wdacStatus.enabled.value &&
    !wdacStatus.dismissed.value,
);
...
function toToolCallViews(msg: ChatMessage): ToolCallView[] {
  ...
  return calls.map((call) => ({
    ...
    backgroundProcessId: call.backgroundProcessId,
  }));
}
...
const streamingLabel = computed(() => {
  const tab = activeTab.value;
  // Coordinator (主持人) live turn: fixed host name (NOT the borrowed role).
  if (tab?.streamingIsCoordinator === true) return coordinatorName();
...
/** Whether a committed message is the unified coordinator (主持人) turn. Such
 *  bubbles render the FIXED host display name + a distinct badge instead of a
 *  borrowed participant role name (coordinator feature). Belt-and-suspenders:
 *  the live turn carries `isCoordinator`, but a PERSISTED coordinator message
 *  reloaded from history may arrive with only the stable `COORDINATOR_SENDER_ID`
 *  sentinel on `senderId` (the `is_coordinator` flag does not persist), so
 *  recognise the sentinel too — never fall back to the raw sentinel id. */
function isCoordinatorMsg(msg: ChatMessage): boolean {
  return (
    msg.role === "assistant" &&
    (msg.isCoordinator === true || msg.senderId === COORDINATOR_SENDER_ID)
  );
}

/** Whether a message is a producer-owned aside (background-job completion
 *  notice etc.) rather than a turn by the user or the model. Set by the
 *  ``aside_message`` frame handler via ``meta.aside``. */
function isAsideMsg(msg: ChatMessage): boolean {
  if (msg.role === "system_notice") {
    return true;
  }
  // Legacy meta.aside path — kept for the transition window until the
  // last producer publishing meta.aside is retired.  §6.8 cleanup step.
  return (msg.meta as Record<string, unknown> | undefined)?.["aside"] === true;
}

/** Fixed coordinator (主持人) display name from i18n. */
function coordinatorName(): string {
  return t("chat.discussion.coordinator.displayName");
}

/** Whether a committed message carries discussion-speaker attribution. A
 *  coordinator turn also renders as a discussion bubble (distinct avatar/name),
 *  even though its `senderId` borrows the roster identity. */
function hasSender(msg: ChatMessage): boolean {
  return (
    (typeof msg.senderId === "string" && msg.senderId !== "") ||
    isCoordinatorMsg(msg)
  );
}
...
function bubbleLabel(msg: ChatMessage): string {
  if (isCoordinatorMsg(msg)) return coordinatorName();
...
/** The user message that STARTED the turn a given assistant message belongs
 *  to — i.e. the nearest `user` message before it. This is the retry anchor
 *  for a tool card's inline SSL fix (re-run THAT turn, exactly like the LLM
 *  error bubble re-runs its failed user message). Null when none precedes it. */
function turnUserMessageFor(msg: ChatMessage): ChatMessage | null {
  const tab = activeTab.value;
  if (tab === null) return null;
  const idx = tab.messages.findIndex((m) => m.id === msg.id);
  if (idx < 0) return null;
  for (let i = idx; i >= 0; i -= 1) {
    const m = tab.messages[i];
    if (m !== undefined && m.role === "user") return m;
  }
  return null;
}

/** A tool card fired "disable TLS verification and retry". Reuse the SAME
 *  executor the turn-level error bubble uses (`configureTlsAndRetry`: confirm →
 *  save global `ssl_verify:false` → re-run the turn) so there is ONE way to
 *  disable SSL. The ctx's retry anchor is the user message that started this
 *  assistant turn; `isCurrent` guards against a stale click by re-checking
 *  that anchor still exists in the tab (the tool result — not a turn-level send
 *  error — is what surfaced the affordance, so there is no `sendErrorCode` to
 *  key off; message-still-present is the correct liveness signal here). */
function onToolConfigureTls(msg: ChatMessage): void {
  const user = turnUserMessageFor(msg);
  if (user === null) return;
  const anchorId = user.id;
  const ctx = {
    failedMessageId: anchorId,
    failedContent: user.content,
    isCurrent: () =>
      (activeTab.value?.messages ?? []).some((m) => m.id === anchorId),
    retry: (id: string, content: string) => {
      onRetry(id, content);
    },
    diagnostics: () => "",
  };
  void runChatErrorAction(CHAT_ERROR_ACTION.configureTlsAndRetry, ctx);
}
...
// Delegated click for images rendered INSIDE an assistant markdown bubble
// (``v-html`` output — no per-``<img>`` binding possible). A screenshot from
// the ``computer`` tool arrives as ``![screenshot](url)`` markdown, so the
// renderer emits a plain ``<img>``; this handler lets such an inline image
// open the SAME lightbox (zoom / drag / close) a user-uploaded thumbnail uses,
// keeping assistant + user image display consistent. Only fires when the
// clicked target is an ``<img>`` carrying a real ``src``.
function onBubbleImageClick(ev: MouseEvent): void {
  const target = ev.target as HTMLElement | null;
  if (target === null || target.tagName !== "IMG") return;
  const src = (target as HTMLImageElement).currentSrc
...
onMounted(() => {
  ...
  void cloudModelStatus.refresh();
  // Probe code-integrity enforcement once so the welcome screen can warn that
  // Model Builder conversion is blocked. Fire-and-forget; fail-open.
  void wdacStatus.ensureFetched();
  void nextTick(() => {
  ...
  <div class="chat-message-pane">
    ...
    >
      ...
      <template v-else>
        ...
        <!-- WDAC / code-integrity heads-up: model conversion (Model Builder)
             cannot work while the OS blocks the unsigned QAIRT converter .pyd.
             Dismissible; only shown when the backend confirms enforcement. -->
        <div
          v-if="showWdacWarning"
          class="wdac-warning"
          role="alert"
          data-testid="wdac-warning"
        >
          <button
            type="button"
            class="wdac-warning-close"
            :aria-label="t('chat.wdacWarning.dismiss')"
            :title="t('chat.wdacWarning.dismiss')"
            @click="wdacStatus.dismiss()"
          >
            ×
          </button>
          <div class="wdac-warning-title">⚠️ {{ t("chat.wdacWarning.title") }}</div>
          <p class="wdac-warning-problem">{{ t("chat.wdacWarning.problem") }}</p>
          <div class="wdac-warning-section">{{ t("chat.wdacWarning.howToTitle") }}</div>
          <ol class="wdac-warning-steps">
            <li>{{ t("chat.wdacWarning.step1") }}</li>
            <li>{{ t("chat.wdacWarning.step2") }}</li>
            <li>{{ t("chat.wdacWarning.step3") }}</li>
            <li>{{ t("chat.wdacWarning.step4") }}</li>
          </ol>
          <div class="wdac-warning-section">{{ t("chat.wdacWarning.riskTitle") }}</div>
          <p class="wdac-warning-risk">{{ t("chat.wdacWarning.risk") }}</p>
        </div>
    ...
    <template v-if="activeTab && !showWelcome && !activeTab.loadingHistory">
      ...
      >
        ...
        >
          ...
          <span v-if="isCoordinatorMsg(msg)" data-testid="coordinator-avatar" aria-hidden="true">🎙️</span>
          <span v-else-if="msg.role !== 'user' && hasSender(msg)">{{ senderInitial(msg.senderName) }}</span>
        ...
        <div class="message-content-wrap">
          <!-- An aside (producer-owned system note: background-job completion
               etc.) carries no speaker and no per-message actions: it is not a
               turn by anyone. Rendering the meta row for it produces a SECOND
               bordered "ⓘ" box above the note — the `.msg-aside` rule styles
               every direct child of `.message-content-wrap` — which reads as a
               phantom empty bubble stamped with an avatar and a timestamp. -->
          <div
            v-if="!isAsideMsg(msg)"
            class="message-meta"
          >
            ...
            <!-- Coordinator (主持人) badge — distinct host marker rendered
                 instead of a borrowed role name (coordinator feature). -->
            <span
              v-if="isCoordinatorMsg(msg)"
              class="discussion-coordinator-badge"
              data-testid="coordinator-badge"
            >🎙️ {{ t("chat.discussion.coordinator.displayName") }}</span>
            ...
            <div class="message-actions">
              ...
              <!-- Per-message fork button 🔀 (V2 fork enhancement 2026-07-25).
                   Only shown on assistant messages — user/tool bubbles have no
                   forkable anchor (a fork always ends on an assistant response).
                   Hover-only via the parent `.message-row:hover .message-actions`
                   opacity rule, same as the ⧉ copy button. Clicking emits so
                   ChatView can open the dialog anchored on this message id. -->
              <button
                v-if="msg.role === 'assistant'"
                type="button"
                class="btn btn-icon"
                data-testid="message-fork-btn"
                :title="t('chat.fork.messageBtnTitle')"
                :aria-label="t('chat.fork.messageBtnTitle')"
                @click="emit('fork-from-message', { id: msg.id })"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><circle cx="18" cy="6" r="3"/><path d="M12 15V12a3 3 0 0 0-3-3H9M12 12a3 3 0 0 1 3-3h0"/></svg>
              </button>
            </div>
          </div>
          ...
          <!-- Sub-agent blocks (V1 index.html:629-669) — rendered BEFORE the
            ...
            <!-- N.53 — "integrated N background notices" chip. Sits ABOVE the
                 answer (and above the reasoning block) because it is
                 provenance for what follows: the user reads "this answer
                 folded in 3 background results" BEFORE reading the answer.
                 Rendered only when the turn actually integrated notices
                 (`integratedNotices` is undefined otherwise — including for
                 every pre-N.53 message, so no reload regression). -->
            <IntegratedNoticesChip
              v-if="msg.role === 'assistant' && msg.integratedNotices && msg.integratedNotices.length > 0"
              :notices="msg.integratedNotices"
            />
            ...
            >
              <ToolCallList
                ...
                @cancel-tool="onCancelTool"
                @configure-tls="onToolConfigureTls(msg)"
                @open-conversation="onOpenScheduledConversation"
              />
            </div>
            ...
            <!-- Perf metrics ([I]/[O] tokens, tok/sec, TTFT, total). These are
                 developer-oriented; per-row hover reveal keeps them out of the
                 way in normal reading. The 🔢 lead-in icon is intentionally
                 dropped — it clashed with both light and dark theme tokens
                 (raw emoji glyph carries its own colour). Hover is driven by
                 `.message-row:hover .token-badge { opacity: 1 }` in chat.css,
                 reusing the same mechanism as `.message-actions` — no new
                 pattern, one source of truth for hover-reveal. -->
            <div
      ...
      >
        ...
        >
          <span v-if="activeTab.streamingIsCoordinator === true" data-testid="coordinator-avatar" aria-hidden="true">🎙️</span>
          <span v-else-if="activeTab.streamingSenderId !== null">{{ senderInitial(activeTab.streamingSenderName ?? undefined) }}</span>
          <!-- eslint-disable-next-line vue/no-v-html -->
        ...
        <div class="message-content-wrap">
          <div class="message-meta">
            ...
            >{{ streamingLabel }}</span>
            <span
              v-if="activeTab.streamingIsCoordinator === true"
              class="discussion-coordinator-badge"
              data-testid="coordinator-badge"
            >🎙️ {{ t("chat.discussion.coordinator.displayName") }}</span>
          </div>
      ...
      <!-- Typing indicator while streaming with no trailing text yet (V1
           index.html:831-836). Shown whenever the turn is streaming OR
           the tab is idle but has background work still in flight
           (§UX-idle-with-bg-work) — the in-flight round's tool cards
           (in ``messages``) carry their own spinners, but the parent
           agent's "thinking" still needs a typing cue until its text
           starts, and completed-turn+running-subagent needs the same
           cue so the user sees "the tab is still working" rather than
           an ambiguous idle screen. -->
      <div
        v-if="
          (activeTab.status === 'streaming' &&
            activeTab.streamingContent === '') ||
            (activeTab.status === 'idle' && store.hasActiveBackgroundWork)
        "
        class="message-row ai"
      >
        <div
...
/* Coordinator (主持人) host badge — a distinct pill next to the bubble name so
   the unified moderator+judge turn is visually set apart from participant
   roles. Theme-aware tokens only (AGENTS.md §3.10 — never hardcoded colours). */
.discussion-coordinator-badge {
  margin-left: 6px;
  padding: 0 6px;
  border-radius: 999px;
...
/* WDAC / code-integrity heads-up banner on the welcome screen. Theme-aware
   (all colors via global CSS variables); amber "warning" accent. */
.wdac-warning {
  position: relative;
  max-width: 640px;
  margin: 0 auto 20px;
  ...
  background: var(--warning-bg, rgba(217, 164, 65, 0.10));
...
.wdac-warning-problem {
  margin: 0 0 12px;
}
.wdac-warning-section {
  font-weight: 600;
  margin-top: 10px;
  margin-bottom: 4px;
}
