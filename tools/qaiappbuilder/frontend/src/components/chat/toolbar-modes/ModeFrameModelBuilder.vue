...
import GallerySubmitDialog from "@/components/app-builder/model-builder/GallerySubmitDialog.vue";
...
// Configurable model workspace root (canonical read from `useForgeConfig`,
// backed by `workspace.model_root`; falls back to `C:\WoS_AI` when unset —
// same fallback the scan utility uses internally, so callers can pass the
// value directly with no `|| undefined` juggling). We `load()` once on
// mount so the value reflects the configured root as soon as possible;
// until then the reactive default keeps the scan usable.
const { workspaceModelRoot, load: loadForgeConfig } = useForgeConfig();
...
function toggleGalleryDialog(): void {
  galleryDialogOpen.value = !galleryDialogOpen.value;
}

function onGenerateModelCard(prompt: string): void {
...
<template>
  ...
  >
    ...
    <div class="rit-submenu-wrap">
      ...
      >
        <PromoteToAppBuilderCard
          ...
          :verified-variants-count="promoteDetection.detectedVariants.value.length"
    ...
    <!-- 7. Submit to Model Gallery -->
    <span class="rit-sep"></span>
    <button
      type="button"
      class="rit-btn"
      :class="{ 'rit-model-upload--active': galleryDialogOpen }"
      :title="t('modelBuilder.gallery.title')"
      data-testid="mb-toggle-gallery-submit"
      @click="toggleGalleryDialog"
    >
      <svg
        width="13"
        height="13"
      ...
      <span>{{ t("modelBuilder.gallery.title") }}</span>
    </button>

    <!-- Gallery Submit Dialog (Teleport-based overlay) -->
    <GallerySubmitDialog
      :session-model-workdir="sessionModelWorkdir"
      :open="galleryDialogOpen"
      @close="galleryDialogOpen = false"
      @generate-model-card="onGenerateModelCard"
    />
  </div>
</template>
