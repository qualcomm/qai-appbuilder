...
const { workspaceModelRoot } = useForgeConfig();
...
// Configured workspace root — canonical read from `useForgeConfig`, backed
// by `workspace.model_root` with a `C:\WoS_AI` fallback for the not-loaded /
// unset case. Mirrors the same pattern in ModeFrameModelBuilder.
    ...
    <div class="rit-submenu-wrap">
      ...
      >
        <PromoteToAppBuilderCard
          ...
          :verified-variants-count="promoteReady.detectedVariants.value.length"
