...
const props = defineProps<{
  ...
  /** Which tabs to show. Omit to show all three. */
  visibleTabs?: TemplateTab[];
...
/** Whether a given tab is allowed by the `visibleTabs` prop. */
function tabVisible(t: TemplateTab): boolean {
  return props.visibleTabs === undefined || props.visibleTabs.includes(t);
}
...
<template>
  <div class="tl-overlay" @click.self="emit('close')">
    ...
    >
      ...
      <!-- Tab bar — hidden entirely when only one tab is visible -->
      <div
        v-if="(tabVisible('agent') ? 1 : 0) + (tabVisible('roster') ? 1 : 0) + (tabVisible('mode') ? 1 : 0) > 1"
        class="tl-tabs"
        role="tablist"
      >
        <button
          v-if="tabVisible('agent')"
          type="button"
          role="tab"
          class="tl-tab"
