...
/**
 * Status to PAINT for a tab, per §UX-idle-with-bg-work.
 *
 * Not `tab.status`: a tab whose main turn finished while a sub-agent or a
 * backgrounded exec keeps running is `idle` yet still WORKING, and painting it
 * grey claimed the work was done. `tabLiveStatus` reports `"streaming"` for
 * that case, so the existing dot / label styling covers it with no new state.
 * Shared with the sidebar's per-conversation dot and the composer's
 * Stop/typing affordances so all three answer the question identically.
 */
function paintStatus(tab: ChatTab): ChatTabStatus {
  return chatTabsStore.tabLiveStatus(tab) ?? "idle";
}
    ...
    >
      <div
        ...
        :class="{
          ...
          [`chat-tab-strip__tab--${paintStatus(tab)}`]: true,
        }"
        :aria-selected="tab.id === activeMainTabId ? 'true' : 'false'"
        :data-tab-id="tab.id"
        :data-tab-status="paintStatus(tab)"
      ...
      >
        <span
          ...
          :class="`chat-tab-strip__dot--${paintStatus(tab)}`"
          aria-hidden="true"
        />
        <!-- Multi-agent (discussion) tab → distinct 👥 glyph so a discussion
             conversation is distinguishable from an ordinary chat at a glance
             (matches the ChatComposer discussion pill + sidebar list icon). The
             tab model carries the authoritative `discussion.isDiscussion`; an
             ordinary tab renders nothing here (unchanged). -->
        <span
          v-if="tab.discussion?.isDiscussion === true"
          class="chat-tab-strip__discussion-icon"
          :title="t('chat.discussion.title')"
          data-testid="chat-tab-discussion-icon"
          aria-hidden="true"
        >👥</span>
        <input
...
/* Multi-agent (discussion) tab marker — distinct glyph before the title so a
   discussion tab reads apart from an ordinary chat. flex-shrink:0 keeps it from
   collapsing when the title ellipsises. */
.chat-tab-strip__discussion-icon {
  flex: 0 0 auto;
