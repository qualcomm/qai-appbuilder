...
interface ProviderConfig {
  ...
  api_key?: string;
  /**
   * How the provider authenticates, emitted by the backend
   * (`list_provider_configs.py`). `"sso"` = keyless (credential minted per
   * user at login); anything else / absent = static API key.
   */
  auth_mode?: string;
  /** Read-only presence flag from the GET; never written back. */
  has_api_key?: boolean;
  pinned?: boolean;
  models?: CloudModel[];
  [key: string]: unknown;
...
/**
 * Whether `row` is a keyless (SSO) provider — the QAI Service token pool.
 * Such a provider has no API key to configure, so the key input is replaced
 * by an explanatory notice.
 */
function isKeylessProvider(row: ProviderRow): boolean {
  return row.config.auth_mode === "sso";
}
...
async function putProviderConfig(
  ...
  // `auth_mode` / `has_api_key` are read-only fields the GET derives; they are
  // not part of the writable config document. Stripped here — the single
  // chokepoint every provider write funnels through — so no call site can
  // accidentally persist them back into kv_user_prefs.
  const body: ProviderConfig = { ...config };
          ...
          >
            ...
            >
              ...
              >
                ...
                />
                <!-- API key input, or a keyless notice for SSO providers.
                     A provider whose `auth_mode` is "sso" (the QAI Service
                     token pool) authenticates with a per-user JWT minted at
                     login — there is no key to type, so offering the field
                     would invite the user to configure something that cannot
                     exist. -->
                <div
                  v-if="isKeylessProvider(row)"
                  style="flex:2;min-width:160px;display:flex;align-items:center;gap:4px;font-size:var(--text-xs);opacity:0.75"
                >
                  <span>🔐</span>
                  <span>{{ t("cloudModels.keylessProviderHint") }}</span>
                </div>
                <div v-else style="flex:2;min-width:160px;display:flex;align-items:center;gap:4px">
                  <input
                    class="cloud-models-search"
