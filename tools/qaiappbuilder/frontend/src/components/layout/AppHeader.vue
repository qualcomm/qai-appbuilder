...
<template>
  ...
  >
    ...
    <div class="topbar-actions">
      ...
      <!-- Persistent scheduled-task notification bell — an inline flex sibling
           of the action buttons (never a fixed overlay), so it can never
           overlap the tab-strip / action row or other top-right floating UI. -->
      <NotificationCenter />
    </div>
  </header>
</template>
...
/* --- Header action button: icon-only. Label lives in `title` (native
 *     tooltip) + `aria-label`; the expand-on-hover pattern was removed
 *     because the button's hitbox growing under the cursor made it
 *     hard to click ("mouse-chase"). --- */
.topbar-action-btn {
  display: inline-flex;
  align-items: center;
