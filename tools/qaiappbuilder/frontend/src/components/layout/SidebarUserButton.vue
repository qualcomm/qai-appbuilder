...
import { rearmAuthGate } from "@/api/http";
...
async function signOut(): Promise<void> {
  ...
  if (!ok) return;
  // Clear server-side cookie without leaving the SPA.
  await logoutSession();
  // Re-arm the HTTP gate so business requests won't fire (and 401) until
  // the user signs back in.
  rearmAuthGate();
  // Update auth store — triggers the LoginPrompt modal via showLoginPrompt.
  const auth = useAuthStore();
  auth.authenticated = false;
  auth.user = null;
  auth.expiresAt = null;
  auth.showLoginPrompt = true;
}
