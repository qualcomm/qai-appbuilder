...
import SidebarQuotaGauge from "@/components/layout/SidebarQuotaGauge.vue";
const { t } = useI18n();
...
//
// Returns `null` when the conversation has no open tab OR its tab is quiet, so
// idle/closed conversations render no dot (avoids visual noise). Only the
// "busy/attention" states surface a dot.
//
// §UX-idle-with-bg-work: reads the store's `tabLiveStatus`, NOT `tab.status`.
// A tab whose main turn finished while a sub-agent / backgrounded exec keeps
// running is `idle` yet still WORKING — reading raw status rendered no dot, so
// the sidebar claimed the conversation was done while it was not. The predicate
// lives in the store so this dot, the tab-strip dot and the composer's
// Stop/typing affordances all answer the question identically.
function convLiveStatus(convId: string): ChatTabStatus | null {
  const tab = chatTabs.tabs.find((t) => t.conversationId === convId);
  if (tab === undefined) return null;
  return chatTabs.tabLiveStatus(tab);
}

/** Whether a conversation is a multi-agent (discussion) conversation, so the
 *  sidebar can render a DISTINCT icon (👥) instead of the ordinary chat glyph.
 *  Reads the REAL persisted truth `Conversation.meta.discussion.is_discussion`
 *  (shipped verbatim on the list projection — never a fabricated field); falls
 *  back to an OPEN tab's live `discussion.isDiscussion` for a conversation whose
 *  meta has not (yet) surfaced the flag. Unknown ⇒ false (ordinary icon,
 *  graceful degrade). */
function isDiscussionConv(conv: ConversationSummary): boolean {
  const disc = (conv.meta as { discussion?: { is_discussion?: unknown } } | null)
    ?.discussion;
  if (disc?.is_discussion === true) return true;
  const tab = chatTabs.tabs.find((t) => t.conversationId === conv.id);
  return tab?.discussion?.isDiscussion === true;
}
...
async function selectConversation(conv: ConversationSummary): Promise<void> {
  ...
  // Re-attach to a still-running background turn for this conversation, if any
  // — the backend no longer kills a turn whose data-plane WS dropped, so a
  // reopened conversation may still be streaming (with a pending `question`).
  // No-op when no live run matches; the tab then keeps its static history.
  await resumeConversationIfRunning(tab.id, tab.conversationId);
  _ensureChatRoute();
}
...
// Rebuilt V2 flow (2026-07-25): the dialog offers rounds + advanced
// message-count slicers, a title editor, tool-call filter toggle, settings
// inheritance toggle, and a live preview. Backend contract + tab-opening
// live in `useConversationFork` so both this entry point and the per-message
// button in ChatView share the same POST/upsert/openTab/toast sequence.
const forkDialogVisible = ref(false);
const forkTarget = ref<ConversationSummary | null>(null);
const { forkConversation } = useConversationFork();

function openForkDialog(conv: ConversationSummary): void {
  forkTarget.value = conv;
  forkDialogVisible.value = true;
}

async function handleForkConfirm(payload: ForkConfirmPayload): Promise<void> {
  if (forkTarget.value === null) return;
  const convId = forkTarget.value.id;
  forkDialogVisible.value = false;
  await forkConversation(convId, payload);
  // `forkConversation` opens + activates the new tab; make sure we're on the
  // /chat route so the user sees it (this component is a sibling of
  // <RouterView>; on non-chat routes the switch alone is invisible).
  _ensureChatRoute();
...
<template>
  ...
  >
        ...
        <template v-if="!searchActive">
          ...
          >
            ...
            >
              ...
              <!-- Multi-agent (discussion) conversation → distinct 👥 glyph so
                   users can tell it apart from an ordinary chat at a glance
                   (matches the ChatComposer discussion pill's 👥). Channel-source
                   conversations keep their channel icon (branches above). -->
              <span
                v-else-if="isDiscussionConv(conv)"
                class="conv-channel-icon conv-channel-icon--discussion"
                :title="t('chat.discussion.title')"
                data-testid="conv-discussion-icon"
                aria-hidden="true"
              >👥</span>
              <span
                v-else
              ...
              >
                ...
                <template v-if="convActionsExpandedFor === conv.id">
                  ...
                  <button
                    type="button"
                    class="btn btn-ghost btn-xs"
                    :title="t('sidebar.forkConversation')"
                    :aria-label="t('sidebar.forkConversation')"
                    data-testid="fork-conversation-btn"
                    @click.stop="openForkDialog(conv)"
                  >
    ...
    <div class="sidebar-footer">
      <!-- Token-pool balance (QAI Service). Above the control row so it sits
           next to the signed-in user whose allowance it reports. Renders
           itself only when a balance is known + the rail is expanded. -->
      <SidebarQuotaGauge :collapsed="ui.sidebarCollapsed" />
  ...
  <ForkConversationDialog
    :visible="forkDialogVisible"
    :conversation-id="forkTarget?.id ?? ''"
    :message-count="forkTarget?.message_count ?? 0"
    :round-count="forkTarget?.round_count ?? 0"
    :source-title="forkTarget?.title ?? ''"
    entry-mode="conversation"
    @confirm="handleForkConfirm"
    @cancel="forkDialogVisible = false"
  />
</template>
