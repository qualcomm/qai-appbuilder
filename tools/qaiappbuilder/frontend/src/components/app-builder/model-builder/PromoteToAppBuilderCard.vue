...
interface Props {
  ...
  /**
   * Count of precision variants the BACKEND has verified on disk for the
   * active conversation (``usePromoteReadyDetection().detectedVariants.length``
   * — the same truth that lights the toolbar ready-dot). Passed through to the
   * composable so it can force a re-scan when the backend detection arrives
   * even though ``sessionModelWorkdir`` did not change (root-cause fix for the
   * "dot lit but card empty until reopen" bug). Defaults to 0.
   */
...
const verifiedCountRef = toRef(props, "verifiedVariantsCount");
const promote = usePromoteToAppBuilder(
  workdirRef,
  () => {
    emit("imported");
  },
...
// ── AI-search clue block (no-bins empty state) ──────────────────────────────
// How many evidence rows the compact clue block shows before it stops. The
// full report is one click away in the chat turn the AI-search button injects,
// so the panel only needs enough for the user to recognise the situation —
// an unbounded list would push the Generate button off a short popover.
const CLUE_LIMIT = 3;

/** Subfolders the backend walked under `output/` (capped, may be empty). */
const clueSubdirs = computed<string[]>(() =>
  (diagnostics.value?.subdirs ?? []).slice(0, CLUE_LIMIT),
);

/** Weight-like files the scan saw but rejected, pre-formatted for display
 *  (size via the shared `fmtSize`, reason verbatim from the backend). */
const clueRejected = computed<string[]>(() =>
  (diagnostics.value?.rejected ?? [])
    .slice(0, CLUE_LIMIT)
    .map((r) =>
      t("modelBuilder.promote.aiSearch.cluesRejectedRow", {
        path: r.path,
...
/** True when there is at least one concrete clue worth rendering — an empty
 *  `diagnostics` object (root missing, nothing walked) would otherwise draw a
 *  bordered box with only a title in it. */
const hasClues = computed<boolean>(
  () => clueSubdirs.value.length > 0 || clueRejected.value.length > 0,
);

/**
 * Which of the two tones the always-available AI-search affordance speaks in.
 *
 * `true` (scan returned variants) → assistive "results look wrong? ask the
 * agent to double-check", rendered as a text link so it does not compete with
 * the primary Generate button on the healthy path.
 * `false` (scan came up empty) → the original strong "can't find the weights?"
 * call to action, which IS the next step there.
 *
 * Also selects the prompt: the default prompt opens with "could not find a
 * usable weight", which would contradict a variant list already on screen, so
 * the verify tone passes `{ verify: true }` for the review-worded prompt.
 */
const aiSearchVerify = computed<boolean>(() => variants.value.length > 0);
...
<template>
  <div class="promote-card">
    ...
    <!-- (b) Workspace branch ─────────────────────────────────────────── -->
    <!-- Rendered whenever the commit card is not shown. It must NOT be gated
         on the candidate-dry-run ``loading`` flag: doing so blanked the whole
         card to a bare "加载中..." on every re-scan (mount / workdir change /
         post-generate), which is what turned the panel into the empty box in
         the bug report. This stage owns its own in-flight affordances — the
         ``scanLoading`` "scanning…" line and the ``exporting`` "生成中..."
         button — so a background dry-run never leaves the user staring at an
         empty popover. -->
    <div
      v-else
    ...
    >
      ...
      <template v-else>
        ...
        <!-- No bins detected: the static "expected format" hint plus the
             compact clue block (what the backend's walk actually saw).
             `needsNormalize` above keeps priority — it has its own, more
             specific guidance. The AI-search affordance itself is NOT in this
             branch: see the standalone block below. -->
        <div
          v-else-if="!scanLoading"
        ...
        >
          ...
          <details
            v-if="hasClues"
            class="promote-card__clues"
            data-testid="promote-weight-clues"
          >
            <summary class="promote-card__clues-summary">
              {{ t("modelBuilder.promote.aiSearch.cluesTitle") }}
            </summary>
            <div
              v-if="clueSubdirs.length > 0"
              class="promote-card__clues-group"
            >
              <span class="promote-card__clues-label">{{ t("modelBuilder.promote.aiSearch.cluesSubdirs") }}</span>
              <code
                v-for="d in clueSubdirs"
                :key="`sub-${d}`"
                class="promote-card__clues-row"
              >{{ d }}</code>
            </div>
            <div
              v-if="clueRejected.length > 0"
              class="promote-card__clues-group"
            >
              <span class="promote-card__clues-label">{{ t("modelBuilder.promote.aiSearch.cluesRejected") }}</span>
              <code
                v-for="r in clueRejected"
                :key="`rej-${r}`"
                class="promote-card__clues-row"
              >{{ r }}</code>
            </div>
        ...
        <!-- AI-search / double-check affordance ────────────────────────────
             Deliberately OUTSIDE the v-if chain above. It used to live in the
             chain's LAST `v-else-if` branch, so any earlier branch winning
             (most commonly: the scan DID find variants) hid it completely —
             and "found something" is not "found the right thing". The bug
             report had one 293 MB file reported as both FP16 and FP32, with
             the FP16 row actually pointing at a 28 MB auxiliary `flow.bin`;
             that is precisely when a user needs the agent to re-check, and
             precisely when there was no way to ask.

             `hasWorkdir` is structurally implied by the enclosing
             `<template v-else>`, but stated because it is the real
             precondition: `askAiToFindWeights()` returns null without a
             workdir (it refuses to ask for an unbounded walk), so the button
             would be a dead control. `!scanLoading` keeps it out of the way
             while the answer is still arriving. `needsNormalize` keeps its
             priority: that state already names the ONE exact next action (run
             Step 6.5), so a competing "ask the agent to look around" would
             only dilute it. -->
        <div
          v-if="hasWorkdir && !scanLoading && !needsNormalize"
          class="promote-card__ai-search"
          :class="{ 'promote-card__ai-search--subtle': aiSearchVerify }"
        >
          <button
            type="button"
            class="promote-card__btn"
            :class="aiSearchVerify ? 'promote-card__btn--link' : 'promote-card__btn--secondary'"
            data-testid="promote-ai-search"
            @click="askAiToFindWeights({ verify: aiSearchVerify })"
          >
            {{ aiSearchVerify
              ? t("modelBuilder.promote.aiSearch.buttonVerify")
              : t("modelBuilder.promote.aiSearch.button") }}
          </button>
          <p class="promote-card__ai-search-hint">
            {{ aiSearchVerify
              ? t("modelBuilder.promote.aiSearch.hintVerify")
              : t("modelBuilder.promote.aiSearch.hint") }}
          </p>
        </div>
...
/* AI-search escape hatch inside the no-bins empty state.

   The clue block is a native `<details>` so it is collapsed by default (the
   empty state must stay one short line for the common case) and needs no JS
   state. Rows are `<code>` because they are paths / sizes / backend reason
   strings — same treatment as `.promote-card__needs-normalize-path`. Colour and
   size come from CSS vars so both themes stay readable. */
.promote-card__clues {
  margin-top: var(--space-1);
}
.promote-card__clues-summary {
  cursor: pointer;
  color: var(--accent);
}
.promote-card__clues-group {
  display: flex;
  flex-direction: column;
  gap: 2px;
  margin-top: var(--space-1);
}
.promote-card__clues-label {
  color: var(--text-muted);
}
.promote-card__clues-row {
  word-break: break-all;
  color: var(--text-muted);
  opacity: 0.85;
}
/* Always-available AI-search affordance. Now a standalone block (it used to be
   nested inside `.promote-card__no-bins`, which supplied the small type), so
   the hint sets its own `font-size` — otherwise pulling it out of that parent
   would have silently grown the text to card size. */
.promote-card__ai-search {
  margin-top: var(--space-1);
}
.promote-card__ai-search-hint {
...
/* "Scan found variants" tone: a text-link button + dimmed hint, so the
   double-check escape hatch stays reachable on the healthy path without
   drawing attention away from the primary Generate action below it. */
.promote-card__ai-search--subtle {
  margin-top: 0;
}
.promote-card__ai-search--subtle .promote-card__btn {
  font-size: var(--text-xs);
  padding-block: 0;
}
.promote-card__ai-search--subtle .promote-card__ai-search-hint {
  margin-top: 2px;
  opacity: 0.85;
}
