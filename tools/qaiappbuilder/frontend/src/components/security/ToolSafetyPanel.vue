 ...
 *   Layer 2 — Policy Guard (FileGuard / PolicyCenter): only the orthogonal
 *             `allow_exec_tool` gate. The FileGuard master on/off
 *             (`file_guard_enabled` + `native_file_guard_enabled`) is driven
 *             by the Overview "Security mode" 3-state control — the single
 *             FileGuard switch (2026-07 dedup) — not a duplicate toggle here.
...
import ExecProfilesTable from "@/components/security/ExecProfilesTable.vue";
...
// ── Layer 2 ──
// The FileGuard master on/off (file_guard_enabled + native_file_guard_enabled)
// is now driven solely by the Overview "Security mode" 3-state control (the
// single FileGuard switch). This panel keeps only the orthogonal
// allow_exec_tool gate below.
...
<template>
  <div class="tool-safety-panel">
    ...
    <!-- Layer 2 — Policy Guard. The FileGuard master on/off lives in the
       Overview "Security mode" 3-state control (the single FileGuard switch);
       this section keeps only the orthogonal exec-tool gate. -->
    <section class="sec-section">
    ...
    <section class="sec-section">
      ...
      <!-- The content behind the switch above: the profiles the exec
           broker actually loaded. Read-only (static compiled TOML assets,
           no write endpoint) + a reload button. Lived at the bottom of the
           Skill panel before — wrong domain (profiles gate commands, not
           skills) and it only rendered the legacy allowed/denied arg lists,
           which every shipped profile leaves empty, so every row showed
           "—". -->
      <div class="tool-safety-row tool-safety-row--column">
        <ExecProfilesTable />
      </div>
