...
/**
 * Localised name / description for a built-in policy template.
 *
 * The backend catalogue (`GET /api/security/templates`, static
 * `config/policy_templates/{id}.*`) ships English-only `name`/`description`.
 * We translate by the stable template `id` (demo / development / strict) and
 * fall back to the backend text when the UI has no translation for that id
 * (e.g. a future template) so it degrades to untranslated rather than blank.
 */
function templateName(tpl: { id: string; name: string }): string {
  const key = `policyTemplates.items.${tpl.id}.name`;
  return te(key) ? t(key) : tpl.name;
}
function templateDescription(tpl: { id: string; description: string }): string {
  const key = `policyTemplates.items.${tpl.id}.description`;
...
/**
 * "AI may access the project directory" boolean derived from the current
 * policy. TRUE (the default) when NO rule with id ``ui-project-access-deny``
 * exists; FALSE when the deny rule is present. The row is a normal
 * ``${'{'}PROJECT_ROOT{'}'}/*`` op=any deny that also shows up in the
 * Allow Lists tab — this switch is a discoverable shortcut, not a
 * separate config bucket.
 */
const projectAccessEnabled = computed<boolean>(() => {
  const rules = store.policy?.rules ?? [];
  return !rules.some((r) => r.rule_id === "ui-project-access-deny");
});

function onProjectAccessToggle(ev: Event): void {
  const target = ev.target as HTMLInputElement | null;
  const wantEnabled = target?.checked ?? true;
  void store.setProjectAccess(wantEnabled);
}
  ...
  <div class="security-section">
    ...
    <template v-else>
      ...
      <div class="sec-cfg-status-card">
        ...
        <!-- Clear-policy escape hatch (2026-07 UX fix). Applying any
             template locks the user into that ruleset; the pre-rewrite
             ``strict`` template shipped a ``*`` catch-all deny that
             silently suppressed the ASK dialog, and there was no in-UI
             way out. This button resets the rule set to empty so the
             ambient allow cascade (workspace / skill / auto-approve /
             file_guard_paths / grants) drives decisions and misses
             prompt via ``dynamic_authorization``. Idempotent — a
             second click on an already-empty ruleset is a no-op. -->
        <div class="sec-cfg-status-row sec-cfg-status-row--footer">
          <div class="sec-cfg-status-label">
            <div class="sec-cfg-status-name">
              {{ t('policyTemplates.clearTitle') }}
            </div>
            <div class="sec-cfg-status-hint">
              {{ t('policyTemplates.clearHint') }}
            </div>
          </div>
          <button
            type="button"
            class="btn btn-ghost btn-sm"
            :disabled="(store.policy?.rules?.length ?? 0) === 0"
            @click="store.clearPolicyRules()"
          >
            {{ t('policyTemplates.clearBtn') }}
          </button>
        </div>
      </div>

      <!-- Project-directory access switch — expose the coarse-grained
           control users expect ("stop the AI from touching my source
           tree") without a dedicated bucket. Implemented as a normal
           policy rule (see store.setProjectAccess) so the exact same
           row is also visible in the Allow Lists tab. -->
      <div class="sec-cfg-status-card">
        <div class="sec-cfg-block-header">
          <div class="sec-cfg-block-title">
            📁 {{ t('security.projectAccess.title') }}
          </div>
        </div>
        <div class="sec-cfg-list-desc">
          {{ t('security.projectAccess.hint') }}
        </div>
        <div class="sec-cfg-status-row">
          <div class="sec-cfg-status-label">
            <div class="sec-cfg-status-name">
              {{ t('security.projectAccess.rowTitle') }}
            </div>
            <div class="sec-cfg-status-hint">
              {{
                projectAccessEnabled
                  ? t('security.projectAccess.rowHintOn')
                  : t('security.projectAccess.rowHintOff')
              }}
            </div>
          </div>
          <label class="sec-depbroker-switch">
            <input
              type="checkbox"
              :checked="projectAccessEnabled"
              @change="onProjectAccessToggle"
            />
            <span>
              {{
                projectAccessEnabled
      ...
      >
        ...
        >
          ...
          <!-- Read-only pill: the native hook state is DERIVED from the
               top-level FileGuard master switch + settings. The 🔒 icon
               makes it visually clear the value cannot be flipped from
               this row (unlike the neighbouring toggle controls). -->
          <span
            class="sec-native-status"
            :class="`sec-native-status--${nativeHookState}`"
            :title="t('security.nativeHook.readOnlyTitle')"
          >
            <span
              aria-hidden="true"
              class="sec-native-status-lock"
            >🔒</span>
