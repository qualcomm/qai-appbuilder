...
<template>
  <!-- 1) installed → installed pill + path + copy + delete
       ``isInstalled`` comes from the DISK scan and therefore outranks the
       in-memory ``done`` download state: once the zip has been unzipped into
       ``models/`` the installer DELETES it (V1 parity), so a lingering
       ``done`` entry describes an artifact that no longer exists. Gating this
       branch on ``!isDone`` used to push an already-installed model back into
       the done-state block below, which re-offered "install to models" and
       failed with ``invalid save_path`` against the deleted zip. Only an
       ACTIVE transfer (preparing / downloading) may outrank the disk truth. -->
  <template v-if="isInstalled && !isDownloadActive">
  ...
  >
    ...
    <template v-else>
      ...
      <!-- no install attempt yet → delete the downloaded zip (V1 line 614).
           ``!isInstalled`` is implied here: branch 1 now claims every
           installed model, so reaching the done-state block means the zip is
           still un-installed. -->
      <button
        v-else
        type="button"
