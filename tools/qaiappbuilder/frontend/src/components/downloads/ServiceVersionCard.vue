...
.dc-card__path {
  ...
  /* ``flex-basis: 0``, NOT ``auto``: with ``auto`` the basis is the path's full
     content width, so this item never yields space and the row's
     ``flex-wrap: wrap`` pushes the ⟨📋 复制路径⟩ / ⟨删除版本⟩ buttons onto their own
     line (measured: row height 27px -> 110px at 1100px and below). With basis 0
     the path absorbs the shrink and the buttons stay inline at every width. */
  flex: 1 1 0;
  min-width: 0;
}
...
/* Keep the action buttons at their intrinsic width and their labels on one
   line, so only ``.dc-card__path`` above gives up space. Same convention as
   ModelCardActions.vue's save-path row (:463-471). */
.dc-card__row > .btn {
  flex-shrink: 0;
  white-space: nowrap;
}
