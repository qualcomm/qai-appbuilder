...
export type StreamFrameType =
  ...
  | "network_retry"
  | "inject_as_new_turn"
  // Emitted by the cloud LLM stream (`llm_stream.py` — search
  // `StreamFrame.quota_usage`) carrying the provider's remaining-quota
  // snapshot. `FRAME_HANDLERS` has had `handleQuotaUsage` all along; this union
  // simply never listed the type, so `frame.frame_type === "quota_usage"` was
  // not a valid narrowing (the dispatch table is keyed by plain `string`, which
  // is why `vue-tsc` never flagged it).
  | "quota_usage";
...
/** Payload of an `aside_message` frame (backend aside seam).
 *  Emitted when the streaming loop folds a producer-owned aside notification
 *  (for example a context-recovery notice) into the current turn.
 *  Renders as a small system-note card, NOT a user bubble. */
export interface AsideMessageFramePayload {
  readonly text: string;
  /** Sub-kind — "system_note" (default), "bg_completion", etc. */
...
export interface ToolResultFramePayload {
  ...
  readonly sender_id?: string;
  /** Auto-background marker (backend appended field §3.1). `true` means the
   *  command outran the foreground-wait threshold while fresh user input
   *  arrived, so the still-running child was handed to the background-process
   *  manager and this tool call returned early. The command has NOT finished
   *  — do NOT paint a "done" card. Absent on ordinary completions. Paired
   *  with `background_process_id` naming the job the model now owns. Present
   *  as a STRUCTURED field so the UI never has to sniff the `[Auto-parked]`
   *  prefix out of `result`. */
  readonly backgrounded?: boolean;
  /** ID of the background-process job this call was adopted into. Present
   *  iff `backgrounded === true`. Shape `bgp_01KZ…`. */
  readonly background_process_id?: string;
  /** Semantically-distinct successful terminal outcome (backend appended field
   *  §3.1). `"stopped"` = background_process(action=stop) terminated the job;
   *  `"restarted"` = background_process(action=restart) replaced it. `ok`
   *  stays true because nothing failed — the flag exists so the UI does not
   *  paint a generic "done" tick over an outcome that means something else.
   *  Absent on ordinary completions. Same pattern as `backgrounded`. */
...
export interface SpeakerChangedFramePayload {
  ...
  /** Coordinator (主持人) marker — true when this floor is the unified
   *  moderator+judge coordinator turn (NOT a borrowed participant role). The UI
   *  renders a fixed host identity + distinct badge. Tail-appended (§3.1);
   *  absent ⇒ false, so ordinary speaker bubbles are unchanged. */
...
/** Payload of an `inject_as_new_turn` frame (backend
 *  `StreamFrame.inject_as_new_turn`, V2 enhancement). Emitted at the end of
 *  a turn's normal-completion path when the injection registry still holds
 *  pending texts the run loop never drained (the user pressed Ctrl+Enter
 *  after the last tool round, or during a turn that never called any tool,
 *  so the inter-round seam never fired). The frontend consumes this by
 *  dropping the tab's pending injection bubbles and submitting the merged
 *  text as an ordinary user turn — byte-identical to pressing Enter. */
export interface InjectAsNewTurnFramePayload {
  /** FIFO-ordered pending texts drained just before the run loop cleared the
   *  registry.  Non-empty and each entry non-empty. */
