...
// element-plus theme — imported here (before app-level scoped overrides) so
// the base tokens land first and our scoped CSS can override them per widget.
// Both themes are pulled in: the light one takes effect by default; the dark
// one is gated on ``html.dark`` (see composables/useTheme.ts which flips that
// class in lockstep with the app's own ``html.light`` gate — element uses the
// opposite convention, so the two classes are always mutually exclusive).
