...
class QnnContext:
    ...
    def load(
        ...
        *,
        ...
        # Two known API versions exist in the wild:
        #   OLD (qai_appbuilder < 2.47):
        #     Config(qnn_lib_path, runtime, log_level, profiling_level, log_path)
        #   NEW (qai_appbuilder >= 2.47):
        #     Config(runtime, log_level, profiling_level, log_path)
        #   (the old version has lib_dir as the FIRST positional param)
        #
        # We use keyword arguments to be safe across both versions.  The old
        # version accepts `runtime=` / `log_level=` / `profiling_level=` by
        # keyword too, so kwargs work universally.
        #
        ...
        if not _qnn_configured and hasattr(m, "QNNConfig"):
            try:
                ...
                if pl_val is not None:
                    kwargs["profiling_level"] = pl_val
                m.QNNConfig.Config(**kwargs)
