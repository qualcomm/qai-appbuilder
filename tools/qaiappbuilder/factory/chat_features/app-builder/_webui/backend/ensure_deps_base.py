...
def _parse_requirements(text: str) -> list[dict]:
    """Parse requirements.txt into ``[{spec, dist, import_name, version_op, version}]`` entries.
    ...
    Also extracts the version constraint (if any) for runtime version checking.
    """
    out: list[dict] = []
    for raw in text.splitlines():
        ...
        for op in ("==", ">=", "<=", "~=", "!=", ">", "<"):
            ...
            if idx != -1:
                ...
                # Version ends at comma, semicolon, space, or bracket.
                ver_chars: list[str] = []
                for ch in rest:
                    if ch in (",", ";", " ", "[", "]"):
                        break
                    ver_chars.append(ch)
                version = "".join(ver_chars).strip()
                break
        ...
        out.append({
            "spec": line, "dist": dist, "import_name": import_name,
            "version_op": version_op, "version": version,
        })
    return out
...
def _version_tuple(v: str) -> tuple[int, ...]:
    """Parse '2.48.40' -> (2, 48, 40). Non-numeric parts become 0."""
    parts: list[int] = []
    for p in v.split("."):
        try:
            parts.append(int(p))
...
def _version_satisfied(entry: dict) -> bool:
    """Check whether the installed version of a package meets the spec constraint.

    Only checks ``>=`` and ``==`` (the constraints that matter for SDK compat).
    Other operators or missing version info → assumed satisfied (let pip decide
    on the next install attempt).
    """
    op, ver = entry.get("version_op", ""), entry.get("version", "")
    if not op or not ver:
        return True  # no constraint → satisfied
    if op not in (">=", "=="):
        return True  # only check >= and == ; others are rare in our usage

    # Get installed version via importlib.metadata (stdlib since 3.8).
    try:
        from importlib.metadata import version as _get_version  # noqa: PLC0415
        installed = _get_version(entry["dist"])
    except Exception:  # noqa: BLE001
        return True  # can't determine → assume OK, let pip sort it out

    inst_t = _version_tuple(installed)
    req_t = _version_tuple(ver)
    if op == ">=":
        return inst_t >= req_t
    if op == "==":
        return inst_t == req_t
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Also flag packages that are importable but don't meet version constraints
    # (e.g. qai-appbuilder>=2.48.40 installed at 2.46.0 → needs upgrade).
    outdated = [
        e for e in entries
        if e not in missing
        and _is_importable(e["import_name"])
        and not _version_satisfied(e)
    ]
    if outdated:
        names = ", ".join(f"{e['dist']}({e['version_op']}{e['version']})" for e in outdated)
        _log(f"version outdated: {names}")
    need_install = missing + outdated
    if need_install:
        names = ", ".join(e["dist"] for e in need_install)
        _log(f"installing: {names}")
        failed = _install_missing(need_install)
        if failed:
            return _fatal(
                "could not install: "
                + ", ".join(failed)
