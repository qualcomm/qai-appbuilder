...
class InferenceSession:
    ...
    def __init__(self, model_path: str, sess_options: Optional[SessionOptions] = None, providers: Optional[List[str]] = None):
        ...
        if (not InferenceSession._qnn_initialized) or (InferenceSession._last_config_key != key):
            ...
            # qai_appbuilder 2.47: Config(runtime, log_level, profiling_level).
            # The old leading lib-dir arg was removed (internal libs/ is used);
            # qnn_libs_dir is still ensured on PATH above for the QNN DLLs.
            QNNConfig.Config(
    ...
    def run(self, output_names: Optional[List[str]], input_feed: Dict[str, np.ndarray], run_options: Optional[Any] = None) -> List[np.ndarray]:
        ...
        # HTP BURST is held resident for the whole session (set in __init__,
        # released in __del__), so run() no longer toggles the perf profile.
