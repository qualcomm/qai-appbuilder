...
def main() -> int:
    ...
    # --- Host-aware wrapper check ---
    # On linux-x64 (no qai_appbuilder wheel), onnxwrapper.py MUST be the x86
    # CPU variant. On windows-x64 the unified onnxwrapper.py + qai_appbuilder
    # x64 wheel handle CPU internally via data/config/host_arch, so no swap.
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    if _script_dir not in sys.path:
        sys.path.insert(0, _script_dir)
    from _host_arch import detect_host_os as _detect_host_os  # noqa: E402
    _host_os = _detect_host_os()
    _wrapper_path = os.path.join(repo_dir, "onnxwrapper.py")
    if _host_os == "linux-x64" and os.path.exists(_wrapper_path):
        with open(_wrapper_path, encoding="utf-8") as _wf:
            if "qai_appbuilder" in _wf.read():
                sys.exit(
                    ...
                    "This host is linux-x64. Use the x86 wrapper instead:\n"
