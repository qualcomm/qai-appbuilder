...
tier: base
triggers: ["0-byte qnn-context-binary-generator.exe", "WinError 193", "SDK file modification required", "B9", "SDK file corrupted"]
sources: ["SKILL.md 179-217", "SKILL.md B8/B9"]
---

# SDK Integrity & Recovery (base)

> 🧭 Diagnosis framework: `${APP_ROOT}/factory/chat_features/model-builder/troubleshooting/_diagnosis-framework.md`; this SKILL is the symptom library for the "SDK write-protection / corruption recovery" domain.
...
- **B8** — context binary generation fails on an HTP host (`windows-arm64` / `linux-aarch64`). 0-byte generator = damaged SDK file → self-heal / manual recovery. Diagnose READ-ONLY.
