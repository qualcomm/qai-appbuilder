...
| Context Binary | `output/{MODEL_NAME}_<precision>.bin` (>= 1 MB) — QNN HTP context binary; runs on real HTP on ARM64 hosts. On x64 hosts the same artifacts apply — loading rules see `${APP_ROOT}/factory/chat_features/_shared/x64-host-notes.md` |
...
> Only produced when the user explicitly asks for the DLL pipeline or specifically needs the `.dll` artifact. **`windows-arm64` host only** — errors out elsewhere.
