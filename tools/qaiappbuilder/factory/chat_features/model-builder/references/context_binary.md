...
`qai_dev_gen_contextbin_x86.py` is the host-side context binary generator invoked automatically by `run_pipeline.py` and `qai_dev_gen_contextbin.py` on non-native-HTP hosts. On x64 hosts, backend selection (HTP simulator vs CPU) and the legacy wrapper-default bug are documented in `${APP_ROOT}/factory/chat_features/_shared/x64-host-notes.md` §3. Direct call:
...
- `--backend`: see `x64-host-notes.md` §3 for x64 backend-selection rules and the wrapper's legacy default. On `linux-aarch64` `--backend htp` uses the native aarch64 HTP.
