...
Parent-process reboot loop replacing the legacy ``start_server.py``
...
Usage::
    ...
    python -m apps.cli.serve --host 0.0.0.0 --port <backend>  # <backend> = factory/config/ports.json backend
...
from qai.platform.config.ports import fallback_ports as _registry_fallback_ports
...
from qai.platform.process import is_admin
...
#
# * ``legacy_packaged`` first — the legacy packaged default, retained here
#   for backward compatibility with existing dev workflows / CORS allow-list
#   entries (the CORS default still lists the ``legacy_packaged`` origin
#   alongside the ``backend`` one). NOTE: this is NO LONGER the documented
#   default port — the documented default is ``ServerSettings.port`` (the
#   ``factory/config/ports.json`` ``backend`` key, pinned by the Okta redirect_uri).
#   ``Start.bat`` reads that ``backend`` port from ``factory/config/ports.json`` and
#   passes it as ``--port`` explicitly so the SSO-critical bind path never
#   consults this fallback list. FALLBACK_PORTS only runs when ``--port``
#   is omitted (bare ``python -m apps.cli.serve``); the first entry is
#   deliberately left at ``legacy_packaged`` to avoid silently changing that
#   dev scenario's behaviour.
...
#
# The first entry (``legacy_packaged``) is paired with same-tail offsets so
# the fallback URL stays recognisable for users familiar with the legacy
# packaged default.
#
# The list is sourced from the language-neutral ``factory/config/ports.json``
# registry (``qai.platform.config.ports`` — pure stdlib, so importing it
# here does NOT pull in the pydantic Settings stack the supervisor must stay
# free of). Edit the ``fallbacks`` array in ``factory/config/ports.json``; a
# missing / malformed file degrades gracefully to the built-in defaults
# (which mirror the JSON) so the supervisor can always start.
FALLBACK_PORTS: tuple[int, ...] = _registry_fallback_ports()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Refuse to run elevated. Files created by an elevated launch (venv
    # caches, ``data/``, endpoint file, logs) become Administrators/root
    # owned, and a later non-elevated run fails to update them — a foot-gun
    ...
    if args.no_supervisor:
        ...
        # FastAPI app factory (``create_app`` → ``load_settings``) sees the
        # SAME port. Critical: the auth router derives Okta ``redirect_uri``
        # from ``settings.server.port`` (build_auth_router server_port=...);
        # without this bridge a supervisor-picked fallback would bind the
        # right socket but advertise the WRONG redirect_uri to Okta, and
        # every SSO login would fail with "redirect_uri mismatch".
        os.environ["QAI_SERVER__PORT"] = str(resolved_port)
...
class _Supervisor:
    ...
    def run(self) -> int:
        ...
        code = self._run_loop()
        # V1 parity: when the user chose to exit via
    ...
    def _run_loop(self) -> int:
        ...
        while True:
            ...
            # Inject the resolved port into the child environment on TWO
            # channels so every consumer sees the ACTUAL bound port even
            # when the FastAPI app factory re-loads Settings without the
            # CLI ``--port`` override (which happens under
            # ``uvicorn.run(factory=True)`` because
            # ``_create_app_for_uvicorn()`` → ``create_app()`` calls
            # ``load_settings()`` fresh, without the CLI overrides that
            # ``main()`` applied):
            #
            # 1. ``QAI_RUNTIME_PORT`` — read by ``apps/api/lifespan.py`` so
            #    the runtime endpoint file (``data/runtime/server.endpoint.json``)
            #    contains the URL users can actually reach.
            # 2. ``QAI_SERVER__PORT`` — the pydantic-settings-native env
            #    override for ``settings.server.port``. WITHOUT this, the
            #    factory-built app has ``settings.server.port`` = the
            #    ``factory/config/ports.json`` ``backend`` default (8989)
            #    while uvicorn actually bound a fallback (e.g. 28688). The
            #    auth router derives Okta ``redirect_uri`` from
            #    ``settings.server.port`` (build_auth_router server_port=...),
            #    so a mismatch there means every SSO login round-trip fails
            #    with "redirect_uri mismatch" even though the socket is up.
            #    Both fallback candidates MUST be registered with Okta —
            #    see ``factory/config/ports.json`` ``fallbacks`` note.
            os.environ["QAI_RUNTIME_PORT"] = str(resolved_port)
            os.environ["QAI_SERVER__PORT"] = str(resolved_port)
    ...
    def _wait_child(self) -> int:
        ...
        menu. (V1 polled at 0.2s for the same
...
def _resolve_bindable_port(
    ...
    *,
    ...
    in order. The documented default is the ``factory/config/ports.json`` ``backend``
    port (pinned by the Okta redirect_uri — see
    ``qai.platform.config.settings.ServerSettings.port``); the fallback
    list's first entry (``factory/config/ports.json`` ``legacy_packaged``) is the
    legacy packaged default retained for backward compatibility and only
    consulted when ``--port`` is omitted.
