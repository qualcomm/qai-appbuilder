...
"""``qai conv`` subcommands — chat conversation / tab / snapshot CRUD.
...
``UploadImageUseCase`` / ``EnhancePromptUseCase``) are **deliberately excluded**
— they require a long-lived API server / WebSocket pipe / inbound HTTP request
body.

Behavioural notes
-----------------
* All commands print JSON to stdout (``ensure_ascii=False`` for Chinese
  conversation titles). Errors / progress logs go to stderr per
...
* All identifiers are passed through the corresponding domain VO
  (``ConversationId.of`` / ``TabId.of``) so input validation (length /
  control-char) is done at the domain boundary, not ad-hoc in the CLI.
"""
...
def _default_json(obj: Any) -> Any:
    ...
    aggregates — frozen / slotted dataclasses with nested VOs
    (``ConversationId(value=...)``). Walk them via ``asdict`` when
    possible, fall through to ``__dict__`` / ``str`` for non-dataclass
    domain objects.
    """
...
def _emit(payload: Any) -> None:
    ...
    contains Chinese conversation titles, and Windows attaches a cp1252 /
    cp936 codec to ``sys.stdout`` by default which cannot encode arbitrary
    CJK / emoji.  ``reconfigure(encoding=
...
def _cmd_conv_compact(args: argparse.Namespace) -> int:
    """CLI ``qai conv compact`` — Task T retirement of the legacy decision-only
    path. Now fires a real force-compact pass via
    :class:`ForceCompactChatUseCase` (same use case ``POST /compact`` and
    channels ``/compact`` drive), reporting before/after tokens + savings.
    """
    ...
    async def _go(c: Container) -> Any:
        ...
        request = ForceCompactChatInput(**kwargs)
        force_uc = c.chat.force_compact_chat_use_case
        if force_uc is None:
            raise RuntimeError(
                "force_compact_chat_use_case not wired on container",
            )
        return await force_uc.execute(request)
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    conv = subparsers.add_parser(
        "conv",
        help="chat conversation / tab CRUD (non-streaming)",
        description=(
            "Operate on chat conversations, tabs and prompt snapshots. "
            "Streaming chat is intentionally not exposed here — run "
            "``qai serve`` and use the WS / SSE API."
        ),
    )
    ...
    p_compact = sub.add_parser(
        "compact",
        help="force a compaction pass on a conversation (before/after tokens + savings)",
    )
    ...
    p_compact.add_argument(
        ...
        type=int,
        default=200_000,
        help="fallback token budget (default 200000); overridden by --model-id when set",
    )
    p_compact.add_argument(
        "--model-id",
        default=None,
        help="model id used to resolve the real context window (e.g. 200K)",
    )
