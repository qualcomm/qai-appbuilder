...
def _hint_pack_expectations(manifest: Any, err: Any) -> None:
    ...
    if kind:
        err.write(t("cli.app.hint.input_kind", kind=kind, flag=kind_to_input_key(kind) or kind))
    variants = getattr(manifest, "variants", ()) or ()
    if variants:
        ids = ", ".join(str(getattr(v, "id", "")) for v in variants)
        err.write(t("cli.app.hint.variants", ids=ids))
    params = getattr(manifest, "params", ()) or ()
    if params:
        err.write(t("cli.app.hint.params_header"))
        for p in params:
...
def _write_output_artifact(
    ...
    if artifact and Path(str(artifact)).is_file():
        ...
        err.write(t("cli.app.ok.wrote_artifact", dest=dest))
    else:
        dest.write_text(
            json.dumps(output or {}, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        err.write(t("cli.app.ok.wrote_output_json", dest=dest))
    err.flush()
...
def _copy_annotated_artifact(
    ...
    if artifact and Path(str(artifact)).is_file():
        ...
        err.write(t("cli.app.ok.wrote_annotated", dest=dest_path))
    else:
        err.write(t("cli.app.error.no_annotated"))
    err.flush()
...
def cmd_app(args: argparse.Namespace) -> int:
    ...
    async def _go(c: Container) -> int:
        ...
        try:
            ...
            sys.stderr.write(t("cli.app.error.pack_not_found", pack=args.pack))
            return 1
    ...
    async with repl_container(
        ...
        while True:
            try:
                ...
                sys.stdout.write(t("cli.app.hint.cancel_prompt"))
                continue
...
def _print_repl_banner(state: _AppReplState) -> None:
    ...
    sys.stdout.write(t("cli.app.hint.repl_banner", pack=state.pack, kind=kind))
    sys.stdout.flush()
...
async def _repl_run_turn(
    ...
    if input_key in _FILE_INPUT_KEYS:
        ...
        p = Path(value)
        if not p.exists():
            sys.stdout.write(t("cli.app.error.file_missing", value=value))
            return
    inputs = build_inputs(
        input_key=input_key,
        input_value=value,
    ...
    try:
        ...
        sys.stdout.write(t("cli.app.hint.turn_interrupted"))
        return
...
def _build_repl_dispatcher(
    ...
    async def _model(rest: str) -> bool:
        new_pack = rest.strip()
        if not new_pack:
            sys.stdout.write(t("cli.app.ok.current_pack", pack=state.pack))
            return True
        state.pack = new_pack
        state.manifest = await _load_manifest(c, new_pack)
        state.variant = None
        sys.stdout.write(t("cli.app.ok.switched_pack", pack=new_pack))
        _print_repl_banner(state)
        return True

    async def _variant(rest: str) -> bool:
        state.variant = rest.strip() or None
        sys.stdout.write(t("cli.app.ok.variant_set", variant=state.variant or t("cli.app.hint.variant_default")))
        return True
    ...
    async def _param(rest: str) -> bool:
        ...
        sys.stdout.write(t("cli.app.ok.params_set", params=json.dumps(state.params, ensure_ascii=False)))
        return True

    async def _params(rest: str) -> bool:
        sys.stdout.write(t(
            "cli.app.ok.params_status",
            params=json.dumps(state.params, ensure_ascii=False),
            variant=state.variant or t("cli.app.hint.variant_default"),
        ))
        return True
    ...
    async def _examples(rest: str) -> bool:
        ...
        if not examples:
            sys.stdout.write(t("cli.app.hint.no_examples"))
            return True
        choice = rest.strip()
        if not choice:
            for i, ex in enumerate(examples):
                sys.stdout.write(f"  [{i}] {getattr(ex, 'name', '')}\n")
            sys.stdout.write(t("cli.app.hint.examples_usage"))
            return True
        ...
        try:
            ...
            sys.stdout.write(t("cli.app.error.bad_example_index", idx=repr(parts[0])))
            return True
        ...
        if override:
            ...
            if input_key is None:
                sys.stdout.write(t("cli.app.error.no_input_override"))
                return True
            ...
            if missing:
                sys.stdout.write(
                    t("cli.app.error.example_files_missing_header", idx=idx)
                    + "".join(t("cli.app.error.example_files_missing_item", m=m) for m in missing)
                    + t("cli.app.hint.example_override_usage", idx=idx)
                )
                return True
    ...
    async def _history(rest: str) -> bool:
        ...
        if uc is None:
            sys.stdout.write(t("cli.app.error.history_unavailable"))
            return True
        runs = await uc.execute(limit=20, offset=0)
        if not runs:
            sys.stdout.write(t("cli.app.hint.no_history"))
            return True
        for run in runs:
            sys.stdout.write(
                f"  {getattr(run, 'id', '?')}  "
    ...
    async def _last(rest: str) -> bool:
        if state.last_result is None:
            sys.stdout.write(t("cli.app.hint.no_last_result"))
            return True
        _render_result(
    ...
    async def _out(rest: str) -> bool:
        if state.last_result is None or state.last_result.output is None:
            sys.stdout.write(t("cli.app.hint.no_output_to_export"))
            return True
        dest = rest.strip()
        if not dest:
            sys.stdout.write(t("cli.app.hint.out_usage"))
            return True
    ...
    dispatcher.register("model", t("cli.app.help.model"), _model)
    dispatcher.register("variant", t("cli.app.help.variant"), _variant)
    dispatcher.register("param", t("cli.app.help.param"), _param)
    dispatcher.register("params", t("cli.app.help.params"), _params)
    dispatcher.register("examples", t("cli.app.help.examples"), _examples)
    dispatcher.register("history", t("cli.app.help.history"), _history)
    dispatcher.register("last", t("cli.app.help.last"), _last)
    dispatcher.register("out", t("cli.app.help.out"), _out)
    dispatcher.register("help", t("cli.app.help.help"), _help)
    dispatcher.register("exit", t("cli.app.help.exit"), _exit, aliases=("quit",))
    return dispatcher
