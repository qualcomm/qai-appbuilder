...
def _resolve_provider_fields(
    ...
    *,
    ...
    if interactive:
        ...
        if not ptype:
            sys.stdout.write(
                t("cli.config.provider.prompt.type_header")
                + ", ".join(
                    f"{i + 1}) {t(f'cli.config.provider.label.{k}')}"
                    for i, k in enumerate(sorted(_PROVIDER_PRESETS))
                )
                + "\n"
            )
            choice = _prompt(t("cli.config.provider.prompt.type_choice")).strip()
        ...
        if base_url is None:
            _base = preset.get("base_url", "")
            base_url = _prompt(
                t("cli.config.provider.prompt.endpoint", base_url=_base),
                default=_base,
            )
        if api_key is None:
            api_key = _prompt("API Key › ", secret=True) or None
        if default_model is None:
            _default = preset.get("default_model", "")
            default_model = _prompt(
                t("cli.config.provider.prompt.default_model", default_model=_default),
                default=_default,
            )
...
def cmd_config_setup(args: argparse.Namespace) -> int:
    ...
    """
    if not _is_tty():
        sys.stderr.write(t("cli.config.setup.error.needs_tty"))
        return 2

    async def _wizard() -> int:
        async with cli_container(**_runtime_kwargs(args)) as c:
            sys.stdout.write(t("cli.config.setup.welcome"))
            # [1] cloud providers
            sys.stdout.write(t("cli.config.setup.step1_header"))
            if _confirm(t("cli.config.setup.prompt.add_provider")):
                while True:
                    await _setup_one_provider(c, args)
                    if not _confirm(t("cli.config.setup.prompt.add_another"), default=False):
                        break
            # [2] preferences
            sys.stdout.write(t("cli.config.setup.step2_header"))
            lang = _prompt(t("cli.config.setup.prompt.language"), default="zh-CN")
            await c.user_prefs.save_document_use_case.execute(
                "ui", updates={"language": lang}
            )
            sys.stdout.write(t("cli.config.setup.done"))
            return 0
...
async def _setup_one_provider(c: Container, args: argparse.Namespace) -> None:
    ...
    if not provider_id:
        sys.stdout.write(t("cli.config.setup.skipped_no_id"))
        return
    await c.model_catalog.update_provider_config_use_case.execute(
        UpdateProviderConfigCommand(provider_id=provider_id, config=config)
    )
    sys.stdout.write(t("cli.config.setup.probing"))
    probe = await c.model_catalog.probe_provider_use_case.execute(
        ProbeProviderCommand(provider_id=provider_id)
    )
    if probe.ok:
        models = ", ".join(probe.model_ids[:5]) or t("cli.config.setup.no_model_list")
        sys.stdout.write(
            t(
