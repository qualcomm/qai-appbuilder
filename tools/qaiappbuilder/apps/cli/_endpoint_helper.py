...
* Same logic is reused by ``cleanup-stale`` to reap a previous server
  ...
  ``netstat | findstr :<port> | taskkill`` heuristic that hard-coded a
  single static port.
...
# App Builder spawns standalone preview servers
# (``python -m uvicorn backend.main:app --host 127.0.0.1 --port <P>``) via the
# API *host*, NOT the supervisor, and on a DISTINCT port pool from the daemon's
# ``FALLBACK_PORTS``. When the host exits non-gracefully (Task Manager, crash,
# window force-close) those previews are neither reaped by the lifespan
# shutdown nor caught by the daemon port sweep above — so they accumulate
# across runs (the "many leftover python.exe" symptom). We reap them here by
# process IDENTITY (command line), because an orphaned preview may have already
# lost its listening socket (two PIDs "on" one port) and a port scan would miss
# it entirely. Imported lazily-safe: the tuple is a plain constant.
try:
    from qai.platform.net.port_allocator import (
        DEFAULT_FALLBACK_PORTS as _APP_PREVIEW_PORTS,
    )
except Exception:  # noqa: BLE001 — reaper must never block launch on import
    _APP_PREVIEW_PORTS = (
        18420,
        18421,
        18422,
...
def _pids_listening_on_ports(ports: "tuple[int, ...]") -> set[int]:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) < 5:
            continue
        local_addr = parts[1]  # e.g. ``0.0.0.0:<port>`` / ``127.0.0.1:<port>``
        # Match the ``:<port>`` suffix exactly so a short port does not also
        # match a longer one that ends with the same digits (substring trap).
...
def _sweep_fallback_ports(ports: "tuple[int, ...]") -> int:
    ...
    if killed:
        ...
        # the freed default port (the factory/config/ports.json ``backend`` when
        # ``Start.bat`` pins it via ``--port``; otherwise the first bindable
        # entry of ``FALLBACK_PORTS``) instead of racing the teardown and picking
...
# ---------------------------------------------------------------------------
# App Builder preview sweep (identity-based, not port-based).
#
# The daemon port sweep above cannot reap orphaned App Builder previews:
#   1. previews live on a DISTINCT port pool (``DEFAULT_FALLBACK_PORTS``,
#      18420-18429), not the daemon ``FALLBACK_PORTS``; and
#   2. an orphaned preview often no longer LISTENS on its port (it lost /
#      never re-bound the socket but the python.exe lingers at 0% CPU — the
#      "two PIDs on one port" seen in Task Manager), so ``netstat`` can't
#      find it.
# We therefore match on the command line: a python running
# ``-m uvicorn <module> ... --port <P>`` where ``<P>`` is one of the preview
# ports. This is deliberately narrow (both the ``uvicorn`` marker AND a
# preview port must be present) so we never kill an unrelated python.
# ---------------------------------------------------------------------------


def _pids_matching_app_preview(ports: "tuple[int, ...]") -> set[int]:
    """Return PIDs of orphaned App Builder preview uvicorn processes (Windows).

    A match requires the command line to contain BOTH ``-m uvicorn`` (the
    App Builder preview launch shape) AND ``--port <P>`` for one of the
    preview ``ports``. Matching on the port token that App Builder itself
    put on the command line lets us reap a preview even after it has lost
    its listening socket (so a ``netstat`` scan would miss it). Best-effort:
    any enumeration failure returns an empty set (the reaper never blocks a
    fresh launch). Non-Windows returns empty.
    """
    ...
    # Enumerate ProcessId + CommandLine for every process in one shot. wmic
    # first (fast, present on Win10/11 today); PowerShell CIM as the
    # deprecation fallback (mirrors ``_parent_pid`` / ``_is_supervisor_pid``).
    ...
    try:
        ...
        # CSV rows are ``Node,CommandLine,ProcessId``. CommandLine may be
        # empty (system processes); ProcessId is the last field.
        for line in result.stdout.splitlines():
            line = line.strip()
            if not line or line.upper().startswith("NODE,"):
                continue
            parts = line.split(",")
            if len(parts) < 3:
                continue
    ...
    if not rows:
        # Fallback: PowerShell CIM (wmic removed on newer builds).
        try:
            result = subprocess.run(
                [
                    "powershell",
    ...
    # ``--port <P>`` where <P> is a preview port and is NOT followed by
    # another digit (so ``--port 1842`` never matches ``--port 18420`` — the
    # substring trap the daemon port sweep also guards against). The trailing
    # boundary is a non-digit or end-of-string.
    port_res = [
        re.compile(rf"--port\s+{p}(?!\d)") for p in ports
    ]
    pids: set[int] = set()
    for pid, cmdline in rows:
        if pid <= 4:  # System / Idle
            continue
        if "-m uvicorn" not in cmdline and "uvicorn " not in cmdline:
            continue
        # Require an exact ``--port <preview-port>`` token so an unrelated
        # uvicorn on a non-preview port is never touched.
        if not any(rx.search(cmdline) for rx in port_res):
            continue
        pids.add(pid)
    return pids


def _sweep_app_preview_orphans(ports: "tuple[int, ...]") -> int:
    """Terminate orphaned App Builder preview uvicorn processes. Returns count.

    Identity-based (command line) rather than port-based — see the section
    banner above. Each match is killed with ``taskkill /F /T`` (via
    :func:`_kill_pid`) so a preview that itself spawned a reloader / worker
    subtree is fully reaped. Idempotent: no matches -> returns 0, no kills.
    """

    pids = _pids_matching_app_preview(ports)
    killed = 0
    for pid in pids:
        print(
            f"[endpoint-helper] terminating orphaned App Builder preview "
            f"pid={pid} (leftover uvicorn on a preview port)...",
            flush=True,
        )
        if _kill_pid(pid):
            killed += 1
            ...
            print(
                f"[endpoint-helper] WARN: failed to kill preview pid={pid}; "
                "continuing anyway.",
                flush=True,
            )
    return killed
...
def cmd_cleanup_stale(data_root: Path) -> int:
    ...
    1. **Port sweep (authoritative).** Scan every fallback port (see
       ...
       collision, a stale instance on (say) a secondary fallback was never
       reaped — the
    ...
    # --- 1b. App Builder preview orphans (identity-based) -----------------
    # Standalone preview uvicorns spawned by a *previous* API host that
    # exited non-gracefully are neither reaped by that host's lifespan
    # shutdown nor caught by the daemon port sweep above (distinct port
    # pool; often no longer listening). Reap them by command-line identity
    # so they do not accumulate across runs.
    preview_killed = _sweep_app_preview_orphans(_APP_PREVIEW_PORTS)
    if preview_killed:
        print(
            f"[endpoint-helper] reaped {preview_killed} orphaned App Builder "
            f"preview process(es).",
            flush=True,
        )
    ...
    while True:
        ...
        if info is not None:
            ...
            if isinstance(candidate, str) and candidate:
                # Normalise a wildcard bind (``--host 0.0.0.0``) to loopback
                # BEFORE the health probe, so both the probe and the browser
                # open target an address that is actually dialable — and the
                # one the Okta redirect_uri round-trips through.
                url = _dialable_url(candidate)
                break
...
#: Bind addresses that mean "every interface" rather than naming a host. A
#: server bound to one of these IS reachable, but the address itself is not
#: dialable: ``write_endpoint`` echoes ``settings.server.host`` verbatim into
#: the endpoint file's ``url``, so a launcher that binds wide — ``start.sh``
#: passes ``--host 0.0.0.0`` — records one of these.
_WILDCARD_BIND_HOSTS = frozenset({"0.0.0.0", "::", "0", ""})

#: Loopback name a wildcard bind is rewritten to. MUST be ``localhost`` and not
#: ``127.0.0.1``: ``interfaces/http/routes/auth.py`` builds the Okta
#: ``redirect_uri`` as ``http://localhost:<port>/callback`` (a hardcoded literal,
#: because Okta strict-matches the whole URI against the registered value). The
#: browser therefore has to start on the ``localhost`` origin, or the session
#: cookie the callback sets lands on a different origin than the tab the user
#: is looking at.
_LOOPBACK_HOST = "localhost"


def _dialable_url(url: str) -> str:
    """Rewrite a wildcard-bind URL to loopback; pass anything else through.

    ``http://0.0.0.0:8989`` → ``http://localhost:8989``. Needed because the
    endpoint file records the *bind* address, which on a wide bind is not a
    host a browser can usefully open — and, more importantly, is not the origin
    the Okta login flow round-trips through (see :data:`_LOOPBACK_HOST`).

    Port, scheme and path are preserved. A malformed URL is returned unchanged:
    this runs on the browser-open path, where degrading to the previous
    behaviour beats raising.
    """
    ...
    try:
        ...
        return url
    if host is None or host.lower() not in _WILDCARD_BIND_HOSTS:
        return url
    netloc = _LOOPBACK_HOST if parts.port is None else f"{_LOOPBACK_HOST}:{parts.port}"
    return urlunsplit(
...
def _health_probe_url(base_url: str) -> str:
    ...
    The endpoint file records the SPA base (e.g. ``http://127.0.0.1:<backend>/``,
    where ``<backend>`` is the ``backend`` port from ``factory/config/ports.json``);
