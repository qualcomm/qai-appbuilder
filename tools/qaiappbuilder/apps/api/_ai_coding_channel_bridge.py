...
v0.5 wechat/feishu CC handler semantics by
...
class AiCodingChannelBridge(MessageBridgePort):
    ...
    * slash-command path binds the real fields:
    ...
    async def _abort(self, *, session_id: str) -> bool:
        ...
        V0.5 OpenCode ``/oc stop`` first tries a soft interrupt and, on failure,
        ...
        *,
        ...
        if verb == "stop":
            ...
            if session_id:
                ...
                text = (
                    t("channels.ai_coding.session.stopped_send_new")
                    if ok
                    else t("channels.ai_coding.session.no_active_task")
                )
                return BridgeReply(
                    reply_text=text, coding_session_id=session_id
                )
            return BridgeReply(
                reply_text=t("channels.ai_coding.session.no_active_task"),
        ...
        *,
        ...
        try:
            if sub == "new":
                ...
                # V0.5 parity: ``/cc new`` REQUIRES an explicit working
                ...
                if not rest:
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.usage_new", verb=verb),
                        coding_session_id=None,
                    )
                ...
                if not _Path(workspace_path).is_dir():
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.dir_not_found", workspace_path=workspace_path),
                        coding_session_id=None,
                    )
                ...
                return BridgeReply(
                    reply_text=t(
                        "channels.ai_coding.session.created",
                        verb=verb, sid=sid, workspace_path=workspace_path,
                    ),
            ...
            if sub == "use":
                ...
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.switched", session_id=session.session_id),
            ...
            if sub == "status":
                ...
                if not sid:
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.no_active"),
                        coding_session_id=None,
                    )
                session = await uc.execute(
                    GetCodingSessionQuery(
            ...
            if sub == "rename":
                ...
                if not rest:
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.usage_rename", verb=verb),
                        coding_session_id=None,
                    )
                ...
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.renamed", new_title=new_title),
                    coding_session_id=str(session.session_id),
                )
            ...
            if sub in ("close", "delete"):
                ...
                if sub == "close":
                    ...
                    #
                    # V0.5 parity (``feishu/cc_handler.py`` close branch): ``/cc close`` is a
                    ...
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.closed", sid=sid, verb=verb),
                        coding_session_id=None,
                    )
                # delete → destroy.
                uc = getattr(
                ...
                await self._bind_owner(message, "")
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.deleted", sid=sid),
                    coding_session_id=None,
                )
            ...
            if sub == "cd":
                ...
                if not rest:
                    ...
                    if cur_dir:
                        return BridgeReply(
                            reply_text=t(
                                "channels.ai_coding.session.current_dir_usage",
                                cur_dir=cur_dir, verb=verb,
                            ),
                            coding_session_id=sid,
                        )
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.usage_cd", verb=verb),
                        coding_session_id=sid,
                    )
                new_path = rest[0]
                await uc.execute(
                ...
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.cd_ok", new_path=new_path),
                    coding_session_id=sid,
                )
            ...
            if sub == "fork":
                ...
                # same field on the aggregate.  Reject a
                ...
                if not await self._session_has_upstream(ai_coding, sid):
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.no_upstream_no_fork"),
                        coding_session_id=sid,
                    )
                ...
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.forked", new_sid=new_sid),
            ...
            if sub == "stop":
                ...
                if sid:
                    ...
                    if ok:
                        return BridgeReply(
                            reply_text=t("channels.ai_coding.session.stopped"),
                            coding_session_id=sid,
                        )
                    # D-5: OpenCode
                    ...
                    # (interrupt-only), so the second
                    # step is OC-only.
                    if provider is Provider.OPEN_CODE:
                        aborted = await self._abort(session_id=sid)
                        if aborted:
                            return BridgeReply(
                                reply_text=t("channels.ai_coding.session.opencode_aborted"),
                                coding_session_id=sid,
                            )
                    return BridgeReply(
                        reply_text=t("channels.ai_coding.session.no_running_task"),
                        coding_session_id=sid,
                    )
                return BridgeReply(
                    reply_text=t("channels.ai_coding.session.no_running_task"),
            ...
            return BridgeReply(
                reply_text=t("channels.ai_coding.error.command_failed", verb=verb, sub=sub, exc=exc),
                coding_session_id=None,
            )
        ...
        * otherwise it is treated verbatim as a full session id.
        ...
        if ref.isdigit():
            ...
            if idx < 0 or idx >= len(rows):
                return BridgeReply(
                    reply_text=t(
                        "channels.ai_coding.error.index_out_of_range",
                        ref=ref, count=len(rows),
                    ),
                    coding_session_id=None,
                )
        ...
        if len(matched) > 1:
            return BridgeReply(
                reply_text=t("channels.ai_coding.error.multiple_matches"),
                coding_session_id=None,
            )
    ...
    async def _cc_oc_models(self, *, ai_coding: Any, verb: str) -> BridgeReply:
        ...
        lines = [t("channels.ai_coding.session.models_header", verb=verb)]
        for i, m in enumerate(model_list, start=1):
            marker = "\u2705" if m == current else f"{i}."
            lines.append(f"{marker} {m}")
        if current not in model_list:
            lines.append(t("channels.ai_coding.session.model_not_in_list", current=current))
        lines.append(t("channels.ai_coding.session.models_hint", verb=verb))
        return BridgeReply(
    ...
    async def _cc_oc_model(
        ...
        if not rest:
            ...
            return BridgeReply(
                reply_text=t(
                    "channels.ai_coding.session.model_current",
                    verb=verb, current=current,
                ),
                coding_session_id=None,
            )
        arg = rest[0].strip()
        if arg.isdigit():
            ...
            if idx < 0 or idx >= len(model_list):
                return BridgeReply(
                    reply_text=t(
                        "channels.ai_coding.error.model_index_out_of_range",
                        arg=arg, count=len(model_list), verb=verb,
                    ),
                    coding_session_id=None,
                )
        ...
        if save_uc is None:
            return BridgeReply(
                reply_text=t("channels.ai_coding.error.model_unavailable", verb=verb),
                coding_session_id=None,
            )
        ...
        return BridgeReply(
            reply_text=t("channels.ai_coding.session.model_switched", verb=verb, new_model=new_model),
            coding_session_id=None,
        )
    ...
    async def _session_has_upstream(ai_coding: Any, session_id: str) -> bool:
        ...
        ``claude_session_id`` on the aggregate, so we
    ...
    async def _send_then_stream(
        ...
        *,
        ...
        ``cc_manager.send_message(...)`` flow where the manager both delivers the
    ...
    async def _stream_message(
        ...
        *,
        ...
        reply_text = "".join(text_parts).strip() or t("channels.ai_coding.usage.no_content")
        return BridgeReply(
    ...
    async def stream_with_tools(
        ...
        *,
        ...
        final_text = (
            "".join(text_parts).strip() or t("channels.ai_coding.usage.no_content")
        )
    ...
    * **sub-agent** (4-M12 / 2-H11) — V1 ``subagent_start`` /
    ...
    if etype == "turn_warning" or isinstance(tw, dict):
        ...
        if not message:
            ...
            message = (
                t("channels.ai_coding.usage.turn_warning_with_count", tc=tc)
                if tc
                else t("channels.ai_coding.usage.turn_warning_generic")
            )
    ...
    if etype in ("subagent_start",) or fkind in ("task_started",):
        ...
        text = (
            t("channels.ai_coding.progress.subagent_started_desc", desc=desc)
            if desc
            else t("channels.ai_coding.progress.subagent_started")
        )
    ...
    if etype in ("subagent_done",) or fkind in ("task_notification",):
        ...
        text = (
            t("channels.ai_coding.progress.subagent_done_desc", desc=desc)
            if desc
            else t("channels.ai_coding.progress.subagent_done")
        )
        return AiCodingStreamEvent(kind="subagent", text=text)
    if etype in ("subagent_error",):
        ...
        text = (
            t("channels.ai_coding.progress.subagent_error_desc", err=err)
            if err
            else t("channels.ai_coding.progress.subagent_error")
        )
        return AiCodingStreamEvent(kind="subagent", text=text)
    ...
    if etype in ("task_progress", "subagent_progress") or fkind in (
        ...
        if tool_uses:
            meta_parts.append(t("channels.ai_coding.progress.tool_count", tool_uses=tool_uses))
        if dur_s:
            meta_parts.append(f"{dur_s:.1f}s")
        progress_line = t("channels.ai_coding.progress.subagent_running", task_short=task_short)
        if meta_parts:
            progress_line += "\n" + " \u00b7 ".join(meta_parts)
