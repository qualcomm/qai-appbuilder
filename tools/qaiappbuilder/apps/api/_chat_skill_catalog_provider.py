...
V1 parity
---------
V1 ``build_skill_catalog`` filters skills
...
# Visibility sets + the per-skill default map come from the shared kernel
# (``qai.platform.skills``) so this gate and the Skills panel cannot disagree —
# see the note above ``_resolve_mode`` at the bottom of this file for the
# ``gh-public`` drift that duplication caused.
_CLOUD_VISIBLE_MODES = CLOUD_VISIBLE_MODES
_LOCAL_VISIBLE_MODES = LOCAL_VISIBLE_MODES
_DEFAULT_SKILL_MODE = DEFAULT_SKILL_MODES
...
class LocalChatSkillCatalogProvider:
    ...
    * **Row shape** — V1 ``build_available_skills_xml`` needs ``<name>`` (= ``skill_id``), ``<description>`` (=
...
# ---------------------------------------------------------------------------
# Mode resolution — delegated to the shared kernel
# ---------------------------------------------------------------------------
#
# This file used to carry its OWN copy of the ladder AND of the default-mode
# map, with a comment saying the map "MUST stay in sync" with the ``user_prefs``
# one. Nothing enforced that, and it drifted: ``gh-public`` was added to the
# other copy only, so on a fresh install (no persisted override) the Skills
# panel showed it OFF while THIS gate resolved ``cloud`` and advertised the
# skill to the model. ``gh-public`` drives the ``gh`` CLI against public
# github.com — creating issues / PRs / releases and pushing branches on a real
# remote — so shipping it reachable-by-default defeated the explicit opt-in.
#
# Re-exported under the historical private name so the call sites below (and any
# test importing it) keep working, while there is now exactly ONE implementation.
_resolve_mode = resolve_skill_mode
