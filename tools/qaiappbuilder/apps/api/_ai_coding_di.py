...
* ``SkillRegistryBridge`` — cross-context bridge onto the
  model_catalog skill registry (see PR-103 below)
...
* ``SkillRegistryBridge`` (``apps/api/_skill_registry_bridge.py``) now
  ...
  ``ai_coding_skill`` table — skills surface from the single
  canonical ``model_catalog_skill`` table.  The legacy
  ``SqliteAiCodingSkillRegistry`` adapter and the
  ``ai_coding_skill`` table were both removed once the bridge
  became the sole runtime path.
...
class AiCodingServices:
    ...
    # the V1 tool-result store).  Appended with a default per
...
def _claude_sdk_probe() -> tuple[bool, str]:
    ...
    * SDK importable, no ``__version__``    → ``(True, "unknown")`` —
      V1 fallback (``getattr(claude_agent_sdk,
      "__version__", "unknown")``).
    * SDK not importable                    → ``(False, "")`` — V1
      ``sdk_version = ""`` initialiser.
...
def _proxy_url_with_auth(container: Container, bare_url: str | None) -> str | None:
    ...
    so the ``web_fetch`` proxy-auth embedding and the "file download" global
...
def _proxy_endpoint_for_log(bare_url: str | None) -> str:
    """Return ``scheme://host:port`` for logging — never any credentials.

    The persisted proxy URL should carry no ``user:pass@`` (AGENTS.md §3.3),
    but an operator may paste one into the field, so this strips userinfo
    defensively rather than trusting the input. ``""`` when no proxy is set.
    """
    if not bare_url:
        return ""
    ...
    try:
        parts = urlsplit(bare_url)
    except ValueError:
        return "(unparseable)"
    host = parts.hostname or ""
    if not host:
        # No scheme/host split (e.g. a bare "host:port") — keep only the part
        # after any "@" so pasted credentials cannot leak.
        return bare_url.rsplit("@", 1)[-1]
...
def build_ai_coding_services(container: Container) -> AiCodingServices:
    ...
    #    fresh on every ``oc_service_start``).  Construction-time
    ...
    #    Fronts ``container.model_catalog.model_skill_registry``
    #    (``SqliteModelSkillRegistry`` from PR-044) so the canonical
    #    ``model_catalog_skill`` table is the single source of truth.
    #    ``Container.build`` always wires the model_catalog namespace
    #    before ai_coding (``di.py`` phase 2), so the source is always
    #    present; a missing namespace is a wiring bug and must fail loudly
    #    rather than silently degrade to a second, divergent skill table.
    skill_registry: SkillRegistryPort = SkillRegistryBridge(
        source=_model_skill_registry_source(container)
    )

    # -- Tool bridge (PR-101).  Wires the 9 production tools (read /
    #    write / edit / glob / grep / exec / web_fetch / apply_patch /
    ...
    if _tool_output_settings is not None:
        ...
        _set_tool_output_thresholds(
            _ToolOutputThresholds(
                ...
                read_count_scan_max_bytes=int(
                    getattr(
                        _tool_output_settings,
                        "read_count_scan_max_bytes",
                        50 * 1024 * 1024,
                    )
                ),
    ...
    if _tools_settings is not None:
        ...
        # 退化 #9 (V1): embed proxy ``user:pass@`` from
        # user_prefs (username) + SecretStore (password) so the ``web_fetch``
        # tool authenticates against an upstream proxy. Bare URL when no creds.
        _bare_proxy = getattr(_tools_settings, "global_proxy", None)
        _effective_proxy = _proxy_url_with_auth(container, _bare_proxy)
        _set_tool_global_proxy(_effective_proxy)
        _set_tool_project_skip_dirs(
            tuple(getattr(_tools_settings, "project_skip_dirs", ()) or ())
        )
        # Startup state: without this line the only way to tell whether
        # outbound traffic is proxied was to read the config file by hand — the
        # value lives in a module-level seam and is not echoed anywhere. Logged
        # here AND in ``apply_tools_runtime_config`` so both the boot value and
        # every later operator edit leave a trace. Credentials are never
        # logged (see ``_proxy_endpoint_for_log``).
        logger.info(
            "tools.proxy_boot_state",
            enabled=bool(_effective_proxy),
            endpoint=_proxy_endpoint_for_log(_bare_proxy),
            has_credentials=bool(
                _effective_proxy and _bare_proxy and _effective_proxy != _bare_proxy
            ),
...
def _model_skill_registry_source(container: Container) -> object | None:
    """Return ``container.model_catalog.model_skill_registry``.

    ``Container.build`` wires the ``model_catalog`` namespace before
    ai_coding, so the registry is always present in every real
    container.  A missing namespace returns :data:`None`, which makes
    :class:`SkillRegistryBridge` raise :class:`TypeError` at
    construction — a loud wiring bug rather than a silent degrade onto
    a second, divergent skill table.

    No type-checked import of ``qai.model_catalog.*`` is performed
    here — the bridge re-validates the duck-typed input.
    """
    model_catalog = getattr(container, "model_catalog", None)
...
def apply_tools_runtime_config(
    ...
    *global_proxy* before it is installed — mirroring the build-time path so a
    ...
    ``None`` (the legacy call shape) installs the
    bare URL unchanged.
    """
    from qai.ai_coding.infrastructure.tools.handlers import (
        set_global_proxy as _set_tool_global_proxy,
    ...
    # Observability: whether outbound requests are proxied is a question you
    # otherwise cannot answer from the outside (the value lives in a module
    # seam, not in any response). NEVER log the effective URL — it carries the
    # embedded ``user:pass@`` credentials; log only the sanitised endpoint plus
    # a flag telling whether credentials were attached.
    logger.info(
        "tools.proxy_applied",
        enabled=bool(effective_proxy),
        endpoint=_proxy_endpoint_for_log(global_proxy),
        has_credentials=bool(
            effective_proxy and global_proxy and effective_proxy != global_proxy
        ),
