...
class ChannelsServices:
    ...
    # R-8 — restores the inbound image flow provided):
    #
...
def build_channels_services(container: "Container") -> ChannelsServices:
    ...
    async def _default_inbound_consumer(
        ...
        #    time (V1 broadcast ``wechat_update_conv`` / ``feishu_update_conv`` with the
        ...
        # conv_id frame the WebUI cannot insert (V1
        # ``if (!conv_id ...) return``).
        if conversation_id:
            try:
    ...
    # flow restored from the legacy WeChat channel.
