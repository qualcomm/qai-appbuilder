...
def wire_background_process_tool_into_chat(
    *,
    ...
    return register_background_process_tool_into_chat(
        ...
        # Wait-suppression wiring — a synchronous
        # ``background_process(wait|status)`` picks up a terminal
        # result; the dispatcher's own auto-notify (via
        # ``BackgroundProcessUpdated``) must not then re-append a
        # SYSTEM_NOTICE for the SAME event.  ``None`` when the
        # chat service is built without the dispatcher (unit stubs).
        dispatcher=getattr(chat, "background_job_dispatcher", None),
    )
