...
the legacy broadcast helpers (``_broadcast_feishu_message``
and the parallel WeChat block emitting
...
def _channel_update_conv_frame(
    *,
    ...
    V1 broadcast this exact shape; the WebUI's
...
class ChannelWebUIInboundEvent(DomainEvent):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        V1 parity broadcast a frame shaped
        ``{type, conv_id, title, messages:[...]}`` that the WebUI's
        conversation list handler used to
...
class ChannelWebUIBroadcaster:
    ...
    Mirrors legacy ``_broadcast_feishu_message`` and the parallel
