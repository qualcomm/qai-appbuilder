...
def register_background_process_tool_into_chat(
    *,
    ...
    async def _chat_background_process(request: ToolInvocationRequest) -> Any:
        ...
        # Wait-suppression — when the main agent picks up a terminal
        # process state synchronously (via wait/status/logs), the
        # ``BackgroundJobDispatcher`` should NOT also fire a
        # SYSTEM_NOTICE + wake for the same event.  Extract the
        # process id + terminal status from ``result`` and mark the
        # matching dedup_key as "already delivered".  Best-effort:
        # no dispatcher wired → no-op (unit stubs stay working).
        _maybe_mark_delivered_bgp(dispatcher, args, result)
        ...
        # human-readable rendering ``_render_background_process`` produces.
        # The renderer knows the shapes (processes / process / logs / wait) and
        # preserves the ``[tool_error]`` sentinel + the ``bgp_...`` id token the
        # chat card salvages, so no downstream contract is affected.
        #
        # Extract the STRUCTURED terminal marker before the renderer stringifies
        # the dict — the LLM-visible text stays byte-for-byte the same (rendered
        # by ``_render_background_process``), and the UI sees the marker via
        # ``ToolInvocationResult.terminal_kind`` → ``StreamFrame.tool_result``
        # tail field → ``ChatToolCall.status``. Same pattern as ``cancelled`` /
        # ``backgrounded``.
        _terminal_kind: str | None = None
        if isinstance(result, dict):
            tk = result.get("terminal_kind")
            if isinstance(tk, str):
                _terminal_kind = tk
        rendered = render_tool_result_text_with_hints(result)
        if _terminal_kind is None:
            # No structured marker → return the string; the invocation runtime
            # wraps it in an ``ok=True`` ``ToolInvocationResult`` (byte-for-byte
            # unchanged for every ordinary result).
            return rendered
...
_BGP_TERMINAL_STATUSES: frozenset[str] = frozenset({"exited", "failed", "stopped"})


def _maybe_mark_delivered_bgp(
    dispatcher: Any,
    args: dict[str, Any],
    result: Any,
) -> None:
    """Suppress the dispatcher's pending auto-notify for a bgp result
    the main agent has just observed synchronously.

    The dispatcher's dedup_key format is ``bgp:<process_id>:<status>``
    (see :class:`BackgroundJobDispatcher._on_bg_updated`).  We can
    only mark it after we know BOTH the id AND the terminal status:

    * ``wait`` — result dict carries ``settled=True`` and
      ``process.status`` on the terminal branch.
    * ``status`` — returns ``process`` with a ``.status`` field, so a
      terminal snapshot the agent just read IS the delivery.  (``logs``
      is deliberately NOT here: it carries no status, so there is
      nothing to key the suppression on.)
    * ``stop`` — the agent asked for this termination, so it already
      knows the outcome: delivering a completion notice for a stop the
      agent itself issued is pure noise.  A process that dies on its
      OWN (crash / non-zero exit / killed from the UI) is NOT
      suppressed — the agent must learn about that, or it keeps
      believing a dead service is still up (State-Truth-First).

    Best-effort: any shape mismatch or missing field silently
    skips.  No dispatcher wired → no-op.
    """
    ...
    if not isinstance(result, dict) or not result.get("ok"):
        return
    action = (args.get("action") or "").strip().lower()
    if action not in ("wait", "status", "stop"):
        return
    ...
    if (
        not isinstance(proc_id, str)
        or not isinstance(status, str)
        or status not in _BGP_TERMINAL_STATUSES
    ):
        return
    try:
        mark(f"bgp:{proc_id}:{status}")
