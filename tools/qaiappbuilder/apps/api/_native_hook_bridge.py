...
fail-closed semantics (HARD-CODED, no knob): any bridge fault — no loop,
coroutine exception, ASK request error, or the dedicated ASK loop being
torn down (service shutdown) — resolves to DENY. Matches the C++
2026-07-23 hard-DENY directive (any abnormal / teardown path denies,
regardless of any historical fail_closed toggle). Phase 2 (2026-07-06):
the bridge NO LONGER auto-denies on any elapsed wall-clock timeout. It
waits INDEFINITELY for the operator's ALLOW/DENY (the inner ASK wait is
...
M-Nat-6 (fileguard-audit-2026-07-26): the ``fail_closed`` __init__ param
was retained after the 2026-07-23 hard-DENY sweep but was set-then-never-
read (dead state; every fault path already returns False unconditionally).
The parameter is removed; the SecuritySettings field
``native_file_guard_fail_closed`` is preserved because it still governs
the DLL-side ``Guard.init_v2`` fail_closed knob (a separate concern —
what the DLL does when the callback itself hangs).
"""
...
class NativeFileInterceptorBridge:
    ...
    request_permission_use_case / wait_registry:
        ...
        stays a hard-coded DENY (see module docstring).
    ask_timeout_sec:
    ...
    def filter(self, evt: FilterEventV2) -> bool:
        ...
        that (and any decide fault) resolves to DENY (hard-coded).
    ...
    async def _ask_user(
        ...
        * **Durable mirror** — the native event context is persisted on
        ...
        # 2026-08-09 observability — the native ASK path had ZERO logging: a
        # dialog could open, be abandoned by a short-lived subprocess and be
        # swept away with nothing in the log to explain it (diagnosing the
        # ``C:\Windows\INF\cpu.PNF`` ghost entries required reading the DB
        # directly). Mirrors the in-process FileGuard facade's
        # ``filesec.ask_created`` / ``filesec.ask_resolved`` pair
        # (``_file_guard_bridge.py``) — same event names, same ``extra``
        # shape — so one grep covers BOTH channels. ``caller`` carries the
        # native subject plus the pid so a sweep-resolved row can be tied
        # back to the process that triggered it.
        _log.info(
            "filesec.ask_created",
            extra={
                "request_id": request_id,
                "op": self._event_to_op(event_bits),
                "path": path_str,
                "caller": f"native.file_guard(pid={pid})",
                "initial_state": state_value,
                "command_line": command_line,
            },
        )
        if state_value in ("approved", "rejected", "cancelled", "expired"):
            # Auto-resolved inside the use case (smart-approval or a policy
            # terminal state) — never reaches the wait below, so emit the
            # paired resolved line here or the ASK looks eternally open.
            allow = state_value == "approved"
            _log.info(
                ...
                extra={
                    ...
                    "allow": allow,
                    "timed_out": False,
                    "resolved_by": f"auto:{state_value}",
                },
            )
            return allow
        ...
        # 2026-08-09 observability — paired with the ``filesec.ask_created``
        # above. ``mark_resolved`` is first-write-wins, so when the
        # subprocess-gone sweep got here first (it now marks BEFORE waking us)
        # the row already carries ``subprocess_gone`` / ``conversation_gone``
        # and this bridge's ``deny`` write above was the no-op. Log the
        # registry's own scope as ``resolved_by`` so the line still says
        # whether the DENY came from a person or from a backstop.
        _log.info(
            "filesec.ask_resolved",
            extra={
                "request_id": request_id,
                ...
                "resolved_by": getattr(resolution, "scope", "") or "",
            },
        )
        return bool(resolution.allow)
