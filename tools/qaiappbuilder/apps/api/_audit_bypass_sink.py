...
class AuditBypassSink:
    ...
    def close(self) -> None:
        """Drain remaining queued denies (bounded), then stop the loop cleanly.

        Idempotent. Called on shutdown BEFORE ``Database.close()`` so the final
        ``facade.record`` writes still have a live DB. Adds a bounded FINAL
        DRAIN, then CANCELS + AWAITS the background ``_drain`` task before
        stopping the loop and JOINING the loop thread — so no pending task is
        left to be garbage-collected (which surfaced as
        ``Task was destroyed but it is pending!`` on WebUI restart). Never hangs
        shutdown: every wait is bounded (铁律5). A DB already being torn down
        just makes each ``record`` a swallowed best-effort miss.
        """
        loop = self._loop
        ...
        if loop is not None and not loop.is_closed():
            ...
            try:
                ...
                pass
            # (2) cancel + await the long-lived drain task ON its own loop so
            # it is not left pending when the loop stops (avoids the
            # "Task was destroyed but it is pending!" warning). Bounded.
            ...
            if drain_task is not None:
                async def _cancel_drain() -> None:
                    ...
                    try:
                        ...
                        pass
                try:
                    cfut = asyncio.run_coroutine_threadsafe(_cancel_drain(), loop)
                    cfut.result(timeout=_FINAL_DRAIN_TIMEOUT_SEC)
                except Exception:  # noqa: BLE001,S110 — bounded; stop loop anyway
                    pass
            # (3) stop the loop (idempotent — a closed loop is skipped above).
            try:  # noqa: SIM105 — explicit swallow, no contextlib dependency
                loop.call_soon_threadsafe(loop.stop)
            except Exception:  # noqa: BLE001,S110
                pass
        # (4) join the loop thread so run_forever() has fully returned and the
        # loop is torn down before we drop our references (bounded — never hang).
        if thread is not None and thread.is_alive():
            try:  # noqa: SIM105
                thread.join(timeout=_FINAL_DRAIN_TIMEOUT_SEC)
            except Exception:  # noqa: BLE001,S110
                pass
        self._loop = None
