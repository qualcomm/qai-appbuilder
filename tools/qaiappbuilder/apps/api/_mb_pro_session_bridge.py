...
class MbProSessionController:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-tab background relay task (long-lived fan-out consumer).  Kept
        # here so ``disconnect`` can cancel the right one without a registry.
    ...
    def peek_manager(self, *, tab_id: str) -> Any:
        """Return the live SessionManager for tab_id, or None (no materialisation)."""
        return self._peek_manager(tab_id)
    ...
    async def connect(
        ...
        *,
        ...
        # Kick off the long-lived RELAY task.  It fans out every subsequent
        # MB Pro SSE event (post-greeting) through the chat_stream_broadcaster
        # so iframe-triggered turns (e.g. "创建转换" clicked in the embedded
        # builder panel) render in the QAI chat exactly like an ordinary
        # user-typed turn.  One task per connected tab; cancelled on
        # ``disconnect``.
        if (
            state.connected
            and conversation_id
            and self._relay_use_case is not None
            and tab_id not in self._relay_tasks
        ):
            from qai.chat.application.use_cases.mb_pro_relay import (
                RelayMbProSessionInput,
            )

            async def _relay() -> None:
                try:
                    await self._relay_use_case.execute(
                        RelayMbProSessionInput(
                            tab_id=tab_id,
                            conversation_id=conversation_id,
                        )
                    ...
                    raise
                except Exception:  # noqa: BLE001 — never propagate to connect
                    _log.warning(
                        "mb_pro.relay_use_case_failed",
                        tab_id=tab_id,
                        conversation_id=conversation_id,
                        exc_info=True,
                    )

            self._relay_tasks[tab_id] = asyncio.create_task(
                _relay(), name=f"mb_pro_relay[{tab_id}]"
            )
        return self._snapshot(state)

    async def disconnect(self, *, tab_id: str) -> dict[str, Any]:
        # Cancel the long-lived relay task BEFORE tearing down the manager so
        # the subscriber queue is released cleanly (the manager's teardown
        # broadcasts _SSE_CLOSED_EVENT to subscribers — cancelling first
        # avoids a pointless final iteration through the state machine).
        relay_task = self._relay_tasks.pop(tab_id, None)
...
async def _find_idle_agent_url(
    *,
    ...
    # A MIXED pool (some busy, some unreachable) previously reported only the
    # busy side, so an operator could not tell that other ports were down.
    # ``_probe_one`` already classifies each failure as ``offline`` / ``network``
    # — carry that count through so the message distinguishes "everything is
    # merely busy" from "the pool is degraded AND the rest are busy".
    unreachable_count = len(last_failure_reasons)
    reasons = sorted(set(last_failure_reasons))
    message = f"当前 {len(last_reachable)} 台机器全部繁忙，请稍后再试"
    if unreachable_count:
        message = (
            f"当前 {len(last_reachable)} 台机器全部繁忙，"
            f"另有 {unreachable_count} 台不可用，请稍后再试"
        )
    raise MbProProbeError(
...
def build_mb_pro_session_controller(*, container: Any) -> MbProSessionController | None:
    ...
    # Relay use case — fans out iframe-triggered MB Pro events through the
    # chat_stream_broadcaster so they render as ordinary chat frames.  Same
    # graceful-degrade contract as greeting: missing chat deps ⇒ ``None`` and
    # the controller still constructs without relay wiring.
    relay_use_case = _build_relay_use_case(
        container=container,
        peek_manager=peek_session_manager,
    )
...
def _build_relay_use_case(*, container: Any, peek_manager: Any) -> Any | None:
    """Build the fan-out relay use case, or ``None`` if deps are missing.

    Same contract as :func:`_build_greeting_use_case` — the composition root
    wires the concrete infra collaborators (MbProMapper / MbProTurnRounder /
    SessionManager subscribe API) behind application-layer Ports so the use
    case module stays free of any infrastructure import.
    """
    chat = getattr(container, "chat", None)
    if chat is None:
        return None
    broadcaster = getattr(chat, "chat_stream_broadcaster", None)
    ids = getattr(container, "ids", None) or getattr(chat, "ids", None)
    if broadcaster is None or ids is None:
        _log.info(
            "mb_pro.relay_disabled_missing_deps",
            has_broadcaster=broadcaster is not None,
            has_ids=ids is not None,
        )
        return None
    try:
        ...
        from qai.chat.infrastructure.query_service.mb_pro_turn_rounder import (
            MbProTurnRounder,
        )
    except Exception:  # pragma: no cover - excluded on external
        return None
    ...
    class _RelayMapper:
        """Adapter satisfying ``RelayMapperPort``.

        ONE ``MbProMapper`` instance is reused across turns of the same
        relay task (the mapper carries no per-turn state — the delta buffer
        lives on ``QueryMappingContext``, which is minted fresh each turn).
        """
        ...
        def new_context(self, *, my_session_id: Any = None) -> Any:
            return QueryMappingContext(
                ids=self._ids, my_session_id=my_session_id
            )
    ...
    class _RelaySubscriber:
        """Adapter satisfying ``RelaySubscriberPort`` — bound to a per-tab
        ``SessionManager`` via the shared ``peek_manager`` factory.

        ``subscribe`` looks up the live manager at call time (the manager is
        created lazily by ``get_session_manager`` when the tab connects; by
        the time the relay task calls ``subscribe`` it always exists).
        """

        def __init__(self, tab_id: str, peek: Any) -> None:
            self._tab_id = tab_id
            self._peek = peek

        def subscribe(self) -> Any:
            mgr = self._peek(self._tab_id)
            if mgr is None:
                # Should never happen — the connect flow that spawns this
                # relay task always creates the manager first. Fall back to
                # an inert queue so the use case exits cleanly.
                import asyncio as _asyncio

                q: Any = _asyncio.Queue()
                q.put_nowait({"__sse_closed__": True})
                return q
        ...
        def peek_session_id(self) -> Any:
            mgr = self._peek(self._tab_id)
            if mgr is None:
                return None
            return mgr.get_state().session_id

    # The relay use case is invoked with a *per-invocation* tab_id via the
    # ``RelayMbProSessionInput`` payload, but its subscriber Port is a bound
    # instance.  To keep the use case reusable across tabs we return a small
    # thin wrapper whose ``execute`` builds a subscriber for the request's
    # tab_id and delegates.  The wrapper mimics the ``execute`` interface the
    # controller invokes so the bridge stays the composition root for both.
    class _RelayUseCaseFacade:
        def __init__(self) -> None:
            self._ids = ids
            self._mapper = _RelayMapper(ids)

        async def execute(self, request: Any) -> None:
            subscriber = _RelaySubscriber(request.tab_id, peek_manager)
            use_case = RelayMbProSessionUseCase(
                broadcaster=broadcaster,
                subscriber=subscriber,
                mapper=self._mapper,
                rounder_factory=MbProTurnRounder,
                ids=self._ids,
            )
            await use_case.execute(request)

    return _RelayUseCaseFacade()
