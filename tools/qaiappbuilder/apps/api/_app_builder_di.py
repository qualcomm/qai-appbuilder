...
class _LazyStickyHost:
    ...
    async def cancel_run(self, run_id: str) -> None:
        ...
        Two runner paths, two kill mechanisms (V1 killed
...
def build_app_builder_services(container: "Container") -> AppBuilderServices:
    ...
    # Only packs that declare a ``runner.requirements`` path participate. The
    ...
    # because it cached the catalog for 10s).
    ...
    # repository scans roots for directories carrying a valid ``app.yaml``.
    # Primary (writable) root: data/app_builder/. Secondary (read-only)
    # root: samples/app_builder/ (built-in demo apps shipped with the repo).
    _app_project_roots: list[Path] = [data_paths.root / "app_builder"]
    _repo_root_for_samples = getattr(container, "repo_root", None)
    if isinstance(_repo_root_for_samples, Path):
        _app_project_roots.append(
            Path(_repo_root_for_samples) / "samples" / "app_builder"
        )
    app_project_repository = FileSystemAppProjectRepository(
        apps_roots=_app_project_roots
    )
    ...
    if isinstance(_repo_root_for_apps, Path):
        ...
        app_project_packager = FileSystemAppProjectPackager(
            ...
            # Mirror the repository's dual-root scan: user-generated apps
            # live under data/app_builder/, while the built-in demos ship
            # under samples/app_builder/. Passing only the data root here
            # made every demo app fail path-containment before the first
            # progress yield, and the WS handler closed silently (no
            # frames emitted). Reuse ``_app_project_roots`` so the two
            # stay in lockstep.
            apps_root=_app_project_roots,
...
# Reverse of V1 ``LEGACY_CATEGORY_MAP``:
