...
async def seed_native_guard_rules(container: object) -> None:
    ...
    op_mask_keys = {
        ...
        for p, _allow, _deny in _resolve_file_guard_masked_paths(container)
    }
    ...
    #     → OP-MASKED THREE-STATE (white_ops). Each entry is
    #     (path, allow_mask, deny_mask) with bits READ=1/WRITE=2/EXECUTE=4/
    #     DELETE=8. Per op: bit in deny_mask → hard DENY; else bit in allow_mask →
    #     ALLOW (skip callback); else (neither) → falls through to the callback
    #     (-> ASK). This expresses "read + execute allowed, write DENIED"
    #     (C:\Qualcomm allow=R|X deny=W|D) or "write ASK" (%ProgramFiles% allow=
    #     R|X deny=0) precisely — and fixes the old flaw where an unlisted op was
    #     silently hard-denied. Same source the Python permission check reads
    ...
    op_mask_count = 0
    for path, allow_mask, deny_mask in _resolve_file_guard_masked_paths(container):
        if path and guard.add_op_mask_rule(
            str(path), int(allow_mask), int(deny_mask), session_only=False
        ):
            op_mask_count += 1
    ...
    # 2e-ter) L-Nat-1 (fileguard-audit-2026-07-26) — the guard's OWN
    #     artefacts (guard64.dll x arch, guard-injector.exe x arch,
    #     factory/config/file_guard_paths.json). READ + EXECUTE allowed
    #     (the host loader must be able to map the DLL / launch the
    #     injector) but WRITE + DELETE are hard-denied natively so a
    #     compromised in-process actor cannot tamper with the trust
    #     anchor of the guard itself. Matches the Python-side extension
    #     of ``BUILTIN_SELF_PROTECTED_FILES`` in ``protected_paths.py``;
    #     the two layers must stay in lock-step (State-Truth-First).
    try:
        from qai.platform.protected_paths import BUILTIN_SELF_PROTECTED_FILES

        _self_files = tuple(BUILTIN_SELF_PROTECTED_FILES)
    except Exception:  # noqa: BLE001 — degrade to no seeding
        _self_files = ()
    self_protected_count = 0
    _self_allow = _OP_READ | _OP_EXECUTE
    _self_deny = _OP_WRITE | _OP_DELETE
    for path in _self_files:
        if path and guard.add_op_mask_rule(
            str(path), _self_allow, _self_deny, session_only=False
        ):
            self_protected_count += 1
    ...
    _log.info(
        ...
        self_protected=self_protected_count,
...
async def _seed_permanent_grants(
    ...
    for grant in grants:
        ...
        grant_effect = getattr(grant, "effect", "allow") or "allow"
        # Dedup on (path, effect): an allow-list rule and a deny op-mask rule
        # for the same path live in DIFFERENT native lists, so both must seed.
        seen_key = f"{grant_effect}\x00{path}"
        if not path or seen_key in seen:
            continue
        seen.add(seen_key)
        # Constraint G (reject side): a permanent DENY-effect grant seeds the
        # native deny_mask op-mask rule (mirror of the runtime _on_created
        # subscriber), not the allow-list. Allow-effect grants keep the
        # historical add_allow_rule seeding.
        if grant_effect == "deny":
            deny_bits = _ace_mask_to_op_bits(getattr(grant, "mask", None))
            if deny_bits and guard.add_op_mask_rule(
                path, 0, deny_bits, session_only=False
            ):
                seeded += 1
            continue
...
# Op-mask bits — MUST match native rule.h kMask* and _workspace_resolver:
...
def _ace_mask_to_op_bits(mask: object) -> int:
    """Fold an :class:`AceMask` into the native op-mask bit set.

    Returns the OR of ``READ/WRITE/EXECUTE/DELETE`` bits set on ``mask`` (0 for
    an empty / None mask). Used to translate a deny-effect grant's mask into the
    native ``deny_mask`` op-mask rule (constraint G reject-side native sync).
    """
    if mask is None:
        return 0
    bits = 0
...
async def subscribe_native_guard_grant_sync(container: object):
    ...
    async def _on_created(envelope) -> None:  # type: ignore[no-untyped-def]
        ...
        #
        # Constraint G (reject side) — a DENY-effect grant is the mirror: it
        # must seed the native deny_mask (op-mask rule) so the native layer
        # re-denies the same ops without a round-trip to the Python ASK bridge.
        # ``effect`` is tail-appended on the event (defaults to "allow"), so an
        # allow grant / an older publisher takes the historical allow-rule path
        # (byte-for-byte compatible).
        event = getattr(envelope, "event", envelope)
        path = getattr(event, "path", "") or ""
        if not path:
            return
        effect = getattr(event, "effect", "allow") or "allow"
        if effect == "deny":
            deny_bits = _ace_mask_to_op_bits(getattr(event, "mask", None))
            if deny_bits:
                # allow_mask=0, deny_mask=deny_bits → native hard-denies these
                # ops under the subtree (deny is evaluated before allow both
                # natively and in the Python check).
                guard.add_op_mask_rule(
                    str(path), 0, deny_bits, session_only=False
                )
            return
    ...
    async def _on_revoked(envelope) -> None:  # type: ignore[no-untyped-def]
        ...
        effect = getattr(event, "effect", "allow") or "allow"
        if effect == "deny":
            # Undo the deny-effect grant's native op-mask rule (mirror of the
            # add_op_mask_rule in _on_created). Falls back silently when the
            # DLL predates the delete export.
            guard.remove_op_mask_rule(str(path), session_only=False)
            return
