...
# strings into the security context.  We intentionally do NOT cache the
# rendered text at module scope: the request locale is not known at
# import time, so caching would freeze the "no active session" reply to
# the zh-CN fallback.  Mirrors how ``_channel_dispatch_bridge`` handles
# ``_fallback_image_text_block``.


def _no_active_session_msg() -> str:
    """Render the /grant 'no active session' reply in the request locale."""
    return t("channels.grant.error.no_active_session")
...
def _format_grant_summary(grants: tuple[Any, ...]) -> str:
    ...
    if not grants:
        return t("channels.grant.list.empty")
    lines = [t("channels.grant.list.header")]
    for g in grants:
        mask = g.mask
        ops: list[str] = []
...
class ChannelGrantBridge:
    ...
    async def handle_grant_command(
        ...
        * ``list`` — returns a textual summary of all current grants
        ...
        if verb_lower in ("revoke", "remove", "rm"):
            if not path:
                return t(
                    "channels.grant.error.revoke_needs_path"
                )
        ...
        if not path:
            return t("channels.grant.error.needs_path")
        if not op:
            return t("channels.grant.error.needs_op")
    ...
    async def _handle_grant(
        ...
        *,
        ...
        if create_uc is None:
            return t("channels.grant.error.module_disabled")
        try:
            ...
            return t("channels.grant.error.grant_failed", exc=str(exc))
            ...
            return t("channels.grant.error.grant_failed", exc=str(exc))
        return t("channels.grant.ok.granted", path=path, op=op)
    ...
    async def _handle_revoke(
        ...
        *,
        ...
        if target_id is None:
            return t("channels.grant.revoke.not_found", path=path)
        try:
            ...
            return t("channels.grant.error.revoke_failed", exc=str(exc))
        except Exception as exc:  # noqa: BLE001
            logger.warning(
            ...
            return t("channels.grant.error.revoke_failed", exc=str(exc))
        return t("channels.grant.ok.revoked", path=path)
    ...
    async def _handle_list(self, *, coding_session_id: str) -> str:
        ...
        try:
            ...
            return t("channels.grant.error.list_failed", exc=str(exc))
        except Exception as exc:  # noqa: BLE001
            logger.warning(
