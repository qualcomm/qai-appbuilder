...
def _tri_state(value: object) -> str:
    """Normalise an op flag to ``"allow"`` / ``"deny"`` / ``"ask"``.

    THREE-STATE schema (version 2): each op value is the string ``"allow"`` /
    ``"deny"`` / ``"ask"``; anything omitted or unrecognised defaults to
    ``"ask"`` (fall through to the prompt — never a silent allow/deny).

    BACKWARD-COMPAT (version 1 bool schema): ``True`` → ``"allow"``; ``False``
    → ``"ask"``. NOTE the deliberate change: the old ``False`` meant "hard-deny
    this op for the whole subtree" (the design flaw being fixed). Under the
    three-state model an unspecified op must fall through to ASK, not be
    silently denied — so a legacy ``False`` maps to ``"ask"``. A path that truly
    needs a hard deny must say so explicitly (``"deny"``); the shipped config is
    migrated to the string schema, so this bool path is defensive only.
    """
    ...
    if value is True:
        return "allow"
    # False / None / anything else -> ask (never a silent deny)
    return "ask"
...
def resolve_file_guard_masked_paths(
    ...
    Returns ``((abs_path, allow_mask, deny_mask), ...)`` — each entry's path
    (``%ENV%``-expanded, absolutised) paired with its THREE-STATE op masks
    (READ=1, WRITE=2, EXECUTE=4, DELETE=8). Per op: ``"allow"`` sets the bit in
    ``allow_mask``; ``"deny"`` sets it in ``deny_mask``; ``"ask"`` (or omitted)
    sets neither (fall through to the prompt). WRITE=allow/deny also sets the
    matching DELETE bit (write implies delete class). Seeded onto the native
    op-masked whitelist and mirrored by the Python permission check. The native
    / Python decision evaluates DENY before ALLOW. De-duplicated by path (last
    entry wins). Ordered MOST SPECIFIC (longest path) FIRST: both consumers
    treat the first matching prefix as authoritative, so a nested entry must
    out-rank its parent or it would be dead config (e.g. ``%SystemRoot%\\INF``
    write=deny under ``%SystemRoot%`` write=ask). Never raises — a missing /
    malformed file (or any per-entry fault) degrades to whatever parsed
    cleanly (empty on total failure), so a config typo cannot wedge boot.
    """
    import json
    ...
    by_key: "dict[str, tuple[str, int, int]]" = {}
    for entry in entries if isinstance(entries, list) else []:
        ...
        allow_mask = 0
        deny_mask = 0
        # read / write / execute each three-state; delete follows write's state.
        for op_key, allow_bit, extra_bit in (
            ("read", _MASK_READ, 0),
            ("write", _MASK_WRITE, _MASK_DELETE),  # write implies delete class
            ("execute", _MASK_EXECUTE, 0),
        ):
            state = _tri_state(entry.get(op_key))
            bits = allow_bit | extra_bit
            if state == "allow":
                allow_mask |= bits
            elif state == "deny":
                deny_mask |= bits
            # "ask" -> neither mask (fall through to the prompt)
        # An explicit "delete" op key overrides the write-derived delete state.
        if "delete" in entry:
            dstate = _tri_state(entry.get("delete"))
            allow_mask &= ~_MASK_DELETE
            deny_mask &= ~_MASK_DELETE
            if dstate == "allow":
                allow_mask |= _MASK_DELETE
            elif dstate == "deny":
                deny_mask |= _MASK_DELETE
        if allow_mask == 0 and deny_mask == 0:
            continue  # entry says ASK for everything -> nothing to seed
        key = abs_path.replace("\\", "/").casefold()
        by_key[key] = (abs_path, allow_mask, deny_mask)  # last entry wins
    # 2026-08-09 — MOST SPECIFIC (longest) prefix FIRST, ties on the path for
    # determinism. This is now a READABILITY/diagnostics aid, not a correctness
    # requirement: ``check_permission``'s op-mask segments UNION the masks of
    # every matching prefix (deny bits winning), mirroring the native guard's
    # whole-table scan, so the outcome no longer depends on order at all.
    #
    # It was briefly load-bearing: the consumers used to take the FIRST
    # matching prefix and ``break``, which made nesting order-dependent in BOTH
    # directions — a nested deny listed after its parent was dead config
    # (``%SystemRoot%\INF`` write=deny lost to ``%SystemRoot%``), and a nested
    # read-only entry listed before its parent shadowed the parent's wider
    # grant (``%APPDATA%`` read=allow hid ``%USERPROFILE%`` rwx=allow). Sorting
    # fixed the first and caused the second, so the consumers moved to union
    # semantics instead. The ordering is kept because a longest-first listing
    # is what a human debugging a prefix decision expects to read.
    return tuple(
        sorted(
            by_key.values(),
...
def _collect_system_read_prefixes() -> list[str]:
    """Collect the Python-runtime READ surface prefixes (absolutised).

    ``sys.prefix`` / ``sys.base_prefix`` / the interpreter directory /
    ``site.getsitepackages()``. These are DYNAMIC (interpreter-derived) paths
    that vary per install / venv, so per the "static-path unification" (todo
    #12) they REMAIN in code — they cannot be expressed as fixed config
    literals. LLM-spawned subprocesses read the interpreter tree en masse;
    seeding it read-only prevents an ASK flood while still routing writes to
    ASK.

    The former STATIC OS entries (``%SystemRoot%``, ``%ProgramFiles%`` /
    ``%ProgramFiles(x86)%``, ``%ProgramData%``, ``%APPDATA%`` + ``%APPDATA%\\uv``,
    ``%LOCALAPPDATA%\\uv``, ``System32\\CatRoot`` / ``catroot2``) have been
    MOVED to ``factory/config/file_guard_paths.json`` (three-state op-mask) and
    are now delivered via :func:`resolve_file_guard_masked_paths`. They are no
    longer emitted here.
    ...
    # Python runtime prefixes (DYNAMIC — stay in code, not config).
...
def resolve_read_only_allow_paths(container: "Container") -> tuple[str, ...]:
    ...
    2. the Python-runtime read surface (``sys.prefix`` / ``sys.base_prefix`` /
       the interpreter dir / ``site.getsitepackages()``) — see
       :func:`_collect_system_read_prefixes`. NOTE: the STATIC OS read surface
       (``%SystemRoot%``, ``%ProgramFiles(x86)%``, ``%ProgramData%``,
       ``%APPDATA%`` + ``uv``, ``%LOCALAPPDATA%\\uv``, CatRoot/catroot2) was
       MOVED to ``factory/config/file_guard_paths.json`` (three-state op-mask,
       todo #12) and is delivered via :func:`resolve_file_guard_masked_paths`;
       only the DYNAMIC interpreter roots remain here;
...
def resolve_system_exec_allow_paths(
    ...
    NOW ALWAYS EMPTY. As part of the "static-path unification" (todo #12) the
    former static entries (``%SystemRoot%\\System32`` and ``%SystemRoot%``,
    both READ+EXECUTE) have been MOVED to
    ``factory/config/file_guard_paths.json`` as a single ``%SystemRoot%`` entry
    (read=allow, execute=allow, write=ask). Because the op-mask is prefix +
    component-boundary aware, that entry's execute=allow already covers the
    ``System32`` subtree, so no separate System32 rule is needed. The system
    EXECUTE surface is therefore expressed in config and delivered via
    :func:`resolve_file_guard_masked_paths`, not here.

    This function is RETAINED (empty) rather than removed to preserve the
    public API / ``__all__`` export and its call sites — the native seeder
    (:func:`apps.api._native_hook_rules.seed_native_guard_rules`) still iterates
    its result; an empty tuple simply seeds zero system-exec op-mask rules
    (a harmless 0-iteration loop, no KeyError / crash). Kept as a stable
    seam so a future host-specific system-exec path could be re-introduced in
    code without re-plumbing callers.

    ``container`` accepted for resolver-signature symmetry (unused).

    Never raises — degrades to empty tuple on failure.
    """
    _ = container  # system exec surface now lives in file_guard_paths.json
    return ()
