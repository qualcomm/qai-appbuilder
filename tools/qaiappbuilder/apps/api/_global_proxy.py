...
service catalog fetch, the ``web_fetch`` tool) should route through.

Design (edition-dual-form §8 "全局代理覆盖范围")
-----------------------------------------------
Two proxy mechanisms coexist in V2:

* **Mechanism B** ``ToolsSettings.global_proxy`` / ``network_proxy`` — what the
  frontend "设置 → 代理" actually writes; this module fronts mechanism B and it
  is the single source of proxy truth for every outbound client.
* **Mechanism C** the channels per-instance proxy (WeChat / Feishu), which
  stays independent and is NOT touched here.

A former **Mechanism A** (``qai.chat.infrastructure.proxy_helper``) was deleted:
nothing in the application ever initialised it, so every consumer that fell back
to it silently got "no proxy" — the browser engine connected directly even with a
proxy configured. Its readers now use the live seam mechanism B keeps updated.
...
already-wired download-engine / catalog / web_fetch outlets.
"""
...
# ``apps/api/_ai_coding_di.py`` (the web_fetch read side).
...
def embed_proxy_auth(container: "Container", bare_url: str | None) -> str | None:
    ...
    V1 parity: the persisted proxy URL carries no credentials;
    ...
    This is the shared implementation behind both the ``web_fetch`` proxy
