...
__all__ = [
    ...
    "push_scheduled_task_to_channels",
]
...
async def push_scheduled_task_to_channels(
    *,
    container: "Container",
    event: "ScheduledTaskFired",
) -> None:
    """Push a scheduled-task result to the channel user bound to its conversation.

    Called (fire-and-forget) by :class:`ScheduledTaskAsideBridge` on each
    :class:`ScheduledTaskFired`. A scheduled task fires headlessly (the runner
    drives ``collect_completion``, bypassing the SSE/WS routes that normally
    trigger :func:`schedule_channel_sync_push`), so without this a task created
    from Feishu / WeChat would deliver its result only into the WebUI — the
    channel user who scheduled it would never be notified in-app.

    We reuse the same resolution the WebUI sync path uses: scan the RUNNING
    channel instances (both kinds), look up the task's ``conversation_id`` in
    each instance's :class:`ChannelBindings`, and push a single line to any
    bound user. Best-effort — a lookup / push failure is logged and swallowed
    so it can never disturb the scheduler tick. No binding ⇒ silent no-op (the
    task was created in the WebUI, not a channel; WebUI already shows it).
    """
    try:
        from qai.channels.domain import ChannelKind, ChannelUserId
        from qai.channels.application.use_cases.push_message import (
        ...
        # Compose the channel line: "⏰ 定时任务「name」已运行:\n<result>"
        # (or 失败 when the run failed). Mirrors the WebUI toast wording.
        head = (
            f"⏰ 定时任务「{event.task_name}」已运行："
            if event.ok
            else f"⏰ 定时任务「{event.task_name}」失败："
        )
        body = event.result_text.strip()
        ...
        for kind in (ChannelKind.WECHAT, ChannelKind.FEISHU):
            ...
            for instance in instances:
                if not instance.is_running():
                    continue
                target = instance.get_bindings().lookup(conversation_id)
                if not target:
                    continue
                await push_uc.execute(
                    PushChannelMessageCommand(
                        instance_id=instance.instance_id,
                        target=ChannelUserId(target),
                        text=text,
                    )
                )
                logger.info(
                    "scheduling.channel_push.success",
                    kind=instance.kind.value,
                    conversation_id=conversation_id,
        ...
        logger.warning(
            "scheduling.channel_push.failed",
