...
def register_appbuilder_tools_into_chat(
    *,
    ...
    file_guard:
        ...
        that look like a path are routed through ``enforce_read`` (the SAME
...
class _AppBuilderToolHandler:
    ...
    async def execute_run(self, request: ToolInvocationRequest) -> str:
        ...
        try:
            ...
            # ``_InputRejected`` is raised by ``_validate_inputs`` when the
            # FileGuard bridge (``enforce_read``) denies an inputs path — the
            # denial source is verified (2026-07-23). Route through the
            # ``confirmed_fileguard`` variant to get the strong ``[FileGuard]``
            # V2 guidance (enforced boundary / retry-won't-work / ask user to
            # authorize) rather than the neutral textual variant.
            from ._chat_tool_result_render import _maybe_append_fileguard_hint
            return _maybe_append_fileguard_hint(
                f"[tool_error] {exc}",
    ...
    async def execute_batch(self, request: ToolInvocationRequest) -> str:
        ...
        for i, item in enumerate(batch):
            ...
            try:
                ...
                # Confirmed FileGuard denial — see the twin call in
                # ``execute_run`` above.
                from ._chat_tool_result_render import _maybe_append_fileguard_hint
                return _maybe_append_fileguard_hint(
                    f"[tool_error] batch[{i}].inputs rejected: {exc}",
                    attribution="confirmed_fileguard",
                )
