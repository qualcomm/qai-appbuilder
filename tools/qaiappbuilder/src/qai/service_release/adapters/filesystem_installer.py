...
from qai.platform.genie_model_config import resolve_model_config_path
...
_LOGGER = get_logger("qai.service_release.installer")
...
def _variant_id_from_model_config(model_dir: Path) -> str:
    """Recover the installed variant_id from a model's single-model config paths.
    ...
    Genie's single-model config (``genie_config.json`` preferred,
    ``config.json`` fallback — see
    :func:`qai.platform.genie_model_config.resolve_model_config_path`) embeds
    the ORIGINAL variant directory in every internal path it references —
    tokenizer / ctx-bins / extensions all look like
    ``models/<variant_id>/<file>`` (e.g.
    ...
    """
    config_path = resolve_model_config_path(model_dir)
...
def _install_dir_name(zip_stem: str, version: str) -> str:
    """Install dir name for ``bin/``, guaranteed to carry ``version``.

    ``scan_versions`` recovers the installed version by regexing the dir
    name (``_INSTALLED_VER_RE``), and ``_install_dirs_for`` /
    ``delete_installed_service`` match on it too — so the version MUST be
    present or an install is silently invisible ("not installed" in the UI,
    no ``root_path`` auto-configure, delete 404s).

    Historically the dir was named after the zip stem alone, which happened
    to embed it (``GenieAPIService_v2.3.7_QAIRT_v2.44.0_v73``). Release
    v2.48.40 renamed the asset to ``GenieService-win-arm64.zip`` — a single
    archive now carrying BOTH driver stubs (v73 + v81) — whose stem has no
    version at all, so the old naming dropped it on the floor even though
    ``install_service`` is handed an explicit ``version``.

    We keep the zip stem (it stays recognisable / distinguishes concurrent
    platform builds) whenever it already *recovers* the right version, so
    existing installs keep their historical names byte-for-byte and are
    neither renamed nor duplicated. Otherwise we prepend the version, since
    ``_INSTALLED_VER_RE`` reads the FIRST ``_v<x.y[.z]>`` in the name —
    appending to a stem that already embeds a different version (e.g.
    ``GenieAPIService_v2.3.7_QAIRT_v2.44.0_v73`` + ``_v2.48.40``) would still
    scan back as ``2.3.7`` and mis-attribute the install.

    The post-condition is checked rather than assumed: the returned name MUST
    regex back to exactly ``version``, else the install goes invisible.
    """
    stem = _safe_dir_segment(zip_stem) or "GenieAPIService"
    ver = version.strip()
    if not ver:
        return stem

    def _recovers(name: str) -> bool:
        found = _INSTALLED_VER_RE.search(name)
        return found is not None and found.group(1) == ver

    if _recovers(stem):
        return stem
    # Prepending puts our version first, so it wins the FIRST-match rule
    # regardless of what the stem embeds.
    candidate = f"GenieAPIService_v{ver}_{stem}"
    if _recovers(candidate):
        return candidate
    # Defensive: an exotic version string the regex can't read back. Fall
    # back to the bare stem rather than emit a name that lies about it.
    return stem


def _long_path(path: Path) -> str:
    r"""Extended-length form (``\\?\...``) of ``path`` on Windows.

    Bypasses the 260-char ``MAX_PATH`` limit for the raw Win32 filesystem
    calls ``zipfile``/``shutil`` make, without requiring the
    ``LongPathsEnabled`` registry key. Needed because AI Hub model archives
    combine long descriptive names (e.g.
    ``qwen3_vl_8b_instruct-geniex_qairt-w4a16-qualcomm_snapdragon_x_elite``)
    with a deep install root (e.g. under a synced OneDrive tree), which
    otherwise trips ``WinError 206`` mid-extraction. No-op on non-Windows.
    """
    if os.name != "nt":
        return str(path)
...
def _extract_zip_to_dir(zip_path: Path, dest_dir: Path, *, tmp_base: Path) -> None:
    ...
    All filesystem calls go through :func:`_long_path` so a long archive name
    stacked on a deep install root can't trip ``WinError 206``.
    """
    try:
        with zipfile.ZipFile(zip_path) as zf:
            ...
            if len(top_level) == 1:
                ...
                shutil.rmtree(_long_path(tmp_extract), ignore_errors=True)
                os.makedirs(_long_path(tmp_extract), exist_ok=True)
                zf.extractall(_long_path(tmp_extract))
                inner = tmp_extract / next(iter(top_level))
                source = inner if os.path.isdir(_long_path(inner)) else tmp_extract
                os.makedirs(_long_path(dest_dir), exist_ok=True)
                for name in os.listdir(_long_path(source)):
                    shutil.move(
                        _long_path(source / name), _long_path(dest_dir / name)
                    )
                shutil.rmtree(_long_path(tmp_extract), ignore_errors=True)
            else:
                os.makedirs(_long_path(dest_dir), exist_ok=True)
...
def _rename_retrying(source: Path, target: Path) -> None:
    """``Path.rename`` that copes with a transient Windows directory lock.

    Reinstalling stages the previous install aside as ``<name>.bak`` before
    extracting. That rename can fail with ``WinError 5`` even when nothing of
    ours holds the dir: an antivirus / Search-indexer scan of the freshly
    written ``GenieAPIService.exe`` briefly opens it without
    ``FILE_SHARE_DELETE``, which blocks renaming the *parent*. The window is
    tens of milliseconds, so a short escalating retry clears it.

    Mirrors :func:`_rmtree_unlocking`: try once, then kill any process
    genuinely running out of the dir (a live daemon is a real lock, not a
    scanner) and retry briefly. Raises the final :class:`OSError` so the
    caller can surface an actionable message.
    """
    with suppress(OSError):
        source.rename(target)
        return
    _kill_processes_under(source)
    last_exc: OSError | None = None
    for _attempt in range(5):
        time.sleep(0.4)  # let the scanner / OS release the handle
        try:
            source.rename(target)
        except OSError as exc:
            last_exc = exc
...
class FileSystemArtifactInstaller(ArtifactInstallerPort):
    ...
    async def install_service(
        ...
        dest_dir = bin_dir / _install_dir_name(zip_path.stem, version)
        # ``with_suffix`` would treat the ``.7`` of a version like ``2.3.7``
        # as the extension and truncate it (``..._v2.3.7`` -> ``..._v2.3.bak``),
        # yielding a backup whose name no longer matches its source and still
        # regexes as a *different* version. Append instead.
        bak_dir = dest_dir.with_name(dest_dir.name + ".bak")
        if dest_dir.exists():
            if bak_dir.exists():
                shutil.rmtree(bak_dir, ignore_errors=True)
            _rename_retrying(dest_dir, bak_dir)
        try:
            _extract_zip_to_dir(zip_path, dest_dir, tmp_base=bin_dir)
        except InstallFailedError:
            if bak_dir.exists() and not dest_dir.exists():
                _rename_retrying(bak_dir, dest_dir)
            raise
        if bak_dir.exists():
            shutil.rmtree(bak_dir, ignore_errors=True)
    ...
    async def install_model(
        ...
        if not zip_path.exists() or zip_path.suffix.lower() != ".zip":
            # A successful install DELETES the source zip (see
            # ``_delete_zip_after_install`` below), so a missing zip is
            # ambiguous: either the caller passed a bad path, or the model is
            # ALREADY installed and this is a repeat request. Distinguish the
            # two by looking at the target dir before failing — reporting
            # "invalid save_path" for an already-installed model told the user
            # the install had failed when it had in fact succeeded.
            existing = self._installed_model_dir(models_root, model_id, install_dir)
            if existing is not None:
                _LOGGER.info(
                    "model_install.already_installed",
                    model_id=model_id,
                    install_path=str(existing),
                    save_path=save_path,
                )
                return ModelInstallResult(
                    ok=True,
                    install_path=str(existing),
                    model_id=model_id or existing.name,
                    zip_deleted=True,
                )
            _LOGGER.warning(
                "model_install.invalid_save_path",
                model_id=model_id,
                save_path=save_path,
        ...
        if marker_variant:
            try:
                ...
                pass
        zip_deleted = _delete_zip_after_install(zip_path)
        # Logged because the zip is now GONE: a later repeat request can only be
        # told apart from a genuinely bad path by the disk state, and without
        # this line the sequence "installed → zip deleted → repeat POST" left no
        # server-side trace at all.
        _LOGGER.info(
            "model_install.ok",
            model_id=model_id or dest_name,
            install_path=str(dest_dir),
            variant_id=marker_variant,
            zip_deleted=zip_deleted,
        )
        return ModelInstallResult(
            ok=True,
    ...
    @classmethod
    def _installed_model_dir(
        cls, models_root: Path, model_id: str, install_dir: str
    ) -> Path | None:
        """Return an EXISTING, non-empty install dir for this model, else None.

        Used to make :meth:`install_model` idempotent: because a successful
        install deletes the source zip, a repeat request cannot be told from a
        genuinely bad ``save_path`` by looking at the zip alone. A dir that
        exists and holds at least one file is treated as installed, matching
        what :meth:`scan_models` reports to the UI (State-Truth-First: the
        answer comes from the disk, not from remembered request state).

        Tries the caller's ``install_dir`` hint first, then the lenient
        ``model_id`` match so a legacy per-variant layout still resolves.
        """
        candidates: list[Path] = []
        hint = _safe_dir_segment(install_dir)
        if hint:
            candidates.append(models_root / hint)
        if model_id:
            candidates.extend(cls._model_dirs_matching(models_root, model_id))
        ...
        if self._paths.models_dir.exists():
            for child in self._paths.models_dir.iterdir():
                if not child.is_dir() or child.name.endswith(".bak"):
                    continue
                has_model = (
                    resolve_model_config_path(child) is not None
                ...
                if not drv:
                    ...
                    # in the model's single-model config internal paths
