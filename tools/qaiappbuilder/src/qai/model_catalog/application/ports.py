...
from qai.model_catalog.domain.qgenie_quota import QGenieQuotaSnapshot
...
@runtime_checkable
class QGenieQuotaPort(Protocol):
    """Read the account's dual-bucket QGenie quota.

    Separate from :class:`ProviderRegistryPort` (which only knows the locally
    stored config) and deliberately NOT folded into the qai-service token-pool
    path: that balance rides along on every broker answer as a
    ``quota_usage`` frame, whereas QGenie exposes no such push and must be
    polled over its own REST endpoints. The two also carry different shapes —
    a single pool versus two buckets x four windows — so a shared abstraction
    would fit neither.

    Adapters own the transport, the upstream's ~60s rate limit on these
    endpoints, and the last-good-snapshot cache, so callers can treat this as
    a plain read that never raises.
    """

    async def fetch(
        self, *, base_url: str, api_key: str | None, force: bool = False
    ) -> "QGenieQuotaSnapshot":
        """Return the current quota, or the last good snapshot marked stale.

        ``force`` bypasses the cache. It exists for the exhaustion check,
        which MUST decide on fresh data: acting on a cached figure could send
        a request to a bucket that has since run dry, or worse, conclude both
        are dry and tell the user to switch models when one is actually free.
        """
        ...
