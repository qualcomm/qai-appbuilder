...
This adapter reads/writes ``model_catalog_skill``, the canonical (and,
since migration 072, the only) skill registry table. The former sibling
``ai_coding_skill`` (§4.5) was dropped once PR-103 routed the ai_coding
``SkillRegistryPort`` here through
``apps/api/_skill_registry_bridge.SkillRegistryBridge`` — the bridge lives
in ``apps/api`` so neither context imports the other (import-linter
``context-isolation``). See schema doc §10.3.
"""
