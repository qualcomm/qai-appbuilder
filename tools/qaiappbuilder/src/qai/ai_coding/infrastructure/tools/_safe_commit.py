...
def backup_to_trash(
    ...
    *,
    ...
    Each manifest line carries the two fields a restore needs — ``source_path``
    (absolute path the bytes came from) and ``backup_path`` (where they were
    saved) — plus ``original_sha256`` so a restore can verify it wrote back
    exactly what was taken. ``layout_key`` is the flattened trash-tree
    directory name and is NOT openable; never restore through it.
    ...
    try:
        ...
        record: dict[str, Any] = {
            ...
            "tool": tool,
            # The absolute path the backup was taken FROM — the only field a
            # restore can act on. This was previously recorded as ``path``
            # holding ``rel`` (the flattened trash-layout key), which LOOKED
            # like a filesystem path but resolved to a bogus location
            # (``C_/Users/...``): the ledger said which bytes were saved but
            # not where to put them back, so it could not drive a recovery.
            "source_path": str(path),
            # Trash-tree layout key: drive letter and separators flattened by
            # ``_relative_for_trash`` so it can name a directory. Diagnostics
            # and grouping ONLY — never a path to open.
            "layout_key": str(rel).replace("\\", "/"),
