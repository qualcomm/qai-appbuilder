...
* :class:`PatternFileScreen` — a **pure-software safety layer** that runs
  ...
  - pre-call **hard-protected write** rejection for write/edit tools
    (injected ``write_dir_guard``; backed by ``qai.platform.protected_paths``
    via the apps-layer bridge — the same floor FileGuard enforces. It is the
    FLOOR ONLY: path policy for system roots / temp / profile trees lives in
    ``factory/config/file_guard_paths.json``, because this screen runs first
    and a second list here would silently override that config);
...
class PatternFileScreen:
    ...
    def _recount_glob_message(
        ...
        TRUNCATED results are exempt. There the leading count is the TOTAL
        number of matches (``total_matched``), deliberately larger than the
        shown ``files`` list, and the message says so ("25 file(s) matched;
        showing the 5 most-recently-modified"). Recounting it to the length of
        the filtered preview produced a self-contradicting message — "4 file(s)
        matched ... Glob matched 25 files" — i.e. the same "a capped count
        presented as the total" defect this count was just fixed to avoid.
        ``files`` is only a sample there, so it is NOT authoritative for the
        count.
        """
