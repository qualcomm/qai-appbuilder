...
This applies to ``cmd`` ONLY. ``sh``/``bash`` and powershell receive their
multi-line payload intact (argv is passed straight to
``create_subprocess_exec``, so no shell re-parses the string), which is why
neither is materialised below.
...
The rewriter returns the materialised temp path(s) alongside the rewritten
argv; the caller unlinks them after the child completes
