...
async def _glob_with_ripgrep(
    ...
    *,
    ...
    # ``rg --files`` honours ``.gitignore`` by default, which HIDES exactly the
    # files an operator most often needs to inspect deliberately: ``.env*``,
    # logs, build output. ``respect_gitignore=False`` lifts that filter.
    if not respect_gitignore:
        cmd.append("--no-ignore")
    # ripgrep skips dotfiles unless asked; ``include_hidden`` surfaces them
    # (pair with ``respect_gitignore=False`` to reach ignored dotfiles).
    if include_hidden:
        cmd.append("--hidden")
...
async def _glob_multi_target(
    ...
    *,
    ...
    """Glob each target independently and return the de-duplicated union.

    Delegates to the single-target :func:`tool_glob` per entry so the ripgrep
    track, policy filtering and per-target caps are reused verbatim. Results
    are merged preserving each target's newest-first order, then de-duplicated
    (a file matching two patterns is listed once).

    A target that matches nothing contributes nothing rather than failing the
    call — with several patterns, one empty target is normal, not an error.
    """
    seen: set[str] = set()
    files: list[str] = []
    failures: list[str] = []
    ...
    for target in targets:
        ...
        single_args["pattern"] = target
        try:
            result = await tool_glob(
                single_args,
        ...
        if not result.get("ok"):
            failures.append(f"{target}: {result.get('message', 'failed')}")
            continue
        ...
        truncated_any = truncated_any or bool(result.get("truncated"))
        # A per-target overflow was persisted by that target's own render;
        # keep the retrieval path or the complete list becomes unreachable.
        stored = result.get("stored_path")
        if isinstance(stored, str) and stored:
            stored_paths.append(stored)

    scope = "; ".join(targets)
    if not files:
        message = f"(no files matched any of {len(targets)} pattern(s): {scope}"
        if failures:
            message += f"; errors: {'; '.join(failures)}"
        message += ")"
        return _ok(message, files=[], truncated=False, patterns=targets)

    # Each target was capped independently, so the union can be N times the
    # single-call ceiling. Re-cap the merged list to the SAME budget a
    # single-pattern call honours, otherwise asking for four patterns quietly
    # returns four times the output a caller (and the context window) expects.
    max_results = get_tool_output_thresholds().glob_max_results
    total_found = len(files)
    merge_capped = total_found > max_results
    # Persist the COMPLETE union BEFORE the cut so the elided paths keep a
    # retrieval path, exactly as the single-target render does. Without this
    # the merge cap dropped paths with no way to get them back — the caller
    # cannot even re-run per pattern, because each per-target call is capped
    # too and the union is what overflowed.
    stored_path: str | None = None
    if merge_capped:
        stored_path = _maybe_store_full_result(
            "\n".join(files), tool_name="glob", store=tool_result_store
        )
        files = files[:max_results]

    message = (
        f"{total_found} file(s) matched across {len(targets)} pattern(s): {scope}"
    )
    if merge_capped:
        message += f"; showing the first {max_results}"
        if stored_path is not None:
            message += (
                f" — the COMPLETE list was saved, call "
                f"read(path={stored_path}) to see all {total_found} paths"
            )
        else:
            message += (
                " — narrow the patterns or glob them one at a time for the rest"
            )
    if truncated_any:
        message += "; one or more patterns also hit their own result cap"
    if failures:
        message += f"; errors: {'; '.join(failures)}"
    result = _ok(
        message,
        files=files,
        truncated=truncated_any or merge_capped,
        patterns=targets,
    )
    if backends:
        result["backend"] = "+".join(sorted(backends))
    if stored_path is not None:
...
async def tool_glob(
    ...
    *,
    ...
    # Multi-target glob: ``"src/**/*.py; tests/**/*.py"`` in one call. Each
    # target is globbed independently and the union is returned, so unrelated
    # subtrees are never dragged in by widening to a common ancestor.
    targets = _split_targets(pattern)
    if len(targets) > 1:
        return await _glob_multi_target(
            args,
            targets=targets,
            file_guard=file_guard,
            tool_result_store=tool_result_store,
    ...
    # ``.gitignore`` is respected by default (matching ripgrep); turning it off
    # is how the model reaches ``.env*`` / logs / build output on purpose.
    ...
    if cwd_str:
        # H-Py-1 fix: relative ``cwd`` MUST resolve under the per-request
        # workspace base BEFORE it drives ``.exists()`` / ``.is_dir()`` /
        # ``enforce_read`` — otherwise those operations anchor on the
        # daemon process cwd and bypass the workspace binding (the
        # auto-approve short-circuit / grant matching would then be
        # computed against the wrong absolute path). Matches
        # ``tool_grep`` (line 1455) and every read/write/patch handler.
        cwd_str = resolve_under_workspace(cwd_str)

    # M10: heartbeat for a glob that may traverse a large tree.
    from qai.platform.tool_progress import emit_progress
    emit_progress(f"glob “{pattern[:60]}”…", "match")
    ...
    rg_result = await _glob_with_ripgrep(
        root,
        pattern,
        extra_skip,
    ...
    if rg_result is not None:
        ...
        # The pure-Python walk has no .gitignore / dotfile machinery: it never
        # consulted ignore files (so nothing is hidden by them) and it matches
        # dotfiles like any other name. When the caller explicitly asked to
        # change that behaviour we must say the request could not be honoured
        # — silently ignoring it is how "I asked for ignored files and got
        # none" becomes a wrong conclusion about the tree.
        flags_unsupported = (not respect_gitignore) or include_hidden
    ...
    if flags_unsupported:
        result["message"] = (
            f"{result.get('message', '')}\n[note] ripgrep is unavailable, so "
            f"the pure-Python walk ran instead: it does not read .gitignore "
            f"(nothing was hidden by ignore rules) and always matches "
            f"dotfiles, so 'gitignore'/'hidden' had no effect here."
        )
    return result
...
def _render_glob_inner(
    ...
    *,
    ...
    # State the TRUE match count, not the number of paths that survived the
    # preview cut. ``len(files)`` is the SHOWN count: reporting it as
    # "N file(s) matched" told the model the search found exactly what it can
    # see (25 matches capped at 5 read as "5 file(s) matched ... more than 5"),
    # the same "a capped count presented as the total" defect fixed in ``read``
    # and ``grep``. ``total_matched`` is the real figure and is always known
    # here, so it is always reported — the store (which supplies a retrieval
    # path) is an optional extra, never a precondition for telling the truth.
    if scan_capped:
        msg = (
            f"{total_matched} file(s) matched; showing the {max_results} "
        ...
        if stored_path is not None:
            ...
            # ``make_glob_advice`` carries its own "[truncation note]" prefix
            # and only says "more than N" (N = the cap), so it cannot state the
            # real figure. Emit the exact total here and reuse only the
            # actionable half of the advice.
            msg += (
                f"[truncation note] Glob matched {total_matched} files but only "
                f"the {max_results} most-recently-modified are shown above -- "
                f"do NOT treat them as the complete set. Tighten the `pattern` "
                f"(e.g. add a subdirectory prefix or a stricter extension "
                f"filter) or split the search into smaller scopes."
            )
    else:
        msg = f"{total_matched} file(s) matched"
    if filtered_count > 0:
        msg += f"; {filtered_count} files filtered by policy"
...
async def _grep_with_ripgrep(
    *,
    ...
    # ``scan_capped`` means rg was KILLED at the streaming ceiling, so it never
    # finished searching: ``match_count`` / ``file_count`` are "what we saw
    # before stopping", NOT totals. The Python fallback already flags this via
    # ``incomplete``; the rg path used to report the partial counts in the same
    # shape as an exhaustive search ("12 match(es) in 3 file(s)"), so the model
    # concluded it had every hit and stopped looking. Say INCOMPLETE explicitly
    # and mark the result so downstream layers preserve that meaning.
    if scan_capped:
        summary = (
            f"INCOMPLETE: too much output to scan -- ripgrep was stopped after "
            f"~{GREP_MAX_SCAN_BYTES // (1024 * 1024)}MB WITHOUT finishing the "
            f"search. Found {match_count} match(es) in {file_count} file(s) SO "
            f"FAR (a PARTIAL, non-exhaustive sample -- more files were NOT "
            f"searched; do NOT treat these counts as totals). Narrow the scope "
            f"(grep a specific sub-directory or pass a tighter `include`) for a "
            f"complete search."
        )
    else:
...
def _apply_grep_inprompt_caps(
    ...
    * re-renders ``output`` grouped by file ONLY when something was actually
    ...
    # File counts for BOTH populations: the visible sample and the real set.
    # ``capped_matches`` is the post-cut preview, so counting distinct paths in
    # it yields the SHOWN file count — reporting that as "in N file(s)" claimed
    # the matches live in fewer files than they do (1763 matches across 7 files
    # were summarised as "100 match(es) in 1 file(s)").
    file_count = len({str(m.get("path", "")) for m in capped_matches})
    total_file_count = len({str(m.get("path", "")) for m in ordered})
    ...
    # An INCOMPLETE upstream scan (walk / content / rg streaming cap stopped
    # before finishing) makes every count here a "so far" figure. This summary
    # is REBUILT from scratch, so the upstream INCOMPLETE prose is gone by now;
    # only the structured ``incomplete`` field survived, which the model does
    # not read. Re-state the warning in the TEXT or a partial search reads
    # exactly like an exhaustive one.
    was_incomplete = bool(result.get("incomplete"))
    if was_incomplete:
        summary = (
            f"INCOMPLETE: the search stopped early WITHOUT finishing -- "
            f"{len(capped_matches)} match(es) in {file_count} file(s) SO FAR "
            f"(a PARTIAL, non-exhaustive sample; do NOT treat these counts as "
            f"totals)"
        )
        if count_capped:
            summary += f"; showing the {max_matches} newest-file matches"
        summary += (
            ". Narrow the scope (grep a specific sub-directory or pass a "
            "tighter `include`) for a complete search."
        )
    else:
        if count_capped:
            # State the TRUE totals; name the sample separately so the two are
            # never conflated (same fix as ``glob``'s match count).
            summary = (
                f"({total_matches} match(es) in {total_file_count} file(s)"
                f"; showing only the {len(capped_matches)} newest-file matches"
...
def _split_targets(raw: str) -> list[str]:
    """Split a semicolon-delimited target list into individual entries.

    ``"src; tests"`` → ``["src", "tests"]``. Empty entries are dropped so a
    trailing ``;`` is harmless. A plain path with no ``;`` yields a
    single-element list, which is why the callers can treat one and many
    uniformly.

    Windows paths are safe here: the separator is ``;`` and a drive letter
    uses ``:``, so ``C:\\a; D:\\b`` splits correctly. (``;`` is legal in a
    POSIX filename but vanishingly rare, and the ambiguity is unavoidable
    once a delimiter is offered at all.)
    """
...
async def _grep_multi_root(
    ...
    *,
    ...
    """Run ``grep`` against each root and merge the results into one receipt.

    Each root is searched by a normal single-root ``tool_grep`` call, so every
    existing capability (ripgrep track, policy filtering, per-root caps) is
    reused verbatim rather than reimplemented. A root that does not exist is
    reported and skipped instead of failing the whole call — with several
    roots, one bad entry should not discard the good ones.

    The in-prompt caps are applied ONCE over the merged match set so the
    combined output honours the same ceilings as a single-root search.
    """
    ...
    for root in roots:
        ...
        try:
            ...
            if "path not found" in message:
                ...
                failures.append(f"{root}: {message}")
            continue
        if not result.get("ok"):
            failures.append(f"{root}: {result.get('message', 'failed')}")
            continue
        found = result.get("matches")
        if isinstance(found, list):
            merged.extend(found)
        backend = result.get("backend")
        if isinstance(backend, str) and backend:
    ...
    scope = "; ".join(roots)
    if not merged:
        parts = [f"(no matches found for pattern: {pattern!r} in {scope}"]
        if missing:
            parts.append(f"; not found: {', '.join(missing)}")
        if failures:
            parts.append(f"; errors: {'; '.join(failures)}")
        parts.append(")")
        return _ok(
            "".join(parts),
            matches=[],
    ...
    file_count = len({str(m.get("path", "")) for m in merged})
    summary = (
        f"({len(merged)} match(es) in {file_count} file(s) "
        f"across {len(roots)} root(s): {scope}"
    )
    if truncated_any:
        # Each root was searched (and capped) on its own, so a per-root cap
        # means these numbers are "what we saw", not totals. Saying so stops
        # the model concluding it has every hit and moving on.
        summary += (
            "; INCOMPLETE — at least one root hit its own result cap, so "
            "these counts are a partial sample, not totals. Narrow the "
            "pattern or search a single root for a complete answer"
        )
    if missing:
        summary += f"; not found: {', '.join(missing)}"
    if failures:
        summary += f"; errors: {'; '.join(failures)}"
    summary += ")"
    ...
    combined = _ok(
        ...
        output=_render_grep_matches_text(merged),
        backend="+".join(sorted(backends)) if backends else "python",
        roots=roots,
    )
    if truncated_any:
        # ``_apply_grep_inprompt_caps`` REBUILDS the summary from scratch, so
        # the prose warning above is discarded. Only the structured
        # ``incomplete`` flag survives that rebuild, and it is what makes the
        # cap pass re-state "PARTIAL, non-exhaustive sample" — without it a
        # merge of per-root-capped results reads exactly like an exhaustive
        # search and reports its partial figure as the true total.
        combined["incomplete"] = True
    if stored_paths:
        combined["stored_paths"] = stored_paths
    ...
    # The per-root recovery files hold the matches each root elided BEFORE the
    # merge, so they stay the only way to reach those hits. The cap pass builds
    # a fresh envelope and would otherwise drop them.
    if stored_paths and "stored_paths" not in capped:
        capped["stored_paths"] = stored_paths
    return capped
...
async def tool_grep(
    ...
    *,
    ...
    # Multi-root search: ``"src; tests"`` scopes ONE call to several roots.
    # Searching each root separately and merging is strictly better than
    # widening to a common ancestor (which drags in unrelated subtrees) or
    # making the model issue N calls (N round-trips, N partial caps).
    roots = _split_targets(path_str)
    if len(roots) > 1:
        return await _grep_multi_root(
            args,
            roots=roots,
            pattern=pattern,
            file_guard=file_guard,
    ...
    if roots:
        target = Path(resolve_under_workspace(roots[0]))
    else:
    ...
    # M10: heartbeat for a grep that may traverse a large tree.
    from qai.platform.tool_progress import emit_progress
    emit_progress(f"grep “{pattern[:40]}”…", "match")

    # 7-L3: merge user-configured project_skip_dirs with the defaults.
    extra_skip = get_project_skip_dirs()

    # L-Py-7: align grep with glob — per-file filtering only kicks in when
    # the search ``target`` is in the STATIC allowlist. A dynamically
    # (ASK) authorised directory implies a tree-wide grant, so re-filtering
    # per file would over-restrict a grant the user already gave. Semantics
    # now match :func:`_glob_filter_by_policy`.
    is_statically_allowed = getattr(file_guard, "is_statically_allowed", None)
    if callable(is_statically_allowed) and await is_statically_allowed(
        path=str(target)
    ):
        # STATIC allow: per-file / per-line read probe (V1
        # ``backend/tools/_grep.py:122-178`` ripgrep + ``:206-233`` Python
        # fallback). Fail-open per-file.
        path_allowed = _make_path_allowed(file_guard)
    else:
        # Dynamic ASK grant OR guard lacks the static probe: tree-wide
        # authorisation is in effect (or the guard is too light to say),
        # so skip per-file filtering entirely. A no-op probe keeps the
        # downstream filter calls tidy without narrowing the grant.
        async def path_allowed(_path: str) -> bool:  # noqa: E306
            return True
