...
async def tool_apply_patch(
    ...
    *,
    ...
    # M10: emit a keep-alive for a multi-file patch (matching + rewrite +
    # verify across many files takes seconds).
    if len(patch.ops) >= 5:
        from qai.platform.tool_progress import emit_progress
        emit_progress(
            f"applying patch across {len(patch.ops)} file(s)…", "match",
        )
    ...
    async def _guarded_apply() -> dict[str, Any]:
        ...
        for op in patch.ops:
            ...
            # A rename/move CREATES a new path (the move target) AND REMOVES
            # the source path. Both events must clear their gates BEFORE any
            # IO. The source already passed enforce_read (UPDATE) + enforce_write
            # above; here we additionally route the source through enforce_delete
            # so the audit trail records BOTH a write (of the new path) and a
            # delete (of the old path) — parity with the SEC-ENHANCE-AUDITUX-1
            # DELETE routing.
            ...
            if move_target:
                ...
                await file_guard.enforce_delete(
                    path=op.path,
                    caller="ai_coding.tool.apply_patch.move_source",
                )
