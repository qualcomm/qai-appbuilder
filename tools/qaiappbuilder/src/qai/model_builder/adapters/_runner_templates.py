...
def _mapping_at(obj: Any, key: str) -> dict[str, Any]:
    """Return ``obj[key]`` only when both ``obj`` and that value are mappings.

    The ``inference_manifest`` is user/AI-authored free-form JSON: a nested key
    the templates expect to be an object (``input`` / ``preprocessing`` /
    ``output`` / ``postprocessing``) may instead arrive as a bare string, list
    or number. The previous ``(x or {}).get(...)`` idiom only guarded against
    ``None``/falsy — a NON-empty string sailed through and then blew up on the
    next ``.get`` with ``AttributeError: 'str' object has no attribute 'get'``,
    failing the whole Pack export. This coerces any non-mapping (including a
    non-mapping ``obj``) to an empty dict so template rendering always falls
    back to its defaults instead of crashing.
    """
...
def render_runner(
    ...
    * everything else -> generic template.
    ...
    # ``infer_manifest`` is untrusted free-form JSON — walk it defensively so a
    # key that arrives as a string/list instead of the expected object falls
    # back to defaults rather than crashing the export (see ``_mapping_at``).
    preproc = _mapping_at(_mapping_at(infer_manifest, "input"), "preprocessing")
    ...
    output_meta = _mapping_at(infer_manifest, "output")
    subcategory_raw = output_meta.get("type", "")
    subcategory = subcategory_raw if isinstance(subcategory_raw, str) else ""
...
def runner_detect(pack_id, model_name, weights_filename, input_h, input_w, timestamp,
    ...
    # Extract detection-specific params from manifest. ``infer_manifest`` is
    # untrusted free-form JSON — use ``_mapping_at`` so a non-object value for
    # ``postprocessing`` / ``output`` degrades to defaults instead of raising
    # ``AttributeError`` and failing the Pack export.
    postproc = _mapping_at(infer_manifest, "postprocessing")
    conf_threshold = postproc.get("confidence_threshold", 0.45)
    iou_threshold = postproc.get("nms_iou_threshold", 0.7)
    num_classes = _mapping_at(infer_manifest, "output").get("num_classes", 80)
