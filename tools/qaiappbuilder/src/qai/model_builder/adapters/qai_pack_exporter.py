...
from qai.platform.model_weights import build_search_report
...
class QaiPackExporter:
    ...
    def _locate_context_binaries(
        ...
        *,
        ...
        if default_precision.plan_key not in per_precision:
            # Attach an evidence-backed diagnostic: which directories were
            # walked, which weight-like files were seen and why each was
            # rejected, plus the concrete next step. An agent driving the
            # model-builder SKILL can self-diagnose from this instead of
            # stalling on a bare "not found".
            report = build_search_report(
                workspace.output_dir,
                requested_precision=default_precision.plan_key,
                found_precisions=tuple(sorted(per_precision)),
                min_bytes=MIN_CONTEXT_BIN_SIZE,
            )
            raise MissingContextBinError(report.to_message())
    ...
    def _write_requirements(
        ...
        *,
        ...
        Pins ``qai-appbuilder`` to at least the currently installed version so
        that a packaged app's ``ensure_deps.py`` auto-upgrades a stale SDK on
        the target machine (prevents "newer context binary on old SDK" crashes).
        """
        log.append("[INFO] Generating requirements.txt ...")
        # Detect installed qai-appbuilder version for the pin.
        qai_ver = ""
        try:
            ...
            pass
        qai_line = f"qai-appbuilder>={qai_ver}\n" if qai_ver else "qai-appbuilder\n"
