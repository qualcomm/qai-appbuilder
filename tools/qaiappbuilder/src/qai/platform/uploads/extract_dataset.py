...
V1 parity: a dataset upload that is a
        ...
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            for info in zf.infolist():
                ...
                if _is_unsafe_member_name(name):
                    raise DatasetExtractionError(
                        f"Archive contains unsafe member path: {name}"
                    )
                if info.is_dir():
                    continue
                members.append((name, zf.read(info)))
    except zipfile.BadZipFile as exc:
        raise DatasetExtractionError(f"Extraction failed: {exc}") from exc
    return members
        ...
        with tarfile.open(fileobj=io.BytesIO(content), mode="r:*") as tf:
            for member in tf.getmembers():
                ...
                if not (member.isfile() or member.isdir()):
                    raise DatasetExtractionError(
                        f"Archive contains unsupported member type: {name}"
                    )
                if member.isdir():
                    continue
