...
Design (mirrors V1 background-runner behaviour, but split into
...
class BackgroundTaskManager:
    ...
    def trigger_now(self, name: str) -> bool:
        """Fire a registered task ONCE immediately, off its periodic schedule.

        Schedules an extra :meth:`_invoke_safely` run as a fire-and-forget
        ``asyncio.Task`` on the current loop, WITHOUT disturbing the periodic
        ``_run_loop`` (its next tick still fires ``interval_seconds`` after the
        previous scheduled run). Used e.g. so a login event can push a fresh
        usage report right away instead of waiting for the next 12h tick.

        Returns ``True`` when the task exists, is not stopping, and a run was
        scheduled; ``False`` otherwise (unknown name / shutting down / no
        running loop). Never raises — a best-effort nudge, like the rest of
        this manager. Concurrent-run safe: ``_invoke_safely`` isolates its own
        exceptions, so an overlap with a scheduled tick cannot crash either.
        """
        if self._stopping:
            return False
        ...
        try:
            ...
            _log.warning("background.trigger_now_no_loop", task=name)
            return False
        loop.create_task(self._invoke_safely(st), name=f"bg-trigger-{name}")
        _log.debug("background.task_triggered", task=name)
        return True
