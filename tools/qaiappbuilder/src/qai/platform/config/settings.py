...
2. ``factory/config/server.toml`` (production deployments)
...
from .ports import backend_port as _default_backend_port
from .ports import cors_allow_origins as _default_cors_allow_origins

# Where ``factory/config/server.toml`` is searched for, relative to the repo root.
...
#: Version reported when no ``build_info.json`` is present — i.e. the dev
#: source tree. A FIXED literal on purpose: the previous implementation asked
#: ``importlib.metadata`` for the installed distribution version, which made
#: the reported version depend on *how the tree was installed* (editable
#: install → ``0.0.0.dev0``; plain checkout with no dist-info → the hardcoded
#: fallback). That non-determinism leaked straight into the FastAPI
#: ``info.version`` and therefore into the byte-pinned OpenAPI snapshot
#: (``tools/openapi/api-snapshot.json``), so the snapshot hash gate flipped
#: red or green according to the developer's install mode rather than to any
#: real API change. Resolving to a constant makes the dev-tree version
#: reproducible; a packaged artifact overrides it from ``build_info.json``.
...
#: The loopback host *name*, as distinct from :data:`LOOPBACK_HOST` (the dotted
#: quad). Needed where a literal IP is not interchangeable with the name: OAuth
#: authorization servers commonly accept only ``http://localhost`` as a native
#: loopback ``redirect_uri`` and reject ``http://127.0.0.1``. Safe to pair with
#: a :data:`LOOPBACK_HOST` bind because ``http.server.HTTPServer`` is
#: ``AF_INET``, so the name resolves to that same IPv4 socket rather than
#: ``::1``. Defined here so consumers need no literal of their own.
...
LOOPBACK_HOSTS: frozenset[str] = frozenset({LOOPBACK_HOST, "::1", LOOPBACK_HOST_NAME})
...
class ServerSettings(BaseModel):
    ...
    # Default port sourced from the language-neutral registry
    # ``factory/config/ports.json`` (``platform.config.ports``) — the single source of
    # truth shared with the frontend (Vite) and desktop (Tauri) runtimes. The
    # registry value is aligned with the Okta SSO redirect_uri registered on
    # the authorization server. ``Start.bat`` reads that same ``backend`` port
    # from ``factory/config/ports.json`` and passes it as ``--port`` to the supervisor
    # so the ACTUAL bind matches this
    # default; ``routes/auth.py`` derives ``redirect_uri`` from ``server.port``
    # so keeping the two in sync is critical — Okta rejects any mismatch.
    # Change the port in ``factory/config/ports.json`` only if the Okta app
    # registration is also updated to accept a different loopback port.
    port: int = Field(default_factory=_default_backend_port, ge=1, le=65535)
    ...
    cors_allow_origins: tuple[str, ...] = Field(
        default_factory=_default_cors_allow_origins,
        description=(
            ...
            "origin). Defaults are derived from the ``factory/config/ports.json`` "
            "registry (``platform.config.ports.cors_allow_origins``) and cover "
            "the SSO/packaged (``backend``), legacy packaged (``legacy_packaged``) "
            "and dev (``frontend_dev``) WebUI hosts. Operators add production origins via "
            "``[server] cors_allow_origins`` in server.toml or "
...
class SecuritySettings(BaseModel):
    ...
    smart_approval_llm_allow_auto_approve: bool = Field(
        default=False,
        description=(
            "When False (default): SmartApproval LLM returning APPROVE is "
            "downgraded to UNDECIDED — the request stays PENDING for human "
            "review. This is the security-conservative default: an LLM "
            "classifier's APPROVE is treated as a hint, not a terminal "
            "authorization. Set True (opt-in) if the operator explicitly "
            "trusts the endpoint's authority; note that ``ssl_verify`` "
            "defaults False, so raise both flags together with care. "
            "REJECT is unaffected — a REJECT auto-rejects regardless of "
            "this flag (REJECT is fail-safe). H-Sec-5 2026-07-28."
        ),
    )
    # L-Sec-3 (2026-07-28): grant TTL knobs. Previously the Approve /
    # Reject use cases hard-coded ``_SCOPE_TTL_SECONDS`` (session=12h,
    # process=8h, permanent=None); an operator that wanted a shorter
    # working-session window had to fork the module.  These fields feed
    # the ``ttl_resolver`` collaborator wired at the DI seam so an
    # operator override takes effect without touching security code.
    # Defaults REMAIN the historical values so no existing deployment
    # observes a behaviour change.
    session_grant_ttl_seconds: int = Field(
        default=12 * 60 * 60,
        ge=1,
        description=(
            "TTL applied to ``scope='session'`` PathGrants written by "
            "ApprovePermissionUseCase / RejectPermissionUseCase.  Default "
            "12h — a working-session window; re-prompts next day.  L-Sec-3."
        ),
    )
    process_grant_ttl_seconds: int = Field(
        default=8 * 60 * 60,
        ge=1,
        description=(
            "TTL applied to ``scope='process'`` PathGrants written by "
            "ApprovePermissionUseCase / RejectPermissionUseCase.  Default "
            "8h — a bounded process lifetime for V2 (no per-process GC "
            "hook).  ``scope='permanent'`` grants remain non-expiring.  "
            "L-Sec-3."
        ),
    )
...
class ChatSettings(BaseModel):
    ...
    max_subagent_recursion_depth: int = Field(
        default=2,
        ge=1,
        le=5,
        description=(
            "Maximum nested sub-agent recursion depth. Depth 1 = a first-level "
            "sub-agent spawned by the main agent; depth 2 = a grand sub-agent "
            "(a sub-agent's sub-agent); and so on, so ``1`` means only the main "
            "agent may spawn and ``2`` (the default) allows exactly one nested "
            "level. Once a spawn would land past this ceiling the ``agent`` tool "
            "call is refused — recursion 封顶 without a hard "
            ...
            "to False (a grand sub-agent could never itself spawn). This "
            "ceiling is therefore mainly a defence-in-depth belt for any UI "
            "that exposes per-level opt-in on the `agent` tool arguments, and "
            "the effective value is re-read every turn from the Settings UI "
            "(``forge_config.chat.max_subagent_recursion_depth``) so a change "
            "lands without a restart. Raise it only when you deliberately need "
            "deeper planner/executor stacks — each extra level multiplies "
            "concurrent model spend."
        ),
    )
    ...
    bash_auto_background_enabled: bool = Field(
        default=False,
        description=(
            "Master switch for the CLOCK arm of bash auto-background: hand a "
            "long-running exec to the background-process manager once "
            "``bash_auto_background_threshold_ms`` has elapsed, even though "
            "nobody asked for it.  Defaults to ``False``: the clock is not a "
            "usable signal for when to background work.  Measured on a real "
            "session, 24 of 49 exec calls (49%) finished inside the 15s "
            "threshold, and the ones that did not were the product's own main "
            "line (model conversion / quantisation / inference) — so the "
            "threshold fired on almost every long task while telling us "
            "nothing about whether backgrounding actually helped.  Handing a "
            "command off when no one is waiting costs a whole extra model "
            "round-trip (the model must be told 'it moved', then read the "
            "result on a later turn) and buys nothing, because a model that "
            "has issued a tool call cannot do anything else until it answers.\n"
            "``False`` removes the threshold arm from the exec race entirely; "
            "Stop and mid-turn injection keep working, and injection is the "
            "signal that actually matters — when the USER sends a new message "
            "mid-command, backgrounding rides the round-trip that message "
            "already forces, so it is free.  Set ``True`` only to restore the "
            "old clock behaviour.  The OFF state is expressed by this "
            "boolean, never by a magic threshold value."
        ),
    )

    bash_auto_background_threshold_ms: int = Field(
        default=15_000,
        ge=1_000,
        le=600_000,
        description=(
            "Foreground wait (ms) before the CLOCK arm hands a long-running "
            "exec to the background-process manager.  Consulted ONLY when "
            "``bash_auto_background_enabled`` is ``True``, which it is not by "
            "default — see that field for why the clock is a poor signal.  "
            "Range 1_000..600_000."
        ),
    )

    # ---- Cloud-image re-encode format/quality (WebUI-configurable) — appended ---

    image_cloud_format: str = Field(
        default="webp",
        description=(
            "Container format for the *cloud copy* of chat images sent to the "
            "remote LLM. One of ``webp`` / ``jpeg`` / ``png`` (default ``webp``). "
            "The local original on disk is never touched — only the in-flight "
            "copy is re-encoded to shrink upstream payload. This is the STATIC "
            "default; the live per-turn value is read from ``forge_config`` "
            "(``chat.image_cloud_format``) and overrides this."
        ),
    )
    image_cloud_quality: int = Field(
        default=50,
        ge=1,
        le=100,
        description=(
            "Encoder quality (1-100, default 50) for the re-encoded *cloud copy* "
            "of chat images. Ignored for lossless ``png``. This is the STATIC "
            "default; the live per-turn value is read from ``forge_config`` "
            "(``chat.image_cloud_quality``) and overrides this."
        ),
    )
    async_job_max_running: int = Field(
        default=16,
        ge=1,
        le=256,
        description=(
            "Maximum number of concurrently RUNNING async background jobs "
            "(sub-agents + backgrounded exec) per process; excess "
            "registrations queue."
        ),
    )

    @field_validator("image_cloud_format")
    @classmethod
    def _valid_image_cloud_format(cls, v: str) -> str:
        allowed = {"webp", "jpeg", "png"}
        lower = v.lower()
        if lower not in allowed:
            raise ValueError(
                f"chat.image_cloud_format must be one of {sorted(allowed)}"
            )
        return lower
...
class ToolsSettings(BaseModel):
    ...
    ssl_verify: bool = Field(
        ...
        description=(
            ...
            "Verify TLS certificates for the ``web_fetch`` tool. "
    ...
    global_proxy: str | None = Field(
        ...
        description=(
            ...
            "(``web_fetch``). ``None`` disables the proxy. V1 parity with the "
...
class WorkspaceSettings(BaseModel):
    ...
    * frontend — surfaced via the config API so the
    ...
    # pydantic v2 warns when a field name starts with ``model_`` (a reserved
    # class-method namespace: ``model_dump``, ``model_validate``, etc.).
    # ``model_root`` here does not shadow any real pydantic method, but the
    # project's pytest ``filterwarnings = ["error", ...]`` promotes the
    # warning to a collection error, breaking every test file that
    # transitively imports this module. Renaming is a wire-level break
    # (documented in server.toml, ``QAI_WORKSPACE__MODEL_ROOT`` env, forge
    # config, three ``ModeFrame*`` views), so we opt out of the check
    # instead — the intent of the ``model_root`` name is domain-specific
    # (a directory root for model artifacts), not pydantic-internal.
...
class UsageSettings(BaseModel):
    ...
    interval_seconds: int = Field(
        ...
        gt=0,
        description=(
            "Delay between two successive reports (12h). Used as "
...
class QaiServiceSettings(BaseModel):
    """QAI Service token-pool broker config (external-developer token pool).

    QAI Service is an OpenAI-compatible LLM broker that meters usage per
    *signed-in user* instead of per static API key. The flow reuses the Okta
    SSO login this app already performs (``AuthSettings``): the verified
    ``id_token`` from the login callback is exchanged at
    ``{base_url}/api/auth/exchange`` for a short-lived (~8 h) per-user service
    JWT, and that JWT — not a user-supplied key — authenticates the
    OpenAI-compatible calls to ``{base_url}/v1/...``. The broker meters the
    signed-in user's token allowance and forwards upstream.

    Consequences of that design, which shape the fields below:

    * There is **no ``api_key``** here. The credential is minted at runtime
      from the login identity (not user-supplied), so there is nothing for an
      operator (or user) to configure. The minted JWT is cached in the
      OS-keyring-backed SecretStore so it survives a restart, but that is an
      internal implementation detail of
      :mod:`interfaces.http.auth.qai_service_token`, not a config field.
    * ``base_url`` is deliberately **not defaulted**. The endpoint is
      deployment data that belongs in the factory config seed
      (``factory/_source/service_config.json`` → ``data/config/``), not in
      this shared-kernel source: ``settings.py`` ships under BOTH editions,
      and a hardcoded intranet host here would trip the external artifact's
      ``check_release.py`` sensitive-word scan (same rationale as
      :class:`UsageSettings.ceflow_url`). Empty ``base_url`` ⇒ the feature is
      inert regardless of ``enabled``.

    Audience + edition plan
    -----------------------
    The token pool exists for **external** developers: an internal user already
    reaches the models through the internal gateway, so metering their usage
    against an external allowance would be double-counting.

    **Current (joint-test) state — deliberately ungated.** BOTH editions run
    the pool so the flow can be exercised on a dev source tree, which always
    self-reports ``edition="internal"`` (a dev tree has no ``build_info.json``;
    see :data:`_DEFAULT_EDITION`). Gating it now would make the feature
    untestable on the very machines developing it.

    **Intended end state — external-only.** Once the flow is stable, gate it
    with ``if not settings.is_internal:`` — note this is the INVERSE of the
    usual internal-only gate (``if settings.is_internal:`` guards
    :class:`UsageSettings`, MB Pro, GoMaster …), and would be this repo's first
    external-only capability. Two consequences worth stating plainly:

    * The cutover is a **runtime gate only**. Do NOT add the pool to
      ``manifest.toml [exclude]`` and do NOT add its endpoint to
      ``check_release.SENSITIVE_KEYWORDS``: those are layers of the
      internal-asset defence, whose job is keeping things OUT of the external
      artifact. Here the external artifact is the one that must ship the
      feature. The endpoint is already absent from every shipped source file
      (it lives only in the factory config seed), so nothing needs scrubbing.
    * After the gate lands, the pool becomes **invisible on a dev tree**. To
      exercise it then, either run a packaged artifact or pin
      ``edition = "external"`` in the local TOML / overrides (both outrank the
      ``build_info.json``-derived default — see :func:`load_settings`).

    The single natural gate point is the apps-layer DI seam that resolves the
    pool's credential (``apps/api/_model_resolver_bridge``): with no JWT the
    provider yields no credential, the pool's models stop resolving, and the
    sidebar gauge hides itself — one branch, and the capability disappears
    coherently front to back.
    """

    enabled: bool = Field(
        default=True,
        description=(
            "Master switch for routing chat through the QAI Service token "
            "pool. Default True (the token pool is a shipped capability, not "
            "an opt-in experiment), but the feature stays inert until "
            "``base_url`` is set by the factory config seed. Set False to "
            "disable the id_token exchange and the pool's models entirely."
        ),
    )
    base_url: str = Field(
        default="",
        description=(
            "QAI Service origin, NO trailing path (the exchange endpoint is "
            "``{base_url}/api/auth/exchange``; the OpenAI-compatible endpoint "
            "is ``{base_url}/v1``). Empty (the safe default in this "
            "shared-kernel source, which ships under both editions) means "
            "'not deployed' → the feature is inert. The real value is "
            "delivered by the factory config seed "
            "(``factory/_source/service_config.json`` → ``data/config/``), "
            "keeping the host literal out of a source file that the external "
            "``check_release.py`` sensitive-word scan inspects."
        ),
    )
    exchange_timeout_seconds: float = Field(
        default=15.0,
        description=(
            "HTTP timeout for the one id_token→service-JWT exchange call made "
            "during the SSO login callback. Kept short: a slow broker must "
            "never stall the login redirect — the exchange is best-effort and "
            "its failure is reported, never fatal to signing in."
        ),
    )
    ssl_verify: bool = Field(
        default=True,
        description=(
            "Verify TLS certificates when talking to QAI Service (the "
            "exchange + quota calls). Defaults True; set False only for a "
            "plain-HTTP / self-signed test deployment. Applies ONLY to the "
            "QAI Service client, never globally."
        ),
    )
...
class ToolOutputSettings(BaseModel):
    ...
    * *Line / byte / length* caps (``read_*`` / ``grep_*_bytes`` /
    ...
    read_count_scan_max_bytes: int = Field(
        default=50 * 1024 * 1024,
        ge=0,
        description=(
            "Maximum bytes of a file the ``read`` tool will SCAN to determine "
            "its exact total line count (default 52428800 = 50MB). Separate "
            "from ``read_max_bytes``, which bounds how much text is RETURNED: "
            "this bounds only the I/O spent counting newlines (an O(1)-memory "
            "raw pass — roughly 2ms for 1MB, 64ms for 50MB). Files larger than "
            "this report a LOWER BOUND ('at least N lines') instead of an exact "
            "total, so a multi-GB file never costs seconds. 0 disables counting."
        ),
    )
...
def _default_auth_enabled() -> bool:
    """Platform-dependent default for :attr:`AuthSettings.enabled`.

    The Okta login gate needs a **local browser** to receive the loopback
    callback (RFC 8252). Windows desktop always has one, so the gate defaults
    on. Linux is the split case: this product runs both as an Ubuntu *desktop*
    app (display attached, browser available — the gate works exactly as it
    does on Windows) and as a *headless* server / container / IoT board, where
    enabling it would 303 every request to ``/auth/login`` with nothing able to
    complete the flow, i.e. lock the deployment out.

    A graphical session is the discriminator: ``DISPLAY`` (X11) or
    ``WAYLAND_DISPLAY`` (Wayland) is set iff there is a desktop session that
    can open a browser. That keeps the original headless protection intact
    while letting Ubuntu desktop behave like Windows, with no per-machine
    configuration.

    Deliberately a heuristic on env vars and not a probe (no ``xdg-open``
    lookup, no X connect): this runs at settings-load time on every boot, must
    not block, and must not depend on a browser being *installed* — an operator
    on a desktop session with no browser still wants the gate on, and gets a
    clear failure from the launcher rather than a silently ungated app.

    Overridable as usual via ``server.toml`` / env / overrides, which all take
...
class AuthSettings(BaseModel):
    ...
    Master switch :attr:`enabled` is **platform-dependent by default**, computed
    by :func:`_default_auth_enabled`: ``True`` on Windows (ARM64 / x64), where
    the Okta login gate is an intended, always-on feature of the desktop
    deployment; on Linux it follows the presence of a graphical session
    (``DISPLAY`` / ``WAYLAND_DISPLAY``) — ``True`` on an Ubuntu desktop that has
    a browser to receive the Okta loopback callback (RFC 8252), ``False`` on a
    headless server / container / IoT board, where the gate would otherwise lock
    every request out. It can still be overridden explicitly via
    ``server.toml`` / env / overrides (which take higher priority than this
    default). When ``False`` the middleware short-circuits, the SPA header shows
    no account button, and the flow is a no-op end-to-end.
    """

    enabled: bool = Field(
        default_factory=_default_auth_enabled,
        description=(
            "Master switch. Platform-dependent default: True on Windows "
            "(ARM64/x64); on Linux, True when a graphical session is present "
            "(DISPLAY / WAYLAND_DISPLAY — e.g. Ubuntu desktop) and False on a "
            "headless server, which has no local browser for the Okta loopback "
            "callback. True = every non-public "
            "path requires a valid session cookie; missing cookie → 303 to "
            "/auth/login for browser requests, 401 JSON envelope for API "
            "requests. False = don't gate any request, don't touch Okta, SPA "
            "renders as-if this feature did not exist. Override via server.toml "
            "/ env / overrides when you need a non-default per-deployment value "
            "(e.g. local pnpm-dev on 5173, or forcing auth on/off on a "
            "specific host)."
        ),
    )
    ...
    client_id: str = Field(default="0oa159m72r5jFnU8o698")
    issuer: str = Field(
    ...
    # ── Device Authorization Grant (RFC 8628) — headless / IoT servers ───────
    device_flow_enabled: bool = Field(
        default=False,
        description=(
            "Enable OAuth 2.0 Device Authorization Grant (RFC 8628) for "
            "headless servers and IoT boards that have no local browser. "
            "When True, ``qai auth device-login`` is the SSO entry point "
            "instead of the loopback OIDC redirect flow."
        ),
    )
    device_poll_max_seconds: int = Field(
        default=600,
        ge=60,
        description=(
            "Hard cap on the device-flow polling loop (seconds). "
            "Matches Okta's default expires_in (~10 min). If the user has "
            "not completed authorization within this window the login attempt "
...
class ComputerSettings(BaseModel):
    """Desktop-control (``computer`` tool) tunables.

    The tool is OFF by default (``enabled=False``); the chat bridge only
    registers it when this flag is set. Screenshots capture at the display's
    NATIVE resolution by default (``max_width`` / ``max_height`` unset) so
    screenshot pixels stay 1:1 with screen pixels and click coordinates are
    exact; setting a cap down-scales to fit (never up-scales) at the cost of
    accuracy. Multi-monitor: ``display`` selects 'all' (default — the whole
    virtual desktop, covering Duplicate and Extend setups), 'primary', or one
    device name; the layout is re-detected on every screenshot.
    """

    enabled: bool = Field(
        default=False,
        description=(
            "Master switch for the desktop-control ``computer`` tool. "
            "Default False; the chat tool bridge registers the tool only "
            "when this is True."
        ),
    )
    backend: str = Field(
        default="auto",
        description=(
            "Desktop backend selection: 'auto' or 'native'. Both forbid a "
            "non-native fallback (there is no simulated backend)."
        ),
    )
    display: str = Field(
        default="all",
        description=(
            "Which monitors to capture. 'all' (default) spans the whole virtual "
            "desktop, so a window on ANY screen is visible and clickable — this "
            "covers both Duplicate and Extend multi-monitor setups. 'primary' "
            "captures only the primary screen (smaller image, fewer tokens). A "
            "device name (e.g. \\\\.\\DISPLAY1) captures that monitor only. The "
            "layout is re-detected on every screenshot, so plugging in or "
            "unplugging a monitor needs no restart."
        ),
    )
    max_width: int | None = Field(
        ...
        le=100000,
        description=(
            "Max screenshot width in pixels. Unset (default) captures at the "
            "display's native width so screenshot pixels stay 1:1 with screen "
            "pixels — this keeps the coordinates the model reads exact. Set a "
            "value only to cap image size (saves tokens, costs click accuracy)."
        ),
    )
    max_height: int | None = Field(
        ...
        le=100000,
        description=(
            "Max screenshot height in pixels. Unset (default) captures at the "
            "display's native height (see max_width)."
        ),
    )
    ...
    def _valid_backend(cls, v: str) -> str:
        ...
        if lower not in allowed:
            raise ValueError(
                f"computer.backend must be one of {sorted(allowed)}"
            )
        return lower
...
class Settings(BaseSettings):
    ...
    # QAI Service token pool (external-developer token pool). Tail-appended
    # per §3.1; inert until the factory seed supplies ``base_url``.
    qai_service: QaiServiceSettings = Field(default_factory=QaiServiceSettings)
    ...
    # Controls ALL outbound HTTPS connections in the process: web_fetch tool,
    ...
    ssl_verify: bool = Field(
        ...
        description=(
            ...
            "Controls: web_fetch tool, LLM stream, LLM title generator, "
    ...
    *,
    ...
    Parameters:
        ...
        repo_root: if given (and ``config_file`` is None), looks for
            ``<repo_root>/factory/config/server.toml``. Also used to resolve the
...
def _resolve_config_file(
    ...
    if repo_root is not None:
        return repo_root / "factory" / "config" / _DEFAULT_TOML_NAME
    return None


def _load_build_info(repo_root: Path | None) -> dict[str, Any] | None:
    """Read ``<repo_root>/build_info.json`` and return its object, or ``None``.

    Single reader shared by :func:`_resolve_build_info_edition` and
    :func:`resolve_app_version` so the marker's robustness rules live in ONE
    place. Robust by design (edition-dual-form-design.md §0 ① / §2.2): a
    missing, unreadable, malformed, or non-object marker NEVER raises —
    resolving the marker must never be able to crash application startup. No
    environment variable participates (hard constraint ①). Read as UTF-8
    (AGENTS.md §8).
    """
...
def resolve_app_version(repo_root: Path | None) -> str:
    """Resolve the version this build self-reports.

    A packaged artifact carries ``build_info.json`` with the ``version`` the
    release pipeline stamped via ``Release.bat`` / ``--release-version``; that
    value is the truth for ``GET /api/system/build-info`` and the FastAPI
    ``info.version``. A dev source tree has no marker and resolves to the fixed
    :data:`_DEV_VERSION_SENTINEL`.

    Deliberately does NOT consult ``importlib.metadata``: the installed
    distribution version is the frozen sentinel ``0.0.0.dev0`` in an editable
    install and simply absent in a plain checkout, so asking it made the
    reported version a function of the developer's install mode — which then
    perturbed the byte-pinned OpenAPI snapshot. Never raises, for the same
    startup-safety reason as the edition resolver.
    """
    data = _load_build_info(repo_root)
    if data is None:
    ...
    if not isinstance(raw, str):
        return _DEV_VERSION_SENTINEL
    value = raw.strip()
    return value or _DEV_VERSION_SENTINEL
...
def _resolve_build_info_edition(repo_root: Path | None) -> str | None:
    ...
    Never raises; see :func:`_load_build_info`.
    """
    data = _load_build_info(repo_root)
    if data is None:
        return None
