...
class SubprocessProcessRunner:
    ...
    async def _run(  # noqa: C901 -- single coroutine; keeps state local
        ...
        async def _await_exit() -> None:
            ...
            await proc.wait()
            # Daemon-fork hang fix: when the direct child exits but a
            # grandchild (e.g. adb daemon) still holds the pipe FDs open,
            # ``_pump`` can never read EOF and never puts its sentinel into
            # the queue. Inject the missing sentinels now so the consumer
            # loop (``while eos_seen < 2``) can unblock and terminate.
            # A pump that reached EOF naturally will have already put its
            # sentinel; an extra sentinel is harmless — the consumer loop
            # only counts up to 2 and then exits, so any extras are drained
            # by the final cleanup below without being processed.
            await queue.put(None)
            await queue.put(None)
