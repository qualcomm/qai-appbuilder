...
Background
----------
``ServerSettings.port`` defaults to the ``factory/config/ports.json`` ``backend``
value (see ``qai.platform.config.settings.ServerSettings.port`` and
``qai.platform.config.ports`` — the value is pinned by the Okta
redirect_uri registered on the authorization server, so any deviation
breaks SSO). ``Start.bat`` also reads that same ``backend`` port from
``factory/config/ports.json`` and passes it as ``--port`` explicitly to the
supervisor so the actual bind matches the SSO contract. However, on Windows the OS
...
to the documented default — the ``factory/config/ports.json`` ``backend`` port)
when the file is absent (server not running) or stale.
...
def read_endpoint(data_root: Path) -> dict[str, Any] | None:
    ...
    documented default — the ``factory/config/ports.json`` ``backend`` port) when
    this returns ``None`` so a stale-or-absent
