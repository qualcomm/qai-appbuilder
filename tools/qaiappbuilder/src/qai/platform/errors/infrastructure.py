...
class ExternalServiceError(InfrastructureError):
    ...
    def __init__(
        ...
        *,
        ...
        # Merge caller-supplied diagnostic fields (e.g. ``log_tail``,
        # ``manual_command``, ``app_id``) so they reach the HTTP response's
        # ``details`` block. ``service``/``status`` win on key collision to
        # keep the envelope's core contract stable.
        if extra_details:
