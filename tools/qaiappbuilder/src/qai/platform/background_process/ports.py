...
class BackgroundProcessManagerPort(Protocol):
    ...
    async def wait(
        ...
        """Block until ``process_id`` reaches a terminal state or timeout.

        This is the missing "await a task I started" primitive: without it
        an agent that backgrounded a long job in order to work in parallel
        has no way to actually WAIT for it once it runs out of other work —
        it can only busy-poll :meth:`get`, or fake a delay by shelling out
        to ``sleep`` / ``timeout``.

        Returns the ``Info`` observed when the wait ends:

        * terminal status (``exited`` / ``failed`` / ``stopped``) — the
          process finished within the budget;
        * non-terminal status (``starting`` / ``running``) — the timeout
          elapsed first; the caller may wait again.

        ``None`` when the id is unknown.  ``timeout_s`` is clamped by the
        implementation to a sane ceiling so a single call can never wedge
        a turn indefinitely.  Never raises on a normal timeout.
        """
        ...
    ...
    async def adopt(
        ...
        *,
        ...
        """Adopt an already-running :class:`asyncio.subprocess.Process` into the manager.

        Used by the M5 auto-background path: when a foreground ``exec`` tool
        has been running long enough that a fresh user message should reach
        the model promptly, exec hands its running child ``Process`` object
        off to the manager and returns a synthetic "backgrounded" tool
        result.  The adopted process keeps running; the manager monitors
        its lifecycle (wait-for-exit; stdout/stderr are NOT recaptured —
        exec was already consuming them and the pipes are typically drained
        by then), emits :class:`BackgroundProcessUpdated` on exit exactly
        like a natively-started process, and cleans up on session stop.

        The manager assigns a fresh ``bgp-`` id independent of the child's
        OS pid.  The child is added to the manager's Job Object (Windows
        orphan-kill defense-in-depth), same as natively-started processes.

        Args:
            proc: The already-spawned :class:`asyncio.subprocess.Process`
                handle from the caller.  MUST be alive (``returncode is None``)
                at call time.
            session_id: The owning session (typically ``TabId.value``).
                Used by :meth:`stop_session` and by the M6 aside bridge to
                route completion events back to the right chat tab.
            command: The originally-issued command line (for display /
                audit; the manager does not re-parse it).
            cwd: The cwd the child was spawned with.
            description: Optional human-readable description; when ``None``
                the manager derives one from ``command``.

        Returns the freshly-minted :class:`Info` snapshot.

        Raises :class:`ManagerError` when the process has already exited or
