...
async def kill_session(
    ...
    * **POSIX** -- send ``SIGTERM`` to the whole process group
    ...
    Known limitation (Chrome/Edge & other self-re-parenting launchers). The
    Windows path tree-kills by the launcher's pid. Chrome's launcher forks the
    real browser/renderer processes and exits almost immediately, and Chrome
    additionally uses ``CREATE_BREAKAWAY_FROM_JOB``; by the time ``taskkill /T``
    walks the tree the launcher is already gone and its children have
    re-parented, so a few idle background ``chrome.exe`` can survive (they hold
    no debug port — that is released with the launcher). The Job Object rail
    only fires on THIS (parent) process exit, not on a single ``stop``. This is
    inherent to Chrome's multi-process model, not a bug in this dispatcher;
    callers that need a guaranteed-clean teardown must match the child's own
    ``--user-data-dir`` and tree-kill (see the browser-automation skill).
    """
