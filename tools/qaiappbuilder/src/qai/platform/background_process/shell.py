...
def _sanitize_win_paths_for_bash(command: str) -> str:
    r"""Convert Windows backslash paths to forward slashes for bash.

    When the model emits a command containing ``C:\Users\foo\bar``, bash's
    ``eval`` interprets ``\U``, ``\f`` etc. as escape sequences and silently
    swallows the backslashes.  This converts drive-letter paths to forward-
    slash form (accepted by both bash and Windows APIs).

    Quoted paths (``"C:\Program Files\..."``) use the closing quote as
    boundary rather than stopping at internal spaces.
    """
    ...
    for m in _WIN_PATH_RE.finditer(command):
        ...
        if quote_char:
            ...
            while i < len(command) and command[i] not in (
                ...
                i += 1
        result.append(command[start:i].replace('\\', '/'))
        last_end = i
...
def args(shell: str, command: str, cwd: str) -> list[str]:
    ...
    if kind == "bash":
        return ["-l", "-c", BASH_BOOTSTRAP, "bgp", cwd, _sanitize_win_paths_for_bash(command)]
    if kind == "zsh":
        return ["-l", "-c", ZSH_BOOTSTRAP, "bgp", cwd, _sanitize_win_paths_for_bash(command)]
