...
class ProcessBackedInferenceService:
    ...
    Parameters:
        ...
        models_root: Directory scanned by :meth:`list_models` for available
            models (each model is a sub-directory containing ``genie_config.json``
            or ``config.json``). Defaults to ``install_dir / "models"`` when empty.
    """
    ...
    async def start(
        ...
        *,
        ...
        The config file name resolves via
        :func:`qai.platform.genie_model_config.resolve_model_config_path`
        (``genie_config.json`` preferred, ``config.json`` fallback), matching
        the probe the GenieAPIService daemon itself performs. Because ``-c``
        is an explicit full path, even the older v2.3.7 daemon (which only
        probes ``config.json`` on its own) loads either name this way.
        ...
        if model_name:
            ...
            config_path = resolve_model_config_path(model_dir) or (model_dir / "config.json")
            args += ["-c", str(config_path.resolve())]
        args.append("-l")
        args += ["-n", "-1"]
    ...
    async def list_models(self, *, models_root: str | None = None) -> list[ModelInfo]:
        """Scan the models root for model dirs containing a single-model config.

        Mirrors V1's ``list_service_models``: walk every model dir carrying
        ``genie_config.json`` or ``config.json`` under the models root (see
        :func:`qai.platform.genie_model_config.resolve_model_config_path`),
        infer each model's runtime format (qnn/gguf/mnn) from its files, and
        drop directories that don't look like a model (``unknown`` format).
        ...
        *models_root* (V1 ``service_launch.models_root_path``) overrides the
        ...
        def _scan() -> list[ModelInfo]:
            ...
            try:
                model_dirs = sorted(
                    {p.parent for p in root.rglob("genie_config.json")}
                    | {p.parent for p in root.rglob("config.json")}
                )
                for model_dir in model_dirs:
                    config_json = resolve_model_config_path(model_dir)
