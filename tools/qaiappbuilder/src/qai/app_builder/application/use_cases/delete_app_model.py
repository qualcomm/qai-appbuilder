...
class DeleteAppModelUseCase:
    ...
    * **After** the DB row + on-disk pack files are gone: the injected
    ...
    # ------------------------------------------------------------------
    # built-in protection (two independent lines of defence)
    # ------------------------------------------------------------------
    _BUILTIN_PROTECTED_CODE = "app_builder.app_model_builtin_protected"

    def _guard_builtin(
        self, model_id: AppModelId, *, user_imported: bool
    ) -> None:
        """Refuse to delete a built-in model. Raises :class:`ForbiddenError` (403).

        Two **independent** checks, because a single one already failed in
        production: an import commit of a Pack sharing a built-in's id
        overwrote ``user_imported`` 0 → 1, which silently disarmed the only
        guard and let the built-in pack + weights be rmtree'd
        irreversibly.

        1. **DB flag** — ``user_imported=False`` marks a built-in row. Cheap,
           always available, but a *derived* marker: any write path that
           mis-sets it disarms this check.
        2. **On-disk anchor** (State-Truth-First §5 铁律 1 — disk is the
           truth) — if the pack physically lives under the built-in anchor
           (``factory/chat_features/app-builder/models/<id>/``) it is
           built-in no matter what the flag claims. Probed through the
           optional :class:`PackOriginProbePort` ``resolve_origin`` member of
           the cleanup port.

        Degradation: check 2 is skipped when no cleanup port is wired (lean
        container), when the port predates ``resolve_origin``, when it is not
        callable, when it raises, or when it answers ``None`` (undecidable —
        e.g. a single-anchor container). In every one of those cases
        protection falls back to check 1 alone, i.e. exactly the pre-change
        behaviour: a missing probe must never turn a legitimate delete into a
        403.
        """
        if not user_imported:
            raise ForbiddenError(
                self._BUILTIN_PROTECTED_CODE,
                f"Built-in model {str(model_id)!r} cannot be deleted. "
                "Only user-imported models can be removed.",
            )
        if self._pack_files is None:
            return
        probe = getattr(self._pack_files, "resolve_origin", None)
        if not callable(probe):
            return
        try:
            origin = probe(model_id.value)
        except Exception as exc:  # noqa: BLE001 — probe must never block
            logger.warning(
                "app_builder.delete.origin_probe_failed: id=%s: %s "
                "(degrading to user_imported-flag protection only)",
                model_id.value,
                exc,
            )
            return
        if origin == "built-in":
            # The row claims user_imported=True but the pack sits under the
            # release-shipped built-in anchor: the flag is wrong (stale row /
            # same-id import / external write). Refuse — deleting here would
            # rmtree release assets that startup seeding cannot restore.
            logger.error(
                "app_builder.delete.builtin_flag_mismatch: id=%s has "
                "user_imported=True but its pack lives under the built-in "
                "anchor; refusing delete (DB flag is stale/corrupted)",
                model_id.value,
            )
            raise ForbiddenError(
                self._BUILTIN_PROTECTED_CODE,
    ...
    async def execute(
        ...
        *,
        ...
        # ── Protection: built-in models are undeletable ────────────────────
        ...
        self._guard_builtin(model_id, user_imported=model.user_imported)
