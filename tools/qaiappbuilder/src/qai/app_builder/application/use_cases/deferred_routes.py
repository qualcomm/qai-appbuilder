...
from qai.platform.model_weights import (
    MIN_WEIGHT_BYTES,
    WEIGHT_SUFFIXES,
    WeightSearchReport,
    build_search_report,
...
# ---------------------------------------------------------------------------
# Precision handling lives in the ``qai.platform.model_weights`` shared
# kernel so this context and ``qai.model_builder`` (Pack export) can never
# disagree. They used to carry separate tables and drifted: this side learned
# ``bf16`` / ``w16a16`` while the export side did not, so such a build showed
# up in the wizard and then failed to export with "no usable .bin".
#
# Discovery is driven by file EXTENSION, never by a precision table — an
# unrecognised or absent suffix resolves to ``fp16`` instead of dropping the
# weight (see the shared kernel's module docstring).
...
class ImportScanBinsUseCase:
    ...
    * **legacy mode** (no ``model_workdir``) — a content-free recursive
    ...
    #: Sourced from the shared kernel so export and import cannot diverge.
    _WEIGHT_SUFFIXES: tuple[str, ...] = WEIGHT_SUFFIXES
    ...
    def _scan_workspace(self, model_workdir: str) -> tuple[BinScanResult, ...]:
        """Enumerate the importable precision variants of one workspace.

        The ordering of authority is the whole design:

        1. ``inference_manifest.json`` — a DECLARATION. When it names a
           ``context_binary``, that file *is* this workspace's weight and its
           ``precision`` field *is* that weight's precision.
        2. Explicit ``_<label>`` filename suffixes — a CLAIM. Admitted as extra
           variants, because a workspace legitimately holds the manifest default
           plus e.g. ``model_w8a8.bin``.
        3. Everything else — a GUESS. Never allowed to invent a precision row.

        Mixing (1) and (3) in one bag is what produced the reported bug: a
        ``melotts_zh`` workspace with one fused weight and five auxiliary
        components showed "FP16 293MB" and "FP32 293MB" — the same file counted
        twice under two invented labels, plus a 28 MB fragment (``flow.bin``)
        holding the fp16 slot.
        """
        ...
        # variant — crucially, it works even when the weight file has NO
        # ``_<label>`` suffix in its name (e.g. ``output/yolov8n.dlc``), which
        # the filename-suffix scan below cannot classify. Best-effort: any error
        # → ``None``, and the filename scan carries the workspace alone.
        manifest_variant = self._read_manifest_variant(workdir, output_dir)

        # ── (2) Every weight, found by EXTENSION (never by precision) ─────────
        # Delegated to the shared kernel so this wizard and Pack export agree.
        # It walks ``output/`` RECURSIVELY — multi-precision context-binary
        # generation is documented to use a separate ``--output_dir`` per
        # precision, which puts real weights at ``output/<prec>/<model>.bin``
        # — and keeps every ``.bin``/``.dlc`` regardless of naming. Nothing is
        # dropped for having an unrecognised suffix, which is what used to lose
        # ``bf16`` / ``w16a16`` / custom ``w<N>a<N>`` builds.
        candidates = discover_weights(output_dir, min_bytes=_SCAN_BIN_MIN_BYTES)

        if manifest_variant is not None:
            # The manifest settled it. Other files in ``output/`` are this
            # model's COMPONENTS (``encoder.bin``, ``flow.bin``, a tokenizer),
            # not alternative precisions — a multi-component export is one
            # precision spread over many files, exactly as the built-in pack
            # manifests record it (a single variant's ``runtime.contextBins``
            # lists the whole component set). Admit an extra file only when it
            # makes its OWN explicit precision claim that DIFFERS from the
            # manifest's: that is the real "manifest default + a separately
            # built ``_w8a8.bin`` variant" workspace, and it is the only shape
            # where a second row is grounded in evidence rather than guessed.
            manifest_precision = manifest_variant.precision or ""
            candidates = tuple(
                c
                for c in candidates
                if c.precision_explicit and c.precision != manifest_precision
            )
        else:
            # No declaration to lean on. Demote a model's parts so five
            # unlabelled components cannot collapse into one bogus ``fp16`` row
            # won by whichever fragment is largest. Demoted files stay visible
            # via ``build_diagnostics``, which walks the tree unfiltered.
            candidates, _auxiliary = partition_component_weights(
        ...
        for cand in candidates:
            mtime_iso = datetime.fromtimestamp(
                cand.mtime, tz=timezone.utc
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
            out.append(
                BinScanResult(
                    path=str(cand.path.as_posix()),
                    size_bytes=cand.size_bytes,
                    # Informational only — the stem's model name may legitimately
                    # differ from the workspace folder name (workspace
                    # ``yolov8`` holding ``yolov8n_fp16.bin``).
                    suspected_model_id=cand.stem_model_name or model_name,
        ...
        # ── (3) Dedup, on TWO keys, in this order ────────────────────────────
        # By real path FIRST: one file must never occupy two precision slots.
        # That is the duplicate the user saw — the manifest declared the 293 MB
        # weight while the filename scan independently re-derived a (wrong)
        # precision for the same bytes, yielding two rows of identical size.
        # Then by precision: App Builder needs exactly ONE weight per precision,
        # since several files can legitimately decode to the same one
        # (``_int8.bin`` beside ``_w8a8.bin``, or one precision present as both
        # ``.bin`` and ``.dlc``). Two rows for one precision produced a picker
        # with duplicate checkboxes toggling in lockstep — it keys selection by
        # the precision string.
        best_by_precision: dict[str, BinScanResult] = {}
        claimed_paths: set[str] = set()
        manifest_keys: set[str] = set()
        if manifest_variant is not None:
            key = manifest_variant.precision or ""
            best_by_precision[key] = manifest_variant
            manifest_keys.add(key)
            claimed_paths.add(self._path_key(manifest_variant.path))
        for r in out:
            if self._path_key(r.path) in claimed_paths:
                continue  # same bytes already listed under another precision
            key = r.precision or ""
            if key in manifest_keys:
                continue  # manifest is the source of truth for this precision
            cur = best_by_precision.get(key)
            if cur is None:
                best_by_precision[key] = r
                claimed_paths.add(self._path_key(r.path))
                continue
            ...
            # Keep ``claimed_paths`` in step with the winner so a later
            # candidate that happens to be the SAME file under a different
            # precision label is still recognised as already-listed (the
            # ousted path is released for the same reason).
            if (r_bin and not cur_bin) or (
                r_bin == cur_bin and (r.mtime or "") > (cur.mtime or "")
            ):
                best_by_precision[key] = r
                claimed_paths.discard(self._path_key(cur.path))
                claimed_paths.add(self._path_key(r.path))
    ...
    def _path_key(path: str) -> str:
        """Identity of a file for dedup purposes.

        Case- and separator-insensitive because the two producers spell the same
        file differently: the manifest path is built from a resolved
        ``workdir / context_binary`` while discovery's comes from walking
        ``output/``. On Windows those can differ in drive-letter case alone, and
        a mismatch here is what let one 293 MB file appear as two variants.
        """
        import os
    ...
    def _read_manifest_variant(
        ...
        The manifest is the authoritative source for the default weight: its
        ``context_binary`` names the file and its ``precision`` field states
        that file's precision. So this works even when the weight's name
        carries NO ``_<label>`` suffix (``output/yolov8n.dlc``) — the case the
        filename-suffix scan cannot classify — and it makes the caller able to
        treat every OTHER ``.bin`` in ``output/`` as a component rather than a
        rival precision.

        Returns a ``BinScanResult`` when the manifest resolves to an existing,
        large-enough weight under ``output/``, else ``None`` (best-effort: any
        parse / IO error is swallowed and the caller falls back to the
        filename scan).
        """
        import json
        from datetime import datetime, timezone
        ...
        # Normalise via the shared kernel. An unrecognised spelling is
        # PRESERVED (``mixed_with_float`` stays ``mixed_with_float``), never
        # folded into ``fp16``: this is a declaration, so relabelling it would
        # make the picker state a precision the weight does not have. Only a
        # blank / structurally unusable value falls back to the default.
        precision = normalise_precision(prec_raw)
        ...
        display = display_label(precision)
    ...
    def build_diagnostics(
        ...
        *,
    ...
    ) -> WeightSearchReport | None:
        """Explain why the workspace scan found no importable weight.

        A blank Import panel is a dead end: it says neither what was looked
        at nor what to do next. This walks ``<model_workdir>/output/``
        unfiltered and returns the evidence — directories seen, weight-like
        files rejected and why, other extensions present, plus the concrete
        next step — so the UI can render an actionable hint and seed the
        "AI search" prompt instead of showing an empty list.

        Callers invoke this ONLY on the empty-scan path (it re-walks the
        tree, so running it on the happy path would be wasted I/O). Returns
        ``None`` when no ``model_workdir`` was supplied — legacy
        ``scan_root`` mode has no workspace to diagnose.

        ``requested_precision`` narrows the diagnosis to the precision the
        caller actually needed ("the requested precision is absent but
        fp16 exists"); the readiness scan leaves it unset because it wants
        every variant.

        Complements (never replaces) :meth:`detect_unnormalized_aihub`: the
        two signals answer different questions ("the model needs Step 6.5
        normalization" vs. "here is everything the search saw"), so both may
        be present on the same response.
        """
        if not model_workdir:
            return None
        try:
            output_dir = Path(model_workdir).resolve() / "output"
        except (OSError, ValueError):
            return None
        # ``build_search_report`` is best-effort and tolerates a missing
        # directory (``root_exists=False``), which is itself a useful signal:
        # "the conversion never produced output/".
        # Guard the walk itself: this helper feeds failure paths, so a fault
        # here must never surface instead of the real error the caller is
        # already reporting.
        try:
        ...
        except Exception:  # noqa: BLE001 — diagnostics never block the caller
            _log.debug(
                "build_diagnostics failed for %s", output_dir, exc_info=True
            )
            return None
    ...
    async def execute(
        ...
        for req in requests:
            try:
                ...
                # past the first frame; the rest is dropped on
