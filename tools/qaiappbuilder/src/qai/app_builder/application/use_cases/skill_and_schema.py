...
class FilesystemSkillFileLoader:
    ...
    -----------------------------------------------
    Since P4 the runtime tracks two Pack anchors (matching
    ``FileSystemWeightsPresence``'s dual-anchor probe semantics —
    State-Truth-First invariant 1):
    ...
    * **user-imported** — ``user_pack_root``
    ...
    (disk is the source of truth). ``load()`` probes built-in first, then
    user; the first anchor that holds a readable file wins.
    ``user_pack_root`` defaults to ``None`` so existing test fixtures /
    lean containers (built-in root only) keep working.
    """
...
class GeneratePackCatalogUseCase:
    """Build the LLM-facing "Local AI models catalog" prompt block.
    ...
    * historical 👍/👎 + quality score + run count —
        ...
        *,
        ...
        """Args:
            origin_by_id: Optional mapping ``{model_id: "built-in" | "user"}``
                ...
                present, ``_render_model`` emits a **Location** block
                ...
                This is the **primary universal channel** for path-awareness — it
    ...
    async def execute(self) -> str:
        ...
        lines: list[str] = [
            "## Local AI models available via `appbuilder_run`",
            "",
            "You can invoke the following installed local models for inference "
            "via the `appbuilder_run` tool.",
            "Each call requires `modelId` and `inputs`; `params` is optional.",
            "",
        ]
        ...
        if truncated:
            # P4 dual-root: built-in packs live under
            # ``factory/chat_features/app-builder/models/``; user-imported
            # packs live under ``data/app_builder/user_models/``. The
            # primary API is the App Builder catalog interface (stable
            # contract, paths may evolve). If direct disk scanning is
            # required, BOTH roots must be covered or user-imported packs
            # will be missed — so path info is only a parenthetical
            # fallback here, never the main clause.
            #
            # Constants come from ``pack_layout_constants`` (single source
            # of truth; see that module's "mirror points" docstring).
            lines.append(
                f"> Note: {total} models available; only the first "
                f"{self._MAX_PACKS} are shown here. Fetch the full list "
                "via the App Builder catalog API (if disk scanning is "
                f"required, packs live under two roots: `{BUILTIN_PACK_REL}/` "
                f"(built-in) + `{USER_PACK_REL}/` (user-imported); both "
                "must be covered)."
            )
            lines.append("")
    ...
    def _render_model(
        ...
        if params_info:
            out.append(f"  - Parameters: {'; '.join(params_info)}")

        perf = self._render_perf(manifest)
        if perf:
            out.append(f"  - Performance: {perf}")
    ...
    def _render_rating(summary: Any) -> str:
        ...
        if summary.rating_count > 0:
            ...
            return (
                f"  - History rating: 👍 {summary.thumbs_up} / 👎 "
                f"{summary.thumbs_down} (quality {q:.2f} over "
                f"{summary.rating_count} feedbacks)"
            )
        if summary.run_count > 0:
            return f"  - Ran successfully {summary.run_count} times (no user ratings yet)"
        return ""
    ...
    def _render_variants(manifest: Any) -> str:
        ...
        return f"  - Available precisions: {', '.join(v_strs)}"
    ...
    def _render_location(self, mid: str) -> list[str]:
        ...
        Renders (only when ``self._origin_by_id`` has an entry for ``mid``)::
            ...
            - Location: built-in pack (V1 built-in layout)
              - Pack dir: `${APP_ROOT}/factory/chat_features/app-builder/models/<id>/`
              - Weights dir: `${APP_ROOT}/models/<id>/`
              - app.yaml: `builtin: true`

        or for user-imported (P4) packs::

            - Location: user-imported pack (P4 user layout)
              - Pack dir: `${APP_ROOT}/data/app_builder/user_models/<id>/`
              - Weights dir: `${APP_ROOT}/data/app_builder/user_model_weights/models/<id>/`
        ...
        if origin == "built-in":
            return [
                "  - Location: built-in pack (V1 built-in layout)",
                f"    - Pack dir: `${{APP_ROOT}}/{BUILTIN_PACK_REL}/{mid}/`",
                f"    - Weights dir: `${{APP_ROOT}}/{BUILTIN_WEIGHTS_REL}/{mid}/`",
                "    - app.yaml: `builtin: true` + the two `${APP_ROOT}/...` paths above",
            ]
        if origin == "user":
            return [
                "  - Location: user-imported pack (P4 user layout)",
                f"    - Pack dir: `${{APP_ROOT}}/{USER_PACK_REL}/{mid}/`",
                f"    - Weights dir: `${{APP_ROOT}}/{USER_WEIGHTS_REL}/{USER_WEIGHTS_MODELS_SUBDIR}/{mid}/`",
                "    - app.yaml: `builtin: false` + the two `${APP_ROOT}/...` paths above",
                f"    - Runtime: `${{APP_BUILDER_USER_MODEL_ROOT}}/{USER_WEIGHTS_MODELS_SUBDIR}/<id>/`"
                f" (note the extra `{USER_WEIGHTS_MODELS_SUBDIR}/` layer) + `${{APP_BUILDER_USER_PACK_ROOT}}/<id>/`",
            ]
    ...
    def _usage_rules() -> list[str]:
        return [
            "### Usage rules",
            "",
            "1. **Input paths**: must be repo-relative paths (e.g. "
            "`data/uploads/images/xxx.png`, `data/outputs/r-xxx.png`) "
            "or an absolute path explicitly specified by the user.",
            "2. **External paths**: if the user gives a path outside the "
            "repo (e.g. `C:\\test\\images`), first verify it exists via "
            "the `glob` or `read` tool, then use that path directly if "
            "the file is indeed present.",
            "3. **Choosing among similar models**:",
            "   - `Ready` status takes priority over `NotInstalled`",
            "   - If historical inference records and quality scores "
            "exist, pick the best performer",
            "   - If no historical data is available, decide based on "
            "the model description and the user's request, and explain "
            "your reasoning",
            "4. **Batching**: only one inference can run at a time (NPU "
            "is serial); invoke `appbuilder_run` multiple times as needed.",
            "5. **Error handling**: on inference failure, analyse the "
            "error and suggest the user adjust parameters or switch models.",
            "6. **Referring to results**: inference output paths (e.g. "
            "`data/outputs/r-xxx.png`) can be used directly as inputs to "
            "the next model.",
            "",
        ]
