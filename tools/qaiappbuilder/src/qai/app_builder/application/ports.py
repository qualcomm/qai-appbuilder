...
@runtime_checkable
class PackOriginProbePort(Protocol):
    """Optional extension of :class:`PackFileCleanupPort`: which anchor holds a pack.

    Second line of defence for the "built-in model must never be deletable"
    invariant (State-Truth-First §5 铁律 1 — **disk is the truth**). The
    first line of defence is the DB flag
    ``app_builder_model_definition.user_imported``; it is a *derived* marker
    and any write path that gets it wrong (a legacy import commit that
    overwrote ``0`` with ``1``, a rollback restoring a stale snapshot, a
    hand-edited row) silently disarms the delete guard. Probing the anchor
    that *physically* holds ``pack_root/<id>`` cannot be spoofed by a bad
    flag, so the two checks fail independently.

    Returns:
        * ``"built-in"`` — the pack dir lives under the release-shipped
          built-in anchor (``factory/chat_features/app-builder/models/<id>/``);
          the model MUST NOT be deleted.
        * ``"user"`` — the pack dir lives under the user-import anchor
          (``data/app_builder/user_models/<id>/``); deletable.
        * ``None`` — undecidable: no anchor holds the pack, no anchor is
          configured, or the container wires a single anchor that doubles as
          both built-in and user-import destination (legacy / lean test
          container). Callers MUST treat ``None`` as "no opinion" and fall
          back to the ``user_imported`` flag alone — never as a denial and
          never as an approval.

    Deliberately a **separate** Protocol rather than a new member of
    :class:`PackFileCleanupPort`: implementations predating this invariant
    (and lean fakes) stay valid ``PackFileCleanupPort``s, and the consuming
    use case probes with ``getattr`` so a cleanup port without this method
    degrades to flag-only protection instead of raising.
    """

    def resolve_origin(self, model_id: str) -> str | None:
        """Return ``"built-in"`` / ``"user"`` / ``None`` for ``model_id``."""
        ...
