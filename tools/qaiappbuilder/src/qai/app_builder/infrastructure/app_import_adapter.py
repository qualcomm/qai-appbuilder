...
class FileSystemAppImportAdapter:
        ...
        *,
        ...
        try:
            for idx, item in enumerate(plan.items):
                ...
                # ── Built-in protection, line 1: never downgrade a built-in
                # row to "user-imported". Importing a Pack that shares a
                # built-in model's id is a legitimate *content update*, but
                # the identity must survive it: writing ``user_imported=True``
                # here used to flip the built-in row's ``0`` → ``1`` (the
                # repository upsert carries
                # ``user_imported=excluded.user_imported``), which disarmed
                # ``DeleteAppModelUseCase``'s guard and let the built-in pack +
                # weights be deleted irreversibly. Read the existing row's
                # flag and hand it to ``_materialise_from_source`` so
                # "once built-in, always built-in" holds.
                new_model = _materialise_from_source(
                    item,
                    version=resolved_version,
                    user_imported=await self._existing_user_imported(
                        item.model_id
                    ),
                )
    ...
    async def _existing_user_imported(self, model_id: AppModelId) -> bool:
        """The ``user_imported`` flag to persist for ``model_id``.

        ``False`` (built-in) **only** when a row already exists and is
        already marked built-in — "once built-in, always built-in". A fresh
        id, or an existing user-imported row, yields ``True``.

        Why the lookup lives here and not in
        :func:`_materialise_from_source`: that function is a pure
        item→definition mapper with no repository handle (and must stay that
        way — no implicit DB dependency inside the mapper). ``commit`` is the
        layer that already owns ``self._app_models``, so the resolved flag is
        passed down as a plain argument.

        A repository failure other than "not found" is deliberately NOT
        swallowed: guessing ``True`` there would re-open exactly the
        downgrade hole this closes.
        """
        try:
            existing = await self._app_models.get(model_id)
        except AppModelNotFoundError:
            return True
        return existing.user_imported
        ...
        for item in applied_items:
            try:
                if item.action == ImportAction.ADD:
                    ...
                    if snap is not None:
                        await self._restore_snapshot(snap)
            except Exception as exc:  # noqa: BLE001
                logger.warning(
    ...
    async def _restore_snapshot(self, snap: dict[str, object]) -> None:
        """Re-save a pre-import snapshot without downgrading a built-in row.

        Built-in protection, line 1 (rollback channel). The snapshot is taken
        at commit time and carries whatever ``user_imported`` the row held
        then; restoring it verbatim would reinstate a stale ``True`` on a row
        that is (now) built-in, re-opening the delete hole through the
        rollback door instead of the commit door. We therefore clamp the
        restored flag with the CURRENT row's value: once built-in, always
        built-in. Every other snapshot field is restored verbatim — rollback
        must still undo the content update.
        """
        model = _model_from_snapshot(snap)
        if model.user_imported and not await self._existing_user_imported(
            model.id
        ):
            model = replace(model, user_imported=False)
        await _save(self._app_models, model)
    ...
    async def rollback(self, commit_id: CommitId) -> None:
        ...
        for raw in items:
            ...
            if action == ImportAction.ADD.value:
                ...
                if snap is not None:
                    # Same built-in clamp as the rollback-on-failure path —
                    # a stale snapshot must not re-mark a built-in row as
                    # user-imported. See ``_restore_snapshot``.
                    await self._restore_snapshot(snap)
...
def _materialise_from_source(
    ...
    *,
    ...
    ``user_imported`` is resolved by the caller from the EXISTING DB row (see
    :meth:`FileSystemAppImportAdapter._existing_user_imported`) so importing a
    Pack over a built-in id updates its content without downgrading its
    built-in identity — which would disarm the delete guard. It defaults to
    ``True`` because a plain import with no pre-existing row IS a user
    import; this mapper stays pure (no repository access).
    """
