...
class FileSystemAppProjectRepository:
    """Read app projects from one or more filesystem roots.

    Each root is expected to contain ``<app_id>/app.yaml`` directories.
    Multiple roots are scanned in order; the first root is considered
    the *writable user root* (typically ``data/app_builder/``), and
    subsequent roots are read-only (e.g. ``samples/app_builder/``).

    When multiple roots supply the same ``app_id``, the earlier root
    wins (user projects shadow built-in samples).  Deletions are only
    permitted against the first (writable) root.
    """
    ...
    def __init__(
        ...
        *,
        ...
        apps_roots: Sequence[Path] | None = None,
    ) -> None:
        if apps_root is not None and apps_roots is not None:
            raise TypeError(
                "pass either apps_root or apps_roots, not both"
            )
        if apps_root is not None:
            if not isinstance(apps_root, Path):
                raise TypeError("apps_root must be a Path")
            self._apps_roots: tuple[Path, ...] = (apps_root,)
        elif apps_roots is not None:
            if not apps_roots:
                raise TypeError("apps_roots must not be empty")
            self._apps_roots = tuple(apps_roots)
        else:
            raise TypeError(
                "either apps_root or apps_roots is required"
            )
        # Backward-compat shorthand used by delete_project (writable root).
        self._apps_root: Path = self._apps_roots[0]
    ...
    async def list_projects(self) -> list[AppProjectDefinition]:
        ...
        Scans every root in order.  When multiple roots contain the same
        ``app_id``, the earlier root wins (user projects shadow samples).
        ...
        for root_idx, root in enumerate(self._apps_roots):
            if not root.is_dir():
                continue
            try:
            ...
            for entry in entries:
                try:
                    ...
                    continue
                definition = self._parse_app_yaml(entry, builtin=root_idx > 0)
                if definition is None:
                    continue
                if definition.id.value in seen_ids:
                    continue
                seen_ids.add(definition.id.value)
    ...
    async def get_project(self, app_id: str) -> AppProjectDefinition:
        """Return one app project by id, searching roots in order.

        Raises :class:`AppProjectNotFoundError` when the id is invalid,
        escapes every root, or no root contains the directory; raises
        :class:`AppProjectInvalidError` when a directory exists but its
        ...
        # 2) Search each root in priority order.
        saw_dir_without_yaml = False
        for root_idx, root in enumerate(self._apps_roots):
            app_dir = root / validated.value
            try:
            ...
            # app.yaml must exist + parse + validate.  When this root has
            # the directory but no valid yaml (e.g. it is a runtime-data
            # directory created by paths.py for uploads/outputs), fall
            # through to the next root — another root may carry the real
            # app project (e.g. samples/app_builder/<id>/).
            definition = self._parse_app_yaml(app_dir, builtin=root_idx > 0)
            if definition is None:
                saw_dir_without_yaml = True
                continue
            return definition

        # Distinguish "dir found but yaml invalid" from "dir never found".
        if saw_dir_without_yaml:
    ...
    async def delete_project(self, app_id: str) -> None:
        """Delete the app project directory from the *writable* root only.

        Only the first root (``self._apps_roots[0]``) is considered writable.
        Projects that exist only in a secondary read-only root (e.g.
        ``samples/app_builder/``) cannot be deleted and raise
        :class:`AppProjectNotFoundError`.  Path-traversal safe: the id must
        be well-formed AND the resolved dir must be contained STRICTLY under
        the writable root before anything is removed.
        """
        ...
        # 3) Directory must exist in the writable root.
        if not app_dir.is_dir():
            # Check if the app exists in a read-only (builtin) root — if so,
            # surface a clear "cannot delete builtin app" message.
            for ro_root in self._apps_roots[1:]:
                if (ro_root / validated.value).is_dir():
                    raise AppProjectNotFoundError(
                        message=(
                            f"cannot delete builtin app {app_id!r}: "
                            f"it exists only in a read-only root"
                        ),
                        details={"app_id": app_id, "builtin": True},
                    )
            raise AppProjectNotFoundError(
                message=f"app project {app_id!r} not found",
                details={"app_id": app_id},
    ...
    # ------------------------------------------------------------------
    def _parse_app_yaml(self, app_dir: Path, *, builtin: bool = False) -> AppProjectDefinition | None:  # noqa: PLR0911 - lenient parse gate; each early return maps a distinct YAML failure to None
