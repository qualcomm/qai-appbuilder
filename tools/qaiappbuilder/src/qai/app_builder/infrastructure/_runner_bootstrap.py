...
def _breadcrumb(stage: str, extra: str = "") -> None:
    ...
    try:
        ...
        line = f"{datetime.now().astimezone().isoformat(timespec='microseconds')} {stage}"
        if extra:
            line += f" | {extra}"
        line += "\n"
...
def _install_logging() -> None:
    ...
    The formatter matches the parent process's console pipeline: a local
    timezone ISO 8601 timestamp (with UTC offset, e.g. ``+08:00``) followed
    by ``[LEVEL] [name] message``. Kept dependency-free (stdlib only) so
    bootstrap stays robust even before the platform package is importable.
    """
    try:
        import logging
        from datetime import datetime

        class _LocalIsoFormatter(logging.Formatter):
            """Render ``record.created`` as a local-tz ISO 8601 timestamp."""
        ...
        if not root.handlers:
            ...
            handler.setFormatter(
                _LocalIsoFormatter("%(asctime)s [%(levelname)-9s] [%(name)s] %(message)s")
            )
...
def _persistent_main() -> int:
    ...
    def _load_runner_module_into(model_id: str, runner_path: str) -> Optional[_LoadedModel]:
        ...
        # Import the runner WITHOUT writing a ``__pycache__/*.pyc`` beside it.
        #
        # The cache would be write-only dead weight: a persistent worker imports
        # each Pack runner exactly once in its lifetime, so nothing ever reads
        # the .pyc back. What it *does* add is a filesystem write into the Pack
        # directory on the critical load path — and that write has no timeout.
        # Any filter driver sitting on the Pack dir (endpoint-security real-time
        # scanning, a sync client, a network/virtualised volume) can stall the
        # CREATE and the whole worker freezes mid-``op:load``: the host only
        # ever sees ``loading`` and eventually fails the load, the model never
        # goes resident, and the UI stays stuck. Observed reproducibly on a
        # Pack staged under a scanned directory — 4 of 5 loads wedged, and even
        # the ``faulthandler`` watchdog could not report because *its* dump
        # write into the same directory blocked too.
        #
        # Scoped to this import only (restored in ``finally``) so the flag never
        # leaks into ``load_model`` / ``run_inference``, where a runner pulling
        # in a heavy dependency (numpy, PIL, ...) still benefits from caching.
