...
class FileSystemPackFileCleanup:
    ...
    * **user-imported** — ``user_pack_root`` + ``user_weights_root``.
    ...
    Also implements
    :class:`qai.app_builder.application.ports.PackOriginProbePort`
    (:meth:`resolve_origin`): the same anchor probe, exposed read-only so
    :class:`DeleteAppModelUseCase` can refuse to delete a pack that
    physically sits under the built-in anchor even if its DB
    ``user_imported`` flag says otherwise.
    """
    ...
    # ------------------------------------------------------------------
    def _same_anchor(self) -> bool:
        """Whether the two pack anchors are indistinguishable.

        True when the user anchor is unwired (legacy / lean container, where
        ``pack_root`` doubles as the user-import destination) or when both
        constants resolve to the same directory. In that state "which anchor
        holds the pack" carries **no** origin information.
        """
        if self._user_pack_root is None or self._pack_root is None:
            return True
        try:
            return self._user_pack_root.resolve() == self._pack_root.resolve()
        except OSError:  # pragma: no cover - unresolvable path
            return self._user_pack_root == self._pack_root

    def resolve_origin(self, model_id: str) -> str | None:
        """Which anchor physically holds ``model_id``'s pack dir.

        Implements :class:`qai.app_builder.application.ports.PackOriginProbePort`
        — the *disk-side* half of the "built-in models are undeletable"
        invariant. See that Protocol for the return contract; in short:
        ``"built-in"`` / ``"user"`` / ``None`` (= no opinion, caller falls
        back to the ``user_imported`` DB flag alone).

        ``None`` is returned whenever the answer would be a guess: no anchor
        holds the pack, or the two anchors are indistinguishable
        (:meth:`_same_anchor`). Never raises — a probe failure must not be
        able to block a legitimate delete.
        """
        same = self._same_anchor()
        try:
            if (
                not same
                and self._user_pack_root is not None
                and (self._user_pack_root / model_id).is_dir()
            ):
                return "user"
            if (
                not same
                and self._pack_root is not None
                and (self._pack_root / model_id).is_dir()
            ):
                return "built-in"
        except OSError as exc:  # pragma: no cover - stat failure
            logger.warning(
                "app_delete.origin_probe_failed: id=%s: %s (degrading to "
                "flag-only protection)",
                model_id,
                exc,
            )
        return None
    ...
    def _locate_pack(
        ...
        picks whichever anchor really contains the pack dir), then built-in.
        Returns ``(None, None)`` when neither anchor is configured or holds
        the pack — caller degrades to a no-op.

        Why the built-in fallback was removed (verified, not assumed)
        ------------------------------------------------------------
        This used to return ``(self._pack_root, self._repo_root)`` when
        *neither* anchor held the pack, justified as "weights may linger
        while the pack dir was already rmtree'd out-of-band".

        Measured behaviour of that branch: it was a **no-op, and it never
        misfired**. Every weights path the deleters touch is read from
        ``manifest.json`` *inside* the pack dir, so with no pack dir
        ``_read_install_paths`` returns ``[]`` and nothing is unlinked — a
        lingering ``repo_root/models/<id>/*.bin`` was left in place, i.e. the
        fallback did not even achieve its stated purpose. It could not reach
        a *different* id's assets either, since every path it builds is
        ``<root>/<model_id>``-scoped.

        It is nevertheless gone: the branch was dead weight whose comment
        promised a guarantee it did not provide, and it aimed a
        soon-to-be-``rmtree``d path at the release-shipped built-in tree on
        the strength of a *miss*. Returning ``(None, None)`` states the same
        outcome honestly ("no files here to remove") and keeps any future
        weights-discovery change from turning that mis-aim into real damage.

        Note what this method deliberately does NOT do: when only the
        built-in anchor holds ``<id>``, it still resolves there — that is
        the correct target for a genuine built-in pack, and on disk it is
        indistinguishable from "a user model whose dir vanished". Refusing
        the delete is not this method's job; :meth:`resolve_origin` reports
        ``"built-in"`` and ``DeleteAppModelUseCase`` raises 403 before any
        deleter is ever called.
        """
        if (
