...
class StickyWorkerHost:
        ...
        async with self._op_lock:
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    return False
                if not line:
                    # stdout EOF: the worker closed its pipe. It may be a clean
                    # console-close teardown (``_quiet_shutdown`` -> exit 0, the
                    # common case at app exit) or a real crash. The exit code is
                    # the discriminator, but at the instant EOF is observed the
                    # process is often not yet reaped (returncode=None), so
                    # _mark_dead would misclassify a clean exit as a crash and
                    # auto-respawn. Briefly await the real exit code first
                    # (bounded so a wedged process can't hang the health loop);
                    # _mark_dead then sees rc==0 and takes the quiet path.
    ...
    def _mark_dead(self, reason: str = "unspecified") -> None:
        ...
        """
        # Capture the pre-transition state BEFORE overwriting to "dead" so we
        # can tell whether this "death" is an intentional shutdown (parent
        # process is exiting; a stdout EOF here is normal, not a crash) vs a
        # real crash. Two collaborators race on the way down: shutdown() sets
        # _state="shutting_down" then cancels tasks; meanwhile a ping()/health
        # loop / stdout watcher may read EOF on the closing pipe and call
        # _mark_dead first. Both paths funnel here; consult the state to keep
        # the shutdown path quiet.
        was_shutting_down = self._state == "shutting_down"
        ...
        # Explicitly close the subprocess transport (and its stdin/stdout/stderr
        # pipe transports) NOW, while the event loop is still running. asyncio's
        # Proactor (Windows) subprocess transports otherwise only close lazily
        # in ``__del__`` during GC — which, if the loop is already torn down
        # (e.g. on a WebUI-triggered restart), raises
        # ``ValueError: I/O operation on closed pipe`` from
        # ``_ProactorBasePipeTransport.__del__``. Closing here makes shutdown
        # clean (no GC-time noise). Best-effort + never raises on the death
        # path; uses the transport handle asyncio exposes on the Process.
        self._close_proc_transport()
        # Expected-exit path: an INTENTIONAL teardown, not a crash. Two
        # independent signals qualify, because a console-close / Ctrl+C races
        # the graceful ``shutdown()`` on the way down:
        #   1. ``was_shutting_down`` — ``host.shutdown()`` already ran and set
        #      the state (the graceful op:shutdown path).
        #   2. ``rc == 0`` — the worker's own signal handler (``_quiet_shutdown``
        #      -> ``os._exit(0)``) exited CLEANLY. On Windows the console CTRL
        #      event reaches the worker (same process group) and it exits 0
        #      BEFORE ``host.shutdown()`` runs, so the health-loop ping reads a
        #      stdout EOF and lands here first with the state still not
        #      "shutting_down". A clean exit code is never a crash, so treat it
        #      as an expected teardown: no stderr_tail dump, no death event, no
        #      auto-respawn (the app is exiting; a new worker would be reaped
        #      milliseconds later). A genuine crash exits with a non-zero
        #      NTSTATUS (e.g. 0xC0000005) and still takes the DEAD path below.
        if was_shutting_down or rc == 0:
            logger.info(
                "StickyWorkerHost worker exited during shutdown pid=%s %s reason=%s",
                pid, _describe_returncode(rc), reason,
            )
            return
    ...
    def _close_proc_transport(self) -> None:
        """Close the current subprocess transport + its pipe transports.

        asyncio's :class:`asyncio.subprocess.Process` keeps the underlying
        transport on ``._transport``; closing it closes the stdin/stdout/stderr
        pipe transports it owns. On Windows (Proactor) these otherwise linger
        until GC, whose ``__del__`` raises ``I/O operation on closed pipe`` when
        the loop is already gone (WebUI-restart shutdown). Best-effort: private
        API + already-closed pipes are swallowed; never raises on the death path.
        """
        proc = self._proc
        if proc is None:
            return
        ...
        if transport is not None:
            with contextlib.suppress(Exception):
                transport.close()
        # Also close the StreamWriter (stdin) explicitly — its transport is a
        # separate write-pipe transport that the subprocess transport may not
        # own on every platform.
    ...
    def _can_respawn(self, reason: str) -> bool:
        ...
        Respawn is suppressed for graceful shutdowns and when the rolling
        attempt budget is exhausted. ``_mark_dead`` already short-circuits
        the shutdown path before we get here, but the string-match on
        ``reason`` remains as a second layer in case a future caller
        bypasses ``_mark_dead`` and reaches respawn logic directly.
        """
        # Never respawn after an intentional shutdown (belt-and-braces —
        # the primary guard is the ``was_shutting_down`` short-circuit in
        # ``_mark_dead``).
