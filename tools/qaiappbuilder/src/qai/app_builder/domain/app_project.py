...
class AppProjectModelNotInstalledError(DomainError):
    """Raised when running an app whose bundled model files are absent on disk.

    Pre-flight guard raised by :class:`AppProjectProcessManager` BEFORE any
    port reservation / subprocess spawn: the app's ``app.yaml`` names one or
    more bundled models, and at least one of them has no weights on disk
    (neither the built-in ``model_root`` layout nor the user weights layout
    holds the required files). Spawning uvicorn anyway would let the app's
    own model-load path crash inside the child process with an opaque error;
    surfacing this at the boundary lets the UI show a clear
    "install this model first" message with the exact missing paths.
    """

    default_code = "app_builder.model_not_installed"
