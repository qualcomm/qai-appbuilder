...
class WeightsProbe(Protocol):
    ...
    def pack_weights_dir_is_present_but_empty(self, pack_id: str) -> bool:
        ...
        Returns ``True`` **only** when the dir is present *and* empty. An
        *absent* dir or a *non-empty* dir both yield ``False`` (matching
        V1, where an absent dir leaves the prior Ready/NotInstalled
        decision intact). The adapter owns the ``pack_root`` join.

        Consulted by :func:`detect_status` **only for a manifest with no
        ``assets.installPath``**. When an installPath IS set and resolves,
        that is the answer — under the P4 split-root layout a pack dir's
        ``weights/`` is deliberately empty (weights live under the weights
        root), so letting this predicate overrule a confirmed installPath
        marked every freshly imported user Pack ``NotInstalled``.
        """
        ...
    ...
    try:
        ...
        if install_path:
            if not probe.install_path_present(install_path):
                return _NOT_INSTALLED
            # ``installPath`` resolved to a real file — that IS the truth for
            # this Pack, so return before the legacy fallback below.
            #
            # The fallback used to run unconditionally and could OVERRULE a
            # confirmed installPath. That is wrong under the P4 split-root
            # layout: ``_install_pack`` deliberately creates an EMPTY
            # ``<pack_dir>/weights/`` (weights live under the weights root,
            # anchored by installPath — see ``_PACK_COPY_DIRS``), so every
            # freshly imported user Pack matched "present but empty" and was
            # reported ``NotInstalled`` even though its weights were staged
            # correctly. The V1 semantics this mirrors had a single root where
            # ``<pack_dir>/weights/`` and the installPath anchor were the SAME
            # directory, so the two could never disagree; once they became
            # separate anchors the fallback stopped being a refinement of the
            # installPath answer and became a contradiction of it.
            return _READY
        # Fallback for a manifest with NO ``installPath`` (nothing to anchor):
        # a *present but empty* ``<pack_dir>/weights/`` dir means the pack was
        # laid down without its weights (V1 ``is_dir() and not any(iterdir())``).
        # An absent or non-empty dir leaves the decision at Ready.
