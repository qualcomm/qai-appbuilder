...
class ConversationRepositoryPort(Protocol):
    ...
    async def save_messages(
        ...
        ``conn`` is an OPTIONAL caller-owned connection the write runs its
        transaction on, for a serialised writer that holds one across many
        transactions and does not want to pay the setup per write (see
        ``Database.lease_persistent_connection``).  Every caller that does
        not manage a lease leaves it ``None`` and the adapter leases its own,
        which is the only shape an in-memory implementation has to honour.
        """
        ...

    async def save_meta(self, conversation: Conversation) -> None:
        """Persist messages + ``meta_json`` (+ status), but PRESERVE ``title``.

        Use from discussion / implementation state write-backs that mutate
        ``meta["discussion"]``: they must persist their meta yet never roll
        back a concurrently-generated auto-title from the turn's stale
        snapshot (fixes "title generated on first frame, lost on restart"
        because the END-frame discussion state-back ``save`` overwrote it).
        On a brand-new row the aggregate's title/meta are still seeded.
        """
        ...
    ...
    async def append_message_atomic(
        ...
        *,
        ...
        """Append ONE message row to a conversation atomically.

        Unlike :meth:`save_messages` — which rewrites every message row
        from an in-memory ``Conversation`` snapshot (DELETE + INSERT
        all) — this operation ONLY inserts the given ``message`` row,
        computing its ``position`` inside the same transaction as
        ``SELECT MAX(position) + 1 FROM chat_message WHERE
        conversation_id = ?``.  Concurrency-safe by design.

        Rationale (§concurrent-writer refactor 2026-08-07): the
        background-job dispatcher appends a SYSTEM_NOTICE row while
        the streaming pipeline may also be writing this conversation
        (a headless follow-up turn spawned by the coordinator, or the
        primary user turn nearing completion).  Both writers loaded
        their own ``Conversation`` snapshot; if both call
        :meth:`save_messages`, the second writer's DELETE-all clobbers
        the first writer's newly-appended row — the dispatcher's
        SYSTEM_NOTICE silently disappears (data loss), the coordinator's
        pending queue is never observed on the next follow-up's wire
        rebuild, and the user sees a redundant "I already summarised
        that above" reply.  This atomic append does NOT touch any
        other row, so parallel snapshot-based saves can no longer
        cross-clobber each other.

        Implementations MUST guarantee:

        * The new row's ``position`` is the current DB tail + 1
          (computed inside the transaction).
        * The conversation header's ``updated_at`` is bumped so
          ``find`` returns a consistent snapshot.
        * ``ConversationNotFoundError`` on a missing conversation.

        ``conn`` is the same OPTIONAL caller-owned connection described on
        :meth:`save_messages`.
        """
        ...
...
class SubAgentSessionRepositoryPort(Protocol):
    ...
    async def save(
        self, session: SubAgentSession, *, conn: Any | None = None
    ) -> None:
        """Insert or upsert ``session`` (by id).

        ``conn`` is the OPTIONAL caller-owned connection described on
        :meth:`ConversationRepositoryPort.save_messages`.
        """
        ...
...
class CompactionCheckpointStorePort(Protocol):
    ...
    async def save(
        ...
        """Save an entire checkpoint (back-compat helper).

        Dispatches to :meth:`save_core` and — when the digest / turn-prefix
        fields are non-empty — the matching fragment writer. Kept for
        callers that hand in a full :class:`CompactionCheckpoint`
        aggregate (e.g. the engine's ``persist`` best-effort path).
        """
        ...
    ...
    async def save_core(
        ...
        """CAS-write the core columns of the checkpoint.

        Returns ``True`` iff the row was written (the incoming
        ``generation`` strictly advanced the stored value). A ``False``
        result means a peer already wrote a newer state — the caller
        MUST NOT swap in the loser checkpoint.
        """
        ...
    ...
    async def save_digest(
        ...
        *,
        ...
        """CAS-write the P2 digest fragment.

        Succeeds iff the CURRENT core row's ``generation`` equals
        ``core_generation``. Returns ``False`` when the fragment write is
        stale (core row missing or advanced past the writer's snapshot).
        """
        ...
    ...
    async def save_turn_prefix(
        ...
        *,
        ...
        """CAS-write the P3 turn-prefix fragment. See :meth:`save_digest`."""
        ...
    ...
    async def update_last_eff_prompt(
        ...
        *,
        ...
        """Advance the TPP-1 delta baseline without bumping ``generation``.

        The ``last_eff_prompt`` field is a per-turn diagnostic baseline.
        Writing it does NOT advance the generation (a stale digest write
        against the same generation is still refused by :meth:`save_digest`).
        Guarded by ``core_generation`` so a peer compaction cannot see its
        baseline overwritten by an even older writer's snapshot.
        """
        ...
...
class ToolInvocationResult:
    ...
    # ``terminal_kind`` (optional appended field, §3.1 tail-only): names a
    # SUCCESSFUL but semantically distinct terminal outcome the UI should not
    # paint as generic "done". Today: ``"stopped"`` / ``"restarted"`` from
    # ``background_process(action=stop|restart)``. Mirrors the field on
    # ``StreamFrame.tool_result`` — same pattern as ``cancelled`` / the
    # ``backgrounded`` pair. ``None`` on every ordinary result.
...
class ToolStreamChunk:
    ...
    * ``backgrounded`` / ``background_process_id`` — optional DONE
      metadata for an auto-backgrounded command: the foreground wait
      exceeded the configured threshold while fresh user input arrived,
      so the still-RUNNING child was handed to the background-process
      manager and this call returned early.  The command has NOT
      finished, so ``ok=True`` alone would read as success; these fields
      let every consumer distinguish "done" from "still running
      elsewhere" WITHOUT parsing ``result`` prose (the same reason
      ``StreamFrame.tool_result`` carries ``cancelled`` rather than
      having the UI sniff a ``[已取消]`` prefix).  ``None`` on every
      ordinary chunk.
    """
    ...
    * Carry ``index`` (0-based) on every per-agent event so multiple
    ...
    def iter_events(
        ...
        * ``subagent_error``  ``{type, index, message}``
        ...
        rejects a call with ``spawn_depth >=
        Settings.max_subagent_recursion_depth``
...
class StreamAbortHandle(Protocol):
    ...
    def abort_event(self) -> "asyncio.Event":
        """The raw abort event, for a tool wait loop to race on.

        Read-only for consumers: they ``.wait()`` on it and NEVER set it —
        :meth:`signal` is the single write point.  Consumed by the exec
        4-way race (:mod:`qai.platform.exec_race`) so a non-interruptible
        tool can shape an explicit ``[Command aborted by user]`` result
        rather than relying solely on task cancellation.  Handles
        predating this attribute are probed via ``getattr`` by callers,
        which then skip that race arm.
        """
        ...
...
class InjectionRegistryPort(Protocol):
    ...
    def on_inject(
        ...
        """Subscribe ``callback`` to every ACCEPTED injection; return a disposer.

        Turns the registry into an event source so a long-running tool can
        react to a mid-turn injection the instant it lands, instead of only
        at the run loop's next inter-round seam.  Consumed by
        :class:`~qai.chat.adapters.injection_signal_bridge.InjectionSignalBridge`,
        which fans the notification out to the per-tab ``asyncio.Event``
        that the exec race waits on.

        Fired synchronously from :meth:`inject`, once per accepted
        injection.  A rejected (empty / whitespace-only) injection fires
        nothing — there is no pending text to react to.  A raising
        callback must be logged and skipped so one bad subscriber cannot
        fail the user's inject.
        """
        ...
...
class ToolResultTruncationRequest:
    ...
    #: 1-based coordinate, in the PRODUCING TOOL's own address space, of the
    #: first item in ``result_text`` (``read``'s ``start_line`` / ``list``'s
    #: ``start_entry``). Needed only by the ordered-slice hard-cap backstop:
    #: when that backstop tail-cuts an oversized slice it must tell the model
    #: which ``offset`` to resume from, and "how many lines I kept" is only an
    #: offset if the slice began at line 1. For a slice that started deeper in
    #: the file (``read(path, offset=800)``) the kept-line count is a
    #: MEANINGLESS offset — it pointed the model back into content it had
    #: already been given, so continuing the read silently looped instead of
...
class RetryCategory(str, Enum):
    ...
    error, read/write timeout, mid-stream socket drop, DNS resolution
    failure, connection refused, or host unreachable — that MAY take a
    while to self-heal (adapter codes ``chat.llm.connect_error`` /
    ``chat.llm.timeout`` / ``chat.llm.read_error`` / ``chat.llm.dns_error`` /
    ``chat.llm.connection_refused`` / ``chat.llm.host_unreachable``).
    Retries with a capped escalating backoff (``3s → 5s → 10s → 30s …``)
    but is bounded by a WALL-CLOCK budget: once cumulative retry time
    exceeds :data:`~qai.chat.adapters.retry_policy.NETWORK_WALL_CLOCK_BUDGET_SECONDS`
    ...
    2026-07-30 note: DNS / connection_refused / host_unreachable were moved
    here from ``BOUNDED_FAST``. In a desktop app they are almost always
    caused by transient WiFi / VPN blips, not permanent mistyped hostnames.
    """
    BOUNDED_FAST = "bounded_fast"
    """Reserved for connectivity faults that recover fast or are effectively
    permanent. Currently unused in-tree — every connect / DNS / read-error
    code maps to :data:`NETWORK` instead (see the 2026-07-30 reclassification
    note there). Retained in the enum for future use and back-compat."""
...
class RetryDecision:
    ...
    matches the legacy ``compress(force=True, target_ratio=0.50)`` path).
    """
...
class SystemPromptResult:
    ...
    *effective* tool mode that was used.  When auto-detection promotes
    ...
    """Optional full SKILL.md body text extracted from feature mode.

    When non-None, this text should be injected as a user-role message
    BEFORE the real user message rather than inlined in the system prompt.
    This keeps the system prompt byte-stable across turns (enabling prompt
    caching) while still providing the LLM with detailed skill instructions.
    The system prompt retains only the skill metadata (name, framing) for
    stable cacheability."""
...
# ---------------------------------------------------------------------------
# Title generation (PR-402 / S7.5 lane L4)
#
# Migrates :func:`backend.title_generator.generate_title` into the chat
# bounded context behind a dedicated port.  Title generation is a thin
# LLM call (one-shot, 30 max_tokens) so it gets its own port rather than
# re-using :class:`LLMStreamPort` (different timeout, different prompt
# template, different post-processing — quotation strip, length cap).
# ---------------------------------------------------------------------------
...
class ContextCompressionPort(Protocol):
    ...
    async def compress(
        ...
        *,
        ...
        "compress to ``bytes_before × target_ratio``" which is unrelated to the
        ...
        conversation's real token density and converts byte counts to tokens
        with it (far more accurate than a fixed bytes/token factor, and calls
        ...
        ``outcome_sink`` (optional, tail-appended per AGENTS.md §3.1): if
        provided, the implementation writes observational fields into it
        (currently ``phase_reached`` — the 4-phase drop pipeline records
        which phase resolved the compression: ``0`` = early-return under
        target before any phase, ``1``/``2``/``3``/``4`` = the phase that
        brought the wire below target). Consumed by the caller's journal
        record. Read-only from the compressor's perspective — the sink
        never influences compression behaviour, so ``None`` (the default)
        reproduces prior behaviour byte-for-byte.

        ``prune_redundant`` (optional, tail-appended per AGENTS.md §3.1):
        enables a pre-pass ("Phase 0") that DROPS whole tool interactions
        which carry no information the model can still use — a read whose
        target has since been re-read (only the newest snapshot is kept) and
        a successful, information-free result. It is drop-only (no message
        content is ever rewritten) and operates on atomic
        ``assistant{tool_calls}`` + paired ``role:tool`` groups, so it can
        never orphan a tool call. Errors are never pruned. ``False`` (the
        default) reproduces the prior 4-phase behaviour byte-for-byte.
        """
        ...
# ---------------------------------------------------------------------------
...
class ResolvedModel:
    ...
    #: Whether this endpoint's credential is a per-user token derived from the
    #: SSO login rather than a key the user can paste (the QAI Service token
    #: pool).  Threaded from :class:`ProviderEndpoint.is_sso` so the transport
    #: can tell "you must sign in again" apart from "you must configure a
    #: key": both arrive here as ``api_key=None`` on a non-local endpoint, but
    #: only the latter is fixable from an API-key dialog.  ``False`` keeps the
    #: pre-existing behaviour for every ordinary keyed cloud provider.
    is_sso: bool = False
...
class ProviderEndpoint:
    ...
    params: dict[str, Any] | None = None
    #: Whether the credential for this provider is minted from the SSO login
    #: (a short-lived per-user token) instead of being a stored static key the
    #: user could supply.  Set by the composition-root bridge, which is the
    #: only layer that knows which providers authenticate that way; the chat
    #: context stays ignorant of *how* the credential was obtained and only
    #: uses this to pick the right error when there is none.
    is_sso: bool = False
...
@runtime_checkable
class CompactionJournalPort(Protocol):
    """Best-effort append-only sink for one compaction event
    (CONTEXT-COMPRESSION-NEXT §5.1).

    :class:`~qai.chat.application.use_cases._compaction_engine.CompactionCheckpointEngine`
    emits one event per successful compaction and NEVER awaits the result:
    the dispatch goes through :func:`asyncio.create_task`, so an
    implementation that blocks or raises cannot reach the caller's turn.

    Layering (v2.7 §3.2 / §3.5)
    ---------------------------
    The concrete JSONL sink is an adapter
    (``qai.chat.adapters.compaction_journal.CompactionJournal``) because it
    owns file I/O + rotation. The application layer MUST NOT name that class
    — not even under ``TYPE_CHECKING``, since import-linter's ``layered-chat``
    contract reads the static import graph and a type-only edge is still an
    edge. This port is that boundary: the engine and
    :class:`StreamChatUseCase` annotate against it, the adapter satisfies it
    structurally, and the apps/api composition root wires the concrete sink.

    Contract
    --------
    :meth:`append` MUST NOT raise: it is a telemetry sink, and a failed write
    is a debug log, never a failed chat turn. Implementations own their own
    serialisation / rotation / encoding decisions.
    """

    async def append(self, event: dict[str, Any]) -> None:
        """Record one compaction event. NEVER raises."""
        ...


@runtime_checkable
class ScheduledTaskToolPort(Protocol):
    """Execute one ``scheduled_task`` action (create / edit / pause / run / …).

    The ONE validated entry point for every scheduled-task mutation: schedule
    parsing, whitelist coercion and next-run recomputation live behind it, so
    the LLM-facing ``scheduled_task`` tool and the WebUI manager panel
    (``interfaces.http.routes.scheduled_tasks``) share a single source of
    truth instead of each re-implementing the validation.

    Layering (``interfaces-stays-thin`` import-linter contract)
    ----------------------------------------------------------
    The concrete handler is an adapter
    (``qai.chat.adapters.scheduled_task_tool.ScheduledTaskToolHandler``); the
    routes layer MUST NOT name it — not even under ``TYPE_CHECKING``, because
    the contract reads the static import graph and a type-only edge is still
    a direct edge. The route annotates against this port and resolves the
    instance off ``container.chat`` at request time; the adapter satisfies it
    structurally.

    Contract
    --------
    :meth:`execute` returns the tool's JSON string payload (the same body the
    LLM receives). Validation failures surface as an exception the caller
    maps onto its own transport error.
    """

    async def execute(self, request: ToolInvocationRequest) -> str:
        """Run one scheduled-task action; return the tool's JSON string."""
        ...
...
__all__ = [
    ...
    "PromoteReadyVariant",
    # best-effort compaction journal sink (adapter owns the JSONL file I/O)
    "CompactionJournalPort",
    # scheduled-task tool handler (single validated mutation entry point)
