...
#: classifier's 2000ms.  30s (not the old 8s) because a DEEP discussion (12+
#: rounds) hands the extractor a very long conclusion + recent-turns context; a
#: cloud model reading that much input and emitting structured JSON — plus a
#: possible one-shot repair round — routinely exceeds 8s and would time out into
#: a spurious ``planning_failed`` exactly when the discussion is richest (the
#: real-world failure this default fixes).  Tunable via
#: ``implementation_planner_timeout_ms`` (up to MAX below); the use case wraps
#: the call in ``asyncio.wait_for`` so a hung extractor never wedges — it
#: degrades to a ``planning_failed`` plan.
DEFAULT_PLANNER_TIMEOUT_MS: Final[int] = 30000
