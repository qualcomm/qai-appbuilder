...
def diagnose_intent(
    ...
    * NO clear direct-out signal — no ``@mention``, no clear stop/cancel, no
    ...
    # 现象3 — audience-wide address: a phrase targeting the WHOLE group
    # ("各角色 / 大家 / 所有人 / everyone") rather than a single @mentioned role.
    has_group_address = rules.contains_any(
        normalized, rules.GROUP_ADDRESS_TERMS, locale
    )
    ...
    # 5) 开场白灰区 (§22A.5 扩展): a NON-short, non-social opener in an idle /
    #    closed conversation that the heuristic did NOT confidently route to a
    #    full task (no clear task verb → it fell to follow_up/scoped, or was a
    #    low-confidence deep_task).  This is the "我想开发一个X，大家有什么想法？"
    #    shape — a genuine multi-role discussion opener whose task verb the
    #    hardcoded lexicon may not cover.  The keyword table can never be
    #    exhaustive, so hand these to the LLM to judge scoped-vs-full by MEANING.
    #    Gated to idle/closed + non-short + no soft social cue so it never fires
    #    on mid-thread follow-ups (those are covered by reasons 1-4) or on tiny
    #    greetings.
    soft_social_cue = is_thanks or is_greeting or is_ack
    if (
        state in ("idle", "closed")
        and not is_short
        and not soft_social_cue
        and not target_roles
        and not (has_task_verb and not is_short)
    ):
        ambiguity_reasons.append("substantive_opener")
    # 6) 全员追问灰区 (现象3, §22A.5 扩展): an AUDIENCE-WIDE follow-up — a phrase
    #    addressing the WHOLE group ("各角色怎么看？" / "大家有什么想法？") + a
    #    question/follow-up cue, with NO single @mention, while a discussion is
    #    live (awaiting_user / active_discussion) or the message overlaps the
    #    prior topic. The heuristic routes such a message to a SCOPED follow_up
    #    (single responder) — wrong for "各角色怎么看" which should re-open the
    #    floor to everyone. Hand it to the LLM to judge scoped-vs-full BY
    #    MEANING; the accepted verdict may escalate to a full re-discussion
    #    (mirrors ``substantive_opener`` as an escalation-permitting reason in
    #    ``_maybe_refine_intent``). Gated to no @mention (a directed follow-up
    #    stays scoped) + a live/awaiting thread so it never fires on a fresh
    #    idle greeting.
    if (
        has_group_address
        and (has_q or has_question_word or has_follow_hint)
        and not target_roles
        and (
            state in ("awaiting_user", "active_discussion")
            or awaiting_user
            or has_topic_overlap
        )
    ):
        ambiguity_reasons.append("audience_wide_followup")
