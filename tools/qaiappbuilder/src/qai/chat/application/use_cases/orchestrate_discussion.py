...
from qai.chat.application.use_cases._agentic_kernel import (
    tool_result_slice_start as _tool_result_slice_start,
)
...
# ---------------------------------------------------------------------------
# Coordinator (主持人) unification (现象2 + feature).  The moderator + judge are
# ONE user-configurable "coordinator" entity with its own model + prompt, shown
# under a fixed host identity (NOT a borrowed roster persona/model).
# ---------------------------------------------------------------------------
#: Synthetic participant id for the coordinator.  A sentinel (double-underscore,
#: never a real roster id) so ``_run_speaker_turn`` can detect a coordinator turn
#: (→ ``is_coordinator=True`` on the frame) and ``_build_persona`` can compose the
#: built-in judge directive + ``coordinator_prompt`` instead of a roster persona.
_COORDINATOR_PARTICIPANT_ID = "__coordinator__"
#: Fixed display name for the coordinator bubble (the front-end localises via the
#: ``is_coordinator`` marker; this is the backend default label).
_COORDINATOR_DISPLAY_NAME = "主持人"
#: Built-in judge directive (the single source of truth, §Rules "no hardcoded
#: prompts beyond framing constants").  ``coordinator_prompt`` (when set) is
#: appended to this — advisory extra guidance, never a roster role's persona.
_DEFAULT_JUDGE_DIRECTIVE = (
...
#: Default tools the SYNTHETIC coordinator (see :data:`_COORDINATOR_PARTICIPANT_ID`)
#: advertises when :meth:`_resolve_judge` synthesises it (DISC-1 coordinator-plan
#: rework, spec ``local://coordinator-plan-spec.md``).  Ordinary discussion
#: roles STILL default to no tools — this list is applied ONLY on the
#: synthetic coordinator so the "生成实施方案" preset turn can (a) write the plan
#: document with ``write``/``edit``/``apply_patch``, (b) register lightweight
#: items with ``implementation_plan``, (c) inspect the workspace with the
#: read-side helpers.  A user override is still possible: the coordinator's
#: ``config`` here is the DEFAULT — future work exposing a per-conversation
#: ``coordinator_allowed_tools`` blob can layer on top of this constant.  The
#: global hard-exclude set (``_DISCUSSION_EXCLUDED_TOOLS``) stays EMPTY — no
#: hardcoded tool bans (user directive 2026-06-21).
_COORDINATOR_DEFAULT_ALLOWED_TOOLS: tuple[str, ...] = (
    "write",
    "edit",
    "apply_patch",
    "implementation_plan",
...
# ---------------------------------------------------------------------------
# Round-budget hint (现象1).  A FIXED per-discussion hint (NO current-round
# number) injected into the OPENING user message's wire on FULL-discussion
# turns so round_robin roles converge instead of always running to max_rounds.
# Stable across rounds → prompt-cache friendly (byte-for-byte identical replay).
# ---------------------------------------------------------------------------
_ROUND_BUDGET_HINT_TEMPLATE = (
    "【讨论节奏】本次讨论最多进行 {n} 轮。请合理把握发言节奏：随着讨论推进逐步"
    "收敛观点，在接近轮次上限时给出结论性意见，避免无谓重复或无限展开。此为整场"
    "讨论的节奏约束，并非要求你立即结束本轮发言。"
)
#: ``meta`` key stamped on the opening user Message carrying the round budget.
_DISCUSSION_ROUND_BUDGET_KEY = "discussion_round_budget"


def _round_budget_hint(n: int) -> str:
    """Render the FIXED round-budget hint for a discussion of ``n`` max rounds.

    Pure + deterministic: the SAME ``n`` always yields the SAME text (no
    current-round number), so replaying the opening user message across rounds
    is byte-for-byte stable (prompt-cache hits, §现象1).
    """
    return _ROUND_BUDGET_HINT_TEMPLATE.format(n=n)
...
_IMPLEMENTATION_MODE_FRAMING = (
    ...
    "本功能项的详细要求写在工作区文件 实施计划.md 里——动手前请先用读文件工具打开 "
    "实施计划.md 查阅对应章节。"
    "你可以使用工具读写文件、执行命令来完成任务。完成后用一段简短中文总结你做了"
    "什么、改动了哪些文件、是否还有未尽事项。\n"
    "【Implementation mode】You are now IN IMPLEMENTATION MODE: execute the "
    "assigned task based on the discussion conclusions. The DETAILED "
    "requirements for this item live in the workspace file 实施计划.md — read "
    "it first (with the read tool) for the relevant section before you start. "
    ...
    "本功能項的詳細要求寫在工作區檔案 实施计划.md，動手前請先讀取查閱對應章節。"
    "可使用工具讀寫檔案、執行命令完成任務，完成後簡短總結。"
)

#: Stable, locale-independent control marker the "生成实施方案" button appends
#: to its preset ``@主持人 …`` message (see frontend slice + spec
#: ``local://coordinator-plan-spec.md``).  Presence of this exact substring in
#: the coordinator turn's latest user text switches the coordinator's persona
#: from the summariser directive to the plan-generation framing below.  ASCII
#: so the same anchor works verbatim across en / zh-CN / zh-TW preset texts.
_PLAN_GENERATION_MARKER = "[[gen_impl_plan]]"

#: The workspace filename the coordinator writes the full implementation plan
#: to.  Single source of truth for the framing prose AND the
#: :class:`ImplementationPlanToolHandler`'s default confirmation text — the
#: frontend + backend agreed on this exact simplified-Chinese filename in the
#: spec (all three UI-locale presets target the same workspace file).
_PLAN_DOC_FILENAME = "实施计划.md"

# DISC-1 coordinator-plan-rework (spec ``local://coordinator-plan-spec.md``) —
# plan-generation framing for the coordinator (主持人) turn.  Mirrors
# :data:`_DEFAULT_DISCUSSION_PROMPT` / :data:`_IMPLEMENTATION_MODE_FRAMING`
# style: a short tri-lingual constant, prepended in place of the ordinary
# summariser directive when the coordinator turn's latest user text carries
# :data:`_PLAN_GENERATION_MARKER`.  Instructs the coordinator to produce THREE
# artefacts in ONE streamed turn — write the FULL plan document to
# :data:`_PLAN_DOC_FILENAME` via the ``write`` tool, register a LIGHTWEIGHT
# structured checklist via the ``implementation_plan`` tool (auto-assign each
# item's ``assigned_role``; do NOT paste the plan detail into the tool args,
# use ``doc_ref`` to point back to the document section), then a short
# in-chat summary.  The executor roles read the document for the requirements;
# see :data:`_IMPLEMENTATION_MODE_FRAMING`.
_COORDINATOR_PLAN_GENERATION_FRAMING = (
    "【生成实施计划】用户已请求你把当前讨论整理成实施计划。请在本轮内："
    "(1) 调用 implementation_plan 工具登记轻量任务清单，每项只写简短 title、"
    "自动指派最合适的 assigned_role（用参与者 id）、可选的 depends_on / "
    "verify_command / doc_ref——不要在 items 里重写细节；"
    f"(2) 尽量使用 write 工具把完整实施计划写入工作区文件 `{_PLAN_DOC_FILENAME}`"
    "（背景/目标、结论与分歧、详细步骤、验收标准、遗留问题）；"
    "(3) 无论文件是否写入成功，都必须在本轮回复正文里用 Markdown 完整给出实施"
    "计划全文——这样即使写文件被安全策略拒绝，用户仍能看到完整计划。若 write "
    "工具报“被安全策略拒绝/权限”类错误，不要反复重试不同路径，直接在回复里给出"
    "完整计划并提示用户：如需保存为文件，请先在设置中为本会话配置并授权一个工作区。\n"
    "【Generate implementation plan】The user asked you to turn the discussion "
    "into an implementation plan. In THIS single turn: (1) call the "
    "implementation_plan tool to register a LIGHTWEIGHT checklist (each item: a "
    "short title, an auto-assigned participant id in ``assigned_role``, optional "
    "``depends_on`` / ``verify_command`` / ``doc_ref`` — DO NOT paste plan detail "
    "into the args); (2) best-effort use the write tool to save the FULL plan to "
    f"the workspace file `{_PLAN_DOC_FILENAME}`; (3) REGARDLESS of whether the "
    "file write succeeds, ALWAYS include the full plan as Markdown in your reply "
    "body, so a FileGuard/permission denial never leaves the user empty-handed. "
    "If write is denied, do NOT retry different paths — just give the full plan "
    "in the reply and tell the user to configure & authorize a workspace for this "
    "conversation in Settings if they want it saved as a file.\n"
    "【產生實施計畫】使用者要求把討論整理成實施計畫。本輪內：(1) 呼叫 "
    "implementation_plan 工具登記輕量清單（每項僅 title、自動指派 assigned_role、"
    "可選 depends_on / verify_command / doc_ref，不要複製細節）；(2) 盡量用 write "
    f"工具把完整計畫寫入工作區檔案 `{_PLAN_DOC_FILENAME}`；(3) 無論檔案是否寫入"
    "成功，都必須在回覆正文用 Markdown 完整給出計畫全文；若 write 被安全策略拒絕，"
    "不要重試不同路徑，直接在回覆給出完整計畫並提示使用者先為本會話配置並授權工作區。"
)
...
def _bare_role_targets(text: str, roster: list[Participant]) -> list[str]:
    """Detect a bare (no ``@``) role name the user is addressing (layer 1 of the
    semantic follow-up routing).

    A follow-up like "反方还有什么要补充的吗？" names the intended speaker in
    prose without an ``@``.  Returns the display names of roster members the
    message ADDRESSES, so the caller treats them like an ``@mention`` (→
    directed, scoped to that role).  Conservative — only fires when a role name
    leads the message (after optional lead-ins) and is followed by an address
    boundary, so a message that merely *discusses* a role is not hijacked.
    Longest name wins.  ``[]`` when nothing clearly addresses a role (caller
    keeps normal non-directed handling — LLM semantic layer / mechanical
    fallback).
    """
    if not isinstance(text, str) or not text.strip():
        return []
    norm = text.strip()
    # Strip leading discourse particles so "那反方…" / "那么请正方…" still match
    # the role at the head. Longest-first + loop-until-stable so a multi-char
    # lead ("那么") is not partially eaten by a single-char one ("那").
    _leads = ("那么", "接下来", "麻烦", "现在", "请", "那", "让", "so ", "now ")
    while True:
        for lead in _leads:
            if norm.startswith(lead):
                norm = norm[len(lead):].lstrip()
                break
        else:
            break
    low = norm.casefold()
    names = sorted(
        (
            (p.display_name or p.id.value).strip()
            for p in roster
            if (p.display_name or p.id.value).strip()
        ),
        key=len,
        reverse=True,
    )
...
def _filter_roster_by_mentions(
    ...
    if not wanted:
        return roster
    # Match a roster member when a mention EQUALS or STARTS WITH its display
    # name (CJK mentions have no trailing space, so "@反方…" arrives as
    # "反方…" — a prefix match resolves it; same fix as _mention_ordered_speakers).
    def _hit(p: Participant) -> bool:
        name = (p.display_name or p.id.value).strip().casefold()
        return bool(name) and any(w == name or w.startswith(name) for w in wanted)
    matched = [p for p in roster if _hit(p)]
...
def _mention_ordered_speakers(
    ...
    if not mentions:
        return []
    # Casefolded display-name → participant (first occurrence wins). Sorted by
    # descending name length so the LONGEST matching prefix wins (a roster with
    # "评审" and "评审员" resolves "@评审员…" to 评审员, not 评审).
    by_name: dict[str, Participant] = {}
    for p in roster:
        key = (p.display_name or p.id.value).strip().casefold()
        if key and key not in by_name:
            by_name[key] = p
    names_by_len = sorted(by_name.keys(), key=len, reverse=True)
    out: list[Participant] = []
    seen: set[str] = set()
    for m in mentions:
        key = m.strip().casefold()
        # Exact match first; else PREFIX match — CJK has no space after a
        # mention ("@反方还有什么要补充的吗" captures "反方还有…"), so a name
        # the candidate STARTS WITH is the intended target. This fixes the
        # greedy-capture bug where "@反方…" routed to the wrong (last) speaker.
        p = by_name.get(key)
        if p is None:
...
class OrchestrateDiscussionUseCase:
    ...
    async def execute(
        ...
        # Sentinel coordinator (``__coordinator__``) as a first-class roster
        # member for the duration of THIS turn (State-Truth-First — user
        # directive 2026-07-26). UNCONDITIONALLY injected whenever the
        # conversation has ``enable_judge`` on, so ``@协调者`` / ``@主持人`` /
        # ``@Coordinator`` etc. reliably resolve to the sentinel via the
        # existing roster-scoped ``_mention_ordered_speakers`` — no fragile
        # token pre-scan. Ordinary rounds are unaffected because
        # :meth:`SpeakerSelectionState.eligible` FILTERS the sentinel out of
        # the round-robin / manager rotation (it only speaks when explicitly
        # scoped to it by an ``@coordinator`` mention). Policy Planner
        # consumers of ``roster_ids`` are safe: the sentinel appends at the
        # END, ``roster_ids[0]`` still returns a real role, and every ``in
        # roster_ids`` check just widens by one id that can only be reached
        # via an explicit mention.
        #
        # The coordinator's ``allowed_tools`` / ``enabled_skills`` come from
        # :meth:`_resolve_judge` (dynamic default = all registered tools minus
        # ``agent`` + every globally enabled skill; overridable per-conversation
        # via ``discussion["coordinator_config"]`` written by the PATCH route),
        # so ``@协调者 读取 X`` actually invokes the tool.
        #
        # In-memory only — never persisted. Skipped when (a) ``enable_judge``
        # is off (conversation opted out), (b) a legacy ``judge_participant_id``
        # already names a roster member (that member IS the coordinator by the
        # §31 ladder — no synthetic overlay), or (c) the sentinel is somehow
        # already present (defensive idempotency).
        if _coerce_bool(discussion.get("enable_judge")):
            _legacy_judge_id = discussion.get("judge_participant_id")
            _has_legacy_judge = (
                isinstance(_legacy_judge_id, str)
                and _legacy_judge_id.strip()
                and any(p.id.value == _legacy_judge_id for p in roster)
            )
            _already_present = any(
                p.id.value == _COORDINATOR_PARTICIPANT_ID for p in roster
            )
            if not _has_legacy_judge and not _already_present:
                _coord = self._resolve_judge(
                    discussion, roster, default_model_id=request.default_model_id
                )
                if _coord is not None:
                    roster.append(_coord)
        ...
        # Semantic follow-up routing (layer 1): when the user addresses a role
        # by name WITHOUT an ``@`` ("反方还有什么要补充的吗？"), treat that bare
        # role name as a mention so the turn is directed to that role — instead
        # of the mechanical last-active pick that made 正方 answer a question
        # aimed at 反方.  Only when there is no explicit ``@`` mention already.
        if not mentions:
            mentions = _bare_role_targets(mention_text, roster)
        ...
        # 现象1 — round-budget hint gate. ONLY a FULL-discussion turn (the
        # first-round opener that runs ``_run_full_path`` AND an audience-wide
        # re-discussion follow-up the LLM escalated to ``deep_task``) gets the
        # FIXED round-budget hint; scoped / lightweight / social / single-role
        # follow-ups do NOT (they are deliberately short). The gate keys on the
        # planner's ``execution_path == "full"`` (both scenarios resolve there).
        # The budget ``n`` mirrors ``_run_full_path.effective_rounds`` (a directed
        # deep_task carries a SCALED cap in ``policy.max_rounds``; a plain
        # deep_task uses the configured ``max_rounds``). We stamp the OPENING user
        # message (just persisted at ①, so ``messages[-1]``) with the budget in
        # its ``meta`` — the content stays the user's original text. ``Message``
        # is frozen, so we ``dataclasses.replace`` a meta-carrying copy in place
        # and re-save. Non-full turns leave the persisted message untouched.
        if policy.execution_path == "full" and conv.messages:
            _budget_n = (
                policy.max_rounds
                if policy.max_rounds and policy.max_rounds >= 1
                else max_rounds
            )
            _opening = conv.messages[-1]
            if _opening.role is MessageRole.USER:
                _meta = dict(_opening.meta) if isinstance(_opening.meta, dict) else {}
                _meta[_DISCUSSION_ROUND_BUDGET_KEY] = int(_budget_n)
                conv.messages[-1] = replace(_opening, meta=_meta)
        ...
        try:
            # DISC-1 coordinator-plan-rework — plan-generation short-circuit.
            # The "生成实施方案" button sends a preset ``@主持人 …`` message
            # carrying :data:`_PLAN_GENERATION_MARKER`.  Route it DIRECTLY to a
            # single coordinator turn (bypassing the selector loop): the
            # coordinator writes the plan doc + calls ``implementation_plan`` +
            # summarises.  Highest priority so the marker never falls through to
            # ordinary discussion / a roster role.  A conversation with no roster
            # yields no coordinator → falls through to normal handling.
            if _PLAN_GENERATION_MARKER in (request.user_message.text or ""):
                routed_to_implementation = True
                async for frame in self._run_plan_generation(
                    conv=conv,
                    request=request,
                    state=state,
                    seq=seq,
            ...
            # A running/parked implementation must NOT trap EVERY message in the
            # control router's "say 继续/暂停/…" prompt. A message that carries no
            # actionable control intent (``general_message`` — e.g. a directed
            # "@主持人 读取 X" tool instruction, or any ordinary discussion line)
            # is let through to the normal directed/discussion dispatch below so
            # the addressed role actually runs it. Real control phrases
            # (stop/pause/resume/retry/skip/status/…) classify to their own
            # intent BEFORE the general_message fallback, so they still route to
            # the control router unchanged. This generalises the original
            # ``failed``-only escape hatch to every live phase (planned /
            # implementing / paused / failed) — the run keeps going on a plain
            # message, and the user is never locked out of talking to a role
            # mid-run.
            if _route_to_control and impl_plan is not None:
            ...
            if routed_to_implementation:
                # Plan-generation short-circuit already ran the coordinator turn
                # above; skip the ordinary control / impl / discussion dispatch.
                pass
            elif _route_to_control:
    ...
    async def _maybe_refine_intent(
        ...
        * Gating: an LLM verdict below :data:`LLM_CONFIDENCE_FLOOR`, or one that
        ...
        if refined.confidence < LLM_CONFIDENCE_FLOOR:
            ...
            return heuristic_verdict
        # A "strong signal" lets the LLM escalate the grey zone to a FULL
        # discussion. Besides the heuristic's task_verb / mention, a
        # ``substantive_opener`` (a real multi-role discussion opener in an idle
        # conversation whose task verb the lexicon may not cover — e.g. "我想
        # 开发一个X，大家有什么想法？") is ALSO a legitimate escalation trigger:
        # the LLM judged BY MEANING that this opener warrants full discussion, and
        # a fresh opener is exactly the case §21.11's "never escalate" was NOT
        # meant to trap (that rule targets mid-thread ambiguous follow-ups).
        # 现象3: an ``audience_wide_followup`` ("各角色怎么看？") is likewise a
        # legitimate escalation trigger — the user explicitly re-opened the floor
        # to the WHOLE group, so the LLM may route it to a full re-discussion
        # (route audience-wide follow-ups to full re-discussion).
        strong_signal = (
            "task_verb" in heuristic_diag.signals
            or "mention" in heuristic_diag.signals
            or "substantive_opener" in heuristic_diag.ambiguity_reasons
            or "audience_wide_followup" in heuristic_diag.ambiguity_reasons
        )
    ...
    def _build_selector(
        ...
        *,
        ...
        """
        strategy = str(discussion.get("selector_mode") or "manager").strip().lower()
        ...
        # provider); fall back to the mode's ``system_model_id``, then to the
        # tab-selected ``default_model_id`` (mirrors the speaker path's
        # ``speaker.model_id or default_model_id`` precedence) so a discussion
        # whose participants LEFT THEIR MODEL BLANK still resolves a real
        # endpoint instead of handing the manager a None hint. Degrades to
        ...
        if policy_participants:
            ...
            if scoped:
                hint_pool = scoped
        # 现象2 + feature: the moderator shares the coordinator's model so
        # moderator + judge run on ONE configured entity. Ladder:
        # coordinator_model_id → first eligible roster model → mode system model
        # → tab default (existing tail unchanged for legacy conversations whose
        # coordinator_model_id is unset).
        _coordinator_model = discussion.get("coordinator_model_id")
        model_hint = (
            (
                _coordinator_model.strip()
                if isinstance(_coordinator_model, str) and _coordinator_model.strip()
                else None
            )
            or (hint_pool[0].model_id if hint_pool else None)
            or (mode.flow_policy.system_model_id if mode else None)
            or default_model_id
        )
    ...
    def _coordinator_model_hint(
        ...
        *,
        ...
        """Resolve the coordinator model per the §31 ladder (现象2 + feature).

        ``coordinator_model_id`` → ``roster[0].model_id`` → ``default_model_id``.
        Shared by the manager moderator (``_build_selector``) AND the judge turn
        so moderator + judge run on ONE configured entity.
        """
        cid = discussion.get("coordinator_model_id")
        coordinator_model = cid.strip() if isinstance(cid, str) and cid.strip() else None
        return (
            coordinator_model
            or (roster[0].model_id if roster else None)
            or default_model_id
        )
    def _coordinator_default_tools(self) -> list[str]:
        """Return the coordinator's default ``allowed_tools`` — ALL registered
        tools EXCEPT ``agent``.

        The built-in coordinator is a system-level moderator/judge/plan-writer
        and by design has access to every tool the user has, so it never fails
        "@coordinator do X" for lack of a tool. ``agent`` (sub-agent spawn) is
        deliberately excluded: the coordinator IS itself the top-level
        orchestrator of the discussion, letting it recursively spawn sub-agents
        collides with that role and with the existing sub-agent recursion
        guard. Users may still narrow this via ``config['allowed_tools']``
        (once the UI editor lands); the returned list is only the DEFAULT.

        ``skill`` is intentionally NOT added here — the ``skill`` tool is
        derived downstream from ``config['enabled_skills']`` (see
        :meth:`_speaker_tool_schemas` 4876-4886). A missing ``self._tools`` /
        registry failure falls back to the historical static default so a
        wiring hiccup never leaves the coordinator toolless.
        """
        static_fallback: list[str] = list(_COORDINATOR_DEFAULT_ALLOWED_TOOLS)
        ...
        if not callable(schemas_fn):
            return static_fallback
        try:
            advertised = schemas_fn() or ()
        except Exception:  # noqa: BLE001 — degrade to the safe static default
            return static_fallback
        names: list[str] = []
        seen: set[str] = set()
        for s in advertised:
            ...
            if not isinstance(name, str) or not name:
                continue
            if name == "agent":  # never let coordinator recursively spawn sub-agents
                continue
            if name == "skill":  # advertised via enabled_skills instead
                continue
            if name in seen:
                continue
            seen.add(name)
            names.append(name)
        return names or static_fallback

    def _coordinator_default_skills(self) -> list[str]:
        """Return the coordinator's default ``enabled_skills`` — EVERY skill
        the system prompt builder currently advertises (i.e. globally enabled).

        Rationale mirrors :meth:`_coordinator_default_tools`: the built-in
        coordinator should never be denied a skill the user has enabled. The
        list is computed live from the same
        :attr:`SystemPromptBuilderPort.skill_catalog_provider` used by
        :meth:`_build_persona` (5112-5122), so it stays in lock-step with the
        user's Settings → Skills toggles without a restart. A missing builder
        / provider / provider error → ``[]`` (no skill catalog; safe — the
        coordinator still has every other tool).
        """
        builder = getattr(self, "_system_prompt_builder", None)
        if builder is None:
            return []
        provider = getattr(builder, "skill_catalog_provider", None)
        if not callable(provider):
            return []
        try:
            rows = tuple(provider() or ())
        except Exception:  # noqa: BLE001 — degrade to no catalog on error
            return []
        # ``skill_catalog_provider`` returns rows of shape ``(path, use_for)``
        # (see :class:`ChatSkillCatalogProvider.__call__` — the concrete DI
        # wiring). The skill id is derived from the path exactly as
        # :func:`filter_skill_catalog_by_ids` does (``basename(dirname(path))``
        # — the parent-directory name IS the skill id, matching the frontend
        # skills store + ``LocalChatSkillCatalogProvider``); the previous
        # ``getattr(row, "id", None)`` lookup silently returned ``None`` for
        # every tuple row, leaving the coordinator with an empty default
        # ``enabled_skills`` list and therefore NO skills — contradicting this
        # method's contract. Also accept the ``{"path": ..}`` dict shape the
        # ``filter_skill_catalog_by_ids`` helper accepts, so any future
        # provider variant is handled the same way.
        import os
        out: list[str] = []
        seen: set[str] = set()
        for row in rows:
    ...
    def _resolve_judge(
        ...
        *,
        ...
        """Resolve the coordinator (主持人) for the final summary turn (现象2).

        Resolution (§31):

        * ``coordinator_model_id`` set → a SYNTHETIC coordinator participant with
          that model, NO roster persona, displayed as the fixed "主持人" host;
          the judge directive = built-in directive + ``coordinator_prompt``.
        * else legacy ``judge_participant_id`` names a roster member → that member
          is the judge, byte-for-byte the OLD behaviour (back-compat — existing
          conversations keep their configured judge role/persona/model).
        * else → a SYNTHETIC coordinator (model = roster[0].model_id or
          ``default_model_id``), replacing the old ``roster[-1]`` borrow so the
          judge is a distinct host identity, not a participant's persona/model.

        ``roster`` is empty → ``None`` (no judge turn).  The synthetic coordinator
        carries the sentinel :data:`_COORDINATOR_PARTICIPANT_ID` so downstream
        (``_run_speaker_turn`` / ``_build_persona``) recognises a coordinator turn.
        """
        if not roster:
            return None
        cid = discussion.get("coordinator_model_id")
        has_coordinator = isinstance(cid, str) and cid.strip()
        if not has_coordinator:
            # Legacy path: an explicit judge_participant_id still runs the OLD way.
            judge_id = discussion.get("judge_participant_id")
            if isinstance(judge_id, str) and judge_id:
                for p in roster:
                    if p.id.value == judge_id:
                        return p
        # Synthetic coordinator (either coordinator_model_id set, or the new
        # default replacing roster[-1]). NO roster persona — persona=None so
        # _build_persona composes the built-in judge directive + coordinator_prompt.
        model_hint = self._coordinator_model_hint(
            discussion, roster, default_model_id=default_model_id
        )
        now = self._clock.now()
        # Coordinator default tools/skills (user directive 2026-07-26): give the
        # built-in coordinator EVERY registered tool (minus ``agent`` to avoid
        # recursive sub-agent spawn) AND every globally enabled skill by default,
        # so "@coordinator read X" never fails for lack of a tool. Users may
        # narrow this via ``discussion["coordinator_config"]`` (a future UI
        # editor writes here) — either key missing / non-list ⇒ use the dynamic
        # default. The dynamic default is computed LIVE (tool registry + skill
        # provider) so it tracks the user's current setup without a restart.
        # The legacy ``judge_participant_id`` roster member returned ABOVE, so
        # this never overwrites a user-configured judge role's tools.
        coord_cfg = discussion.get("coordinator_config")
        if not isinstance(coord_cfg, dict):
            coord_cfg = {}
        override_tools = coord_cfg.get("allowed_tools")
        override_skills = coord_cfg.get("enabled_skills")
        allowed_tools = (
            [t for t in override_tools if isinstance(t, str)]
            if isinstance(override_tools, list)
            else self._coordinator_default_tools()
        )
        enabled_skills = (
            [s for s in override_skills if isinstance(s, str)]
            if isinstance(override_skills, list)
            else self._coordinator_default_skills()
        )
        return Participant.create(
            participant_id=ParticipantId(_COORDINATOR_PARTICIPANT_ID),
            conversation_id=roster[0].conversation_id,
            kind=ParticipantKind.NAMED_AGENT,
            now=now,
            display_name=_COORDINATOR_DISPLAY_NAME,
            model_id=model_hint,
            persona=None,
            config={
    ...
    async def _run_plan_generation(
        ...
        *,
        ...
        """Run ONE coordinator turn that generates the implementation plan.

        Triggered by the "生成实施方案" button, which sends a preset ``@主持人``
        message carrying :data:`_PLAN_GENERATION_MARKER` through the ordinary
        chat send path.  Bypasses the discussion selector loop entirely — the
        coordinator alone runs, with its default tools (``write`` /
        ``implementation_plan`` / read) advertised via ``_speaker_tool_schemas``
        and its persona switching to the plan-generation framing (marker-gated
        inside ``_build_persona``).  The coordinator writes the FULL plan
        document, calls the ``implementation_plan`` tool to register the
        lightweight items (auto-assigned roles), and gives a short in-chat
        summary — all in ONE streamed turn.

        Roster is empty → nothing to run (no coordinator can be synthesised
        without at least one roster member for the model-hint ladder).
        """
        roster = list(state.participants)
        if not roster:
            return
        discussion = conv.discussion or {}
        coordinator = self._resolve_judge(
            discussion, roster, default_model_id=request.default_model_id
        )
        ...
        *,
        ...
        for round_index in range(1, rounds + 1):
            for speaker in speakers:
                ...
                # Coordinator (主持人) is not a "substantive speaker" for
                # ``last_active_speaker`` purposes: it is a fixed
                # arbitration/summary role, NOT one of the ordinary discussion
                # roles the responder ladder is meant to route to (§21.6 /
                # §21.14#4). Path A (lightweight) already excludes ITSELF from
                # writing this field on the same principle. If we let a
                # ``@主持人 read X`` scoped turn stamp the coordinator sentinel
                # here, the very next social/ack turn's responder ladder
                # (``select_social_responder``: last_active → recent → first)
                # would return the coordinator as the "Hi/thanks" replier —
                # a distinct UX regression (the arbitrator becomes the small-
                # talk bot). Skip the sentinel; a real roster role written
                # here still flows into the ladder as designed.
                if speaker.id.value != _COORDINATOR_PARTICIPANT_ID:
    ...
    async def _run_implementation_path(
        ...
        *,
        ...
        3. **conservative tool intersection** — ``compute_effective_implementation_tools``
           ...
           ``web_fetch``, denied by default), injected via ``tool_filter_override``.
    ...
    async def _persist_plan(
        ...
        try:
            ...
            await self._conversations.save_meta(conv)
    ...
    async def _run_planned_implementation(
        ...
        *,
        ...
        # DISC-1 三期-step1: stratify items into topological LAYERS so each
        # layer can run concurrently once its dependencies (prior layers) are
        # settled. A cycle / unsatisfiable graph degrades to the original order
        # (State-Truth-First — never drop items). A plan with no ``depends_on``
        # is returned as a single layer ⇒ byte-for-byte the 二期 serial behaviour
        # when concurrency == 1.
        layers, topo_ok = topological_item_layers(plan.items)
        if not topo_ok:
            _log.warning(
                "chat.discussion.implementation_depends_on_cycle",
                run_id=run_id,
            )
        concurrency = self._resolve_impl_concurrency(conv.discussion)
        plan_ref: list[ImplementationPlan] = [plan]

        try:
            for layer in layers:
                async for frame in self._run_layer_concurrently(
                    layer=layer,
                    conv=conv,
                    request=request,
                    plan_ref=plan_ref,
                    ...
                    yield frame
                plan = plan_ref[0]
        except ImplementationRunBudgetExceeded:
            # Already handled (paused + frame yielded) inside _run_single_item;
            # stop the run without further processing.
            return
    ...
    # ------------------------------------------------------------------
    # DAG concurrency helpers (S2/S4/S5)
    # ------------------------------------------------------------------

    def _resolve_impl_concurrency(self, discussion: dict[str, Any] | None) -> int:
        """Read the implementation concurrency cap from discussion config.

        Default 1 (serial, byte-for-byte current behaviour). 0 = unlimited.
        """
        if not discussion:
            return 1
        impl = discussion.get("implementation")
        if not isinstance(impl, dict):
            return 1
        raw = impl.get("concurrency")
        if raw is None:
            return 1
        try:
            val = int(raw)
    ...
    async def _run_layer_concurrently(
        self,
        *,
        layer: tuple[FeatureItem, ...],
        conv: Conversation,
        request: OrchestrateDiscussionInput,
        plan_ref: list[ImplementationPlan],
        state: SpeakerSelectionState,
        seq: _SequenceCounter,
        handle: StreamAbortHandle | None,
        budget_tracker: "ImplementationRunBudgetTracker",
        ...
        """Run a single DAG layer, respecting concurrency and local-model constraints.

        Items using a local model (``local::`` prefix) always run serially (S5).
        Cloud items run up to ``concurrency`` in parallel (S2).
        When ``concurrency == 1``, ALL items run serially (= current behaviour).
        """
        # If concurrency is 1, just run all items serially (fast path, zero-change).
        if concurrency == 1:
            for item in layer:
                async for frame in self._run_single_item(
                    item=item, conv=conv, request=request,
                    plan_ref=plan_ref, state=state, seq=seq,
                    handle=handle, budget_tracker=budget_tracker, run_id=run_id,
                ):
                    yield frame
            return

        # S5: split into local (serial) and cloud (concurrent) groups.
        local_items: list[FeatureItem] = []
        cloud_items: list[FeatureItem] = []
        for item in layer:
            speaker = state.find(item.assigned_role) if item.assigned_role else None
            if speaker is None:
                # Will be handled as 'skipped' inside _run_single_item.
                cloud_items.append(item)
                continue
            effective_model = speaker.model_id or state.default_model_id
            if _is_local_model_hint(effective_model):
                local_items.append(item)
            else:
                cloud_items.append(item)

        # Run cloud items concurrently (S2).
        if cloud_items:
            sem = asyncio.Semaphore(concurrency) if concurrency > 0 else None
            queue: asyncio.Queue[StreamFrame | _ItemDone] = asyncio.Queue()

            async def _run_cloud_item(it: FeatureItem) -> None:
                async with (sem if sem else contextlib.nullcontext()):
                    async for frame in self._run_single_item(
                        item=it, conv=conv, request=request,
                        plan_ref=plan_ref, state=state, seq=seq,
                        handle=handle, budget_tracker=budget_tracker,
                        run_id=run_id,
                    ):
                        await queue.put(frame)
                    await queue.put(_ItemDone(item_id=it.id))

            tasks = [asyncio.create_task(_run_cloud_item(it)) for it in cloud_items]
            done_count = 0
            while done_count < len(cloud_items):
                ev = await queue.get()
                if isinstance(ev, _ItemDone):
                    done_count += 1
                else:
                    yield ev
            # Propagate any task exceptions.
            for t in tasks:
                if t.done() and t.exception():
                    raise t.exception()  # type: ignore[misc]

        # Run local items serially (S5: NPU items never concurrent).
    ...
    async def _run_single_item(
        ...
        *,
        ...
        """Execute a single plan item (extracted from the serial for-loop body).

        Reads and writes the mutable ``plan_ref[0]`` for single-point state merge.
        ``ImplementationRunBudgetExceeded`` and ``ChatStreamAbortedError`` propagate
        to the caller.
        """
        plan = plan_ref[0]
        # Re-read the live item off the (possibly re-persisted) plan so its
        # status reflects any prior item's update within this run.
        item = next((it for it in plan.items if it.id == item.id), item)
        if item.status != "pending":
            return
        # 三期-step1: a dependency in a *blocking-incomplete* state blocks
        # this item — mark it ``failed`` (its prerequisite is unmet) rather
        # than running it against a missing/broken foundation.
        ...
        # ② per-item: recompute the effective tool set (§22.5#1).
        config = speaker.config or {}
        role_tools = {
            t
        ...
        try:
            ...
            # §22.5 :1349 — a run-level budget hit pauses the run.
            _log.info(
            ...
            # Re-raise so the outer loop / layer runner can stop.
            raise

        # ③ done/failed: layered gates (§22.4).
    ...
    async def _run_full_path(
        ...
        *,
        ...
        for round_index in range(1, effective_rounds + 1):
            ...
            state.last_speaker_id = speaker.id.value
            # Same rule as _run_scoped_path: the coordinator (主持人) sentinel
            # is not a "substantive speaker" for last_active_speaker purposes.
            # It only lands in `_run_full_path` when the caller scoped
            # `state.participants` down to only the sentinel (@coordinator
            # directed path); a real roster speaker chosen by the selector is
            # unaffected.
            if speaker.id.value != _COORDINATOR_PARTICIPANT_ID:
                outcome.last_active_speaker = speaker.id.value
    ...
    async def _run_speaker_turn(
        ...
        *,
        ...
        # 现象2: a coordinator (主持人) turn = the judge turn run by the synthetic
        # coordinator identity → mark the frame so the front-end renders it as a
        # distinct host/总结 bubble (never a borrowed role name). A legacy roster
        # judge (``judge_participant_id`` back-compat) is NOT the coordinator, so
        # its frame keeps ``is_coordinator=False``.
        is_coordinator = is_judge and sender_id == _COORDINATOR_PARTICIPANT_ID
        ...
        * **Live exec partials**: when ``self._tools`` exposes the optional
        ...
        def _truncate(
            ...
            *,
            ...
            ``raw_result`` is the STRUCTURED tool envelope when one is available,
            used only to derive the ordered-slice continuation coordinate so a
            tail-cut ``read`` slice points forward (see
            :func:`tool_result_slice_start`).
            """
            ...
            try:
                tr = truncator.truncate(
                    ToolResultTruncationRequest(
                        ...
                        tool_name=tool_name,
                        result_text=result_text,
                        slice_start_line=_tool_result_slice_start(raw_result),
                    )
                )
        ...
        async def _run_one(name: str, args: dict[str, Any], call_id: str) -> Any:
            """Execute ONE call, returning ``(result_text, ok, raw)`` or raising.
            ...
            """
            if _tools is None:
                # 3-tuple like every other return: ``_shape_final`` unpacks
                # ``(result_text, ok, raw_envelope)``. No envelope exists on
                # this path, so pass ``None`` (which ``tool_result_slice_start``
                # maps to offset 1).
                return ("[tool_error] no tool executor wired", False, None)
            ...
            if callable(_stream_fn):
                ...
                if _agen is not None:
                    ...
                    return (str(result_raw), ok, result_raw)
        ...
        def _shape_final(slot: SlotResult) -> ToolExecutionItem:
            ...
            # ``run_one`` returns ``(result_text, ok, raw_envelope)``.
            result_text, ok, raw_envelope = slot.raw
            result_text, truncated, original_length = _truncate(
                result_text,
                tool_name=slot.tool_name,
                ok=ok,
                raw_result=raw_envelope,
            )
            return ToolExecutionItem(
                call_id=slot.call_id,
                tool_name=slot.tool_name,
        ...
        *,
        ...
        for msg in conv.messages:
            ...
            if msg.role is MessageRole.USER:
                ...
                # 现象1: if this USER message carries the round-budget meta (stamped
                # on a FULL-discussion opener), append the FIXED hint to its WIRE
                # content only (the persisted text is untouched). The hint text is
                # stable across rounds (no current-round number) → the replayed
                # opening message is byte-for-byte identical every round, so the
                # prompt-cache prefix stays warm.
                user_content = content_text
                budget = (
                    msg.meta.get(_DISCUSSION_ROUND_BUDGET_KEY)
                    if isinstance(msg.meta, dict)
                    else None
                )
                if isinstance(budget, int) and budget >= 1:
                    user_content = f"{content_text}\n\n{_round_budget_hint(budget)}"
                wire.append({"role": "user", "content": user_content})
    ...
    def _build_persona(
        ...
        * ``social_mode`` / ``followup_mode`` / ``wrapup_mode`` → the
        ...
        if is_judge:
            # Built-in judge directive is the single source of truth. For the
            # coordinator (主持人) turn, append the advisory ``coordinator_prompt``
            # (现象2 + feature) — NOT a roster persona (the synthetic coordinator
            # already has ``persona=None``, so ``base`` is just the framing).
            judge_directive = _DEFAULT_JUDGE_DIRECTIVE
            if speaker.id.value == _COORDINATOR_PARTICIPANT_ID:
                # DISC-1 coordinator-plan-rework: the "生成实施方案" button sends a
                # preset ``@主持人 …`` message carrying
                # :data:`_PLAN_GENERATION_MARKER`.  When that marker is present
                # in the latest user text the coordinator's directive is the
                # dedicated plan-generation framing — write the doc + call
                # ``implementation_plan`` + summarise — INSTEAD OF the ordinary
                # summariser directive; ``coordinator_prompt`` is still appended
                # as advisory extra guidance so a user override composes.
                latest_user = self._latest_user_text(conv) or ""
                if _PLAN_GENERATION_MARKER in latest_user:
                    judge_directive = _COORDINATOR_PLAN_GENERATION_FRAMING
                discussion = conv.discussion or {}
                extra = discussion.get("coordinator_prompt")
                if isinstance(extra, str) and extra.strip():
                    judge_directive = f"{judge_directive}\n\n{extra.strip()}"
    ...
    async def _persist_user_message(
        ...
        *,
        ...
        ``round_budget`` (现象1): when a positive int, stamp it onto the message
        ``meta[_DISCUSSION_ROUND_BUDGET_KEY]`` so ``_build_speaker_wire`` can
        append the FIXED round-budget hint when it replays this opening message
        on a FULL discussion. The message ``content`` stays the user's ORIGINAL
        text (the hint is a wire-only augmentation — never persisted into the
        visible text). ``None`` (scoped / lightweight / social / single-role
        turns) persists with no meta → byte-for-byte the legacy shape.
        """
        meta = (
            {_DISCUSSION_ROUND_BUDGET_KEY: int(round_budget)}
            if isinstance(round_budget, int) and round_budget >= 1
            else None
        )
        msg = Message(
...
class _ItemDone(NamedTuple):
    """Sentinel queued by ``_run_layer_concurrently`` when one cloud item finishes."""
