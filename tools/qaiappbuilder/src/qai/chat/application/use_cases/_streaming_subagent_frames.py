...
def accumulate_sub_agent_block(
    ...
    if ftype is StreamFrameType.SUBAGENT_START:
        ...
        # Initial lifecycle state comes from the frame: an async spawn reports
        # ``status="background"`` (handle returned at t=0, sub-agent still
        # running detached, result arrives later), and the block must keep
        # that distinction so a reloaded transcript renders the same card the
        # live stream did. Any other value — including a legacy frame with no
        # ``status`` at all — folds to ``"running"``.
        _raw_status = payload.get("status")
        initial_status = (
            "background" if _raw_status == "background" else "running"
        )
    ...
    if ftype is StreamFrameType.SUBAGENT_OUTPUT:
        ...
        # every ``subagent_done`` frame that reaches this accumulator
        # represents a GENUINE terminal completion of the sub-agent (there
        # is no synth-done path anymore — see stream_frame.subagent_done
        # docstring).  Always flip the block to ``done`` for that reason.
        block["status"] = "done"
        ...
        # §N.54 typed-failure tail: a FAILED run also terminates through
        # ``subagent_done`` (one terminal frame type per run under the async
        # contract), carrying the classification trio. When present the block
        # is an ERROR block — flip the status so a reloaded transcript renders
        # the same typed error card the live stream did, and carry the trio so
        # the front-end can pick its icon / colour / recovery hint without
        # re-deriving anything.
        _err_kind = payload.get("error_kind")
        if isinstance(_err_kind, str) and _err_kind:
            block["status"] = "error"
            block["error_kind"] = _err_kind
            _err_msg = payload.get("error_message")
            if isinstance(_err_msg, str) and _err_msg:
                block["error_message"] = _err_msg
                # ``error`` is the historical single-string field the card
                # already renders; keep it in step so the plain message path
                # never goes blank on a typed failure.
                block["error"] = _err_msg
            _hint = payload.get("recovery_hint")
            if isinstance(_hint, str) and _hint:
                block["recovery_hint"] = _hint
...
def build_assistant_meta(
    *,
    ...
    integrated_notice_ids: Iterable[str] | None = None,
    headless_followup: bool = False,
) -> dict[str, Any] | None:
    ...
    * ``integrated_notice_ids`` — §N.53: the SYSTEM_NOTICE dedup_keys this
      turn folded into its OWN wire (Batch 7 turn-internal integration).
      Written SORTED so the persisted value is stable across runs (a set's
      iteration order is not), and OMITTED when the turn integrated nothing —
      the front-end reads "absent ⇒ no chip".  This is the only durable trace
      that an answer digested background notices: the notices themselves are
      wire-fold-only rows and never render as bubbles.
    * ``headless_followup`` — §N.41: marks this assistant message as the
      product of a HEADLESS follow-up turn (the coordinator-driven turn that
      exists solely to integrate background SYSTEM_NOTICE rows).  Written
      ONLY when true, so "absent ⇒ ordinary user-visible turn".  This is the
      durable watermark
      :class:`~qai.chat.adapters.pending_state_rebuilder.PendingStateRebuilder`
      reads at startup: every SYSTEM_NOTICE at a ``position`` ABOVE the last
      such message is provably un-integrated and must be pushed back into the
      coordinator's pending map (the in-memory map dies with the process).
    ...
    # §N.53 — SORTED so the persisted list is stable (the source is a set);
    # omitted entirely when empty so "absent" unambiguously means "this answer
    # integrated no background notices" on the front-end.
    if integrated_notice_ids is not None:
        _notice_ids = sorted(set(integrated_notice_ids))
        if _notice_ids:
            meta["integrated_notice_ids"] = _notice_ids

    # §N.41 — written ONLY when true so an ordinary turn's meta is unchanged
    # byte-for-byte, and "absent" unambiguously means "not a headless turn".
    # Lands in the SAME dict as ``integrated_notice_ids`` above: the two keys
    # are independent and neither replaces the other's envelope.
    if headless_followup:
        meta["headless_followup"] = True
