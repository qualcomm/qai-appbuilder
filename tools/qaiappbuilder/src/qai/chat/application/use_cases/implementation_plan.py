...
def topological_item_layers(
    items: tuple[FeatureItem, ...] | list[FeatureItem],
) -> tuple[tuple[tuple[FeatureItem, ...], ...], bool]:
    """Partition items into dependency layers (Kahn layering).

    Returns ``(layers, topo_ok)`` where each layer is a tuple of items whose
    dependencies are ALL satisfied by earlier layers.  Items within a layer
    have no mutual dependency and MAY run concurrently.

    * Empty input → ``((), True)``.
    * No ``depends_on`` anywhere → single layer containing all items (max concurrency).
    * Cycle detected → each item gets its own layer (= serial fallback), ``topo_ok=False``.
    * Items with a blank ``id`` are appended as a final single-item layer.
    """
    seq = list(items)
    if not seq:
        return (), True
    ids = {it.id for it in seq if it.id}
    index = {it.id: i for i, it in enumerate(seq) if it.id}
    # Build in-degree + reverse adjacency (same edge rules as topological_item_order).
    deps: dict[str, list[str]] = {
        it.id: [d for d in it.depends_on if d in ids and d != it.id]
        for it in seq
        if it.id
    ...
    by_id = {it.id: it for it in seq if it.id}
    layers: list[tuple[FeatureItem, ...]] = []
    scheduled = 0

    while True:
        # Collect all nodes with in-degree 0, sorted by original order.
        ready = sorted(
        ...
        if not ready:
            break
        layer = tuple(by_id[n] for n in ready)
        layers.append(layer)
        scheduled += len(ready)
        for n in ready:
            del indegree[n]
            for dep in dependants[n]:
                if dep in indegree:
                    indegree[dep] -= 1

    # Blank-id items as a trailing layer.
    blank_items = tuple(it for it in seq if not it.id)
    if blank_items:
        layers.append(blank_items)
        scheduled += len(blank_items)

    total = len([it for it in seq if it.id]) + len(blank_items)
    if scheduled < total:
        # Cycle: degrade to one-item-per-layer (= serial).
        return tuple((it,) for it in seq), False
