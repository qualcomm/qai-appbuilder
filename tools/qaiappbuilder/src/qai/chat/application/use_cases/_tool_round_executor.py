...
async def execute_tools_in_parallel_stream(
    *,
    ...
    # M5+M6 full alignment: if provided, per-tool interruptibility is
    # resolved via ``interruptible_resolver(tool_name, args)``.  True →
    # additionally race the events in ``interruptible_events`` (typically
    # the handle's ``steer_hard_event`` + ``irc_hard_event``); firing any
    # of them yields the same ``InterruptedError`` shape as ``abort_event``.
    # False (default) → only the classic ``abort_event`` is raced —
    # non-interruptible tools are safe from mid-turn hard-cancel.
    interruptible_resolver: Callable[[str, dict[str, Any]], bool] | None = None,
    interruptible_events: tuple[asyncio.Event, ...] = (),
    ...
    async def _run_body(slot: int, name: str, args: dict[str, Any]) -> tuple[Any, bool]:
        ...
        try:
            # Fast path: no abort event AND no per-call cancel check AND no
            # interruptible events → run the call directly (zero racing
            # overhead, byte-identical to the old non-abort path).
            _interruptible = False
            if interruptible_resolver is not None:
                try:
                    _interruptible = bool(interruptible_resolver(name, args))
                except Exception:  # noqa: BLE001 — safest default
                    _interruptible = False
            _extra_events: tuple[asyncio.Event, ...] = (
                interruptible_events if _interruptible else ()
            )
            if (
                abort_event is None
                and cancel_check is None
                and not _extra_events
            ):
            ...
            # NEW (M5+M6): interruptible-event waiters — one asyncio.Task per
            # event so we can identify which one fired.
            interruptible_tasks: list[asyncio.Task[Any]] = [
                asyncio.ensure_future(ev.wait()) for ev in _extra_events
            ]
            watch: set[asyncio.Future[Any]] = {exec_task}
            if abort_task is not None:
                watch.add(abort_task)
            for it in interruptible_tasks:
                watch.add(it)
            ...
            try:
                while True:
                    ...
                    if abort_task is not None and abort_task in done:
                        outcome_kind = "abort"
                        break
                    # M5+M6: any interruptible-event won → treat like "abort"
                    # (returns InterruptedError to the caller).  Only reached
                    # for tools that ``interruptible_resolver`` marked as
                    # interruptible; non-interruptible tools have empty
                    # ``interruptible_tasks``.
                    _interruptible_fired = next(
                        (it for it in interruptible_tasks if it in done),
                        None,
                    )
                    if _interruptible_fired is not None:
                        outcome_kind = "abort"
                        break
                ...
                for t in (exec_task, abort_task, *interruptible_tasks):
