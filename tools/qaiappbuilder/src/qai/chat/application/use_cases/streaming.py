...
from qai.chat.application.use_cases._agentic_kernel import (
    ...
    overflow_recovery_target_ratio as _overflow_recovery_target_ratio,
    tool_result_already_truncated as _kernel_tool_already_truncated,
    tool_result_slice_start as _kernel_tool_slice_start,
...
from qai.chat.application._token_estimate_helpers import (
    ...
    authoritative_provider_size as _authoritative_provider_size,
    compute_overhead_from_last_wire_measurement as _compute_overhead_from_last_wire_measurement,
...
from qai.chat.application.use_cases._streaming_helpers import (
    ...
    SYSTEM_NOTICE_WIRE_PREFIX as _SYSTEM_NOTICE_WIRE_PREFIX,
    TURN_WARNING_STEP as _TURN_WARNING_STEP,
    assemble_multimodal_messages as _assemble_multimodal_messages_fn,
    split_atomic_groups as _split_atomic_groups_shared,
    compute_turn_prefix_at_boundary as _compute_turn_prefix_at_boundary_shared,
...
from qai.chat.application.use_cases._long_term_memory import (
    ...
    resolve_persona_memory as _resolve_persona_memory_fn,
)
...
from qai.platform.i18n import t as _t
...
# kwarg ``max_followup_rounds`` explicitly.
#   -1 = UNLIMITED (no artificial cap; production default)
#    0 = disabled (single-turn, no followup loop)
#   >0 = hard cap at that many rounds (user-configurable safety net)
DEFAULT_MAX_FOLLOWUP_ROUNDS: int = -1
...
# persisted new SYSTEM_NOTICE row(s) for this tab, the loop reopens ONE more
# round so the model folds them into the SAME turn — instead of closing and
# letting the coordinator fire a separate headless follow-up per notice (the
# reported "同一批 sub-agent 完成触发 3-4 次连续 headless followup" bug). This
# caps how many such consecutive re-opens one turn may run.
#
# THREE distinct ceilings guard the notice-integration path; they are NOT
# redundant and MUST NOT be merged — each catches a different runaway mode:
#   * ``_MAX_PENDING_INTEGRATION`` (here) — turn-internal integration retries:
#     bounds "finished → new notice landed → reopen a round" WITHIN one live
#     user-visible turn.
#   * ``agent_session_coordinator._MAX_ITER`` — external headless drain:
#     bounds iterations inside ONE ``_run_one`` call (runner that consumes and
#     re-adds keys forever).
#   * ``agent_session_coordinator._DRAIN_WINDOW_SECONDS`` / ``_DRAIN_WINDOW_MAX``
#     — cross-call re-entry rate: bounds how often a FRESH drain task may start
#     for a tab (steady trickle of notices, each call individually legal).
# Each constant lives in the layer that consumes it — ``application`` must not
# import ``adapters`` (``.importlinter`` contract ``layered-chat``).
_MAX_PENDING_INTEGRATION: int = 3

# How long the turn-internal pending integration waits for a producer's
# SYSTEM_NOTICE write to become readable before folding it into the wire.
# Reopening a round for a row that is not on disk yet would send a wire
# WITHOUT the notice and then ack it anyway (the coordinator's ack surface is
# the union of pre-run pending and observed keys) — the notice would be lost.
# A producer that persists inline settles instantly, so this budget only
# matters once writes are queued.
_PENDING_SETTLE_TIMEOUT_MS: int = 2000
...
def _split_atomic_groups_local(
    messages: list[dict[str, Any]],
) -> list[list[dict[str, Any]]]:
    """Split a flat wire list into atomic trim units (P3 §7.1 helper).

    Thin delegate to
    :func:`qai.chat.application.use_cases._streaming_helpers.split_atomic_groups`
    — the SINGLE implementation, shared with the force-compact path. Kept
    under this name because ``test_turn_prefix_atomic_boundary`` cross-checks
    it against the adapters-layer implementation to catch drift.
    """
    return _split_atomic_groups_shared(messages)
...
#     ``_build_assistant_meta``) now live in
...
#: convention (same pattern as ``workspace_context_files`` / ``_session_workspace_root``):
...
def _agent_tool_schema() -> dict[str, Any]:
    ...
    read/glob/grep/web_fetch, no write/edit/exec). Omitting it (or passing
    ...
    return {
        ...
        "function": {
            ...
            "description": (
                ...
                "Returns immediately with a handle naming the sub-agent id; "
                "the sub-agent then works on its own.\n"
                "Its result is delivered to you AUTOMATICALLY when it "
                "finishes — you never need to poll for it, and you do not "
                "have to keep the conversation waiting on it. Use "
                "sub_agent(action=wait, id=<sid>) ONLY when you are "
                "genuinely blocked with nothing else to do; otherwise "
                "finish your turn and the result will reach you.\n"
                "If a new user message reaches you BEFORE that result, the "
                "sub-agent has NOT failed or been cancelled — it is still "
                "running. Do not re-spawn it and do not tell the user it "
                "failed; handle the new message and its result will still "
                "arrive.\n"
                "When several tasks finish together their results arrive in "
                "one message, each under its own id header — attribute each "
                "result by that id, never by position."
            ),
            "parameters": {
                "type": "object",
                "required": ["prompt"],
                "properties": {
                    ...
                    "subagent_type": {
                        "description": (
                            ...
                            "READ-ONLY search (read/glob/grep/web_fetch only)."
                        ),
                        "type": "string",
...
class AsyncioStreamAbortHandle:
    ...
    def __init__(self) -> None:
        ...
        # NEW (M5): cooperative "soft steer" signal.  Independent of every
        # other event; set by the injection registry (or any producer of
        # mid-turn user input) so long-running side-effect tools like
        # ``exec`` can voluntarily hand off to the background-process
        # manager instead of blocking until they finish.  Never trips
        # :meth:`is_set` — the turn continues.
        self._soft_steer = asyncio.Event()
        # NEW (M5+M6 full alignment): hard-cancel events for *interruptible*
        # tools (pure waits: question / vibe_wait / hub wait).  These get
        # ``AbortSignal.any([external, _steer_hard, _irc_hard])``-style
        # composition in the executor; a non-interruptible tool (exec /
        # write / edit) NEVER observes these events — it only gets the
        # cooperative :attr:`soft_steer_event`.  Independent of every
        # other signal; setting either DOES NOT trip :meth:`is_set`.
        self._steer_hard = asyncio.Event()
        self._irc_hard = asyncio.Event()
    ...
    def abort_event(self) -> asyncio.Event:
        """Expose the raw abort event so a tool wait loop can race on it.

        Read-only for consumers: they ``.wait()`` on it and NEVER set it —
        :meth:`signal` is the single write point.  Used by the exec 4-way
        race (:mod:`qai.platform.exec_race`) so a non-interruptible tool
        can shape a proper ``[Command aborted by user]`` result instead of
        relying solely on task cancellation.
        """
        return self._event
    ...
    def notify_soft_steer(self) -> None:
        """Signal that new user input has arrived mid-turn.

        Idempotent (a second call after the event is set is a no-op).  Does
        NOT abort the turn; long-running side-effect tools (e.g. ``exec``)
        may observe :attr:`soft_steer_event` and voluntarily background
        themselves so the fresh input can be picked up promptly at the
        next inter-round seam.
        """
        if not self._soft_steer.is_set():
            self._soft_steer.set()

    def clear_soft_steer(self) -> None:
        """Clear the soft-steer flag (called at the inter-round seam AFTER
        the injection registry has been drained into the wire, so the next
        tool batch starts fresh)."""
        self._soft_steer.clear()

    @property
    def soft_steer_event(self) -> asyncio.Event:
        """Expose the raw event so tool wait loops can race on it."""
        return self._soft_steer

    def notify_steer_hard(self) -> None:
        """Hard-signal that queued steering messages need immediate handling.

        Set by the kernel's mid-batch peek (~250ms) when it observes pending
        entries in the injection registry AND the executor should immediately
        abort any interruptible tool wait so the message can inject at the
        next boundary.  Idempotent.  Independent of :meth:`signal` — the
        turn continues.
        """
        if not self._steer_hard.is_set():
            self._steer_hard.set()

    def clear_steer_hard(self) -> None:
        """Clear the hard-steer flag (called at the boundary AFTER drain)."""
        self._steer_hard.clear()

    @property
    def steer_hard_event(self) -> asyncio.Event:
        return self._steer_hard

    def notify_irc_hard(self) -> None:
        """Hard-signal that a peer / IRC message needs immediate handling.

        Reserved for future peer-agent messaging (v1 has no producer so
        this is effectively dormant).  Same semantics as
        :meth:`notify_steer_hard`; the executor composes both into a single
        interruptible-signal via :attr:`interruptible_wait_events`.
        """
        if not self._irc_hard.is_set():
            self._irc_hard.set()

    def clear_irc_hard(self) -> None:
        self._irc_hard.clear()

    @property
    def irc_hard_event(self) -> asyncio.Event:
        return self._irc_hard

    def interruptible_wait_events(self) -> tuple[asyncio.Event, ...]:
        """Return the tuple of events an interruptible tool should race on.

        Mirrors the reference implementation's
        ``AbortSignal.any([external, steering, irc])`` composition:
        an interruptible tool's wait resolves the instant ANY of these
        events fires — external abort, hard steer, or hard IRC.

        Non-interruptible tools do NOT race these events (see
        :attr:`soft_steer_event` for their cooperative signal).
        """
        return (self._event, self._steer_hard, self._irc_hard)
...
class StreamChatInput:
    ...
    #: Optional metadata to attach to the persisted user Message.meta.  When
    #: the caller is the park auto-wakeup path this carries
    #: ``{"aside": True, "asideKind": "subagent_completion"|"bg_completion"}``
    #: so the frontend renders the injected user message as a system-note
    #: card (``.msg-aside`` CSS) rather than a user bubble.  ``None`` (the
    #: default) keeps the pre-2026-08 behaviour byte-for-byte — a real user
    #: turn shows as a user bubble.
    user_message_meta: dict[str, Any] | None = None
    #: Headless follow-up turn — ``True`` when the caller is
    #: :class:`AgentSessionCoordinator._run_one`, i.e. a background-job
    #: completion just landed on an idle tab and we're spinning one more
    #: agent round to observe the SYSTEM_NOTICE that dispatcher already
    #: appended.  ``user_message`` carries a placeholder (never
    #: persisted) so the downstream helpers that read
    #: ``request.user_message.text`` have a valid object; the
    #: :meth:`StreamChatUseCase._prepare_turn` skips the user-message
    #: append + publish (the notice is already in the conversation).
    #: All other turn setup (tab lock, abort registration, system
    #: prompt, soft-steer ctx) runs identically to a normal turn.
    headless: bool = False
...
class _TurnBodyState:
    ...
    #: Injections the teardown ``finally`` swept out of the registry before the
    #: normal-completion tail could drain them (bug fixed 2026-08-10).
    #:
    #: The ``finally`` that clears the tab's pending-injection registry is
    #: INSIDE the ``try`` wrapping the whole stream drain, while
    #: :meth:`_finalize_turn` — which owns the ``inject_as_new_turn`` deferral —
    #: runs AFTER that ``try`` exits. So ``clear`` always beat ``drain``, the
    #: deferral frame could never fire, and any injection that arrived in a turn
    #: with no further tool round (a headless sub-agent follow-up, or a plain
    #: text reply) was silently destroyed: the grey bubble hung forever and the
    #: user's text never reached the model.
    #:
    #: The ``finally`` cannot emit the frame itself (it runs under
    #: ``GeneratorExit`` where awaiting is illegal), so it hands what it swept
    #: over here instead of dropping it, and ``_finalize_turn`` emits from this
    #: list. Empty on every turn that folded its injections on time.
    swept_injections: list[str] = field(default_factory=list)
    ...
    #: Every cache-read signal observed this TURN, one entry per MEASURED
    #: round (see ``_observe_cache_round``). The prompt-cache capability
    #: verdict is fed to the registry ONCE per turn, from the turn's MAXIMUM
    #: (``_flush_cache_capability``).
    #:
    #: WHY PER-TURN AND NOT PER-ROUND (the bug this fixes): the registry
    #: converts a model to ``no_cache`` — a one-way absorbing state that pins
    #: aging ON for the rest of the process — after 2 consecutive zero-read
    #: observations. Its docstring states the intent as "the first round of a
    #: fresh model is a pure cache-write establishing round, so wait for a
    #: SECOND confirming round", i.e. a second TURN. But marking per round let
    #: ONE multi-round turn spend both credits: in the field a model's very
    #: FIRST turn (round 1 zero-read, then round 0 zero-read) condemned it
    #: before it had any opportunity to read a cache back. That model then hit
    #: 99.98% cache on later turns, all ignored via ``sticky_hit``, while
    #: aging needlessly broke the byte-stable prefix. Aggregating per turn and
    #: taking the MAX means a turn that read cache back on ANY round counts as
    #: a hit, and a fully zero-read turn spends exactly one credit.
    #:
    #: Taking the max loses nothing: the registry's own signal is "did a real
    #: read land", and one landed read anywhere in the turn proves support.
    #:
    #: This also makes the former ``cache_capability_marked_round0`` guard
    #: obsolete — with a single flush site per turn, double-counting is
    #: structurally impossible rather than defended against, and the
    #: accumulator is idempotent under repeated observation of the same round.
    #: §concurrent-writer refactor 2026-08-07 — deterministic round
    #: message ids across incremental persists.  Root cause it fixes:
    #: ``_persist_completed_rounds`` truncates ``conv.messages[baseline:]``
    #: then rebuilds via :func:`build_round_messages`, which mints a
    #: fresh :class:`MessageId` for every round on EVERY call.  The
    #: pre-refactor ``save_messages`` did ``DELETE all + INSERT all``,
    #: so stale ids were swept on each save — invisible.  Post-refactor
    #: ``save_messages`` is non-destructive upsert-by-id, so new ids on
    #: every call ACCUMULATE in the DB (K incremental persists → K× the
    #: same tool-call round rows, all pointing at the same
    #: ``tc_frame.frame_id`` inside).  Cache the round-index → message-id
    #: mapping here so subsequent rebuilds reuse the same message id and
    #: the upsert lands on the SAME row instead of stacking duplicates.
    #: Populated lazily by :func:`build_round_messages` (via
    #: ``ids_by_round`` threaded through :meth:`_build_tool_call_message`).
    round_message_ids: dict[int, str] = field(default_factory=dict)
    #: §concurrent-writer refactor 2026-08-07 — same rationale as
    #: ``round_message_ids`` but for the ``subagent_summary`` per-round
    #: message :meth:`_build_subagent_summary_message` produces.  Keyed
    #: by parent-round index → cached message id value; the rebuild
    #: reuses the id so upsert-by-id lands on the same DB row.
    subagent_summary_message_ids: dict[int, str] = field(default_factory=dict)
    cache_read_signals: list[int] = field(default_factory=list)
    #: Recursion depth of the agent whose turn this is: ``0`` for the main
    #: agent (a main-agent turn is by definition the ROOT of the sub-agent
    #: tree), and ``SubAgentSession.depth`` for a turn running INSIDE a
    #: sub-agent.  Read by the sub-agent spawn dispatch to tell the spawn
    #: bridge how deep the child would sit, so preflight can reject a spawn
    #: that would exceed the recursion cap.  Deliberately NOT sourced from
    #: the request / settings on the main-agent path — see
    #: :meth:`StreamChatUseCase._run` where it is hardcoded to ``0``.
    parent_depth: int = 0
    #: §N.53 — dedup_keys of the SYSTEM_NOTICE rows this turn INTEGRATED into
    #: its own wire (Batch 7's ``_integrate_pending_notices`` fold).  Written
    #: by that hook as it acks each key; serialised onto the finalised
    #: assistant message's ``meta["integrated_notice_ids"]`` (sorted) so the
    #: front-end can render a "📥 integrated N background notices" chip above
    #: the answer, and the user can see WHICH notices this answer folded in.
    #:
    #: Without this the integration is invisible on reload: the notices were
    #: never persisted as bubbles (they are wire-fold-only rows), so an
    #: integrated answer looked identical to one that simply ignored them.
    #:
    #: A ``set`` because the same key can be observed by more than one
    #: integration attempt within a turn and the chip must count each notice
    #: once.  Empty for a turn that integrated nothing → the key is omitted
    #: from ``meta`` entirely (no migration needed: ``meta_json`` is a TEXT
    #: JSON blob, and absent means "no chip").
    pending_integration_keys_this_turn: set[str] = field(default_factory=set)
...
class StreamChatUseCase:
    ...
    * ``retry_policy`` — when supplied, the use case looks at the
      ...
      to decide whether to retry (with backoff for throttling; PROMPT_TOO_LONG
      is terminal — the error is forwarded to the caller without retry).  See
    ...
    * ``max_followup_rounds`` — upper bound on follow-up LLM rounds
    ...
    def __init__(
        ...
        *,
        ...
        # through ``sub_agent_bridge`` (below), and the bridge is what pulls
        # this port's event stream in the background.  Retained here because
        # the DI layer reads it back off the use case
        # (``apps/api/di.py`` — workspace-root rebinding of the handler's
        # inner tool executor).
        ...
        # ---- Per-turn max-rounds reader (user-configurable safety net) ----
        # When wired, called once per turn to read the user's optional round
        # cap from forge_config (``agent.max_rounds``). Returns a positive int
        # to cap, or ``0``/``None`` for "no user cap" (use the constructor's
        # ``max_followup_rounds`` default). Allows runtime changes without
        # restart. ``None`` disables the reader — the constructor value is
        # always used.
        max_rounds_provider: (
            Callable[[], Awaitable[int | None]] | None
        ) = None,
        sleep: Callable[[float], Any] | None = None,
        # ---- Per-turn turn-warning settings reader ----
        # When wired, called per turn to read user's turn-warning prefs from
        # forge_config (``turn_warning.{enabled, start, step}``). Returns a
        # dict with optional keys. ``None`` → use module defaults.
        turn_warning_provider: (
        ...
        # ---- Desktop-control (``computer``) per-turn enable gate ----
        # Optional zero-arg SYNC callable returning the live ``computer.enabled``
        # flag (forge_config override > ``settings.computer.enabled``). The
        # ``computer`` tool is ALWAYS registered on the chat registry; this gate
        # decides per turn whether it is advertised, so flipping the Settings UI
        # toggle takes effect on the next turn WITHOUT a restart (mirrors the
        # ``chat_hooks_enabled`` re-read). ``None`` → treat as disabled (secure
        # default): ``computer`` is dropped from every turn's tool set.
        computer_enabled_provider: Callable[[], bool] | None = None,
        # ---- Cloud image encode config (format/quality) ----
        # Optional zero-arg SYNC callable returning ``(format, quality)`` for
        # the copy of a screenshot / uploaded image SENT TO THE CLOUD MODEL
        # (forge_config chat.image_cloud_format / image_cloud_quality, surfaced
        # in the Agent settings panel). Threaded into the vision-block resolver
        # so a ``computer`` screenshot / ``question`` image the model sees honors
        # the user's chosen format+quality. ``None`` → the resolver's default
        # (webp / 50). The on-disk original is never affected.
        cloud_image_config_provider: (
        ...
        # ---- Exec 4-way race injection signal (Batch 5) ----
        # Optional bridge that turns the injection registry's inject events
        # into a per-tab ``asyncio.Event``.  A running ``exec`` races that
        # event as its 4th arm, so a Ctrl+Enter injection hands the command
        # to the background-process manager IMMEDIATELY instead of waiting
        # out the auto-background threshold.  Only a TAB-BOUND ``subscribe``
        # closure is published (via the soft-steer context), never the
        # bridge itself — the consuming tool lives in another bounded
        # context and must not import a ``qai.chat`` symbol.  ``None``
        # disables that race arm; Stop / threshold / completion still race.
        injection_signal_bridge: "Any | None" = None,
        # NEW (M5): shared background-process manager.  Currently only
        # published to the per-turn soft-steer context (see
        # :mod:`qai.platform.turn_soft_steer_ctx`) so tools like ``exec`` can
        # discover it without a signature change.  ``None`` = feature off.
        background_process_manager: "Any | None" = None,
        # M5: foreground-wait threshold.  Accepts either a static value or an
        # async reader (``Callable[[], Awaitable[int | None]]``) so the
        # forge_config override (Settings UI) can take effect per turn
        # WITHOUT a restart.  ``None`` means auto-background is off for this
        # turn — either the user switched it off or neither the forge_config
        # keys nor the ChatSettings defaults resolved.  Consumers then drop
        # the exec race's threshold arm (Stop and mid-turn injection keep
        # racing) rather than invent a phantom default.
        auto_bg_threshold_ms: int | Callable[[], Awaitable[int | None]] | None = None,
        ...
        # ---- Per-model prompt-cache-capability registry ----
        # Optional shared ``ProviderCacheCapabilityRegistry`` that learns at
        # runtime whether the routed MODEL reads prompt cache back (keyed on
        # model_hint). The per-round aging gate consults ``aging_enabled`` BEFORE
        # calling ``age_old_tool_outputs``: unknown (first rounds) / supports-cache
        # → aging OFF (keep the cached prefix byte-clean); no-cache → aging ON
        # (real byte savings). ``_on_round_end`` writes the round's actual
        # cache-read volume back via ``mark`` so the registry learns whether the
        # model reads cache back. ``None`` (legacy / unit stubs) disables the
        # gate → aging runs unconditionally, byte-for-byte the prior behaviour
        # (full backward compatibility). The SAME instance is shared with the
        # sub-agent handler (DI, ``_chat_di.py``).
        ...
        # ---- Best-effort JSONL journal for compaction events (§5.1) ----
        # Optional :class:`CompactionJournalPort` sink for one line per
        # successful compaction. Piped straight through to the underlying
        # :class:`CompactionCheckpointEngine`. ``None`` (default) disables
        # emission — byte-for-byte the prior behaviour.
        journal: "CompactionJournalPort | None" = None,
        # ---- Session digest refresh (P2 — CONTEXT-COMPRESSION-NEXT §6) ----
        # Optional :class:`RefreshDigestUseCase` invoked fire-and-forget after
        # every successful compaction. ``None`` (default) disables the
        # digest entirely — the wire-assembly path never sees
        # ``ckpt.digest_text`` populated and is byte-for-byte identical to
        # pre-P2 behaviour. Small windows (``context_window < 32_000``) also
        # skip the digest (gate lives on the engine's ``digest_enabled``).
        refresh_digest_uc: "RefreshDigestUseCase | None" = None,
        # ---- Long-turn prefix summariser (P3 — CONTEXT-COMPRESSION-NEXT §7.1) ----
        # Optional :class:`SummarizeTurnPrefixUseCase` invoked fire-and-forget
        # after a compaction whose dropped prefix contains a "long turn"
        # (rolling wire > ``0.6 × PROTECT_WINDOW_RATIO × context_limit``).
        # ``None`` (default) disables the summariser entirely — the wire-
        # assembly path never sees ``ckpt.turn_prefix_summaries_json``
        # populated and is byte-for-byte identical to pre-P3 behaviour. Small
        # windows also skip (gate lives on the engine's
        # ``turn_prefix_enabled``). Sub-agents intentionally do NOT wire this
        # in (no durable checkpoint to persist against).
        summarize_turn_prefix_uc: "SummarizeTurnPrefixUseCase | None" = None,
        agent_session_coordinator: "Any | None" = None,
        # ---- Unified sub-agent spawn bridge (async spawn contract) ----
        # When wired, an ``agent`` tool_call spawns through this bridge and
        # returns a :class:`SubAgentHandle` at t=0: the parent turn emits a
        # ``subagent_start(status="background")`` + a handle ``tool_result``
        # ack and moves on, instead of blocking the turn while it drains the
        # sub-agent's event stream.  The sub-agent's completion reaches the
        # LLM later as a SYSTEM_NOTICE (delivered by the async job manager),
        # and its live output reaches the UI through the sub-agent stream
        # broadcaster.  ``None`` disables ``agent`` dispatch entirely (the
        # spawn path has no synchronous fallback).
        sub_agent_bridge: "Any | None" = None,
        # ---- Per-conversation write serialiser (concurrent-write safety) ----
        # When wired, every parent-conversation ``save_messages`` this use
        # case performs goes through the queue instead of straight to the
        # repository, so the turn's incremental round saves, its terminal
        # save, and the background dispatcher's SYSTEM_NOTICE append are
        # serialised per conversation (and repeated saves of the same
        # aggregate collapse into one transaction).  Annotated ``Any`` and
        # injected — the application layer must not import the adapter that
        ...
        # BackgroundJob completion coordinator — used by
        # :meth:`_complete_turn` to fire pending headless follow-ups when
        # a live turn hands the tab back to IDLE (see
        # :meth:`AgentSessionCoordinator.on_tab_idle`).  ``None`` when
        # the use case is built without the §6.4 pipeline (unit stubs).
        self._agent_session_coordinator = agent_session_coordinator
        # Optional :class:`ChatStreamBroadcaster` — when wired
        # (production DI), :meth:`run_headless_followup` publishes
        # every frame to it so a frontend WebSocket subscribed to
        # ``/api/chat/active-runs/{tab}/ws`` can attach and stream the
        # headless follow-up live.  ``None`` (unit stubs) keeps
        # behaviour unchanged.  Late-bound from DI (see
        # ``_chat_di.py``) after both the use case and the coordinator
        # are constructed.
        self._chat_stream_broadcaster: Any = None
        self._conversation_write_queue = conversation_write_queue
        # PR-401c
        ...
        # The sub-agent runner adapter.  Not consumed by this use case (the
        # spawn bridge drains it); held so the DI layer can read it back —
        # see the constructor-parameter comment above.
        ...
        # Unified sub-agent spawn bridge: the ONLY way an ``agent`` tool_call
        # becomes a running sub-agent.  ``None`` ⇒ no spawn capability.
        self._sub_agent_bridge = sub_agent_bridge
        # ``max_followup_rounds`` semantics:
        #   -1 (or any negative) = UNLIMITED — no artificial round cap (default
        #      in production); the loop runs until the model converges or hits
        #      the no-progress circuit breaker.
        #    0 = DISABLED — single-turn mode, no followup loop at all.
        #   >0 = hard cap at that many rounds (user-configurable safety net).
        self._max_followup_rounds = int(max_followup_rounds)
        self._max_rounds_provider = max_rounds_provider
        self._turn_warning_provider = turn_warning_provider
        ...
        # Desktop-control per-turn enable gate (see constructor param). Called
        # synchronously in ``_collect_tool_schemas``; ``None`` → disabled.
        self._computer_enabled_provider = computer_enabled_provider
        # Cloud image encode config provider (see constructor param). Called
        # synchronously when resolving vision blocks; ``None`` → resolver
        # default (webp / 50).
        self._cloud_image_config_provider = cloud_image_config_provider
        ...
        # Exec 4-way race (Batch 5): source of the per-tab injection event.
        # ``None`` = not wired → the exec race skips its injection arm.
        self._injection_signal_bridge = injection_signal_bridge
        self._background_process_manager = background_process_manager
        self._auto_bg_threshold_ms = auto_bg_threshold_ms
        ...
        # Per-turn turn-warning settings (refreshed from provider each turn).
        self._turn_warning_settings: dict[str, Any] = {}
        ...
        # Shared per-model prompt-cache-capability registry (see __init__ arg
        # doc). ``None`` → aging gate falls back to unconditional aging (prior
        # behaviour). Read in ``_on_round_open`` (gate) + written in
        # ``_on_round_end`` (learn from the round's actual cache-read volume).
        ...
        # accumulation, no-progress breaker, exec
    ...
    async def run_headless_followup(
        self,
        *,
        tab_id: TabId,
        conversation_id: ConversationId,
        model_hint: str | None = None,
    ) -> tuple[str, dict[str, Any] | None, frozenset[str]]:
        """Run ONE headless follow-up turn on ``tab_id``.

        Called by :class:`AgentSessionCoordinator` when a background
        job completes on a tab that is currently idle (no user turn in
        flight) — the SYSTEM_NOTICE has just been appended by
        :class:`BackgroundJobDispatcher` and no live stream will
        otherwise observe it.  We spin one more agent round whose sole
        purpose is to let the main agent see the notice and react
        (call a tool, produce a summary, whatever).

        Wire is IDENTICAL to a normal turn — every SYSTEM_NOTICE
        sitting in the conversation at role ``system_notice`` is
        folded down to ``role:user + [System notification]`` by
        :func:`rebuild_history_wire_messages` (see §6.3), so the model
        sees the same shape it would on a user-triggered follow-up.
        The prompt phrasing (below) is N-tolerant so a single turn
        integrates 1, 2, or N accumulated background completions into
        one unified reply — the coordinator's drain loop then acks
        every observed key and DOES NOT trigger a redundant "just
        repeat what I said" follow-up.

        Returns ``(text, usage, observed_dedup_keys)``.  The third
        item is the set of ``SYSTEM_NOTICE_META_DEDUP_KEY`` values
        present on the conversation at turn-open (the exact set the
        LLM's wire folded in), used by
        :meth:`AgentSessionCoordinator._run_one` to ack the observed
        keys — regardless of whether they were in the pending map at
        drain-iteration start or landed mid-turn.

        Errors propagate — the caller
        (:class:`AgentSessionCoordinator._run_one`) logs and swallows
        them; observed_keys is meaningless on the exception path.
        """
        request = StreamChatInput(
            tab_id=tab_id,
            ...
            user_message=MessageContent(
                text=(
                    "One or more background tasks have completed. "
                    "Their results are the messages above marked "
                    "'[System notification]' — REVIEW EVERY pending "
                    "notification, INTEGRATE all outcomes into a single "
                    "unified response, and report back to the user. "
                    "Do NOT reply about only the most recent one; if "
                    "multiple notifications are present, summarise all "
                    "of them together as one coherent answer. "
                    "IMPORTANT — the '[System notification]' text is an "
                    "INTERNAL wire mechanism the USER CANNOT SEE. Present "
                    "the sub-agent results as YOUR OWN direct answer to "
                    "the user's task. NEVER tell the user to 'check the "
                    "system notification', 'see the notification above', "
                    "or any similar phrasing — quote/summarise the "
        ...
        # Register a ChatStreamBroadcaster entry for this tab so the
        # frontend can attach via ``/api/chat/active-runs/{tab}/ws``
        # and see the headless follow-up frames live.  Without this,
        # frames are drained locally into ``collect_completion``'s
        # ``parts`` and NEVER reach the UI — the user only sees the
        # persisted assistant message on a subsequent history reload
        # (the "close and reopen tab" workaround).  Best-effort: a
        # None broadcaster (unit stubs) keeps the pre-refactor
        # behaviour byte-for-byte; a busy tab (broadcaster already
        # has a non-terminal entry) also skips — the primary WS is
        # doing its own fan-out.
        broadcaster = self._chat_stream_broadcaster
        broadcaster_registered = False
        if broadcaster is None:
            _log.warning(
                "chat.headless.broadcaster_unwired",
                tab_id=tab_id.value,
                conversation_id=conversation_id.value,
            )
        else:
            try:
                entry = broadcaster.register(
                    tab_id=tab_id,
                    conversation_id=conversation_id,
                    model_id=model_hint,
                )
                broadcaster_registered = entry is not None
                _log.info(
                    "chat.headless.broadcaster_registered",
                    tab_id=tab_id.value,
                    conversation_id=conversation_id.value,
                    registered=broadcaster_registered,
                )
            except Exception as exc:  # noqa: BLE001 — never break the turn
                _log.warning(
                    "chat.headless.broadcaster_register_failed",
                    tab_id=tab_id.value,
                    error=type(exc).__name__,
                    error_msg=str(exc),
                )
        # Collect the set of SYSTEM_NOTICE dedup_keys present on the
        # conversation at turn open — the LLM's wire folds these in,
        # so the headless turn "observed" all of them regardless of
        # whether they were in the coordinator's ``pending_keys`` at
        # drain-iteration start.  Fetched via the same conversations
        # port the use case already uses.  Best-effort — a lookup
        # failure leaves ``observed`` empty and the coordinator falls
        # back to its pre-run snapshot (byte-identical to the
        # pre-refactor ack surface, no worse).
        observed_keys: set[str] = set()
        try:
            conv_snapshot = await self._conversations.find(conversation_id)
            if conv_snapshot is not None:
                for _m in conv_snapshot.messages:
                    if _m.role is not MessageRole.SYSTEM_NOTICE:
                        continue
                    _meta = _m.meta if isinstance(_m.meta, dict) else None
                    if _meta is None:
                        continue
                    _k = _meta.get(SYSTEM_NOTICE_META_DEDUP_KEY)
                    if isinstance(_k, str) and _k:
                        observed_keys.add(_k)
            _log.info(
                "chat.headless.observed_keys_collected",
                tab_id=tab_id.value,
                conversation_id=conversation_id.value,
                observed_count=len(observed_keys),
                observed_keys=sorted(observed_keys),
                total_messages=(
                    len(conv_snapshot.messages) if conv_snapshot is not None else -1
                ),
            )
        except Exception as exc:  # noqa: BLE001 — never break the turn
            _log.warning(
                "chat.headless.observed_keys_scan_failed",
                tab_id=tab_id.value,
                error=type(exc).__name__,
                error_msg=str(exc),
            )
        ...
        try:
            ...
            async for frame in agen:
                if broadcaster_registered:
                    try:
                        broadcaster.publish(tab_id, frame)
                    except Exception as exc:  # noqa: BLE001
                        _log.warning(
                            "chat.headless.broadcaster_publish_failed",
                            tab_id=tab_id.value,
                            error=type(exc).__name__,
                        )
                if frame.frame_type is StreamFrameType.CHUNK:
            ...
            if broadcaster_registered:
                try:
                    if aborted:
                        broadcaster.mark_aborted(tab_id, reason="headless_error")
                    else:
                        broadcaster.mark_terminal(tab_id)
                except Exception as exc:  # noqa: BLE001
                    _log.warning(
                        "chat.headless.broadcaster_terminal_failed",
                        tab_id=tab_id.value,
                        error=type(exc).__name__,
                    )
        return "".join(parts), usage, frozenset(observed_keys)
    ...
    async def _network_retrying_stream(
        ...
        because it also inspects PROMPT_TOO_LONG errors to short-circuit
        (a concern the sub-agent loop does not have).
        """
    ...
    def _observe_cache_round(
        ...
        *,
        ...
        """Record one round's REAL cache-read volume onto the TURN.

        Single source of truth for the main agent's TWO observation points —
        round 0 (observed right after ``_drain_main_stream`` finishes) and
        every follow-up round (``_on_round_end``). Both call this so the
        signal口径 cannot drift between them.

        The true-hit signal is the number of tokens actually read back from
        cache this round. Two mutually exclusive fields carry it
        (``_extract_usage`` sets AT MOST one > 0):

        * ``cache_read_tokens`` — preserved on the non-corrected path;
        * ``cache_read_observed`` — stashed BEFORE the corrected branch zeros
          ``cache_read_tokens`` (so a real hit is not masked by the eff-prompt
          correction). Only present when > 0.

        Their SUM is the round's true cache-read (mutually exclusive → sum ==
        max).

        This does NOT touch the capability registry: the verdict is fed ONCE
        per turn by :meth:`_flush_cache_capability`, from the maximum of the
        signals collected here. See ``_TurnBodyState.cache_read_signals`` for
        why the registry must be marked per TURN and not per round.

        No-ops when no registry is wired or the round carried no usage dict at
        all (an unmeasured round must leave the state machine untouched).
        Returns the round's signal, or ``None`` when nothing was observed.

        ``state`` carries the turn accumulator. It is optional only so the
        legacy no-``state`` call shape stays usable; when absent the signal is
        forwarded to the registry immediately (single-round semantics), which
        is what a caller with no turn context can honestly promise.

        NB: the sub-agent path keeps its OWN equivalent implementation in
        ``qai.chat.adapters.agent_tool`` (its ``_on_round_end``, which logs
        ``agent="subagent"`` and carries a ``subagent_id``). It is deliberately
        NOT merged here — it runs with different round semantics — but it
        writes to the SAME registry, so keep the two口径 in step if either
        changes.
        """
        if self._provider_cache_registry is None or not isinstance(usage, dict):
            return None
        cache_read_observed = int(usage.get("cache_read_observed") or 0)
        cache_read_tokens = int(usage.get("cache_read_tokens") or 0)
        cache_read_signal = cache_read_observed + cache_read_tokens
        # DIAG: the RAW inputs to the capability signal, so a round where the
        # registry "saw nothing" can be told apart from one where the gateway
        # reported nothing. Paired with ``chat.diag.cache_capability_mark``
        # (emitted by the registry itself when the turn flushes).
        _log.info(
            "chat.diag.cache_read_signal",
            agent="main",
            round_no=round_no,
            model_hint=model_hint,
            cache_read_observed=cache_read_observed,
            cache_read_tokens=cache_read_tokens,
            cache_read_signal=cache_read_signal,
        )
        if state is None:
            # No turn context to aggregate on: mark straight through, which
            # preserves the single-round口径 for callers outside a turn.
            self._provider_cache_registry.mark(model_hint, cache_read_signal)
            return cache_read_signal
        state.cache_read_signals.append(cache_read_signal)
        return cache_read_signal

    def _flush_cache_capability(
        self,
        *,
        model_hint: str | None,
        state: _TurnBodyState,
    ) -> None:
        """Feed the TURN's cache-capability verdict to the registry, once.

        The verdict is the MAXIMUM signal observed across the turn's measured
        rounds: a read that landed on ANY round proves the model reads cache
        back, while a turn whose every round read nothing spends exactly ONE
        of the registry's two zero-read credits.

        No-op when the turn observed no measured round at all — an unmeasured
        turn is not evidence of "zero read" (State-Truth-First). Draining the
        accumulator makes a second call in the same turn a no-op, so the
        old per-round double-counting hazard cannot return.
        """
        if self._provider_cache_registry is None:
            return
        if not state.cache_read_signals:
            return
        turn_signal = max(state.cache_read_signals)
        state.cache_read_signals.clear()
        self._provider_cache_registry.mark(model_hint, turn_signal)
    ...
    async def _handle_initial_tool_call(
        ...
        *,
        ...
        async for fu_frame in self._run_followup_loop(
            ...
            # Follow-up rounds RECORD their cache-read signal onto the turn;
            # the registry verdict is flushed once after this loop returns.
            cache_signal_state=state,
        ):
    ...
    async def _save_parent_conv(
        self, conv: Any, *, is_takeover: bool, defer: bool = False
    ) -> None:
        ...
        When a ``ConversationWriteQueue`` is wired (production DI) the write
        goes through it: same-conversation writes serialise behind one worker
        instead of racing for SQLite's write lock, and repeated saves of THIS
        aggregate inside the queue's batch window collapse into a single
        transaction. Semantics are unchanged — ``defer=False`` awaits the
        queue's ack future, so the rows are durable on return exactly as the
        direct call was.

        ``defer=True`` submits without awaiting AND lets the write coalesce
        inside the batch window: used by the per-round incremental
        persistence, whose whole point is a crash safety net that must not
        add turn latency. The cost is bounded by the batch window (see the
        queue's module docstring on the accepted loss). Every other call
        site awaits and pre-empts the window, so no user-visible save is
        ever slower than before.
        """
        if is_takeover:
            return
        queue = self._conversation_write_queue
        if queue is None:
            await self._conversations.save_messages(conv)
            return
        future = await queue.submit_save_messages(
            conv.id.value, conv, immediate=not defer
        )
        if not defer:
            await future
    ...
    async def _prepare_turn(
        ...
        # Append user message — SKIPPED in headless mode (§6.5 refactor):
        # a headless follow-up turn triggered by
        # :class:`AgentSessionCoordinator` fires because a SYSTEM_NOTICE
        # is ALREADY sitting in the conversation waiting to be observed.
        # Appending an extra user message would (a) leak a phantom user
        # bubble in the transcript, and (b) require inventing content
        # for :class:`MessageContent` (which enforces non-empty text).
        # Downstream ``_run`` still sees a valid ``user_msg`` — for
        # headless we synthesise a lightweight standin whose sole
        # purpose is to carry an id + role for later reference lookups;
        # it is NEVER persisted or streamed.
        if request.headless:
            # No user append — the SYSTEM_NOTICE that triggered this
            # follow-up is already on the conv (dispatcher appended it
            # before firing the coordinator).  Downstream persist paths
            # thread ``user_msg.id`` as the parent for the assistant
            # message they write; return the LIVE conv tail (the
            # SYSTEM_NOTICE itself) so ``conv.append_message`` on the
            # assistant finds a real parent in ``conv.messages`` and
            # the aggregate's parent-id integrity check (see
            # :meth:`Conversation.append_message`) passes.  When the
            # conv is somehow empty (defensive — should not happen for
            # a coordinator-triggered turn) we fall back to a synthetic
            # placeholder that is never appended anywhere: better a
            # downstream parent_id error than a NullPointer here.
            _tail = conv.last_message()
            if _tail is not None:
                user_msg = _tail
            else:
                user_msg = self._build_message(
                    role=MessageRole.USER,
                    content=MessageContent(text="[headless followup]"),
                    now=self._clock.now(),
                    parent_id=None,
                    meta=None,
                )
            ...
            # Merge optional caller-supplied metadata
            # (``request.user_message_meta``) with the auto-detected
            # ``image_url`` so both survive in ``msg.meta``.
            _user_meta: dict[str, Any] = {}
            if _user_image_url:
                _user_meta["image_url"] = _user_image_url
            if isinstance(request.user_message_meta, dict):
                _user_meta.update(request.user_message_meta)
            user_msg = self._build_message(
                role=MessageRole.USER,
                content=request.user_message,
                now=self._clock.now(),
                parent_id=conv.last_message().id if conv.last_message() else None,
                meta=_user_meta or None,
            )
        ...
        # M5 BUGFIX (2026-08-03): register the tab-keyed soft-steer context
        # HERE — the earliest point every turn passes through — not inside
        # ``_run_followup_loop``.  The FIRST tool_call of a turn is dispatched
        # by ``_handle_initial_tool_call`` (round 0), which never enters the
        # follow-up loop, so a register placed there missed every round-0
        # exec and the whole feature stayed dormant
        # (observed: ``exec.m5.arm_probe has_soft_ctx=False``).
        _soft_ev = getattr(handle, "soft_steer_event", None)
        if _soft_ev is not None:
            _resolved_threshold_ms: int | None
            if callable(self._auto_bg_threshold_ms):
                try:
                    _resolved_threshold_ms = await self._auto_bg_threshold_ms()
                except Exception:  # noqa: BLE001 — never break a turn
                    _resolved_threshold_ms = None
            else:
                _resolved_threshold_ms = self._auto_bg_threshold_ms
            # Exec 4-way race (Batch 5): bind the bridge's ``subscribe`` to
            # THIS tab so a tool in another bounded context gets the
            # capability without a ``qai.chat`` import.  ``None`` when the
            # bridge is not wired → the consumer skips that race arm.
            _injection_subscribe = None
            if self._injection_signal_bridge is not None:
                _bridge = self._injection_signal_bridge
                _race_tab = tab.id

                def _injection_subscribe() -> tuple[
                    asyncio.Event, Callable[[], None]
                ]:
                    return _bridge.subscribe(_race_tab)
            # The race's Stop arm.  ``AsyncioStreamAbortHandle`` exposes the
            # raw abort event through ``abort_event``; a handle predating it
            # yields None and the consumer skips that arm (Stop still lands
            # through task cancellation, as before).
            _abort_ev = getattr(handle, "abort_event", None)
            try:
                from qai.platform.tab_soft_steer_registry import (
                    TabSoftSteerCtx,
                ...
                register_tab_soft_steer(
                    tab.id.value,
                    TabSoftSteerCtx(
                        soft_steer_event=_soft_ev,
                        background_process_manager=(
                            self._background_process_manager
                        ),
                        owner_session_id=tab.id.value,
                        # The bgp tool's ownership key (see the dataclass
                        # docstring) — adopt registers under THIS so the
                        # model can status/logs/stop its own adopted job.
                        conversation_id=conv.id.value,
                        threshold_ms=_resolved_threshold_ms,
                        abort_event=_abort_ev,
                        injection_subscribe=_injection_subscribe,
                    ),
                )
                _log.info(
                    "chat.m5.soft_steer_registered",
                    tab_id=tab.id.value,
                    threshold_ms=_resolved_threshold_ms,
                    has_bg_manager=(
                        self._background_process_manager is not None
                    ),
                )
            except Exception as _reg_exc:  # noqa: BLE001 — never break a turn
                _log.warning(
                    "chat.m5.soft_steer_register_failed",
                    tab_id=tab.id.value,
                    error=repr(_reg_exc),
                )
    ...
    async def _run(
        ...
        # instance so ``_maybe_schedule_background_compaction`` (called at
        # turn end) can drive an off-turn compaction with the SAME
        # request object.  ``_compress_via_checkpoint`` reads
        # ``request.user_message`` (to drop the trailing user turn from the
        # history) and ``request.model_hint`` (for the context window),
        # so a stripped-down stub would AttributeError.
        self._last_turn_request = request  # type: ignore[attr-defined]
        ...
        state = _TurnBodyState(
            ...
            # A main-agent turn is BY DEFINITION the root of the sub-agent
            # tree, so its depth is a structural fact — hardcoded rather than
            # read from the request / settings, where a misconfiguration could
            # silently shift the recursion cap for every spawn this turn makes.
            parent_depth=0,
        )
        ...
        try:
            ...
            # prompt builder splices it in (mirrors the ``hook_context``
            # mechanism itself). No directive → nothing injected (unchanged).
            ...
            if handle.is_set():
                ...
                # follow-up loop itself is entered ONLY when round 0 emitted a
                # TOOL_CALL. So a turn that answers directly — no tool call at
                # all — never reached the registry, and a model used *only*
                # that way stayed ``unknown`` forever (observed in production:
                # ``claude-4-6-opus`` used for a full turn while
                # ``known_model_count`` stayed at 2). ``unknown`` keeps aging
                # OFF, so a model that genuinely cannot read cache back never
                # earned its byte savings.
                #
                # THIS is the single convergence point for the whole turn: the
                # drain is finished AND the follow-up loop (if any) has
                # returned, so ``state.first_round_usage`` is final no matter
                # WHICH END-frame branch captured it and every follow-up round
                # has already recorded its signal on ``state``.
                #
                # Observe round 0, then flush the TURN's verdict exactly once.
                # The registry converts on 2 consecutive zero-read
                # observations, so a per-ROUND mark let a single multi-round
                # turn spend both credits and condemn a model on its very
                # first turn — see ``_TurnBodyState.cache_read_signals``.
                #
                # NB: deliberately NOT folded into the round-0 budget fold in
                # ``_run_followup_loop`` even though that solves the same
                # "round 0 misses ``_on_round_end``" problem: that site sits
                # behind the follow-up loop (tool-call turns only) AND behind
                # the budget ``enabled`` gate, whereas capability learning
                # must happen on every measured turn, cap configured or not.
                self._observe_cache_round(
                    model_hint=request.model_hint,
                    usage=state.first_round_usage,
                    round_no=0,
                    state=state,
                )
                self._flush_cache_capability(
                    model_hint=request.model_hint,
                    state=state,
                )
            ...
            try:
                ...
                try:
                    ...
                    # Advance the running full-history token counter on this
                    # path too (gap found 2026-08-10). CancelledError
                    # (``:4069``) and the generic Exception block (``:4235``)
                    # both do it; GeneratorExit did not — so a consumer
                    # ``aclose()`` (SSE client disconnect / page refresh
                    # mid-turn) persisted the partial turn into ``conv`` (the DB
                    # grew) while ``full_history_tokens`` stayed at its old
                    # value, freezing the context badge until some later turn
                    # happened to recompute it. Same 口径 as the normal
                    # finalize, and awaiting is legal here (no ``yield`` in this
                    # handler).
                    await self._update_full_history_counter_on_interrupt(
                        conv=conv, state=state,
                        model_hint=request.model_hint or "",
            ...
            # Mid-turn user injections that ALREADY reached the wire must be
            # persisted here too (gap found 2026-08-10). The other three
            # termination paths — CancelledError (``:4078``), GeneratorExit
            # (``:4153``) and the normal tail (``:4358``) — all call this; this
            # generic-Exception block did not, so an injection the model had
            # already seen vanished from the DB whenever a turn died on a
            # provider crash / compaction bug. The user's text is gone from the
            # transcript while the model acted on it — the transcript then
            # contradicts the reply.
            #
            # Best-effort + shielded, exactly like the tab release below: a save
            # failure here must never mask the original error.
            try:
                await asyncio.shield(
                    self._persist_injections_on_interrupt(
                        conv=conv, state=state,
                    )
                )
            except Exception:  # noqa: BLE001 — never mask the original error
                _log.warning(
                    "chat.injection_persist_on_error_failed",
                    tab_id=tab.id.value,
                    conversation_id=conv.id.value,
                )
            ...
            # M5: drop the tab-keyed soft-steer context (turn teardown).
            # Idempotent; safe on every termination path.
            try:
                from qai.platform.tab_soft_steer_registry import (
                    unregister_tab_soft_steer,
                )
                unregister_tab_soft_steer(tab.id.value)
            except Exception:  # noqa: BLE001 — cleanup must never raise
                pass
            # Mid-turn user injection (V2 enhancement): take the tab's PENDING
            # injections out of the process-local registry (texts not yet
            # drained into a round). The already-drained ones were minted into
            # ``state.injected_messages`` and are durably persisted to ``conv``
            # on EVERY termination path by this point — the normal path
            # (``_finalize_turn`` / ``_persist_completed_rounds`` →
            # ``_reinsert_injected_messages``) and the interrupt/abort paths
            # (``_persist_injections_on_interrupt`` runs in the CancelledError /
            # GeneratorExit / abort handlers BEFORE this).
            #
            # ``drain`` — NOT ``clear`` (bug fixed 2026-08-10). This ``finally``
            # belongs to the ``try`` that wraps the whole stream drain, so it
            # runs BEFORE :meth:`_finalize_turn` (which sits after that ``try``
            # and owns the ``inject_as_new_turn`` deferral). ``clear`` here
            # therefore always beat that drain: the deferral frame could NEVER
            # fire, and an injection landing in a turn with no further tool
            # round — a headless sub-agent follow-up, or any plain text reply —
            # was destroyed without a trace (grey bubble hung forever, the text
            # never reached the model). Observed 2026-08-10: two injections,
            # zero ``inject_as_new_turn`` in the log.
            #
            # We cannot emit the frame here (this block runs under
            # ``GeneratorExit`` where awaiting is illegal), so hand the swept
            # texts to ``state`` and let ``_finalize_turn`` emit them. Still
            # synchronous + side-effect-only, so it stays GeneratorExit-safe.
            if self._injection_registry is not None:
                _swept = self._injection_registry.drain(tab.id)
                if _swept:
                    state.swept_injections.extend(_swept)
    ...
    async def _finalize_turn(
        ...
        *,
        ...
        tool_call_msg = self._build_tool_call_message(
            ...
            ids_by_round=state.round_message_ids,
        )
        ...
        if joined:
            ...
            async for fr in self._finalize_assistant_message(
                ...
                stable_subagent_summary_ids=state.subagent_summary_message_ids,
                # §N.53 — SYSTEM_NOTICE dedup_keys this turn integrated.
                integrated_notice_ids=(
                    state.pending_integration_keys_this_turn
                ),
            ):
                yield fr
            ...
            self._insert_subagent_summary_messages_per_round(
                ...
                stable_ids_by_round=state.subagent_summary_message_ids,
            )
            await self._save_parent_conv(
        ...
        # ── Deferred mid-turn injection tail (V2 enhancement) ─────────────
        # If any pending injection text survives to this point the run loop
        # never folded it into a round: either the turn produced no tool rounds
        # (``KernelFinished`` on the first LLM turn — a plain text reply, or a
        # headless sub-agent follow-up) so the inter-round seam never fired, or
        # the user pressed Ctrl+Enter AFTER the last tool round while the
        # assistant was still writing its final answer.
        #
        # Emit an ``inject_as_new_turn`` frame carrying the FIFO texts and let
        # the frontend resubmit them at the HEAD of its send queue — the pending
        # grey bubbles disappear and the text reaches the model on the next turn,
        # ahead of any ordinary backlog (user ruling 2026-08-09).
        #
        # TWO sources, both required (bug fixed 2026-08-10):
        #   * ``state.swept_injections`` — what the teardown ``finally`` took
        #     out of the registry. That ``finally`` belongs to the ``try`` around
        #     the stream drain, so it ALWAYS runs before this tail; when it used
        #     to ``clear`` instead of hand over, this drain saw an empty mailbox
        #     and the frame could never fire. This is the common case.
        #   * ``registry.drain`` — an injection that arrived AFTER the ``finally``
        #     (the user pressing Ctrl+Enter during this very tail). Narrow, but
        #     dropping it would resurrect the same silent-loss bug.
        #
        # Only the normal-completion path emits this frame: abort / error exits
        # already ``return`` above and are semantically closer to "user
        # cancelled" — dropping their pending injections is expected.
        if self._injection_registry is not None or state.swept_injections:
            _deferred_texts = list(state.swept_injections)
            state.swept_injections.clear()
            if self._injection_registry is not None:
                _deferred_texts.extend(self._injection_registry.drain(tab.id))
            if _deferred_texts:
                state.synth_seq += 1
                _deferred_frame = StreamFrame.inject_as_new_turn(
                    frame_id=f"inject-defer-{state.synth_seq}",
                    sequence=state.synth_seq,
                    texts=_deferred_texts,
                )
                await self._publish(
                    ChatStreamFrameEvent(
                        tab_id=tab.id,
                        conversation_id=conv.id,
                        frame=_deferred_frame,
                    ),
                )
                yield _deferred_frame
                state.frame_count += 1
                _log.info(
                    "chat.inject_as_new_turn.emitted",
                    tab_id=tab.id.value,
                    conversation_id=conv.id.value,
                    count=len(_deferred_texts),
                )
        ...
        # M8 predictive background compaction: if the wire has climbed past a
        # soft threshold (60% of the model window) but has NOT yet crossed the
        # 70% inline-trigger, spawn an off-turn compaction task now.  When the
        # next user message arrives the checkpoint is already in place → the
        # presend gate re-computes ``used`` from the freshly-compacted head +
        # empty increment and the model call fires immediately (no inline
        # 3-10s pause).  The task is fire-and-forget; a failure inside the
        # engine falls through to the inline gate on the next turn as before.
        # No-op when no compressor is wired.
        try:
            self._maybe_schedule_background_compaction(conv=conv)
        except Exception:  # noqa: BLE001 — never break turn finalization
            pass

    def _maybe_schedule_background_compaction(self, *, conv: Any) -> None:
        """M8: fire-and-forget predictive compaction after a turn ends.

        Runs on the event loop as a detached task guarded by an internal
        per-conversation dedup set so at most one background compaction
        is in flight per conversation at a time.  Delegates to the SAME
        :meth:`_compress_via_checkpoint` the presend gate uses, with the
        FULL request captured at turn start (model_hint + user_message +
        extra) so the engine picks the correct context-window ceiling
        and drops the correct trailing user turn.  Cost of a no-op call:
        one sync dict lookup + engine bail-out.

        The next turn's presend gate then sees the fresh checkpoint,
        skips the inline compaction step, and starts the LLM request
        without the ~3-10s pause.  Failures are silent — the inline gate
        on the next turn is the authoritative fallback.

        Race note: if the user sends a new message BEFORE this bg task
        finishes, the next turn's presend gate may also trigger a
        concurrent inline compaction.  The compaction engine has no
        per-key lock, so whichever finishes second overwrites the
        checkpoint.  Both are valid (same input + same algorithm), just
        wasted CPU.  Not corruption; not fixed here.
        """
        if self._context_compressor is None:
            return
        # Reuse the last observed request (stashed at turn start).  If
        # nothing has been recorded yet (fresh instance / test path), bail
        # out — ``_compress_via_checkpoint`` needs both model_hint and
        # user_message from the request object.
        last_request = getattr(self, "_last_turn_request", None)
        if last_request is None:
            return
        if getattr(self, "_bg_compaction_tasks", None) is None:
            self._bg_compaction_tasks = set()  # type: ignore[attr-defined]
        conv_key = self._conv_key(conv)
        # Dedup: at most one in-flight bg compaction per conversation.
        # Snapshot the set to avoid iterating a container that could be
        # mutated by a done-callback that fires during this loop.
        for _t in list(self._bg_compaction_tasks):  # type: ignore[attr-defined]
            if getattr(_t, "_conv_key", None) == conv_key and not _t.done():
                return  # already in flight for this conv
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            return

        # Capture the request into the closure at scheduling time so a
        # subsequent turn's ``_last_turn_request`` reassignment does not
        # affect this task.
        captured_request = last_request

        async def _run_bg() -> None:
            try:
                await self._compress_via_checkpoint(
                    conv=conv,
        ...
        task = loop.create_task(_run_bg(), name=f"bg-compact[{conv_key[:16]}]")
        setattr(task, "_conv_key", conv_key)
        self._bg_compaction_tasks.add(task)  # type: ignore[attr-defined]
        task.add_done_callback(self._bg_compaction_tasks.discard)  # type: ignore[attr-defined]
    ...
    def _compute_turn_warning_threshold(
        ...
        ``self._turn_warning_thresholds`` dict explicitly; supports
        user-configurable enabled/start/step via ``_turn_warning_settings``.
        """
        tw = self._turn_warning_settings
    ...
    async def _refresh_turn_warning_settings(self) -> None:
        """Refresh ``_turn_warning_settings`` from the user's config provider."""
        if self._turn_warning_provider is None:
            return
        try:
            result = self._turn_warning_provider()
            if isinstance(result, Awaitable):
                result = await result
            if isinstance(result, dict):
                self._turn_warning_settings = result
    ...
    async def _release_streaming_tab(
        ...
        *any* failure (provider crash, malformed on-device SSE frame, a bug
        ...
        # Same §6.4 hook — a failing turn also releases the tab; any
        # pending SYSTEM_NOTICE work queued during it must still fire
        # a follow-up now that we're back to idle.  Same ordering
        # invariant as :meth:`_complete_turn`: mark the broadcaster
        # entry terminal FIRST so the follow-up's ``register`` can
        # succeed (see the doc block in ``_complete_turn`` for the
        # detailed root cause).  The transport's own ``mark_terminal``
        # is idempotent so this cannot double-mark harmfully.
        if self._chat_stream_broadcaster is not None:
            try:
                self._chat_stream_broadcaster.mark_terminal(tab.id)
            except Exception as exc:  # noqa: BLE001
                _log.warning(
                    "chat.broadcaster.mark_terminal_failed",
                    tab_id=tab.id.value,
                    error=type(exc).__name__,
                    error_msg=str(exc),
                )
        if self._agent_session_coordinator is not None:
            try:
                self._agent_session_coordinator.on_tab_idle(tab.id)
            except Exception as exc:  # noqa: BLE001
                _log.warning(
                    "chat.coord.on_tab_idle_failed",
                    tab_id=tab.id.value,
                    error=type(exc).__name__,
                    error_msg=str(exc),
                )
    ...
    def _build_tool_call_message(
        ...
        *,
        ...
        Thin wrapper over
        :func:`_streaming_helpers.build_tool_call_message` — forwards
        ``self._ids`` explicitly.  ``text_by_round`` (self-contained
        agent) interleaves per-round text with tool rounds so a reload
        matches the live order.  ``ids_by_round`` (§concurrent-writer
        refactor 2026-08-07) is the shared cache of stable round
        message ids used across incremental persists so the SAME row
        in ``chat_message`` is updated on each rebuild, instead of
        stacking a fresh copy per call — see the state field docstring
        (``_TurnBodyState.round_message_ids``) for the full rationale.
        """
    ...
    async def _update_full_history_counter(
        ...
        * checkpoint EXISTS → post-compaction DELTA growth: seed
        ...
        if ckpt is None:
            ...
            new_last_eff = ckpt.last_eff_prompt
            _ckpt_changed = False
            if ckpt.last_eff_prompt is None:
                # First post-compaction turn: seed baseline, no delta this
                # turn (char ``est`` at compaction creation is a different
                #口径 from the cloud measurement).
                new_last_eff = eff_prompt
                _ckpt_changed = True
            else:
                ...
                if conv.full_history_tokens is None:
                    # Never-measured conversation that nonetheless got
                    # compacted (e.g. a long local-model history whose
                    # bytes/4 trigger fired while usage stayed 0). Seed
                    # the counter from this turn's measured size rather
                    # than growing a delta up from 0 (which would badly
                    # undercount the frozen full history).
                    conv.full_history_tokens = eff_prompt + completion_tokens
                else:
                ...
                if eff_prompt != ckpt.last_eff_prompt:
                    new_last_eff = eff_prompt
                    _ckpt_changed = True
            if _ckpt_changed and new_last_eff is not None:
                # Task R: the checkpoint is now a frozen dataclass. Update
                # the delta baseline through the engine's dedicated
                # in-memory helper (rebuilds the immutable instance under
                # the hood) so a wire reader that captured the prior
                # reference is untouched. Persist through a targeted core-
                # column update that does NOT advance ``generation`` — the
                # baseline is per-turn diagnostic, not a state advance
                # that fragment CAS writers should observe.
                self._compaction_engine.replace_last_eff_prompt(
                    self._conv_key(conv), int(new_last_eff),
                )
                store = getattr(self._compaction_engine, "_checkpoint_store", None)
                persist_id = self._conv_id_for_key(conv)
                if store is not None and persist_id is not None:
                    try:
                        await store.update_last_eff_prompt(
                            persist_id,
                            last_eff_prompt=int(new_last_eff),
                            core_generation=int(ckpt.generation),
                        )
    ...
    async def _finalize_assistant_message(
        ...
        *,
        ...
        first_round_usage: dict[str, Any] | None = None,
        turn_persist_baseline: int = -1,
        stable_subagent_summary_ids: dict[int, str] | None = None,
        integrated_notice_ids: "set[str] | None" = None,
    ) -> AsyncIterator[StreamFrame]:
        ...
        * when a snapshot id was produced, yield a supplementary
        ...
        _assistant_meta = _build_assistant_meta(
            ...
            sub_agent_blocks={},
            # §N.53 — the SYSTEM_NOTICE dedup_keys THIS turn folded into its
            # own wire (recorded by ``_integrate_pending_notices`` as it acked
            # them). Persisting them is the only durable trace that this answer
            # digested background notices, since the notices themselves are
            # wire-fold-only rows that never render as bubbles.
            integrated_notice_ids=integrated_notice_ids,
            # §N.41 — durable watermark for crash recovery.  ``request`` IS
            # the headless follow-up's own ``StreamChatInput`` (built with
            # ``headless=True`` by :meth:`run_headless_followup`), so the flag
            # is read from the authoritative in-scope request rather than
            # threaded through another parameter.  This is the LAST assistant
            # message the follow-up produces and the row that
            # ``conv.append_message`` + ``_save_parent_conv`` below actually
            # commit — so its ``position`` is a sound "everything below is
            # already integrated" boundary for
            # ``PendingStateRebuilder.rebuild_from_db``.
            headless_followup=bool(request.headless),
        )
        ...
        self._insert_subagent_summary_messages_per_round(
            ...
            stable_ids_by_round=stable_subagent_summary_ids,
        )
        ...
        # Terminal supplementary END frame. Historically emitted ONLY when a
        # prompt-snapshot request_id existed (to attach the 📄 button). We now
        # ALSO emit it whenever a persisted assistant message exists, carrying
        # its DB id as ``assistant_message_id`` so the frontend commits its
        # optimistic bubble with the SAME id the DB holds — enabling per-message
        # fork on a just-finished turn without a reload (§3.1 tail-append; the
        # snapshot-only case is byte-for-byte unchanged when there is no
        # assistant id).
        _assistant_msg_id = assistant_msg.id.value if assistant_msg else None
        if _snapshot_request_id is not None or _assistant_msg_id is not None:
            synth_seq += 1
    ...
    async def _emit_turn_warning(
        ...
        *,
        ...
        emit one ``turn_warning`` frame when a new band (30 / 40 / 50 /
        ...
        """
        # Refresh per-turn turn-warning settings from user config (if wired).
        await self._refresh_turn_warning_settings()
        frame_count = outcome.frame_count
        synth_seq = outcome.synth_seq
        try:
    ...
    async def _complete_turn(
        ...
        *,
        ...
        self._abort_registry.unregister(tab.id)
        # M5: drop the tab-keyed soft-steer context alongside the abort handle
        # so a completed turn leaves no stale entry.  Idempotent.
        try:
            from qai.platform.tab_soft_steer_registry import (
                unregister_tab_soft_steer,
            )
        ...
        # §6.4 refactor — a live turn that queued SYSTEM_NOTICE
        # notifications during its run (background-job completions
        # that landed while tab was busy) triggers the pending
        # headless follow-up NOW, so a queued dedup_key is not
        # silently stranded.  Best-effort: any exception is caught
        # (never breaks the turn's finalisation).
        #
        # State-Truth-First (AGENTS.md 🔴 铁律5) — the broadcaster
        # entry for THIS just-completed turn MUST be marked terminal
        # BEFORE we let ``on_tab_idle`` fire a headless follow-up.
        # Otherwise the follow-up's own :meth:`run_headless_followup`
        # will try to :meth:`ChatStreamBroadcaster.register` a fresh
        # entry for the same tab and get ``None`` back (the semantic
        # guard: existing non-terminal entry blocks a new register),
        # so its frames flow nowhere and the frontend
        # ``/api/chat/active-runs/{tab}/ws`` attach sees an empty
        # replay + immediate close — the exact "headless output
        # invisible until tab reopen" symptom.
        #
        # We mark terminal HERE rather than only inside the primary
        # WS/SSE route because the completion tail runs BEFORE the
        # transport's own ``finally`` clause; the transport's
        # ``mark_terminal`` stays as an idempotent backstop for the
        # abort / error paths that never reach this method.
        if self._chat_stream_broadcaster is not None:
            try:
                self._chat_stream_broadcaster.mark_terminal(tab.id)
    ...
    def _build_subagent_summary_message(
        ...
        *,
        ...
        """Build an INDEPENDENT assistant message carrying sub-agent fold blocks.

        SUBAGENT-RELOAD-PERSIST-INDEPENDENT-MSG (2026-07-02): sub-agent
        blocks live on their own message so a reload's row order matches
        the live-stream card order (the frontend already keeps the blocks
        on their own message id).

        §concurrent-writer refactor 2026-08-07 — ``stable_id`` (optional)
        pins the message id across incremental persists.  The caller
        (:meth:`_insert_subagent_summary_messages_per_round`) reads
        ``state.subagent_summary_message_ids[round_index]`` and passes
        the cached value on the second+ rebuild so ``save_messages``
        upsert-by-id updates the SAME DB row instead of appending a
        fresh copy every incremental save.  ``None`` (legacy / one-shot
        callers) mints a new id — byte-for-byte parity for callers that
        do not care about stability.

        Returns ``None`` when ``sub_agent_blocks`` is empty.
        """
        ...
        if stable_id is not None and stable_id:
            # Reuse pinned id — construct the Message directly so we can
            # override the ``id`` field (``self._build_message`` generates
            # a fresh id every call, which is exactly what we need to
            # avoid here on rebuild).
            return Message(
                id=MessageId.of(stable_id),
                role=MessageRole.ASSISTANT,
    ...
    def _insert_subagent_summary_messages_per_round(
        ...
        *,
        ...
        stable_ids_by_round: dict[int, str] | None = None,
    ) -> None:
        ...
        * inserts the summary IMMEDIATELY AFTER that round message via
        ...
        for ri in sorted(by_round.keys(), reverse=True):
            ...
            if target_pos is None:
                ...
                continue
            _cached_id = (
                stable_ids_by_round.get(ri)
                if stable_ids_by_round is not None
                else None
            )
            summary_msg = self._build_subagent_summary_message(
                now=now,
                parent_id=conv.messages[target_pos].id,
                sub_agent_blocks=by_round[ri],
                stable_id=_cached_id,
            )
            # Populate the cache on first build so subsequent rebuilds
            # (incremental persists) reuse this id.
            if (
                stable_ids_by_round is not None
                and _cached_id is None
                and summary_msg is not None
            ):
                stable_ids_by_round[ri] = summary_msg.id.value
    ...
    async def _resolve_effective_max_rounds(self) -> int:
        """Resolve the effective max-rounds for this turn.

        Priority:
          1. If ``_max_rounds_provider`` is wired and returns a positive int,
             use it (user-configured cap from Agent Settings panel).
          2. Otherwise fall back to the constructor's ``_max_followup_rounds``:
             - negative → 0 (kernel "unlimited")
             - positive → that cap value
        """
        if self._max_rounds_provider is not None:
            try:
                user_cap = await self._max_rounds_provider()
                if isinstance(user_cap, int) and user_cap > 0:
                    return user_cap
            except Exception:  # noqa: BLE001 — pref read must never break a turn
                pass
        # Constructor default: negative = unlimited (kernel 0), positive = cap.
        return max(0, self._max_followup_rounds)
    ...
    async def _open_with_retry(
        ...
        *,
        ...
        ``PROMPT_TOO_LONG`` errors are terminal: the error frame is forwarded
        to the caller without retry (the provider rejected the prompt for
        exceeding its context window — no amount of retry will change that).
        ...
        # Bounds the THROTTLING / BOUNDED_FAST / BOUNDED_SERVER retries; we
        # reach a terminal decision long before this ceiling unless the
        # policy is mis-configured. Only NETWORK is
        ...
        # Context-overflow recovery: single-shot gate (RED LINE — never loop).
        # Set the moment a recovery attempt is COMMITTED and never cleared, so
        # a second ``prompt_too_long`` on this same round-0 stream is terminal.
        # This local is the AUTHORITY on "have we already tried?" — deliberately
        # not ``attempt_number`` (a per-category counter the policy owns) and
        # not any instance attribute (which would leak across turns / tabs).
        # Function scope == this turn's round-0 stream, which is exactly the
        # lifetime the gate needs. Two further independent bounds back it up:
        # ``non_network_iterations`` (counted against ``max_iterations``) and
        # the no-op / still-over-window checks inside the recovery block.
        ...
        while True:
            ...
            if category is RetryCategory.PROMPT_TOO_LONG:
                ...
                # The provider rejected the wire for exceeding its context
                # window. Re-sending it unchanged is hopeless, but compressing
                # the HISTORY to a strictly lower target CHANGES the wire, so
                # exactly one retry can genuinely succeed.
                #
                # WHY THIS EXISTS AGAIN. An earlier revision of this branch ran
                # a force-compress + retry; the v3 compaction simplification
                # (docs/90-refactor/CONTEXT-COMPRESSION.md) removed it on the
                # grounds that "re-running the SAME compression algorithm is
                # pointless". That reasoning was sound for what it described —
                # the old path re-compressed at the SAME target, so it produced
                # the SAME wire and burned a request to be rejected identically.
                # It is not an argument against recovery as such, and the two
                # remaining defences (inter-round compaction + the presend
                # estimate) are both TRIGGER-side: they fire on an ESTIMATE
                # crossing a threshold, so whenever the estimate is wrong (or
                # the user switches to a smaller-window model mid-session) the
                # wire sails past the window and the turn is simply lost.
                # Observed live: a single round reporting
                # ``prompt_tokens=294,371`` — survivable on a 1M window, an
                # instant wall on a 200K one.
                #
                # So recovery returns, with the original objection designed out
                # rather than ignored: the pass aims at a STRICTLY LOWER target
                # (``overflow_recovery_target_ratio``), and if it still fails to
                # change anything we do NOT retry (see the no-op gate below).
                # Three independent bounds make looping structurally impossible:
                # (1) ``overflow_recovery_used`` — single-shot, never cleared;
                # (2) ``non_network_iterations`` against ``max_iterations``;
                # (3) the no-op + still-over-window checks, which abandon
                #     BEFORE spending the retry request.
                #
                # Nothing is dropped from the failed turn: a ``prompt_too_long``
                # is a FIRST-FRAME rejection, so no assistant text and no tool
                # round-trip exists to discard. The user's own message is
                # structurally safe — compaction only ever sees
                # ``_history_messages`` (which excludes the trailing current
                # user turn) and ``_build_base_wire_messages`` re-appends it
                # verbatim from ``request`` on the retry.
                if not overflow_recovery_used:
                    non_network_iterations += 1
                    overflow_recovery_used = True
                    # ``_recover_from_context_overflow`` is an async generator
                    # (it yields the user-visible progress / notice frames), so
                    # its verdict comes back through this sink rather than a
                    # return value.
                    recovery_outcome: dict[str, Any] = {}
                    async for _rec_frame in self._recover_from_context_overflow(
                        conv=conv,
                        request=request,
                        ...
                        yield _rec_frame
                    recovered = bool(recovery_outcome.get("recovered"))
                    if recovered:
                        # New checkpoint stored AND the reassembled wire fits.
                        # ``_build_llm_request`` picks the checkpoint up on the
                        # next loop pass (its ``_has_checkpoint`` branch), so
                        # the retry sends the compacted wire + the user's
                        # verbatim message.
                        continue
                # Terminal: either recovery was already spent on this turn, or
                # it could not shrink the wire any further. Forward the error so
                # the user sees an honest, actionable failure.
                _log.info(
            ...
            if decision.delay_seconds > 0:
                if category in (
                    ...
                    # why the THROTTLING branch below keeps the original
                    # single-shot ``_sleep`` call.
                    ...
                    # THROTTLING: short, bounded wait — single-shot ``_sleep``
                    # keeps the observable cadence unchanged.
    ...
    def _extract_round_real_prompt(
        ...
        **Invariant**: ``real_prompt_tokens`` is **always cumulative** in the
        ...
        if isinstance(usage, dict):
            ...
            # CCD-4 (Bug-3 fix, 2026-07-26 audit): also strip
            # ``reasoning_tokens`` from ``completion_tokens`` when the
            # provider itemises them under ``completion_tokens_details``
            # (o1 / o3 / Gemini thinking-mode). ``_extract_usage`` already
            # does this on the canonical path, so an already-normalised
            # ``usage`` dict has ``ct`` unchanged (details block gone or
            # ``reasoning_tokens`` accounted). But if a caller ever hands us
            # a raw provider dict directly, ``derived = tt - ct`` would
            # secretly bake reasoning back into the prompt count. Subtract
            # it up front so the belt-and-braces stays truly idempotent.
            ...
            if isinstance(details, dict):
                try:
                    ...
                    reasoning = 0
                if reasoning > 0 and reasoning <= ct:
                    ct = ct - reasoning
            if tt > 0 and ct >= 0:
                derived = tt - ct
                if derived > pt:
    ...
    def _compute_turn_prefix_at_boundary(
        ...
        *,
        ...
        Thin delegate to
        :func:`qai.chat.application.use_cases._streaming_helpers.compute_turn_prefix_at_boundary`
        — the SINGLE implementation shared with
        :class:`ForceCompactChatUseCase` so a user-issued ``/compact`` fires
        the SAME P3 gate an auto-compaction does. See the helper's docstring
        for the boundary + threshold semantics.
        """
        return _compute_turn_prefix_at_boundary_shared(
            wire, context_limit=context_limit,
        )
    ...
    def _maybe_inject_question_images(
        ...
        for tr in round_results:
            # V2: both the ``question`` tool answer (user-attached images) and
            # the ``computer`` tool result (a captured screenshot embedded as
            # ``![screenshot](/api/images/files/..)`` markdown) carry image
            # refs the model must actually SEE as pixels. Any other tool's
            # result is left untouched (its ``role:tool`` reply stays plain
            # text), and the ``_extract_image_refs`` gate below means a
            # ``question``/``computer`` result WITHOUT an image ref is a no-op
            # too — so the question path is byte-for-byte unchanged.
            if (tr.get("tool_name") or "") not in ("question", "computer"):
                continue
            result_text = str(tr.get("result", ""))
            image_refs = _extract_image_refs(result_text)
            if not image_refs:
                continue
            _img_fmt, _img_q = self._cloud_image_cfg()
            blocks = _resolve_image_refs_to_vision_blocks(
                store=store,
                image_refs=image_refs,
                source_text=result_text,
                cloud_format=_img_fmt,
                cloud_quality=_img_q,
            )
            if blocks:
                wire_messages.append(
    ...
    async def _build_llm_request(
        ...
        *,
        ...
        # CLOUD-only: read persona files (memory/SOUL.md, USER.md, IDENTITY.md)
        # and publish them on ``extra`` so the cloud prompt builder injects the
        # <long_term_memory> block and applies the IDENTITY.md override. Same
        # scope/gate as the workspace-context block above.
        self._resolve_persona_memory(extra=extra, request=request)
        ...
        # Persist skill instructions onto request.extra for follow-up rounds
        # (same pattern as system_prompt above). On round 0,
        # ``_apply_system_prompt`` sets ``extra["_skill_instructions"]`` from
        # the builder result; on follow-up rounds ``_apply_system_prompt``
        # short-circuits (system_prompt already present) so this persisted
        # value ensures the adapter still sees the skill body.
        _skill_instr = extra.get("_skill_instructions")
        if isinstance(_skill_instr, str) and _skill_instr:
            if request.extra is None:
                object.__setattr__(request, "extra", {})
            req_extra = request.extra
            if isinstance(req_extra, dict) and "_skill_instructions" not in req_extra:
                req_extra["_skill_instructions"] = _skill_instr
    ...
    def _computer_enabled(self) -> bool:
        ...
        Calls the injected sync provider (forge_config override > settings).
        A missing provider or any failure yields ``False`` so the
        desktop-control tool is dropped unless explicitly enabled.
        """
        provider = self._computer_enabled_provider
        if provider is None:
            return False
        try:
            return bool(provider())
        except Exception as exc:  # noqa: BLE001 — never break a turn on the gate
            _log.warning("chat.computer_enabled_gate_failed", error=str(exc))
            return False

    def _cloud_image_cfg(self) -> "tuple[str, int]":
        """Live (format, quality) for cloud-bound image encoding this turn.

        Calls the injected sync provider (forge_config chat.image_cloud_*).
        A missing provider / any failure yields the resolver default
        (webp, 50) so a turn is never broken.
        """
        provider = self._cloud_image_config_provider
        if provider is None:
            return ("webp", 50)
        try:
            fmt, quality = provider()
            return (str(fmt), int(quality))
        except Exception as exc:  # noqa: BLE001 — never break a turn on cfg
            _log.warning("chat.cloud_image_cfg_failed", error=str(exc))
            return ("webp", 50)
    ...
    def _collect_tool_schemas(
        ...
        * **The ``agent`` tool** is injected on every depth-0 turn (V1 injects
        ...
        # autonomous sub-agent set (``SUB_AGENT_EXCLUDED_TOOLS`` = agent /
        # question / sub_agent / scheduled_task excluded by default —
        # recursion + background-dialog guards), then per-tab opt-in
        # switches un-exclude:
        #   * ``self_allow_spawn`` → un-exclude ``agent`` so the taken-over
        #     sub-agent runs through THIS main loop's full ``agent``
        #     dispatch, and its children default to no further spawn.
        #     State-Truth-First: gated on the LIVE take-over context so an
        #     errant flag on a non-take-over turn never changes the set.
        #   * ``allow_question`` → un-exclude ``question`` (its blocking
        #     dialog opt-in for an interactive take-over).
        ...
        # Per-turn disabled set: session + persona restrictions, plus the
        # desktop-control gate. ``computer`` is always REGISTERED but only
        # advertised when the live ``computer.enabled`` flag (forge_config
        # override > settings) is truthy — so the Settings UI toggle takes
        # effect on the next turn without a restart. A missing/failing
        # provider is treated as disabled (secure default).
        _disabled = _session_disabled_tools(extra) | _persona_disabled_tools(extra)
    ...
    def _resolve_persona_memory(
        self,
        *,
        extra: dict[str, Any],
        request: "StreamChatInput",
    ) -> None:
        """Read persona files and publish them on ``extra`` (CLOUD only).

        Reads ``memory/SOUL.md`` + ``memory/USER.md`` into a single
        ``<long_term_memory>`` block (published on
        ``extra["long_term_memory_block"]``) and ``memory/IDENTITY.md`` as an
        identity override (published on ``extra["persona_identity"]``). The
        cloud prompt builder consumes both via ``_maybe_long_term_memory_block``
        and ``_effective_identity``.

        Same scope as :meth:`_resolve_workspace_context_files`: cloud-only
        (local on-device models build a minimal prompt elsewhere), best-effort
        (a missing/empty file is simply omitted and never breaks assembly),
        read fresh every turn (the reader's (path,mtime,size) cache serves
        unchanged files without re-reading while an edit invalidates it).
        Persona is global (repo ``memory/`` dir), so no workspace root is
        needed. Pre-populated keys are respected so a caller can override.
        """
        if _is_local_model_hint(getattr(request, "model_hint", None)):
            return
        block, identity = _resolve_persona_memory_fn()
        if block and MEMORY_BLOCK_EXTRA_KEY not in extra:
            extra[MEMORY_BLOCK_EXTRA_KEY] = block
        if identity and PERSONA_IDENTITY_EXTRA_KEY not in extra:
            extra[PERSONA_IDENTITY_EXTRA_KEY] = identity
        _log.info(
            "chat.persona_memory.resolved",
            soul_user_block=bool(block),
            block_chars=len(block) if block else 0,
            identity_override=bool(identity),
            identity_chars=len(identity) if identity else 0,
        )
    ...
    def _apply_system_prompt(
        ...
        * **Cloud model** — delegates to the rich
        ...
        # Prompt-cache optimisation: when the builder separated the SKILL body
        # from the system prompt, propagate it so the LLM adapter can inject
        # it as a user-role message (keeps system prompt byte-stable).
        if sp_result.skill_body:
            extra["_skill_instructions"] = sp_result.skill_body
    ...
    @property
    def compaction_engine(self) -> "CompactionCheckpointEngine":
        """Public accessor for the wired :class:`CompactionCheckpointEngine`.

        Exposed so the DI composition can hand the SAME engine instance to
        :class:`GetContextDecisionUseCase` (P5 escalation read path) without
        reaching into the private ``_compaction_engine`` attribute. The
        engine is always constructed alongside the stream use case (see
        ``__init__``), so this is unconditional — no ``None`` case.
        """
    ...
    def estimate_compacted_tokens(
        ...
        * ``int`` — the conversation HAS a checkpoint; this is the
          best-known token count of the compacted wire that is ACTUALLY
          sent to the model now.

        Single-truth-source (audit §3.7 fix): the source of the branch-X
        reading is now :func:`authoritative_provider_size` — the SAME
        helper the trigger gate uses. Prior code read
        ``last_round_prompt_tokens`` directly; on multi-round agentic
        turns that key exceeds the round-0 wire by 20-40% (in-flight
        tool outputs that AGE / TRUNCATE strip before the next turn),
        so the badge showed a size 20-40% larger than what the trigger
        gate judged, restoring the §8.7 76%→56% jitter class. Using the
        same helper aligns both readings on the SAME provider figure —
        the round-0 stamped ``first_round_prompt_tokens``.

        Degraded fallback (audit §3.5): a legacy usage row (no
        ``first_round_prompt_tokens``) yields ``degraded=True``. The
        badge MUST NOT show the poisoned ``last_round_prompt_tokens``
        (that reading is exactly the 30x-inflation class of numbers the
        §10.F fix was designed to hide from users) — instead the badge
        falls back to the checkpoint's bootstrap
        ``estimated_tokens`` (the char-based figure stamped at
        compaction creation, uncontaminated). Same fallback the
        pre-audit ``branch Y`` used when no post-anchor assistant
        existed at all.

        Anchor filter: the helper is called with
        ``checkpoint_anchor_message_id=ckpt.anchor_message_id`` so
        assistants at or before the anchor (pre-compaction) can NEVER
        leak into the badge. Same rule the pre-audit code had via
        ``_last_assistant_with_usage(after_message_id=...)``.
        """
        ckpt = self._compaction_checkpoints.get(self._conv_key(conversation))
        if ckpt is None:
            # DIAG (token-display investigation): no checkpoint → badge
            # must stay un-compacted. If the UI shows a "压缩后/省%" state
            # while this logs None, the bug is on the display side.
            _log.info(
            ...
            return None
        reading = _authoritative_provider_size(
            conv=conversation,
            checkpoint_anchor_message_id=ckpt.anchor_message_id,
            request_model_id=model_id,
        )
        if reading is not None and not reading.degraded:
            # Branch X: post-anchor assistant with a non-degraded round-0
            # measurement — the provider's authoritative wire size.
            _log.info(
                "chat.diag.estimate_compacted",
                conversation_id=getattr(
                    getattr(conversation, "id", None), "value", None,
                ...
                has_checkpoint=True,
                source="authoritative_first_round",
                source_key=reading.source_key,
                source_model_id=reading.source_model_id,
                result=reading.base_size,
                ckpt_estimated_tokens=ckpt.estimated_tokens,
                ckpt_last_eff_prompt=ckpt.last_eff_prompt,
            )
            return int(reading.base_size)
        # Branch Y (bootstrap or degraded): the stashed char-based
        # estimate from checkpoint creation — the same fallback the
        # pre-audit code used when no post-anchor assistant existed at
        # all. On a degraded reading this is STRICTLY better than
        # showing ``last_round_prompt_tokens``: the latter re-introduces
        # the §10.F 30x-inflation class of numbers on the user-visible
        # badge, while ``ckpt.estimated_tokens`` was stamped at
        # compaction creation from the ACTUAL compacted wire size and is
        # never poisoned.
        _log.info(
            "chat.diag.estimate_compacted",
            conversation_id=getattr(
            ...
            reading_degraded=(reading.degraded if reading is not None else None),
            reading_source_key=(reading.source_key if reading is not None else None),
            result=ckpt.estimated_tokens,
            ckpt_last_eff_prompt=ckpt.last_eff_prompt,
        )
        return ckpt.estimated_tokens

    def compacted_badge_detail(
        self, conversation: Any, model_id: str | None,
    ) -> dict[str, Any] | None:
        """Badge sidecar for :meth:`estimate_compacted_tokens`.

        Returns ``None`` under exactly the same condition that method does (no
        checkpoint), else a dict with:

        * ``compacted_tokens`` — identical value to
          :meth:`estimate_compacted_tokens` (ONE caliber, one code path: this
          method delegates, it never recomputes);
        * ``overhead_tokens`` — the per-turn runtime overhead (system prompt +
          tool schemas + persona + skill blocks) folded into the numerator.
          Derived from the checkpoint, NOT re-estimated: the engine stamped
          ``estimated_tokens = estimate_wire_tokens(compacted_wire) +
          overhead`` (``_compaction_engine.py``), so subtracting a fresh
          estimate of the SAME stored wire recovers the overhead it folded in.
          ``None`` when the checkpoint carries no stashed estimate.
        * ``measured_model_id`` — the ``model_id`` of the assistant turn whose
          provider measurement produced ``compacted_tokens``, or ``None`` when
          the figure came from the checkpoint bootstrap instead. DIAGNOSTIC
          provenance only: the badge does NOT decorate the figure when this
          differs from the selected model. A cross-model reading is a real
          measurement of a real wire (different models genuinely assemble
          different tool schemas), not a degraded value, and it refreshes on the
          next turn — marking it would imply the number is untrustworthy.

        WHY A SIDECAR AND NOT A SECOND NUMBER: the numerator keeps exactly one
        caliber (provider-measured whole wire). ``overhead_tokens`` is reported
        as a DIAGNOSTIC / EXPLANATORY figure — how much of that wire is fixed
        per-turn cost compaction can never reclaim. It MUST NOT be added to the
        badge's denominator: ``conv.full_history_tokens`` is NOT a pure history
        counter, it is SEEDED from a provider-measured whole wire
        (:meth:`_update_full_history_counter`, the ``ckpt is None`` branch:
        ``full_history_tokens = eff_prompt + completion``, ``eff_prompt``
        INCLUDING overhead) and afterwards grows only by pure-history deltas
        (the ``ckpt`` branch, where the two ``eff_prompt`` readings cancel their
        overhead). It therefore already carries exactly ONE overhead share —
        the same single share this numerator carries — so
        ``1 - compacted/full_history`` is already a same-baseline ratio. Adding
        the overhead again would make the baseline ``pure_history +
        2 * overhead``, a wire that never existed. See
        ``docs/90-refactor/CONTEXT-COMPACTION.md`` §六.
        """
        ckpt = self._compaction_checkpoints.get(self._conv_key(conversation))
        if ckpt is None:
            return None
        compacted = self.estimate_compacted_tokens(conversation, model_id)
        # Which assistant (if any) supplied the measurement. Mirrors the
        # branch-X condition inside ``estimate_compacted_tokens`` EXACTLY
        # (audit §3.7 single-truth-source): both paths run
        # :func:`authoritative_provider_size` with the same anchor filter
        # and both consider only a non-degraded reading a real
        # measurement. Prior code re-implemented the ``last_round or
        # prompt_tokens > 0`` predicate inline; single truth is safer.
        measured_model_id: str | None = None
        _sidecar_reading = _authoritative_provider_size(
            conv=conversation,
            checkpoint_anchor_message_id=ckpt.anchor_message_id,
            request_model_id=model_id,
        )
        if _sidecar_reading is not None and not _sidecar_reading.degraded:
            measured_model_id = _sidecar_reading.source_model_id
        overhead_tokens: int | None = None
        if ckpt.estimated_tokens is not None:
            try:
                bare_wire = _estimate_wire_tokens(
                    ckpt.compacted_wire, model_hint=model_id,
                )
                derived = int(ckpt.estimated_tokens) - int(bare_wire)
                # A negative result means the stashed estimate predates the
                # overhead fold-in (or was written by a path that passed
                # ``overhead_tokens=0``): report no overhead rather than a
                # nonsense value that would INFLATE the denominator.
                overhead_tokens = derived if derived > 0 else None
            except Exception:  # noqa: BLE001 — badge sidecar is best-effort
                overhead_tokens = None
        return {
            "compacted_tokens": compacted,
            "overhead_tokens": overhead_tokens,
            "measured_model_id": measured_model_id,
        }
    ...
    def _presend_eff_estimate(
        ...
        *,
        ...
        """Provider-authoritative predictor of the NEXT turn's wire size.

        Single-truth-source (audit §4.1): every "how big is the wire that
        gets sent to the model?" consumer — trigger gate here, badge
        :meth:`estimate_compacted_tokens`, A′ overhead reverse-compute
        :func:`compute_overhead_from_last_wire_measurement`, ``/compact``
        force path — now derives ``base`` from ONE helper
        :func:`authoritative_provider_size`. Same reading, same anchor
        filter, same CCD-2 model resolution everywhere; the §8.7 76%→56%
        jitter class is structurally impossible instead of hand-audited
        every time a new consumer joins (State-Truth-First 铁律 1).

        Formula (composition of the next turn's starting wire, ≈):

            overhead + compacted_head + prev_final_assistant + AGED_tools
                     + new_user

        Prior turn's round-0 usage measured EXACTLY:

            overhead + compacted_head + AGED_tools + this_turn_user

        The tool aging state is invariant across turn boundaries (see
        ``age_old_tool_outputs`` + ``AdaptiveToolResultTruncator``:
        outputs older than the 6-most-recent are replaced with a
        fixed-length placeholder). Delta:

            delta = prev_final_assistant_completion + new_user
                  - this_turn_user_at_round_0

        Dropping ``- this_turn_user_at_round_0`` (unknown, typically
        ≤ new_user_at_round_0 which was already inside the round-0
        measure) yields a slight conservative over-count well under the
        ~1K tool-placeholder drift already tolerated by this gate.

        Return contract — the ONE number the trigger gate consumes:

        * ``> 0`` → a NON-DEGRADED provider reading was available; the
          returned value IS the predicted next-turn wire size **already
          including runtime overhead** (system prompt + tool schemas +
          persona + skill blocks are inside the round-0 measurement).
          ``_compaction_engine.maybe_compress`` compares this against
          ``bytes_estimate + overhead_tokens`` and takes the max — the
          two are apples-to-apples (both include overhead exactly
          once), so the audit §3.6 double-count is structurally
          impossible.
        * ``0`` → EITHER no assistant with real usage exists (fresh
          conversation, local-model history) OR the reading was
          ``degraded=True`` (legacy pre-2026-07 row without
          ``first_round_prompt_tokens``). The trigger gate collapses
          to the ``bytes_estimate + overhead_tokens`` path — the same
          shape it uses when no assistant is available at all. This
          is the audit §3.5 fix: the pre-fix code fell through to
          ``last_round_prompt_tokens`` on legacy rows and re-
          introduced the §10.F 30x inflation (a poisoned base +
          ``max()`` = still poisoned; ``max`` cannot pull a large
          value down). Returning 0 hands the decision to the
          uncontaminated bytes path.

        Anchor filter (audit §3.10): ``authoritative_provider_size`` is
        called with ``checkpoint_anchor_message_id`` when a checkpoint
        exists, so the bootstrap window between checkpoint creation and
        the first post-compact turn CANNOT let the (much larger)
        pre-compaction assistant leak in as the ``prior``.

        New-user-message sizing:
          * ``len(text) > 2000`` → precise BPE (``precise_text_tokens``);
            on tiktoken unavailability falls back to ``len(text) // 2``.
          * otherwise → ``len(text) // 2``.

        Best-effort, never raises. On ANY internal error the trigger
        falls back to ``bytes_estimate + overhead_tokens`` via the
        ``return 0`` sentinel — never a spurious large trigger.
        """
        prior = 0
        prev_completion = 0
        try:
            # ANCHOR FILTER (audit §3.10): keep pre-compaction assistants
            # out of the reading during the bootstrap window.
            _anchor_id: str | None = None
            _ckpt = self._compaction_checkpoints.get(self._conv_key(conv))
            if _ckpt is not None:
                _anchor_id = getattr(_ckpt, "anchor_message_id", None)
            reading = _authoritative_provider_size(
                conv=conv,
                checkpoint_anchor_message_id=_anchor_id,
                request_model_id=model_id,
            )
            # DEGRADED SENTINEL (audit §3.5): a legacy row's contaminated
            # ``last_round_prompt_tokens`` MUST NOT participate in the
            # prediction. Hand off to ``bytes_estimate + overhead`` by
            # returning 0 (the same shape as "no assistant available").
            if reading is None or reading.degraded:
                return 0
            prior = reading.base_size
            # PROVIDER-EXACT DELTA: the prior assistant's completion size
            # is the provider figure, not a local estimate. Read from the
            # same usage row the helper selected via anchor filter.
            _last_asst = _last_assistant_with_usage(
                conv, after_message_id=_anchor_id,
            )
            if _last_asst is not None:
                prev_completion = int(
                    (_last_asst.usage or {}).get("completion_tokens") or 0
                )
        ...
        if len(text) > 2000:
            ...
            new_user = len(text) // 2
        return int(prior) + int(prev_completion) + int(new_user)
    ...
    async def _compress_via_checkpoint(
        ...
        *,
        ...
        Estimates the assembled wire (BPE-free bytes/4口径 via
        :func:`estimate_wire_tokens`, which counts content + tool_calls
        args).  When the estimate exceeds
        ``INTER_ROUND_COMPRESS_THRESHOLD_RATIO × context_limit`` the FULL
        assembled wire is compressed and stored as a new checkpoint anchored at
        ...
        Anthropic) of the MOST-RECENT completed round THIS turn. The trigger
        ...
        used DIRECTLY as the ``实发`` reading; when ``None`` (presend path)
        ...
        # ``实发`` fallback for the trigger gate: only needed when no live
        # measured eff_prompt is threaded (presend path). Computed lazily so
        # the conv-derived estimate is taken ONLY when the engine would
        # actually use it (byte-for-byte: the original computed it in the same
        # else-branch).
        ...
        # P2 (§6): capture the PRIOR checkpoint's digest before the engine
        # replaces it, so the kick below has the "previous digest" the
        # incremental-update prompt branch needs. This must happen BEFORE
        # ``maybe_compress`` runs — the engine's write-through overwrites
        # ``_checkpoints[full_key]`` in place, and re-reading afterwards
        # would return the fresh (digest-less) checkpoint. ``None`` on the
        # first-ever compaction for this conversation → the use case picks
        # the "first summary" prompt (A) instead of the "incremental
        # update" prompt (B).
        _conv_key_val = self._conv_key(conv)
        _prev_ckpt_for_digest = (
            self._compaction_engine.get_checkpoint(_conv_key_val)
        )
        _prev_digest_text: str | None = (
            _prev_ckpt_for_digest.digest_text
            if _prev_ckpt_for_digest is not None
            else None
        )

        # P0-2 (Task O): extract file / URL / exec references from the wire
        # BEFORE compaction runs. ``reference_extractor`` scans every
        # assistant tool_call's arguments so the engine's ledger merge folds
        # THIS turn's new references into the prior checkpoint's ledger —
        # without this step ``new_refs`` stays ``None`` forever, the merged
        # ledger collapses to ``None``, and the ``[References Ledger]`` wire
        # block never appears. We walk ``assembled`` (the wire the model
        # actually sees) so references from BOTH the anchored head and the
        # since-anchor tail are captured; the extractor's built-in path is a
        # pure function (no I/O, never raises) so this stays cheap.
        from qai.chat.application.reference_extractor import (
            extract_from_wire_messages as _extract_refs_from_wire,
        )
        _new_refs = _extract_refs_from_wire(assembled)
        # §八 / §8.11: reverse-compute the runtime wire overhead the
        # engine cannot see (system prompt + tool schemas + persona + skill
        # instructions) and thread it into ``maybe_compress`` so the
        # checkpoint's stashed ``estimated_tokens`` folds that overhead in
        # up front — no bootstrap-window jitter between /compact and the
        # first post-compact turn. The helper's four-layer guard collapses
        # bad signals to ``overhead_pre = 0`` (byte-for-byte identical to
        # the pre-fix behaviour).
        _wire_estimate_before = _estimate_wire_tokens(
            assembled, model_hint=model_id,
        )
        _overhead_pre, _overhead_source = _compute_overhead_from_last_wire_measurement(
            conv=conv,
            wire_estimate_before=_wire_estimate_before,
            # ERA MATCHING (see the helper's docstring): the anchor must be
            # the one THIS compaction writes (``anchor_index`` above), so the
            # selected assistant measured the same wire generation
            # ``_wire_estimate_before`` estimates. Passing the PRIOR
            # checkpoint's anchor selected a pre-compaction assistant and
            # folded the compaction GAIN into overhead, which inflated the
            # stashed bootstrap estimate and made the badge numerator go
            # BACKWARDS once the first post-compact turn landed.
            wire_anchor_index=anchor_index,
        )
        _log.info(
            ...
            wire_estimate_before=_wire_estimate_before,
            overhead_tokens=_overhead_pre,
            overhead_source=_overhead_source,
        )
        new_ckpt = await self._compaction_engine.maybe_compress(
            ...
            new_refs=_new_refs,
            overhead_tokens=_overhead_pre,
            target_window_ratio_override=target_window_ratio_override,
        )
        if new_ckpt is None:
            return False
        ...
        # P2 (§6): fire the async digest refresh (D2 trigger point).
        # ``digest_enabled`` gates on (1) a use case being wired and (2) the
        # model's context window being at least the small-window threshold —
        # both fail-closed to a no-op preserving pre-P2 wire bytes. The
        # dropped slice fed to the summariser is the WIRE we just compressed
        # (``assembled``): it is the exact prefix now folded into
        # ``new_ckpt.compacted_wire``, so the digest describes precisely
        # the head the model can no longer see verbatim. The ledger block
        # is rendered once here (already-cached inside the ledger object)
        # and threaded into the use case as pre-rendered text so the
        # summariser sees the SAME ledger the wire injection will show.
        if self._compaction_engine.digest_enabled(context_limit):
            # Deferred import to avoid a top-of-file circular import
            # (refresh_digest imports the ports module; the ports module
            # is imported here transitively via LLMStreamPort).
            ...
            _persist_id = self._conv_id_for_key(conv)
            if _persist_id is not None:
                self._compaction_engine.kick_digest_refresh(
                    checkpoint_key=_conv_key_val,
                    input=RefreshDigestInput(
                        conversation_id=_persist_id,
                        checkpoint_key=_conv_key_val,
                        prev_digest=_prev_digest_text,
                        dropped_wire=[dict(m) for m in assembled],
                        reference_ledger_block=ledger_block,
                        model_id=model_id,
                        context_window=context_limit,
                    ),
                )
        # P3 (§7.1): after the digest kick, evaluate whether the dropped
        # prefix contained a "long turn" — a single turn (last user →
        # everything that followed inside it) whose rolling wire crossed
        # ``0.6 × PROTECT_WINDOW_RATIO × context_limit``. When it did, fire a
        # side-band turn-prefix summariser targeting that turn's PREFIX
        # (atomic-group boundary observed). The summariser runs INDEPENDENTLY
        # from the P2 digest — different columns, different task registry,
        # both may be in flight concurrently for the same conversation.
        if self._compaction_engine.turn_prefix_enabled(context_limit):
            _prefix_wire, _user_msg_id = self._compute_turn_prefix_at_boundary(
                assembled, context_limit=context_limit,
            )
            if _prefix_wire and _user_msg_id is not None:
                # Deferred import (matches the digest kick above — avoids
                # dragging the use case's ports import into module load).
                from qai.chat.application.use_cases.summarize_turn_prefix import (
                    SummarizeTurnPrefixInput,
                )
                _persist_id_tp = self._conv_id_for_key(conv)
                if _persist_id_tp is not None:
                    self._compaction_engine.kick_turn_prefix_summary(
                        checkpoint_key=_conv_key_val,
                        input=SummarizeTurnPrefixInput(
                            conversation_id=_persist_id_tp,
                            checkpoint_key=_conv_key_val,
    ...
    def _recovery_notice_frame(
        self,
        *,
        text: str,
        kind: str,
    ) -> StreamFrame:
        """Build a non-terminal system-note frame for the recovery path.

        Rendered by the frontend as a small system-note card (never a user
        bubble), so the user learns WHY this turn was slow and what happened to
        their history. ``sequence`` is a local ``0`` — the same convention the
        other transient progress frames use (``_network_retry_frame``); the
        unique ``frame_id`` keeps them distinguishable.
        """
        return StreamFrame.aside_message(
            frame_id=f"ctx-recovery-{self._ids.new_id()}",
            sequence=0,
            text=text,
            kind=kind,
        )

    def _recovery_progress_frame(self, *, state: str, message: str | None) -> StreamFrame:
        """Build a ``compaction_progress`` frame for the recovery compaction.

        Reuses the existing compaction indicator so the UI shows its usual
        passive "compressing context" banner while the recovery pass runs —
        the recovery path needs no new frame type or client handler.
        """
        return StreamFrame.compaction_progress(
            frame_id=f"ctx-recovery-{state}-{self._ids.new_id()}",
            sequence=0,
            state=state,
            message=message,
        )
    ...
    async def _recover_from_context_overflow(
        ...
        *,
        ...
        original error. In particular a compaction that changes NOTHING must
        NOT be retried: the wire would be byte-identical and the request would
        be rejected again, burning a round-trip for no reason.

        The user's current message is untouched by construction: compaction
        operates on ``_history_messages`` (trailing current-user turn already
        dropped) and the retry's wire builder re-appends the message verbatim
        from ``request``.
        """
        ...
        tokens_before = _estimate_wire_tokens(wire_before, model_hint=model_id)
        _raw_message = error_frame.payload.get("message")
        _log.info(
            "chat.overflow_recovery.detected",
            conversation_id=conversation_id,
            model_id=model_id,
            context_limit=context_limit,
            error_code=error_frame.payload.get("code"),
            error_message_excerpt=(
                _raw_message[:200] if isinstance(_raw_message, str) else None
            ),
        ...
        if self._context_compressor is None:
            ...
            return
        # Target: strictly below the target that was already in force (the
        # user's live preference, so a tuned target is honoured).
        _ratios = await self._resolve_compaction_ratios()
        normal_target_ratio = float(_ratios["target"])
        ...
        yield self._recovery_notice_frame(
            text=_t(
                "chat.context_recovery.succeeded",
                before_tokens=tokens_before,
                after_tokens=tokens_after,
            ),
            kind="context_recovered",
        )
    ...
    async def _maybe_presend_compress(
        ...
        *,
        ...
        # compaction state).
        self._presend_compressed: tuple[Any, ...] | None = None  # type: ignore[attr-defined]
        # P5 (CONTEXT-COMPRESSION-NEXT v3.1 §7.2 / D11): a new turn is
        # starting — the previous turn's mid-turn compaction counter is by
        # definition stale. Reset here (presend is the earliest hook we have
        # in the new-turn sequence) so the "≥ 3 mid-turn compactions in ONE
        # turn" trigger is per-turn, not cross-turn accumulated. Cheap dict
        # pop; no-op when nothing is recorded.
        self._compaction_engine.reset_consecutive_mid_turn(
            self._conv_key(conv),
        )
        if self._context_compressor is None:
            return
        ...
        try:
            ...
            # P2-1 (Task O): also log a warning on the slow-path (progressing=
            # True) so a regression that adds a raising failure path inside
            # ``_compress_via_checkpoint`` is not silently discarded on turns
            # where compaction ran >2s. The exception is only re-raised on
            # the fast-path (no progress frame emitted) — after we've yielded
            # a ``compressing`` frame to the transport the turn is committed
            # to a ``done`` frame, so raising now would corrupt the client's
            # frame sequence.
            if task.done() and not task.cancelled():
                exc = task.exception()
                if exc is not None:
                    _log.warning(
                        "chat.presend_compress_task_failed",
                        error=str(exc),
                        error_type=type(exc).__name__,
    ...
    async def _execute_single_tool_call(
        ...
        *,
        ...
        # Structured, user-fixable failure code (e.g. ``web_search`` behind an
        # enterprise TLS gateway → ``chat.llm.tls_cert_untrusted``). Only the
        # ``invoke`` path below can set it (from a returned
        # ``ToolInvocationResult``); every other branch leaves it ``None``.
        tool_error_code: str | None = None
        _terminal_kind_from_invocation: str | None = None
        if _hook_denied:
            ...
            # M10 (tool progress preview): install a per-call emitter so a slow
            # tool (web_fetch / web_search / grep / large read/write / apply_patch)
            # can publish keep-alive ``tool_result(partial=True, delta=...)``
            # frames straight to the tab's SSE stream via ``_publish``.  The
            # emitter is sync (tools call it inline), we fire-and-forget the
            # async publish via ``asyncio.ensure_future`` — best-effort, never
            # blocks the tool.  Restore the prior emitter in a ``finally`` so a
            # subsequent tool call in the same batch does NOT keep this call's
            # tool_call_id.
            #
            # The emitter contract lives in ``qai.platform`` because its
            # PRODUCERS (the ai_coding tool handlers) and its CONSUMER (this
            # loop) sit in different bounded contexts — see that module's
            # docstring and ``.importlinter``'s ``context-isolation`` contract.
            from qai.platform.tool_progress import (
                reset_tool_progress_emitter,
                set_tool_progress_emitter,
            )
            _tab_id_for_progress = tab.id
            _conv_id_for_progress = conv.id
            _call_id_for_progress = tc_frame.payload.get("tool_call_id") or tc_frame.frame_id
            _tool_name_for_progress = tool_name
            _progress_seq_counter = [0]

            # Task-leak guard: ``asyncio.ensure_future`` returns a Task, but
            # if no strong reference is kept the event loop CAN GC the task
            # before it runs (Python asyncio's known ``never awaited`` gotcha).
            # A per-call set of pending publishes keeps every task alive
            # until it settles; a done-callback removes it so the set does
            # not grow unbounded.
            _pending_publishes: set[asyncio.Task[None]] = set()

            def _publish_progress(status: str, _kind: str | None) -> None:
                # Cheap fire-and-forget publish.  The frame is a partial
                # tool_result carrying ``delta=status`` — the frontend's
                # existing partial-frame handler picks it up and appends
                # to the live tool card.
                _progress_seq_counter[0] += 1
                _sub_seq = _progress_seq_counter[0]
                try:
                    _frame = StreamFrame.tool_result(
                        frame_id=f"fu-tp-{_call_id_for_progress}-{_sub_seq}",
                        sequence=0,  # transient progress; not part of gap detection
                        tool_name=_tool_name_for_progress,
                        result=status,
                        partial=True,
                        delta=status,
                        tool_call_id=_call_id_for_progress,
                        phase="progress",
                    )
                    _publish_task = asyncio.ensure_future(
                        self._publish(
                            ChatStreamFrameEvent(
                                tab_id=_tab_id_for_progress,
                                conversation_id=_conv_id_for_progress,
                                frame=_frame,
                            )
                        )
                    )
                    # Strong-ref until settled to prevent GC-drop of the
                    # task before it runs.
                    _pending_publishes.add(_publish_task)
                    _publish_task.add_done_callback(_pending_publishes.discard)
                except Exception:  # noqa: BLE001 — progress is best-effort
                    pass

            _progress_token = set_tool_progress_emitter(_publish_progress)
            try:
                ...
                if isinstance(invocation, ToolInvocationResult):
                    ...
                    tool_error_code = invocation.error_code
                    # ``terminal_kind`` (§3.1 tail): the adapter can flag a
                    # semantically distinct successful outcome (today: the
                    # background_process bridge marks ``stop``/``restart`` so
                    # the UI renders "已停止"/"已重启" instead of a generic
                    # ✓ 已完成). Read HERE because the bridge stringifies its
                    # dict result BEFORE returning, so a later
                    # ``raw_result.get("terminal_kind")`` would miss it.
                    _terminal_kind_from_invocation = invocation.terminal_kind
                else:  # adapter returned a bare value
                    raw_result = invocation
                    success = True
                    _terminal_kind_from_invocation = None
            except Exception as exc:  # pragma: no cover - defensive
                raw_result = f"[tool_error] {exc}"
                success = False
            finally:
                # Restore the previous emitter so a subsequent tool call in
                # this batch does not inherit our tool_call_id. The reset is
                # a PUBLIC function of the platform module precisely so this
                # consumer never touches the ContextVar handle, and it
                # swallows a stale-token failure internally.
                reset_tool_progress_emitter(_progress_token)
        ...
        # Extract advisory quota_warning (set by web_search tool on keyed-engine
        # threshold crossings) from the structured result before truncation
        # stringifies it away. Only present on dict envelopes from web_search.
        _quota_warning_payload: dict[str, Any] | None = None
        if isinstance(raw_result, dict) and "quota_warning" in raw_result:
            _quota_warning_payload = raw_result.pop("quota_warning")
        # ``terminal_kind`` (structured; §3.1 tail-only): the tool signalled
        # that its outcome is semantically distinct from a plain completion
        # (today: ``background_process(action=stop|restart)`` emits
        # ``"stopped"`` / ``"restarted"`` so the card can render "已停止" /
        # "已重启" rather than the generic "已完成" tick). Read AND pop before
        # ``str()`` stringifies it away for the LLM's transcript.
        _terminal_kind_payload: str | None = _terminal_kind_from_invocation
        if _terminal_kind_payload is None and (
            isinstance(raw_result, dict)
            and isinstance(raw_result.get("terminal_kind"), str)
        ):
            _terminal_kind_payload = raw_result.pop("terminal_kind")
        ...
        trunc = self._tool_result_truncator.truncate(
            ToolResultTruncationRequest(
                ...
                # Where this slice STARTS, so the ordered-slice hard-cap
                # backstop can name a forward continuation offset instead of
                # one derived from its own kept-line count.
                slice_start_line=_kernel_tool_slice_start(raw_result),
            ),
        )
        tool_results.append(
            {
        ...
        return StreamFrame.tool_result(
            ...
            # persistence pairing.
            tool_call_id=tc_frame.payload.get("tool_call_id"),
            # User-fixable failure code (e.g. ``chat.llm.tls_cert_untrusted``)
            # lifted from a returned ``ToolInvocationResult`` so the tool card
            # can render its inline fix affordance. ``None`` for ordinary
            # results → field omitted from the payload.
            error_code=tool_error_code,
            quota_warning=_quota_warning_payload,
            terminal_kind=_terminal_kind_payload,
        )
        ...
        *,
        ...
        trunc = self._tool_result_truncator.truncate(
            ToolResultTruncationRequest(
                ...
                # ``stored_path``) so its recovery footer is never corrupted.
                already_truncated=_kernel_tool_already_truncated(raw_result),
                # See the one-shot path above: keeps a tail-cut slice's
                # continuation offset pointing FORWARD.
                slice_start_line=_kernel_tool_slice_start(raw_result),
            ),
        )
        tool_results.append(
        ...
        # ``backgrounded`` + ``background_process_id`` propagate the terminal
        # DONE chunk's structured "still running, not finished" marker (see
        # ``ToolStreamChunk`` docstring). Absent on ordinary completions →
        # byte-for-byte unchanged for those callers. When present, the UI
        # renders a distinct "moved to background" state without parsing the
        # ``[Auto-parked]`` prose out of ``result``.
        _bg_flag: bool | None = None
        _bg_id: str | None = None
        if done_chunk is not None:
            if done_chunk.backgrounded is not None:
                _bg_flag = done_chunk.backgrounded
            if done_chunk.background_process_id is not None:
                _bg_id = done_chunk.background_process_id
        yield StreamFrame.tool_result(
            frame_id=f"fu-tr-{local_seq}",
            sequence=local_seq,
            tool_name=tool_name,
            ...
            # Tool wall-clock run time (ms) — see StreamFrame.tool_result.
            duration_ms=max(0, _now_ms(self._clock) - _tool_started_ms),
            backgrounded=_bg_flag,
            background_process_id=_bg_id,
        )
    ...
    async def _run_followup_loop(
        ...
        *,
        ...
        cache_signal_state: "_TurnBodyState | None" = None,
    ) -> AsyncIterator[StreamFrame]:
        ...
        * ``on_round_end`` = usage accumulation + empty-completion retry +
        ...
        def _context_usage_frame(*, seq: int) -> "StreamFrame | None":
            ...
            # The turn-boundary authoritative badge (``GetContextDecisionUseCase`` via
        ...
        async for fr in self._dispatch_pending_tool_calls(
            ...
            round_index=0,
            parent_depth=(
                cache_signal_state.parent_depth
                if cache_signal_state is not None
                else 0
            ),
        ):
            _result_rid = (
        ...
        # ── Turn-internal SYSTEM_NOTICE integration state (§A3) ───────────
        # ``_pending_integration_count`` counts CONSECUTIVE re-opens caused
        # by newly-landed pending notices; reset to 0 at the start of every
        # kernel round (a round that ran for any other reason means the
        # streak is over). ``_pending_integration_limited`` latches once the
        # streak hits ``_MAX_PENDING_INTEGRATION`` so the terminal path
        # surfaces an aside note instead of silently swallowing the notices
        # (they stay pending → the coordinator's headless drain picks them
        # up after the turn releases the tab).
        _pending_integration_count: int = 0
        _pending_integration_limited: bool = False

        async def _integrate_pending_notices(
            round_no: int,
        ) -> list[dict[str, Any]] | None:
            """Fold newly-landed SYSTEM_NOTICE rows onto this turn's wire.

            Returns the wire to reopen a round with (this turn's wire plus
            one folded notice entry per pending key), or ``None`` to let
            the turn close.

            Why this exists: a SYSTEM_NOTICE that lands while a turn is
            running used to be left entirely to the coordinator, which
            fires ONE headless follow-up per notice once the tab goes
            idle.  A batch of sub-agents finishing together therefore
            produced 3-4 consecutive follow-up turns, each re-summarising
            what the previous one had just said.  Folding them into the
            LIVE turn makes the whole batch one integrated answer.

            The fold shape is deliberately IDENTICAL to the one
            :func:`rebuild_history_wire_messages` produces for a
            ``role=system_notice`` row (``role:user`` +
            :data:`SYSTEM_NOTICE_WIRE_PREFIX`), so the model sees the same
            bytes whether a notice arrives mid-turn or is replayed from
            history on a later turn.

            Keys are acked as they are folded: they are on THIS wire, so
            the model reads them this turn and the post-turn headless
            drain must not re-summarise them.  Appending to
            ``wire_messages`` is safe — it is the loop's own send-wire
            truth, never ``conv.messages`` (durable UI history untouched).

            Best-effort throughout: no coordinator wired, a snapshot
            failure, a drain already running, or notice rows we cannot
            read back all mean "let the turn close" — the notices stay
            pending and the coordinator handles them as it did before.
            """
            nonlocal _pending_integration_count, _pending_integration_limited
            coord = self._agent_session_coordinator
            if coord is None:
                return None
            try:
                snap = coord.snapshot(tab.id)
            except Exception as exc:  # noqa: BLE001 — never break a turn
                _log.warning(
                    "chat.coord.pending_snapshot_failed",
                    tab_id=str(tab.id),
                    error=type(exc).__name__,
                    error_msg=str(exc),
                )
            ...
            if not snap.has_pending:
                return None
            # A headless drain already owns these keys — it rebuilds its own
            # wire from the live conversation, so folding them here too would
            # double-summarise the same rows.
            if snap.is_headless_running:
                return None
            if _pending_integration_count >= _MAX_PENDING_INTEGRATION:
                _pending_integration_limited = True
                _log.warning(
                    "coord.pending_integration.limit_reached",
                    tab_id=str(tab.id),
                    conversation_id=conv.id.value,
                    round_no=round_no,
                    integrations=_pending_integration_count,
                    limit=_MAX_PENDING_INTEGRATION,
                    pending_count=len(snap.pending_keys),
                    pending_keys=list(snap.pending_keys),
                )
                return None
            # The rows must be readable before we can fold their text in.
            settled, flushed = await coord.settle_pending(
                tab.id, _PENDING_SETTLE_TIMEOUT_MS
            )
            if not settled:
                # Partial flush: fold nothing and ack nothing. The un-acked
                # keys go to the post-turn headless drain, which rebuilds its
                # wire after the write lands.
                return None
            wanted = set(snap.pending_keys)
            try:
                snapshot_conv = await self._conversations.find(conv.id)
            except Exception as exc:  # noqa: BLE001 — never break a turn
                _log.warning(
                    "chat.coord.pending_notice_lookup_failed",
                    tab_id=str(tab.id),
                    error=type(exc).__name__,
                    error_msg=str(exc),
                )
                return None
            if snapshot_conv is None:
                return None
            folded: list[dict[str, Any]] = []
            folded_keys: list[str] = []
            #: ``(source_id, body)`` per readable notice, in transcript order.
            _bodies: list[tuple[str, str]] = []
            for _m in snapshot_conv.messages:
                if _m.role is not MessageRole.SYSTEM_NOTICE:
                    continue
                _meta = _m.meta if isinstance(_m.meta, dict) else None
                if _meta is None:
                    continue
                _key = _meta.get(SYSTEM_NOTICE_META_DEDUP_KEY)
                if not isinstance(_key, str) or _key not in wanted:
                    continue
                _text = getattr(getattr(_m, "content", None), "text", "") or ""
                if not _text:
                    continue
                _src = _meta.get(SYSTEM_NOTICE_META_SOURCE_ID)
                _bodies.append(
                    (_src if isinstance(_src, str) else "", _text)
                )
                folded_keys.append(_key)
            if not _bodies:
                # Pending keys with no readable row (a producer that notified
                # before persisting): nothing to fold, nothing to ack.
                return None
            # ONE wire message for the whole batch.  One message PER notice
            # (the previous shape) left several adjacent user rows the model
            # had to attribute by position: when two tasks settle in the same
            # window it could credit task A's output to task B.  A single
            # envelope with an explicit per-source header makes the mapping
            # textual, which is how the reference design carries concurrent
            # results too (one batched message, one header per job).
            #
            # Each body is inserted VERBATIM — no re-wrap, no truncation, no
            # added commentary.  A background result must read exactly like the
            # same work's foreground result; only the batch header (which task
            # this is) is added, because with several results in one message
            # that header is what makes attribution possible at all.
            if len(_bodies) == 1:
                _batch_text = _bodies[0][1]
            else:
                _parts: list[str] = [
                    f"{len(_bodies)} background tasks have completed."
                    f" Resume your work using the results below."
                ]
                for _src, _body in _bodies:
                    _parts.append(
                        f"\u2500\u2500 {_src} \u2500\u2500\n{_body}"
                        if _src
                        else _body
            ...
            folded.append(
                {
                    "role": "user",
                    "content": f"{_SYSTEM_NOTICE_WIRE_PREFIX}{_batch_text}",
                }
            )
            for _key in folded_keys:
                coord.ack(tab_id=tab.id, dedup_key=_key)
            # §N.53 — remember WHICH notices this turn digested, so the
            # finalised assistant message can carry them in
            # ``meta["integrated_notice_ids"]`` and the UI can show the
            # "integrated N background notices" chip.  Recorded right beside
            # the ``ack`` (same keys, same moment) so the chip can never claim
            # a notice the turn did not actually fold in.
            if cache_signal_state is not None:
                cache_signal_state.pending_integration_keys_this_turn.update(
                    folded_keys
                )
            _pending_integration_count += 1
            _log.info(
                "pending.wire_injection.count",
                tab_id=str(tab.id),
                conversation_id=conv.id.value,
                round_no=round_no,
                # Count of NOTICES digested, not wire messages appended: the
                # batch fold emits exactly one message for any number of
                # notices, and this key's long-standing meaning (used when
                # reading production logs) is "how many completions did this
                # turn take in".
                notice_count_injected=len(_bodies),
                wire_messages_appended=len(folded),
                notice_keys=folded_keys,
                integration_attempt=_pending_integration_count,
                flushed_futures=flushed,
            )
            return list(wire_messages) + folded
        ...
        async def _inject_hook(round_no: int) -> list[InjectedContent]:
            ...
            # Retire the steer flags now that the drain has happened — this is
            # the "boundary AFTER drain" their ``clear_*`` docstrings name, and
            # until now nothing in production called either one, so a flag set
            # once stayed set for the rest of the turn.
            #
            # ``steer_hard`` is the one that really latched: it IS set in
            # production (``:12287`` wires ``notify_steer_hard`` as the
            # kernel's ``on_steer_detected_hard``) and it is raced by every
            # INTERRUPTIBLE tool (``interruptible_wait_events`` returns it), so
            # the next ``question`` / ``vibe_wait`` / ``hub_wait`` of the same
            # turn resolved instantly as interrupted — the user saw a question
            # card die on arrival.
            #
            # ``soft_steer`` and ``irc_hard`` are cleared for symmetry, not
            # because they latch today — both are wired but have no live
            # producer:
            #   * ``notify_soft_steer`` has NO production caller (verified
            #     across ``src/`` + ``apps/``: only the definitions at
            #     ``:1152`` and ``stream_abort_registry.py:121``, plus that
            #     method's own forwarding at ``:139``). exec's background
            #     hand-off does NOT go through it: ``di.py:1156`` passes a
            #     LOCAL ``_adopt_signal`` into ``stream_exec`` (whose parameter
            #     merely happens to be named ``soft_steer_event``) and ``:1129``
            #     sets it from the exec-race outcome.
            #   * ``irc_hard`` IS wired (``:12298`` → ``on_irc_detected_hard``)
            #     and IS raced by the same interruptible tools, but its only
            #     probe ``stream_abort_registry.is_irc_pending`` returns a
            #     hard-coded ``False`` ("v1: always False — no peer message
            #     source wired yet", ``:163-166``).
            #
            # Clearing every event ``interruptible_wait_events`` returns
            # (``:1225`` = external abort + steer_hard + irc_hard) keeps the
            # set/clear pairing complete, so wiring a producer later cannot
            # resurrect the latch bug on a path nobody remembered to clear.
            # The external-abort event is deliberately NOT cleared: an abort is
            # terminal for the turn, not a per-seam signal.
            #
            # Cleared unconditionally (before the empty-texts early return):
            # the flags mean "someone wants attention NOW", and once a seam has
            # been reached that request is satisfied whether or not it produced
            # text (a withdrawn injection sets the flag then leaves the mailbox
            # empty).  ``Event.clear`` is idempotent.
            for _clear_name in (
                "clear_soft_steer",
                "clear_steer_hard",
                "clear_irc_hard",
            ):
                _clear = getattr(handle, _clear_name, None)
                if callable(_clear):
                    _clear()
            if not texts:
                return []
            ...
            for text in texts:
                ...
                if image_refs and store is not None:
                    _inj_fmt, _inj_q = self._cloud_image_cfg()
                    vision_blocks = _resolve_image_refs_to_vision_blocks(
                        store=store,
                        image_refs=image_refs,
                        source_text=text,
                        cloud_format=_inj_fmt,
                        cloud_quality=_inj_q,
                    )
        ...
        async def _on_round_end(
            ...
            nonlocal _pending_integration_count
            ...
            # Learn this model's prompt-cache read-back behaviour from the
            # helper the round-0 site calls (see
            # :meth:`_observe_cache_round`), so the signal can never drift
            # between the two observation points.
            #
            # PER-TURN VERDICT: this only RECORDS the round's signal; the
            # registry is marked once, after this loop returns, by
            # ``_flush_cache_capability`` at the round-0 convergence point.
            # Marking here per round let one multi-round turn spend both of
            # the registry's zero-read credits and condemn a model on its
            # first turn — see ``_TurnBodyState.cache_read_signals``.
            #
            # ROUND NUMBERING (why this cannot double-count round 0): this
            # hook only ever fires for FOLLOW-UP rounds — round 0's stream is
            # drained in the outer ``_run`` before the follow-up loop starts,
            # so its usage never reaches here (see the budget round-0 fold
            # note in ``_run_followup_loop``). Round 0 is observed at its own
            # single convergence point instead.
            self._observe_cache_round(
                model_hint=request.model_hint,
                usage=_round_usage_in,
                round_no=round_no,
            ...
            if has_pending:
                ...
                # A round that did real tool work ends any pending-integration
                # streak: the notice-driven re-opens we are guarding against
                # are CONSECUTIVE text-only re-opens. Resetting here keeps the
                # budget available to a long legitimate turn while still
                # capping a producer that floods notices at a stalled model.
                _pending_integration_count = 0
                ...
                if _repeat_count >= _NO_PROGRESS_LIMIT:
                    ...
                    return RoundEndDecision(stop=True, final_text="")
                return RoundEndDecision()
            # ── Turn-internal SYSTEM_NOTICE integration (§A3) ─────────────
            # No tool calls and no retry: the model is DONE talking, so this
            # is the moment a background completion that landed during the
            # turn would otherwise be stranded until the tab goes idle.
            # Reuse the kernel's ``retry`` seam — it is precisely "reopen one
            # more round with THIS wire, without executing tool calls" — with
            # the folded notices appended, so the model integrates the whole
            # batch into the answer it is already writing.
            _notice_wire = await _integrate_pending_notices(round_no)
            if _notice_wire is not None:
                return RoundEndDecision(retry=True, retry_wire=_notice_wire)
            return RoundEndDecision()
        ...
        # NEW (M5): publish the soft-steer context so tools running INSIDE
        # the kernel (e.g. ``exec``) can discover the abort handle's
        # soft-steer event and the background-process manager via
        # ``qai.platform.turn_soft_steer_ctx.get_turn_soft_steer_ctx()``.
        # No signature threading needed — ContextVars propagate to spawned
        # tasks automatically.  When ``handle`` predates the soft-steer
        # capability (``getattr`` returns None), we skip publishing so
        # pre-M5 handles remain byte-identical.
        _soft_steer_ev = getattr(handle, "soft_steer_event", None)
        if _soft_steer_ev is not None:
            from qai.platform.turn_soft_steer_ctx import (
                TurnSoftSteerCtx,
                _CTX_VAR,
            )
            # Resolve threshold: if a reader was injected (per-turn forge_config
            # lookup) call it here so a UI slider change takes effect on the
            # next turn; else use the static value threaded from DI.  Failures
            # inside the reader fall through to None → ChatSettings → sidecar.
            _resolved_threshold_ms: int | None
            if callable(self._auto_bg_threshold_ms):
                try:
                    _resolved_threshold_ms = await self._auto_bg_threshold_ms()
                ...
                _resolved_threshold_ms = self._auto_bg_threshold_ms
            # Exec 4-way race (Batch 5): same two capabilities the tab-keyed
            # registration publishes in ``_prepare_turn`` — the Stop event and
            # a TAB-BOUND injection ``subscribe`` closure.  ``None`` on either
            # means that race arm is not wired and the consumer skips it.
            _turn_injection_subscribe = None
            if self._injection_signal_bridge is not None:
                _turn_bridge = self._injection_signal_bridge
                _turn_race_tab = tab.id

                def _turn_injection_subscribe() -> tuple[
                    asyncio.Event, Callable[[], None]
                ]:
                    return _turn_bridge.subscribe(_turn_race_tab)
            _CTX_VAR.set(
                TurnSoftSteerCtx(
                    soft_steer_event=_soft_steer_ev,
                    background_process_manager=self._background_process_manager,
                    owner_session_id=tab.id.value,
                    threshold_ms=_resolved_threshold_ms,
                    abort_event=getattr(handle, "abort_event", None),
                    injection_subscribe=_turn_injection_subscribe,
                )
            )
            # NOTE (M5): the tab-keyed registration lives in ``_prepare_turn``
            # (the earliest point every turn passes through) — NOT here.  The
            # first tool_call of a turn is dispatched by
            # ``_handle_initial_tool_call`` (round 0) which never enters this
            # follow-up loop, so registering here missed every round-0 exec.
            # The ContextVar set above is kept for pre-existing consumers that
            # already read it from within this generator's own context.
            # No reset token stored: each turn runs in its own asyncio task
            # with an isolated context, so the ContextVar goes out of scope
            # when the task ends.  Nested turns on the same task are
            # prevented by :class:`ConversationLockedError` at register-time.
        # M5+M6 full alignment: mid-batch peek callbacks.  Every 250ms the
        # kernel's tool-round peek loop asks these whether pending steer /
        # IRC messages exist for THIS tab.  On True, ``on_*_detected``
        # fires the corresponding hard-event on the abort handle so
        # interruptible tools (question / vibe_wait / hub wait) abort
        # promptly and non-interruptible tools observe the cooperative
        # soft-steer signal via :attr:`turn_soft_steer_ctx`.  When the
        # injection registry is None (legacy DI / test stubs) all four
        # closures fall back to no-op so the kernel walks the pre-M5 path.
        _has_steering_peek = None
        _has_irc_peek = None
        _on_steer_detected_hard = None
        _on_irc_detected_hard = None
        if self._injection_registry is not None:
            _reg = self._injection_registry
            _tab_id_peek = tab.id
            _abort_reg = self._abort_registry

            async def _has_steering_peek_impl() -> bool:
                try:
                    return _reg.has_pending(_tab_id_peek)
                except Exception:  # noqa: BLE001
                    return False

            async def _has_irc_peek_impl() -> bool:
                probe = getattr(_abort_reg, "is_irc_pending", None)
                if probe is None:
                    return False
                try:
                    return bool(probe(_tab_id_peek))
                except Exception:  # noqa: BLE001
                    return False

            _has_steering_peek = _has_steering_peek_impl
            _has_irc_peek = _has_irc_peek_impl
            _notify_hard = getattr(handle, "notify_steer_hard", None)
            _notify_irc_hard = getattr(handle, "notify_irc_hard", None)
            _on_steer_detected_hard = _notify_hard if callable(_notify_hard) else None
            _on_irc_detected_hard = _notify_irc_hard if callable(_notify_irc_hard) else None
        async for kev in self._kernel.run(
            ...
            # Kernel convention: 0 = unlimited, >0 = hard cap.
            # Per-turn resolution: user cap from settings overrides the
            # constructor default. Negative constructor value → unlimited (0).
            max_rounds=await self._resolve_effective_max_rounds(),
            ...
            has_steering_peek=_has_steering_peek,
            has_irc_peek=_has_irc_peek,
            on_steer_detected=_on_steer_detected_hard,
            on_irc_detected=_on_irc_detected_hard,
            steer_poll_s=0.25,
            ...
            if isinstance(kev, KernelError):
                ...
                # Context overflow INSIDE the tool loop is not auto-recoverable.
                # By the time this event arrives the kernel has already returned
                # (``drain.error_seen`` ends its round loop), so there is no
                # round left to re-open with a compacted wire — and a mid-turn
                # recovery would have to fold the in-flight, not-yet-persisted
                # tool round-trips into a checkpoint while ``compact_hook``'s
                # rebuild owns that same wire. The turn-internal defences (the
                # every-4th-round compaction and the per-round tool-output
                # aging) are what cover this case instead. Give the user an
                # ACTIONABLE next step rather than a bare failure.
                if (
                    kev.frame is not None
                    and kev.frame.payload.get("code") == "prompt_too_long"
                ):
                    _log.info(
                        "chat.overflow_recovery.skipped",
                        conversation_id=getattr(
                            getattr(conv, "id", None), "value", None,
                        ),
                        reason="mid_tool_loop",
                        round_no=kev.round_no,
                        model_id=request.model_hint or "",
                    )
                    seq += 1
                    yield self._recovery_notice_frame(
                        text=_t("chat.context_recovery.unavailable_mid_tool"),
                        kind="context_recovery_failed",
                    )
                if kev.frame is not None:
                    seq += 1
            ...
            if isinstance(kev, KernelFinished):
                ...
                # Turn-internal notice-integration budget exhausted (§A3):
                # more SYSTEM_NOTICE rows are still pending than this turn is
                # willing to fold in. Tell the user why the turn closes with
                # work outstanding — the rows stay pending, so the
                # coordinator's headless drain picks them up once the tab
                # releases. Emitted as an ASIDE (a small system-note card),
                # NOT a chunk: it is meta-commentary about the turn, not part
                # of the assistant's answer, so it must not land in the
                # persisted assistant text. Precedes the terminal branches
                # below so the turn's real END reason stays authoritative.
                if _pending_integration_limited:
                    seq += 1
                    yield StreamFrame.aside_message(
                        frame_id=f"fu-pending-limit-{seq}",
                        sequence=seq,
                        text=(
                            "后台任务的完成通知仍在陆续到达，本轮已整合"
                            f"{_MAX_PENDING_INTEGRATION} 次，先在此收尾。"
                            "剩余通知会在下一轮自动汇总。"
                        ),
                        round_index=kev.round_no,
                    )
            ...
            if isinstance(kev, KernelMaxRoundsReached):
                ...
                # This path is only reachable when the user configured a
                # positive ``agent.max_rounds`` cap in the settings panel.
                _cap = kev.max_rounds
                seq += 1
                notice = (
                    f"\n\n_(已达到工具循环上限 {_cap} 轮，"
    ...
    def _dispatch_round_as_producer(
        ...
        * a SUBAGENT_START frame (from an ``agent`` spawn) → a
        ...
        number for this tool-execution batch. It lands on the
        ``subagent_start`` frame's payload so the front-end can route
        per-round SUBAGENT_START to a FRESH per-round message — without it,
        two sub-agents spawned in different rounds of the same turn (A round
        0, B round 1) collapse onto the same parent message and B's
        ``index=0`` de-dup drops A.
    ...
    async def _maybe_compress_round(
        ...
        *,
        ...
        try:
            ...
            return compression_token
        # P5 (CONTEXT-COMPRESSION-NEXT v3.1 §7.2 / D11): every time an
        # in-flight (mid-turn) compaction fires we bump the per-conversation
        # counter. The trigger IS "threshold crossed", so N successive fires
        # inside ONE turn means the turn keeps re-tripping the same threshold
        # without converging — the signal P5 uses to expose the "migrate to
        # a fresh conversation" entry (gated to ``>= 3`` inside
        # :meth:`CompactionCheckpointEngine.should_suggest_handoff`). The
        # counter is cleared at the next turn's presend gate (see
        # ``_maybe_presend_compress``); a no-op compaction (``changed`` False)
        # never advances it.
        if changed:
            self._compaction_engine.increment_consecutive_mid_turn(
                self._conv_key(conv),
            )
    ...
    async def _dispatch_pending_tool_calls(
        ...
        * the consolidated agent text still flows back into
        ...
        """
        seq = outcome.seq
        # ── Async spawn contract: ``agent`` tool calls are split out and
        # SPAWNED (t=0 handle + background run) via
        # :meth:`_dispatch_agent_calls_streaming`; everything else executes
        # inline.  The split is gated on the spawn bridge because that is
        # what a spawn requires — with no bridge wired an ``agent`` call is
        # an ordinary tool and goes through ``ToolInvocationPort`` like any
        # other (unchanged legacy behaviour for deployments that never wired
        # the sub-agent stack).
        agent_tc_frames: list[StreamFrame] = []
        other_tc_frames: list[StreamFrame] = []
        if self._sub_agent_bridge is not None:
    ...
    async def _dispatch_agent_calls_streaming(
        ...
        *,
        ...
        """Spawn each ``agent`` tool_call in the background; return at t=0.

        The async spawn contract (N.16): an ``agent`` tool_call is a
        *spawn request*, not a blocking call.  For each call we ask the
        spawn bridge for a :class:`SubAgentHandle` and immediately emit

        1. ``subagent_start(status="background")`` — so the UI allocates a
           card whose id/open-tab/stop affordances work right away
           (``handle.subagent_id`` IS the persisted session id and the
           broadcaster / abort-registry key);
        2. a ``tool_result`` ack carrying the handle text, so the parent
           LLM's tool call is closed on THIS round and the turn converges
           instead of stalling on a sub-agent that may run for minutes.

        The sub-agent's live output reaches the UI through the sub-agent
        stream broadcaster (its own tab), and its RESULT reaches the LLM
        later as a SYSTEM_NOTICE delivered by the async job manager.  So
        this method never drains the sub-agent's event stream, owns no
        merge queue, and cannot block the parent turn — which is what the
        deleted 15s-soft-steer "park race" existed to work around.

        A rejected spawn (recursion cap, isolation failure, …) raises
        :class:`SubAgentError`; we render it into the ack ``tool_result``
        with ``ok=False`` and emit NO ``subagent_start`` (there is no
        sub-agent to show a card for).  A rejection is never fatal to the
        turn: the LLM reads the reason and decides what to do next.

        ``tool_results`` is mutated in place (one entry per agent call), so
        the parent agentic loop continues exactly as for any other tool.
        """
        seq = seq_start
        total = len(agent_tc_frames)

        if self._sub_agent_bridge is None:
            # No spawn capability wired: report it as an ordinary failed tool
            # call rather than silently dropping the LLM's request.
            for tc_frame in agent_tc_frames:
                async for fr in self._emit_agent_ack(
                    tc_frame=tc_frame,
                    tool_results=tool_results,
                    seq=seq,
                    text=(
                        "[sub-agent error: sub-agent spawning is not"
        ...
        # Whether the sub-agents spawned THIS turn may themselves spawn
        # (grand) sub-agents.  Driven by the per-tab "allow_child_spawn"
        # switch the SSE/WS layer placed in ``request.extra`` (default off →
        # the historical hard recursion guard): it controls ONLY the main
        # agent's direct children, so the tree stops at the next level unless
        # the user opts that level in too.
        allow_child_spawn = bool((request.extra or {}).get("allow_child_spawn"))
        # Per-session ("this conversation only") + persona-mandated tool
        # blocks, so a sub-agent inherits the user's SessionToolsPopover
        # choices instead of silently regaining a tool the user switched off.
        ...
        for idx, tc_frame in enumerate(agent_tc_frames):
            ...
            if not isinstance(args, dict):
                args = {}
            spawn_request, extra = self._build_spawn_request_from_tool_call(
                args=args,
            ...
            try:
                sub_handle = await self._sub_agent_bridge.spawn(
                    conv_id=conv.id.value,
                    tab_id=tab.id.value,
                    parent_depth=parent_depth,
                    # A main-agent turn is the ROOT of the sub-agent tree, so
                    # there is no parent AGENT type to inherit from.
            ...
            _log.info(
                ...
                job_id=sub_handle.job_id,
                conversation_id=conv.id.value,
                agent_type=spawn_request.agent_type,
                parent_depth=parent_depth,
            )
            yield StreamFrame.subagent_start(
                frame_id=f"sa-start-{seq}",
                sequence=seq,
                index=idx,
                total=total,
                prompt_preview=spawn_request.prompt[:500],
                subagent_id=sub_handle.subagent_id,
                subagent_type=spawn_request.agent_type,
                name=extra.get("subagent_name"),
                round_index=round_index,
                status="background",
            )
            seq += 1
            async for fr in self._emit_agent_ack(
                ...
                tool_results=tool_results,
                seq=seq,
                text=self._render_spawn_ack(sub_handle),
                ok=True,
            ):
                yield fr
            seq += 1

    def _build_spawn_request_from_tool_call(
        self,
        *,
        args: dict[str, Any],
        ...
        total_agents: int,
        model_hint: str | None,
    ) -> tuple[SubAgentSpawnRequest, dict[str, Any]]:
        """Split one ``agent`` tool call into (typed request, passthrough bag).

        The typed :class:`SubAgentSpawnRequest` carries the per-spawn
        CAPABILITIES (which profile, what prompt, may it spawn children).
        Everything else the sub-agent runner needs is per-dispatch PLUMBING
        (which tab aborted, which round dispatched it, which tools the user
        switched off) and travels in the ``extra`` bag the bridge splats onto
        ``iter_events`` — keeping the frozen domain type from growing a field
        per wiring detail.

        Every ``extra`` key is included only when actually set, so a light
        ``SubAgentEventStreamPort`` test double need not declare the optional
        kwargs.  All parsing is defensive: a malformed value degrades to the
        default rather than failing the turn.
        """
        raw_prompt = args.get("description") or args.get("prompt") or ""
        prompt = raw_prompt if isinstance(raw_prompt, str) else ""
        raw_type = args.get("subagent_type")
        agent_type = (
            raw_type if isinstance(raw_type, str) and raw_type else "general"
        )
        ...
        # Wake handle: the LLM may continue a sub-agent it spawned earlier so
        # it keeps its prior memory.  An invalid id is ignored (fresh spawn)
        # rather than failing the turn.
        raw_resume = args.get("resume_subagent_id")
        if isinstance(raw_resume, str) and raw_resume:
            try:
                extra["resume_session_id"] = SubAgentSessionId(raw_resume)
            except (ValueError, TypeError):
                pass
        # Human-readable card title the LLM chose (falls back to
        # ``SubAgent N`` in the UI when absent).
        raw_name = args.get("name")
        if isinstance(raw_name, str) and raw_name:
            extra["subagent_name"] = raw_name
        if disabled_tools:
            extra["disabled_tools"] = disabled_tools
        if tool_mode:
            extra["tool_mode"] = tool_mode
        if workspace_root:
            extra["workspace_root"] = workspace_root
        if round_index is not None:
            extra["parent_round_index"] = round_index
        # The main agent is the direct parent, and it lives outside the
        # sub-agent table — hence no parent row to point at.
        extra["parent_subagent_id"] = None
        # Parent ⏹ perception: the sub-agent's round loop observes the PARENT
        # tab's cooperative-abort predicate, so a Stop on this tab reaches
        # every sub-agent it spawned.
        extra["parent_abort_check"] = handle.is_set
        # Per-tool cancel: lets the sub-agent honour a ``cancel_tool``
        # targeting one of ITS OWN in-flight calls without tripping the
        # whole-turn Stop.  ``getattr`` so a handle stub that predates the
        # capability degrades to "no per-tool cancel" instead of raising.
        consume_cancel = getattr(handle, "consume_cancel_tool", None)
        if consume_cancel is not None:
            extra["parent_consume_cancel_tool"] = consume_cancel
        return spawn_request, extra

    @staticmethod
    def _render_spawn_ack(sub_handle: SubAgentHandle) -> str:
        """The tool-result text a successful spawn returns to the parent LLM.

        Deliberately states the three things the model needs to behave well
        with an async handle: the id it can act on, that the work is running
        (not finished, not failed, not empty), and the two ways the result
        can arrive.  Without the "do not assume it failed" wording models
        routinely narrate an empty result to the user.
        """
        return (
            f"[Started sub-agent {sub_handle.subagent_id}. It is running in"
            " the background — this is NOT its result and NOT a failure."
            " Its completion will arrive automatically on a later turn with"
            " no user action required. Continue with your other work; if you"
            " need the result before you can proceed, call"
            f' sub_agent(action="wait", id="{sub_handle.subagent_id}").]'
        )

    async def _emit_agent_ack(
        self,
        *,
        tc_frame: StreamFrame,
        tool_results: list[dict[str, Any]],
        ...
        """Close one ``agent`` tool call with ``text`` as its result.

        Appends the ``tool_results`` entry the parent agentic loop feeds to
        the next LLM round AND yields the paired ``TOOL_RESULT`` frame that
        persistence needs.  ``tool_call_id`` is carried on both so the agent
        card pairs to its call at persist time — without it the card's
        ``output`` persists as ``None`` and the whole tool_call is dropped
        from the next turn's wire (the "main agent forgets it dispatched a
        sub-agent" amnesia bug).
        """
        args = tc_frame.payload.get("arguments") or {}
        if not isinstance(args, dict):
            args = {}
