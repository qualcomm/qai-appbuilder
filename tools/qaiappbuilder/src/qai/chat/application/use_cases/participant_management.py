...
class SetDiscussionConfigInput:
    ...
    #: Coordinator (主持人) unification — tail-appended (§3.1). The coordinator is
    #: the single user-configurable moderator + judge identity (现象2 + feature).
    #: ``coordinator_model_id`` chooses the model the coordinator (both the
    #: manager moderator AND the final judge turn) uses; resolution ladder at
    #: orchestrator read time: this → roster[0].model_id → default_model_id.
    #: ``coordinator_prompt`` is advisory extra text appended to the built-in
    #: judge directive (and manager prompt). Both ``None`` leaves any existing
    #: value untouched (PATCH semantics); a missing key resolves to the built-in
    #: default at read time. The DEPRECATED ``judge_participant_id`` is still
    #: read for back-compat (legacy conversations keep their old judge).
    coordinator_model_id: str | None = None
    coordinator_prompt: str | None = None
    #: Coordinator per-turn tools/skills override — tail-appended (§3.1). A
    #: dict with optional ``allowed_tools`` (list[str]) and ``enabled_skills``
    #: (list[str]) keys. When either key is a list, it wins over the dynamic
    #: default computed at ``_resolve_judge`` synthesis time (all registered
    #: tools minus ``agent`` + every globally enabled skill). ``None`` (the
    #: request field, whole dict) leaves the persisted override untouched
    #: (PATCH semantics); an EXPLICIT ``{}`` clears the override so the
    #: dynamic default reapplies. Persisted under meta key
    #: ``coordinator_config``.
    ...
    async def execute(
        ...
        if request.is_discussion is False:
            ...
            for _knob in (
                ...
                # Coordinator (主持人) unification — tail-appended (§3.1). Both
                # are str knobs (meta key == attr name, 1:1) so they ride the
                # same explicit-value whitelist; ``None`` ⇒ untouched (PATCH).
            ...
            # Coordinator tools/skills override (dict — outside the scalar
            # whitelist above). ``None`` leaves the persisted override
            # untouched (PATCH semantics); an EXPLICIT ``{}`` clears the
            # override so the dynamic default reapplies at read time. Only
            # ``allowed_tools`` / ``enabled_skills`` are honoured; unknown
            # keys are stripped defensively so a rogue client cannot smuggle
            # arbitrary state into ``meta["discussion"]``.
            if request.coordinator_config is not None:
                _cc: dict[str, Any] = {}
                _tools = request.coordinator_config.get("allowed_tools")
                _skills = request.coordinator_config.get("enabled_skills")
                if isinstance(_tools, list):
                    _cc["allowed_tools"] = [
                        t for t in _tools if isinstance(t, str)
                    ]
                if isinstance(_skills, list):
                    _cc["enabled_skills"] = [
                        s for s in _skills if isinstance(s, str)
                    ]
                discussion["coordinator_config"] = _cc
...
# ---------------------------------------------------------------------------
# Generate implementation plan (DISC-1 二期-step2, §22.4)
# ---------------------------------------------------------------------------
@dataclass(frozen=True, slots=True, kw_only=True)
...
class GenerateImplementationPlanUseCase:
    """DEPRECATED — return the current plan WITHOUT running the LLM extractor.

    DISC-1 coordinator-plan-rework (spec ``local://coordinator-plan-spec.md``):
    plan generation moved OFF this synchronous POST path (which fed the whole
    discussion summary into an 8-second :class:`LlmFeatureItemExtractor` turn —
    a deep discussion reliably timed the POST out and mislabelled the run
    failed).  The "生成实施方案" button now sends a preset ``@主持人`` message down
    the ordinary discussion send/SSE path; the coordinator writes the plan
    document and calls the ``implementation_plan`` tool.  This use case (and
    its ``…/implementation/generate`` route) is kept ONLY for wire-compat: it
    returns the conversation's CURRENT discussion blob (the existing plan, if
    any) and NEVER invokes the extractor.  The injected ``planner`` is retained
    so DI wiring is unchanged, but it is no longer called.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._conversations = conversations
        self._participants = participants
        # Retained for DI-wiring compat only; DEPRECATED — never invoked (the
        # coordinator generates the plan via the discussion path instead).
        self._planner = planner
        self._clock = clock

    async def execute(
        self, request: GenerateImplementationPlanInput
    ) -> dict[str, Any] | None:
        # DEPRECATED no-op: DO NOT run the LLM extractor (the 8-s POST path is
        # gone — the coordinator generates the plan on the discussion turn).
        # Return the conversation's current discussion blob so the route's
        # response shape is unchanged for any legacy caller still hitting it.
        conv = await _require_conversation(
            self._conversations, request.conversation_id
        )
        return conv.discussion
