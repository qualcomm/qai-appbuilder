...
class RoundEndDecision:
    ...
    about — usage accumulation, empty-completion retry, and the no-progress
...
#: ``aside_hook(round_no) -> list[wire content]`` — non-interrupting side-channel
#: seam (M6).  Fires at the SAME inter-round boundary as :data:`InjectHook`,
#: right after the user-injection drain.  Producers (typically the
#: background-process aside bridge) deliver system-generated notifications
#: here; the kernel folds each as a synthetic ``role:user`` message tagged
#: ``meta["aside"] = True`` on the current turn's wire so the model can react
#: on its next round.  ``None`` (default) → no-op; sub-agent / discussion /
#: legacy callers see byte-identical behaviour.
AsideHook = Callable[[int], Awaitable[list[str | list[dict[str, Any]]]]]
...
class SingleAgentTurnKernel:
    ...
    async def run(
        ...
        *,
        ...
        aside_hook: "AsideHook | None" = None,
        # M5+M6 full alignment: mid-batch 250ms peek callbacks + poll interval.
        # ``has_steering_peek`` returns True iff there is a pending mid-turn
        # injection to fold in; ``has_irc_peek`` returns True iff a peer/IRC
        # message needs handling.  When either fires, the kernel fires the
        # matching hard event on ``interruptible_signal_provider(handle)``
        # so *interruptible* tools abort promptly.  All None ⇒ no peek.
        has_steering_peek: Callable[[], Awaitable[bool]] | None = None,
        has_irc_peek: Callable[[], Awaitable[bool]] | None = None,
        on_steer_detected: Callable[[], None] | None = None,
        on_irc_detected: Callable[[], None] | None = None,
        steer_poll_s: float = 0.25,
        ...
        * ``on_round_end(round_no, end_payload, round_text, tool_calls)``
          ...
          stop (usage accumulation happens as a side
          effect inside the hook).
        * ``grow_wire_hook(round_no, assistant_text, tool_metas, finals)``
        ...
        """
        # ``max_rounds == 0`` means UNLIMITED (no artificial cap); the loop
        # runs until the model finishes (no tool calls), is aborted, or hits
        # the no-progress circuit breaker.  A positive value caps the loop.
        _unlimited = max_rounds == 0
        _upper = max_rounds if not _unlimited else 2**31
        for round_no in range(1, _upper + 1):
            ...
            # ②-bis (M6): non-interrupting side-channel drain.  Runs AFTER
            # the user-injection loop above so producer-owned notifications
            # (background-process completions etc.) land AFTER any pending
            # user text on this round's wire.  Guarded on abort_check per the
            # same "state-truth-first" invariant as inject_hook: an external
            # abort MUST NOT strand aside entries — the caller's aside adapter
            # keeps them pending for the next live turn.  ``None`` (default)
            # → no-op, byte-identical to pre-M6 behaviour.
            if aside_hook is not None and (
                abort_check is None or not abort_check()
            ):
                for aside_content in await aside_hook(round_no):
                    aside_msg: dict[str, Any] = {
                        "role": "user",
                        "content": aside_content,
                        "meta": {"aside": True},
                    }
                    if assistant_timestamp is not None:
                        aside_msg["created_at"] = assistant_timestamp()
                    wire_messages.append(aside_msg)
            ...
            # completion retry / no-progress. Fires
            ...
            async for ev in self._execute_tool_round(
                ...
                finals_out=finals,
                has_steering_peek=has_steering_peek,
                has_irc_peek=has_irc_peek,
                on_steer_detected=on_steer_detected,
                on_irc_detected=on_irc_detected,
                steer_poll_s=steer_poll_s,
            ):
                yield ev
            ...
            # When ``_unlimited`` (max_rounds == 0) the cap never fires — the
            # loop is only bounded by model convergence / abort / no-progress.
            if not _unlimited and round_no == max_rounds:
    ...
    async def _execute_tool_round(
        ...
        *,
        ...
        # M5+M6 full alignment: spawn a mid-batch peek task that polls the
        # caller-provided ``has_steering_peek`` and ``has_irc_peek`` every
        # ``steer_poll_s`` seconds.  On first truthy result, fires the
        # matching ``on_*_detected`` callback (typically ``handle.notify_steer_hard``
        # / ``handle.notify_irc_hard``) so interruptible tools abort promptly.
        # Idempotent (both callbacks are idempotent no-ops after the first
        # fire).  When both peek callbacks are None, no task is spawned —
        # byte-identical to pre-M5+M6 behaviour.
        import asyncio as _asyncio
        import contextlib as _contextlib

        _peek_stop = _asyncio.Event()

        async def _peek_loop() -> None:
            if has_steering_peek is None and has_irc_peek is None:
                return
            fired_steer = False
            fired_irc = False
            while not _peek_stop.is_set():
                try:
                    await _asyncio.wait_for(
                        _peek_stop.wait(), timeout=steer_poll_s,
                    )
                    return  # stop signalled
                except _asyncio.TimeoutError:
                    pass
                # Steering peek
                if not fired_steer and has_steering_peek is not None:
                    try:
                        pending = await has_steering_peek()
                    except Exception:  # noqa: BLE001 — peek must not break tool round
                        pending = False
                    if pending:
                        fired_steer = True
                        if on_steer_detected is not None:
                            try:
                                on_steer_detected()
                            except Exception:  # noqa: BLE001
                                pass
                # IRC peek
                if not fired_irc and has_irc_peek is not None:
                    try:
                        pending = await has_irc_peek()
                    except Exception:  # noqa: BLE001
                        pending = False
                    if pending:
                        fired_irc = True
                        if on_irc_detected is not None:
                            try:
                                on_irc_detected()
                            except Exception:  # noqa: BLE001
                                pass
                # Both fired → loop can exit early.
                if fired_steer and fired_irc:
                    return
                # If only one channel is wired and it fired, done.
                if fired_steer and has_irc_peek is None:
                    return
                if fired_irc and has_steering_peek is None:
                    return

        _peek_task: _asyncio.Task[None] | None = None
        if has_steering_peek is not None or has_irc_peek is not None:
            _peek_task = _asyncio.create_task(
                _peek_loop(), name=f"kernel-peek-r{round_no}",
            )
        ...
        try:
            ...
            # M5+M6: reap the peek task on every exit path so it never
            # lingers after the tool round returns.
            if _peek_task is not None:
                _peek_stop.set()
                if not _peek_task.done():
                    _peek_task.cancel()
