...
#: Group-address terms (现象3) — phrases that address the WHOLE discussion group
#: ("各角色 / 大家 / 所有人 / everyone") rather than a single @mentioned role.
#: Combined with a question signal in an awaiting_user / active_discussion state
#: these mark an AUDIENCE-WIDE follow-up that is eligible for the grey-zone LLM
#: to escalate into a FULL re-discussion (§22A.5 extension). Substring match
#: (CJK terms are not whitespace-delimited); scanned via :func:`contains_any`.
GROUP_ADDRESS_TERMS: dict[str, frozenset[str]] = {
    "en": frozenset(
        {
            "everyone",
            "everybody",
