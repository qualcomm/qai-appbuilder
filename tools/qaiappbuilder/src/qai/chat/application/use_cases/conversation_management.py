...
class DeleteConversationUseCase:
    ...
    def __init__(
        ...
        *,
        ...
        clock: Clock,
        events: EventBus | None = None,
        on_delete: "Callable[[ConversationId], Awaitable[None]] | None" = None,
    ) -> None:
        self._conversations = conversations
        self._clock = clock
        self._events = events
        # Optional cleanup hook run BEFORE the conversation row is removed —
        # wired by the composition root to stop + delete the conversation's
        # scheduled tasks (so they never keep firing against a gone
        # conversation). Best-effort: a hook failure must not block the delete.
        self._on_delete = on_delete

    async def execute(self, request: DeleteConversationInput) -> None:
        if self._on_delete is not None:
            try:
                await self._on_delete(request.conversation_id)
            except Exception:  # noqa: BLE001 — cleanup never blocks the delete
                _log.warning(
                    "chat.conversation_delete_cleanup_failed",
