...
class SpeakerSelectionState:
    ...
    * ``terminated`` — a hard stop signal (user "喊停"); any selector returns
    ...
    def eligible(self) -> list[Participant]:
        ...
        The synthetic coordinator (participant id ``__coordinator__`` — see
        :data:`~qai.chat.application.use_cases.orchestrate_discussion._COORDINATOR_PARTICIPANT_ID`)
        is a first-class @-mentionable role but is NEVER a spontaneous speaker:
        the round-robin / manager selectors must not rotate to it on their own.
        We drop it here so ordinary rounds always pick a real roster member.
        When the caller has already scoped ``state.participants`` down to ONLY
        the coordinator (the @coordinator directed path — the orchestrator's
        mention-scoping filter narrows the state to the mentioned ids), we
        return that scoped roster verbatim so selection resolves to the
        coordinator instead of dead-ending on an empty eligible list.
        """
        named = named_agents(self.participants)
        non_coord = [p for p in named if p.id.value != "__coordinator__"]
        return non_coord if non_coord else named
