...
# ``bytes_before × ratio`` which is unrelated to the window). USER-CONFIGURABLE
# via the settings panel; this is only the default. 0.35 of a 200K window = 70K.
COMPRESS_TARGET_WINDOW_RATIO: float = 0.35

# ── Context-overflow recovery targets ─────────────────────────────────────
# When the provider REJECTS a request for exceeding its context window
# (``prompt_too_long``), the ordinary compaction target has by definition
# already failed to keep the wire inside the window — re-running compaction
# with the SAME target would reproduce the SAME wire (the compressor is a
# deterministic drop-only pass), so the recovery pass MUST aim strictly
# lower. It scales the user's effective target instead of hard-coding an
# absolute ratio, so "more aggressive than normal" holds for ANY configured
# ``COMPRESS_TARGET_WINDOW_RATIO`` (including a user-tuned one).
#
# Scale 0.5 against the 0.35 default yields 0.175 of the window (35K on a
# 200K window). The remaining ~0.08 headroom absorbs the two costs the
# compressor cannot touch: the fixed runtime overhead (system prompt + tool
# schemas + persona + skill instructions, measured at 15-25K on tool-heavy
# sessions) and the current user turn, which is protected verbatim.
OVERFLOW_RECOVERY_TARGET_SCALE: float = 0.5

# Absolute floor for the scaled recovery target. Guards the degenerate case
# where a user pinned an already-tiny target (e.g. 0.02 → scaled 0.01): the
# recovery pass would then aim at a size that cannot hold the protected
# current turn, wasting the one recovery attempt on a guaranteed no-op.
OVERFLOW_RECOVERY_MIN_TARGET_RATIO: float = 0.15
...
# Long-turn prefix summarisation threshold as a fraction of the PROTECTED
# window (``budget × PROTECT_WINDOW_RATIO``). When a SINGLE turn's rolling
# tokens exceed ``LONG_TURN_PREFIX_RATIO × PROTECT_WINDOW_RATIO × budget``
# the compaction path fires the P3 turn-prefix summariser at the atomic
# group boundary just before this turn's user message
# (:meth:`streaming.py._compute_turn_prefix_at_boundary`). Extracted as a
# named constant (P3-2, Task O) so tuning "when is a turn long enough to
# summarise?" is one-source and no longer sprinkled as a magic ``0.6`` next
# to :data:`PROTECT_WINDOW_RATIO` in ``streaming.py``. For the default 0.35
# protect ratio: ``0.6 × 0.35 = 0.21`` of the model context window (42 000
# tokens on a 200 K window, 6720 on a 32 K window).
LONG_TURN_PREFIX_RATIO: float = 0.6

# Default sub-agent round budget (the main loop's budget is configured
# separately via ``StreamChatUseCase`` / runtime settings — V1 parity).
# ``0`` = unlimited (kernel convention); the no-progress circuit breaker
# prevents true runaway loops. Profile-level overrides (e.g. EXPLORE=5)
# take precedence. Production DI explicitly passes ``max_rounds=0``.
DEFAULT_SUB_AGENT_MAX_ROUNDS: int = 0
...
class CompactionCheckpoint:
    ...
    * CCD-5 (PENDING-WORK.md §1): this checkpoint is now PERSISTED to sqlite
    ...
    # P1.b (CONTEXT-COMPRESSION-NEXT §5.2): bounded, deduplicated ledger of
    # file / URL / command references extracted from the tool calls the model
    # issued during the compacted head. Persisted to sqlite as
    # ``reference_ledger_json`` (migration 059) so it survives a process
    # restart alongside the rest of the checkpoint. ``None`` when the
    # conversation has not yet accumulated any references, or when loaded
    # from a pre-migration-059 row (the wire-assembly path then treats the
    # ledger as absent — byte-for-byte identical to pre-P1.b behaviour).
    # Optional appended field (tail-append, AGENTS.md §3.1).
    reference_ledger: ReferenceLedger | None = None
    # P2 (CONTEXT-COMPRESSION-NEXT §6): opaque, model-generated "session
    # digest" — a structured Markdown summary of the compacted head (the
    # messages folded into ``compacted_wire`` before every subsequent
    # verbatim increment). Written asynchronously by
    # :class:`qai.chat.application.use_cases.refresh_digest.RefreshDigestUseCase`
    # after a successful compaction; the wire-assembly path injects a
    # ``[Session Digest]`` system message BEFORE the ledger block when this
    # is non-empty, so the model always sees the digest right next to the
    # user's latest turn. Persisted to sqlite as ``digest_text`` /
    # ``digest_updated_at`` (migration 060) so it survives a process restart
    # alongside the rest of the checkpoint. ``None`` when the digest has
    # not yet been generated (first compaction is asynchronous; the write
    # lands on a later turn) or when loaded from a pre-migration-060 row —
    # both cases collapse to "no digest block on the wire", byte-for-byte
    # identical to pre-P2 behaviour. Optional appended fields (tail-append,
    # AGENTS.md §3.1).
    digest_text: str | None = None
    digest_updated_at: str | None = None
    # P3 (CONTEXT-COMPRESSION-NEXT §7.1): opaque JSON payload holding at most
    # THREE per-turn "turn-prefix" summaries. Each entry is
    # ``{"user_message_id": str, "summary_text": str, "created_at": str}``,
    # appended when a single turn's rolling wire crosses the long-turn
    # threshold (``0.6 × PROTECT_WINDOW_RATIO × context_limit``); FIFO cap at
    # three so the checkpoint's row size stays bounded no matter how many
    # tool-heavy turns pile up. Persisted to sqlite as
    # ``turn_prefix_summaries_json`` (migration 062). ``None`` when no
    # long-turn summary has landed yet or the row predates migration 062 —
    # both collapse to "no ``[Turn Prefix Summary]`` block on the wire",
    # byte-for-byte identical to pre-P3 behaviour. Optional appended field
    # (tail-append, AGENTS.md §3.1).
    turn_prefix_summaries_json: str | None = None
    # Task R (concurrency root-cause refactor): monotonically increasing
    # version tag stamped on every ``maybe_compress`` result. The generation
    # is the SINGLE authority a Compare-And-Swap writer checks:
    #
    #   * ``save_core`` refuses to update a row whose stored generation is
    #     >= the incoming one — so two racing compactors NEVER blindly
    #     overwrite each other, and a stale in-flight save cannot resurrect
    #     state after ``/compact clear`` dropped the row (a fresh compaction
    #     starts at generation 1 again, matching the row that came back).
    #   * Fragment writers (``save_digest`` / ``save_turn_prefix``) carry the
    #     ``core_generation`` they observed at ``load`` time; a mismatch on
    #     the core row is a stale write and the fragment is refused.
    #   * The engine's in-memory ``replace_digest_fragment`` /
    #     ``replace_turn_prefix_fragment`` compare the incoming ``core_gen``
    #     to the frozen checkpoint they are trying to update — the checkpoint
    #     is IMMUTABLE (``frozen=True``), so a matched update is expressed
    #     via ``dataclasses.replace`` and lands a brand-new frozen instance.
    #
    # Legacy rows loaded from a pre-migration-065 database default to 1
    # (matching the SQL column DEFAULT); the persistence writer stamps this
    # value on every save so old and fresh rows are indistinguishable to the
    # CAS. See ``docs/qai-db-schema.md`` and migration 065.
    generation: int = 1

#: Byte-count threshold above which :func:`estimate_wire_tokens` upgrades
#: from the coarse ``bytes/4`` heuristic to a real tiktoken BPE pass over
#: the text fragments (images stay on the byte-equivalent path). Sized so
#: the trigger-side pre-send loop keeps its fast path on tiny wires and pays
#: the BPE cost only when the wire is genuinely large enough to matter.
BPE_THRESHOLD_BYTES: int = 2000

#: Upper cap on the BPE pass — above this **total** wire size we stay on the
#: coarse ``bytes/4`` heuristic even though the wire is over
#: ``BPE_THRESHOLD_BYTES``. Tiktoken's BPE is O(n) with a small constant on
#: heterogeneous English text, and above ~2 MB the estimate is already in
#: "definitely-triggering" territory where extra precision buys no compaction
#: decision (the ``PROMPT_TOO_LONG`` retry is the safety net either way).
#: 2 MB covers the full 200K-token context-window envelope (a 200K-token wire
#: ≈ 800 KB of UTF-8) so BPE precision is preserved across the full CJK-dense
#: mid/large-wire band where ``bytes/4`` would otherwise under-count by
#: 20-40%. Task M P1-9: raised from 256 KB to 2 MB after unifying trigger and
#: compressor口径 (Task L P0-1) — pathological single-fragment runs are now
#: guarded per-fragment via :data:`_BPE_PER_FRAGMENT_MAX_BYTES` below rather
#: than by capping the whole wire.
_BPE_MAX_BYTES: int = 2 * 1024 * 1024

#: Per-fragment safety cap on the BPE pass. Above this per-fragment size a
#: single text fragment stays on the coarse ``bytes/4`` heuristic even when
#: the wire's total is inside ``_BPE_MAX_BYTES``. cl100k_base BPE degrades
#: badly on pathological patterns — a long run of the SAME character (e.g.
#: 400 KB of ``'x'`` in an over-sized tool-call argument) can push CPU into
#: seconds because the merge loop iterates over the repeating pair. Per-
#: fragment protection is preferred over a global cap because a well-formed
#: 800 KB CJK conversation is split across many message fragments (each a
#: few KB of real text) and stays inside precision, while a rogue
#: single-fragment blob falls through to the fast口径 without dragging the
#: rest of the wire down with it.
_BPE_PER_FRAGMENT_MAX_BYTES: int = 256 * 1024


def overflow_recovery_target_ratio(normal_target_ratio: float) -> float:
    """Return the compaction target a context-overflow recovery pass MUST use.

    ``normal_target_ratio`` is the target that was already in force when the
    provider rejected the wire (the user's configured
    ``COMPRESS_TARGET_WINDOW_RATIO``, resolved per compaction).

    INVARIANT: the result is ALWAYS strictly less than ``normal_target_ratio``.
    That is the entire reason a recovery retry can succeed where the first
    attempt did not — the compressor is a deterministic drop-only pass, so
    re-running it at an equal (or higher) target reproduces the very wire the
    provider just rejected.

    Ordinarily the result is ``normal × OVERFLOW_RECOVERY_TARGET_SCALE``.
    :data:`OVERFLOW_RECOVERY_MIN_TARGET_RATIO` then raises that back up when
    the scaled value would be uselessly small — but only while it stays under
    ``normal``; a floor at or above the failed target would violate the
    invariant, so in that regime the plain scaled value wins.

    Pure arithmetic (single source of truth shared by the retry policy and the
    streaming recovery path); never raises. A non-finite or non-positive input
    degrades to the floor rather than propagating a bad ratio into the
    compressor.
    """
    try:
        normal = float(normal_target_ratio)
    except (TypeError, ValueError):
        return OVERFLOW_RECOVERY_MIN_TARGET_RATIO
    if not math.isfinite(normal) or normal <= 0.0:
        return OVERFLOW_RECOVERY_MIN_TARGET_RATIO
    scaled = normal * OVERFLOW_RECOVERY_TARGET_SCALE
    if normal > OVERFLOW_RECOVERY_MIN_TARGET_RATIO:
        # Floor is usable — lift a too-small scaled target up to it.
        return max(OVERFLOW_RECOVERY_MIN_TARGET_RATIO, scaled)
...
def estimate_wire_tokens(
    ...
    *,
    ...
    Single口径 used by every main-loop compaction TRIGGER. NOT the user-facing
    /context number (that uses the provider-measured cloud running counter
    ``conv.full_history_tokens``).

    Two-tier estimate for correctness × speed:

    * **Fast path** — ``total_bytes <= BPE_THRESHOLD_BYTES`` returns
      ``total_bytes // 4``, the same coarse heuristic the reference tokenizer
      uses by default (aligned with ``Buffer.byteLength / 4``). UTF-8 bytes
      (not Python chars) so multi-byte texts (CJK, emoji) are not
      under-counted by 2-3×.
    * **Precise path** — larger wires (``BPE_THRESHOLD_BYTES < total_bytes
      <= _BPE_MAX_BYTES``) tokenize each text fragment through the offline
      tiktoken encoder and SUM the per-fragment token counts (P1-8: no
      ``"\n".join`` — every fragment is already an independent piece of
      wire, joining would inject phantom ``\n`` tokens the model never
      sees). Image / audio parts stay on the byte-equivalent path
      (``NON_TEXT_PART_BYTE_EQUIVALENT // 4`` per part, ≈1200 tokens/image),
      since tiktoken cannot count binary tiles. P1-6: fragments are only
      collected in a second pass, AFTER the byte-based fast-path guard —
      the vast majority of chat turns skip the list-append work entirely.

    Multimodal content lists have their text parts contribute their real
    UTF-8 byte length; every non-text part
    (image_url / input_image / input_audio / etc.) contributes
    :data:`NON_TEXT_PART_BYTE_EQUIVALENT` — delegated to
    :func:`non_text_content_bytes` so every byte-based estimator on the
    compaction path applies the SAME image-cost rule. ``len(str(content))``
    on the raw list would base64-inflate a data-URL image to millions of
    chars → premature trigger, or drop an external URL to ~100 chars →
    missed trigger.

    A tokenizer failure (vendored tiktoken absent, unknown encoding, etc.)
    silently degrades to the fast path so the compaction trigger never
    throws in the hot loop — the PROMPT_TOO_LONG retry stays as the safety
    net either way.
    """
    # First pass: byte-only accounting (fast path answers here for the
    # vast majority of chat turns — no fragment list built).
    total_bytes = 0
    image_bytes_extra = 0
            ...
            for part in content:
                if isinstance(part, dict):
                    if part.get("type") == "text":
                        total_bytes += len(part.get("text", "").encode("utf-8"))
                    else:
                        total_bytes += NON_TEXT_PART_BYTE_EQUIVALENT
                        image_bytes_extra += NON_TEXT_PART_BYTE_EQUIVALENT
    ...
    if total_bytes <= BPE_THRESHOLD_BYTES or total_bytes > _BPE_MAX_BYTES:
        # Fast path (small wire) or safe fallback (huge wire) — no BPE.
        return total_bytes // 4
    # Second pass: collect fragments only when tiktoken will actually be
    # invoked, avoiding wasted list-appends on the hot small-wire path.
        ...
        for tc in m.get("tool_calls") or ():
            if isinstance(tc, dict):
                ...
                if isinstance(fn, dict):
                    ...
                    if not isinstance(args, str):
                        args = str(args)
                    text_fragments.append(args)
    # P1-8: sum per-fragment BPE instead of ``"\n".join``-then-encode. Every
    # fragment is already an independent piece of wire (a message content
    # or a tool_call arguments string); joining with newlines would inject
    # extra ``\n`` tokens the provider never sees. Per-fragment BPE is
    # byte-identical to what the model will tokenize on receive.
    try:
        enc = _get_encoder(_tiktoken_encoding_name(model_hint))
        if enc is not None:
            precise = 0
            for f in text_fragments:
                fb = len(f.encode("utf-8"))
                # Pathological single-fragment runs (e.g. 400 KB of the same
                # character) can force cl100k_base BPE into seconds; degrade
                # that fragment alone to bytes/4 so the well-behaved rest of
                # the wire still gets precise tokenization.
                if fb > _BPE_PER_FRAGMENT_MAX_BYTES:
                    precise += fb // 4
                else:
                    precise += len(enc.encode(f))
            return int(precise) + image_bytes_extra // 4
    except Exception:  # noqa: BLE001 — tokenizer never breaks the hot loop
        pass
    return total_bytes // 4
...
def tool_result_already_truncated(raw_result: Any) -> bool:
    ...
    * ``truncated=False`` — the result is WHOLE but may still exceed the
      ...
      page, a 35KB ``grep`` result, a large ``web_fetch`` body). Splitting it
    ...
    design and replaces the earlier "only skip when truncated is truthy", which
      let whole-but-large ``read`` / ``list`` / ``grep`` / ``web_fetch`` results
...
def tool_result_slice_start(raw_result: Any) -> int:
    """Return the 1-based coordinate the tool's slice STARTS at (1 when N/A).

    Ordered-slice tools (``read`` / ``list``) report where their window begins
    (``start_line`` / ``start_entry``). The truncator's hard-cap backstop needs
    it: when that backstop tail-cuts an oversized slice it must name the
    ``offset`` to resume from, and a count of kept lines is only an offset if
    the slice began at line 1. Without this, a slice from
    ``read(path, offset=800)`` was told to continue at ~offset 516 — backwards,
    into already-seen content, so the read looped instead of advancing.

    Defensive: only positive ints are honoured; anything else (missing field,
    string, ``None``, a non-slice tool) degrades to ``1``, which is both the
    common case and the prior behaviour.
    """
    if isinstance(raw_result, dict):
        for field in ("start_line", "start_entry"):
...
def truncate_tool_result(
    ...
    *,
    ...
    ``slice_start_line`` is where the tool's slice BEGINS in its own address
    space (``read``'s ``start_line``); see :func:`tool_result_slice_start`. The
    ordered-slice hard-cap backstop needs it to name a correct continuation
    ``offset`` when it tail-cuts an oversized slice.
    ...
    result = truncator.truncate(
        ToolResultTruncationRequest(
            ...
            slice_start_line=slice_start_line,
        ),
    )
...
def age_old_tool_outputs(
    ...
    *,
    ...
    """
    # Per-message token estimate reuses the SAME bytes-based口径 as
    # ``estimate_wire_tokens`` (content + tool_calls args, tiktoken-precise
    # above ``BPE_THRESHOLD_BYTES``) by wrapping the single message in a
    # one-element list — one authoritative estimator, no drift between the
    # aging gate and the compaction trigger.
...
async def maybe_compress_wire(
    ...
    *,
    ...
    # to tiktoken BPE above ``BPE_THRESHOLD_BYTES``. Small wires stay on the
    # bytes/4 fast path; the PROMPT_TOO_LONG retry remains the safety net.
    ...
    try:
        # NOTE: ``used`` here is a bytes-based token ESTIMATE (upgraded to
        ...
        # use its fallback bytes/token factor honestly. The main loop
