...
backfill + terminal retention eviction + background-task ownership +
terminal-snapshot fallback), kept entirely inside the ``qai.chat`` context
(§3.2 — no reuse of another context's ``RunFrame`` / ``RunStreamEntry``).
...
* **Terminal retention eviction**: terminal entries are GC'd lazily
  ``_RETENTION_SECONDS`` after the terminal mark, on the next
  :meth:`register` call. LIVE entries are never evicted.
...
* **Terminal double-track fallback**: when no in-memory entry exists
  (server restarted / retention window elapsed) :meth:`replay` reads the persisted
...
#: Retention window for TERMINAL sub-agent entries in the broadcast registry.
#: After the terminal mark the entry stays around at least this long so a
#: *reconnecting* consumer (tab reload / WS drop / re-subscribe) still finds
#: the just-finished run's frame buffer and can backfill it from its
#: ``from_seq`` cursor. Terminal entries older than this are GC'd lazily on the
#: next :meth:`register`. A LIVE entry is never evicted regardless of this
#: value, so shortening it cannot truncate a running sub-agent.
#:
#: 30s deliberately matches
#: :attr:`qai.chat.adapters.async_job_manager.AsyncJobManager.retention_seconds`
#: (same default, same name): both express the SAME policy — "keep a terminal
#: result around 30s so a reconnecting consumer can still read it, then treat
#: it as a dead letter". The alignment is intentional, not coincidence; change
#: them together.
#:
#: Past this window :meth:`replay` degrades to the persisted
#: :class:`SubAgentSession` snapshot (status + result), which is what the
#: client's own REST snapshot fetch already renders — so the only thing a
#: late subscriber loses is a per-frame replay of a transcript it has already
#: painted. That is why this is 30s rather than the App Builder buffer's 10min.
_RETENTION_SECONDS: float = 30.0
...
class SubAgentStreamBroadcaster:
    ...
    def __init__(
        ...
        *,
        ...
        # How long a TERMINAL entry is retained for reconnecting consumers.
        # Same name + default as ``AsyncJobManager.retention_seconds`` on
        # purpose (see :data:`_RETENTION_SECONDS`).
    ...
    def register(self, subagent_id: str) -> SubAgentStreamEntry:
        ...
        Lazily evicts terminal entries past their retention window so
        ...
        **Sequence monotonicity across runs (resume)** — a new entry
        ...
        that was already evicted or the new run that just began), never an echo
        ...
        """
        now = time.monotonic()
        # Capture the sequence-lifetime cursor BEFORE the eviction pass —
        # otherwise a resume that arrives after this sub-agent's OWN prior
        # entry has aged out of retention would evict that entry (line below),
        # see `prior=None`, and restart sequences at 0. A client that still
        # holds `last_applied_seq` from the prior run (e.g. a sub-agent tab that
        # stayed open across the retention window) would then send
        # `from_seq=<big>` on its next re-subscribe and the replay would
        # silently swallow every new-run frame until the counter caught up — a
        # "resume tab stays frozen" bug that the docstring above promises NOT to
        # have. `_streams.get` is O(1); the extra lookup costs nothing.
        ...
        expired = [
            ...
            and now - entry.terminal_at > self._retention_seconds
        ]
        for sid in expired:
        ...
        # when the prior entry has aged out of retention — resume across a long
    ...
    async def replay(
        ...
        *,
        ...
        registry has no entry (server restarted / retention elapsed) the generator
