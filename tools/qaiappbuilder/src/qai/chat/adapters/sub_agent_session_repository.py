...
class SqliteSubAgentSessionRepository:
    ...
    def database(self) -> "Database":
        """The engine behind this repository — see the conversation repository."""
        return self._db
    ...
    async def save(
        ...
        * for an existing row, UPDATE only when the stored ``version`` still
        ...
        ``conn`` lets a serialised writer supply a connection it holds across
        many transactions (see
        :meth:`Database.lease_persistent_connection`), removing the
        per-transaction connection setup.  ``None`` leases a fresh one.
        """
        common = (
        ...
        try:
            async with write_transaction(self._db, conn) as tx:
                ...
                if row is None:
                    ...
                    await upd.close()
                    if affected == 0:
                        # ``write_transaction`` rolls back on the way out,
                        # so the CAS loser leaves neither the row nor the
                        # aggregate's ``version`` touched — which is what
                        # makes replaying a lock-timed-out save safe.
...
class QueuedSubAgentSessionRepository:
    """Routes every :meth:`save` through the conversation write serialiser.

    Why a decorator and not a change at each call site
    ==================================================

    ``chat_subagent_session`` rows take the SAME file write lock as
    ``chat_message``, with their own ``BEGIN IMMEDIATE``.  While the
    conversation writes were serialised per conversation by
    :class:`~qai.chat.adapters.conversation_write_queue.ConversationWriteQueue`
    and the session writes were not, concurrent conversations contended
    directly on the one file and exceeded ``busy_timeout`` — the session
    row was then simply ABSENT, so a sub-agent reported an error notice
    with no session left to inspect.

    Every session belongs to exactly one conversation through
    ``root_conversation_id``, so submitting the save under that key puts it
    in the SAME per-conversation worker as the conversation's message
    writes: the two are mutually exclusive by construction, no retry loop
    involved.

    Wrapping the port rather than editing each caller is what makes that
    property hold for ALL of them — the spawn path's early / per-round /
    finalize saves, the take-over turn's CAS save, and the model-change
    REST route — and keeps a future caller from re-introducing the race by
    reaching for the raw repository.  Reads pass straight through: SQLite
    readers do not take the write lock (WAL), so serialising them would
    only add latency.
    """

    __slots__ = ("_inner", "_queue")

    def __init__(
        self,
        *,
        inner: "SubAgentSessionRepositoryPort",
    ...
    async def save(self, session: SubAgentSession) -> None:
        """Save through the conversation's write worker; await durability.

        Relays the repository's outcome unchanged — including
        :class:`SubAgentSessionConflictError`, which the take-over path
        reloads and replays from.
        """
        future = await self._queue.submit_save_subagent_session(
            session.root_conversation_id.value, session
        )
        await future
    ...
    async def find(
        self, session_id: SubAgentSessionId
    ) -> SubAgentSession | None:
        return await self._inner.find(session_id)

    async def delete(self, session_id: SubAgentSessionId) -> None:
        await self._inner.delete(session_id)

    async def list_by_root_conversation(
        self, root_conversation_id: ConversationId
    ) -> tuple[SubAgentSession, ...]:
        return await self._inner.list_by_root_conversation(
            root_conversation_id
        )

    async def list_by_parent_subagent(
        self, parent_subagent_id: SubAgentSessionId
    ) -> tuple[SubAgentSession, ...]:
        return await self._inner.list_by_parent_subagent(parent_subagent_id)

    async def delete_by_parent(
        self, root_conversation_id: ConversationId
    ) -> int:
        return await self._inner.delete_by_parent(root_conversation_id)
