...
* :data:`RetryCategory.PROMPT_TOO_LONG` — the upstream LLM rejected the
  request because the prompt exceeded its context window.  Recoverable
  EXACTLY ONCE: the policy returns ``should_retry=True`` together with a
  ``compress_target_ratio`` that is strictly more aggressive than the
  ordinary compaction target, and the caller compresses the history to that
  target before re-issuing.  A second overflow on the same turn is terminal
  (the caller's own single-shot gate stops there), because a repeat pass
  with identical inputs cannot shrink a deterministic drop-only compressor
  any further — what remains over the window is the fixed overhead plus the
  protected current turn, neither of which compaction may drop.
...
class DefaultStreamRetryPolicy(RetryPolicyPort):
    ...
    normal_target_ratio: float = COMPRESS_TARGET_WINDOW_RATIO
    """Ordinary post-compaction target (fraction of the model context window)
    that was in force when a ``PROMPT_TOO_LONG`` rejection arrived. The
    recovery branch scales it DOWN to derive its own, strictly-lower target;
    the default mirrors the compaction engine's default so a caller that
    cannot resolve the user's live preference still gets a sane ratio."""
    ...
    def next_attempt(
        ...
        *,
        ...
        if category is RetryCategory.PROMPT_TOO_LONG:
            # Recoverable ONCE. The provider rejected the wire for exceeding
            # its context window, so re-sending it unchanged is hopeless — but
            # compressing the HISTORY to a strictly lower target first changes
            # the wire, and that is a retry which can genuinely succeed.
            #
            # This policy owns the "how aggressively" number, not the "how
            # many times": the caller keeps a single-shot gate because only it
            # can observe whether the compression pass actually changed
            # anything (a no-op pass means a retry would burn a request for a
            # byte-identical wire). ``attempt_number`` is therefore always 1
            # here in practice; a caller that asks again still gets a ratio,
            # and its own gate is what stops the loop.
            #
            # ``compress_target_ratio`` is a fraction of the MODEL CONTEXT
            # WINDOW derived from :attr:`normal_target_ratio` (the target that
            # was already in force when the rejection happened). A caller that
            # can resolve the user's LIVE configured target should recompute
            # via the same ``overflow_recovery_target_ratio`` helper so a
            # user-tuned target is honoured; this value is the fallback when
            # no live preference is resolvable.
            return RetryDecision(
                should_retry=True,
                delay_seconds=0.0,
                compress_target_ratio=overflow_recovery_target_ratio(
                    self.normal_target_ratio
                ),
