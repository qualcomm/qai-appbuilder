...
class HttpPromptEnhancer(PromptEnhancerPort):
    ...
    async def _resolve_endpoint(
        ...
        """Pick (base_url, api_key, model, params) for this enhance request.

        ``params`` is the resolved model's catalog ``params`` dict, carried
        out so the caller can set the QGenie metering ``User-Agent``: an
        enhance call must be billed to the same quota bucket as the
        conversation it serves, not silently to the other one.
    ...
    async def enhance(
        ...
        base_url, api_key, model, model_params = await self._resolve_endpoint(
            request
        )
        if not base_url:
            return None
        ...
        # QGenie picks which of two independent daily allowances to bill from
        # this header alone; without it an enhance call always lands in ``Api``
        # while the gauge may say ``IDE/CLI`` is in use. No-op elsewhere.
