...
from qai.chat.application.use_cases.tool_advertise import order_tool_names


__all__ = ["RegistryBackedToolInvocation", "ToolHandler"]
...
class RegistryBackedToolInvocation:
    ...
    def schemas(self) -> tuple[dict[str, Any], ...]:
        ...
        Order is deterministic and follows
        :data:`~qai.chat.application.use_cases.tool_advertise.TOOL_ORDER`
        (most-used,
    ...
    async def invoke(
        ...
        # A handler may return a fully-formed ``ToolInvocationResult`` when it
        # needs to carry structured metadata (e.g. a user-fixable
        # ``error_code`` such as ``chat.llm.tls_cert_untrusted`` from
        # ``web_search`` behind an enterprise TLS gateway) alongside the
        # rendered ``result`` text. Respect it verbatim rather than re-wrapping
        # (which would nest the envelope and drop the code).
