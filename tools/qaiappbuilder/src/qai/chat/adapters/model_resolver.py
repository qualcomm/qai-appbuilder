        ...
        if self._lookup is not None:
            ...
            if endpoint is not None and endpoint.base_url:
                ...
                return ResolvedModel(
                    ...
                    # Carried through so the transport can emit a "sign in
                    # again" error instead of an unsatisfiable "set your API
                    # key" one when an SSO provider has no credential.
                    is_sso=endpoint.is_sso,
                )
