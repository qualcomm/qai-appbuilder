...
class InMemoryStreamAbortRegistry:
    ...
    def notify_soft_steer(self, tab_id: TabId) -> bool:
        """Signal soft-steer on a tab's handle so long-running tools can yield.

        Additive helper (AGENTS.md §3.1: append-only).  Independent of
        :meth:`abort` — does NOT tear the turn down; it merely raises the
        cooperative "soft steer" flag on the tab's handle so tools like
        ``exec`` may voluntarily hand off long-running work to the
        background-process manager and return a synthetic "backgrounded"
        result promptly, letting the mid-turn user input reach the model
        at the next inter-round seam.  Called by the ``inject`` control
        frame handler after successfully recording the injection.
        Returns ``False`` when no handle is registered, or when the
        handle predates the soft-steer capability.
        """
        record = self._records.get(tab_id.value)
        if record is None:
            return False
        notify = getattr(record.handle, "notify_soft_steer", None)
        if notify is None:
            return False
        notify()
        return True

    def notify_steer_hard(self, tab_id: TabId) -> bool:
        """Signal hard-steer on a tab's handle (interruptible-tool abort).

        Independent of :meth:`abort` — does NOT tear the turn down.
        Getattr-probed so pre-hard-steer stubs remain compatible.
        """
        record = self._records.get(tab_id.value)
        if record is None:
            return False
        notify = getattr(record.handle, "notify_steer_hard", None)
        if notify is None:
            return False
        notify()
        return True

    def is_irc_pending(self, tab_id: TabId) -> bool:
        """Return True iff there is a peer/IRC message pending for this tab.

        v1: always False (no peer message source wired yet — reserved for
