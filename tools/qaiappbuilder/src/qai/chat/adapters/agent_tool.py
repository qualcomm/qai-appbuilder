...
#   * ``sub_agent`` — excluded because sub-agent orchestration (list /
#     status / inspect / wait / stop) is a main-agent-only decision.
#     A child sub-agent inspecting its siblings would leak sibling
#     context into the child run.
...
# ``_SUB_AGENT_EXCLUDED_TOOLS`` (agent / question / sub_agent /
# scheduled_task) and ``_CONDITIONAL_TOOL_NAMES`` (appbuilder_run /
# appbuilder_batch_run) now live in the SHARED application-layer helper
# ``tool_advertise`` (single source of truth — previously duplicated here
# and in ``streaming``). Imported above with their historical private
...
def _sub_agent_tool_schemas(
    ...
    *,
    ...
    (read / write / edit / exec / web_fetch / glob / grep / apply_patch …)
    ...
    * ``agent`` is removed by default (recursion guard) — UNLESS
      ...
      too).
    * ``question`` is removed unless ``allow_question=True`` (the spawn-time
      grant on ``SubAgentSpawnRequest``): its blocking dialog is not reliably
      reachable from a background sub-agent, so the default stays off and only
      an explicit per-spawn opt-in re-advertises it.  Mirrors the take-over
      loop's ``allow_question`` un-exclusion in ``streaming`` — ONE 口径.
    ...
    (read/glob/grep/web_fetch/list). The filter runs LAST so it also
    ...
    # ``allow_spawn`` / ``allow_question`` are per-spawn UN-exclusions: the
    # sub-agent set drops ``agent`` and ``question`` by default, and each
    # grant puts exactly one of them back.  The take-over loop derives its
    # ``excluded`` set the SAME way (see ``streaming._collect_tool_schemas``),
    # so the two paths share one口径.  ``sub_agent`` (orchestration API) and
    # ``scheduled_task`` stay excluded either way — both are main-agent-only
    # decisions with no per-spawn grant.
...
class AgentToolHandler:
    ...
    def __init__(
        ...
        *,
        ...
        max_subagent_recursion_depth: int = 2,
        ...
        # ---- Best-effort JSONL journal for sub-agent compaction events ----
        # Optional ``CompactionJournal`` sink piped into this handler's
        # internal :class:`CompactionCheckpointEngine` so sub-agent compaction
        # emits the SAME one-line-per-event JSONL the main agent emits
        # (CONTEXT-COMPRESSION-NEXT §5.1). Fire-and-forget — the sink swallows
        # exceptions so a sub-agent turn never fails because of observability.
        # ``None`` (default, legacy / unit stubs) disables emission —
        # byte-for-byte the prior behaviour. Wired at ``_chat_di.py`` from
        # the SAME journal instance the main use case receives, so main +
        # sub-agent events land in one file for cross-referencing.
        journal: "CompactionJournal | None" = None,
        # Unified spawn bridge (:class:`SubAgentBridge`).  When the sub-agent's
        # own LLM calls the ``agent`` tool, :meth:`execute` delegates to this
        # bridge, which admits the spawn, launches it in the BACKGROUND and
        # hands back a handle at once — so a spawning agent is never blocked on
        # its child.  ``None`` (legacy / unit stubs) makes ``execute`` fall
        # back to running the sub-agent inline to completion.
        bridge: "Any | None" = None,
        # This handler's own recursion depth: 0 for the MAIN agent's handler,
        # so a sub-agent it spawns is depth 1.  Threaded into
        # ``bridge.spawn(parent_depth=...)`` so the preflight ceiling is
        # measured from the real position in the spawn tree.
        parent_depth: int = 0,
        # BackgroundJobDispatcher wiring (§6.4 refactor): when a sub-agent
        # run reaches a terminal state (DONE / ERROR / INTERRUPTED), the
        # handler publishes :class:`SubAgentSessionTerminated` on this
        # bus so the process-scoped
        # :class:`BackgroundJobDispatcher` can append a SYSTEM_NOTICE
        # to the parent conversation and notify the coordinator to
        # resume the main agent if it has already unwound.  ``None``
        # (legacy / unit stubs) disables emission — byte-for-byte the
        # prior behaviour, but the main-agent-still-parked-when-sub-
        ...
        # that depth reaches ``max_subagent_recursion_depth`` — a diagnostic
        # ``[error: max sub-agent nesting depth (N) reached]`` string is
        # returned to the spawning LLM instead of another recursion. This
        # replaces the old
        ...
        # DI wires this from ``Settings.max_subagent_recursion_depth``
        # (default 2); a clamp to >= 1 keeps a degenerate 0/negative value from
        # silently blocking every first-level spawn.
        self._max_subagent_recursion_depth = max(
            1, int(max_subagent_recursion_depth)
        )
        # Shared per-model prompt-cache-capability registry (SAME instance as
        # the main agent, via DI). The per-round aging gate in
        # ``_open_round_stream`` consults ``aging_enabled`` before calling
        # ``age_old_tool_outputs``; ``_on_round_end`` writes the round's actual
        # cache-read volume back via ``mark`` so the registry learns whether
        # this model reads cache back. ``None`` (legacy / unit stubs) → the gate
        # falls back to unconditional aging (prior behaviour, byte-for-byte).
        ...
        self._compaction_engine = CompactionCheckpointEngine(
            ...
            key_prefix="subagent:",
            journal=journal,
            # Sub-agent runs stay on the pre-Phase-0 compaction shape
            # (byte-equivalent). The engine defaults this to ``True`` for the
            # main agent; a sub-agent is a deliberate, explicitly-declared
            # exception, so the opt-out is stated here rather than relying on
            # a default. See ``_compaction_engine.__init__``'s note on why the
            # engine and compressor defaults intentionally differ.
        ...
        self._bridge = bridge
        self._parent_depth = max(0, int(parent_depth))
        self._event_bus = event_bus

    async def execute(self, request: ToolInvocationRequest) -> str:
        """Spawn a sub-agent and return its handle, or run it inline.

        With a spawn bridge wired (production), this ADMITS the sub-agent and
        returns immediately: the run continues in the background and its result
        is delivered later, so a spawning agent is never blocked on its child.
        The returned string names the handle and the two follow-up actions, so
        the model can collect the result or cancel without inventing an id.

        Without a bridge (legacy callers, unit stubs, the scheduled-task
        runner) it falls back to draining :meth:`iter_events` to completion and
        returning the consolidated final text — byte-for-byte the historical
        behaviour.

        A :class:`SubAgentError` is rendered into the returned string rather
        than raised: this method IS the registered ``agent`` tool handler, and
        the tool contract is "return the text the model sees".  Raising would
        surface as an infrastructure tool failure and lose the recovery hint,
        whereas ``render_for_llm`` gives the model a typed, actionable
        rejection in the same shape every other sub-agent failure uses.
        """
        if self._bridge is not None:
            return await self._spawn_via_bridge(request)
        text_parts: list[str] = []
    ...
    async def _spawn_via_bridge(self, request: ToolInvocationRequest) -> str:
        """Admit a background sub-agent; return the handle text for the model."""
        try:
            handle = await self._bridge.spawn(
                conv_id=request.conversation_id.value,
                tab_id=request.tab_id.value,
                parent_depth=self._parent_depth,
                # ``None`` is correct for every caller that reaches HERE: this
                # method backs the REGISTERED ``agent`` tool, which only the
                # main agent is advertised (``_SUB_AGENT_EXCLUDED_TOOLS`` keeps
                # ``agent`` off a sub-agent's tool list).  A sub-agent that was
                # explicitly opted into spawning does not come through here — it
                # re-enters ``iter_events`` from ``_execute_tool_call`` — so
                # there is no sub-agent type to guard against self-spawn.
                parent_agent_type=None,
                spawn_request=self._build_spawn_request(request.arguments),
            )
        except SubAgentError as err:
            return err.render_for_llm()
        return (
            f"[Started sub-agent {handle.subagent_id}. It runs in the "
            f"background; call sub_agent(action=wait, id={handle.subagent_id}) "
            f"to get its result or sub_agent(action=stop, "
            f"id={handle.subagent_id}) to cancel it.]"
        )

    @staticmethod
    def _build_spawn_request(arguments: dict[str, Any]) -> SubAgentSpawnRequest:
        """Turn the LLM's raw ``agent`` arguments into the typed request.

        Replaces the free-form dict the spawn path used to thread: the
        capabilities a spawn may grant are now an enumerable, greppable
        contract rather than a bag of keys nobody can list.
        """
        prompt = arguments.get("description") or arguments.get("prompt") or ""
        ...
        return SubAgentSpawnRequest(
            ...
            prompt=str(prompt),
            allow_child_spawn=bool(arguments.get("allow_spawn", False)),
            allow_question=bool(arguments.get("allow_question", False)),
    ...
    async def iter_events(
        ...
        *,
        ...
        allow_spawn: bool = False,
        # Per-spawn grant for the blocking ``question`` dialog.  Default off:
        # a background sub-agent has no reliable path to the user's answer, so
        # only an explicit grant on the spawn request re-advertises the tool.
        allow_question: bool = False,
        ...
        # Pre-minted sub-agent id (unified spawn path).  The spawn bridge must
        # hand a usable id back to its caller BEFORE the run starts, so it
        # mints one and passes it here for the fresh session to adopt.  One id
        # then serves as the spawn handle, the persisted session id, the
        # broadcaster key and the abort-registry key.  ``None`` (legacy /
        # direct callers) keeps the historical behaviour: the id is minted
        # inside ``_resolve_session``.  Ignored on a resume, which already has
        # an id.
        preassigned_subagent_id: SubAgentSessionId | None = None,
        ...
        * ``{"type": "subagent_error", "index": N, "message": "..."}``
        ...
        already ``>= self._max_subagent_recursion_depth`` the caller
        (``_execute_tool_call``) emits a diagnostic string and never even gets
        here — recursion 封顶
        ...
        try:
            async for event in self._iter_loop(
                ...
                preassigned_subagent_id=preassigned_subagent_id,
            ):
                _publish(event)
                yield event
            ...
            if (
                ...
                try:
                    ...
                    # Also publish ``SubAgentSessionTerminated`` so the
                    # BackgroundJobDispatcher notifies the main agent
                    # that this run reached a (forced) terminal state.
                    # Same guards as the normal path in
                    # :meth:`_finalize_session`.
                    if self._event_bus is not None:
                        try:
                            await asyncio.shield(
                                self._event_bus.publish(
                                    SubAgentSessionTerminated(
                                        parent_tab_id=request.tab_id,
                                        parent_conversation_id=(
                                            _session.root_conversation_id
                                        ),
                                        subagent_id=_session.id.value,
                                        subagent_name=(
                                            _session.title
                                            or _session.subagent_type
                                        ),
                                        status="interrupted",
                                        result_text="",
                        ...
                        except Exception:  # noqa: BLE001 — never mask unwind
                            pass
    ...
    async def _iter_loop(
        ...
        *,
        ...
        async def _compact_hook(
            ...
            # P0-2 (Task O): extract references from the sub-agent's own
            # in-flight wire before compaction so the engine's ledger tracks
            # tool_call arguments the sub-agent touched. Ephemeral (no
            # persist_id) but still surfaced on the sub-agent's own wire via
            # the shared ``[References Ledger]`` block.
        ...
        async def _on_round_end(
            ...
            if isinstance(_usage, dict):
                ...
                # NOTE: sub-agent DOES NOT mark the shared
                # ProviderCacheCapabilityRegistry. The registry holds a
                # per-model *capability* judgment — "does this provider read
                # cache back?" — and is populated by the main agent's
                # per-TURN max signal
                # (streaming.py::_flush_cache_capability). Sub-agent rounds
                # are a poor signal source for that judgment:
                #
                #   • A sub-agent ``execute`` builds a fresh, short wire
                #     (field log: 2709 prompt tokens on the first round). A
                #     brand-new wire has never been cached, so cache_read=0
                #     is STRUCTURALLY guaranteed on the opening round — it
                #     says nothing about the provider's capability.
                #   • The shared registry has a small zero-read tolerance
                #     before it flips unknown→no_cache (single-writer credit
                #     accounting). If the sub-agent spends credits with its
                #     structurally-zero opening rounds, the main agent's
                #     same-model turns can be permanently condemned even
                #     though they routinely read tens of thousands of cached
                #     tokens back (field log: main-agent sticky_hit rounds
                #     at 27K-129K cache_read AFTER the sub-agent's opening
                #     zero flipped the state).
                #   • The sub-agent's rounds are per-ROUND, the main agent's
                #     are per-TURN (max-folded). Mixing two granularities
                #     into one credit pool is asymmetric on its face and
                #     has no justifiable session-boundary substitute here
                #     (a sub-agent invocation is not a "turn").
                #
                # Sub-agent remains a REGISTRY CONSUMER at the aging gate
                # above (:2056-2058) — the capability judgment is a property
                # of the model / gateway, not of the agent type, so
                # following the main agent's verdict is physically correct.
                # If a future scenario ever needs a sub-agent to contribute
                # to capability judgment, introduce a SEPARATE registry
                # with an explicit session boundary; do not re-enable
                # marking against the shared one.
        ...
        # Detach the PARENT tab's injection arm for the whole sub-agent run: a
        # spawned sub-agent is an independent run, not one of the parent's
        # parallel tool calls, so the parent's Ctrl+Enter must not win the exec
        # race of a child THIS agent started. Everything else in the soft-steer
        # context (threshold, abort event, bg manager) stays inherited.
        from qai.platform.turn_soft_steer_ctx import (
            detach_injection_arm_for_subagent,
        )

        with detach_injection_arm_for_subagent():
        ...
        # §N.54 typed-failure tail (2026-08-08): classify a run that ended
        # badly so the front-end can render a TYPED error card (icon +
        # colour + recovery hint per kind) instead of a bare red string.
        # The two signals are already computed above for session
        # finalisation, so this adds no new derivation — it just maps them
        # onto the SAME :class:`SubAgentErrorKind` vocabulary the spawn
        # path (preflight / isolation) uses, giving the UI ONE closed set
        # of kinds across every failure origin.
        #
        # ``interrupted`` wins over ``had_error``: a user-stopped run whose
        # last round happened to also error is, to the user, a stop.
        _fail_kind: SubAgentErrorKind | None = None
        if interrupted:
            _fail_kind = SubAgentErrorKind.INTERRUPTED
        elif had_error:
            _fail_kind = SubAgentErrorKind.EXECUTION
        if _fail_kind is not None:
            _fail = SubAgentError(
                kind=_fail_kind,
                message=(
                    "Sub-agent was stopped before it finished."
                    if interrupted
                    else "Sub-agent hit an error during one of its rounds."
                ),
                subagent_id=subagent_id,
                recovery_hint=(
                    "Re-dispatch it if you still need the task done."
                    if interrupted
                    else (
                        "Check the round error above; narrow the task or "
                        "re-dispatch with a corrected prompt."
                    )
                ),
            )
            done_event["error_kind"] = _fail.kind.value
            done_event["error_message"] = _fail.message
            # ``recovery_hint`` is Optional on the domain type but always
            # supplied on both branches here.
            if _fail.recovery_hint is not None:
                done_event["recovery_hint"] = _fail.recovery_hint
        yield done_event
    ...
    async def _resolve_session(
        ...
        * ``prior_wire`` — the persisted wire history to seed this run with
        ...
        session = SubAgentSession.start(
            # The spawn bridge's pre-minted id wins when present: it has
            # already been handed to the spawning caller, so minting a second
            # id here would leave the handle pointing at a session that does
            # not exist.
            session_id=(
                preassigned_subagent_id
                if preassigned_subagent_id is not None
                else SubAgentSessionId(self._ids.new_id())
            ),
    ...
    async def _finalize_session(
        ...
        *,
        ...
        # Emit ``SubAgentSessionTerminated`` for the BackgroundJobDispatcher
        # to pick up.  Guarded on the caller having wired both a bus AND a
        # parent tab id — legacy unit stubs pass neither, so this stays a
        # no-op for them while production DI always supplies both.  Best
        # effort: a bus-publish failure never blocks the run.
        if self._event_bus is not None and parent_tab_id is not None:
            try:
                status_name = (
                    "interrupted" if interrupted
                    else "error" if had_error
                    else "done"
                )
                await self._event_bus.publish(
                    SubAgentSessionTerminated(
                        parent_tab_id=parent_tab_id,
                        parent_conversation_id=session.root_conversation_id,
                        subagent_id=session.id.value,
                        subagent_name=(
                            session.title or session.subagent_type
                        ),
                        status=status_name,
                ...
                _log.warning(
                    "chat.agent_tool.session.event_publish_failed",
                    error=str(exc),
                    error_type=type(exc).__name__,
                    subagent_id=session.id.value,
                )
    ...
    def _truncate_sub_tool_result(
        ...
        *,
        ...
        slice_start = _kernel_tool_slice_start(tool_result)
        ...
        if tool_name in ("read", "list", "skill"):
            ...
            # Rebase onto where the slice STARTED: a kept-line count is only an
            # offset for a slice beginning at line 1. Without this, cutting a
            # ``read(path, offset=800)`` slice pointed the model BACKWARDS into
            # content it already had, so the retry re-fetched the same window.
            next_offset = slice_start + kept_lines
            advice = (
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        ``self._max_subagent_recursion_depth`` (from
        ``Settings.max_subagent_recursion_depth``): the moment a call would
        exceed the ceiling this method returns a
        ...
        # code path the main agent uses. Recursion 封顶 =
        # ``self._max_subagent_recursion_depth`` (from Settings).
        # State-mutation safety: an EXPLORE (read-only) profile
        ...
        if tool_name == "agent":
            ...
            # spawn attempt.
            if spawn_depth >= self._max_subagent_recursion_depth:
                return (
                    f"[error: max sub-agent nesting depth "
                    f"({self._max_subagent_recursion_depth}) reached — refuse "
                    "to spawn another level. Continue the task inline or "
                    "return a "
            ...
            # ``self._max_subagent_recursion_depth`` regardless of
            # ``allow_spawn`` votes.
