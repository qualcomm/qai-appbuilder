...
#: Error-code markers that mean "the allowance is gone", not "slow down".
#: Mirrors ``llm_stream._QUOTA_EXHAUSTED_KEYWORDS`` (kept as a separate
#: literal there to avoid an infrastructure→adapters import). Narrow on
#: purpose: these are machine-emitted codes (OpenAI ``insufficient_quota``,
#: QAI Service ``quota_exhausted``), NOT the looser ``"quota exceeded"``
#: prose in _THROTTLING_INCLUSIONS above, which is Google/Azure wording for
#: a per-minute rate limit and genuinely retryable.
...
def classify_api_error(
    ...
    * ``"quota_exhausted"``  — the account's allowance is spent (resets on the
      provider's billing period, so retrying can never succeed — terminal)
    ...
    * ``"unknown"``          — unclassifiable
    ...
    if status_code is not None:
        # 429 is ambiguous: a rate limit (retry helps) or an exhausted
        # allowance (retry can never help — it resets on the provider's
        # billing period). The body's error code disambiguates; mirrors
        # ``llm_stream._classify_http_error`` so the two never drift.
        if status_code == 429:
            if any(kw in message for kw in _QUOTA_EXHAUSTED_KEYWORDS):
