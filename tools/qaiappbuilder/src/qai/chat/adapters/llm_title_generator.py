...
class HttpLLMTitleGenerator(TitleGeneratorPort):
    ...
    async def _resolve_endpoint(
    ...
    ) -> tuple[str | None, str | None, str, dict[str, object] | None]:
        """Pick (base_url, api_key, model, params) for this title request.

        ``params`` is the resolved model's catalog ``params`` dict, carried
        out so the caller can set the QGenie metering ``User-Agent`` — a title
        request must be billed to the same quota bucket as the conversation
        it titles, not silently to the other one.
        ...
        * **Real model id** — we only use the resolver's endpoint when it
        ...
        if self.model_resolver is not None and request.model_id:
            try:
                ...
                if resolved.is_local:
                    ...
                    return (None, None, request.model_id, None)
                if resolved.base_url:
                    return (
                        ...
                        or request.model_id,
                        resolved.params
                        if isinstance(resolved.params, dict)
                        else None,
                    )
        ...
        if _is_loopback_endpoint(self.base_url):
            ...
            return (None, None, request.model_id or self.model_name, None)
        _log.info(
    ...
    async def generate(
        ...
        base_url, api_key, model_name, model_params = (
            await self._resolve_endpoint(request)
        )
        if not base_url:
            _log.info(
        ...
        # based on this header alone. Without it a title request lands in the
        # ``Api`` bucket regardless of which one the user chose, so the gauge
        # would show ``IDE/CLI`` in use while part of the spend accrued to the
        # other side — and the switch decision would rest on incomplete
        # numbers. A no-op for every non-QGenie endpoint.
        apply_qgenie_user_agent(headers, base_url, model_params)
