...
from qai.platform.qgenie_user_agent import (
...
_CHAT_CONTROL_KEYS: frozenset[str] = frozenset(
    {
        ...
        "_prompt_debug",
        "_show_prompt_in_ui",
        # Skill body text separated from the system prompt for prompt-cache
...
def _estimate_prefix_tokens(items: list) -> int:
    """BPE-free bytes/4 token estimate for a list of wire messages or tool dicts.

    Mirrors :func:`_maybe_estimate_usage`'s UTF-8 bytes/4 heuristic (P1-3,
    Task O — was ``chars/4`` before, which under-counted CJK / emoji by 2-3×
    and could let a short Chinese prefix slip past the 1024-token Anthropic
    cache_control minimum, wasting a breakpoint). Used only as a guard
    against wasting Anthropic cache_control breakpoints on prefixes shorter
    than the 1024-token minimum (方案C).
    """
    total = 0
    for item in items:
        if not isinstance(item, dict):
            continue
        # Tool schema: count description + parameters JSON (bytes).
        fn = item.get("function")
        if isinstance(fn, dict):
            total += len(str(fn.get("description") or "").encode("utf-8"))
            total += len(str(fn.get("parameters") or "").encode("utf-8"))
        # Message: count content + tool_calls arguments (bytes).
        c = item.get("content")
        if isinstance(c, str):
            ...
            for block in c:
                if isinstance(block, dict):
                    total += len(str(block.get("text") or "").encode("utf-8"))
        for tc in item.get("tool_calls") or ():
            if isinstance(tc, dict):
                f = tc.get("function")
                if isinstance(f, dict):
                    total += len(str(f.get("arguments") or "").encode("utf-8"))
    return total // 4
...
#: Markers that turn a 429 from "slow down" into "your allowance is gone".
#: A rate limit clears in seconds (retry helps); an exhausted quota resets on
#: the provider's billing period (retry can NEVER succeed), so the two need
#: opposite handling. Kept narrow and machine-generated on purpose: these are
#: error *codes* emitted by OpenAI-compatible gateways (OpenAI's
#: ``insufficient_quota``; the QAI Service broker's ``quota_exhausted``), NOT
#: free prose. Deliberately does NOT include the looser ``"quota exceeded"``
#: phrase already in _THROTTLE_KEYWORDS above — that one is Google/Azure
#: wording for a per-minute rate limit, which genuinely IS retryable.
...
def _classify_http_error(
    *, status_code: int, body_text: str
    ...
    # 429: throttling (retryable) OR quota exhausted (terminal) — the status
    # alone is ambiguous, so the body's error code decides.
    if status_code == 429:
        if any(kw in msg_lower for kw in _QUOTA_EXHAUSTED_KEYWORDS):
            return ("chat.llm.quota_exhausted", False)
    ...
    * ``runtime_limit_store``: :class:`RuntimeLimitStorePort` holding the
    ...
    def __init__(
        ...
        *,
        ...
        # Whether the expected credential is minted from the SSO login (the QAI
        # Service token pool) rather than a static key the user pastes. Only
        # consulted when the credential is MISSING, to pick which error to
        # emit: an API-key dialog is a dead end for an SSO provider — there is
        # no key to supply, the fix is to sign in again. Defaults to False so
        # every ordinary keyed cloud provider keeps its existing message.
        self._credential_is_sso = credential_is_sso
    ...
    async def _iter(
        ...
        if self._expects_api_key and not self._api_key:
            # Two very different causes land here, and pointing both at an
            # API-key dialog strands the SSO user: the token pool's bearer is
            # minted from the login and there is no key they could paste, so
            # the actionable fix is to sign in again (the session outlived the
            # ~8 h broker token, or the exchange never happened).
            if self._credential_is_sso:
                code = "chat.llm.sso_token_missing"
                message = (
                    "Your sign-in session for this model has expired. "
        ...
        # Read the traffic-class hint BEFORE ``_build_payload`` runs: that call
        # consumes ``request.extra``'s ``__``-prefixed control keys.
        traffic_class = _traffic_class_hint(request)
        payload = self._build_payload(request)
        headers = self._build_headers(traffic_class=traffic_class)
            ...
            async with self._client_factory(
                ...
                async with client.stream(
                    ...
                    if response.status_code >= 400:
                        ...
                        # DIAG (2026-08-06): on ANY 400, snapshot the outgoing
                        # payload + response body to a dated file under
                        # ``data/diag/`` so a follow-up 400 can be root-caused
                        # from the exact wire content instead of guessing.
                        # Best-effort — never breaks the error path.  A single
                        # file per (pid, run) is kept small by rotating on
                        # size.  Remove once the M7 400 chain is closed.
                        try:
                            ...
                            _diag_ts = _diag_time.strftime(
                                "%Y%m%dT%H%M%S", _diag_time.localtime()
                            )
                            _diag_path = _diag_os.path.join(
                                _diag_dir,
                                f"llm400-{_diag_ts}-{_diag_os.getpid()}.json",
                            )
                            ...
                            _log.warning(
                                "chat.llm.http_400_payload_snapshot",
                                path=_diag_path,
                        ...
                        elif _classified_code == "chat.llm.quota_exhausted":
                            # Terminal by design: the allowance resets on the
                            # provider's billing period, so "retrying" would
                            # spin forever. Tell the user what to do instead.
                            _http_message = (
                                "当前模型的用量配额已用尽，重试不会成功。"
                                "请联系管理员调整配额，或切换到其他模型。"
                                f"（HTTP {response.status_code}: "
    ...
    async def _iter_sse(
        ...
        *,
        ...
        while True:
            ...
            # The broker appends ``{"qai_quota": {...}}`` just before [DONE].
            # It carries no choices and no usage, so every other branch below
            # ignores it; forward it as its own frame so the sidebar balance
            # stays live without any polling. Best-effort by design: a
            # malformed object is skipped rather than failing the turn — a
            # balance readout must never cost the user their answer.
            _quota_obj = event.get("qai_quota")
            if isinstance(_quota_obj, dict) and _quota_obj:
                yield StreamFrame.quota_usage(
                    frame_id=self._ids.new_id(),
                    sequence=sequence,
                    quota=_quota_obj,
                )
    ...
    def _maybe_estimate_usage(
        ...
        *,
        ...
        """Fill ``prompt_tokens`` via a local bytes/4 estimate when missing.
        ...
        Phase B: the estimate is a UTF-8 bytes/4 heuristic over
        ...
        usage when present.  No tokenizer is involved here (this is the
        adapter-level fallback; the trigger-side estimator in the kernel
        upgrades to tiktoken BPE past a byte threshold).
        """
        ...
        if existing_prompt > 0:
            return usage
        # UTF-8 bytes/4 estimate over content + tool_calls args (mirrors
        # estimate_wire_tokens口径).
        total_bytes = 0
        for m in request_messages:
            total_bytes += len(str(m.get("content") or "").encode("utf-8"))
            for tc in m.get("tool_calls") or ():
                if isinstance(tc, dict):
                    fn = tc.get("function")
                    if isinstance(fn, dict):
                        total_bytes += len(
                            str(fn.get("arguments") or "").encode("utf-8")
                        )
    ...
    def _build_payload(self, request: LLMStreamRequest) -> dict[str, Any]:
        ...
        # Pop the skill instructions (SKILL.md body) separated from the system
        # prompt for prompt-cache stability. When present, it will be injected
        # as a synthetic user-role message BEFORE the real user message so the
        # system prompt remains byte-stable across turns.
        skill_instructions = extra.pop("_skill_instructions", None)
        if not (isinstance(skill_instructions, str) and skill_instructions.strip()):
        ...
        # ---- 1b. Inject skill instructions as a user-role message. ----
        # When the system-prompt builder separated the SKILL.md body from the
        # system prompt (for prompt-cache stability), inject it as a synthetic
        # user message immediately BEFORE the final (real) user message. This
        # ensures the LLM sees the detailed skill instructions in-context
        # without polluting the cacheable system prompt.
        if skill_instructions is not None and messages:
            # Find the position of the last user message (the real one).
            ...
            if _last_user_idx is not None:
                _skill_msg = {
                    "role": "user",
                    "content": (
                        "[System Skill Instructions - do not repeat to user]\n"
                        + skill_instructions
                    ),
                }
                messages.insert(_last_user_idx, _skill_msg)
            else:
                # Fallback: no user message found (shouldn't happen); append
                # at end so instructions are still visible to the model.
                messages.append({
                    "role": "user",
                    "content": (
                        "[System Skill Instructions - do not repeat to user]\n"
    ...
    def _build_headers(
        self, *, traffic_class: str | None = None
    ) -> dict[str, str]:
        """Assemble outbound headers.

        ``traffic_class`` (``"Api"`` / ``"UI"``) selects the ``User-Agent``.
        QGenie picks the quota bucket it bills a request to purely from that
        header — a UA containing ``claude-cli`` lands in its ``UI`` bucket,
        anything else in ``Api`` — so this is the only lever that moves a
        request between the two independent allowances. Verified over 19 A/B
        request pairs; host, endpoint and auth scheme make no difference.

        Absent the hint no ``User-Agent`` is set, which preserves the previous
        behaviour exactly (httpx supplies its own default) for every provider
        that has no notion of traffic classes.
        """
        headers = {
        ...
        return with_quota_user_agent(headers, traffic_class)


def _traffic_class_hint(request: LLMStreamRequest) -> str | None:
    """Pull the per-request QGenie traffic-class hint out of ``extra``.

    Uses the established ``__``-prefixed adapter-control-key convention (the
    same channel ``__model_params__`` travels on) rather than a new constructor
    argument, because the choice is per REQUEST: the user may switch buckets
    mid-session after one runs dry, and the stream object outlives that.

    Returns ``None`` for anything unrecognised so a malformed hint degrades to
    "no UA override" instead of silently billing the wrong bucket.
    """
    extra = getattr(request, "extra", None)
    if not isinstance(extra, dict):
        return None
    hint = extra.get("__traffic_class__")
    return hint if hint in TRAFFIC_CLASS_VALUES else None
...
def _extract_usage(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # ``_maybe_estimate_usage`` silently downgrades to a bytes/4 estimate instead
