...
from qai.platform.qgenie_user_agent import (
    ...
    configured_traffic_class as _configured_traffic_class,
    is_qgenie_host,
)
...
# QGenie's dual-bucket metering lives in the platform shared kernel
# (``qai.platform.qgenie_user_agent``): the host gate, the stored-preference
# read and the outbound UA are needed by callers in two bounded contexts
# (``qai.chat`` here, ``qai.security`` for smart approval), and the
# import-linter contract only permits ``qai.** -> qai.platform.**``. Keeping ONE
# definition matters because the ``claude-cli`` marker is externally imposed and
# load-bearing: two copies would drift, and the failure is silent (usage keeps
# landing in one bucket while the UI claims the other).


def _qgenie_traffic_class(
    base_url: str | None, requested: Any
) -> str | None:
    """Resolve the traffic class for a QGenie endpoint, else ``None``.

    ``None`` means "not a QGenie endpoint" and leaves outbound headers exactly
    as they were for every other provider — this feature must not perturb
    qai-service or a user's own gateway.
    """
    if not base_url or not is_qgenie_host(base_url):
        return None
    return (
        requested
        if requested in TRAFFIC_CLASS_VALUES
        else DEFAULT_TRAFFIC_CLASS
    )
...
class ProviderRoutingLLMStream:
    ...
    * ``local_stream_factory`` — ``Callable[[ResolvedModel], LLMStreamPort]``
    ...
    async def _select_target(
        ...
        # QGenie meters each request into one of two independent quota buckets
        # and picks the bucket from the outbound ``User-Agent`` alone. Thread
        # the user's chosen bucket so the transport can set that header.
        #
        # The preference is read from the resolved model's own catalog entry
        # (``models[].params.traffic_class``), i.e. the SAME channel the
        # sampling constraints above travel on. That keeps it per MODEL — the
        # allowances are per-model, so one model running dry must not move
        # another — and makes it durable across restarts without inventing a
        # second persistence path: the frontend writes it with the provider
        # config it already owns, and the resolver already surfaces it here.
        # A per-request override on ``request.extra`` still wins when present.
        #
        # Gated on the endpoint actually being QGenie: every other provider
        # keeps its previous outbound headers untouched (no ``User-Agent`` set).
        traffic_class = _qgenie_traffic_class(
            resolved.base_url,
            merged_extra.get("__traffic_class__")
            or _configured_traffic_class(model_params),
        )
        if traffic_class is not None:
            merged_extra["__traffic_class__"] = traffic_class
        else:
            # A stale preference must never ride along to a foreign host.
            merged_extra.pop("__traffic_class__", None)
        target_model_hint = (
            resolved.model_id
    ...
    def _transport_for(
        ...
        kwargs: dict[str, Any] = {
            ...
            # An SSO provider's bearer is minted from the login, not pasted by
            # the user. Forwarded so the transport's missing-credential guard
            # can say "sign in again" instead of pointing at an API-key dialog
            # the user can never satisfy for this provider.
            "credential_is_sso": bool(resolved.is_sso),
