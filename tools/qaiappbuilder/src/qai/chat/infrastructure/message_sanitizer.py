...
# Collapse runs of 3+ underscores only — preserves the ``__`` MCP tool-name
# separator (``{server}__{tool}``) so dispatch lookups still find the handler.
_REPEATED_UNDERSCORE = re.compile(r"_{3,}")
...
def _sanitize_tool_call_name(name: str) -> str:
    ...
    2. Collapse runs of 3+ consecutive ``_`` into a single ``_`` (preserves
       the ``__`` MCP tool-name separator).
