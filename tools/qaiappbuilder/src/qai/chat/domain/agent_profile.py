...
* :data:`EXPLORE` — a STRICTLY READ-ONLY codebase-search specialist: it may
  only read / search (``read`` / ``glob`` / ``grep`` / ``web_fetch`` /
...
# exclusions (``agent`` / ``question`` / ``sub_agent`` are already dropped by
# ``_SUB_AGENT_EXCLUDED_TOOLS``; they are re-listed here so the deny-list is
...
_EXPLORE_DENIED_TOOLS: frozenset[str] = frozenset(
    {
        ...
        "edit",
        "apply_patch",
        # Structural (AST) rewrite — a codemod across many files at once, so
        # even more state-mutating than a single ``edit``. Its preview mode
        # (``apply=false``) is read-only, but a profile deny-list is per-TOOL,
        # not per-argument: allowing the tool would allow ``apply=true``.
        "ast_edit",
        "exec",
        "appbuilder_run",
        ...
        "todowrite",
        "background_process",
        # Executes arbitrary Python in a live interpreter — as state-mutating
        # as ``exec`` regardless of what any single cell happens to do.
        "run_code",
...
_EXPLORE_ALLOWED_TOOLS: frozenset[str] = frozenset(
    {
        ...
        "glob",
        "grep",
        # Structural (AST) search — read-only, and exactly explore's job: it
        # answers "where is this actually called" without the string/comment
        # false positives that make ``grep`` hits need manual triage.
        "ast_grep",
        "web_fetch",
        "list",
    }
...
GENERAL = AgentProfile(
    ...
    description=(
        ...
        "glob/grep/web_fetch/…), runs a multi-step agentic loop until the "
...
EXPLORE = AgentProfile(
    ...
    description=(
        ...
        "web_fetch (no write/edit/exec). Use for fast, safe code-base search "
    ...
    # No artificial round cap — the no-progress circuit breaker prevents
    # infinite loops; removing the cap avoids premature INCOMPLETE results
    # when investigating complex codebases.
    max_rounds=None,
)
