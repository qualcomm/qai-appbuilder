...
class Conversation:
    ...
    #: and read by ``GetContextDecisionUseCase`` as the "before" figure of the
    ...
    def set_discussion(
        ...
        * ``soft_stop_mode`` (str): soft-stop aggressiveness
          (absent ⇒ ``"conservative"``).
        * ``coordinator_model_id`` (str | None): the model the coordinator
          (主持人 — the unified moderator + judge) uses; resolution ladder at
          orchestrator read time: this ⇒ ``roster[0].model_id`` ⇒
          ``default_model_id`` (absent ⇒ ladder falls through).
        * ``coordinator_prompt`` (str | None): advisory extra text appended to
          the built-in judge directive (and manager prompt); absent/empty ⇒
          built-in default only. The DEPRECATED ``judge_participant_id`` is
          still read for back-compat (legacy conversations keep their judge).
