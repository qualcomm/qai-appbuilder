...
class MessageRole(str, Enum):
    ...
    ``SYSTEM_NOTICE`` is a first-class transcript type for automated
    background-task completion notifications (sub-agent finished /
    background exec job exited).  Persisted as its own role so:

    * The UI renders it as a system-note card, never a user bubble
      (no meta.aside masking of a role:user message).
    * History reload / compaction / rebuild identify it as a distinct
      type and never miscount it as a user turn.
    * The wire builder folds it onto ``role:user`` with a
      ``[System notification]`` envelope so LLMs (and provider APIs
      that only accept the four core roles) still see valid content
      while the internal transcript keeps the true origin.
    """
...
def compute_context_usage(used_raw: int, budget: int) -> ContextUsage:
    ...
    Shared口径 between the main agent's :class:`GetContextDecisionUseCase` and the
