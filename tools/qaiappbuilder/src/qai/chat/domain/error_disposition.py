...
* ``"never"``          — deterministic, non-transient. No auto-retry; the
  ...
  not loop on it.
* ``"bounded_fast"``   — reserved for connectivity faults that recover
  fast or are effectively permanent (currently unused in-tree — every
  connect / DNS / read-error code maps to ``network_wait`` below because
  in this desktop app they are almost always transient network blips).
  Retained in the vocabulary for future use.
* ``"network_wait"``   — a transient network fault that may take a while to
  self-heal (connect / DNS / read / timeout — WiFi/VPN blip, etc.).
  Escalating backoff (3-5-10-30 s) capped by a wall-clock budget
  (default 10 min).
...
_CODE_TO_DISPOSITION: dict[str, str] = {
    ...
    # Provider quota exhausted (HTTP 429 whose body carries an
    # ``insufficient_quota`` / ``quota_exhausted`` marker). Distinct from
    # plain ``throttling``: a rate limit clears in seconds, but an exhausted
    # allowance only resets on the provider's billing period (typically
    # monthly), so retrying is guaranteed to fail and would leave the user
    # watching a "retrying…" spinner that can never succeed. Terminal, with
    # an actionable message instead.
    "chat.llm.quota_exhausted": DISPOSITION_NEVER,
    "chat.llm.unsupported_param": DISPOSITION_NEVER,
    "chat.llm.provider_api_key_missing": DISPOSITION_NEVER,
    # SSO-authenticated provider (QAI Service token pool) with no bearer: the
    # credential is minted from the login, so an auto-retry would send the same
    # empty credential. Terminal until the user signs in again — distinct from
    # ``provider_api_key_missing`` precisely because the remedy differs.
    "chat.llm.sso_token_missing": DISPOSITION_NEVER,
    ...
    # ── network_wait (transient connectivity fault — persistent retry with
    # wall-clock budget). This covers connect / DNS / read faults which,
    # for a desktop app talking to a cloud LLM, are almost always caused by
    # WiFi/VPN blips that self-heal within seconds to a few minutes — NOT
    # by permanently mistyped hostnames. Grouping DNS + connection_refused +
    # host_unreachable here (previously bounded_fast → gave up after 14 s)
    # lets the retry loop keep trying for the full ``NETWORK_WALL_CLOCK_
    # BUDGET_SECONDS`` window (default 10 min) with a 3-5-10-30 s backoff.
    "chat.llm.dns_error": DISPOSITION_NETWORK_WAIT,
    "chat.llm.connection_refused": DISPOSITION_NETWORK_WAIT,
    "chat.llm.host_unreachable": DISPOSITION_NETWORK_WAIT,
