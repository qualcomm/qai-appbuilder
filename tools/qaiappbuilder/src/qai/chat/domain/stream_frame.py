...
class StreamFrameType(str, Enum):
    ...
    # M6 — non-interrupting side-channel notification (e.g. a background-
    # process completion event forwarded to the tab).  Renders on the
    # frontend as a small system-note card, NOT a user bubble.
    ASIDE_MESSAGE = "aside_message"
    ...
    # ── Token-pool balance (QAI Service) ──────────────────────────────────
    # Emitted when the upstream is the QAI Service token pool, which appends a
    # balance object to its stream just before ``[DONE]``:
    # ``{"qai_quota": {"allocated": int, "used": int, "remaining": int,
    # "reset_at"?: str, "period"?: str, "unit"?: str}}``. That object carries
    # no ``choices`` and no ``usage``, so a standard OpenAI-compatible parser
    # skips it — surfacing it as its own frame is purely additive.
    #
    # Why a frame rather than a poll: the balance the user cares about changes
    # exactly when they spend it, and the broker already tells us in the same
    # stream. Forwarding it costs one frame per turn and keeps the sidebar
    # widget live with NO polling loop and no extra round-trip.
    #
    # NON-terminal and best-effort: absent on every turn against every other
    # provider, so the wire stays byte-for-byte unchanged for existing
    # consumers. AGENTS.md §3.1: tail-only enum growth — existing values
    # unchanged; WS / SSE adapters blind-forward it (no enum whitelist), so no
    # transport edit is needed.
    QUOTA_USAGE = "quota_usage"
    # ── Mid-turn injection deferred to a new turn (V2 enhancement) ─────────
    # Emitted at the tail of a turn's normal-completion path when the
    # injection registry still holds pending texts that the run loop never
    # drained (typical case: the user hit Ctrl+Enter after the last tool
    # round, or during a turn that never called any tool at all — the
    # inter-round seam never fired, so ``_inject_hook`` was never invoked).
    # The frontend consumes this frame by removing the corresponding
    # ``待注入`` bubbles and issuing a fresh user-message submission through
    # the SAME path as pressing Enter in the composer — the injected content
    # becomes a real new turn, byte-identical in behaviour to an ordinary
    # user message. Payload: ``{"texts": list[str]}`` — the FIFO-ordered
    # pending texts that were drained just before the run loop cleared the
    # registry (multi-inject sessions carry >1 entry; the frontend merges
    # them with ``\n\n`` to preserve the user's ordering). Absent when no
    # pending injection remains (byte-for-byte unchanged wire for every
    # turn that folded its injections on time). AGENTS.md §3.1: tail-only
    # enum growth — existing values unchanged; SSE/WS adapters blind-forward
    # it (no enum whitelist), so no transport edit is needed.
    INJECT_AS_NEW_TURN = "inject_as_new_turn"
    ...
    * ``payload`` must be a ``dict`` (possibly empty).  Frames whose
    ...
    def aside_message(
        ...
        *,
        ...
        """Convenience factory for an M6 aside notification frame.

        Emitted when the streaming loop folds a producer-owned aside
        notification (for example a context-recovery notice) into the
        current turn's wire.  Frontend renders these as small system-note
        cards, NOT user bubbles.

        ``kind`` distinguishes sub-flavours (``"system_note"`` default;
        ``"bg_completion"`` for M5 auto-background exits, etc.).
        """
        if not isinstance(text, str) or not text:
            raise ValueError("text must be a non-empty string")
        ...
        return cls(
            frame_id=frame_id,
            frame_type=StreamFrameType.ASIDE_MESSAGE,
            sequence=sequence,
            payload=payload,
        )
    ...
    def quota_usage(
        ...
        *,
        ...
        """Factory for a ``quota_usage`` token-pool balance frame.

        ``quota`` is the broker's balance object, forwarded with its own field
        names (``allocated`` / ``used`` / ``remaining`` and optionally
        ``reserved`` / ``reset_at`` / ``period`` / ``unit``). It is copied —
        never aliased — so a later mutation of the adapter's parsed dict cannot
        reach a frame already handed to the transport.

        Deliberately permissive about the object's contents: the numbers are
        the broker's to define, and a field we do not recognise today should
        reach the UI rather than be dropped by an over-eager schema here. The
        one hard requirement is that it IS a mapping, so the frontend can index
        it without a type check.
        """
        if not isinstance(quota, dict):
            raise TypeError(f"quota must be dict, got {type(quota).__name__}")
        return cls(
            frame_id=frame_id,
            frame_type=StreamFrameType.QUOTA_USAGE,
            sequence=sequence,
    ...
    def tool_result(
        ...
        *,
        ...
        quota_warning: dict[str, Any] | None = None,
        ...
        * ``partial=False`` / ``None`` — the *final* frame (the existing
        ...
        # ``error_code`` (optional appended field, §3.1 tail-only): a stable,
        # machine-readable failure code lifted from a user-fixable tool
        # envelope (``{ok: False, error_code, message}``) — e.g. ``web_search``
        # returns ``chat.llm.tls_cert_untrusted`` when it hits an enterprise
        # TLS-intercepting gateway with an untrusted CA. The rendered ``result``
        # text drops the code (V1-style ``[tool_error] …`` projection), so it
        # is carried here verbatim to drive the chat tool card's inline
        # "disable TLS verification and retry" affordance. Absent on every
        # ordinary result → byte-for-byte unchanged for current consumers.
        if error_code is not None:
            payload["error_code"] = error_code
        # ``quota_warning`` (optional appended field, §3.1 tail-only): an advisory
        # dict surfaced by the independent web-search provider when a keyed
        # engine's monthly usage crosses a notification threshold. Carried here so
        # the frontend can show an inline system notice without a new frame type.
        # Shape: ``{engine_id, kind, usage_count, monthly_limit, message}``.
        if quota_warning is not None:
            payload["quota_warning"] = quota_warning
        # ``backgrounded`` / ``background_process_id`` (optional appended
        # fields, §3.1 tail-only): the command was auto-backgrounded — it
        # outlived the foreground-wait threshold while fresh user input
        # arrived, so the still-RUNNING child was handed to the
        # background-process manager and the tool call returned early.
        # ``ok`` stays true (nothing failed), which is exactly why a
        # dedicated marker is needed: without it a consumer can only read
        # "success" and paints a "done" card for a command that has not
        # finished. Mirrors ``cancelled`` above — a distinct terminal state
        # carried as a field so the UI never parses ``result`` prose.
        # ``background_process_id`` names the job to reach it later.
        # Absent on every ordinary result → byte-for-byte unchanged.
        if backgrounded is not None:
            payload["backgrounded"] = backgrounded
        if background_process_id is not None:
            payload["background_process_id"] = background_process_id
        # ``terminal_kind`` (optional appended field, §3.1 tail-only): names a
        # SUCCESSFUL but semantically distinct terminal outcome the UI should
        # not paint as "done". Today's values:
        #   * ``"stopped"`` — background_process(action=stop) succeeded; the
        #     process was terminated by the caller, not by natural exit.
        #   * ``"restarted"`` — background_process(action=restart) succeeded;
        #     the process was replaced.
        # ``ok`` stays true (nothing failed); the flag lets the card render
        # "已停止/Terminated" or "已重启/Restarted" rather than a generic
        # ✓ 已完成 tick. Absent on every ordinary result → byte-for-byte
    ...
    def end(
        ...
        *,
        ...
        if assistant_message_id is not None:
            # §3.1 tail-append: the persisted assistant Message id for THIS
            # turn, so the frontend commits its optimistic assistant bubble
            # with the SAME id the DB holds (rather than a client-local id).
            # Enables per-message fork (``up_to_message_id``) on a
            # just-finished turn without a reload. Omitting keeps the wire
    ...
    def subagent_start(
        ...
        *,
        ...
        ``status`` (§3.1 tail-appended) is the sub-agent's lifecycle state AT
        SPAWN TIME.  Under the async spawn contract a spawn returns a handle
        at t=0 and the sub-agent then runs detached, so the honest initial
        state is ``"background"`` (the default): the card renders as live
        background work whose result will arrive later, and the composer's
        "is there work in flight" scan sees it.  A caller that really does
        drive a sub-agent to completion within the parent turn passes
        ``"running"`` instead.
        """
        ...
        if not isinstance(status, str) or not status:
            raise ValueError(
                f"status must be a non-empty str, got {status!r}"
            )
    ...
    def subagent_done(
        ...
        *,
        ...
        the parent stream NEVER receives a synthetic ``subagent_done`` for
        a running-in-background sub-agent — the ``agent()`` tool returns a
        handle immediately and the sub-agent's real completion flows via
        ``SubAgentSessionTerminated`` event → BackgroundJobDispatcher →
        SYSTEM_NOTICE (NOT a subagent_done frame on this stream).  So this
        factory only ever produces genuine terminal-completion frames now
        — the 2026-08-07 ``parked=True`` tail-appended flag is REMOVED
        because no synth-done path remains.

        §N.54 typed-failure tail (2026-08-08): a sub-agent that terminated
        because it FAILED still reaches the parent as a ``subagent_done``
        frame (the async contract has exactly one terminal frame type per
        run), so the failure classification travels on this frame rather
        than on ``subagent_error`` (which only covers the legacy inline
        dispatch path).  ``error_kind`` is the wire form of
        :class:`~qai.chat.domain.sub_agent_error.SubAgentErrorKind`
        (``preflight`` / ``isolation`` / ``execution`` / ``budget`` /
        ``interrupted``), ``error_message`` the human-readable cause and
        ``recovery_hint`` the actionable next step.  All three are
        tail-appended optionals (§3.1): a clean completion writes NONE of
        them, so a successful run's payload is byte-for-byte what it was
        before this change.  The front-end renders the trio as a typed
        error card (``SubAgentBlock.vue`` ``ERROR_STYLES``).
        """
        cls._validate_index(index)
        ...
        for _name, _value in (
            ...
            if _value is not None and (
                not isinstance(_value, str) or not _value
            ):
                raise ValueError(
                    f"{_name} must be a non-empty string or None",
                )
        payload: dict[str, Any] = {
            "index": index,
            "result": result,
        ...
        if subagent_id is not None:
            payload["subagent_id"] = subagent_id
        # Typed-failure tail — written only when supplied so a clean
        # completion's payload keeps its historical shape exactly.
    ...
    def speaker_changed(
        ...
        **optional appended field** (§3.1) carrying the speaker's backing
        ...
        ``is_coordinator`` (optional appended field, §3.1; default ``False``):
        set ``True`` for the discussion coordinator (主持人) turn — the unified
        moderator + judge identity — so the front-end renders that bubble as a
        distinct host/总结 identity rather than a borrowed role name. Absent /
        ``False`` keeps the payload byte-for-byte for every ordinary speaker.
        """
    ...
    def inject_as_new_turn(
        ...
        *,
        ...
        """Factory for ``inject_as_new_turn`` — deferred injection tail frame.

        Emitted at the end of a turn's normal-completion path when the
        injection registry still holds pending texts that the run loop
        never drained (see :data:`StreamFrameType.INJECT_AS_NEW_TURN`).

        ``texts`` MUST be a non-empty list of non-empty strings, in FIFO
        order — the exact ``list[str]`` that
        :meth:`InjectionRegistryPort.drain` returned. The frontend consumes
        this frame by dropping the tab's ``pending`` injection bubbles and
        submitting the merged text through the SAME composer-submit path a
        user Enter would take, so the injected content becomes a real new
        turn (byte-identical behaviour to an ordinary user message).

        Image parity: images ride INSIDE each text as ``![](url)`` markdown
        (the same shape a normal submit uses); the frontend passes the
        merged text through unchanged and the standard submit path resolves
        the refs to vision blocks again.
        """
        ...
        for _t in texts:
            if not isinstance(_t, str) or not _t:
                raise ValueError(
                    "texts entries must be non-empty strings",
                )
        return cls(
            frame_id=frame_id,
            frame_type=StreamFrameType.INJECT_AS_NEW_TURN,
            sequence=sequence,
