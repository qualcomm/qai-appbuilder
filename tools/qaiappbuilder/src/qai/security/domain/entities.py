...
class PolicyRule:
    ...
    def matches(
        ...
        *,
        ...
        ``env`` is forwarded to :meth:`PathPattern.matches` so any
        ``${VAR}`` placeholder in the pattern (e.g. ``${PROJECT_ROOT}``)
        is expanded before matching. When ``env`` is ``None`` (the
        default / legacy call shape) the pattern text is used verbatim,
        preserving byte-for-byte backward compatibility.
        """
        return self.pattern.matches(path, env=env)
    ...
    def matches_request(
        ...
        *,
        ...
        ``env`` is forwarded to :meth:`PathPattern.matches` so
        placeholders in the pattern are expanded before matching (see
        :meth:`matches`). ``None`` (the default / legacy call shape)
    ...
    * no two rules with the same ``(scope, pattern)`` disagree on
    ...
    def evaluate(
        ...
        *,
        ...
        ``env`` is forwarded to :meth:`PolicyRule.matches` so any
        ``${VAR}`` placeholder (e.g. ``${PROJECT_ROOT}``) in a rule's
        pattern is expanded before matching. ``None`` (the default /
        legacy call shape) keeps pre-existing behaviour byte-for-byte.
        """
        for rule in self.rules:
            if rule.matches(path, env=env):
                return rule.action
        return None
    ...
    def evaluate_request(
        ...
        *,
        ...
        ``env`` — optional ``${VAR}`` bindings forwarded to both the
        exec-deny gate and the positive-pass rule match, so a pattern
        like ``${PROJECT_ROOT}/*`` (shipped by the built-in ``strict`` /
        ``development`` / ``demo`` templates) is expanded to a real
        absolute path before matching. The security *domain* stays
        side-effect-free — the application layer (``check_permission``)
        resolves the bindings from configuration and injects them; the
        domain never reads ``os.environ``. ``None`` (the default /
        legacy call shape used by ``evaluate`` shadow detection and by
        callers who don't carry an env context) preserves pre-fix
        behaviour byte-for-byte, so a legacy call site whose patterns
        never used placeholders is unaffected.
        ...
        if execute:
            for rule in self.rules:
                if rule.op is PolicyOp.EXEC_DENY and rule.pattern.matches(
                    path, env=env
                ):
        ...
        for rule in self.rules:
            ...
            if rule.matches_request(
                path, read=read, write=write, execute=execute, env=env
            ):
...
class PathGrant:
    ...
    #: Grant EFFECT (tail-appended, additive). ``"allow"`` (DEFAULT) is the
    #: historical behaviour — the grant AUTHORISES its (subject, path, mask) so a
    #: matching request is ALLOWED without a prompt. ``"deny"`` is a persisted
    #: NEGATIVE grant: it REMEMBERS a user's rejection so a matching request is
    #: DENIED without re-prompting (constraint G, reject side). Deny-effect
    #: grants are consulted BEFORE allow-effect grants in the decision cascade
    #: (a remembered rejection wins over a remembered approval for the same
    #: scope). DEFAULT ``"allow"`` keeps legacy rows / callers byte-for-byte.
    #: Only ``permanent`` deny grants seed the native layer (as a deny_mask
    #: op-mask rule); session/process deny grants are Python-layer only, mirroring
    #: allow-grant native seeding (native has no session/process concept).
    effect: str = "allow"
    ...
    def __post_init__(self) -> None:
        ...
        if self.effect not in ("allow", "deny"):
            raise ValueError(
                f"effect must be one of allow/deny, got {self.effect!r}"
            )
        if (
