    ...
    def matches(
        ...
        *,
        ...
        if target.match_kind is PolicyMatchKind.REGEX:
            ...
            # H-Sec-4 — NFC-normalise both sides so a rule keyed on
            # ``Café`` (U+00E9) still matches a decomposed path
            # ``Cafe`` + U+0301 (and vice versa). NTFS lets the same
            # file object surface under either form; pure byte matching
            # would let a decomposed spelling slip past a deny-list.
            normalized_path = unicodedata.normalize("NFC", path)
            normalized_pat = unicodedata.normalize("NFC", target.pattern)
            return re.search(normalized_pat, normalized_path, flags) is not None
        ...
        #
        # H-Sec-4 (2026-07-28) — additionally fold NFD → NFC so an
        # attacker cannot bypass a policy by feeding a decomposed
        # spelling of a rule that was authored in NFC (or vice versa).
        def _norm(s: str) -> str:
