...
class PathGrantCreatedEvent(DomainEvent):
    ...
    source: GrantSource
    occurred_at: datetime
    #: Grant EFFECT — ``"allow"`` (DEFAULT, historical) or ``"deny"``
    #: (constraint G reject-side deny-grant). Tail-appended with a default so
    #: existing publishers / subscribers that ignore it stay byte-compatible;
    #: the native-guard sync subscriber reads it to decide allow-rule vs
    #: deny_mask op-mask seeding. Mirrors ``PathGrant.effect``.
    effect: str = "allow"
...
class PathGrantRevokedEvent(DomainEvent):
    ...
    occurred_at: datetime
    #: Grant EFFECT of the revoked grant — ``"allow"`` (DEFAULT) or ``"deny"``.
    #: Tail-appended so the native-guard sync subscriber undoes the correct
    #: native rule (allow-rule vs op-mask deny rule). Mirrors
    #: :attr:`PathGrantCreatedEvent.effect`.
    effect: str = "allow"
