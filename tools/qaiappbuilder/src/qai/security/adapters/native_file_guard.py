...
class NativeFileGuard:
    ...
    def _default_filter(self, _evt: FilterEventV2) -> bool:
        # No ASK bridge wired (hand-rolled / test container): fail-CLOSED (DENY).
        # A guard with no decision path must never silently allow file ops
        # (user directive 2026-07-23); the fail_closed knob no longer weakens
        # this. Production always wires the real bridge via set_filter_callback.
        return False
    ...
    def add_op_mask_rule(
        ...
        *,
        ...
        """Add an OP-MASKED THREE-STATE (white_ops) prefix.

        ``allow_mask`` / ``deny_mask`` are bitfields (READ=1, WRITE=2,
        EXECUTE=4, DELETE=8). For a path event under this prefix: op bit in
        ``deny_mask`` -> hard DENY; else op bit in ``allow_mask`` -> ALLOW
        (skips the callback); else (op in neither) -> falls through to the
        callback (-> ASK). This is the general op-aware primitive (e.g.
        read+execute allowed, write DENIED, everything else ASK). Degrades to
    ...
    def add_op_mask_allow_rule(
        ...
        """Back-compat shim: add an allow-only op-mask prefix (deny_mask=0).

        Retained so existing callers passing a single allow ``mask`` keep
        working; delegates to :meth:`add_op_mask_rule` with ``deny_mask=0``.
        """
        return self.add_op_mask_rule(
            path, mask, 0, session_only=session_only
        )
    ...
    def remove_op_mask_rule(
        ...
        """Remove an op-masked (white_ops) prefix by path.

        Mirror of :meth:`remove_allow_rule` for the op-mask list — used to
        undo a deny-effect grant's native ``deny_mask`` rule on revocation
        (constraint G reject-side native sync). Degrades to ``False`` when the
        loaded DLL predates the ``DeleteOpMaskWhiteRules`` export.
        """
        return self._mutate("delete_op_mask_white_rules", path, session_only)
