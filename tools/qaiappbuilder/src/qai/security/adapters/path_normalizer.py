...
__all__ = ["normalize_path", "normalize_windows_path"]
...
# Group C (M-Py-4): the canonical multi-shape normaliser lives in
# :mod:`qai.platform.path_normalize` (the shared kernel) so
# :mod:`qai.platform.protected_paths` — one of its three consumers —
# can reach it without importing back into ``qai.security``
# (``.importlinter`` contract 3: contexts must not depend on each other).
# Re-exported here to keep the historical adapter surface intact for
# callers that import ``normalize_windows_path`` from this module.
from qai.platform.path_normalize import normalize_windows_path  # noqa: E402
