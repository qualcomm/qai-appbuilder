...
class InMemorySkillCapabilityRegistry:
    ...
    async def list_active_items(self) -> list[tuple[str, SkillCapability]]:
        """Return ``(skill_name, capability)`` pairs, preserving the key.

        ``list_active`` drops the registration name; callers that need it
        (skill discovery's ``by_name`` map) must not fall back to
        ``capability.capability_name``, which differs whenever a skill
        registers under another capability name.
        """
