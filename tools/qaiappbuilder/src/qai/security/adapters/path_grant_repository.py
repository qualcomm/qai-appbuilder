...
_GRANT_COLUMNS = (
    ...
    "is_directory, is_program, effect"
)
...
class SqlitePathGrantRepository:
    ...
    def _row_to_grant(
        ...
        # Tail-appended grant effect (migration 058). Rows written before the
        # migration read back the DEFAULT 'allow' (positive-grant semantics).
        effect = str(row[12]) if len(row) > 12 and row[12] is not None else "allow"
        return PathGrant(
            grant_id=grant_id,
            subject=subject,
            path=path,
