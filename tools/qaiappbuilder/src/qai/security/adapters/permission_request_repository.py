...
class SqlitePermissionRequestRepository:
    ...
    async def resolve_if_pending(self, request: PermissionRequest) -> bool:
        """Compare-and-set: write the resolution only if still pending.

        ``save`` is an unconditional ``INSERT OR REPLACE``, which is correct
        for the approve / reject use cases (they own the decision). It is NOT
        safe for a background writer holding a frozen snapshot: between its
        read and its write the user may have clicked, and the upsert would
        overwrite that. ``PermissionRequest`` being frozen means the domain's
        ``_ensure_pending`` only validates the caller's stale copy, so the
        guard has to live in SQL — hence ``WHERE state = 'pending'``.

        Returns ``True`` when this call resolved the row, ``False`` when it
        had already been resolved elsewhere (a no-op, not an error).
        """
        try:
            async with self._db.connection() as conn:
                try:
                    cur = await conn.execute(
                        ...
                        "SET state = ?, resolved_at = ?, resolution_reason = ? "
            ...
            raise PersistenceError(
                "security.permission_request.resolve_if_pending_failed",
                f"failed to resolve request {request.request_id.value!r}: {exc}",
                operation="permission_request.resolve_if_pending",
                cause=exc,
            ) from exc
