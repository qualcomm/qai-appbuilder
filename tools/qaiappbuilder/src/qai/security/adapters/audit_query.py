...
def _build_time_window(
    ...
    """Return an ``(sql_where_prefix, params)`` tuple clamping
    ``occurred_at`` to ``[since, until)``. ``sql_where_prefix`` is empty
    or ends with a trailing space so callers can simply concatenate it
    into their query.
    """
...
class SqliteAuditQuery:
    ...
    async def recent(
        ...
        *,
        ...
        """Return the ``limit`` most-recent entries in the ``[since, until)``
        window, newest first, skipping the first ``offset`` rows.

        ``limit < 0`` is treated as ``limit = 0`` per the port contract
        (returns an empty list); negative values are not propagated to
        SQLite where they would mean "no limit". ``offset < 0`` is
        clamped to 0.
        """
        ...
        where, params = _build_time_window(since, until)
        try:
    ...
    async def count(
        ...
        *,
        ...
        """Return the total number of audit entries in ``[since, until)``.

        Both bounds are optional (``None`` disables that side). Uses the
        ``ix_security_audit_entry_time`` index when a time clamp is
        provided; a bare count falls back to a full scan (cheap on
        SQLite for the 50k-row cap).
        """
        ...
        try:
            async with self._db.connection() as conn:
                cur = await conn.execute(
                    f"SELECT COUNT(*) FROM security_audit_entry {where}",
                    tuple(params),
                )
            ...
            raise PersistenceError(
                "security.audit_entry.count_failed",
                f"failed to count audit entries: {exc}",
                operation="audit_entry.count",
                cause=exc,
            ) from exc
