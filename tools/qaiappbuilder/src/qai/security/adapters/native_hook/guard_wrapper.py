...
class Guard:
    ...
    def __init__(self, dll_path: str):
        if not dll_path:
            raise GuardLoadError("dll_path must not be empty")
        self._dll_path: str = os.path.normpath(dll_path)
        if not os.path.isfile(self._dll_path):
            raise GuardLoadError(f"DLL file not found: {self._dll_path}")
        try:
            self._dll: ctypes.CDLL = ctypes.CDLL(self._dll_path)
        except OSError as e:
            raise GuardLoadError(f"Failed to load DLL: {self._dll_path}") from e
    ...
    def _setup_prototypes(self):
        ...
        if hasattr(self._dll, "AddOpMaskWhiteRules"):
            # THREE-STATE: (path, allow_mask, deny_mask, session_only).
            self._dll.AddOpMaskWhiteRules.argtypes = [c_char_p, c_int, c_int, c_bool]
    ...
    def init(self, fn: FilterCallable, fail_closed: bool = False) -> bool:
        ...
        def _wrapper(pid: int, event: int, raw_path) -> bool:
            try:
                ...
                # Callback threw -> fail-CLOSED (DENY). A Python-side fault in
                # the decision path must never leak a file op through, regardless
                # of the fail_closed knob (user directive 2026-07-23).
                return False
    ...
    def destroy(self) -> None:
        ...
        self._dll.Destroy()
        self._inited = False
        # Obs-Py-1 (fileguard-audit-2026-07-26): CFUNCTYPE holder references
        # kept the Python trampolines alive so the native pipe thread could
        # invoke them. After Destroy() the DLL side no longer calls them; the
        # holders are the only remaining strong ref, so clear them to release
        # the closures (and their enclosed Python callbacks + settings).
        self._filter_ref = None
        self._filter_ref_v2 = None
    ...
    def add_op_mask_white_rules(
        ...
        # Op-masked whitelist (THREE-STATE): for an op whose bit (READ=1,
        # WRITE=2, EXECUTE=4, DELETE=8) is set in ``deny_mask`` -> hard DENY;
        # else set in ``allow_mask`` -> ALLOW; else (neither) -> falls through to
        # the callback (ASK). Returns False when the loaded DLL predates the
        # AddOpMaskWhiteRules export.
        if not hasattr(self._dll, "AddOpMaskWhiteRules"):
            return False
        return bool(
            self._dll.AddOpMaskWhiteRules(
                rule.encode("utf-8"), int(allow_mask), int(deny_mask), session_only
            )
        )
    ...
    def init_v2(
        ...
        if not hasattr(self._dll, "InitV2"):
            raise RuntimeError("guard64.dll does not export InitV2; a V2-capable DLL is required")
        ...
        def _wrapper(evt_ptr) -> bool:
            try:
                ...
                # M-Nat-5 (fileguard-audit-2026-07-26): the V2 decision path
                # (file_path / process_path / command_line) uses errors='strict'
                # — any invalid UTF-8 raises UnicodeDecodeError, which the
                # outer except catches and returns False (DENY). The DLL emits
                # utf-8 bytes; a mismatch means the event is corrupt / attacker-
                # controlled and MUST NOT be silently smuggled through as U+FFFD
                # replacement characters (which then flow into pattern match /
                # audit). The List* diagnostics below keep errors='replace' —
                # they are for humans, never for decisions.
                fp = evt.file_path.decode("utf-8", errors="strict") if evt.file_path else ""
                pp = evt.process_path.decode("utf-8", errors="strict") if evt.process_path else ""
                cl = evt.command_line.decode("utf-8", errors="strict") if evt.command_line else ""
                ...
                # Callback threw -> fail-CLOSED (DENY), regardless of the
                # fail_closed knob: a Python-side fault must never leak a file
                # op through (user directive 2026-07-23).
                return False
    ...
    def add_process_exception(self, exe_path: str) -> bool:
        ...
        if not hasattr(self._dll, "AddProcessException"):
            raise RuntimeError("guard64.dll does not export AddProcessException")
    ...
    def remove_process_exception(self, exe_path: str) -> bool:
        ...
        if not hasattr(self._dll, "RemoveProcessException"):
            raise RuntimeError("guard64.dll does not export RemoveProcessException")
    ...
    def exempt_self(self) -> bool:
        ...
        if not hasattr(self._dll, "ExemptSelf"):
            raise RuntimeError("guard64.dll does not export ExemptSelf; a R13+ DLL is required")
