...
class PermissionRequestRepositoryPort(Protocol):
    ...
    async def resolve_if_pending(self, request: PermissionRequest) -> bool:
        """Persist ``request`` ONLY while the stored row is still pending.

        Returns ``True`` when the row was updated, ``False`` when it had
        already been resolved by someone else, or when no such row exists
        (nothing written in either case — use :meth:`save` to create one).

        Exists because :meth:`save` is an unconditional upsert while
        :class:`PermissionRequest` is a frozen snapshot: a caller that read a
        PENDING row, awaited, then wrote a resolved copy would silently
        clobber a decision made in between — e.g. the subprocess-gone sweep
        overwriting an ``approved`` the user had just clicked. The domain's
        ``_ensure_pending`` guard cannot catch that; it validates the caller's
        own stale snapshot, not the stored state. Compare-and-set in the
        adapter is the only layer that can.
        """
        ...
    ...
    * adapters can apply different concurrency / consistency strategies
    ...
    async def recent(
        ...
        *,
        ...
        """Return a slice of audit entries, newest first.

        ``limit`` must be a non-negative int; ``limit=0`` is allowed and
        returns an empty list. ``offset`` is a non-negative row offset
        applied AFTER ordering by ``occurred_at DESC`` — used by the UI
        for page-N navigation. ``since`` / ``until`` (if provided) clamp
        the returned window to ``[since, until)`` on ``occurred_at``.
        Implementations should leverage the ``occurred_at DESC`` index on
    ...
    async def count(
        ...
        *,
        ...
        """Return the total number of audit entries in the ``[since, until)``
        window (both bounds optional; ``None`` disables that side of the
        clamp). Used by the paginated Audit UI to render "N of M".
        """
        ...
...
class SkillCapabilityRegistryPort(Protocol):
    ...
    async def list_active_items(self) -> list[tuple[str, SkillCapability]]:
        """Return ``(skill_name, capability)`` for every registration.

        The registry is keyed by ``skill_name``, but a
        :class:`SkillCapability` carries only ``capability_name`` — the two
        differ whenever a skill registers a capability under another name.
        :meth:`list_active` returns bare values, so the key is lost and a
        caller that needs the registration name has to fall back to
        ``capability_name`` (which silently mis-keys the skill-discovery
        ``by_name`` map). This accessor preserves the key.
        """
        ...
    ...
    * process exceptions — :meth:`add_process_exception`. An exe whose
    ...
    def add_op_mask_rule(
        ...
        *,
        ...
        """Add an OP-MASKED THREE-STATE whitelist prefix rule.

        ``allow_mask`` / ``deny_mask`` are bitfields (READ=1, WRITE=2,
        EXECUTE=4, DELETE=8). For an event whose path matches the prefix: op bit
        in ``deny_mask`` ⇒ hard DENY; else op bit in ``allow_mask`` ⇒ bypass the
        filter (ALLOW); else (op in neither) ⇒ falls through to the filter
        (⇒ ASK). No-op (``False``) when the loaded DLL predates the op-masked
        whitelist export.
        """
        ...
    ...
    def add_op_mask_allow_rule(
        ...
        """Back-compat: add an allow-only op-mask prefix (deny_mask=0).

        Delegates to :meth:`add_op_mask_rule` with ``deny_mask=0``. Retained so
        existing callers passing a single allow ``mask`` keep working.
        """
        ...
    ...
    def remove_op_mask_rule(
        self, path: str, *, session_only: bool = True
    ) -> bool:
        """Remove an op-masked (white_ops) prefix rule by path (idempotent).

        Mirror of :meth:`remove_allow_rule` for the op-mask list — undoes a
        deny-effect grant's native ``deny_mask`` rule on revocation. No-op
        (``False``) when the loaded DLL predates the delete export.
        """
        ...
