...
class RequestPermissionUseCase:
    ...
    * ``APPROVE``    → *when* ``allow_smart_approve_auto`` is True
      auto-approve the request (publish :class:`PermissionApprovedEvent`);
      the returned request is APPROVED. **Default is False** (H-Sec-5,
      2026-07-28) — an LLM classifier's APPROVE is treated as a hint and
      the request stays PENDING for human review. Flip the flag on at the
      DI seam via ``Settings.security.smart_approval_llm_allow_auto_approve``
      only when the operator explicitly trusts the endpoint's authority.
    * ``REJECT``     → auto-reject (publish :class:`PermissionRejectedEvent`);
      the returned request is REJECTED. **REJECT is fail-safe and is not
      gated by** ``allow_smart_approve_auto`` — a REJECT auto-rejects
      regardless.
    * ``UNDECIDED``  → leave the request PENDING for a human reviewer
    ...
    def __init__(
        ...
        *,
        ...
        allow_smart_approve_auto: bool = False,
    ) -> None:
        ...
        # H-Sec-5 (2026-07-28): security-conservative default. See
        # ``Settings.security.smart_approval_llm_allow_auto_approve``.
        self._allow_smart_approve_auto = allow_smart_approve_auto
    ...
    async def _apply_smart_approval(
        ...
        *,
        ...
        # H-Sec-5 (2026-07-28) gate: unless the operator opted in via
        # ``allow_smart_approve_auto=True``, a bare APPROVE is downgraded
        # to UNDECIDED — the request stays PENDING for human review.
        # Rationale: a poisoned / mis-configured LLM endpoint that
        # returns APPROVE must NOT be a terminal authorization; the
        # operator dialog is the second line of defence. REJECT is
        # UNAFFECTED below (fail-safe: auto-reject is always allowed).
        if (
            decision is SmartApprovalDecision.APPROVE
            and not self._allow_smart_approve_auto
        ):
            logger.info(
                "security.request_permission.smart_approve_downgraded",
                request_id=request.request_id.value,
            )
            return None
