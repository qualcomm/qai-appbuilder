...
def _path_under_dir(dir_path: str, resource_path: str) -> bool:
    """Return ``True`` when ``resource_path`` is nested under ``dir_path``.

    A boundary-safe, case-insensitive (Windows) lexical prefix test used by
    :meth:`PermissionWaitRegistry.pending_request_ids_under_path` to batch-
    resolve sibling ASKs when the user authorizes a whole directory. Mirrors
    the domain matcher ``check_permission._grant_path_ancestor_of`` (a real
    path-component boundary — ``C:\\foo`` matches ``C:\\foo\\bar`` but NOT
    ``C:\\foobar``) but is duplicated here (documented) so the application
    registry stays dependency-free of the use-case layer. Purely lexical: no
    ``resolve()`` / disk IO (the dedupe-index path is the native
    ``FilterEventV2.file_path``, already an absolute OS path). Never raises.

    Unlike the domain matcher, EQUALITY also counts as "under" here: a
    pending ASK whose exact path IS the granted directory belongs to that
    directory's batch. Callers that need strict-descendant semantics can
    still compare separately.
    """
    if not dir_path or not resource_path:
        return False
    d = dir_path.replace("\\", "/").rstrip("/")
    r = resource_path.replace("\\", "/").rstrip("/")
    if not d:
        return False
    d_cmp = d.casefold()
    r_cmp = r.casefold()
    return r_cmp == d_cmp or r_cmp.startswith(d_cmp + "/")
...
class PermissionResolution:
    ...
    Invariant (L-Py-3): ``allow`` and ``timed_out`` are mutually exclusive.
    A timed-out ASK is by construction a DENY (the V1 60s-elapsed path
    ALWAYS synthesised a ``Decision.DENY``), so an ``allow=True`` +
    ``timed_out=True`` combination is contradictory and would leak into
    downstream audit / grant persistence as an "approved timeout". Raising
    here catches the misuse at construction time rather than corrupting
    the record.
    """
    ...
    def __post_init__(self) -> None:
        if self.allow and self.timed_out:
            raise ValueError(
                "PermissionResolution: allow=True is mutually exclusive with "
                "timed_out=True — a timed-out ASK is by construction a DENY."
            )
...
class PermissionWaitRegistry:
    ...
    def pending_request_ids_under_path(
        self, dir_path: str, *, exclude: str = ""
    ) -> list[str]:
        """Return the request_ids of LIVE native ASKs whose path is under ``dir_path``.

        Companion to :meth:`pending_request_ids_by_pid` but keyed by PATH:
        walks the ``(pid, path, event)`` dedupe index and returns every live
        (unresolved) request_id whose registered ``path`` equals or is nested
        under ``dir_path`` (boundary-safe, case-insensitive — see
        :func:`_path_under_dir`). Introduced for the directory batch-release
        (``ApprovePermissionUseCase``): when the operator authorizes a WHOLE
        directory, the already-queued sibling ASKs whose files fall under that
        directory are resolved ALLOW in one shot instead of each waiting for
        its own click.

        ``exclude`` drops one request_id (the primary request the operator
        actually clicked — the caller resolves it separately via the normal
        approve path). Only reflects native-subprocess ASKs (those registered
        with a ``(pid, path, event)`` key via :meth:`register_or_dedupe`);
        in-process ASKs carry no path key and are irrelevant here. Snapshot
        semantics like :meth:`list_pending` (a concurrent resolve may prune an
        id between listing and touching it — the caller's :meth:`resolve` is a
        no-op on an already-resolved id, so the race is benign). Never raises.
        """
        rids: list[str] = []
        for (_pid, path, _event), rid in self._dedupe_index.items():
            if rid == exclude:
                continue
            fut = self._waiters.get(rid)
            if fut is None or fut.done():
                continue
            if _path_under_dir(dir_path, path):
                rids.append(rid)
        return rids
