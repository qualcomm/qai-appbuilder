...
async def _handle_inject_frame(
    *,
    ...
    into THIS turn: if the turn ends with no further tool round, there is no
    inter-round seam to fold it into. In that case the run loop's
    normal-completion tail emits an ``inject_as_new_turn`` frame carrying the
    text, and the frontend inserts it at the HEAD of the tab's send queue — so
    it reaches the model on the next turn, ahead of any ordinary backlog. No
    user content is lost on the completion path.

    Abort / error exits still drop pending injections: they are semantically
    "user cancelled", where resurrecting an instruction the user just stopped
    would be wrong.
    """
    tab_id_raw = frame.get("tab_id")
    text = frame.get("text")
    ...
    # NEW (M5): if the target tab is streaming, fire the cooperative soft-steer
    # signal on its abort handle so long-running side-effect tools (bash /
    # exec) may voluntarily background themselves and let this fresh user
    # input reach the model promptly at the next inter-round seam.  Best
    # effort: `getattr` probe keeps older registries safe.  Never aborts the
    # turn (independent of `abort` / `signal`).
    _notify = getattr(
        container.chat.abort_registry, "notify_soft_steer", None,
    )
    if callable(_notify):
        try:
            _notify(TabId.of(tab_id_raw))
        except Exception:  # noqa: BLE001 — never break the inject path
            pass
    # ALSO fire the HARD steer signal so *interruptible* tools (question /
    # vibe_wait / hub_wait) abort their pure-wait immediately, per the M5+M6
    # full-alignment plan.  Non-interruptible tools ignore this event —
    # only ``notify_soft_steer`` (above) is cooperative for them.
    _notify_hard = getattr(
        container.chat.abort_registry, "notify_steer_hard", None,
    )
    if callable(_notify_hard):
        try:
            _notify_hard(TabId.of(tab_id_raw))
        except Exception:  # noqa: BLE001 — never break the inject path
            pass
    logger.info(
        "chat.control.inject",
