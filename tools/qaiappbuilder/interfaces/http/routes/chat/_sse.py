...
def _extract_image_refs(prompt_text: str) -> tuple[str, ...]:
    ...
    ONE口径. V1 parity: the WebUI prepends uploaded
...
def _resolve_image_refs_to_vision_blocks(
    *,
    ...
    fmt, quality = _read_cloud_image_config(container)
...
def _read_cloud_image_config(container: "Container") -> "tuple[str, int]":
    """Read the cloud-image (format, quality) from forge_config, per turn.

    ``chat.image_cloud_format`` (webp|jpeg|png) + ``chat.image_cloud_quality``
    (1..100) are written by ``PUT /api/settings/image_cloud`` (Agent settings
    panel) into ``<data>/config/forge_config.json``; read fresh each turn so a
    change takes effect on the next message WITHOUT a restart. Defaults
    webp / 50 (matches the encoder default). Never raises — a missing/malformed
    config degrades to the defaults so a turn is never broken.
    """
    import json
    ...
    try:
        ...
        if isinstance(f, str) and f.strip().lower() in ("webp", "jpeg", "jpg", "png"):
            fmt = f.strip().lower()
...
async def _stream_chat_sse(
    *,
    ...
    # ── Fire-and-forget channel sync push (V1) ──────
...
async def _with_heartbeat(
    ...
    V1 parity: a single long-lived producer
    ...
    async def stream_chat(
        ...
        wechat_sync_user_id: str | None = Query(
            ...
            description=(
                ...
                "user after stream completion (V1 parity). "
