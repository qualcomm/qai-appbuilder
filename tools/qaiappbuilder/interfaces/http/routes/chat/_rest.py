...
* ``POST   /api/chat/conversations/{id}/compact``  force a compaction pass
...
#: Terminal background-process statuses (mirrors
#: ``qai.platform.background_process.ports.TERMINAL_STATUSES``).  Liveness is
#: derived by EXCLUSION so a status added later counts as live by default —
#: listing the live values instead is what dropped ``ready`` (the steady state
#: of a dev server past its readiness probe) and ``stopping``.
_BGP_TERMINAL_STATUSES: frozenset[str] = frozenset(
    {"exited", "failed", "stopped"}
)

def _iso_from_epoch_ms(value: int) -> str:
    """Render an epoch-millisecond stamp as an ISO-8601 UTC string.

    The background-process manager keeps ``Info.time.{started,updated}`` as
    epoch **milliseconds** (``ports.py`` ``Time``), while every other
    ``ActiveRunItem`` producer already holds a ``datetime``.  Converting here
    keeps ``started_at`` / ``last_active_at`` a single comparable format, which
    the route's final ``sorted(key=...)`` depends on — ISO-8601 UTC strings
    sort lexicographically in time order (fixed-width date, offset always
    ``+00:00``).

    An unusable stamp yields a FAR-FUTURE sentinel rather than the Unix epoch:
    this feeds a busy-state list sorted oldest-first, and epoch would promote
    the one row we know least about to the top.  Sorting it last keeps the
    damage to "shown in the wrong place" instead of "shown first".
    """
    try:
        ms = int(value)
        if ms <= 0:
            # 0 / negative is not "the epoch", it is a missing stamp: the
            # manager fills these in on spawn, so absence means we could not
            # read the record.  Fall through to the sentinel rather than
            # claiming 1970 and sorting this row to the very top.
            raise ValueError(f"non-positive epoch stamp: {ms}")
...
class MessageItem(BaseModel):
    ...
    # ``_row_to_message`` promotes these from
...
class ForkConversationRequest(BaseModel):
    """``POST /api/chat/conversations/{id}/fork`` body.

    Fork creates a new conversation containing a subset of messages from the
    source. The four COUNT slicers (``keep_first_n``, ``keep_last_n``,
    ``keep_first_rounds``, ``keep_last_rounds``) are mutually exclusive — at
    most one may be set. ``up_to_message_id`` is an ANCHOR (cut point): it
    may be used alone (keep everything up to and INCLUDING that message) OR
    combined with ANY one count slicer. When combined, the anchor cuts the
    history first, then the slicer runs on the resulting prefix (e.g.
    ``up_to_message_id + keep_last_rounds=2`` = "the last 2 rounds up to
    this message"; ``up_to_message_id + keep_first_rounds=2`` = "the first
    2 rounds of the history up to this message"). All omitted → full clone.
    ``include_tool_calls=False`` (default) filters the sliced history down to
    a clean user↔assistant Q&A. ``title`` optionally overrides the default
    ``"<source> (fork)"`` name. ``inherit_settings=True`` (default) copies the
    source's ``workspace`` / ``discussion`` meta into the fork.
    """

    keep_first_n: int | None = Field(default=None, ge=0)
    keep_last_n: int | None = Field(default=None, ge=0)
    keep_first_rounds: int | None = Field(default=None, ge=0)
    keep_last_rounds: int | None = Field(default=None, ge=0)
    up_to_message_id: str | None = Field(default=None, max_length=128)
...
class ForceCompactChatRequest(BaseModel):
    """``POST /api/chat/conversations/{id}/compact`` body.

    Body of the FORCE-compact endpoint (Task T retired the legacy
    "just decide, don't compact" contract). Both fields are optional —
    when omitted the endpoint resolves the context window from the
    conversation's default model (falling back to
    :attr:`ForceCompactChatInput.budget_tokens`).
    """
...
class CompactMigrateResponse(BaseModel):
    """``POST /api/chat/conversations/{id}/compact/migrate`` response.

    P5 handoff: fork the source conversation into a fresh one seeded with the
    current session digest so the user can keep the assistant responsive
    once compaction can no longer reclaim enough space (:attr:`ContextSizeResponse.escalation`
    == ``"suggest_handoff"``).
    """
...
class CompactClearResponse(BaseModel):
    """``POST /api/chat/conversations/{id}/compact/clear`` response.

    Drops the persisted compaction checkpoint (digest + reference ledger +
    mid-turn counter) so the next turn compacts from scratch.
    """
...
class ContextSizeResponse(BaseModel):
    ...
    #
    # The "SAME-口径" claim HOLDS (a prior audit wrongly flagged it as false).
    # Both figures are whole-wire measurements carrying exactly ONE share of the
    # per-turn runtime overhead: ``compacted_tokens`` because the provider
    # measured the whole wire, and ``used_tokens`` because
    # ``conv.full_history_tokens`` is SEEDED from such a measurement
    # (``streaming.py:5155``) and then grows by overhead-cancelling deltas
    # (``streaming.py:5178-5180``). See ``overhead_tokens`` below and
    ...
    raw_ratio: float = 0.0
    # Appended field (AGENTS.md §3.1 — tail-only growth): P5 handoff escape
    # hatch signal (CONTEXT-COMPRESSION-NEXT v3.1 §7.2 / D11). ``"ok"`` is
    # the byte-for-byte pre-P5 payload — old clients that never read the
    # field see EXACTLY what they always did. ``"suggest_handoff"`` iff the
    # current turn tripped ≥ 3 mid-turn compactions AND a P2 session digest
    # already exists; the UI then exposes a "migrate to a fresh
    # conversation" entry. Sourced from
    # :attr:`ContextDecisionResult.escalation` (or
    # :attr:`ForceCompactChatResult.escalation` on ``POST /compact``), which
    # reads the streaming use case's :class:`CompactionCheckpointEngine`.
    escalation: Literal["ok", "suggest_handoff"] = "ok"
    # Appended field (AGENTS.md §3.1 — tail-only growth): whether the P2
    # session-digest refresh is STILL RUNNING for this conversation. The
    # ``/compact`` reply returns as soon as the 4-phase drop finishes, but the
    # LLM summary behind it takes 20-40s (growing with history length), and
    # ``/compact migrate`` REQUIRES that summary. Without this flag the UI had
    # no way to tell the user "the summary is still being written" — they saw
    # "✅ compacted" and immediately hit a migrate refusal. ``False`` is the
    # byte-for-byte pre-feature payload for clients that never read it.
    digest_summarising: bool = False
    # Appended fields (AGENTS.md §3.1 — tail-only growth): badge caliber
    # provenance (CONTEXT-COMPACTION.md §六).
    #
    # ``overhead_tokens`` — the per-turn runtime overhead (system prompt + tool
    # schemas + persona + skill blocks) folded into ``compacted_tokens``.
    # DIAGNOSTIC / EXPLANATORY ONLY: it MUST NOT be added to
    # ``raw_used_tokens`` to compute "省 N%".
    #
    # WHY NOT: ``raw_used_tokens`` (``conv.full_history_tokens``) is NOT a pure
    # history counter. It is SEEDED from a provider-measured WHOLE wire at
    # ``streaming.py:5155`` (``full_history_tokens = eff_prompt + completion``,
    # where ``eff_prompt`` includes the overhead) and afterwards grows only by
    # pure-history deltas (``streaming.py:5178-5180``, where the two
    # ``eff_prompt`` readings cancel their overhead). So it already carries
    # exactly ONE overhead snapshot — the SAME single share
    # ``compacted_tokens`` carries. ``1 - compacted/raw_used`` is therefore
    # already the same-baseline "how much less did this turn actually send"
    # ratio. Adding the overhead again makes the baseline
    # ``pure_history + 2×overhead``, a wire that never existed (it overstated
    # the saving by 6-7 points and broke the badge's on-screen arithmetic).
    # ``None`` when it cannot be derived (no checkpoint / pre-fold-in
    # checkpoint).
    overhead_tokens: int | None = None
    # The model whose provider measurement produced ``compacted_tokens``.
    # Differs from the caller's ``model_id`` right after a model switch, when
    # the new model has not yet run a round: the figure then describes a wire
    # assembled with ANOTHER model's tool schemas and cache state. Surfacing
    # the provenance lets the UI mark the number provisional instead of
    # implying it measures the current model. ``None`` when the figure came
    # from the checkpoint bootstrap estimate rather than a measurement.
    compacted_measured_model_id: str | None = None
...
class ListChatToolsResponse(BaseModel):
    ...
    glob / grep / web_fetch / web_search / agent / sub_agent /
...
class DiscussionConfigBody(BaseModel):
    ...
    * omitted / ``null`` — keep the current on/off state and merge only the
    ...
    # Coordinator (主持人/协调者) unification — tail-appended (§3.1). Both
    # ``coordinator_model_id`` and ``coordinator_prompt`` are advisory user
    # switches that ride the same PATCH pipeline as the other discussion
    # knobs. ``coordinator_config`` is the tools/skills override dict — its
    # sole allowed keys are ``allowed_tools`` (list[str]) and
    # ``enabled_skills`` (list[str]); either missing / non-list ⇒ the
    # orchestrator falls back to its dynamic default (all registered tools
    # minus ``agent`` + every globally enabled skill) at ``_resolve_judge``
    # synthesis time. ``None`` on any field leaves the persisted value
    # untouched (PATCH semantics).
    coordinator_model_id: str | None = Field(default=None, max_length=128)
    coordinator_prompt: str | None = Field(default=None, max_length=4000)
    coordinator_config: dict[str, Any] | None = Field(default=None)
...
def build_router(*, container: "Container") -> APIRouter:
    ...
    def _digest_summarising(conversation_id: str) -> bool:
        """Whether a P2 session-digest refresh is still running (best-effort).

        The composer badge renders a "summary in progress" indicator off this
        flag: ``/compact`` returns in milliseconds while its background LLM
        summary takes 20-40s, and until that lands the conversation has no
        digest to hand off. Degrades to ``False`` on ANY miss (engine unwired,
        stream use case absent, probe raised) — a diagnostic badge must never
        break a context read.
        """
        stream_uc = getattr(container.chat, "stream_chat_use_case", None)
        engine = (
            getattr(stream_uc, "compaction_engine", None)
            if stream_uc is not None
            else None
        )
        probe = getattr(engine, "digest_refresh_in_flight", None)
        if not callable(probe):
            return False
        try:
            return bool(probe(conversation_id))
        except Exception:  # noqa: BLE001 — badge flag, never fatal
            return False
    ...
    async def fork_conversation(
        ...
        use_case = container.chat.fork_conversation_use_case
        if use_case is None:  # pragma: no cover
            from fastapi import HTTPException
            raise HTTPException(status_code=501, detail="Fork not available")
        try:
            result = await use_case.execute(
                ForkConversationInput(
                    source_id=ConversationId.of(conversation_id),
                    keep_first_n=body.keep_first_n,
                    keep_last_n=body.keep_last_n,
                    keep_first_rounds=body.keep_first_rounds,
                    keep_last_rounds=body.keep_last_rounds,
                    up_to_message_id=body.up_to_message_id,
                    include_tool_calls=body.include_tool_calls,
                    title=body.title,
                    inherit_settings=body.inherit_settings,
                ),
            )
        except ValueError as exc:
            # The use case raises a plain ``ValueError`` for bad slicer combos
            # / an ``up_to_message_id`` not found in the source. Translate to
            # the project's semantic ``ValidationError`` so the GLOBAL error
            # handler serializes it into the unified ``{type, code, message}``
            # envelope the frontend's ``parseApiError`` expects. A bare
            # ``HTTPException(detail=...)`` here would emit ``{"detail": ...}``,
            # which the client rejects as a non-conforming error body — hiding
            # the real reason behind a generic "HTTP 400 returned a
            # non-conforming error body" toast (the reported fork bug).
            from qai.platform.errors import ValidationError

            raise ValidationError(
                code="chat.fork.invalid_request",
                message=str(exc),
            ) from exc
    ...
    async def compact_chat(
        conversation_id: str,
        body: ForceCompactChatRequest,
    ) -> ContextSizeResponse:
        # Task T: this endpoint now FORCES a compaction pass (was: pure
        # read-only decision — that identity moved to GET /context via
        # ``GetContextDecisionUseCase``). ``model_id`` resolves the true
        # context window through the same helper GET /context uses so both
        # endpoints share one budget-口径. Falls back to
        # ``ForceCompactChatInput.budget_tokens`` (200K) when the caller does
        # not supply a model.
        force_uc = container.chat.force_compact_chat_use_case
        if force_uc is None:  # pragma: no cover — always wired via build_chat_services
            raise RuntimeError(
                "force_compact_chat_use_case is not wired on the container",
            )
        budget_kwargs: dict[str, Any] = {}
        if body.model_id:
            budget_kwargs["budget_tokens"] = await _resolve_context_window(
                body.model_id, body.provider,
            )
        ...
        # Map ``ForceCompactChatResult`` (before / after / savings / compacted)
        # to the shared ``ContextSizeResponse``:
        #   * ``used_tokens`` = pre-compaction wire size (``before_tokens``);
        #   * ``compacted_tokens`` = post-compaction figure when the engine
        #     actually compacted (``None`` on a no-op — preserves the shape
        #     GET /context uses);
        #   * ``compacted`` mirrors :attr:`ForceCompactChatResult.compacted`;
        #   * ``needs_compaction`` derives from the POST-compaction ratio —
        #     ``True`` iff the pass could not fit inside the window (rare;
        #     signals the migrate escape hatch is the safer bet).
        return ContextSizeResponse(
            used_tokens=result.before_tokens,
            budget_tokens=result.budget_tokens,
            ratio=(
                result.before_tokens / result.budget_tokens
                if result.budget_tokens > 0
                else 0.0
            ),
            needs_compaction=(
                result.ratio_after >= INTER_ROUND_COMPRESS_THRESHOLD_RATIO
            ),
            raw_used_tokens=result.before_tokens,
            raw_ratio=(
                result.before_tokens / result.budget_tokens
                if result.budget_tokens > 0
                else 0.0
            ...
            compacted_tokens=result.after_tokens if result.compacted else None,
            # A successful pass kicks the P2 summariser, so this is normally
            # ``True`` right here — the reply can tell the user the summary is
            # still being written instead of implying everything is done.
            digest_summarising=_digest_summarising(conversation_id),
        )
    ...
    async def get_context_size(
        ...
        # Read-only context-decision query. Backs the UI context badge.
        ...
        # model_ids under different providers. Without ``model_id`` the
        # caller-supplied ``budget_tokens`` default (8192) is honoured —
        # fully backwards-compatible with legacy badge callers.
        if model_id:
            budget_tokens = await _resolve_context_window(model_id, provider)
        result = await container.chat.context_decision_use_case.execute(
            GetContextDecisionInput(
                conversation_id=ConversationId.of(conversation_id),
        ...
        #
        # ``compacted_badge_detail`` DELEGATES to ``estimate_compacted_tokens``
        # for the figure itself (one caliber, one code path) and additionally
        # reports the overhead folded into it plus which model measured it, so
        # the badge can put the denominator in the same caliber and flag a
        # post-model-switch figure as provisional.
        badge_detail = await asyncio.to_thread(
            container.chat.stream_chat_use_case.compacted_badge_detail,
            conv,
            model_id,
        )
        compacted_tokens = (
            badge_detail.get("compacted_tokens")
            if badge_detail is not None
            else None
        )
        overhead_tokens = (
            badge_detail.get("overhead_tokens")
            if badge_detail is not None
            else None
        )
        measured_model_id = (
            badge_detail.get("measured_model_id")
            if badge_detail is not None
            else None
        )
        ...
        # the UI computes. ``overhead_tokens`` / ``measured_model_id`` make the
        # caliber and the provenance of the numerator auditable from one line.
        _log.info(
            ...
            compacted_measured_model_id=measured_model_id,
        ...
        return ContextSizeResponse(
            ...
            escalation=result.escalation,
            # Surface the in-flight P2 summary so the composer badge can show
            # a "summary in progress" indicator (and the user knows why
            # ``/compact migrate`` would refuse right now).
            digest_summarising=_digest_summarising(conversation_id),
            overhead_tokens=overhead_tokens,
            compacted_measured_model_id=measured_model_id,
        )
    ...
    @router.post(
        "/conversations/{conversation_id}/compact/migrate",
        response_model=CompactMigrateResponse,
    )
    async def compact_migrate(
        conversation_id: str,
    ) -> CompactMigrateResponse:
        # Task V F8 fix: expose the P5 handoff (fork the current conversation
        # into a fresh one seeded with the session digest) to web/CLI clients.
        # Was previously reachable only via the IM `/compact migrate` verb.
        migrate_uc = container.chat.migrate_conversation_use_case
        if migrate_uc is None:
            from qai.platform.errors import ValidationError

            raise ValidationError(
                code="chat.compact.migrate_unavailable",
                message="Conversation migration is not available.",
            )
        try:
            result = await migrate_uc.execute(
                MigrateConversationInput(
                    source_conversation_id=ConversationId.of(conversation_id),
                ),
            )
        except ValueError as exc:
            # Business refusal (no checkpoint / empty session summary) — NOT a
            # server fault. Translate to the project's semantic
            # ``ValidationError`` so the GLOBAL error handler serializes it
            # into the unified ``{type, code, message}`` envelope the
            # frontend's ``parseApiError`` expects (same pattern as the fork
            # endpoint above — a bare ``HTTPException`` / hand-rolled
            # ``JSONResponse`` emits a shape the client rejects as
            # non-conforming, hiding the real reason).
            from qai.platform.errors import ValidationError

            raise ValidationError(
                code="chat.compact.migrate_no_summary",
                message=str(exc),
            ) from exc
        return CompactMigrateResponse(
            new_conversation_id=result.new_conversation_id.value,
            source_conversation_id=result.source_conversation_id.value,
        )

    @router.post(
        "/conversations/{conversation_id}/compact/clear",
        response_model=CompactClearResponse,
    )
    async def compact_clear(
        conversation_id: str,
    ) -> CompactClearResponse:
        # Task V F7 fix: expose "drop the persisted compaction checkpoint"
        # (digest / reference ledger / mid-turn counter) to web/CLI clients.
        # Was previously reachable only via the IM `/compact clear` verb.
        # Delete the store row FIRST, then invalidate the engine's in-memory
        # cache — same ordering as ``apps/api/_chat_message_bridge.compact_clear``
        # (Task L P0-3): if store.delete raises we skip the engine invalidate
        # and surface a 500 so the two sides stay coherent.
        store = container.chat.compaction_checkpoint_store
        stream_uc = container.chat.stream_chat_use_case
        engine = (
            getattr(stream_uc, "compaction_engine", None)
            if stream_uc is not None
            else None
        ...
        if engine is not None:
            engine.invalidate(conv_id.value)
        return CompactClearResponse(cleared=True)
    ...
    async def list_chat_tools() -> ListChatToolsResponse:
        ...
        # ``computer`` (desktop control) is registered UNCONDITIONALLY so the
        # Settings toggle can take effect per-turn without a restart; the real
        # gate lives in ``StreamChatUseCase._computer_enabled``. Enumerating the
        # raw registry here therefore advertised a tool the model would never
        # receive: every front-end surface fed by this route (the per-session
        # tool popover, the Discussion role allowlist) listed ``computer`` while
        # it was disabled, inviting the user to "enable" something that did
        # nothing. Worse, desktop control is exactly the capability that must be
        # an explicit opt-in — showing it as available misrepresents the security
        # posture.
        #
        # Reuse the SAME reader the streaming gate uses (wired onto
        # ``ChatServices.computer_enabled``) so the advertised set and the
        # enforced set cannot drift. Absent / failing reader ⇒ treated as
        # disabled, matching the streaming gate's secure default.
        _computer_on = False
        _reader = getattr(container.chat, "computer_enabled", None)
        if callable(_reader):
            try:
                _computer_on = bool(_reader())
            except Exception:  # noqa: BLE001 — never fail the catalogue
                _computer_on = False
        ...
        for schema in advertised:
            ...
            if not isinstance(name, str) or not name:
                continue
            if name == "computer" and not _computer_on:
                continue
            descriptors.append(
                ChatToolDescriptor(
                    name=name,
    ...
    async def list_active_runs() -> ActiveRunsResponse:
        ...
        # §UX-idle-with-bg-work — background processes still in flight.
        # Without this row the UI shows a tab as IDLE (no typing indicator, no
        # Stop) while a command it started is still running, because the front
        # end's busy predicate only ever knew about sub-agents.  A
        # long-running exec is just as much "the agent is still working" from
        # the user's point of view.
        #
        # Scope: this endpoint is deliberately PROCESS-WIDE — it takes no
        # conversation / tab parameter and the two producers above also list
        # every live chat and sub-agent run, because its consumer (the
        # composer's "running sessions" popover) exists to show work the user
        # may have navigated away from.  So an unfiltered ``list()`` is the
        # matching behaviour, not an oversight: each row carries its owning
        # ``conversation_id`` (for a managed exec the manager's ``session_id``
        # IS the conversation id — see ``di.py``'s ``adopt(session_id=...)``,
        # which passes ``_soft_ctx.conversation_id``) so the caller can scope
        # client-side.  Tab-level busy state does NOT come from here; it is
        # driven by the store's SSE-fed index.
        #
        # ``openable=False``: a process has no transcript to drill into,
        # unlike a chat tab or a sub-agent session.
        _bgp_manager = getattr(container.background_process, "manager", None)
        if _bgp_manager is not None:
            try:
                _procs = await _bgp_manager.list()
            except Exception:  # noqa: BLE001 — never break the listing
                _procs = []
            for _p in _procs:
                # Live == NOT terminal.  Enumerating the live values instead
                # silently dropped ``ready`` (a dev server that has passed its
                # readiness probe — the steady state for the very processes
                # this row exists to surface) and ``stopping``.
                if getattr(_p, "status", None) in _BGP_TERMINAL_STATUSES:
                    continue
                _started = getattr(_p, "time", None)
                _started_ms = getattr(_started, "started", 0) or 0
                _updated_ms = getattr(_started, "updated", 0) or _started_ms
                items.append(
                    ActiveRunItem(
                        kind="background_process",
                        id=_p.id,
                        conversation_id=_p.session_id or None,
                        title=_p.description or _p.command,
                        status=_p.status,
                        started_at=_iso_from_epoch_ms(_started_ms),
                        last_active_at=_iso_from_epoch_ms(_updated_ms),
                        openable=False,
                        stop=ActiveRunStopSpec(
                            path=f"/api/background_process/{_p.id}/stop",
                            body={},
                        ),
                    ),
                )
    ...
    async def interrupt_subagent(
        ...
        #
        # N.51: cancellation goes through ``sub_agent_bridge`` rather than
        # straight to the abort registry. The bridge is the ONE place that
        # knows a sub-agent has TWO pieces of live state — its async-job
        # admission slot and its abort flag — and cancels both. Signalling only
        # the flag would leave the job ``RUNNING`` forever, holding a
        # concurrency slot that no longer corresponds to any work and starving
        ...
        # ``True`` only when something was actually cancelled (i.e. the
        # sub-agent really was in-flight). When the sub-agent has already
        # finished, ``aborted`` is ``False`` and the request is still ``ok``
        # (idempotent no-op). Response shape is unchanged (§3.1).
        await container.chat.subagent_sessions.get(
            SubAgentSessionId.of(subagent_id),
        )
        aborted = await container.chat.sub_agent_bridge.cancel(subagent_id)
    ...
    async def get_discussion_config(
        ...
        return DiscussionConfigBody(
            ...
            coordinator_model_id=d.get("coordinator_model_id"),
            coordinator_prompt=d.get("coordinator_prompt"),
            coordinator_config=d.get("coordinator_config"),
        )
    ...
    async def set_discussion_config(
        ...
        discussion = await container.chat.set_discussion_config_use_case.execute(
            SetDiscussionConfigInput(
                ...
                coordinator_model_id=body.coordinator_model_id,
                coordinator_prompt=body.coordinator_prompt,
                coordinator_config=body.coordinator_config,
            ),
        )
        d = discussion or {}
        return DiscussionConfigBody(
    ...
    # ---- Multi-agent discussion: implementation plan generation (§22.4) -

    @router.post(
        "/conversations/{conversation_id}/implementation/generate",
        response_model=ImplementationPlanBody,
        status_code=201,
    )
    async def generate_implementation_plan(
        ...
        """Generate an implementation plan from the discussion conclusion (DISC-1 §22.4).

        Calls the LLM feature-item extractor on the conversation's current
        discussion state and persists the resulting plan.  Returns 201 on success
        (plan created), 409 if a non-empty plan already exists.
        """
        discussion = (
            await container.chat.generate_implementation_plan_use_case.execute(
                GenerateImplementationPlanInput(
                    conversation_id=ConversationId.of(conversation_id),
                ),
            )
