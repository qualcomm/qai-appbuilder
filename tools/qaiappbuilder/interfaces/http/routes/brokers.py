...
class _ExecProfileDTO(BaseModel):
    ...
    ask_rules: list[dict] = []
    # ``ask_always``: every invocation of a matching binary prompts,
    # regardless of args (whole programs whose mere use is a non-file risk
    # — curl / powershell / reg / certutil …). Surfaced so the WebUI can
    # show WHY a profile with no arg lists still gates a command; without
    # it such a profile looked rule-less in the UI.
        ...
        return ExecProfilesResponse(
            profiles=[
                _ExecProfileDTO(
                    ...
                    ask_always=bool(getattr(p, "ask_always", False)),
                )
