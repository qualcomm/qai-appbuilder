...
class QGenieCounterDTO(BaseModel):
    """One rate-limit window (``limit``/``used``/``remaining``)."""
...
class QGenieBucketDTO(BaseModel):
    """The four windows of one traffic class.

    Every field is optional because the upstream omits some per model, and a
    missing counter must reach the UI as "unknown" rather than a zeroed bar.
    """

    day: QGenieCounterDTO | None = None
    week: QGenieCounterDTO | None = None
    rpm: QGenieCounterDTO | None = None
    tpm: QGenieCounterDTO | None = None


class QGenieModelQuotaDTO(BaseModel):
    """Both traffic-class buckets for one model.

    Keyed by the wire spellings (``Api`` / ``UI``) the upstream uses so the
    frontend needs no translation table beyond its display labels.
    """
...
class QGenieCostDTO(BaseModel):
    """Account-wide QGenie spend.

    Carries NO model dimension: the upstream's observe-only writer does not
    store model identifiers, so this is the total across every QGenie model.
    The UI must label it as such — a reader would otherwise assume it belongs
...
class QGenieQuotaResponse(BaseModel):
    """Envelope for ``GET /api/model-catalog/qgenie-quota``.

    ``available`` false means "nothing to show" (no provider, no credential,
    or never reached) and the gauge hides. ``stale`` true means these numbers
    came from the last good read after a refresh failed — shown with their
    ``fetched_at`` rather than blanked, because when the network is down the
    LLM is unusable anyway and the last figures are strictly more useful.
    """
    ...
    fetched_at: str = ""
    stale: bool = False
    # Why there is nothing (or only stale data) to show: ``no_api_key`` /
    # ``rate_limited`` / ``unreachable`` / ``not_configured`` / ``bad_base_url``
    # / ``http_<code>``. Lets the UI point at the fix instead of leaving the
    # user to guess why the gauge vanished. ``None`` on a healthy read.
    error: str | None = None
    models: dict[str, QGenieModelQuotaDTO] = {}
    cost: QGenieCostDTO | None = None
...
def _qgenie_counter_to_dto(
    counter: "QGenieCounter | None",
) -> QGenieCounterDTO | None:
    if counter is None:
        return None
    return QGenieCounterDTO(
        limit=counter.limit,
        used=counter.used,
        remaining=counter.remaining,
        reset_in_seconds=counter.reset_in_seconds,
    )


def _qgenie_bucket_to_dto(bucket: "QGenieBucket") -> QGenieBucketDTO:
    return QGenieBucketDTO(
        day=_qgenie_counter_to_dto(bucket.day),
        week=_qgenie_counter_to_dto(bucket.week),
        rpm=_qgenie_counter_to_dto(bucket.rpm),
        tpm=_qgenie_counter_to_dto(bucket.tpm),
    )


def _qgenie_snapshot_to_dto(
    snapshot: "QGenieQuotaSnapshot",
) -> QGenieQuotaResponse:
    """Project the domain snapshot onto the wire shape.

    ``available`` is derived from "are there any models" rather than trusted
    from a flag: an empty snapshot is exactly what every failure path returns,
    so this keeps the gauge hidden without the route having to classify why.
    """
    models = snapshot.models or {}
    cost = snapshot.cost
    return QGenieQuotaResponse(
        ...
        models={
            model_id: QGenieModelQuotaDTO(
                Api=_qgenie_bucket_to_dto(quota.api),
                UI=_qgenie_bucket_to_dto(quota.ide),
            )
        ...
        cost=(
            QGenieCostDTO(
                ...
                day_usd=cost.day_usd,
                day_by_class=cost.day_by_class,
                month_usd=cost.month_usd,
                tier=cost.tier,
                day_cap_usd=cost.day_cap_usd,
                month_cap_usd=cost.month_cap_usd,
            )
            if cost is not None
            else None
        ),
...
def build_router(*, container: "Container") -> APIRouter:
    ...
    @router.get("/qgenie-quota", response_model=QGenieQuotaResponse)
    async def get_qgenie_quota(force: bool = False) -> QGenieQuotaResponse:
        """Current QGenie dual-bucket quota + account spend.

        Feeds the sidebar gauge while a QGenie model is selected. Deliberately
        NOT polled: the adapter is read when the user switches onto a QGenie
        model and again after each turn ends, which is exactly when the numbers
        can have moved. A read costs ~860 ms upstream and the endpoints throttle
        at roughly 60 s, so a polling loop would spend the budget the
        exhaustion check needs.

        ``force=true`` bypasses the adapter cache. The exhaustion check uses it
        because deciding to switch buckets — or telling the user both are dry —
        on a cached figure could be wrong in either direction.

        **internal-only**: the QGenie provider ships only with the internal
        edition (``src/qai/platform/edition/`` is excluded from the release
        bundle), so an external build answers ``available: false`` without
        touching the network. Mirrors the ``/query-services`` gate below.

        Always 200. Every failure path (external edition, no provider, no
        credential, throttled, unreachable) answers ``available: false``, or the
        last good snapshot with ``stale: true``; the gauge hides on the former
        and keeps showing the latter. A quota read is never a reason to fail a
        request.
        """
        settings = getattr(container, "settings", None)
        if settings is None or not getattr(settings, "is_internal", False):
            return QGenieQuotaResponse(error="not_configured")
        use_case = getattr(
            container.model_catalog, "get_qgenie_quota_use_case", None
        )
        if use_case is None:
            return QGenieQuotaResponse(error="not_configured")
        snapshot = await use_case.execute(force=force)
        return _qgenie_snapshot_to_dto(snapshot)
