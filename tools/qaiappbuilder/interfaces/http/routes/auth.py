...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from qai.platform.config.settings import AuthSettings, QaiServiceSettings
...
def _resolve_qai_service_config() -> "QaiServiceSettings":
    """Return the effective QAI Service config (typed defaults + deployment).

    Two layers, deliberately:

    * :class:`QaiServiceSettings` supplies the typed defaults and — crucially
      — carries NO endpoint literal, because ``settings.py`` ships in both
      editions and a hardcoded intranet host there would trip the external
      artifact's sensitive-word scan.
    * The deployment values are authored in
      ``factory/_source/forge_config.json`` and land, via the factory
      compiler, in ``user_config.toml`` under ``[forge] qai_service`` (the
      compiler folds the whole forge document into that one table — see
      ``tools/build/factory_compiler/forge_config.py``).

    Read order: the runtime copy ``data/user_config.toml`` first (an operator
    edit wins), then the shipped default ``factory/user_config.toml``. Read
    live per call so an edit applies without a restart.

    Two seams that look right and are NOT:

    * ``Settings.get_legacy_config("forge_config", …)`` resolves against
      ``data.config_dir``, whose default is the repo-root ``config/`` this
      project no longer has (PROJECT-RULES §3.8.1) — it silently always
      returns the fallback.
    * ``data/config/forge_config.json`` exists but holds only the subset the
      running app has written back (``genie_service``, runtime security
      state, …); the factory defaults never land there.

    Any failure degrades to the typed defaults, whose empty ``base_url`` means
    "not deployed" → the caller no-ops.
    """
    from qai.platform.config.settings import QaiServiceSettings, load_settings

    try:
        settings = load_settings()
        defaults = settings.qai_service
    except Exception:  # noqa: BLE001 — never break login on a config fault
        return QaiServiceSettings()

    repo_root = Path(__file__).resolve().parents[3]
    candidates = (
        Path(settings.data.data_dir) / "user_config.toml",
        repo_root / "data" / "user_config.toml",
        repo_root / "factory" / "user_config.toml",
    )
    for path in candidates:
        block = _read_toml_forge_block(path, "qai_service")
        if block is None:
            continue
        try:
            return QaiServiceSettings(
                ...
                base_url=str(
                    block.get("base_url", defaults.base_url) or ""
                ).rstrip("/"),
                exchange_timeout_seconds=float(
                    block.get(
                        "exchange_timeout_seconds",
                        defaults.exchange_timeout_seconds,
                    )
                    or defaults.exchange_timeout_seconds
                ),
                ssl_verify=bool(block.get("ssl_verify", defaults.ssl_verify)),
            )
        except (TypeError, ValueError):
            # A malformed value must not break login; keep looking, then fall
            # through to the typed defaults.
            continue
    return defaults


def _read_toml_forge_block(path: Path, name: str) -> "dict[str, object] | None":
    """Return ``[forge].<name>`` from the TOML at *path*, or ``None``.

    ``None`` covers every "nothing usable here" case — file absent,
    unparsable, or the table missing — so the caller can simply try the next
    ...
    if not isinstance(forge, dict):
        return None
    block = forge.get(name)
    return block if isinstance(block, dict) else None


async def _exchange_qai_service_token(id_token: str) -> None:
    """Trade the verified ``id_token`` for a QAI Service JWT (best effort).

    Called from ``/callback`` while the ``id_token`` is still fresh, just
    before the Okta tokens are discarded. The minted JWT is held by
    ``interfaces.http.auth.qai_service_token`` (in memory, and cached in the
    SecretStore so it survives a restart) and read per-request by the chat
    model-resolution bridge as the outbound bearer for the token pool's
    models.

    Never raises: an exchange failure must not block a login that already
    succeeded. The local session is valid regardless, and every model outside
    the pool keeps working.

    A failure IS however logged at ERROR with the HTTP status, because a user
    who completed SSO but cannot obtain a pool token is a defect on our side
    (their broker account should exist) — not an expected end-user state. The
    status distinguishes the causes: 401 invalid token, 403 email domain not
    allowed, 400 no email claim, 0 transport/config.

    Configuration is read live from the forge config document (deployment
    data) with the typed :class:`QaiServiceSettings` defaults as the
    fallback, so an operator edit applies to the next login without a
    restart. An empty ``base_url`` means "not deployed" → silent no-op.
    """
    from interfaces.http.auth.qai_service_token import (
        ExchangeError,
        exchange_id_token,
        store_service_jwt,
    )

    cfg = _resolve_qai_service_config()
    if not cfg.enabled or not cfg.base_url:
        return
    if not id_token:
        logger.error("auth.qai_service.exchange_failed", reason="no id_token in bundle")
        return

    try:
        token, expires_in = await exchange_id_token(
            id_token,
            base_url=cfg.base_url,
            ssl_verify=cfg.ssl_verify,
            timeout=cfg.exchange_timeout_seconds,
        )
    except ExchangeError as exc:
        logger.error(
            "auth.qai_service.exchange_failed",
            status=exc.status,
            error=str(exc),
        )
        return
    except Exception as exc:  # noqa: BLE001 — login must never break
        logger.error("auth.qai_service.exchange_failed", error=repr(exc))
        return

    store_service_jwt(token, expires_in)
    logger.info("auth.qai_service.exchange_ok", expires_in=expires_in)
...
def build_router(
    *,
    ...
    async def auth_login(request: Request) -> RedirectResponse:
        ...
        # ``reexchange=1`` forces a full Okta round-trip EVEN when a valid
        # session cookie is present. This exists for one job: re-minting the
        # QAI Service pool JWT when the cookie is still valid but the broker
        # token is gone (backend restart with an expired persisted token, or a
        # first launch that never had one). That token is only ever minted in
        # ``/callback``, so the "already signed in" short-circuit below would
        # otherwise make silent recovery impossible. When the Okta IdP session
        # is still live the ``/authorize`` step is a transparent 302 back to
        # ``/callback`` (no login page), so a hidden-iframe caller re-runs the
        # exchange without the user seeing anything. Query flag, so a normal
        # navigation to ``/auth/login`` keeps its fast short-circuit.
        force_reexchange = request.query_params.get("reexchange") == "1"
        ...
        # Already signed in → skip Okta, bounce straight to ``next`` — UNLESS
        # a re-exchange was explicitly requested (see above), in which case we
        # deliberately proceed to Okta to obtain a fresh id_token.
        if (
            not force_reexchange
            and get_current_user(request, settings.session_cookie_name, secret)
            is not None
        ):
            return RedirectResponse(next_path, status_code=303)
    ...
    async def auth_callback(request: Request) -> Any:
        ...
        # LOGIN-ONLY for the local session: the Okta access_token /
        # refresh_token in ``bundle`` have served their purpose (proving
        # possession of the code and anchoring the id_token). Discard them —
        # the HMAC-signed cookie below is the entire session state.
        #
        # The id_token, however, is spent here first: when the QAI Service
        # token pool is configured, it is traded for a short-lived per-user
        # broker JWT while it is still fresh (see
        # ``interfaces/http/auth/qai_service_token``). That JWT — not any
        # user-supplied key — authenticates the pool's models, which is what
        # lets an external developer chat without provisioning a credential.
        await _exchange_qai_service_token(bundle.get("id_token", ""))
        ...
        # Feed the process-level holder immediately at login so the usage
        # reporter reports the logged-in name even before the SPA issues its
        # next authenticated request (the middleware also records it per
        # request). Cleared on logout below.
        ...
        logger.info(
            ...
            # Session lifetime, so a "why was I asked to sign in again?"
            # report can be answered from the log alone: compare this against
            # the next ``auth.renew.*`` / ``login_ok`` entry. Without it the
            # only way to tell an EXPIRY from a spurious re-prompt was to
            # reason about the TTL setting by hand.
            session_ttl_seconds=int(settings.session_ttl_seconds or 0),
        )
        return response
    ...
    async def auth_logout() -> RedirectResponse:
        ...
        # Forget the recorded login name so a post-logout usage report falls
        # back to the OS name (or waits, per the gating in lifespan) rather
        # than reporting the signed-out user.
        clear_last_login_name()
        # Drop the QAI Service JWT too: it is bound to the identity that just
        # signed out, so leaving it live would let the pool keep billing that
        # user's allowance after they logged off.
        from interfaces.http.auth.qai_service_token import clear_service_jwt
    ...
    @router.get("/auth/popup-done", response_class=HTMLResponse)
    async def auth_popup_done() -> HTMLResponse:
        """Landing page shown inside the login popup after a successful
        Okta callback.  Posts a message to the opener (main SPA window)
        and auto-closes.  The cookie is already set by /callback's
        redirect; this page just signals completion."""
        logger.info("auth.popup_done.served")
        return HTMLResponse(_POPUP_DONE_HTML)
    ...
    async def quota_me() -> JSONResponse:
        """Current user's QAI Service token-pool balance.

        A thin, low-frequency proxy to the broker's ``GET /api/quota/me``,
        authenticated with the JWT minted at login. Its only job is to SEED the
        sidebar gauge at page load: a freshly opened window has streamed
        nothing yet, so it has no balance to show. During a conversation the
        number updates for free from the end-of-stream ``quota_usage`` frame —
        this is deliberately NOT polled.

        Always 200 with ``{"quota": null, "error": "..."}`` on every failure
        path (pool not deployed, no token yet, broker unreachable, non-200).
        The frontend hides the gauge when ``quota`` is null, so a user who does
        not use the pool sees nothing — surfacing a 4xx/5xx here would turn an
        irrelevant condition into a visible error.
        """
        cfg = _resolve_qai_service_config()
        if not cfg.enabled or not cfg.base_url:
            return JSONResponse({"quota": None, "error": "not_configured"})

        from interfaces.http.auth.qai_service_token import get_service_jwt

        jwt_token = get_service_jwt()
        if not jwt_token:
            # No exchange yet (auth disabled / pre-login) or the token aged
            # out. Not an error the user can act on from here.
            return JSONResponse({"quota": None, "error": "no_session_token"})

        url = f"{cfg.base_url}/api/quota/me"
        try:
            async with httpx.AsyncClient(
            ...
            logger.warning("auth.qai_service.quota_unreachable", error=str(exc))
            return JSONResponse({"quota": None, "error": "unreachable"})

        if resp.status_code != 200:
            logger.warning(
                "auth.qai_service.quota_rejected", status=resp.status_code
            )
            return JSONResponse(
                {"quota": None, "error": f"http_{resp.status_code}"}
            )
        try:
            body = resp.json()
        except ValueError:
            return JSONResponse({"quota": None, "error": "malformed"})
        # The broker returns the balance FLAT (allocated / used / reserved /
        # remaining / reset_at / period / unit), so it is forwarded under a
        # `quota` key — the shape `stores/quota.ts` narrows.
        return JSONResponse({"quota": body if isinstance(body, dict) else None})
    ...
    async def auth_renew(request: Request) -> JSONResponse:
        ...
        if payload is None or not isinstance(user, dict):
            logger.info("auth.renew.expired_or_missing")
            return JSONResponse(
                {
        ...
        logger.info("auth.renew.ok", username=user.get("username"), new_exp=new_exp)
        return response
...
<head>
  ...
  <style>
    ...
    .msg p { margin: 0; font-size: 14px; color: #666; }
  </style>
</head>
<body>
  <div class="msg">
  ...
  <script>
    (function() {
      try {
        // Signal completion to the SPA. This page is reached via a chain of
        // 302s that detoured cross-origin through Okta, after which a same-
        // origin `window.parent` reference can degrade to a restricted proxy
        // and drop the message silently — the cause of the ~15s of dead
        // waiting we saw (the /callback exchange itself is ~3.6s). Prefer the
        // real top-level window (iframe case), fall back to the opener (popup
        // case), and post with targetOrigin "*": the payload is a bare marker
        // with no secrets, and the receiver (api/auth.ts onMessage) already
        // rejects any message whose event.origin is not our own origin.
        var target = (window.top && window.top !== window)
          ? window.top
          : (window.opener || window.parent);
        if (target && target !== window) {
          target.postMessage({ type: "qai_auth_complete" }, "*");
        }
      } catch (e) {}
      // If in iframe, no need to auto-close (parent removes the overlay).
      // If in popup, close after a brief delay.
      if (window.opener) {
