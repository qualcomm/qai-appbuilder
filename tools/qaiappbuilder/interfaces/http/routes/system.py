...
class WdacStatusResponse(BaseModel):
    """``GET /api/system/wdac`` payload.

    ``enabled`` is True when this host enforces code integrity in a way that
    blocks the UNSIGNED QAIRT converter ``.pyd`` (Model Builder conversion) --
    i.e. UMCI Enforced or Smart App Control On/Eval. The frontend surfaces a
    heads-up on the chat welcome screen when True. Fail-open: any non-Windows
    host or probe error reports False.
    """
...
def build_router(*, container: "Container") -> APIRouter:
    ...
    async def build_info() -> BuildInfoResponse:
        return BuildInfoResponse(
            ...
            version=_app_version(container.repo_root),
    ...
    @router.get("/wdac", response_model=WdacStatusResponse)
    async def wdac_status() -> WdacStatusResponse:
        """Report whether this host enforces code integrity that blocks the
        unsigned QAIRT converter .pyd (so the Web UI can warn that Model
        Builder conversion will not work). Cached; fail-open off-Windows."""
        from apps.api._wdac_probe import get_wdac_status
...
def _app_version(repo_root: Path) -> str:
    ...
    Takes the DI-resolved ``repo_root`` (never re-detects it from ``__file__``)
    so the reported version matches the tree this app was actually built for.
    """
    from qai.platform.config import resolve_app_version

    return resolve_app_version(repo_root)
