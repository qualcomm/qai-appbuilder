...
class AppEntry(BaseModel):
    ...
    models_missing: list[dict[str, object]] | None = None
    builtin: bool = False
...
# ---------------------------------------------------------------------------
# Lightweight model-presence check (mirrors AppProjectProcessManager logic
# without requiring the full process-manager instance — just Path.exists).
# ---------------------------------------------------------------------------
def _dir_has_content(path: Path) -> bool:
    """Return True iff *path* is an existing directory with at least one entry."""
    try:
        if not path.is_dir():
            return False
...
def _expand_model_placeholders(
    raw: str, repo_root: Path, app_dir: Path, model_root: Path, pack_root: Path,
    user_model_root: Path | None, user_pack_root: Path | None,
) -> Path:
    """Expand ``${...}`` tokens mirroring the process manager's expansion."""
    substitutions = {
        "${APP_ROOT}": str(repo_root),
        "${APP_PROJECT_ROOT}": str(app_dir),
        "${APP_BUILDER_MODEL_ROOT}": str(model_root),
        "${APP_BUILDER_PACK_ROOT}": str(pack_root),
    }
    if user_model_root is not None:
        substitutions["${APP_BUILDER_USER_MODEL_ROOT}"] = str(user_model_root)
    if user_pack_root is not None:
        substitutions["${APP_BUILDER_USER_PACK_ROOT}"] = str(user_pack_root)
    expanded = raw
...
def _check_app_models(
    ...
    *,
    ...
    """Return ``(models_ready, missing_list_or_none)`` for a definition.

    Pure filesystem probe — no lock, no shared state. Cheap enough to call
    in a loop inside ``GET /apps`` (one ``is_dir`` + ``iterdir`` per model).
    """
    ...
    for ref in definition.models:
        if not ref.model_dir and not ref.pack_dir:
            continue  # opaque — skip

        paths_tried: list[str] = []
        found = False

        if ref.model_dir:
            resolved = _expand_model_placeholders(
                ref.model_dir, repo_root, app_dir, model_root, pack_root,
                user_model_root, user_pack_root,
            )
            paths_tried.append(str(resolved))
            if _dir_has_content(resolved):
                found = True
            else:
                # Fallback: <model_root>/<id>
                candidate = (model_root / ref.id).resolve()
                paths_tried.append(str(candidate))
                if _dir_has_content(candidate):
                    found = True
                elif user_model_root is not None:
                    candidate2 = (user_model_root / "models" / ref.id).resolve()
                    paths_tried.append(str(candidate2))
                    if _dir_has_content(candidate2):
                        found = True
        else:
            # No model_dir — check fallback layouts
            candidate = (model_root / ref.id).resolve()
            paths_tried.append(str(candidate))
            if _dir_has_content(candidate):
                found = True
                ...
                if _dir_has_content(candidate2):
                    found = True
            # Also check pack_dir presence
            if not found and ref.pack_dir:
                pack_resolved = _expand_model_placeholders(
                    ref.pack_dir, repo_root, app_dir, model_root, pack_root,
                    user_model_root, user_pack_root,
                )
        ...
        if not found:
            # Read provisioning / setup_guide from weights.json if pack_dir exists
            ...
            if ref.pack_dir:
                ...
                weights_path = pack_resolved / "weights.json"
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        if isinstance(guide_raw, str) and guide_raw.strip():
                            guide_abs = (pack_resolved / guide_raw).resolve()
                            try:
...
def _raise_conforming(exc: Any, *, fallback: str) -> NoReturn:
    ...
    Status mapping (via the raised class):
      ...
      * ``AppProjectModelNotInstalledError`` -> global DomainError (422)
      * ``AppProjectStartFailedError`` /
    ...
    if isinstance(exc, AppProjectModelNotInstalledError):
        # Missing weights is a pre-flight domain violation, not an
        # infrastructure failure: the app spec is fine, the environment is
        # missing an install step. Re-raise as the raw DomainError so the
        # global handler maps it to 422 with the ``{code, message, details}``
        # envelope (details carries ``missing[]`` + ``paths_tried``).
        raise exc
    # Start / package failures are infrastructure-level (subprocess spawn /
    # readiness / packaging IO) -> 503 via ExternalServiceError. Forward the
    # domain error's ``details`` (log_tail, manual_command, ports_tried, …)
    # via ``extra_details`` so the UI receives the child's real error, not
    # just the concise summary.
    raise ExternalServiceError(
        code,
        message,
        service="app_builder",
        extra_details=details or None,
    ) from exc
...
def register(router: APIRouter, *, container: Container) -> None:  # noqa: PLR0915 - route-registration closure factory: nested handlers must capture the DI container
    ...
    async def list_apps() -> AppListResponse:
        ...
        Each app entry includes ``models_ready`` (bool) and optionally
        ``models_missing`` (list of missing model descriptors) so the
        frontend can auto-expand setup guidance without a user click.
        """
        ...
        # Resolve paths for the model-presence check (same anchors the
        # process manager uses at spawn time).
        repo_root = getattr(container, "repo_root", None)
        entries: list[AppEntry] = []
        for p in projects:
            if repo_root is not None and p.models:
                model_root = repo_root / "models"
                pack_root = repo_root / "factory" / "chat_features" / "app-builder" / "models"
                # User roots: derive from data_paths when available.
                data_paths = getattr(container, "data_paths", None)
                user_model_root: Path | None = None
                user_pack_root: Path | None = None
                if data_paths is not None:
                    dr = getattr(data_paths, "root", None)
                    if dr is not None:
                        user_model_root = Path(str(dr)) / "app_builder" / "user_model_weights"
                        user_pack_root = Path(str(dr)) / "app_builder" / "user_models"
                ready, missing = _check_app_models(
                    p,
                    repo_root=repo_root,
                ...
                entries.append(_to_entry(p, models_ready=ready, models_missing=missing))
            else:
                entries.append(_to_entry(p))
        return AppListResponse(apps=entries)
        ...
        try:
            ...
            async for snap in iterator:
                ...
                if not await _safe_send_json(
                    ...
                    return
            # SSE parity: the ``done`` frame carries the finished artefact
            # (``_package_frames`` below emits the same payload).  Without it
            # a consumer following the documented contract silently receives
            # nothing and has to scrape the last ``progress`` frame instead.
            await _safe_send_json(
                websocket,
                {
                    "type": "done",
