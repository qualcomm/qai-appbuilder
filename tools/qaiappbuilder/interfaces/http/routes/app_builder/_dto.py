...
from qai.platform.model_weights import WeightSearchReport
...
class LoadedModelDTO(BaseModel):
    ...
    # ``model_id`` is the legitimate domain term; opt out of pydantic's
    # ``model_*`` protected-namespace warning (matches the pattern used by
    # `RunResponse`, `ImportPlanItemPayload`, etc. above).
    model_config = {"protected_namespaces": ()}
    model_id: str
...
class WeightSearchRejectedPayload(BaseModel):
    """One weight-like file the search saw but did not accept.

    ``path`` is relative to the searched root; ``reason`` is the
    human-readable rejection cause (``"too small (…)"``, wrong extension,
    …). These near-misses are what actually identify the problem — a
    0-byte stub weight means the real binary landed elsewhere.
    """
...
class WeightSearchDiagnosticsPayload(BaseModel):
    """Evidence explaining why a weight search came up empty.

    Wire mirror of :class:`qai.platform.model_weights.WeightSearchReport`.
    Set when a readiness scan / export found no usable NPU weight, so the UI
    can render an actionable hint (which directories were walked, which files
    were rejected and why, the concrete next step) instead of a bare "not
    found" — and can seed the "AI search" prompt from ``message``.

    ``message`` is the pre-rendered ``WeightSearchReport.to_message()``
    block; the structured fields are for clients that want to lay the
    evidence out themselves. Every field carries a default so the payload
    degrades gracefully (v2.7 §3.1).
    """
    ...
    rejected: list[WeightSearchRejectedPayload] = Field(default_factory=list)
    other_extensions: list[str] = Field(default_factory=list)
    found_precisions: list[str] = Field(default_factory=list)
...
class BinScanResponse(BaseModel):
    ...
    # Optional (tail-appended): present only when results is empty and a
    # ``model_workdir`` was supplied — the unfiltered walk of
    # ``<model_workdir>/output/`` explaining what blocked the search.
    # Coexists with ``needs_normalize`` (different questions, not exclusive).
...
class AutoExportResponseBody(BaseModel):
    ...
    * ``diagnostics`` — weight-search evidence; set only when the export
      completed but produced no usable weight (a hard "no precision
      binary" failure raises 409 instead, and carries the same evidence in
      the error body's ``details.diagnostics``).
    """
...
def _weight_search_report_to_dto(
    report: WeightSearchReport,
) -> WeightSearchDiagnosticsPayload:
    """Map the shared-kernel weight-search report to its wire DTO.

    ``message`` carries the pre-rendered ``to_message()`` block so the UI can
    display the diagnosis verbatim (or paste it into the "AI search" prompt)
    without re-implementing the rendering rules.
    """
    return WeightSearchDiagnosticsPayload(
        root=report.root,
        root_exists=report.root_exists,
        searched_recursively=report.searched_recursively,
        subdirs=list(report.subdirs),
        rejected=[
            WeightSearchRejectedPayload(
                path=item.path,
        ...
        other_extensions=list(report.other_extensions),
        found_precisions=list(report.found_precisions),
        requested_precision=report.requested_precision,
