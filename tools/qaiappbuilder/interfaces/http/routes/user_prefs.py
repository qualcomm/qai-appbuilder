...
#: ``save_preferences`` behaviour byte-for-byte
...
class ComputerEnabledResponse(BaseModel):
    """``GET`` / ``PUT /api/settings/computer_enabled`` payload."""

    enabled: bool


class ComputerEnabledSaveRequest(BaseModel):
    """``PUT /api/settings/computer_enabled`` body."""

    enabled: bool


class ImageCloudResponse(BaseModel):
    """``GET`` / ``PUT /api/settings/image_cloud`` payload."""

    format: str
    quality: int


class ImageCloudSaveRequest(BaseModel):
    """``PUT /api/settings/image_cloud`` body."""

    format: str
    quality: int


#: Legal values of ``chat.image_cloud_format`` (cloud-copy re-encode container).
#: Mirrors ``ChatSettings.image_cloud_format`` validator (webp|jpeg|png).
_IMAGE_CLOUD_FORMATS: frozenset[str] = frozenset({"webp", "jpeg", "png"})
...
def build_router(*, container: Container) -> APIRouter:
    ...
    # ===========================================================================
    # Desktop-control master enable gate: GET/PUT /api/settings/computer_enabled
    #
    # Reads / writes the ``computer.enabled`` boolean of the on-disk
    # ``<data>/config/forge_config.json`` document — the SAME file + key the
    # chat DI ``_build_computer_enabled_reader`` re-reads each turn (precedence
    # forge_config > ``settings.computer.enabled``). Flipping this therefore
    # takes effect on the next turn WITHOUT a service restart: the ``computer``
    # tool is always registered, but only advertised to a turn when this gate
    # is truthy. The file shallow-merge preserves every unrelated section
    # (mirrors ``put_chat_hooks_enabled``).
    # ===========================================================================

    @router.get(
        "/api/settings/computer_enabled",
        response_model=ComputerEnabledResponse,
    )
    async def get_computer_enabled() -> ComputerEnabledResponse:
        doc = _read_forge_config_file(container)
        section = (doc or {}).get("computer")
        override = (
            section.get("enabled") if isinstance(section, dict) else None
        )
        if isinstance(override, bool):
            return ComputerEnabledResponse(enabled=override)
        computer_settings = getattr(container.settings, "computer", None)
        return ComputerEnabledResponse(
            enabled=bool(getattr(computer_settings, "enabled", False))
        )
    ...
    async def put_computer_enabled(
        req: ComputerEnabledSaveRequest,
    ) -> ComputerEnabledResponse:
        enabled = bool(req.enabled)
        doc = _read_forge_config_file(container)
        section = doc.get("computer")
        if not isinstance(section, dict):
            section = {}
        section["enabled"] = enabled
        doc["computer"] = section
        _write_forge_config_file(container, doc)
        return ComputerEnabledResponse(enabled=enabled)

    # ===========================================================================
    # Cloud-image re-encode format/quality: GET/PUT /api/settings/image_cloud
    #
    # Reads / writes the ``chat.image_cloud_format`` (webp|jpeg|png) and
    # ``chat.image_cloud_quality`` (1-100) values of the on-disk
    # ``<data>/config/forge_config.json`` document — the SAME file + keys the
    # chat SSE ``_read_cloud_image_config`` / DI ``_build_cloud_image_reader``
    # re-read each turn (precedence forge_config > ``settings.chat.*``). Editing
    # these re-encodes the *cloud copy* of chat images on the next turn WITHOUT
    # a restart; the local original on disk is never touched. The file
    # shallow-merge preserves every unrelated section (mirrors
    # ``put_chat_hooks_enabled``).
    # ===========================================================================

    @router.get(
        "/api/settings/image_cloud",
        response_model=ImageCloudResponse,
    )
    async def get_image_cloud() -> ImageCloudResponse:
        doc = _read_forge_config_file(container)
        chat_section = (doc or {}).get("chat")
        chat_settings = getattr(container.settings, "chat", None)
        # Format: forge_config override → settings.chat fallback (default webp).
        fmt_override = (
            chat_section.get("image_cloud_format")
            if isinstance(chat_section, dict)
            else None
        )
        if isinstance(fmt_override, str) and fmt_override.lower() in _IMAGE_CLOUD_FORMATS:
            fmt = fmt_override.lower()
        else:
            fmt = str(getattr(chat_settings, "image_cloud_format", "webp")).lower()
            if fmt not in _IMAGE_CLOUD_FORMATS:
                fmt = "webp"
        # Quality: forge_config override → settings.chat fallback (default 50).
        q_override = (
            chat_section.get("image_cloud_quality")
            if isinstance(chat_section, dict)
            else None
        )
        if isinstance(q_override, int) and not isinstance(q_override, bool) and 1 <= q_override <= 100:
            quality = q_override
        else:
            quality = int(getattr(chat_settings, "image_cloud_quality", 50))
            quality = min(100, max(1, quality))
        return ImageCloudResponse(format=fmt, quality=quality)

    @router.put(
        "/api/settings/image_cloud",
        response_model=ImageCloudResponse,
    )
    async def put_image_cloud(req: ImageCloudSaveRequest) -> ImageCloudResponse:
        fmt = req.format.lower() if isinstance(req.format, str) else ""
        if fmt not in _IMAGE_CLOUD_FORMATS:
            raise DomainError(
                "user_prefs.image_cloud_format.invalid",
                f"format must be one of {sorted(_IMAGE_CLOUD_FORMATS)}",
            )
        quality = req.quality
        if not isinstance(quality, int) or isinstance(quality, bool) or not (1 <= quality <= 100):
            raise DomainError(
                "user_prefs.image_cloud_quality.invalid",
        ...
        if not isinstance(chat_section, dict):
            chat_section = {}
        chat_section["image_cloud_format"] = fmt
        chat_section["image_cloud_quality"] = quality
        doc["chat"] = chat_section
        _write_forge_config_file(container, doc)
        return ImageCloudResponse(format=fmt, quality=quality)
