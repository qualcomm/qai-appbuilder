...
def _register_permission_routes(router: "APIRouter", *, container: "Container") -> None:
    ...
    async def reject_permission(
        ...
        # Constraint G (reject side) — thread the same scope context approve
        # threads so a session/process-scoped rejection persists a deny-effect
        # grant keyed correctly (session → the ASK's conversation id popped
        # single-use from the bridge registry; process → this backend's boot
        # id). Empty conversation id (native-hook ASK / pre-wiring) keys the
        # session deny-grant to "" exactly as approve does.
        try:
            from apps.api._file_guard_bridge import ASK_CONVERSATION_REGISTRY
