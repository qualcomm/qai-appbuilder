...
class RejectPermissionRequest(BaseModel):
    ...
    decided_by: _SubjectDTO | None = None
    reason: str = ""
    # Constraint G (reject side) — remembered rejection scope, mirroring
    # ApprovePermissionRequest.grant: ``once`` (this call only, no persisted
    # grant — the historical default) / ``session`` / ``process`` /
    # ``permanent`` (persist a deny-effect PathGrant so the next identical
    # access is re-denied without re-prompting). Tail-appended (v2.7 §3.1) —
    # defaults to ``once`` so existing callers that omit it keep the
    # single-use reject behaviour.
    grant: str = "once"
    # Constraint G (reject side) — remembered-rejection range, mirroring
    # ApprovePermissionRequest.grant_range: ``file`` (default, the single
    # rejected file) / ``directory`` (remember the rejection for the parent
    # directory the user chose) / ``program`` (exec only: remember the
    # rejection for the whole program binary token). Tail-appended (§3.1) —
    # defaults to ``file``.
    grant_range: str = "file"
...
class AuditRecentResponse(BaseModel):
    ...
    Paginated: ``entries`` is the page slice; ``total`` is the row count
    in the same ``[since, until)`` time window (independent of paging)
    so the UI can render "N of M" and drive prev/next buttons.
    """
...
class SecurityHealthResponse(BaseModel):
    ...
    Restores V1 ``/api/security/health`` parity. The frontend polls
...
class ApplyTemplateRequest(BaseModel):
    ...
    Mirrors V1 ``applyPolicyTemplate``
...
class SkillPolicyUpdateRequest(BaseModel):
    ...
    Mirrors the legacy ``saveSkillPolicy`` payload —
