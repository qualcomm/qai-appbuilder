...
"""Security routes — skill_policy / channels / skills / skill-discovery / templates.

Split out of ``security.py``; the handlers are nested inside per-section
registrars, each capturing the ``container`` passed in.

``skill_policy`` is the write surface behind the Security > Skill panel
editor: the saved per-skill ``read`` / ``write`` / ``trusted_binaries``
lists are merged onto the skill's on-disk ``skill.policy.json`` by the
``skill_allow_provider`` (``apps/api/_security_di.py``) and enforced by
``CheckPermissionUseCase``. See :func:`_register_skill_policy_routes`.
"""
...
def _register_skill_policy_routes(
    router: APIRouter, *, container: Container
) -> None:
    """Per-skill path policy: read the effective view, write the override.

    Split out of :func:`_register_skills_routes` so each registrar stays
    within the statement budget; the two policy routes are also the one
    cohesive pair here (read + write of the same resource).
    """
    ...
    async def skill_policy_put(
        ...
        override — the write half of the Security > Skill panel editor.

        The override lands in the :class:`SecurityRuntimeStateService`
        ``skill_policies`` bucket, which
        :func:`SecurityRuntimeStateService.update_settings` persists into
        ``forge_config.json`` (so an operator edit survives a restart).
        Clearing every list revokes the policy (the entry is removed).

        The capability registry itself is NOT mutated — the on-disk
        ``skill.policy.json`` stays the skill author's declaration. The
        override is layered on top at read time by
        :class:`GetSkillPolicyUseCase` / :class:`SkillDiscoveryUseCase`
        (what the panel renders) AND by the ``skill_allow_provider``
        wired in ``apps/api/_security_di.py`` (what
        :class:`CheckPermissionUseCase` enforces). Both sides merge the
        same way, so the surface shown in the UI is the surface enforced
        by FileGuard — a saved policy takes effect on the next check with
        no reboot.
        """
        # Whitespace-only names would create an unreachable, unnamed
        # override bucket that the panel can never show or delete.
...
def _register_skills_routes(router: APIRouter, *, container: Container) -> None:
    _register_skill_policy_routes(router, container=container)
