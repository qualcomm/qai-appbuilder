...
- :class:`AuthMiddleware` (in :mod:`interfaces.http.middleware.auth`) —
  ...
  ``settings.auth.enabled``, whose default is per-platform (on wherever a local
  browser exists — Windows, and Linux with a graphical session; off on headless
  Linux). When disabled the middleware short-circuits so such a deployment is
  unaffected.
