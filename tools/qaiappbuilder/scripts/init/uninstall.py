...
2. **Virtual envs + bundled tools** — ``%LOCALAPPDATA%\\QAIModelBuilder\\``
   recursively (covers ``.venv_arm64_313``, ``.venv_x64_313``,
   ``.venv_x64_310``, the WDAC-fallback signed Python under ``python\\3.13``
   and ``python\\3.10``, optional ``git/`` PortableGit, ``node/`` portable
   Node.js + pnpm). A signed Python REUSED from an existing system install
   outside this tree (e.g. a python.org build already registered elsewhere)
   is intentionally left untouched — it is not ours to remove.
...
def _resolve_reboot_url() -> str:
    ...
    default port was inside a Windows excluded range). Falls back to the
    ``factory/config/ports.json`` ``legacy_packaged`` port when the file is absent
    or unparseable so a pre-endpoint-file install (or an already-stopped
    server) still gets the best-effort attempt — the caller wraps
    everything in a broad except so a wrong URL just fails silently.
    """

    # 1. Prefer the runtime endpoint file — the ACTUAL bound port/URL.
    try:
        from qai.platform.process.runtime_endpoint import read_endpoint

        info = read_endpoint(_detect_repo_root() / "data")
        if info is not None:
            url = info.get("url")
    ...
    # 2. Fall back to the registry's ``legacy_packaged`` port. NO literal is
    #    authored here: we defer to the registry (and its own built-in DEFAULTS,
    #    which mirror factory/config/ports.json) so the port value lives in
    #    exactly one place. If the ports module itself is unimportable we return
    #    a portless best-effort URL rather than re-hardcoding a number — the
    #    caller swallows the resulting connection error either way.
    try:
        from qai.platform.config.ports import DEFAULTS, load_ports

        fallback_port = int(load_ports().get("legacy_packaged", DEFAULTS["legacy_packaged"]))
    except Exception:  # noqa: BLE001 — ports module unimportable (broken tree).
        return "http://localhost/api/system/reboot"
    return f"http://localhost:{fallback_port}/api/system/reboot"
...
def stop_running_processes(targets: UninstallTargets, *, dry_run: bool = False) -> None:
    ...
    # the documented default (factory/config/ports.json legacy_packaged) inside a
