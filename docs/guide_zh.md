...
或者从 [GitHub Release](https://github.com/qualcomm/qai-appbuilder/releases) 下载对应版本的 `.whl` 文件，再运行：
...
| `.onnx` | HTP/CPU | 从 v2.0.0 开始支持直接加载，自动使用 ONNX Runtime（推荐） |
...
#### 5.3.4 ONNX 模型直接加载（推荐）

**ONNX Runtime 特性**：

- ✅ **自动检测**：QNNContext 会自动检测 `.onnx` 文件并使用 ONNX Runtime
- ✅ **HTP 加速**：自动选择 HTP 后端以获得最佳性能
- ✅ **FP16 精度**：默认启用 FP16 精度以提高性能
- ✅ **CPU 回退**：当 QNN 提供程序不可用时自动回退到 CPU
- ✅ **向后兼容**：现有的 `.bin` 用户无需任何 API 更改
...
- ONNX 模型会自动在 HTP 上执行，无需手动转换
- 首次加载时可能需要一些时间进行初始化
- 支持 ONNX 模型的所有标准操作符
- 对于不支持的操作符，会自动回退到 CPU 执行
