...
class Qwen2VLQnn():
    ...
    def Inference(self, image_path: str, prompt: str) -> str:
        ...
        # Use pixel_values for images instead of pixel_values_videos
        if 'pixel_values' in inputs:
            ...
            raise ValueError("No pixel values found in inputs")
            
        image_embeddings = self.veg.Inference(input_data)[0]
        token_ids = inputs['input_ids']
        
        # Use image_token_id for images instead of video_token_id
        token_id = self.llm_config.image_token_id if hasattr(self.llm_config, 'image_token_id') else self.video_token_id
        
        llm_outputs = self.llm.Inference(token_id, token_ids, image_embeddings)
        return llm_outputs