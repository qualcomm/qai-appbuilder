...
# Resolve execution workspace to audio/whisper_base_en directory
...
jfk_wav_path = os.path.join(execution_ws, "..", "assets", "jfk.wav")
jfk_npz_path = os.path.join(execution_ws, "..", "assets", "jfk.npz")
mel_filter_path = os.path.join(execution_ws, "..", "assets", "mel_filters.npz")
...
def transcribe_single_chunk(audio: np.ndarray):
    ...
    # Feed prefix tokens first (not added to decoded_tokens output)
    ...
    # Skip prefix tokens (TOKEN_SOT + prefix_tokens) when decoding
