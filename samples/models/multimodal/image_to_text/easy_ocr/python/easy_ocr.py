...
# ─────────────────────────────────────────────────────────────────────────────
# Inline replacements for qai_hub_models.utils.asset_loaders.load_image
# and qai_hub_models.utils.display.display_or_save_image
# ─────────────────────────────────────────────────────────────────────────────
...
def load_image(image_path) -> "Image.Image":
    ...
    if path_str.startswith("http"):
        ...
        suffix = _os.path.splitext(path_str)[-1] or ".png"
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            urllib.request.urlretrieve(path_str, tmp.name)
            return _PILImage.open(tmp.name).copy()
    return _PILImage.open(path_str)
...
def display_or_save_image(
    ...
    def _save(img, base_dir, fname):
        ...
        fpath = _os.path.join(base_dir, fname)
        img.save(fpath)
        print(f"Saving {desc} to {fpath}")

    if output_dir is not None:
        _save(image, str(output_dir), filename)
        return False
    ...
    try:
        headless = (
            _os.environ.get("SSH_TTY") is not None
            or _os.environ.get("SSH_CLIENT") is not None
        )
        if not headless:
    ...
    _save(image, _os.path.join(str(_Path.cwd()), "build"), filename)
    return False
...
MODEL_NAME = "easy_ocr"
MODEL_NAME_Detector = "easy_ocr_Detector"
MODEL_NAME_Recognizer = "easy_ocr_Recognizer"
MODEL_NAME_CN_Detector = "easy_ocr_CN_Detector"
MODEL_NAME_CN_Recognizer = "easy_ocr_CN_Recognizer"
...
char_file = execution_ws / ".." / "assets" / "Char" / "ch_en_character.bin"
lang_char_file = execution_ws / ".." / "assets" / "Char" / "ch_en_lang_char.bin"
#input_image_path = execution_ws + "\\english.png"
input_image_path = execution_ws / ".." / "assets" / "ch_en.png"

font_path = execution_ws / ".." / "assets" / "simsun.ttc"
...
def model_download():
    ...
    if PLATFORM in ("wos", "x86_win"):
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_Detector, detector_model_path, desc=desc, fail=fail, hub_id=HUB_ID_SY)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_Recognizer, recognizer_model_path, desc=desc, fail=fail, hub_id=HUB_ID_SY)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_CN_Detector, cn_detector_model_path, desc=desc, fail=fail, hub_id=HUB_ID_SY)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_CN_Recognizer, cn_recognizer_model_path, desc=desc, fail=fail, hub_id=HUB_ID_SY) 
    else:
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_Detector, detector_model_path, desc=desc, fail=fail)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_Recognizer, recognizer_model_path, desc=desc, fail=fail)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_CN_Detector, cn_detector_model_path, desc=desc, fail=fail)
        ret = install.download_qai_hubmodel(SOC_ID, MODEL_NAME_CN_Recognizer, cn_recognizer_model_path, desc=desc, fail=fail) 
