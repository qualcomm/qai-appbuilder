...
def _load_imagenet_labels():
    ...
    # Try loading from the downloaded TXT file
    if os.path.exists(imagenet_classes_path) and os.path.getsize(imagenet_classes_path) > 0:
        try:
            ...
            pass
        # File is corrupt or too short — delete it so it gets re-downloaded next run
        print(f"[WARN] {imagenet_classes_path} is invalid, removing it.")
