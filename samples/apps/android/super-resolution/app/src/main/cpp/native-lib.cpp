...
// Return the appropriate backend library path based on the model file extension.
// .bin  -> QNN HTP backend (libQnnHtp.so)
// .dlc  -> SNPE backend    (libSNPE.so)
static std::string GetBackendLibPath(const std::string& libs_dir,
                                     const std::string& model_path) {
    //if (model_path.size() >= 4 &&
    //    model_path.compare(model_path.size() - 4, 4, ".dlc") == 0) {
    //    return libs_dir + "/libSNPE.so";
    //}
    return libs_dir + "/libQnnHtp.so";
}
...
// C++-based pre/post-processing path: read the input image, do all
// pre/post-processing with OpenCV + xtensor, run the inference, and write the
// output image file.
int super_resolution(std::string libs_path, std::string model_path, std::string input_img, std::string output_img) {

    std::string  backend_lib_path = GetBackendLibPath(libs_path, model_path);
...
// JNI entry point for the Java-based pre/post-processing path. Java supplies the
// pre-processed input tensor via a direct ByteBuffer and receives the raw output
// tensor; the native library only runs the inference.
...
Java_com_example_superresolution_MainActivity_SuperResolution(
    ...
    std::string  backend_lib_path = GetBackendLibPath(libs_dir, model_path);
...
// JNI entry point for the C++-based pre/post-processing path. Java only passes
// file paths; the native library reads the input image, does all
// pre/post-processing with OpenCV + xtensor, and writes the output image file.
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_superresolution_MainActivity_SuperResolutionCpp(
        JNIEnv* env,
    ...
    My_Log("Java_com_example_superresolution_MainActivity_SuperResolutionCpp");
    super_resolution(libs_dir, model_name, input_img, output_img);
    My_Log("Java_com_example_superresolution_MainActivity_SuperResolutionCpp end.");
