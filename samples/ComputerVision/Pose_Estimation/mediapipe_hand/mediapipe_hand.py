...
def _cv2_has_gui() -> bool:
    """Return True if OpenCV was built with GUI (highgui) support."""
    try:
...
class HandDetector(QNNContext):
    def __init__(
        ...
        with MediaPipePyTorchAsRoot() as repo_path:
            ...
            # Build absolute path to anchors file using the repo root
            if repo_path and not os.path.isabs(detector_anchors):
                anchors_path = os.path.join(repo_path, detector_anchors)
            else:
                anchors_path = detector_anchors

            handDet = BlazePalm()
            handDet.load_anchors(anchors_path)
...
def model_download():
    ...
    ret = install.download_qai_hubmodel(SOC_ID, LANDMARK_DETECTOR_MODEL_NAME, landmark_detector_model_path, desc=desc, fail=fail)
    ret = install.download_qai_hubmodel(SOC_ID, HAND_DETECTOR_MODEL_NAME, hand_detector_model_path, desc=desc, fail=fail)
...
def capture_and_display_processed_frames(
    ...
    Inputs:
        ...
        output_dir: str
            Directory to save frames if GUI is not available.
    """
    global gesture
    ...
    if not _CV2_GUI:
        ...
        print(
            f"[WARNING] OpenCV was built without GUI support (opencv-python-headless is installed).\n"
            f"          Camera mode requires a GUI-capable OpenCV build.  [{_sys_info}]\n"
            f"\n"
            f"          Fallback: Saving processed frames to disk instead of displaying.\n"
            f"\n"
            f"          Alternative options:\n"
            f"          1. Use --imagefile to run on a static image (no GUI needed):\n"
            f"             python mediapipe_hand.py --imagefile path\\to\\hand_image.jpg\n"
            f"\n"
            f"          2. Install opencv-python (GUI-capable) for your platform:\n"
            f"             On x64 Windows:  pip install opencv-python==4.10.0.84\n"
            f"             On ARM64 Windows: obtain opencv_python-*-cp313-cp313-win_arm64.whl\n"
            f"                               and install with: pip install <wheel_file>\n"
            f"\n"
            f"          3. Use the x64 Python environment which has opencv-python with GUI:\n"
            f"             py -3.12-64 ComputerVision\\Pose_Estimation\\mediapipe_hand\\mediapipe_hand.py\n"
        )

        if output_dir is None:
            output_dir = str(execution_ws / "output_frames")
        ...
        try:
            while has_frame and frame_count < 100:
                assert isinstance(frame, np.ndarray)
                frame_count += 1
                frame = np.ascontiguousarray(frame[:, ::-1, ::-1])
                processed_frame = frame_processor(frame)

                output_path = _os.path.join(output_dir, f"frame_{frame_count:04d}.jpg")
                cv2.imwrite(output_path, processed_frame[:, :, ::-1])
                print(f"Saved frame {frame_count} to {output_path}")

                has_frame, frame = capture.read()
                if frame_count >= 100:
                    print(f"Reached frame limit (100). Stopping capture.")
                    break
        finally:
            capture.release()

        print(f"Capture complete. {frame_count} frames saved to {output_dir}")
        return
