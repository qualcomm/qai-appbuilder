...
**AOT-GAN** (Aggregated Contextual Transformations GAN) is an image inpainting model that fills in masked regions of an image with realistic content, running on the Snapdragon NPU (HTP) via QAI AppBuilder. It uses aggregated contextual transformations for high-quality inpainting.
...
AOT-GAN uses a GAN-based architecture with aggregated contextual transformations:
...
python ComputerVision\Image_Editing\aotgan\aotgan.py
```
...
input.png + mask.png
    ↓ PIL.Image.open
    ↓ preprocess_inputs (from image_processing.py):
...
- The mask preprocessing uses the shared `preprocess_inputs` function from `common/image_processing.py`.
- The input image is center-cropped to 512×512 (not padded).
- Both the original image and the inpainted output are displayed.
- For a similar model with different architecture, see [lama_dilated](../lama_dilated/).
